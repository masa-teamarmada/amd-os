...
## 2026-09-23 追記: l2m共有ボス用の独立したDB領域

OSスイートのSupabase内に、別リポジトリ `l2m` が使うボス予定・メンバー資格・討伐履歴・Web Push用の表と関数を追加した。migration履歴とEdge Functionは `ios/supabase/` に保存する。AMD OSのPWA画面・認証・通知契約は変更していない。l2m側の公開画面とSwift版だけがこの領域を利用し、メンバーのユーザー名と4桁の暗証番号はl2m専用の認証関数で扱う。

本番DB、認証関数、30秒ごとの通知ジョブは反映済み。仮ユーザーの登録、承認前の遮断、承認後の予定・履歴取得、管理者承認を確認した。Web Pushの実機着信と複数端末同時更新はl2m側で未確認。

---

## 2026-09-16 追記: ZMPワークスペースの実行導線

PWAのZMPワークスペースは旧「テーマ」タブを外し、`ゴールツリー / タスク / ガント`を実行の先頭に置いた。旧`#theme-progress`はゴールツリーへ互換遷移する。承認済みで問いに紐づかない単純タスクはゴールツリーへ表示せず、タスク一覧とガントで読む。iOS / macOS / Androidのネイティブ画面は今回変更していない。
...
## 2026-09-26 PWA月次報告書タブ

PWAの通常PJ・研究機関PJに「PJ管理 → 月次報告書」を追加。月と社内版・提出版の切替、タブ内帳票、未生成表示、印刷への入口。共通DB/schemaは変更なし。iOS・macOS・Androidは未移植で、既存画面は維持。

| 同期対象 | 2026-09-26 月次報告書タブの棚卸し |
|---|---|
| manual | 2-3、9-3更新済み |
| spec | 3-2、3-8、6-1更新済み |
| bzm/model | 理論変更なし |
| DB schema/EF | 変更なし。既存社内版・提出版テーブルを使用 |
| iOS/macOS | タブ追加は未移植。既存帳票・保存データは共通 |
| routine | クラウド25日16時JST。旧ローカル月末タスクは停止。クラウド保存用認証の設定不足は未解消 |
