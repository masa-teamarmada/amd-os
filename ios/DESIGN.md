...
> 「コスト試算」タブ（2026-08-23追加、**全PJ常設**、2026-09-13 二段階化・シミュレーター化・作業の流れとオフサイト、2026-09-14 方式と装置の分離・誰がやるか・誰が持つか・年間処理量から年に作る量・共通の作業単価と CAPEX / OPEX の区分・上端の槽を外し効かない前提を薄く出す・顧客工場の装置の費用を顧客に・金額をカンマ区切りの円に）。前提（変数）・費用明細・作業リスト
...
> **PWA専用画面（Native未移植）**: PJコックピット `?tab=technology&tech=cost-fuel`（技術タブの中のタブ）とPJワークスペース `#technology` の
> 「コスト試算（燃料）」（2026-09-14追加、**燃料の試算 `case_kind = 'biodiesel'` を持つPJだけ**。いまは SX）。燃料（バイオディーゼル、FAME）を作って売る事業の燃料1Lあたりの総コストを、
...
> 2026-09-06: PWAのPJワークスペースは、内部メンバー向けに`実行 / 計画・根拠 / 経営・会社 / 資料`の二段ナビを持つ。`計画・根拠`は技術・事業計画だけで、`PJ概要`はワークスペースへ置かず社内コックピット専用とする。外部はテーマ（存在時）・ガント・関係先・ドライブだけ。`動向・会議`は経営会議を含むため、全PJメンバー向けのワークスペースへ出さない。PWA専用で、ネイティブ画面への移植はない。
