...
>
> 最終更新: 2026-09-01 (PWA p19のガント／目的構造切替と外部PJメンバー共有)

---

## PWAコックピットのタブ配置（2026-08-31）

KUTE (`p25`) 限定で、今期の仕事を「認定制度」「関連6規程」「シーズ発掘」「桑折先生」「自走化・連携」「年度報告」の6区分・38タスクへ再編し、「ガント」タブ (`?tab=gantt`) の全体ガントに統合する。規程案の作成と大学の決裁・施行、候補調査と接触承認、調査完了と実証受注を別タスクにする。根拠を確認できた9件だけ完了とし、日付根拠がない行は日程未設定で保持する。旧6工程は編集・依存関係がないことを確認してsoft-deleteし、履歴を残す。連携シーズ比較は「シーズ」タブ (`?tab=seeds`) のまま。シーズの評価正本、他研究機関PJ、スキーマ、権限は変更しない。他研究機関PJへの適用はKUTEの設計確認後。画面変更はPWA限定、iOS / macOS / Androidのネイティブ画面には未移植だが、共通DBのKUTE計画データは既存の認可済み読取りに反映される。詳細仕様は `../pwa/spec/3-8-cockpit-current-spec.md`。
...
> 2026-09-01: PWAのp19コックピット`?tab=themes`にも同じ画面を追加。テーマ面は別の経緯台帳を
> 持たず、既存のガントと関係先台帳への索引にする。ガント区画は時間順の`ガント`と、最上位目的から
> 成立条件・親子タスク・関係先・現在のボールを逆算する`目的構造`を切替表示する。正本は同じ
> `project_management_*`。水素循環PJは目的`都内で水素をつくる・ためる・つかう`と3成立条件へ分離し、
> シーズリスト作成は完了、堂脇先生は一旦停止、pHydrogenはAMD側ボールとして登録した。旧水素7行の
> `project_theme_profiles.history_rows`は空にして二重編集を止めた。個別PJ権限を持つ外部メンバーにも
> テーマ、ガント／目的構造、関係先、共有資料を読み取り表示し、AMD内部のホーム／コックピット導線と
> 社内情報は出さない。migration `20260901153000_zmp_hydrogen_management_ledger.sql`は本番適用済み。
