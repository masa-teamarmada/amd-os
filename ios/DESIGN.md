...
> 「コスト試算」タブ（2026-08-23追加、**全PJ常設**、2026-09-13 二段階化・シミュレーター化・作業の流れとオフサイト、2026-09-14 方式と装置の分離・誰がやるか・誰が持つか・年間処理量から年に作る量）。前提（変数）・費用明細・作業リスト
...
> 上半分は「左 操作パネル（作業の流れと工数・切り替え・前提・作業リスト・明細の書き換え）／右 結果（菌体1kgの原価・方式と装置ごとの総コストを内訳の色で積んだ棒・選んだ組み合わせの内訳の棒グラフ・SXの作業工数。顧客がやる作業の時間は出さない）」の1枠で、
