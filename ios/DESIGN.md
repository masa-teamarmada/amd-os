...
> 2026-09-09: 独立タブとカード式フィッシュボーンを廃止し、再帰的な親子タスクをガントの固定左列へ統合した。
> 画面に出るすべての階層ノードは`タスク`である。子を持つカード本体のクリックだけで直下を開閉し、特定階層専用の表示切替や一括開閉ボタンは置かない。右側の時間軸に計画期間と依存線を描く。
