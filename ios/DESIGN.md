...
KUTE (`p25`) 限定で、年度内ロードマップを「ガント」タブ (`?tab=gantt`) の先頭へ移し、連携シーズ比較を「シーズ」タブ (`?tab=seeds`) に独立させる。進捗管理上部から両者を外す。既存の全体ガントとシーズ比較の内容・データ正本は変更しない。他研究機関PJへの適用はKUTEでの設計合意後。これはPWA限定の変更で、iOS / macOS / Androidのネイティブ画面には未移植。詳細仕様は `../pwa/spec/3-8-cockpit-current-spec.md`。
