...
2026-09-17: PWAのZMP(p19)コックピットはタスクを既定表示し、未完了を「いま動かす / 7日以内 / 未設定 / その先」へ分ける。発生元と最終確認日を併記し、未整備の事業計画グループを通常導線から隠す。データと既存URLは維持。PWA専用で、iOS/macOS/Androidのネイティブ画面は未移植。2026-09-17: PWA adminの`/admin/change-history`で、メンバー台帳の最終ログイン履歴を対象者・件数・日本時間の実効差分つきで要約し、日時型の差分値も日本時間表示へ統一。2026-09-16: PWA adminに`/admin/change-history`を追加。2026-09-16以降のOS全体のデータ変更を、実行者・日時・対象・変更前後のセットで表示する。`/admin/access`最上段に未許可外部アカウントの承認待ちを追加し、まさへのSlack DMから研究機関ワークスペースの閲覧許可/拒否を決められる。PWA専用で、iOS/macOS/Androidのネイティブ画面は未移植。

2026-09-09: PWAのVC台帳に`/vcs/investments`を追加。VC参加単位で、出資・完了日と公表日、VC個別額とラウンド総額、成立状態と確認状態を分ける。自動収集は候補止まりで、根拠本文にない日付・金額は未確認。iOS/macOS/Androidのネイティブ画面は未移植。詳細は `../pwa/design/vc_list.md`。
...
>
> 最終更新: 2026-09-16 (PWAデータ変更履歴・外部アクセス承認)
...
進捗管理は「MS・月次 / 動向・会議 / 週次差分 / ガント / 関係先 / ゴールツリー」、通常PJの事業計画は「スコア詳細 / 技術 / 事業計画 / コスト試算 / 知財 / 資本政策」、PJ管理は「PJ概要 / ドライブ / 会社概要」を持つ。タスクの親子構造はガントの固定左列に統合し、右の時間軸と同じ行で読む。独立した目的構造タブは持たず、旧`?tab=objective-structure`はガントへ解決する。活動・実績（助成金、獲得台帳、AMDの貢献）はPJ概要へ、資本政策は事業計画グループへ置く。ZMPの旧テーマ作業面はPJワークスペースからも外し、ゴールツリー / タスク / ガントを実行入口にする。画面変更はPWA限定で、iOS / macOS / Androidのネイティブ画面には未移植。PCでは親グループのhover/focusで子一覧を直下へフロートし、タッチ端末では子列を常時表示する。詳細仕様は `../pwa/spec/3-8-cockpit-current-spec.md`。

## 0. 全体アーキテクチャ概要

### 参画終了月とpt（2026-09-26、PWA v3.145.9）

PWAの月次報酬・将来原価・シーズン予実は、共有 `project_members.join_ym/leave_ym` の両端を含む期間で新規ptを配分し、終了者の過去の未払繰越を保持する。MS設計表は参画終了月と期間内設計ptを表示する。新規画面・DB列の追加なし。iOS/macOS/Androidの独自計算・表示は未移植であり、終了月を無視した独自計算をPWAと同値とはみなさない。
...
> 「コスト試算」タブ（2026-08-23追加、**全PJ常設**、2026-09-13 二段階化・シミュレーター化・作業の流れとオフサイト、2026-09-14 方式と装置の分離・誰がやるか・誰が持つか・年間処理量から年に作る量・共通の作業単価と CAPEX / OPEX の区分・上端の槽を外し効かない前提を薄く出す・顧客工場の装置の費用を顧客に・金額をカンマ区切りの円に・オフサイトの売価と年間処理量を別に・オフサイトで引き取る液の濃さを別に、2026-09-15 菌体のバッチ間のマスバランス（回収率は次のバッチへ回す割合）・目標放流水濃度・金属回収の中央回収設備の CAPEX）。前提（変数）・費用明細・作業リスト
> （`project_cost_tasks`。作業ごとに誰がやるか `performer` を持ち、SXがやる作業だけを原価に入れる）をDBに持ち、株×用途×方式（オンサイト / オフサイト）×装置（循環カートリッジ / 直接投入）×槽（オンサイトの槽を SX が持つときだけ既設/新設。オフサイトは新設だけ）をクライアントで再計算する。計算結果は保存せず常に導出する。
> 上半分は「左 操作パネル（作業の流れと工数の下に、前提・作業リスト・明細を「事業と処理の条件 / CAPEX / OPEX」の枠と小分けに並べて書き換える。作業単価はすべての作業に共通の1つ。数字の欄は3桁カンマ。開いたときは自然株。上端に槽は出さず、選んだ組み合わせで効かない前提は薄く出す）／右 結果（菌体1kgの原価・方式と装置ごとの総コストを内訳の色で積んだ棒・選んだ組み合わせの内訳の棒グラフ・SXの作業工数。顧客がやる作業の時間は出さない）」の1枠で、
...
> SOL は先頭に事業全体のまとめ（本文のピクト図でお金とモノの流れ、案Cの役割分担と収益の柱の表）を置き、その後ろにオフサイトの廃液処理の検証（2026-09-14）を置く（計11トピック・社内限定。評価・オンサイトとオフサイトの比較・事業の形の選択肢・量・値段・菌で処理できる濃さ・許可・競合・反社会的勢力・確認事項）。
...
> **退役表示（データ・互換APIは保持）**: p19 PJワークスペースの旧 `#theme-progress`（表示名「テーマ」）
...
> 2026-09-09: 目的構造の独立タブとカード式フィッシュボーンを廃止し、再帰的な親子タスクをガントの固定左列へ統合した。
> すべての階層ノードは同じ`project_management_tasks`のタスクで、右側の時間軸に計画期間と依存線を描く。
> 左列は固定し、横スクロールは時間軸だけに効く。旧`?tab=objective-structure`はガントへ解決する。
> タスクは手動追加でき、グリップのドラッグで親子と順序を変更できる。
...
> ゴールツリー、タスク、ガント、関係先、共有資料を読み取り表示し、AMD内部のホーム／コックピット導線と
...
> 2026-09-16: PWAのPJワークスペースは、内部メンバー向けに`実行 / 計画・根拠 / 経営・会社 / 資料`の二段ナビを持つ。`実行`の先頭はゴールツリー / タスク / ガント。`PJ概要`はワークスペースへ置かず社内コックピット専用とする。外部はゴールツリー・タスク・ガント・関係先・ドライブだけ。`動向・会議`は経営会議を含むため、全PJメンバー向けのワークスペースへ出さない。PWA専用で、ネイティブ画面への移植はない。

> 2026-09-16: p21の関係先・週次・経営管制・カレンダー・出力資料に表示する現行略称は`SOL`。旧`SX`は内部キー、過去の証跡、既存URLとの互換に限って残し、画面の保有側・担当・案内には出さない。PWA専用で、ネイティブ画面への移植はない。

> 2026-09-16: PWAのゴールツリーの状態は「検討中 / 進行中 / 対応待ち / 判断待ち / 要見直し / 完了 / 中止」。子のない項目は検討中とし、実行中・阻害は承認済みTODOの状態から導く。仕様は `../pwa/spec/3-21-question-tree-current-spec.md`。DB変更なし。ネイティブには未移植。

> 2026-09-16: PWAのゴールツリーでは、最上位TODOの左のつまみを別の論点へ落として、TODOが答える論点を移せる。TODOの親子・日程・担当・前後関係は変えない。複数の論点に付くTODOは、前の論点から外すか、両方に残すかを選ぶ。PWA専用で、ネイティブ画面への移植はない。
...
### 2026-09-16 SOLガントの定例資料工程
PWAコックピット／共有ワークスペースのガントはp21のみ `project_gantt_roadmaps` の4区分13工程を上位表示する。詳細は既存ゴールツリーと同じレコード・編集経路。法人設立2027-04、プレシード2027-05は月精度の計画。ネイティブ表示の変更はない。正本: `pwa/spec/3-21-question-tree-current-spec.md`。
...
- 2026-09-16: SOLガントの資料風配色・帯配置を撤去。既存ガントの名前列＋日程列・配色に統一し、展開時も左列固定、長い名称は列内で折返し。MS/タスク投影と親日程集計は維持。
