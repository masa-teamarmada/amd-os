...
KUTE (`p25`) 限定で、年度内ロードマップの6工程を既存 `project_management_*` 台帳へ移し、「ガント」タブ (`?tab=gantt`) の全体ガントに期間バーとして統合する。独立したロードマップ枠は廃止する。「制度整備」5工程と「シーズ発掘・after GTIE」1工程、目的・全成果物・月単位の仮日程を保持し、実績や担当は推測で埋めない。連携シーズ比較は「シーズ」タブ (`?tab=seeds`) のまま。シーズのデータ正本・評価は変更しない。他研究機関PJへの適用はKUTEでの設計合意後。画面変更はPWA限定、iOS / macOS / Androidのネイティブ画面には未移植だが、共通DBのKUTE計画データは既存の認可済み読取りに反映される。新テーブル・権限変更なし。詳細仕様は `../pwa/spec/3-8-cockpit-current-spec.md`。
