...
> 「コスト試算」タブ（2026-08-23追加、**全PJ常設**、2026-09-13 二段階化・シミュレーター化・作業の流れとオフサイト、2026-09-14 方式と装置の分離・誰がやるか・誰が持つか・年間処理量から年に作る量・共通の作業単価と CAPEX / OPEX の区分）。前提（変数）・費用明細・作業リスト
> （`project_cost_tasks`。作業ごとに誰がやるか `performer` を持ち、SXがやる作業だけを原価に入れる）をDBに持ち、株×用途×方式（オンサイト / オフサイト）×装置（循環カートリッジ / 直接投入）×槽（既設/新設。オフサイトは新設だけ）をクライアントで再計算する。計算結果は保存せず常に導出する。
> 上半分は「左 操作パネル（作業の流れと工数の下に、前提・作業リスト・明細を「事業と処理の条件 / CAPEX / OPEX」の枠と小分けに並べて書き換える。作業単価はすべての作業に共通の1つ。数字の欄は3桁カンマ。開いたときは自然株）／右 結果（菌体1kgの原価・方式と装置ごとの総コストを内訳の色で積んだ棒・選んだ組み合わせの内訳の棒グラフ・SXの作業工数。顧客がやる作業の時間は出さない）」の1枠で、
