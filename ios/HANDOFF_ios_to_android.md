...
- 共通DBでは、旧KR経営改革の枝を履歴保持のsoft-deleteにし、「増額・追加契約が成立した場合だけ再検討する」という完了済み判断へ集約する。スタッフ研修・リハーサルは問いとの接続を外し、実行タスクとして保持する。
- iOS / Androidのネイティブ画面・データモデルは変更しない。ネイティブ側でワークスペース相当を実装するときは、旧テーマ別画面を復元せず、ゴールツリー / タスク / ガントを同じ正本から読む。
...
- `TextbookReaderView`
  ...
  - 文字送りは詰め、段と段の間だけ余白を取る Kindle 風の縦組みにする。画面高に応じて 1 段あたりの文字数を増やす。
