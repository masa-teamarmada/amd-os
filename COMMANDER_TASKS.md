...
- 現状どうなってるか
  ...
  - ただし、まさが使える本番状態まで進め切れていないため、最新mainへ合わせて本番反映するworkerを再始動した。
  - workerから着手報告を受領済み。
- 残課題は何か
  - 最新mainに合わせて差分を取り込む。
  - 画面確認、型チェック、ビルド、本番反映状態を確認する。
...
- 現状どうなってるか
  - まず実装せず、全体設計workerを切った。
  - L2の10個、Atlas、その他定期処理を棚卸しし、Claudeへ移すもの・Codexに残すもの・束ねるものを設計する。
  - MTGサマリ抽出は高頻度が必要なので、Codex残しの有力候補として扱う。
- 残課題は何か
  - 現状の定期処理一覧を確定する。
  - Claude Code routinesへ移す束ね方と頻度を決める。
  - 1日15件制限内で回るcron案を作る。
  - 実装に入る前に、事故防止、重複実行、通知、司令塔報告の設計を確認する。
