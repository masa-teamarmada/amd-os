...
この台帳は、AMD OS 司令塔が受けている依頼を「コードを読んでいない人でも分かる」粒度で整理するためのもの。worker報告をそのまま貼らず、司令塔がまさ向けに要約して更新する。
...
### 1. L2会議サマリ抽出を、MMOマシンで確実に毎時起動させる
...
- 現状どうなってるか
  ...
  - Windows側の予約実行の最小部品はMMOマシンに配置済み。まだ本番登録も本番起動もしていない。
  - 試験実行では、予約実行そのものの土台は使えそうだが、中でCodexに設計書ファイルを読ませる段階で止まった。
- 残課題は何か
  - Windows側の予約実行で、会議サマリ抽出を安全に起動できるか確認する。
  - 二重起動を防ぐ仕組み、実行ログ、タイムアウト、失敗時の戻し方を設計する。
  - まず安全な設定変更で、Codexが必要なファイルを読めるようにできるか確認する。
  - それでも解けない場合は、試験実行だけに限定して強い権限の実行モードを試すか判断する。
  - いきなり本番書き込みや本番登録はしない。

### 2. L2データ抽出全体を、MMOマシンと現行仕様に合わせて安定稼働させる
...
### 4. コックピットのスコア詳細統合をmainへ安全に取り込む

- お願いしたタスク内容
  - コックピット上部にAMDスコアとXRLグラフを常時表示する。
  - その下に「進捗管理」と「スコア詳細」のタブを置き、既存コックピット内容とスコア詳細を切り替えられるようにする。
- お願いした背景
  - コックピットはプロジェクト判断の中心画面。
  - スコア推移と詳細根拠を同じ流れで見られると、月次レビューや意思決定がしやすくなる。
- 現状どうなってるか
  - worker実装は完了済み。
  - 司令塔レビューでは、まさの依頼内容に沿っていると判断済み。
  - 型チェック、ビルド、ログイン状態での画面確認も通っている。
- 残課題は何か
  - 他のmain変更と衝突しない順序で取り込む。
  - 取り込み後、本番画面で期待どおり見えるか最終確認する。
...
- お願いしたタスク内容
  - Textbookを単なるBZM理論の解説書ではなく、Before Zeroの現場判断、失敗、仮説修正、関係構築、ケースを扱う実践テキストにする。
  - L2データからTextbookに載せるべき知見を抽出し、通知で承認後に追記する流れを作る。
- お願いした背景
...
- 現状どうなってるか
  ...
  - BZM司令塔とTextbook司令塔には、同じ運用を伝播する必要がある。
- 残課題は何か
  - BZM司令塔・Textbook司令塔にも、それぞれの台帳を作らせる。
