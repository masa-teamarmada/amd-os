...
- 現状どうなってるか
  ...
  - 実DB確認では、6月月初に新規材料が少ないと判定されて「新規」が落ちたことまでは分かった。
  - しかし、表示されているシグナルの中に、AMD経営バイタルとして不適切そうなものが混ざっている。
- 残課題は何か
  - どのテーブル・抽出ルールから不適切なシグナルが入っているかを特定する。
  - PJ個別の技術/設立メモと、AMD会社全体の経営バイタルを分離する。
  - 古いシグナルの有効期限、今月への効き方、香川大のような新規獲得活動の反映漏れを検証する。
  - 必要なら、スコア計算前のフィルタ、重み、月次集計、表示ラベルを修正する。
