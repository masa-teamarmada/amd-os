...
- workerには、完了・停止・要判断時にworker内finalだけで終わらせず、必ずAMD OS司令塔へ能動報告させる。
- 報告漏れは起きる前提で、司令塔側のheartbeatや定期確認で、worker終了・停止・報告漏れを検知する。
...
- 現状どうなってるか
  - 動作状態: active worker。前回worker thread `019e808b-5b8d-7841-9e8e-53fbe6cd0666` に差し戻し済み。
  ...
  - ただし、添付の月次試算表そのものを「予算 / 実績 / 差額」で比較できる表にする部分が未完。ここが今回の差し戻し対象。
- 残課題は何か
  - `GasMonthlySimulationPanel` / 月次試算表の行構造を、予算・実績・差額または達成率が並ぶ形へ修正する。
  - 実績source/confidenceをセルや行ラベルで分かるようにする。
  - 修正後にbuild/typecheck/lint、main push、production deploy、ログイン済み画面確認を行う。
