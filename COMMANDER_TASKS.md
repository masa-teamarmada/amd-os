...
- 方針が変わったとき
- workerを切ったとき
- workerから明示closeout報告・blocker報告・差し戻し報告が来たとき
- タスクを完了扱い、保留扱い、またはarchive扱いにするとき

未完タスク監視ルール:
- 未完タスクがあるのに、司令塔が作成したworkerが全員停止・完了・待機で、次のアクションも切られていない状態を作らない。
- workerは親司令塔チャットへ進捗・中間報告・自己判断の完了報告を原則送らない。
- workerスレッド内でまさが「完全に完了」「OK」「これでよし」などと明示した後だけ、親司令塔へ最終closeoutを1回送る。
- 例外は、UU conflict、未分類dirty、権限/破壊的操作/外部判断、同じblocking conditionで進行不能など、司令塔側の介入が必要な場合だけ。その場合も短いblocker/handoffを1回だけ送る。
...
- COMMANDER_TASKS.mdにはworker詳細ログを貼らず、Active workerあり、worker id、状態、次回確認条件、まさ要判断だけを短く残す。
- 以後のworker promptでは、旧「完了・停止・要判断時は必ず親司令塔へ能動報告」を削除/上書きし、このworker quiet modeを優先する。
...
### 7. worker quiet modeを司令塔運用へ反映する

- お願いしたタスク内容
  - workerから親司令塔への進捗・中間報告・自己判断の完了報告を止める。
  - workerスレッド内でまさが「完全に完了」「OK」「これでよし」等と明示した後だけ、最終closeoutを1回送る。
  - 例外は、UU conflict、未分類dirty、権限/破壊的操作/外部判断、同じblocking conditionで進行不能など、司令塔介入が必要な場合だけにする。
- お願いした背景
  - worker報告が親司令塔チャットを流し、まさや司令塔が本当に必要な要判断を見失いやすくなっていた。
  - 司令塔側がheartbeat/read_threadで静かに確認すればよく、workerは必要以上に親チャットへ流れ込まない方がよい。
- 現状どうなってるか
  - 2026-06-03方針変更として、root台帳の未完タスク監視ルールをworker quiet modeへ更新した。
  - 以後のworker promptでは、旧「完了・停止・要判断時は必ず親司令塔へ能動報告」を削除/上書きする。
  - COMMANDER_TASKS.mdにはworker詳細ログを貼らず、Active workerあり、worker id、状態、次回確認条件、まさ要判断だけを短く残す。
- 残課題は何か
  - 運用ルール自体はroot台帳へ反映済み。
  - BZM/Textbook/PWA配下worker promptにも同じquiet modeを優先ルールとして入れる。
