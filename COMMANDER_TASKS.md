...
- 現状どうなってるか
  - 2タブ化そのものはmain反映と本番確認まで完了済み。
  - 本番URLで、上部グラフが残ったまま「進捗管理」と「スコア詳細」を切り替えられることを確認済み。
  - 追加改善として、スコア詳細のキャッシュ/先読みと、横幅いっぱいのタブUI改善をworkerへ差し戻した。
- 残課題は何か
  - スコア詳細タブの2回目以降表示、またはタブ切り替え時の待ち時間を減らす。
  - タブクリック領域を横幅いっぱいの2等分にして、選択中タブが分かる見た目にする。
  - 追加改善後に本番確認し、司令塔がレビューする。
