...
### 1. AMDスコアをMXFモデルから新モデルへ移行する

- お願いしたタスク内容
  - まだ旧いMXFモデルのまま残っているAMDスコアを、新モデルへ移行する。
  - 計算ロジック、コックピット表示、スコア詳細、Venture Map、設計書の整合を取る。
- お願いした背景
  - AMDスコアはプロジェクト判断の中核で、旧モデルが残るとコックピットやTextbook、L2抽出の解釈がズレる。
  - 直近でFRLやTextbook側の整理も進んだため、スコア側も新しい考え方へ合わせる必要がある。
- 現状どうなってるか
  - 専用workerを切り、まずrepo内の正本から「新モデル」が何を指すかを確定するよう指示済み。
  - 新モデルが設計書上で一意に確定できる場合は、実装・検証・commit/pushまで進める。
  - 新モデルが曖昧な場合は、推測で式やDBを作らず、要判断として司令塔へ戻す運用にしている。
- 残課題は何か
  - 旧MXFモデルが残っている箇所を洗い出す。
  - 新モデルの正本定義を確定する。
  - DB変更や本番データ移行が必要か、既存データの互換性をどう守るかを判断する。
  - UIと設計書を新モデルへ揃える。

### 2. NIMS external workspace role の実装準備を進める

- お願いしたタスク内容
  - NIMS Pilot向けに、外部機関ユーザーが安全に入れるworkspaceの権限設計を進める。
  - 初手は実装本体ではなく、schema、認証、API guard、監査ログ、NIMS workspace導線の設計を固める。
- お願いした背景
  - NIMSへAMD OSを見せるには、AMD内部情報、他PJ、報酬、raw source、未公開資料を絶対に漏らさない境界が必要。
  - CX `p20` はNIMS由来の初期ユースケースだが、NIMSという機関workspaceそのものとは同一ではない。
  - 外部ユーザーの編集は正本DB直書きではなく、候補投稿やコメントに限定する必要がある。
- 現状どうなってるか
  - AMD総司令塔から実装準備メモを受領済み。
  - 推奨方針は、NIMSの機関IDは既存DBに合わせて `inst_nims`、りりはAMD側担当、松本さんは機関側owner候補。
  - 専用workerを切り、既存の機関権限設計、DB schema、認証まわり、現行ERS/APIを読ませている。
  - 本番DB適用や外部ユーザー作成はまだ禁止している。
- 残課題は何か
  - 外部機関ユーザーのrole管理、NIMSとCXの紐付け、候補投稿、監査ログのmigration案を固める。
  - 生データを見せないredacted API / view方針を確定する。
  - どこまでmigration作成へ進めるか、どこから本実装workerへ分けるか判断する。
  - 松本さんの正式な権限位置づけは、あとでまさ確認が必要。

### 3. NIMS研究機関コックピットの画面構成を整える

- お願いしたタスク内容
  - NIMSコックピットで、MTGツリーが画面の一番上に出ている違和感を直す。
  - 研究機関コックピットも、PJコックピットと同じように進捗管理、MTGサマリ、月次サマリ、スコア詳細を見られる構成へ寄せる。
- お願いした背景
  - NIMSは新規PJとして増やすのではなく、既存の研究機関カードからコックピットへ入る方針にした。
  - ただし、研究機関はSU/PJとはスコアの付け方が違うため、画面構成は揃えつつ、スコア詳細の中身は研究機関向けにする必要がある。
- 現状どうなってるか
  - worker実装は完了し、branchへpush済み。
  - 画面順は、研究機関概要、評価・関連PJの要約、進捗管理/スコア詳細タブの順に整理された。
  - MTGツリーは画面最上部から外れ、進捗管理タブの中へ移動した。
  - スコア詳細タブは、SU向けのAMDスコアではなく、研究機関向けのERS評価、評価項目、根拠メモを見る構成になった。
  - typecheck、対象lint、buildは通過済み。
  - ただしVercelの日次deploy上限に当たり、本番反映とログイン済み目視確認はまだできていない。
  - quota回復確認用のheartbeatを設定済み。
- 残課題は何か
  - Vercel quota回復後に、最新mainを確認してからbranchをmainへ取り込む。
  - production deployを実行し、ログイン済み画面でNIMSコックピットの並びとタブを目視確認する。
  - 確認できるまでは完了扱いにせず、外部quota待ちとして残す。
...
- 現状どうなってるか
  ...
  - まさがCalendar連携を一度削除して追加し直したが、予定色やカレンダー既定色の取得権限はまだ付かなかった。
  - そのため、追加の再認証ループではなく「予定そのものに明示色がある場合だけ処理候補にし、色が確定できない予定は安全に止める」代替設計へ進めている。
  - そのため、監視付きの1回実行はまだ登録していない。
  - 毎時稼働はまだ有効化していない。
- 残課題は何か
  - まず、予定に明示色がある場合だけ色→PJ判定を続ける代替設計を、正本設計書と実行ルールへ安全に反映できるか確認する。
  - 色が無い予定や既定色の確認が必要な予定は、タイトルだけで勝手にPJ決定せず、skipまたは要確認へ回す。
  - 代替設計が小さく安全に実装できたら、書き込みなし診断を再実行し、候補が見つかった時だけ監視付きの1回実行へ進める。
