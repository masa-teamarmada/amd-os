...
> **BZM 3.0 以降の位置づけ（2026-08-24 追記）**：2026-08-24 の BZM 3.0 採用（承認 #2026-08-24-10）以降、スコアの現行の定義・入力・精度向上の経路は[モデルページ](/model)にある。本章は、その時点までの履歴版として残す。本章の推進力と戦略余力の扱いは BZM 3.0 の変換能力 $c$・八機能の充足・バーンレートへ引き継がれている。
...
その費用は$CF_t<0$として$J$と$P$へ一度だけ入る。

同時に、その支出が性能証拠、再現可能な手順、契約上の権利、顧客の確約、次に実行できる行動を変えたなら、その効果は状態遷移として別に記録する。

費用を正の便益へ言い換えるのではない。

次の状態から可能になる行動の将来CFと終端価値を、後続の分岐で評価する。

この順序を守ると、同じ支出を「費用」と「能力向上の価値」として二度足すことがない。
...
この図の矢印は因果効果の実証結果ではない。

現時点では、事象、状態効果、制約、遷移を混ぜずに記録し、どこに根拠がないかを見えるようにする設計図である。

### 状態から価値までの六つの区別

OSを読むときは、次の六つを同じものとして扱わない。

| 区別する対象 | 問うこと | 例 | まだ言えないこと |
|---|---|---|---|
| **登録方針** | 試算の前提として何を固定したか | $\pi_{\mathrm{reg}}$ | 経営陣が選んだ事実、最適な方針 |
| **実行可能性** | その行動を今実行できるか | $\Gamma_{\mathrm{exec}}^{\mathrm{reg}}$ | 未確認条件も満たすこと |
| **権限** | 誰がその行動を決め、同意できるか | 取締役会、大学、顧客の同意 | 実行可能なら承認済みであること |
| **選択** | 権限者が実際に何を選んだか | $z_k^{\mathrm{obs}}=(a_k,i_k)$ | gate日や試算入力から推測した選択 |
| **イベント** | いつ何が起き、いつ観測されたか | 着金、試験、契約条件充足 | 同月に並ぶ出来事の因果関係 |
| **状態遷移** | 行動後に何が変わったか | $\mathbf s_t\to\mathbf s_{t+1}$ | 変化がその行動の因果効果であること |

登録方針と選択を混同すると、試算が「会社の意思決定を再現した」ように見える。

実行可能性と権限を混同すると、資金や設備があれば誰でも実行できるように見える。

イベントと状態遷移を混同すると、日付が近いだけの二つの出来事から、能力や売上の因果関係を捏造する。

### 八層状態は資源の棚卸しではない

画面の「状態」欄は、資産目録でも固定資源分類でもない。

八層は説明のための状態契約であり、自然なMECE分類、十分統計量、因果グラフを主張しない。

評価日時点$t$で、将来の支払、制約、遷移を予測するために必要な情報を、八つの型へ分けて保持する。

| 層 | 記号 | 画面で探す証拠 | 同じものに見えても分ける理由 |
|---|---|---|---|
| 進捗と知識 | $\mathbf x_t$ | 実験、再現性、有償評価 | 設備保有ではなく、設備で確認した性能 |
| 利用可能な資源 | $\mathbf r_t$ | 現金、工数、設備枠、試料 | 人数ではなく、実際に使える容量 |
| 実行能力の推定 | $\widehat{\mathbf c}_t$ | 反復実績、品質管理、学習記録 | 成果そのものではなく、能力に関する未校正推定 |
| 権利と統治 | $\mathbf k_t$ | 契約、許認可、決裁、利用権 | 前向きな反応ではなく、拘束力ある権利 |
| 関係と確約 | $\mathbf n_t$ | 顧客、共同開発、調達交渉の段階 | 相手方の社数ではなく、撤回可能性と依存 |
| 正当性と受容 | $\boldsymbol\ell_t$ | 顧客、投資家、行政、大学の受容根拠 | 一者の受容を他者の受容へ読み替えない |
| 外部環境と期限 | $\mathbf e_t$ | 規制、予算、競合、失効日 | 内部努力で直ちに変えられない条件 |
| 未確定事象への信念 | $\mathbf b_t$ | 成功、調達、採用の根拠と更新 | 観測事実そのものではなく、その条件付き見込み |

採択通知は、条件付きの権利、政策上の受容、将来資金への信念、報告義務を同時に変えうる。

それでも採択事実は一件である。

一件の事象を複数の状態効果へ写しても、同じ入出金脚や同じ尤度を複数回使わない。

### $J$、$P$、$Q$、$S$を一つの点数にしない

pilotの四記号は、同じ問いへの別表現ではない。

| 記号 | 何を固定するか | 何を数えるか | 現pilotでの地位 |
|---|---|---|---|
| $P$ | 登録方針と全gate通過 | 月次CFと終端価値 | 成功条件付きの現在価値 |
| $Q$ | 登録gateの順序 | 全gateを通り切る代理指数 | 未校正のgate積 |
| $S$ | 登録gateと登録stress | 最悪stress下のgate積 | 未校正のstress最小積 |
| $J$ | 登録方針と全分岐 | 途中CF、成功時価値、停止時価値 | 全経路を重みづけした現在価値 |

$d_t=(1+r_d)^{-t/12}$は、評価日時点から$t$か月後の金額を現在価値へ直す割引係数である。

$r_d$は年率の割引率、$H$は評価する月数、$d_H$は最終月の割引係数である。

$G$は登録gateの集合、$t_i$はgate $i$を判定する月、$p_i$は先行gateを通ったことを条件にしたgate $i$の通過値である。

$W_t=\prod_{i\in G:t_i\le t}p_i$は、その月まで経路が続いている重みである。

現pilotでは、gate月のCFを判定後に発生するものとしてこの$\le$を使う。

判定前CFを含める計算契約なら、$W_t$は$t_i<t$で定義し直さなければならない。

$Q=\prod_{i\in G}p_i$は、全gateを通る経路だけに付く重みである。

$CF_t$は月次経済CF、$TV$は目標到達後の終端価値、$RV_i$はgate $i$で停止したときの残存価値である。

$\Delta_{\mathrm{reg}}$は登録stressの集合であり、$m_{i\delta}$はstress $\delta$の下でgate $i$の通過値へ掛ける補正である。

したがって、完全式は次になる。

$$
J
=\sum_{t=1}^{H}d_tW_tCF_t
+d_HQ\,TV
+\sum_i d_{t_i}\left(\prod_{j<i}p_j\right)(1-p_i)RV_i,
\qquad
...
戦略余力は、実行可能行動集合、頑健捕捉領域、方針を失う時点を含む診断であり、$S$はその一部を画面用に射影した値にすぎない。

### なぜ$J\neq QP$なのか

$QP$は、全gateを通ったときの価値$P$に、全gateを通る重み$Q$だけを掛ける。

この計算では、失敗が確定する前に発生したCFと、どのgateで止まったかに応じて残る価値が消える。
...
内部pilotのp03では、$Q=0.13572$、$P=¥384M$、$J=¥12M$、停止寄与は$-¥20M$だった。

$QP$は約$¥52M$となる。

この約$¥40M$の差は、計算誤差ではない。

途中CFが全gate失敗後まで一律に$Q$で縮められないことと、停止時の残存価値が負になりうることを反映した差である。

この恒等式は、逐次条件付きの$p_i$、gate時点の規約、$RV_i$とgate識別子の対応が正しい場合だけ成り立つ。

### 月次試算表と時間軸を照合する

OSの横時間軸では、M0から$H$までの各月に、登録方針、gate、資金イベント、事業・技術イベント、月次PL、$CF_t$が同じ列で表示される。

月次PLは会計上の売上、費用、利益を読むための帳票であり、$CF_t$はBZMの経済評価へ入る入力である。

両者が同じ月に並ぶことは、同じ値、同じ認識基準、同じ因果関係を意味しない。

資金調達の着金も、流動性と実行可能性を変えるが、会社PJの価値創造CFへ便益として加えない。

そうしなければ、調達額を事業の売上や価値として二度数えるからである。

### 演習と未検証境界

1. 任意のgate一つについて、$t_i$、$p_i$、$RV_i$、根拠資料、判定主体を探しなさい。どれか一つが欠けるとき、そのgateは何を主張できないかを書きなさい。
2. 月次PLの一行と同じ月の$CF_t$を比べなさい。差があるなら、会計認識、非現金項目、評価範囲のどれが理由として登録されているかを確認しなさい。根拠がなければ、不明として残しなさい。
3. ある支出を一つ選び、八層のどれを変えると仮定しているか、その後にどの行動の実行可能性が変わるかを書きなさい。支出額を正の便益として別に足していないかも点検しなさい。
4. $p_i=0.45$の流動性代理gateを見つけたら、証拠から算定された確率ではなく、欠測時の共通priorが使われている可能性を確認しなさい。現pilotでは12PJすべてでこの0.45 priorが採用され、PJ間の比較情報を運んでいない。
5. $S$が高くても、$\Gamma_{\mathrm{exec}}^{\mathrm{reg}}$が狭い、共有資源制約が未実装、または複数経路が共通故障点を持つなら、「戦略余力が高い」とは結論できない理由を説明しなさい。

現pilotは12PJの内部shadow試算であり、前向き検証は0件である。

共有資源制約は未実装である。

可視資料から集めた証拠には、private資料、権限、保存、探索順による非ランダム欠測がある。

状態変化と後続成果の時間順が合っても、行動が原因だったとは識別できない。

八層、三値制約、gate積、stress最小積、$J/P/Q/S$の表示はBZM固有の設計判断であり、既存文献が実証済みの結論として与えるものではない。

この試算をPJ順位、投資、撤退・継続、資源配分の判断には使わない。

---

## 17. 一次文献とBZM 2.2の追加範囲
