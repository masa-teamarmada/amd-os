...
**Figure 3. The same projects, scored twice.** Open markers are the score from the operating database alone; filled markers add facts obtainable only by asking; arrows mark movements above 5%. Ten projects are identical by construction (no elicited inputs); of the eleven that had them, five move by more than a factor of two. Colour gives the sample layer.

Four things in this picture are worth more than the headline number.

**The effect is concentrated, not diffuse.** The median ratio across all twenty-one projects is 1.00 and the rank correlation between the two scorings is 0.93: most of the ledger is unmoved, and a selector reading only summary statistics would conclude that the record is adequate. It is the tail that matters — five projects move by a factor of two or more, two by an order of magnitude, and the largest ratio is a factor of 2,306. Rankings shift by up to seven positions. Missing information does not degrade this instrument gracefully; it leaves most estimates untouched and destroys a few.
...
**Where the record is silent, defaults decide.** The largest movement is instructive precisely because it is an artifact. One project's evidence stage was not derivable from the database at all; the records-only pass therefore fell back to the Tier-0 default of "before the first gate", which drove its value to approximately zero and its rank to last. Adding one elicited fact restored it to fifteenth. The framework's defaults, in other words, are not a neutral representation of ignorance — they are a decision to treat the unrecorded as the unstarted. For a selection instrument this is the wrong direction of error: it penalizes projects whose operators keep poor records rather than projects that are actually behind, and it does so silently. Calibrating the default set is therefore not housekeeping but a substantive item of the research program (§8.4).

**The main sample did not move at all.** None of the six pre-incorporation projects — the layer this framework exists for — changed under dual scoring. The reason follows from the two findings above: what elicitation added for those projects was organizational, and organizational information is damped; the financial facts that do move scores exist as *facts to be missed* mainly once operations are running and generating documents. The honest reading is therefore narrower than the one we set out to test: for projects before incorporation, on this ledger, **it is the default set and not the elicited evidence that determines the score**. That is a finding about where the effort should go — into defaults calibrated on the pre-incorporation population, and into representing carrier effects strongly enough that asking about people changes something — and it is registered as such in §8.4.
