...
**The movement runs one way, and the top of the ranking does not move.** Of the projects that moved, all but one moved *down* when elicited facts were added: the operating record, left to itself, scores systematically high. For a selector this is the most actionable result here — a database-only score is an upper bound, not an estimate — and it follows mechanically from the default condition of §7.2, since defaults are set at a comfortable level and reality is usually less comfortable. The top five positions were identical under both passes; the reshuffling happened in the middle of the ranking, among the projects a committee would actually be deciding between.

**The effect is concentrated, not diffuse.** The median ratio across all twenty-one projects is 1.00 and the rank correlation between the two passes is 0.89: a selector reading only summary statistics would conclude the record is adequate. It is the tail that matters — five projects move by a factor of two or more, two by an order of magnitude — and rankings shift by up to nine positions. Missing information does not degrade this instrument gracefully; it leaves most estimates untouched and destroys a few.

**The gaps that move rankings are financial, not organizational.** Affected projects were missing several inputs at once, so we identified the contributions by re-scoring each project with one input withheld at a time (33 single-withholding runs; SM-D). Withholding what a project can actually spend moves its score by a median factor of 5.8 and a maximum of 24; its burn rate, by 1.6 and 26; its own earnings, up to 4.4; whether unit economics close, up to 4.0. Withholding organizational facts moves almost nothing: the evangelist function, missing in eight projects, has a median effect of 1.03 and a maximum of 1.27, and the vacancy of the technical core — the single fact that nobody can build the product — moves the score by 3%.
...
**In the main sample the exercise had almost no power, and we report that rather than a null result.** None of the six pre-incorporation projects moved by more than 3%. It would be convenient to read this as evidence that the record is adequate before incorporation, and we cannot: only four of the six had any elicited input, and only one of those included a financial input. The design could not have detected in the main sample the effect it detected elsewhere. Substantively, the financial facts that move scores largely come into existence as documented facts capable of being missed once operations run; before incorporation there is less of that kind of fact to lose, and what elicitation supplied instead was organizational, which the instrument damps. Methodologically, testing information sufficiency where this framework is meant to be used requires collecting financial elicitation for pre-incorporation projects deliberately rather than opportunistically — registered as a design requirement in §8.4.
