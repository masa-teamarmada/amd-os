...
**The gaps that move rankings are financial, not organizational.** Every project that moved by more than a factor of two was missing the same kind of fact: how much cash is genuinely its own to spend, what it actually burns, whether unit economics close, what evidence stage it has really reached. Projects whose only missing input concerned the carrier — whether the evangelist function is filled, how fast the team converts resources into progress — barely moved at all. That asymmetry is not evidence that people matter less. It is the signature of §7.4's limitation seen from the data side: the framework damps organizational information by construction, so the record's silence about people is currently invisible to the score. A framework that represented carrier effects with the strength the founding-team literature reports<sup>[64,65]</sup> would show a second cluster of movement that this one cannot.
...
**The main sample did not move at all.** None of the six pre-incorporation projects — the layer this framework exists for — moved by more than 3% under dual scoring. The reason follows from the two findings above: what elicitation added for those projects was organizational, and organizational information is damped; the financial facts that do move scores exist as *facts to be missed* mainly once operations are running and generating documents. The honest reading is therefore narrower than the one we set out to test: for projects before incorporation, on this ledger, **elicitation moved nothing that the framework was able to register**. That is a finding about where the effort should go — into representing carrier effects strongly enough that asking about people changes something, and into calibrating the pre-incorporation defaults that currently carry those scores — and it is registered as such in §8.4.
