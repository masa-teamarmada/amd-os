...
*Draft v2.2 (full working draft). Target: Journal of Technology Transfer. Word count (main text): ~8,900. The model version is frozen (main commit 34aaa284; approvals #2026-08-29-1 and #2026-08-29-2; 29 Aug 2026 — recorded in SM-G). Four figures are computed at the frozen version; the remaining placeholder is the dual-scoring distribution.*
...
A fifth premise breaks more quietly. Nearly every quantitative treatment of young-organization survival models death as absorption at zero resources (Levinthal, 1991). That logic presupposes an entity with payroll to meet. A research laboratory that runs out of project money does not die: it slows down, survives on institutional infrastructure, and waits for the next public grant. Before incorporation, cash-out changes *speed*, not existence. Any valuation that imports absorption-at-zero into this domain therefore imports a firm premise — and, as Figure 3 shows, the error is not decorative: whether cash matters enormously or hardly at all turns out to be the sharpest behavioral difference between the pre- and post-incorporation regimes.
