...
The model was applied to the twenty-one projects on one venture builder's 2026 screening ledger, spanning process, device, software, and service types across regulated and unregulated sectors. Fifteen of the twenty-one are already incorporated — a fact that matters, because the framework's domain is the pre-incorporation interval. The evaluation therefore stratifies: the six pre-incorporation projects are Before Zero applications proper; the incorporated fifteen are re-scored as *retrospective* applications, with the evaluation date moved back before incorporation and inputs restricted to information datable to that time, and any project where this restriction cannot be enforced is reported as dropped rather than silently included. All numbers in this section are computed at the model version approved as of 29 August 2026 and are **preliminary**: the definitive computation happens at a declared input freeze (version hash, approval identifier, and freeze date recorded in SM-G), and this paper reports the freeze-date numbers when they exist. [Placeholder: final stratified counts and freeze date — input freeze at S3 close.]
...
## 7.3 The representative case: 2nd to 19th on four facts

Project A (assembly-type, unregulated; wireless-power components) illustrates the mechanism with uncomfortable clarity. Scored from the database alone — patents re-verified at the national registry, a live development contract, evidence level upgraded on documented filings — it ranked **2nd of 21, at ≈¥103bn**. Structured elicitation then surfaced four facts the database did not hold: the cash runway was months, not years; volume production had never been entered; the principal's credit standing blocked ordinary financing; and the addressable-market figure conflated the component's market with the system's. Re-scored, the project fell to **19th of 21, at ≈¥0.05bn** — a revision of three orders of magnitude, produced not by changing the model but by changing what the model was allowed to know. The general statement of which this is an instance: *the ranking is dominated by information the operational record does not capture.*

## 7.4 The structural counterpart: one organizational input

The dual-scoring result has a structural echo inside the model itself. Of the eight carrier functions, the currently approved version accepts exactly one as a per-project input (the evangelist-function prospect $e$); the technical core is fixed "never vacant" by convention, and the remaining functions fill by portfolio-common supply processes. In two live cases — one a project whose only person able to build the product had left, one a project whose principal was degrading key negotiations — sweeping $e$ across its entire admissible range moved the score by only a few percent; the evaluators could represent organizational collapse only by re-assigning the *evidence level*, a workaround that is neither reproducible across evaluators nor reversible when the organization recovers. The lesson is the same as §7.3 read from the model's side: the binding constraint is what the observation system can receive. (A model revision opening all eight functions to per-project observation, together with the operational "organization" registry that now exists to feed it, is under review; §8.4.)
...
During input preparation the ceiling convention was revised twice in three days — from total addressable market to obtainable segment, then to the serviceable slice net of the share already carried by the model's own take-rate — and individual project values moved by factors of ×289 and ×1,600. We report this not as an embarrassment but as a finding about the object: since $V$ is constructively proportional to the ceiling (A6), *ex-ante selection output is order-of-magnitude sensitive to how the market is sliced*, and any framework that hides this behind a stable-looking composite is not more robust, only less honest. Two governance consequences follow and are implemented: ceiling conventions are versioned with approval records, and the reported 10/50/90 bands are labeled as **not** propagating ceiling uncertainty (A7) — calling a band a confidence interval while its largest driver sits outside it would be the quiet lie this section exists to prevent.
