...
**Figure 3. The same projects, scored twice.** Open markers are the score from the operating database alone; filled markers add facts obtainable only by asking; arrows mark movements above 5%. Ten projects are identical by construction (no elicited inputs); of the eleven that had them, five move by more than a factor of two and the largest ratio is 86. Colour gives the sample layer.

Four things in this picture are worth more than the headline number.

**The effect is concentrated, not diffuse.** The median ratio across all twenty-one projects is 1.00 and the rank correlation between the two scorings is 0.89: most of the ledger is unmoved, and a selector reading only summary statistics would conclude that the record is adequate. It is the tail that matters — five projects move by a factor of two or more, two by an order of magnitude, and the largest ratio is a factor of 86. Rankings shift by up to nine positions. Missing information does not degrade this instrument gracefully; it leaves most estimates untouched and destroys a few.

**The gaps that move rankings are financial, not organizational.** In the paired comparison the affected projects were missing several inputs at once, so we identified the contributions by re-scoring each project with one input withheld at a time (33 single-withholding runs; SM-D). The separation is unambiguous. Withholding what the project can actually spend moves the score by a median factor of 5.8 and a maximum of 24; its burn rate, by 1.6 and 26; its own earnings, up to 4.4; whether unit economics close, up to 4.0. Withholding organizational facts moves almost nothing: the evangelist function, missing in eight projects, has a median effect of 1.03 and a maximum of 1.27, and the vacancy of the technical core — the single fact that a project has nobody able to build the product — moves the score by 3%.

That asymmetry is not evidence that people matter less; it is §7.4's limitation seen from the data side. The clearest case is a project whose recorded difficulty is entirely organizational: the only person able to build the product had left, and voting control prevented replacing the chief executive. The framework marks it down by a factor of eleven — but the withholding analysis attributes almost all of that to its cash position, and almost none to the organizational facts that a human evaluator would give as the reason. The score is right about the project and wrong about why. A framework that represented carrier effects with the strength the founding-team literature reports<sup>[64,65]</sup> would show a second cluster of movement that this one cannot, and until it does, the resistance of these scores to organizational evidence should be read as a property of the instrument rather than of the projects.

**What "records only" means had to be decided, and the decision matters.** Constructing the records-only condition requires saying what the database alone establishes. The natural implementation — omit the elicited input and let the framework's Tier-0 default stand — is wrong wherever the record fixes a *lower* bound that the default ignores. Two projects made this concrete: elicitation had denied that they had reached a *further* evidence stage, but the database on its own already documented years of operation and recorded sales, so falling back to the default of "before the first gate" would have priced a fourteen-year-old operating company as pre-demonstration and produced a spurious ratio in the thousands. Those inputs are therefore classified as record-derived. The general rule we adopted, and recommend to anyone repeating this exercise, is that an input counts as elicited only when the record establishes neither its value nor a bound that constrains it.

**In the main sample the exercise had almost no power, and we report that rather than a null result.** None of the six pre-incorporation projects moved by more than 3%. It would be convenient to read this as evidence that the record is adequate before incorporation, and we cannot: of those six, only four had any elicited input at all, and only one of the four included a financial input. The design therefore could not have detected in the main sample the effect it detected elsewhere. Two things follow. Substantively, the financial facts that move scores — a balance, a burn rate, a closing unit margin — largely come into existence as *documented facts capable of being missed* once operations run; before incorporation there is less of that kind of fact to lose, and what elicitation supplied instead was organizational, which the instrument damps. Methodologically, testing information sufficiency where this framework is meant to be used requires collecting financial elicitation for pre-incorporation projects deliberately rather than opportunistically, and that is registered as a design requirement of the prospective registry in §8.4.
