# PAPER_P1_PROGRESS.md — P1 (1報目論文) 進捗台帳 (L3)

> **v2 再始動（2026-08-29）**: BZM 3.0 ベースで全面再設計し S1 完了。旗艦=追加性評価の作法を法人前の案件単位事前選別へ移す（評価対象を〈登録された計画〉へ）。投稿先 = **Journal of Technology Transfer**（RP は第2報へ温存）。計画正本 = `PAPER_P1_MASTER_PLAN.md` v2、監査と判断の経緯 = `PAPER_P1_V2_PLAN_2026-08-28.md`。**以下の v1 ステージ表と改稿ログは履歴**（旧稿 = `PAPER_P1_DRAFT.md` は撤回済み定理の上に建つ。素材としてのみ参照）。
>
> **v2 ステージ（2026-08-30 更新）**: S1 骨格 ✅ ／ S2 outline ✅ ／ S3 初稿 ✅ ／ **S5 模擬査読 ✅（5人格＋引用照合）** ／ **S5 改稿 進行中（v4.1）**
>
> **査読の判定**: 編集長=desk reject（再投稿は歓迎）／評価経済学者=major（reject寄り）／技術移転研究者=major／方法論=major／実務家=条件つきyes（「円の数字を委員会に出さないこと」）。引用照合=72件中71件正しく捏造ゼロ。
> 統合と改稿計画 = `S5_REVISION_PLAN_V2.md`（R-A〜R-Q）。個別の指摘はえいみが正本・実装で裏を取ってから採否を決めた。
>
> **対応済み（v3.1〜v4.1）**: 実装と食い違う表現6箇所を式の直後で開示／§7.1 の遡及評価は実行していないと明記（除外ゼロも）／
> 「記録のみ」条件は実質「既定条件」だと書き直し／動いた向き（記録だけは系統的に高く出る）と上位の安定を報告／
> 運用コスト（初回1件30〜60時間、年60件で2〜3人）と最小構成（円を出さない4部品）を §8.2 に／
> ゲーム耐性の抜け道4つ／反証条件に判定者・期間・閾値・代理アウトカム＋外部寄託／新規性を3点に絞り先行研究5件追加／
> 利益相反の記述の誤り（投資・支配）を削除し測定独立性の限界へ置き換え／設計命題を反証可能な2件と設計判断に二分。
>
> **分類の凍結と再計算（2026-08-30）**: 3条件（値がある→その値／境界だけ→境界／沈黙→既定）で入力分類をやり直し、
> **計算の前に凍結**（`CLASSIFICATION_FREEZE.md`、sha256 記録、commit `a4534aef`）。その後 21件×2条件42回＋単独マスク34回を実行。
> 結果は `paper_p1_dual_scoring.md`（比の中央値 1.00・最大 ×88.2・順位相関 0.9325・順位変化の最大7位・
> 金の側 中央×1.96/最大×26.8 vs 人の側 中央×1.03/最大×1.29）。前回より効果はやや小さく、結論3点は保たれた。
>
> **専有可能性を外した値**: 価値の積み上げにしか入らず判定に効かないので、スコア÷取り分で厳密に出る（8桁で数値確認）。再計算不要。
>
> **図2の差し替えと二重採点ツールの修正（2026-08-30）**: 図2を凍結版 `a149fc30` の21件へ差し替え（`73fd5bae`）。
> 丸めた `model/cases/SCORES.md` ではなく DB `seed_bzm30_scores` の値を使った（13位1.823億と14位1.800億が表示上どちらも「1.8億」で順位が付かないため）。
> 21件とも幅の倍率が SCORES.md と一致。キャプションの上限を 11.6→**11.8**。図5点を再生成し、本文幅736pxの1倍実描画で全図の文字の可読性を確認。
> 差し替えの過程で `bzm/tools/dual_scoring.cjs` の欠陥2件を発見・修正——(1) 会社化後の計画バーン `burn_post_yen_month` を
> 読んでおらず、未会社化でこの値を持つ2件（B・L）だけ凍結版を再現できていなかった（修正後は21件すべて一致。動いたのは §7 の表の水準2行だけで、
> 比・順位・要約・単独マスクは不変）／(2) p09 と p04 のラベルが入れ替わっており、それが原因の観察
> 「JOYCLE の資金調達ラウンド記録の不整合」を取り下げた。凍結ファイル2点は sha256 を保つため書き換えず、訂正は `CLASSIFICATION_FREEZE.md` へ。
>
> **稿の点検と7件の修正（2026-08-30、`8805ab9f`）**: **語数の刈り込みは不要と判明**——JTTの投稿規定に本文の語数制限は無い
> （上限は要旨の150〜250語だけ。「10,000語へ」は模擬査読エージェントの発言で規定ではなかった）。代わりに改稿計画と突き合わせて7件直した:
> ①図5が図2と同じ版ずれ（承認 #-3 前の縮退検査表）→凍結版へ。図4は二つの前提の比較なので両腕を #-3 前でそろえたまま版差を開示
> ②凍結の識別子3箇所が古い（91385d77→a149fc30、承認-3追加）③§1と§7.3の矛盾（「効果が皆無」vs「検出力が無かった」）を§7.3側へ統一
> ④§5.2の「専有可能性を外した値を併記する」が未実施→比例が厳密なので寄託方式へ ⑤報告様式に順位帯の規則を追加（R-L）
> ⑥§7.5の重複段落を解消＋較正が切れている事実を明記（R-I残り）⑦要旨268→250語、実証の数字が会社化済み層から出ていることを明記（R-B）
>
> **残り**: ①**補足資料 SM-A〜SM-G を v2 で作る**（本稿は21箇所から参照しているのに実体は旧版のみ。投稿の前提が欠けている。
> 棚卸しと供給元は `HANDOFF_P1_2026-08-30.md`。旧SMは流用不可、英語で8,000〜12,000語＋表10点前後、大半は正本からの起こし）。
> **まさ判断待ち**: 匿名化（3人が特定可能と判定）／投稿先（JTT再投稿か誌を変えるか）。
>
> （旧注記 2026-07-29: 旧S1〜S6成果は、Book A版の旧定理撤回とBZM 2.0への改訂により投稿停止。進捗達成は取り消さず履歴として残すが、投稿可能性を意味しない。）

*L1 = `PAPER_P1_MASTER_PLAN.md` / 旧S2 成果物 = `PAPER_P1_OUTLINE.md`。ステージ定義は L1 §7。*
...
| 2026-07-11 | **S6 前倒し照合の完了分を反映**: 引用62件照合 = 59 OK / FIX 3件 (Zhang–Guo 著者順・Perkmann 全16著者・Fishburn 号数) を本文・SM に適用、✓マーク廃止 (全件 DOI 照合済みのため)。査読者ロングリスト10+2名を S6_PACKAGE §8 に収載。PREVIEW v0.8。**[verify] タグ照合のみ保留 (まさが照合エージェントを停止 → 再開判断待ち)**。改称+照合後の語数 9,955w (数式除外、上限内) | えいみ |
| 2026-08-30 | **図2を凍結版 a149fc30 へ差し替え + 図5点再生成 + 本文幅での可読性を実描画で確認**。あわせて `bzm/tools/dual_scoring.cjs` の欠陥2件（`burn_post_yen_month` の読み落とし・p09/p04 のラベル入れ替わり）を修正し、影響範囲を再計算（§7 の結論は不変）。凍結ファイルの sha256 は維持し、訂正は `CLASSIFICATION_FREEZE.md` に記録 | えいみ |
| 2026-08-30 | **投稿規定を確認し語数刈り込みが不要と判明**（JTTは本文の語数制限なし）。稿を改稿計画と突き合わせて7件修正（図5の版ずれ・凍結識別子・§1の矛盾・§5.2の未実施・報告様式・§7.5の重複と較正・要旨250語）。**残る最大の穴 = v2の補足資料が未作成**（21箇所から参照、旧版は流用不可）| えいみ |
