...
- `g05_timeline_before_zero.svg` と `g06_uncertainty_map.svg` は初版の図意図が弱かったため、`2026-06-14-g05-g06-figure-design-spec.md` を設計正本として再作成。
- `g05` は「長い助走区間」と「助走区間に打ち込まれる後戻りしにくい判断」を分離して読ませる。ゼロはゴールではなく、手前の選択が制約として表面化する地点として扱う。
- `g06` は中央シーズを主役にしすぎず、七つの未整理領域と領域間の連動を主役にする。中央からの強い放射矢印は避け、外周の相互依存を強める。
