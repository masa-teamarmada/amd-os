...
const inputV3 = JSON.parse(fs.readFileSync(path.join(here, "sps-2-0-sx-inputs-v0.3.json"), "utf8"));
...
function runCase(caseSpec) {
  ...
  for (let i = 0; i < runs; i += 1) {
    ...
    const tBiz = dB1 + dB2;
    const tF = Math.max(tTech, tBiz) + samplePert(nodeTime.G, rng, scale);
...
const results = {
  version: "sps-2.0-sx-results-v0.3",
...
const outPath = path.join(here, "sps-2-0-sx-results-v0.3.json");
