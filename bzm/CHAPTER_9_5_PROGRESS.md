...
| 9.5.5 | 観測レンズ — CRL L0-L5 / ICT: 許容事象集合・自動降格・機械抽出・三値区別 (ECR 非合算の公理的正当化) | 4.5 |
| 9.5.6 | S 動学への接続 — 命題 9.5.4 (排他的主経路割当 + leave-one-out ECR₋ᵢ) + バイアス符号の系 | 3.5 |
...
| K3 | 独禁の提示形式 | (a) 仮定 L (formal 格納) + 本文 box (非断定文面) 併用 + Ch 37.5 番号割当。**⚠️ D-058 追加補注 (2026-07-02): 仮定 L の位置づけは「書籍上の Tier 規律の留保 (学術書として断定を避ける)」であり「実務ブロック」ではない。実務適法性は AMD の20超SU独占契約実績で既に担保。弁護士発注はしない**。本文 Box には「実務で契約設計する際は当該案件の専門家と確認せよ」の注意喚起を1文入れる。 |
...
| K5 | 記号ブロックの D-051 整合 | (a) ℛ = (d*, 𝒩, 𝒦, 𝒳, Γ)、主経路タグ m(e)、ECR₋ᵢ。付録A冒頭表に一括追記 |
| K6 | Tier A 観測記述の開示粒度 (SX/SIER・KENQ) | (a) CRL 到達レベル + 事象種別のみ (金額なし or 幅表記)、composite 宣言、案件全体像は Ch 15 に分担。**金額幅表記の要否はまさ判断** |
