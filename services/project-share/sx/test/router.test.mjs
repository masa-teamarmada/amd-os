...
function withEnv(fn, { allowedEmails = EMAIL } = {}) {
  return async () => {
    ...
    if (allowedEmails === null) {
      ...
      process.env.SX_ALLOWED_EMAILS = allowedEmails;
    }
    try {
      await fn();
    } finally {
...
test(
  ...
  async () => {
    delete process.env.SX_ACCESS_PASSWORD;
    delete process.env.SX_AUTH_SECRET;
    delete process.env.SX_ALLOWED_EMAILS;
    delete process.env.SX_ADMIN_PASSWORD;
...
test(
  ...
  withEnv(async () => {
    ...
    assert.match(res.headers["Set-Cookie"], /sx_auth=/);
...
test(
  ...
  withEnv(
    async () => {
      ...
      process.env.SX_ALLOWED_EMAILS = "someone-else@example.com";
...
test(
  ...
  withEnv(async () => {
    const cookie = authCookieHeader();
    process.env.SX_ACCESS_PASSWORD = "a brand new password";
    const req = makeReq({ method: "GET", url: "/", headers: { cookie } });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "GET",
      url: "/",
      headers: { cookie: "sx_auth=not-a-valid-token" },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      ...
      body: { pathname: "sx/files/deck.pdf" },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "POST",
      url: "/api/logout",
      headers: { origin: "https://sx.example.com", cookie: authCookieHeader() },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
