...
test("listFiles maps blobs to name/size/uploadedAt and drops the bare prefix marker and folder objects", async () => {
  ...
  const listFn = async (opts) => {
    assert.equal(opts.prefix, "sx/files/");
    return {
      blobs: [
        { pathname: "sx/files/", size: 0, uploadedAt, url: "https://example/should-not-leak" },
        { pathname: "sx/files/reports/", size: 0, uploadedAt, url: "https://example/should-not-leak" },
        { pathname: "sx/files/legacy/.folder", size: 0, uploadedAt, url: "https://example/should-not-leak" },
        { pathname: "sx/files/deck.pdf", size: 1234, uploadedAt, url: "https://example/should-not-leak" },
      ],
      hasMore: false,
    };
  };
  ...
  assert.equal(files[0].pathname, "sx/files/deck.pdf");
...
test("listDirectory returns immediate folders and files, hiding markers and nested blobs", async () => {
  ...
  const listFn = async (opts) => {
    ...
    return {
      blobs: [
        { pathname: "sx/files/deck.pdf", size: 10, uploadedAt, url: "https://x/should-not-leak" },
        { pathname: "sx/files/reports/.folder", size: 0, uploadedAt, url: "https://x/should-not-leak" },
        { pathname: "sx/files/reports/q1.pdf", size: 20, uploadedAt, url: "https://x/should-not-leak" },
        { pathname: "sx/files/reports/nested/deep.pdf", size: 30, uploadedAt, url: "https://x/should-not-leak" },
        { pathname: "sx/files/あいう/.folder", size: 0, uploadedAt, url: "https://x/should-not-leak" },
      ],
      hasMore: false,
    };
  };
...
test("listDirectory dedupes folders discovered via multiple nested blobs", async () => {
  ...
  const listFn = async () => ({
    blobs: [
      { pathname: "sx/files/reports/.folder", size: 0, uploadedAt },
      { pathname: "sx/files/reports/a.pdf", size: 1, uploadedAt },
      { pathname: "sx/files/reports/nested/deep.pdf", size: 1, uploadedAt },
    ],
    hasMore: false,
  });
...
test("listDirectory scopes to a nested folder prefix and returns relative paths", async () => {
  ...
  const listFn = async (opts) => {
    ...
    return {
      blobs: [
        { pathname: "sx/files/reports/.folder", size: 0, uploadedAt },
        { pathname: "sx/files/reports/q1.pdf", size: 5, uploadedAt },
        { pathname: "sx/files/reports/archive/.folder", size: 0, uploadedAt },
      ],
      hasMore: false,
    };
  };
  const { folder, folders, files } = await listDirectory("reports", { listFn });
  assert.equal(capturedPrefix, "sx/files/reports/");
...
test("listDirectory follows Vercel Blob pagination via hasMore/cursor", async () => {
  ...
  const listFn = async (opts) => {
    ...
    if (!opts.cursor) {
      return {
        blobs: [{ pathname: "sx/files/a.pdf", size: 1, uploadedAt }],
        hasMore: true,
        cursor: "page2",
      };
    }
    return {
      blobs: [{ pathname: "sx/files/b.pdf", size: 2, uploadedAt }],
      hasMore: false,
    };
  };
...
test("listDirectory recognizes native empty folder objects (trailing-slash pathname)", async () => {
  ...
  const listFn = async () => ({
    blobs: [
      { pathname: "sx/files/reports/", size: 0, uploadedAt },
      { pathname: "sx/files/deck.pdf", size: 10, uploadedAt },
    ],
    hasMore: false,
  });
...
test("createFolder creates a native private folder object using a trailing-slash pathname", async () => {
  ...
  assert.equal(captured.pathname, "sx/files/reports/q2/");
...
test("deleteEmptyFolder deletes the native folder object when the folder has no other entries", async () => {
  const uploadedAt = new Date("2026-07-01T00:00:00.000Z");
  const listFn = async () => ({
    blobs: [{ pathname: "sx/files/reports/", size: 0, uploadedAt }],
    hasMore: false,
  });
  const deletedPaths = [];
  ...
  assert.deepEqual(deletedPaths, ["sx/files/reports/"]);
});

test("deleteEmptyFolder deletes a legacy .folder marker when the folder has no other entries", async () => {
  const uploadedAt = new Date("2026-07-01T00:00:00.000Z");
  const listFn = async () => ({
    blobs: [{ pathname: "sx/files/reports/.folder", size: 0, uploadedAt }],
    hasMore: false,
  });
  const deletedPaths = [];
  const delFn = async (pathname) => {
  ...
  await deleteEmptyFolder("reports", { listFn, delFn });
  assert.deepEqual(deletedPaths, ["sx/files/reports/.folder"]);
});
...
test("deleteEmptyFolder deletes both the native folder object and a legacy .folder marker when both are present", async () => {
  ...
  assert.deepEqual(
    ...
    new Set(["sx/files/reports/", "sx/files/reports/.folder"])
  );
});
...
test("deleteEmptyFolder rejects a folder containing a nested entry without deleting the marker", async () => {
  ...
  const listFn = async () => ({
    blobs: [
      { pathname: "sx/files/reports/", size: 0, uploadedAt },
      { pathname: "sx/files/reports/nested/deep.pdf", size: 5, uploadedAt },
    ],
    hasMore: false,
  });
  const delFn = async () => {
...
test("deleteFile delegates to the provided delete function with the pathname", async () => {
  ...
  await deleteFile("sx/files/deck.pdf", { delFn });
  assert.equal(calledWith, "sx/files/deck.pdf");
});
...
test("moveFile renames a root-level file into a target folder, checking the destination first", async () => {
  ...
  const result = await moveFile("sx/files/deck.pdf", "reports", { headFn, renameFn });
  assert.deepEqual(result, { pathname: "sx/files/reports/deck.pdf" });
  assert.deepEqual(calls, [
    { op: "head", pathname: "sx/files/reports/deck.pdf" },
    {
      op: "rename",
      from: "sx/files/deck.pdf",
      to: "sx/files/reports/deck.pdf",
...
test("moveFile renames a nested file back to the root, keeping its basename", async () => {
  ...
  const result = await moveFile("sx/files/reports/q1.pdf", "", { headFn, renameFn });
  assert.equal(renamedTo, "sx/files/q1.pdf");
  assert.equal(result.pathname, "sx/files/q1.pdf");
});
...
test("moveFile rejects an unsafe target folder", async () => {
  await assert.rejects(() => moveFile("sx/files/deck.pdf", "../etc"));
});
...
test("moveFile rejects moving a file to the folder it is already in", async () => {
  ...
  await assert.rejects(
    () => moveFile("sx/files/reports/deck.pdf", "reports", { renameFn }),
    SameLocationError
  );
});
...
test("moveFile rejects moving a root file back to the root", async () => {
  ...
  await assert.rejects(() => moveFile("sx/files/deck.pdf", "", { renameFn }), SameLocationError);
});

test("moveFile rejects when a same-named file already exists at the destination, without renaming", async () => {
  const headFn = async () => ({ pathname: "sx/files/reports/deck.pdf" });
  const renameFn = async () => {
    throw new Error("should not be called");
  };
  await assert.rejects(
    () => moveFile("sx/files/deck.pdf", "reports", { headFn, renameFn }),
    DestinationExistsError
  );
});
...
test("moveFile propagates unexpected head() failures instead of treating them as not-found", async () => {
  ...
  await assert.rejects(() => moveFile("sx/files/deck.pdf", "reports", { headFn }), /network down/);
});
...
test("renameFile renames a nested file in place, checking the destination first", async () => {
  ...
  const result = await renameFile("sx/files/reports/q1.pdf", "q2.pdf", { headFn, renameFn });
  assert.deepEqual(result, { pathname: "sx/files/reports/q2.pdf" });
  assert.deepEqual(calls, [
    { op: "head", pathname: "sx/files/reports/q2.pdf" },
    {
      op: "rename",
      from: "sx/files/reports/q1.pdf",
      to: "sx/files/reports/q2.pdf",
      options: { access: "private", allowOverwrite: false },
    },
  ]);
});
...
test("renameFile rejects unsafe names and unchanged names", async () => {
  ...
  await assert.rejects(() => renameFile("sx/files/deck.pdf", "../etc", { renameFn }));
  await assert.rejects(() => renameFile("sx/files/deck.pdf", "deck.pdf", { renameFn }), SameLocationError);
});

test("renameFile rejects when the destination already exists", async () => {
  const headFn = async () => ({ pathname: "sx/files/renamed.pdf" });
  const renameFn = async () => {
    throw new Error("should not be called");
  };
  await assert.rejects(
    () => renameFile("sx/files/deck.pdf", "renamed.pdf", { headFn, renameFn }),
    DestinationExistsError
  );
});
...
test("createSignedAccessUrl issues a short-lived GET delegation and presigns the exact pathname", async () => {
  ...
  const presignUrlFn = async (signedToken, options) => {
    ...
    assert.equal(options.pathname, "sx/files/共同レビュー.html");
    return { presignedUrl: "https://blob.example/deck.pdf?signed=1" };
  };
  const { url, expiresAt } = await createSignedAccessUrl("sx/files/共同レビュー.html", {
    issueSignedTokenFn,
    presignUrlFn,
  });
...
test("createSignedAccessUrl returns the presigned URL unchanged when download is not requested", async () => {
  ...
  const { url } = await createSignedAccessUrl("sx/files/deck.pdf", {
    issueSignedTokenFn,
    presignUrlFn,
    getDownloadUrlFn,
  });
