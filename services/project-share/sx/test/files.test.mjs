...
test("PATCH /api/files requires authentication", async () => {
  const req = makeReq({
    method: "PATCH",
    url: "/api/files",
    body: { pathname: "sx/files/deck.pdf", targetFolder: "reports" },
  });
  const res = makeRes();
...
test("PATCH /api/files rejects a target folder with path traversal", async () => {
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://sx.example.com", "content-type": "application/json" },
    body: { pathname: "sx/files/deck.pdf", targetFolder: "../etc" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, { isAuthed: true });
  assert.equal(res.statusCode, 400);
...
test("PATCH /api/files moves the file and returns the new pathname on success", async () => {
  ...
  await handleFilesRoute(req, res, {
    ...
    moveFileFn: async (pathname, targetFolder) => {
      ...
      return { pathname: "sx/files/reports/deck.pdf" };
    },
  });
  assert.equal(res.statusCode, 200);
  assert.deepEqual(capturedArgs, { pathname: "sx/files/deck.pdf", targetFolder: "reports" });
  const body = JSON.parse(res.body);
...
test("PATCH /api/files returns 400 when the destination is the same location as the source", async () => {
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://sx.example.com", "content-type": "application/json" },
    body: { pathname: "sx/files/reports/deck.pdf", targetFolder: "reports" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, {
    isAuthed: true,
