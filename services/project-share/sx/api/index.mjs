...
export default async function handler(req, res, { hasMemberFn } = {}) {
  const password = process.env.SX_ACCESS_PASSWORD;
  const secret = process.env.SX_AUTH_SECRET;
  const adminPassword = process.env.SX_ADMIN_PASSWORD;
  const initialEmails = parseAllowedEmails(process.env.SX_ALLOWED_EMAILS);
