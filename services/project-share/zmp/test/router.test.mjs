...
function withEnv(fn, { allowedEmails = EMAIL } = {}) {
  return async () => {
    const prevPassword = process.env.ZMP_ACCESS_PASSWORD;
    const prevSecret = process.env.ZMP_AUTH_SECRET;
    const prevAllowedEmails = process.env.ZMP_ALLOWED_EMAILS;
    const prevAdminPassword = process.env.ZMP_ADMIN_PASSWORD;
    process.env.ZMP_ACCESS_PASSWORD = PASSWORD;
    process.env.ZMP_AUTH_SECRET = SECRET;
    process.env.ZMP_ADMIN_PASSWORD = ADMIN_PASSWORD;
    if (allowedEmails === null) {
      delete process.env.ZMP_ALLOWED_EMAILS;
    } else {
      process.env.ZMP_ALLOWED_EMAILS = allowedEmails;
    }
    try {
      await fn();
    } finally {
      process.env.ZMP_ACCESS_PASSWORD = prevPassword;
      process.env.ZMP_AUTH_SECRET = prevSecret;
      if (prevAdminPassword === undefined) {
        delete process.env.ZMP_ADMIN_PASSWORD;
      } else {
        process.env.ZMP_ADMIN_PASSWORD = prevAdminPassword;
      }
      if (prevAllowedEmails === undefined) {
        delete process.env.ZMP_ALLOWED_EMAILS;
      } else {
        process.env.ZMP_ALLOWED_EMAILS = prevAllowedEmails;
      }
    }
  };
}
...
test(
  ...
  async () => {
    delete process.env.ZMP_ACCESS_PASSWORD;
    delete process.env.ZMP_AUTH_SECRET;
    delete process.env.ZMP_ALLOWED_EMAILS;
    delete process.env.ZMP_ADMIN_PASSWORD;
  ...
  withEnv(async () => {
    const req = makeReq({
      ...
      headers: {
        ...
        origin: "https://zmp.example.com",
        host: "zmp.example.com",
      },
...
test(
  ...
  withEnv(async () => {
    ...
    assert.match(res.headers["Set-Cookie"], /zmp_auth=/);
...
test(
  ...
  withEnv(
    async () => {
      ...
      process.env.ZMP_ALLOWED_EMAILS = "someone-else@example.com";
...
test(
  ...
  withEnv(async () => {
    const cookie = authCookieHeader();
    process.env.ZMP_ACCESS_PASSWORD = "a brand new password";
    const req = makeReq({ method: "GET", url: "/", headers: { cookie } });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "GET",
      url: "/",
      headers: { cookie: "zmp_auth=not-a-valid-token" },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      ...
      body: { pathname: "zmp/files/deck.pdf" },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "POST",
      url: "/api/logout",
      headers: { origin: "https://zmp.example.com", cookie: authCookieHeader() },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
