...
export default async function handler(req, res, { hasMemberFn } = {}) {
  const password = process.env.KUTE_ACCESS_PASSWORD;
  const secret = process.env.KUTE_AUTH_SECRET;
  const adminPassword = process.env.KUTE_ADMIN_PASSWORD;
  const initialEmails = parseAllowedEmails(process.env.KUTE_ALLOWED_EMAILS);
