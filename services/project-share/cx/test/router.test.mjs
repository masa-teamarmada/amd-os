...
function withEnv(fn, { allowedEmails = EMAIL } = {}) {
  return async () => {
    ...
    if (allowedEmails === null) {
      ...
      process.env.CX_ALLOWED_EMAILS = allowedEmails;
    }
    try {
      await fn();
    } finally {
      process.env.CX_ACCESS_PASSWORD = prevPassword;
      process.env.CX_AUTH_SECRET = prevSecret;
...
test(
  ...
  async () => {
    delete process.env.CX_ACCESS_PASSWORD;
    delete process.env.CX_AUTH_SECRET;
    delete process.env.CX_ALLOWED_EMAILS;
...
test(
  ...
  withEnv(
    async () => {
      ...
      process.env.CX_ALLOWED_EMAILS = "someone-else@example.com";
...
test(
  ...
  withEnv(async () => {
    const cookie = authCookieHeader();
    process.env.CX_ACCESS_PASSWORD = "a brand new password";
    const req = makeReq({ method: "GET", url: "/", headers: { cookie } });
    const res = makeRes();
    await handler(req, res);
    assert.equal(res.statusCode, 200);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "GET",
      url: "/",
      headers: { cookie: "cx_auth=not-a-valid-token" },
    });
    const res = makeRes();
    await handler(req, res);
    assert.equal(res.statusCode, 200);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      ...
      body: { pathname: "cx/files/deck.pdf" },
    });
    const res = makeRes();
    await handler(req, res);
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "POST",
      url: "/api/logout",
      headers: { origin: "https://cx.example.com", cookie: authCookieHeader() },
    });
    const res = makeRes();
    await handler(req, res);
    assert.equal(res.statusCode, 200);
