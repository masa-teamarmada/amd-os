...
test("FILES_PREFIX is the expected blob namespace", () => {
  assert.equal(FILES_PREFIX, "se/files/");
});
...
test("buildFilePathname prefixes a sanitized basename", () => {
  assert.equal(buildFilePathname("deck.pdf"), "se/files/deck.pdf");
});
...
test("isWithinFilesPrefix accepts single-segment paths under the prefix", () => {
  assert.equal(isWithinFilesPrefix("se/files/deck.pdf"), true);
});

test("isWithinFilesPrefix rejects paths outside the prefix", () => {
  assert.equal(isWithinFilesPrefix("other/deck.pdf"), false);
  assert.equal(isWithinFilesPrefix("se/files/"), false);
});

test("isWithinFilesPrefix accepts nested user file paths", () => {
  assert.equal(isWithinFilesPrefix("se/files/sub/deck.pdf"), true);
  assert.equal(isWithinFilesPrefix("se/files/a/b/c/deck.pdf"), true);
});

test("isWithinFilesPrefix rejects traversal", () => {
  assert.equal(isWithinFilesPrefix("se/files/../secret.pdf"), false);
  assert.equal(isWithinFilesPrefix("se/files/sub/../secret.pdf"), false);
});

test("isWithinFilesPrefix rejects any .folder segment", () => {
  assert.equal(isWithinFilesPrefix(`se/files/${FOLDER_MARKER}`), false);
  assert.equal(isWithinFilesPrefix(`se/files/sub/${FOLDER_MARKER}`), false);
});

test("isWithinFilesPrefix rejects empty segments from doubled slashes", () => {
  assert.equal(isWithinFilesPrefix("se/files/sub//deck.pdf"), false);
});

test("isWithinFilesPrefix rejects unsanitized basenames", () => {
  assert.equal(isWithinFilesPrefix("se/files/deck.pdf "), false);
  assert.equal(isWithinFilesPrefix("se/files/ deck.pdf"), false);
  assert.equal(isWithinFilesPrefix("se/files/deck\n.pdf"), false);
  assert.equal(isWithinFilesPrefix("se/files/.."), false);
});
...
test("buildFilePathname builds a nested path from folder and name", () => {
  assert.equal(buildFilePathname("a/b", "deck.pdf"), "se/files/a/b/deck.pdf");
  assert.equal(buildFilePathname("", "deck.pdf"), "se/files/deck.pdf");
});
...
test("buildFolderMarkerPath builds a marker path for a nested folder", () => {
  assert.equal(buildFolderMarkerPath("a/b"), `se/files/a/b/${FOLDER_MARKER}`);
});
...
test("buildFolderMarkerPath still builds the marker path for a normal nested folder (not broken by reserved-name check)", () => {
  assert.equal(buildFolderMarkerPath("reports"), `se/files/reports/${FOLDER_MARKER}`);
});
