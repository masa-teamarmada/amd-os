...
function withEnv(fn, { allowedEmails = EMAIL } = {}) {
  return async () => {
    ...
    const prevAdminPassword = process.env.SE_ADMIN_PASSWORD;
    process.env.SE_ACCESS_PASSWORD = PASSWORD;
    process.env.SE_AUTH_SECRET = SECRET;
    process.env.SE_ADMIN_PASSWORD = ADMIN_PASSWORD;
    ...
    try {
      ...
      if (prevAdminPassword === undefined) {
        delete process.env.SE_ADMIN_PASSWORD;
      } else {
        process.env.SE_ADMIN_PASSWORD = prevAdminPassword;
      }
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      ...
      body: { pathname: "se/files/deck.pdf" },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
...
test(
  ...
  withEnv(async () => {
    const req = makeReq({
      method: "POST",
      url: "/api/logout",
      headers: { origin: "https://se.example.com", cookie: authCookieHeader() },
    });
    const res = makeRes();
    await handler(req, res, { hasMemberFn: async () => false });
    assert.equal(res.statusCode, 200);
