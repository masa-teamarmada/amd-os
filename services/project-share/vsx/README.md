...
### ドラッグ中の行プレビューを実際の行へ統一（2026-07-31）

- ファイル移動中にポインタへ追従していた黒いファイル名札を廃止し、対象ファイル行を複製して
  半透明のまま追従させる表示へ変更した。名前・形式・サイズ・更新日・操作欄を含む行の列幅を保つため、
  元行の実測幅を複製側へ反映する。
- ポインタを行のどこでつかんだかを記録し、その相対位置を保って移動する。元の行は薄く残し、
  移動先フォルダの青い受け皿表示、行ダブルクリックで開く操作、Finder / エクスプローラーからの
  ファイルアップロードは変更していない。
- VSX / CX / SE / SX / ZMP / KUTEの各`npm run build`・`npm run check`・`npm test`は成功。
  ZMP本番で、行クローン・半透明表示・黒い札の不在・つかんだ位置に追従する計算式を確認し、
