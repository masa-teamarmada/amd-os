  ...
  await handleFilesRoute(req, res, {
    ...
    listDirectoryFn: async (folder) => {
      ...
      return {
        ...
        files: [{ pathname: "vsx/files/meeting/deck.pdf", name: "deck.pdf", size: 12, uploadedAt: "2026-07-31T00:00:00.000Z" }],
      };
    },
...
test("PATCH /api/files requires authentication", async () => {
  const req = makeReq({
    method: "PATCH",
    url: "/api/files",
    body: { pathname: "vsx/files/deck.pdf", targetFolder: "reports" },
  });
  const res = makeRes();
...
test("PATCH /api/files rejects an invalid JSON body", async () => {
  const req = makeReq({
    method: "PATCH",
    url: "/api/files",
    headers: { origin: "https://vsx.example.com", "content-type": "application/json" },
    body: "{not json",
  });
  const res = makeRes();
  await handleFilesRoute(req, res, { isAuthed: true });
...
test("PATCH /api/files rejects a target folder with path traversal", async () => {
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://vsx.example.com", "content-type": "application/json" },
    body: { pathname: "vsx/files/deck.pdf", targetFolder: "../etc" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, { isAuthed: true });
  assert.equal(res.statusCode, 400);
...
test("PATCH /api/files moves the file and returns the new pathname on success", async () => {
  ...
  await handleFilesRoute(req, res, {
    ...
    moveFileFn: async (pathname, targetFolder) => {
      ...
      return { pathname: "vsx/files/reports/deck.pdf" };
    },
  });
  assert.equal(res.statusCode, 200);
  assert.deepEqual(capturedArgs, { pathname: "vsx/files/deck.pdf", targetFolder: "reports" });
  const body = JSON.parse(res.body);
  assert.equal(body.ok, true);
  assert.equal(body.pathname, "vsx/files/reports/deck.pdf");
...
test("PATCH /api/files renames the file and returns the new pathname on success", async () => {
  ...
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://vsx.example.com", "content-type": "application/json" },
    body: { pathname: "vsx/files/deck.pdf", newName: "deck-final.pdf" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, {
    isAuthed: true,
    renameFileFn: async (pathname, newName) => {
      ...
      return { pathname: "vsx/files/deck-final.pdf" };
    },
  });
  assert.equal(res.statusCode, 200);
  assert.deepEqual(capturedArgs, { pathname: "vsx/files/deck.pdf", newName: "deck-final.pdf" });
  const body = JSON.parse(res.body);
  assert.equal(body.ok, true);
  assert.equal(body.pathname, "vsx/files/deck-final.pdf");
});
...
test("PATCH /api/files rejects an invalid rename name", async () => {
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://vsx.example.com", "content-type": "application/json" },
    body: { pathname: "vsx/files/deck.pdf", newName: "../etc" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, { isAuthed: true });
  assert.equal(res.statusCode, 400);
...
test("PATCH /api/files returns 409 when the rename destination exists", async () => {
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://vsx.example.com", "content-type": "application/json" },
    body: { pathname: "vsx/files/deck.pdf", newName: "renamed.pdf" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, {
    isAuthed: true,
...
test("PATCH /api/files returns 400 when the destination is the same location as the source", async () => {
  const req = makeReq({
    ...
    url: "/api/files",
    headers: { origin: "https://vsx.example.com", "content-type": "application/json" },
    body: { pathname: "vsx/files/reports/deck.pdf", targetFolder: "reports" },
  });
  const res = makeRes();
  await handleFilesRoute(req, res, {
    isAuthed: true,
