...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  tbody tr.pinned td { background: var(--pale); font-weight: 700; }
  ...
  @media (max-width: 720px) {
    ...
    tbody tr.pinned { background: var(--pale); }
...
<body>
      ...
      <table aria-describedby="status">
        ...
        <tbody id="file-rows">
          <tr class="pinned" data-pinned="true" data-row-type="pinned" tabindex="0" data-testid="pinned-row">
            <td class="name-cell"><span class="name-content">AgVenture Lab 事業資料</span></td>
            <td class="col-type" data-label="形式">HTML資料</td>
            <td class="col-size" data-label="サイズ">—</td>
            <td class="col-date" data-label="更新日">—</td>
  ...
  <script type="module">
    ...
    function render() {
      ...
      rowsEl.innerHTML = "";
      if (pinnedRow) rowsEl.appendChild(pinnedRow);

      const folderList = sortedFilteredFolders();
      const fileList = sortedFilteredFiles();

      if (folderList.length === 0 && fileList.length === 0 && !pinnedRow) {
    ...
    function activateRow(tr) {
      ...
      if (type === "pinned") {
        window.open("/documents/agventure-lab", "_blank", "noopener");
      } else if (type === "folder") {
    ...
    async function uploadFiles(fileList) {
      ...
      for (const file of list) {
        ...
        setProgress(0);
        const prefix = currentFolder ? "vsx/files/" + currentFolder + "/" : "vsx/files/";
        try {
