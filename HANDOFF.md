...
| 項目 | 値 |
|---|---|
| **凍結コミット** | `34aaa284`（main。2026-08-29 push・本番 v3.99.x デプロイ済み） |
...
| **スコア一覧** | [model/cases/SCORES.md](model/cases/SCORES.md)（34aaa284 の版）。上位: OptQC 489.7億 / マテリアル・コンセプト 76.9億 / CryoX 62.2億 / LiSTie 53.3億 / KENQ 45.8億 |
...
1. **CrestecBio の月次損益の実測を取る** — バーン推定（月1,500万の置き・確度低）で 18億→8,662万 まで
   動いた。感度が21件で最大。実測が入れば最優先で差し替えて再計算
...
5. **ティエム 9.8億が BWE（3億）・CCC（9.1億）を上回った件** — 実測バーン月300万（既定450万より小）で
   延命が長く読まれた。支出を絞ったゾンビの残存価値をどこまで認めるか。提案5 が入ると絞られる方向
   （SCORES.md「気をつけて読むところ」1）
