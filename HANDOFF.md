...
| 項目 | 値 |
|---|---|
| **凍結コミット** | **`b8f4d6f6`**（main。2026-08-29 push）。経緯: `34aaa284`（改訂4点）→ 入力の置き方の修正2巡（`8a054516`・`91385d77`）→ **#2026-08-29-3（困窮時の出口の実態化）を含む再計算で `b8f4d6f6`**。§6.I-11 の検算表（Tier 0 の縮退・収束）は取り直し中で、完了後の追いコミットで最終化する——**21件のスコアの数値は b8f4d6f6 で確定**（検算表はスコアに影響しない） |
...
| **スコア一覧** | [model/cases/SCORES.md](model/cases/SCORES.md)（b8f4d6f6 の版）。上位: OptQC 167.7億 / LiSTie 161.5億 / CryoX 128.4億 / マテリアル・コンセプト 71.9億 / KENQ 43.5億。M&A の裾で持っていた案件が下がった（OptQC 490→168億・CCC 9.1→3.8億・チャレナジー 4,034万→601万）。序列: ティエム 1.2億 ＜ BWE 6.3億。**BWE 6.3億 ＞ CCC 3.8億 の入れ替わりは要確認**（残高の差が主因。CCC の残高1,600万は逆算推定・確度低） |
...
1. **LiSTie の自由資金の持ち出しの実額** — 暫定月500万（確度低）のまま2位。300万なら残り月数1.7倍、
   800万なら0.6倍の感度。SBIR約16億・NEDO数億の採択の交付期間・残額もあわせて確認できるとよい
