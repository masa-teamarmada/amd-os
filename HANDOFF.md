...
| 項目 | 値 |
|---|---|
| **凍結コミット** | **`91385d77`**（main。2026-08-29 push）。当初 `34aaa284` → まさの指摘で入力の置き方を2巡直した: (1) CrestecBio・LiSTie・CryoX のバーンの境界（残高と支出は同じ側で揃える）とティエムの天井（担える範囲 13.3億へ再絞り）→ `8a054516`、(2) CryoX のバーン誤読の訂正（AMDへの業務委託料を案件支出と取り違え）と BWE の調達1.5億円の反映 → `91385d77`。規約は `model/cases/README.md` |
| **承認ID** | [APPROVALS](model/APPROVALS.md) **#2026-08-29-1**（会社化前の速度規則〈「休眠」という状態は廃止〉・変換能力 $c$〈費用→戦略余力の増分の比率の移動平均〉・無風期間 $t_q$〈12か月0.5・24か月0.1・36か月以上0.05〉）と **#2026-08-29-2**（八機能の充足 $f_1$〜$f_7$〈機能2は失われた観測がある案件だけ空席指定・恒久喪失率を外す〉・案件ごとのバーンレート〈評価日の会社化状態の側の $\mu$ を差し替え〉） |
| **再計算** | 21件全件を 2026-08-29 に完了。`seed_bzm30_scores` の最新行は `approval_ref='2026-08-29-2'` |
| **スコア一覧** | [model/cases/SCORES.md](model/cases/SCORES.md)（91385d77 の版）。上位: OptQC 489.7億 / LiSTie 161.6億 / CryoX 128億 / マテリアル・コンセプト 76.9億 / KENQ 45.8億。ティエム 1.4億 ＜ BWE 7億 ＜ CCC 9.1億 の序列はまさの実感と整合確認済み |
