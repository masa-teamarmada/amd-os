...
| 項目 | 値 |
|---|---|
| **凍結コミット** | **`8a054516`**（main。2026-08-29 push）。当初 `34aaa284` で凍結した直後、まさの指摘で入力の置き方4件——CrestecBio・LiSTie・CryoX のバーンの境界（残高と支出は同じ側で揃える）、ティエムの天井（担える範囲 13.3億へ再絞り）——を直して再計算したため、凍結を `8a054516` へ更新。規約は `model/cases/README.md` |
...
| **スコア一覧** | [model/cases/SCORES.md](model/cases/SCORES.md)（8a054516 の版）。上位: OptQC 489.7億 / LiSTie 161.6億 / CryoX 127.8億 / マテリアル・コンセプト 76.9億 / KENQ 45.8億。ティエム 1.4億 ＜ BWE 3億 ＜ CCC 9.1億 の序列はまさの実感と整合確認済み |
...
2. **変換能力 $c$ と無風期間 $t_q$**（#2026-08-29-1）。21件全件に根拠つきで置いた。ティエムは天井を
   SAM 本来の66.5億へ戻し、伸びの停止を到達確率の側で表す形に
3. **八機能の充足**（#2026-08-29-2）。ORLIB は f2=0＋証拠水準を段階3へ戻す（134万→3億）、SE は f4=0.3。
   第一便は明確な観測（《組織》`project_org_observations`）がある案件・機能だけ指定
4. **案件ごとのバーンレート**（#2026-08-29-2）。21件全件で実測・逆算が計算に入った。
   CrestecBio 18億→8,662万・LiSTie 162→53億・輝翠 21→5億。逆に支出を絞った延命（ティエム・OPTMASS）は上がった
...
2. **BWE の初回調達の確認** — まさ 2026-08-29「BWEは初回の資金調達を終えて、これからいよいよ
   オンサイトPoC」。OS の記録は「プレシード5,000万は想定で実行の記録なし・残高1,000万と置いた」のままで
   食い違う。実行済みなら額と時期を聞いて残高を置き直す（証拠水準も「これからオンサイトPoC」との整合を確認）
