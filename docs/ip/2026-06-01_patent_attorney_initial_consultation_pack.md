...
- 現在の内部版出願書類たたき台を、国内出願可能な明細書・請求項・図面形式へ整えること
...
- `docs/ip/2026-06-01_inventorship_ownership_internal.md`
...
- 内部版の出願書類たたき台を作成済み。
- 明細書主要章、請求項1〜16、要約、Fig.1〜Fig.8の図面案、抽象テーブル例、疑似処理フロー、安全な合成実施例を作成済み。
- 請求項対応表で、請求項1〜16はいずれも明細書サポート、図面案、抽象テーブル、疑似フローの対応が確認済み。
- 現時点の論点は「support不足」ではなく、請求項の広狭、分割方針、図面清書、装置/プログラム請求項の形式整備、営業秘密の出しすぎ防止。
...
1. 請求項1は、protocol生成まで含めるべきか、HITL + feedbackまでを主請求項にしてprotocol生成を従属項へ下げるべきか。
2. 請求項5のfeedbackは、prompt注入に寄せるべきか、抽出条件、判定ルール、設定値、ワークフロー定義への抽象反映として広く書くべきか。
3. 請求項10の異種evidence統合参照は、FHIR/OMOP等との差別化として十分か。`outcome_evidence_ref` / `evidence_refs` の抽象構造は明細書サポートとして足りるか。
4. 請求項11〜12のsystem parameter governanceは、基幹出願に含めるべきか、分割候補にするべきか。
5. 請求項13のBefore-Zero / 設立タイミング推奨は従属項・補強項に留めるべきか、独立請求項としても残す価値があるか。
6. 請求項14の統合確認UIは、侵害検出に効く範囲とUI限定で狭くなるリスクのバランスが妥当か。
7. prompt、score重み、calibration、実PJ事例を営業秘密として外したまま、実施可能要件・サポート要件を満たせるか。
8. 発明者=まさ、出願人=AMDで進める際、まさからAMDへの特許を受ける権利の承継書面等が必要か。
9. 15〜30万円、30〜50万円、100万円超で、それぞれ何をどこまで依頼できるか。
10. 先に国内出願し、1年以内にPCT判断する方針でよいか。
11. 出願前に追加で最低限やるべき正式サーチは何か。Seek AI、BigID、Ciena、Optum、国内J-PlatPatのどこが必須か。
12. 外部営業資料やNIMS説明で「出願中」と言う場合、どの表現なら安全か。
...
- [ ] 内部版出願書類たたき台を、外部相談用に削った版へ変換したか
- [ ] 弁理士へ送る資料から営業秘密を外したか
- [ ] 弁理士へ送らず、画面共有/口頭説明に留める情報を分けたか
- [ ] Fig.1〜Fig.8の図面案を、特許図面として清書する必要があるか確認したか
