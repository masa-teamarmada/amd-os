/** 172_TsukuyomiContextRepo
 * 172_TsukuyomiContextRepo / つくよみ人格DB（DB_TsukuyomiContext）を読む・書く運用API
...
function getTsukuyomiContextSheet(){
  ...
  const name = "DB_TsukuyomiContext";
...
function tsukuyomi_getPersonaFromContextDb(payload){
  ...
  const sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("DB_TsukuyomiContext");
...
function tsukuyomi_buildSystemPromptBaseFromContextDb(payload){
  ...
  function layerOf(ct){
    ...
    // 互換：空や未知は「人格/口調」側に寄せる（判断層に紛れ込ませない）
    return "tone";
  }

  const layerOrder = { judge: 10, role: 20, memory: 30, tone: 40, safety: 90 };

  // ソート：層順 → priority昇順（小さいほど強い）→ createdAt昇順
  rows.sort((a,b) => {
    const la = layerOrder[layerOf(a.contextType)];
    const lb = layerOrder[layerOf(b.contextType)];
  ...
  // ===== 4) 合成（層のまとまりごとに区切る） =====
  // LLMに「これは判断OS」「これは口調」みたいな境界を渡すための軽い見出しを入れる
  const parts = [];
  ...
  // 4層（＋安全）
  pushLayer("JUDGE_PROTOCOL｜判断プロトコル", "judge");
  pushLayer("ROLE_SCOPE｜役割スコープ", "role");
  pushLayer("MEMORY_POLICY｜記憶の使い方", "memory");
  pushLayer("PERSONA_TONE｜口調・演出", "tone");
  pushLayer("SAFETY_GUARD｜事故防止", "safety");
