...
- 設計はまさ確定（骨格、予備50%、余りはシーズン合算で獲得pt比、検収実績への切り替え、Slack配信を既定、支払済み残作業は pt 0、SXの到達点直下は5本、CEO業務は定常継続、pt配分の暫定値）。正本は 3-22。**実装は未着手。**
...
- **日程未設定行**: 日程が無いTODOをまとめて出し、ここで担当（複数可）・期限・見積ptをその場で入れられる。会議後のアサイン作業面。新しい一覧画面は作らない。
...
SXとZMPの論点・仮説タブで、根に到達点、直下にMS、その下に既存の論点・TODOが1本の木で見え、担当か期限が空のTODOに印が出て、ガントの左列がその木で、日程未設定行から担当・期限・見積ptを入れられる。本番反映済み。3-21 / 2-9 改訂済み。
