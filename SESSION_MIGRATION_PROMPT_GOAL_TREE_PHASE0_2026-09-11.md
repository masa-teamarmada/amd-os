...
- 設計はまさ確定（骨格、予備50%、余りはシーズン合算で獲得pt比、検収実績への切り替え、Slack配信を既定、支払済み残作業は pt 0、SXの到達点直下は5本、CEO業務は定常継続、pt配分の暫定値）。正本は 3-22。**アサインとptの割り振りはゴールツリー上で行う（まさ確定。ガントは日程だけ）。** 実装は未着手。
...
- **アサインの入口はここ**（まさ確定 2026-09-11「ptの割り振りはMSからでなくゴールツリーからやりたい」）。TODOの詳細モーダルに見積pt欄と担当の複数化を足し、担当・期限・見積ptをツリーの中で付ける。ヘッダーに「未アサイン n件」を出し、押すと最初の該当行へ移る。MSリストやガントからは配らない。
...
SXとZMPの論点・仮説タブで、根に到達点、直下にMS、その下に既存の論点・TODOが1本の木で見え、担当か期限が空のTODOに印が出て、ツリーのTODO詳細から担当・期限・見積ptを入れられ、ガントの左列がその木で日程を付けられる。本番反映済み。3-21 / 2-9 改訂済み。
