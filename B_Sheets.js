...
function getBillingSpreadsheetId(){
  ...
  try{
    const p = String(PropertiesService.getScriptProperties().getProperty("BILLING_SPREADSHEET_ID") || "").trim();
    if (p) return p;
  } catch(e){}
  ...
  throw new Error("BILLING_SPREADSHEET_ID missing. Set ScriptProperties BILLING_SPREADSHEET_ID or BILLING_CFG.SPREADSHEET_ID.");
}
