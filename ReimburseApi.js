...
function reimburse_listMyRecentExpenses(payload){
  ...
  function isAdminLocal(){
    var ok = false;
    try{ if (typeof canAccessAdmin === "function") ok = !!canAccessAdmin(); } catch(e){}
    if (!ok){
      try{ if (typeof canAccessAdminForCodeSearch === "function") ok = !!canAccessAdminForCodeSearch(); } catch(e2){}
    }
    return ok;
  }
...
function admin_reimburseMigrateHeadersToCanonical(){
  // adminだけ実行可
  var isAdmin = false;
  try{ if (typeof canAccessAdmin === "function") isAdmin = !!canAccessAdmin(); } catch(e){}
  if (!isAdmin){
    try{ if (typeof canAccessAdminForCodeSearch === "function") isAdmin = !!canAccessAdminForCodeSearch(); } catch(e2){}
  }
