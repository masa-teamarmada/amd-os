...
function loadCharter_(projectId, projectName, opt) {
  ...
  // ===== 1) DB_ProjectCharter：projectId の行だけ拾う =====
  let memo = "";
  ...
  // ===== 2) DB_ProjectMembers：projectIdだけ拾う =====
  ...
  // ===== 3) DB_Projects：projectId行だけ拾う（TextFinder版） =====
  const pj = (function(){
    const out = {
      clientName: "",
      freeePartnerId: "",
    ...
    try{
      ...
      const iFreeePname = header.indexOf("freeePartnerName");
      ...
      out.freeePartnerName = (iFreeePname>=0) ? String(row[iFreeePname]||"").trim() : "";
  ...
  const result = {
    ...
    // ★追加：DB_Projects由来の取引先情報
    clientName: pj.clientName || "",
    freeePartnerId: pj.freeePartnerId || "",
    freeePartnerName: pj.freeePartnerName || "",
