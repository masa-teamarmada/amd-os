...
// ================================================================
// 月次ルーティン Blocks 組み立て（つくよみが投稿する用）
// ================================================================

/**
 * BillingCycleの状態を見て、次にやるべきアクションのメッセージ＋ボタンを組み立てる
 * 返却: { text, blocks } → slackNotifyPostToChannelTsukuyomi_ にそのまま渡せる
 */
function slackNotifyBuildBillingRoutineMessage_(projectId, ym){
  ...
  var cur = {};
  try{ cur = b_getCycleOne_(projectId, ym) || {}; } catch(e){ cur = {}; }

  var projectName = String(cur.projectName || "").trim() || projectId;
  var ymLabel = String(ym).slice(0,4) + "年" + String(ym).slice(4,6).replace(/^0/,"") + "月";

  // 状態判定
  var hasBudget = !!cur.budgetConfirmedAt;
  var hasInvoice = !!cur.freeeInvoiceId;
  var hasSent = !!cur.invoiceSentAt;
  var hasPayment = !!cur.paymentConfirmedAt;

  // 完了済み
  if (hasPayment){
    return {
      text: "✅ " + projectName + "の" + ymLabel + "度ルーティンは完了してるよ",
      blocks: [
        { type:"section", text:{ type:"mrkdwn", text: "✅ *" + projectName + "* の" + ymLabel + "度ルーティンは全部完了してるよ" } }
      ]
    };
  }

  // ===== 入金待ち =====
  if (hasSent && !hasPayment){
    return {
      text: projectName + " " + ymLabel + "度：請求書送付済み。入金待ち",
      blocks: [
        { type:"section", text:{ type:"mrkdwn", text: "💰 *" + projectName + "* " + ymLabel + "度\n請求書は送付済み。入金確認待ちだよ" } },
        { type:"context", elements:[
          { type:"mrkdwn", text: "入金確認はきよさんの末日チェックで自動通知されるから、待ってて大丈夫" }
        ]}
      ]
    };
  }
  ...
  if (hasInvoice && !hasSent){
    ...
    return {
      text: projectName + " " + ymLabel + "度：請求書発行済み。送付がまだ",
      blocks: [
        { type:"section", text:{ type:"mrkdwn", text:
          "📨 *" + projectName + "* " + ymLabel + "度\n"
          + "請求書番号: " + String(cur.freeeInvoiceNumber || "（取得中）") + "\n"
          + "請求書は発行済みだけど、まだ送付してないよ。freeeから送付してね"
        }},
        { type:"context", elements:[
          { type:"mrkdwn", text: "送付が完了したら、OSが自動でステータスを更新するよ" }
        ]}
      ]
    };
  }
  ...
  if (hasBudget && !hasInvoice){
    ...
    try{
      ...
      reimbPack = { ok:true, items:[], totalYen:0 };
    }

    var items = (reimbPack && Array.isArray(reimbPack.items)) ? reimbPack.items : [];
    var reimbTotal = Number(reimbPack && reimbPack.totalYen ? reimbPack.totalYen : 0);

    var budget65 = Number(cur.budgetTotal || cur.budgetYen || 0);
    var gross = (budget65 > 0) ? Math.round(budget65 / 0.65) : 0;

    // ── 基本行プレビュー ──
    var baseLines = [];
    try{
      var blRaw = String(cur.invoiceBaseLinesJson || "").trim();
      if (blRaw){
    ...
    if (baseLines.length > 0){
      baseLines.forEach(function(ln){
        var desc = String(ln.description || ln.desc || ln.name || "").trim();
        var qty = Number(ln.quantity || ln.qty || 1);
        var unit = Number(ln.unitPrice || ln.unit_price || ln.amount || 0);
        var lineTotal = qty * unit;
        baseLinesTotal += lineTotal;
        baseLinesText += "\n　　" + desc;
        if (qty !== 1) baseLinesText += " ×" + qty;
        baseLinesText += " ¥" + lineTotal.toLocaleString();
      });
    }

    // ── 請求書件名 ──
    var subject = String(cur.invoiceSubject || "").trim();

    // ── メッセージ組み立て ──
    var bodyText = "📄 *" + projectName + "* " + ymLabel + "度 請求書プレビュー\n";
    ...
    bodyText += "\n*【基本行】*";
    if (baseLines.length > 0){
      bodyText += baseLinesText;
      bodyText += "\n　　─────────";
      bodyText += "\n　　小計 ¥" + baseLinesTotal.toLocaleString();
    } else {
      bodyText += "\n　　（未設定 — 初回は編集ボタンから設定してね）";
    }
    ...
    bodyText += "\n\n*【立替精算】*";
    if (items.length > 0){
      bodyText += " " + items.length + "件 合計 ¥" + reimbTotal.toLocaleString();
      items.forEach(function(it){
        var d = String(it.date || "").trim().replace(/^\d{4}-/, "").replace(/-/g, "/");
        var cat = String(it.categoryLabel || it.category || "").trim();
        var amt = Number(it.amount || 0);
        var desc = String(it.desc || "").trim();
        bodyText += "\n　　" + d + " " + cat + " ¥" + amt.toLocaleString() + " " + desc;
      });
    } else {
      bodyText += " なし";
    }
    ...
    // 請求日・支払期日
    var billDate = String(cur.invoiceBillingDate || "").trim();
    var payDate = String(cur.invoicePaymentDate || "").trim();
    if (billDate || payDate){
      bodyText += "\n";
      if (billDate) bodyText += "請求日: " + billDate + "　";
      if (payDate) bodyText += "支払期日: " + payDate;
    }

    bodyText += "\n\nこの内容で請求書を発行していい？";

    // ── ボタン ──
    var baseUrl2 = String(PropertiesService.getScriptProperties().getProperty("WEBAPP_BASE_URL") || "").trim();
    var reimbUrl2 = baseUrl2 ? (baseUrl2 + "?page=reimburse") : "";
    var billingEditUrl = baseUrl2 ? (baseUrl2 + "?page=admin&tab=billing&edit=" + projectId + "&ym=" + ym) : "";
    ...
    var buttons = [
      {
        ...
        style:"primary",
        action_id:"invoice_issue",
        value: JSON.stringify({ projectId: projectId, ym: ym })
      },
      {
        type:"button",
        text:{ type:"plain_text", text:"基本行を編集" },
        url: billingEditUrl || undefined,
        action_id:"invoice_edit_lines"
      },
      {
        type:"button",
        text:{ type:"plain_text", text:"立替を再集計" },
        action_id:"reimb_refresh",
        value: JSON.stringify({ projectId: projectId, ym: ym })
      }
    ];

    // url が空のボタンは除外（Slackがエラーになる）
    buttons = buttons.filter(function(btn){ return btn.action_id !== "invoice_edit_lines" || btn.url; });

    if (reimbUrl2){
      buttons.push({
        type:"button",
        text:{ type:"plain_text", text:"立替ページで修正" },
        url: reimbUrl2,
        action_id:"reimb_open_page"
      });
    }

    return {
      text: projectName + " " + ymLabel + "度：請求書プレビュー",
      blocks: [
  ...
  // ===== 予算未確定 =====
  if (!hasBudget){
    var clientGrossYen = 0;
    try{
      // 前月のBillingCycleから請求額を推定
      var prevYm = b_addMonths_(ym, -1);
      var prevCycle = b_getCycleOne_(projectId, prevYm) || {};
      var prevBudget = Number(prevCycle.budgetTotal || prevCycle.budgetYen || 0);
      if (prevBudget > 0){
        clientGrossYen = Math.round(prevBudget / 0.65);
      }
    } catch(e){}

    var budgetText = "💰 *" + projectName + "* " + ymLabel + "度\n"
      + "まだ予算が確定してないよ。";

    if (clientGrossYen > 0){
      var est65 = Math.round(clientGrossYen * 0.65);
      budgetText += "\n前月ベース: 請求額 ¥" + clientGrossYen.toLocaleString()
        + " → PJ予算 ¥" + est65.toLocaleString();
      budgetText += "\n\nこの金額で確定していい？";

      return {
        text: projectName + " " + ymLabel + "度：予算を確定していい？",
        blocks: [
          { type:"section", text:{ type:"mrkdwn", text: budgetText } },
          { type:"actions", elements:[
            {
              type:"button",
              text:{ type:"plain_text", text:"この金額で確定する" },
              style:"primary",
              action_id:"budget_confirm",
              value: JSON.stringify({ projectId: projectId, ym: ym, clientGrossYen: clientGrossYen })
            }
          ]}
        ]
      };
    ...
    budgetText += "\n金額をコックピットかSlackで教えてね。";

    return {
      text: projectName + " " + ymLabel + "度：予算を確定してね",
      blocks: [
        { type:"section", text:{ type:"mrkdwn", text: budgetText } }
      ]
    };
...
/**
 * 指定PJ×ymの月次ルーティンメッセージをつくよみとして投稿する
 */
function slackNotifyPostBillingRoutineTsukuyomi_(projectId, ym){
  var channelId = slackNotifyGetProjectChannelId_(projectId);
  if (!channelId) return { ok:false, message:"channelId missing for " + projectId };

  var msg = slackNotifyBuildBillingRoutineMessage_(projectId, ym);
  if (!msg) return { ok:false, message:"no routine message to post" };

  return slackNotifyPostToChannelTsukuyomi_(channelId, msg);
}

/**
 * 月次報告会スケジューリング用のBlock Kitメッセージを組み立てる
 * meetingStartAtが空のPJに対して送る
 */
function slackNotifyBuildMeetingScheduleMessage_(projectId, projectName, ym) {
  var ymLabel = String(ym||"").replace(/^(\d{4})(\d{2})$/, "$1年$2月");
  var pjLabel = String(projectName || projectId || "");

  var slots = [];
  try {
    slots = getAvailableMeetingSlots_(
      ["masa@team-armada.jp", "kyoko@team-armada.jp"],
      14, 10, 18
    );
  } catch(e) {
    Logger.log("freebusy error: " + e.message);
  }

  if (slots.length === 0) {
    // フォールバック：空き枠が取れなかった場合のみdatepicker
    var fallbackText = "📅 *" + pjLabel + "* " + ymLabel + "度\n"
      + "今月の月次報告会、日程を入れてね。\n"
      + "日付と開始時間を選んで「確定」を押すだけでOK（30分枠で自動生成されるよ）";
    return {
      text: pjLabel + " " + ymLabel + "度：月次報告会の日程を入れてね",
      blocks: [
        { type:"section", text:{ type:"mrkdwn", text: fallbackText } },
        {
          ...
          elements:[
            ...
            { type:"timepicker", action_id:"meeting_time_pick", placeholder:{ type:"plain_text", text:"開始時間" } },
            {
              ...
              value: JSON.stringify({ projectId: projectId, ym: String(ym||"") })
            }
          ]
        }
      ]
  ...
  var text = "📅 *" + pjLabel + "* " + ymLabel + "度\n"
    + "今月の月次報告会、まさ・きよの空き枠を選んでね（30分枠）";

  var buttons = slots.slice(0, 6).map(function(slot) {
    var label = Utilities.formatDate(slot.start, "Asia/Tokyo", "M/d(EEE) HH:mm");
    return {
      type: "button",
