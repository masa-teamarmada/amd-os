...
function listAdminUsers(){
  ...
  const iSkillProfile  = idx_("defaultSkillProfile");
  ...
  for (let r=1; r<vals.length; r++){
    ...
    out.push({
      ...
      joinYm,
      leaveYm,
      defaultSkillProfile: s_(get_(row, iSkillProfile)),
...
function admin_updateMemberRoleStatus(payload){
  ...
  const nextSkillProfile = String(payload.defaultSkillProfile || "").trim();

  const nextGoogleState = String(payload.googleState || "").trim().toLowerCase();
  const nextSlackState  = String(payload.slackState || "").trim().toLowerCase();

  const allowLicState = new Set(["", "free", "cancelled"]);
  if (!allowLicState.has(nextGoogleState)) return { ok:false, message:"invalid googleState: " + nextGoogleState };
  if (!allowLicState.has(nextSlackState))  return { ok:false, message:"invalid slackState: " + nextSlackState };
  ...
  const header = ensureHeader_(sh, ["memberId","status","roleTag","notifyChannel","isAdmin","joinYm","leaveYm","defaultSkillProfile","googleState","slackState","updatedAt"]);
  ...
  const iSkill    = h.indexOf("defaultSkillProfile");
  ...
  if (iSkill < 0)    return { ok:false, message:"DB_Members missing defaultSkillProfile" };
  ...
  for (let r=0; r<vals.length; r++){
    ...
    prevSkillProfile = String(vals[r][iSkill] || "").trim();
    break;
  }
  ...
  const changed =
    ...
    (prevSkillProfile !== nextSkillProfile);
  ...
  if (!changed) {
    return {
      ...
      joinYm: prevJoinYm,
      leaveYm: prevLeaveYm,
      defaultSkillProfile: prevSkillProfile
    };
  }
  ...
  sh.getRange(hitRow, iSkill+1).setValue(nextSkillProfile);

  const iGS = h.indexOf("googleState");
  const iSS = h.indexOf("slackState");
  if (iGS >= 0) sh.getRange(hitRow, iGS+1).setValue(nextGoogleState);
  if (iSS >= 0) sh.getRange(hitRow, iSS+1).setValue(nextSlackState);

  sh.getRange(hitRow, iUpdated+1).setValue(new Date());

  logAdminAction_("admin_update_member_role_status", "", {
    memberId,
    prev: { status: prevStatus, roleTag: prevRoleTag, notifyChannel: prevNotify, isAdmin: prevIsAdmin, joinYm: prevJoinYm, leaveYm: prevLeaveYm, defaultSkillProfile: prevSkillProfile },
    next: { status: nextStatus, roleTag: nextRoleTag, notifyChannel: fixedNotify, isAdmin: nextIsAdmin, joinYm: nextJoinYm, leaveYm: nextLeaveYm, defaultSkillProfile: nextSkillProfile }
  });
  ...
  return {
    ...
    joinYm: nextJoinYm,
    leaveYm: nextLeaveYm,
    defaultSkillProfile: nextSkillProfile
  };
}
...
function admin_sendGoogleDowngradeNotice(memberEmail){
  ...
  try{
    ...
    return { ok:false, message: "mail send failed: " + (e.message || String(e)) };
  }
}
