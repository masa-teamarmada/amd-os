...
`project_meeting_summaries` の要約だけを根拠に本文を書いてはいけない。要約層は名称変更や申請可否のような単発の意思決定を落とすことがある (実例: 2026-08 に SX のラウンドテーブル改称 SIER→SIER と、KUTE の GTIE 9月エントリー申請断念の両方が要約層に無く、月報へ旧情報が載った)。各PJについて、本文を書く前に次を必ず行う。
