...
description: AMD OS L2 ①〜⑨ 全 9 種を 1 routine / automation 群で順次抽出する集約版 (= claude.ai Cloud routines daily run cap 15/day 対策、2026-05-26 採用、2026-05-29 L2①を正式対象に訂正)。daily 08:00 JST 発火、Phase 0 で env 準備後、L2 ①/⑥/②/④/⑤/⑦/⑧/⑨/③ の順 (= monthly_reports と議事録を先、MS 進捗を最後) で各 L2 の SKILL を inline 実行 → Supabase / outbox helper で反映 + 通知。
---
...
- **2026-05-29 訂正**: L2 ① `monthly_reports` も「定額 subscription automation で安定抽出する L2」の対象。R313 は旧有料API経路であり、定期 trigger は置かない。
- **発火**: daily 08:00 JST (= L2① monthly_reports の 05:30 automation 後)
...
5. **⑤ メンバーナレッジ** (= member_activities + 議事録 依存)
6. **⑦ OS 台帳差分** (= 5 生データ vs OS 台帳、独立)
7. **⑧ XRL 根拠** (= 5 生データ + 既存 L2 依存、後段)
8. **⑨ 経営ハイライト** (= 5 生データ + OS snapshot + 既存 L2 依存、最後段)
9. **③ MS 進捗** (= monthly_reports + meeting_summaries 依存、旧 PWA hourly-estimate は停止済み。入力依存上最後に実行)

## 【絶対】 動く前に必ず Read

1. `pwa/manual/8-3-l2-extraction-routines-spec.md` (= L2 ①〜⑨ subscription automation 統一仕様)
...
═══════════════════════════════════════════════════
Phase A: ① monthly_reports 抽出 (= 当月 / 前月)
═══════════════════════════════════════════════════
...
═══════════════════════════════════════════════════
Phase B: ⑥ MTG サマリ抽出 (= 過去 24h 終了 events)
═══════════════════════════════════════════════════
...
🚀 L2 全抽出 routine daily 08:00 完了:
  - ① monthly_reports: <N> reports saved, <M> skipped final
...
- R313 / PWA report route / Anthropic API を L2①の定期抽出に使う
