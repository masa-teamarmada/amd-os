...
- **🚨 人の性質・組織の状態は除外** (= まさ 2026-08-28 確定): 経営ハイライトは**日付を持つ事象**の棚。人の性格・対人の癖・チームの成熟度・経営者の構えのような、特定の日に起きたと言えない**状態**は抽出しない。行き先は PJ コックピット「スコア詳細」タブの《組織》(`project_org_observations`) で、経営チームの八機能の充足を判定する材料として使われる。判定表は `pwa/spec/3-6-strategy-signals-current-spec.md` の「人・組織の話は《組織》へ回す」。
  - 除外の例: 「起源PIが商談で壁を作り関係づくりを妨げている」「チームに一体感があり成熟している」「事業範囲を広げない判断が固定されている」
