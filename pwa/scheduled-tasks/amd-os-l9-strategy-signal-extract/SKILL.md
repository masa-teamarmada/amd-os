...
- **🚨 経営ハイライトは会社としての大きな動きに限定** (= まさ 2026-08-28 / 2026-09-04 確定): 特定の日に起きたと言えない**状態**（人の性格・対人の癖・チームの成熟度・経営者の構え）に加えて、**些細な人事の出来事**と**個々のメンバーの参画可否・貢献ぶり・処遇の相談**も抽出しない。行き先は PJ コックピット「スコア詳細」タブの《組織》(`project_org_observations`) で、経営チームの八機能の充足を判定する材料として使われる。判定表は `pwa/spec/3-6-strategy-signals-current-spec.md` の「人・組織の線引き」。
  - 除外の例: 「起源PIが商談で壁を作り関係づくりを妨げている」「チームに一体感があり成熟している」「事業範囲を広げない判断が固定されている」「メンバーの参画がほぼ確定した / 所属を断ってきた / 制度上の理由で正式参画できない」
  - 入れる例: 代表・CxO級の就任や離脱、技術の担い手の恒久喪失、新会社設立の動きが母体組織へ及ぼした影響
  - **「人と組織の話だから除外」ではない。粒度の問題。迷ったら抽出せず《組織》側の材料として残す。**
  - 残す例: 「技術の鍵を握るメンバーを外し、3名で誰も作れなくなった」「CEO候補と初回接触し参画可能性を探ることで合意した」（どちらも起きたこと。人が絡んでいても事象なら経営ハイライト）
- **signal_type** (10 種):
