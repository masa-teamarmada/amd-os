...
レイヤー:
  ① マクロ指数: 領域ごとに 1本の実線（5本、太め、領域色）
  ② 論文数: 領域ごとに 1本の破線（5本、薄め、領域色）
  ③ 政策イベント: 縦線 + 上端ラベル
     例: パリ協定 / IRA / EU CBAM / GX法成立 / カーボンプライシング / FRB利上げ局面 / AMED認知症重点 …
  ④ 16社プロット: その時点の "そのSUの所属領域のマクロ指数" の高さに点を打つ
     色 = AMDのアウトカムタグ（離陸/燃え尽き/UE失敗 など）
  ⑤ シーズ予兆: 現在ライン右の "次に来そうな指数の山" 上に点滅マーカー

乖離可視化:
  ⑥ 論文線がマクロ線より先行している領域 → 紫の塗りつぶし帯（"研究先行 = シーズ仕込み期"）
  ⑦ マクロ線が論文線より先行している領域 → 橙の塗りつぶし帯（"政策先行 = 研究を集中せよ"）
```
