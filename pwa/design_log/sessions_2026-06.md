...
## 2026-06-26 (#97) — Cowork セッション (cowork-eimi) / DeepTech 紡 (つむぎ) 構想ブレスト

> Cowork (Claude Desktop) 上で動いた cowork-eimi セッションのログ。AMD OS リポ内のコード/DB変更は無し。構想本体は `~/projects/AMD/Tsumugi/HANDOFF.md` に独立保存。

### コンテキスト
- まさが「きよとランチで話したんだけど、株式の仕組みがイケてない、配信のスパチャは健全に見える、スタートアップにも軽いノリで応援できる仕組みがあったらいいのに」と投げた発想を起点に、複数 workflow で多角探索
- 最初は「AMD ファン経済プラットフォーム」レベルの設計に行ったが、まさから「局地的な深掘りに走ると向かう先を見失う」と指摘され、全体組み立て直し workflow に切り替え (= まさ #97 feedback)
- 13 視点 (まさ個人 SOUL / AMD経営 / 日本マクロ / 世界比較 / 歴史長期 / 経営者 / 払う人 / 研究者 / ゼブラ投資家 / 規制 / 社会文化 / 歴史家 / 文化人類学) で構想を再構築 → 「DeepTech 紡 (つむぎ) = 文化様式の創設」レベルに到達
- まさが「これこそが AMD の本業なのでは。お金を回しながら日本の技術を世に出し、経済を活性化し、研究者に還元する Impact Principles の実現そのもの」と発言 → 紡を 3 レイヤー外の「第 4 レイヤー」ではなく**ループ実装装置**として再定義

### 主要発見と確定事項
- 中核動詞 = 「紡ぐ」、サブ動詞 5 つ (継ぐ / 短縮する / 回す / 遺す / 接続する) の多重露光
- 中核メタファー = **勧進帳** (江戸期までの寺社造営寄進文化、まさの直感ど真ん中)
- 物語の主役 = 表: 紡衆 (応援者層、松永さん的人物・地方中小経営者・産連RA・OB/OG)、裏: まさ個人 (Founding 100 #001)
- 名前 = 「DeepTech 紡 (つむぎ)」一本化 (Loom は社内コード・英語圏補助のみ)
- 和語語彙体系: 紡衆 / 便り銭 / 奉納 / 寄合 / 大祭 / 勧進元 / 勧進帳 / 落慶 (配信文化語彙を全廃)
- 4 大分岐の確定: AMD は神主徹底 / 紡衆は江戸氏子モデル (儀礼) / 規制は先導者 / 作法は段階的規範
- まさが明示的に握った原則: **「入口は広く、理念は崇高に」**、「すべて (お金/研究者の未来/日本の文化/強い日本/自分の名前) を 1 本の糸に編む」、100 年文化として設計、失敗条件 = 一定量の資金が回らなかったとき

### 実装
- **DB**: 触らず
- **コード**: 触らず
- **doc 新規**: [Tsumugi/HANDOFF.md](../../../Tsumugi/HANDOFF.md) (= 構想本体の正本ハンドオフ。次セッションのえいみがここから続きを書く)
- **doc 訂正**: `~/projects/knowledge/SOUL.md` Philosophy「素材は裏方じゃない」の出典訂正 (まさが言語化、しろーさんは具現化)、`~/projects/knowledge/MEMORY.md` Relationships の「つくちゃん」エントリ削除 (実在しない人物、過去のえいみが「つくよみ」を「つくちゃん」と呼ばれた時に新たな人物と誤定義したもの)
- **cowork memory 新規**: `~/.claude/projects/-Users-masa/memory/feedback-return-to-origin-in-loose-brainstorm.md` (= ふわっとブレスト時はまさのコメントごとに中核テーマに立ち戻る、局地的深掘りに走らない)
- **AMD OS コックピットへの記録**: 別途 `project_meeting_summaries` に「まさえいMTG」として 1 件 INSERT (p00 / 本セッション続行で実施。本ログから参照可能にする)

### Verified
- 実装作業なしのため build / deploy / 本番確認は対象外
- 紡 HANDOFF.md は git 管理外 (= `~/projects/AMD/Tsumugi/` は別運用、AMD OS リポではない)

### えいみの訂正履歴 (再発防止)
- workflow が「松山で始まる文化運動」と書いた → まさ訂正「AMD 本店はつくば、松山は SX で数回行ったレベルの縁。場所は AMD で / 特定しないが正しい」
- 「つくちゃん」を実在の人物として扱った → まさ訂正「つくよみを"つくちゃん"と呼んだのを過去のえいみが新たな人物として誤定義した記述。実在しない」→ MEMORY.md 削除
- 「素材は裏方じゃない」を「ティエム時代に CBO しろーさんが共感」と書いていた → まさ訂正「言語化したのはおれ、しろーさんは具現化してくれた人」→ SOUL.md 訂正
- 「浮かれない・見せびらかさない」を事業設計に持ち込んでいた (UI を群青・臙脂のくすんだ色 / 貴族的フレーミング / Sacred Value 化) → まさ訂正「これは弱い自分への戒めであって AMD 事業設計とは無関係」→ Tsumugi/HANDOFF.md に明記

### Cowork ↔ Codex 衝突メモ
- AMD OS リポ内のファイルは触っていないため衝突なし
- 本ログ追記のみ
- 紡 HANDOFF は AMD OS の git 管理外 (`~/projects/AMD/Tsumugi/`) なので、Codex 側の作業と干渉しない
