...
- **テーブル列** (`scope="all"`、実質 `/seeds`): シーズ / **シーズ状態** / 研究機関 / 研究者・PI / PJ状態 / SPS(億円)・根拠Lv / TRL・BRL・GRL・SRL・HRL / 事業化情報 / 資料。シーズ状態はシーズ本体の `seeds.status` を独立列に置き、シーズ名セルへ混在させない。PJ状態はライフサイクル文言やPJの `active` / `ended` を併記せず、紐付いたPJ名だけを表示する。`spun_off` は `seed_projects.venture_name`（会社名）を優先し、PJコードではなく会社名を表示する。旧SPS・M・P・R・S列は **2026-08-16 まさ承認で非表示化** (下記参照)、PJ/機関cockpit比較表 (`scope!=="all"`) は従来どおり旧SPS・M・P・R・Sも表示する
