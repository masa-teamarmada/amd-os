...
- **テーブル列** (`scope="all"`、実質 `/seeds`): シーズ / 研究機関 / 研究者・PI / PJ状態 / SPS(億円)・根拠Lv / TRL・BRL・GRL・SRL・HRL / 事業化情報 / 資料。旧SPS・M・P・R・S列は **2026-08-16 まさ承認で非表示化** (下記参照)、PJ/機関cockpit比較表 (`scope!=="all"`) は従来どおり旧SPS・M・P・R・Sも表示する
...
- **旧SPS列の非表示化** (2026-08-16、まさ裁定「旧バージョンはOSに出さないでOK」、[SPS_NAT_VALUE_MEASURE_PROPOSAL_2026-08-16.md](../bzm/SPS_NAT_VALUE_MEASURE_PROPOSAL_2026-08-16.md) §8): `/seeds` (`scope="all"`) の「旧SPS・M・P・R・S」(`seed_sps_assessments` 由来、持分価値版9軸ルーブリック) 列を非表示にする。**データ (`seed_sps_assessments`) と計算コード (`seed-sps.ts` / `CockpitKuteSeeds.tsx` の `sps` 参照) は削除せず保持**、`CockpitKuteSeeds.tsx` 側は `screeningBand===undefined` (= `scope!=="all"`) のときだけ列を出す条件分岐で表示を切る。PJ/機関cockpit比較表 (`scope!=="all"`) は産業創出価値版の帯を持たないため旧SPSを引き続き表示する。SPS(億円)列と根拠Lv列は本項の対象外 (現行の産業創出価値版として存続)。
