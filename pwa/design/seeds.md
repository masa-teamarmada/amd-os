...
- **テーブル列** (`scope="all"`、実質 `/seeds`): シーズ / **会社名** / 研究機関 / 研究者・PI / SPS(億円)・根拠Lv / TRL・BRL・GRL・SRL・HRL / 事業化情報 / 資料。**PJ状態カラムは置かない**。シーズ名の左には外部サイトのfaviconではなく、`domain_lane` を形でも判別できるLucideアイコンを表示する (`gx_energy=Zap` / `gx_circular=Recycle` / `life=Dna` / `materials=Atom` / `robo=Bot` / `ict=Cpu` / `other・null=FlaskConical`)。20pxの淡色コンテナと領域名tooltip / `aria-label` を持ち、色だけに依存せず、行高とsticky列幅は増やさない。会社名セルは枠で囲わず太字を主表示にし、`seed_projects` に紐付く行だけ右上の `PJ` バッジで示す。会社名は `seed_projects.venture_name` を正本とし、`commercialization_stage='pre_incorporation'` は「会社名（未設立）」、紐付く会社名が未登録またはPJ未紐付けなら「未設立」と表示する。状態語の「スピンアウト済み」「協議中」は一覧に表示しない。旧SPS・M・P・R・S列は **2026-08-16 まさ承認で非表示化** (下記参照)、PJ/機関cockpit比較表 (`scope!=="all"`) は従来どおり旧SPS・M・P・R・Sも表示する
...
- **一次選別スクリーニング帯セクション** (2026-08-20 追加、同日 スコア詳細タブ同等の数式パネルへ拡張、`seed_screening_bands` に `measure_version='sps-ind-v1'` の帯がある場合のみ表示): 見出し「一次選別スクリーニング帯 (SPS = 産業創出価値)」の下に、PJコックピットの「スコア詳細」タブ (`AmdScoreFormulaPanel`) と同じ情報量の数式パネル [`SpsFormulaPanel`](../src/components/sps/SpsFormulaPanel.tsx) を置く (まさ確定 2026-08-20「PJコックピットのスコア詳細タブと同じくらいの情報量を入れてほしい。元の数式もLaTeXでちゃんと書いて、その中にあるパラメータがそれぞれどういう数字になっているかも書いてほしい」)。パネルは KaTeX ([`Tex`](../src/components/venture-map/Tex.tsx)) で数式を描き、7ブロック構成にする: (1) SPS定義式 `SPS = Σ_o q_o P^ind_o` と現行データの単一経路縮退 `SPS = (q/100) P^ind`、q / P^ind / o の意味チップ。(2) 帯の端点 `SPS_min = (q_min/100) P^ind_min` / `SPS_max = (q_max/100) P^ind_max` に**このシーズの実値を代入した式**（右辺はDBの `sps_lower_yen` / `sps_upper_yen` を億円で出す。式は単調なので端点同士の積が厳密な最小・最大、§6確定8）。(3) 中央値 `SPS_median = (SPS_min + SPS_max)/2` の実値代入と、算術中点が仮置きである注記 (§6確定11 第二裁定)。(4) パラメータ実値表 (q帯 / P^ind帯 / SPS帯 / 中央値 / 段階仮説 + 根拠タグ / q主要因 / p_class / 根拠Lv / 評価者・評価日時 / measure_version・ruleset_version・凍結)。(5) q判断層: `q ≠ Σ w_i x_i` を明示して「11要因は点検表であって合成関数ではない、係数を発明しない」と説明し (`SEED_Q_RUBRIC_2026-08-15.md` §4運用規律1)、11要因を番号・名前・観点・direction (上振れ/下押し/帯を広げる/中立/未評価) の一覧で出す。`q_evidence` に無い要因は「未評価」として11行すべて表示する。(6) 段階仮説: S0〜S5 の6段を並べ `stage_lower`〜`stage_upper` の区間をハイライトし、`stage_tag` を和訳して出す。(7) 根拠Lv: Lv0〜Lv3 を並べ当該Lvをハイライト。各ブロック末尾にbzm正本の出典を書く。**数式は現行 SPS = 産業創出価値版 (`sps-ind-v1`) だけを書く**。旧9軸 Cobb-Douglas と旧 `sps-eq-v0` (持分価値版) は退役済みなので数式パネルに持ち込まない。LaTeX内に日本語 (`\\text{...}`) は書かない (既存 `AmdScoreFormulaPanel` に前例がなく、フォント欠落で豆腐になるため)。単位「億円」は数式の外のラベル側に置く。パネルの下に固定注記「この帯は接触と調査の優先順位づけの下書き。上限は楽観シナリオの包絡であり評価額ではない。投資判断・対外表示には使わない」と、判断根拠の共通UI [`SpsBandRationale`](../src/components/sps/SpsBandRationale.tsx) (`notes` を「総合判断」段落で常時表示、`q_evidence` 11要因を折りたたみ) を置く。詳細DTOは `/api/seeds/screening-bands?seedId=` から取得 (member認証、`seed_screening_bands` はRLSポリシー無しのservice_role専用テーブルのため直読みしない)。取得に失敗してもシーズ詳細本体の表示は落とさない
