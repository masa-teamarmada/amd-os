...
| **Phase 4** ⭐ (本仕様) | 本体GAS の毎時 trigger (`gas/155_L2KnowledgeExtractor.js` `nav_project_knowledge_pollAll`) で稼働。**入力は二次集約** (monthly_reports + project_meeting_summaries)。5 生データ直結は Phase 4.x 改善案。**既存 2024 行は破壊しない設計** (entity_name 単位で SELECT → 既存有り UPDATE / 無し INSERT) |
...
### 本番 (毎時)
GAS time-trigger `nav_project_knowledge_pollAll` が毎時 1 回発火。
