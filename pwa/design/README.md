...
| 先手 TODO / 採否判断レビュー / 旧先手力維持ループ ⭐ | [../spec/2-4-proactive-todo-current-spec.md](../spec/2-4-proactive-todo-current-spec.md) / [../spec/3-7-notifications-current-spec.md](../spec/3-7-notifications-current-spec.md) / [proactive_operating_loop.md](proactive_operating_loop.md) — 先手TODOの汎用生成は停止。既存 `amd-os-proactive-heartbeat` はL2採否判断レビューを担当。旧 commander outbox配送は廃止済み |
| 研究機関・シーズ・PJ / 外部ワークスペース認可 ⭐ | [institution_seed_project_model.md](institution_seed_project_model.md) — 現行正本。旧NIMS固有案は [institution_tenant_access.md](institution_tenant_access.md) |
