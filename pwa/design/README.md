# pwa/design/ — 実装設計の正本フォルダ

> **新セッションのえいみが manual の次に読むフォルダ。**
> `pwa/manual/` が読み手向けの正本、ここは PWA の実装設計・DB 境界・cron / prompt 境界を追う正本。
> `AGENTS.md` / `CLAUDE.md` からこのフォルダに案内される構造になっている。
...
| 順 | ファイル | 内容 |
|---|---|---|
| **0** | [**L2_DATA.md**](L2_DATA.md) ⭐⭐⭐ | **AMD OS 中核データ正本** — L2 9 種 (monthly report / AMDプロトコル / MS進捗 / PJナレッジ / メンバーナレッジ / MTGサマリ / OS台帳差分 / XRL根拠 / 経営ハイライト) + レポート + 全 cron。データに触る前に必ず読む |
...
| OS Manual / Help | [os_manual.md](os_manual.md) ⭐ — `/manual` の章構成、audience 分離、本の目次型 UI、番号体系 |
...
| 経営ハイライト (重要方針/事業進捗/リスク) ⭐ | [project_strategy_signals.md](project_strategy_signals.md) |
