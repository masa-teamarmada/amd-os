...
- **D-14G Governance Email Sweep**: `GET /api/cron/governance-email-sweep`。既定は `apply=false` で `/api/governance/extract` に候補投入し、`source_cache(source='gmail_governance')` に source ref / snippet / hash を残す。`apply=1` のときだけ canonical `project_shareholder_meetings` 反映 + 添付Drive保存まで進める。通常運用は候補優先だが、**開催日・会議種別・議事録/決議/書面決議という開催済み証跡がそろうものだけ**を候補にする。ジョブカン等の承認ワークフロー、招集通知だけ、既存正本と重複するものは候補・通知にしない。LST のように高確度で日付・種別・PJ が揃う場合だけ apply 実行する。
