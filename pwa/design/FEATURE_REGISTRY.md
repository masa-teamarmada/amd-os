...
- Round 18（2026-07-27、v3.50.0）: **PJ週次管制を別URLで追加** — `/project/[projectId]/weekly-control`。既存`/workspace`を置換せず、上段を「先週→今週 / 今回の会議 / 決定→介入」の3段レール、中央を4本柱の例外行、主面を論点・仮説の`要整理→検証中→判断待ち→決定・棄却`に固定。担当・次期限・最終更新・更新切れ・期限超過を表示し、仮説/根拠/検証/判断/決定後actionを既存management APIへ追加・更新できる。判断済みでも未完了actionは要フォローに残す。カードの次の動きは停止→期限超過→期限順で決定後action/判断/検証から選ぶ。親論点の鮮度は仮説・根拠・判断・actionの最新確認日も含め、次期限は子要素を含む最短日から導出。週次差分未接続は0件でなく`抽出接続待ち`、既存値は`現行台帳の仮表示`と明示。`projectScopedPathAllowed`、埋め込みshell/月初合意除外、SSR/client titleを両route対応。migrationなし。
