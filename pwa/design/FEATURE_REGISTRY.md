...
- `pwa/scripts/check_pwa_critical_ui.cjs` が `/admin/ms-overview` の route / page / client / sidebar anchor (`MS一覧` / `全MS` / `メンバー別 年計` / `cap_extra` / `編集モード` / `MS追加` / `担当share` / `MS内金額` / `保存先 DB` / `DB値に戻す` / `保存して DB へ反映` / `recomputeMsOverview` / `regularPtUnitYen` / `extraPtUnitYen`) を検査する。
