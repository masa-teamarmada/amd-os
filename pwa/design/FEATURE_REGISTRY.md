...
- 月次モーダル: 月次カードから `CockpitMonthlyModal` を開き、report / reward / invoice を確認できる。右上の操作は4つ、全PJ共通ラベル。`社内版` (`template=internal` の印刷ビューを新規タブで開く。総合判定・MS前月/当月/差分表・ガント・体制・次月計画を可視化した帳票) と `提出版` (`template=submission` の印刷ビューを新規タブで開く) は同格のリンクで、どちらも版を開くだけの操作。`本文を編集` はこの2つとは別の第三の操作で、可視化帳票が参照する社内版本文 (Markdown) の直接編集パネルを開閉する。`提出版を編集` は第四の操作で、`monthly_reports_external.body_md` を直接編集し、保存後の提出版PDFへ同じ本文を反映する。二つの編集パネルは同時に開かず、保存先も社内版と混ぜない。提出版は保存時に帳票構成と内部用語を検査する。社内版本文の編集・保存・確定は非LLM操作で、生成・再生成・AI修正ボタン、提出先名を埋め込んだ操作ラベルは置かない。routine step 起動は廃止済み。p00 (= AMD 会社全体) でも他 PJ と同じく月次カード + 月次モーダルが出る (`billing_cycles` を 12 行 backfill 済)。
