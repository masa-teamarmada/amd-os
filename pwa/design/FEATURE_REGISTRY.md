...
- `npm run test:project-management-rls` (`npm run test:sx-management-rls`) は23個のsoft-delete対象member_select（`project_management_partner_interactions`/`project_management_partner_roles`/`project_management_partner_work_items`含む）、184/185/191/192の分類・約束・決定・現在ボール・期限精度・行為主体側・`source_kind`・完了必須条件・成果物受入必須条件CHECK、補正履歴、partner_interactions/partner_roles/partner_work_items各表のtrigger（block_delete/parent_project_guard/field_audit/touch_updated_at）・列存在（`completed_on`/`completion_evidence`/`accepted_by`/`accepted_on`/`source_kind`/`source_ref`含む）・soft-delete可視性・親PJ guard（`related_milestone_id`が他PJのゲートを指す直接probeを含む）、`project_management_partner_roles_one_primary_192`（1partner1primary roleのunique partial index）、p21 primary role seed 6件・保有事項seed 4件、interaction actor_side backfillは固定件数ではなくライブ行のsource_ref+既知summaryでactor_side='unknown'が残っていないかで判定、`project_management_partner_roles`/`project_management_partner_work_items`双方のsoft-delete→migration再適用相当probe（idの重複・復活が無いこと、work_itemはcompletion/related_milestone_id等が変わらないことも検査）、provenance probeは通常編集が`source_kind='manual'`/`source_ref='PWA共有管理画面'`へ再スタンプしsoft-delete/restoreはそれを変えないことを検査、`status='completed'`/`item_kind='deliverable'`の未確認完了を拒否するDB CHECK probe、authenticated roleによるPJ境界を検査する。migration 192未適用のDBでは新規assertionが失敗する（既存のmigration 191相当の検査は通る）。
