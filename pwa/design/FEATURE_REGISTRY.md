...
- Round 6（2026-07-23 関係先台帳化、migration `191_sx_partner_ledger_upgrade.sql`）: `project_management_partners` へ `current_ball_side`（`sx`/`partner`/`shared`/`none`/`unknown`）・`current_ball_owner`・`next_ball_owner`・`target_state`・`due_date_precision`（`day`/`month`/`unknown`）を additive 追加し、新設の `project_management_partner_interactions`（面談/メール/合意/成果物/引き継ぎ/状況更新/メモの履歴、他 `project_management_*` 表と同水準の soft delete/RLS/parent guard/field audit）と合わせて、機関ごとに「誰がボールを持ち」「次に誰へ渡り」「いつまでにどの状態になるか」を初見で読める関係先台帳へ再設計した。`due_date`/`due_date_precision`（partners）と`occurred_on`/`occurred_on_precision`（interactions）はDB CHECK（`project_management_partners_due_date_consistency_191` / `project_management_partner_interactions_date_consistency_191`）とmanagement APIの合成検証（`assertDatePrecisionConsistency`）の両方で、unknown精度に日付あり・day/month精度に日付なしを拒否する。`SxPartnerPipeline` は上部に小集計帯（対応中 / 相手ボール / SXボール / ボール未確認 / 期限未設定の件数）を持ち、各行/カードは stage rail・合意・現在ボール（側+実名）・次の受け渡し（受け手）・次の一手（`partner.nextCommitment`、閉じた行/カードでも常時表示、受け手/目標状態と別行）・目標状態・期限（precision付き、月精度は「日付未確認」を明示し具体日を作らない）・最終接点・合意/未合意・関連ゲート・履歴件数/最新記録を一続きで表示する。表は`xl`（1280px）以上・カードは`xl`未満に切り替え、1024-1279pxでは表を出さずカード表示にすることで比較表の横overflow切れを防ぐ（グリッド列は固定px＋`minmax(0,fr)`混在で実測幅へ収め、目標状態は1行truncateではなく`line-clamp-2`で最低2行読める）。カードは`md`（768px）未満は単一カラム、`md`〜`xl`未満（768-1279px）は`md:grid md:grid-cols-2`のカード内2カラムにして右半分の空白を作らない（ヘッダー/stage railは2カラムに跨り、左カラムに現在ボール・次の受け渡し・次の一手・目標状態・期限/最終接点、右カラムに合意/未合意・担当+関連ゲート・最新記録、操作ボタンと展開履歴は2カラムに跨る。2026-07-23 Round 7）。各機関から開閉できる履歴タイムラインは、閉じても最新記録要約が残り、開くと全履歴と相手の約束/SX側の次アクションを同じ文脈で確認できる。「最新記録」は`occurredOn ?? createdAt.slice(0,10)`を実効キーとした記録時刻ベースの並びで、日付未確認を無条件に既知日付より新しいとは扱わない（`sxLatestInteraction`/`sxSortInteractionsByRecency`）。管理者は行内から履歴の追加・編集ができる。management APIの resource `interaction` が create/edit/soft-delete/restore と親PJ検証を担う。partner seedは`ON CONFLICT (project_id,slug) DO UPDATE`でcurrent truth列を再適用ごとに反映し（`deleted_at`/`deleted_by`は対象外、勝手に復元しない）、interaction seedはmigration実行時刻ではなく固定`TIMESTAMPTZ '2026-07-23 00:00:00+09'`起点のオフセットで順序を保存し、末尾検査は当該`source_ref`＋想定summaryの件数が下限以上であることを確認する（exact countではないため、後からユーザー履歴が増えても再適用できる）。
...
- `npm run test:project-management-rls` (`npm run test:sx-management-rls`) は21個のsoft-delete対象member_select（`project_management_partner_interactions`含む）、184/185/191の分類・約束・決定・現在ボール・期限精度CHECK、補正履歴、`project_management_partner_interactions`のtrigger（block_delete/parent_project_guard/field_audit/touch_updated_at）・列存在・soft-delete可視性・親PJ guard、authenticated roleによるPJ境界を検査する。
- `npm run test:critical-ui` は目的-KPI、測定、決定ループ、POST/新規追加、運用準備、選択文脈、lg以上の比較表・lg未満のカード、effort接続、PJ-scoped effort APIのanchorに加え、`SxProofOutcomes` / `SxPartnerPipeline` / `sx-proof-mapping.ts` / 管理台帳・編集への集約anchor、現在ボール/次の受け渡し/次の一手/目標状態/ボール未確認/最新記録/期限精度表示・やり取り履歴・migration 191（date precision整合CHECK・`ON CONFLICT ... DO UPDATE`含む）・management APIの`interaction` resourceと`assertDatePrecisionConsistency`のanchorに加え、768-1279pxカードの`md:grid`2カラムanchorを検査する。
