...
目的: 支払月単位で、対象cycleの報酬確認、PJ別収支確認、支払データ自動同期、支払通知書発行、入金確認nudgeを一画面で運用する。
...
- 支払額の自動同期: 画面の支払額・PDF生成の元データは、最新の `billing_cycles.reward_summary_json` から計算した値を正にする。`monthly_reward_payout.total_pay` と `payout_notices.total_yen` は保存済み額で表示値を固定するためではなく、自動同期される税抜スナップショットとして扱う。
...
- 支払データ自動同期: `monthly_reward_payout` に明細、`payout_notices.total_yen` にメンバー別通知額を税抜で自動保存する。役員または `exclude_from_payout_notice` のメンバーは通知対象外にする。保存時点ではメール送信せず、金額が変わった未送付PDFだけ再生成対象へ戻す。月初合意gate・本契約cap blocker がある場合だけ `自動保存できない` と表示し、admin override または blocker 解消を待つ。
