...
- 契約ファミリー集約: `DocuSign送付先確認`、`送付依頼`、`微修正`、`確認・...` のような作業行が同じ契約に紐づく場合、主リストでは1契約1行へまとめる。行には「統合 N 行」を表示し、現在状態は統合元から導出する。
...
- `pwa/scripts/check_pwa_critical_ui.cjs` が台帳の最低幅、PJ → 契約名の列順、2列の固定表示、契約ファミリー集約の実装アンカーを検査する。
