...
- 先12か月 目的別4表: `/admin/payouts` と `/management-score` 下部表の両方で、先12か月を `先12か月 キャッシュ支払` / `先12か月 会社留保` / `先12か月 報酬債務` / `先12か月 cap超過チェック` の4表に分ける。会社留保を支出扱いしない。会社留保は `cap/売上枠 - 外部支払` として読む。報酬債務は月末残高なので12か月分を足さず、各月残・ピーク・最終月残で見る。cap超過チェックだけが `報酬需要 - cap/売上枠` を見る。
