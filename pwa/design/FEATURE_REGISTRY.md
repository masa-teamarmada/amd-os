...
- **SX「事業計画」タブの投資判断表現** (2026-07-28 更新): 配色は白・slate・indigoを基調とし、緑/黄/赤は成功/警告/エラー、複数色は株主構成棒と凡例だけに限定する。GRLは内閣府SIPの Governance Readiness Level（制度・規制・標準・ガイドライン、1〜8）で、フェーズ到達値を `1 / 3 / 5 / 6 / 8` とする。組織の採用・役割・内部統制はHRLへ置く。`CapitalPlanMatrix` は `overflow-x-auto` のみでページ縦スクロールを妨げず、各株主はFD比率サマリー1行を初期表示、`aria-expanded` 付きボタンで金額・株数・発行済・FD株数の詳細を開く。年次試算表は百万円・整数・3桁区切りで、売上原価、役員報酬、給与・賞与、研究開発費、販管費、助成金の特別利益/圧縮損、助成金入金を分け、会計PLと資金繰りを混同しない。
