...
- PJ先手レーダー: dashboard 上段に `ProactiveRadarBoard` を置き、PJごとに `MTG準備` / `先手TODO` / `経営ハイライト` / `不在検知` / `月次・契約` の5センサーを横断表示する。これは旧5段ループ盤面の復活ではなく、既存 `proactive_todos` に加えて、まだTODO化されていない「空白提案」(例: 次回MTGが近いのにprepが弱い、high/critical signalが次の一手化されていない、Coverage gapが未ルート、active PJの接点が薄い) を dashboard 内で計算して、今日先に張るPJを見つける入口。自動対象は current PJ (`active` / `sales` / `draft`) に限り、旧PJ・停止PJは明示的な未対応TODOがある場合だけ出す。admin のみ表示し、詳細は `/proactive` / `/notifications` / 各PJ cockpit へ送る。
...
- 今シーズン収支: `CockpitSeasonFinance` を今期MSの直下、月次カードの手前に表示する。シーズン合計と月次行で `クライアント支払` / `バッファ` / `原資上限` / `PJ予算` / `メンバー支払` / `会社留保` / `期末未払` / `残` を出す。クライアント支払は `contractBackedClientAmount` + 別財布売上、schedule_based 契約では `contract_terms_json.monthlySchedule.amountTaxExcl`、バッファは `value_plan_cycles.buffer_breakdown_json` 優先、原資上限は `(クライアント支払 - バッファ) × 65%`、PJ予算は `budget_yen + extra_budget_yen`、メンバー支払・会社留保・未払残は `reward_summary_json` を正本にする。期末未払または原資超過が 1 円でも残る場合は `不足` 表示と赤い停止帯を出し、報酬計算側の自動上乗せでゼロに見せない。
