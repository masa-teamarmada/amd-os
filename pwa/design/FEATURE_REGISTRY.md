...
- 関係先リストは段階、合意済み/未合意、最終接点、SX側の次アクション、期限、担当、関連ゲート、約束履歴を表示し、機関ごとに技術試験・資金・RACI・容量へ辿れる。約束履歴は「相手の約束」と「SX側の次アクション」を分け、相手担当・約束日・一次根拠が揃う場合だけ前者に分類する。desktop列は 関係先 / 当方の保有事項 / 先方の保有事項 / 次の一手 / 目標状態 / 現在ボール・期限 / やり取り履歴 の7列固定（`xl:grid-cols-[128px_minmax(130px,1fr)_minmax(130px,1fr)_120px_120px_108px_124px]`）、tablet/mobileは当方/先方の二車線＋残りフィールドを全幅ブロックで縦積みする。ユーザー向けラベルとして「次の受け渡し」「受け渡し先」は出さない。旧`next_ball_owner`/`handoff_to`列はDB互換のため内部に残すが、編集フォーム・表示ラベルには出さない。
