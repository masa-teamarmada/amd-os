...
- 月次モーダル: 月次カードから `CockpitMonthlyModal` を開き、report / reward / invoice を確認できる。右上の操作名は全PJ共通の `社内版` / `提出版`。社内版は非LLMの直接編集・保存・確定だけを提供し、提出版は `template=submission` の印刷ビューを開く。生成・再生成・AI修正ボタン、提出先名を埋め込んだ操作ラベルは置かない。routine step 起動は廃止済み。p00 (= AMD 会社全体) でも他 PJ と同じく月次カード + 月次モーダルが出る (`billing_cycles` を 12 行 backfill 済)。
