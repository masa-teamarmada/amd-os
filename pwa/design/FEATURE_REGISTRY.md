...
- **SX「事業計画」タブ** (`CockpitBusinessPlan`, `src/lib/sx-business-plan.ts`, 2026-07-27追加): `p21` のみ、`進捗管理 / スコア詳細 / 事業計画 / 会社概要` の4タブ目として表示する。最上段は4行（事業開発 / 技術開発 / 組織開発 / 資金調達）×5列（PSI / Seed→A / A→B / B→C / C→IPO）のフェーズマトリクス。各セルに活動、費用、出口条件、関連XRL、列見出しに総額・調達源・固定費バーン上限・5軸XRLを表示する。その下へ100%株主構成推移と資本政策表 (`CapitalPlanWorkspace`)、最後に年次試算表を置く。SXの計画資本政策だけは会社概要タブからこのタブへ移し、確定済み会社情報と法的株式台帳は会社概要に残す。原案前提は自社培養・自社包装・顧客拠点への輸送、自社工場の段階投資、中島先生を含めない設立時持分 `CEO 75 / 杉浦先生 20 / まさ・AMD 5`。migration `196_sx_capital_policy_draft.sql` は同名プランが無い場合だけSeed 1.5億円からIPOまでの編集可能な計画を投入し、既存編集を上書きしない。
