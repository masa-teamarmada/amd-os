...
- PJ先手レーダー: dashboard 上段に `ProactiveRadarBoard` を置き、PJごとに `MTG準備` / `先手TODO` / `経営ハイライト` / `不在検知` / `月次・契約` の5センサーを横断表示する。これは旧5段ループ盤面の復活ではなく、既存 `proactive_todos` に加えて、まだTODO化されていない「空白提案」(例: 次回MTGが近いのにprepが弱い、high/critical signalが次の一手化されていない、Coverage gapが未ルート、active PJの接点が薄い) を dashboard 内で計算して、今日先に張るPJを見つける入口。admin のみ表示し、詳細は `/proactive` / `/notifications` / 各PJ cockpit へ送る。
