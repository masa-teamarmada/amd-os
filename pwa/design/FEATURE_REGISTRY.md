...
- **cap table 統合ワークスペース `CapitalPolicyWorkspace`** (2026-07-16 追加、同日 UX 再設計で `CapitalTimeline` / 別枠 `CapTableHistoryMatrix` / 選択イベントだけを再掲する冗長 `cap table｜{event}` セクション / 表から離れた `NextRoundSimulator` を統合・廃止): 会社概要タブの cap table は「資本政策」という単一セクションに集約されている。イベント列見出し (設立/Seed/QST現物出資等の日付+ラベル、実 `<button>` で focus-visible・選択時は下線) の直下に、confirmed 状態の株式イベントから積み上げた holderName ベースの縦積み100%持株構成バーを共通0-100%スケールで揃えて配置し、列をクリックすると次回ラウンド試算のベース断面が切り替わる。その下に `data-testid="cap-table-history-matrix"` の履歴行 (発行済株式・新規発行・調達額・発行価額・pre/post-money → holderName別 `shares株 · pct%` の compact 1行表記) を sticky label 128px / event列 140px のコンパクト幅で並べる。正史が `incorporation` から始まらない場合は `capTableOriginWarning` が `data-cap-table-origin-warning` の警告をワークスペース内に出し「創業時のシェアを遡って再現できない」旨を明示、創業時株式イベントの入力導線 (`onAddFoundingEvent`) を提示する。同じ表の最終列が `data-testid="next-round-simulator"` の `次回ラウンド（試算）`（`仮・FD` ラベル、青破線区切り）で、他の履歴列と同じ行構造 (発行価格・新規投資家株数・SOプール・post-money・完全希薄化後・各holderの試算後 `shares株 · pct%`) に試算値を表示する — 個別カードや前後バーの再掲はせず、この列自体が試算プレビューを兼ねる。試算コントロール (ベース断面・pre-money・調達額・post-round SOプール目標・「転換前証券を含める」明示トグル `includeConvertibles`・保護株主と目標最低比率) は同じワークスペース内で、広い画面は右レール (sticky)、狭い画面は表の直前に配置し、`computeNextRoundScenario` / `minimumPreMoneyForTarget` / `nextRoundSensitivity` (いずれも `src/lib/company-overview.ts`) で発行価格・新規投資家株数・SOプール・post-money・完全希薄化後・保護株主の目標維持に必要な最低pre-money・感度分析を `<dl>` definition row + 感度チップで算出表示する。未入力 (`estimated_conversion_shares` 未設定) の転換前証券株数を推測することはない。**この試算は未保存・仮の数値で、確定した現在値の cap table や法的な株式イベント台帳には一切反映しない**。Excel 出力 (`downloadCompanyOverviewXlsx`) も UI と同じ計算エンジンを再利用し、`ラウンド別cap table` シートに履歴マトリクスを書き出す。削除禁止理由: 現在値だけでなく資本政策の経緯を1つの正本タブ・1つの統合グリッドに揃えるため、編集→スクロールして確認→また編集の往復を無くすため (まさ確定 2026-07-16)。
- **資本政策プラン台帳 `CapitalPlanWorkspace`** (2026-07-17 追加、旧・単発未保存の `NextRoundSimulator`「次回ラウンド試算」列と保護株主概念を置き換え): `CapitalPolicyWorkspace` の cap table 統合グリッド直下に置く、全 AMD メンバーが閲覧・編集できる名前付きシナリオ台帳。DB: migration `179_project_capital_plans.sql` の `project_capital_plans` (作業中シナリオ、`amd_os_is_member()` gate) / `project_capital_plan_versions` (freeze 時の append-only 確定版、`UPDATE`/`DELETE` は trigger で拒否)。ロジック: `src/lib/capital-plan.ts`。必須機能: (1) シナリオに名前を付けて複数保存できる、(2) 編集は800msデバウンスで自動保存され `revision` 列の楽観ロックで同時編集競合を検知する、(3) `incorporation` から `equity_issue` / `option_pool` / `convertible_issue` / `convertible_conversion` / `secondary` / `share_split` / `ipo` までの複数ラウンドを1本のイベント列で連結し、上流イベントの変更を `deriveCapitalPlan` が下流ラウンドへ再計算・伝播する、(4) イベント/割当のすべての数値フィールドは `EditableValue{value, source}` を持ち、`source` は `input`/`calculated`/`imported`/`confirmed`/`override` のいずれかで、`override` は元の自動計算値 (`calculatedValue`) を保持したまま手動置換できる、(5) **保護株主 (protect-holder) 概念は存在しない** — 目標比率は割当ごとの `targetOwnershipPercentage` を編集するだけで、特定株主を自動保護する仕組みは作らない、(6) cap table 履歴マトリクスと同じイベント列に揃えた holderName ベースの縦積み100%グラフを表示する、(7) `validateCapitalPlan` の `severity='error'` が1件でもあれば freeze ボタンを無効化し、確定は `freeze_capital_plan()` SECURITY DEFINER 関数が revision検証・version insert・`latest_frozen_version`更新を単一トランザクションで行う、(8) VC向け Excel 出力 (`createCapitalPlanXlsx`) は frozen version からのみ生成でき作業中シナリオからは出力できない、(9) `md:hidden` 幅では表/グラフの代わりに選択中の1イベントを◀/▶で切り替えるモバイル一イベントフォーカス編集に切り替わる。削除禁止理由: 保存されない単発試算と保護株主という決め打ち概念では複数ラウンドを跨いだ資本政策の意思決定を積み上げられなかったため、名前付きシナリオ + 全値editable + freeze版という設計に置き換えた (まさ確定 2026-07-17)。
