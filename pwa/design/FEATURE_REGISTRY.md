...
- 関係先リストは大学・金融機関・試作先・PoC先を含む唯一の一覧で、独立したPoC候補先リストは持たない。段階、確認済み接点、未完了作業、現在ボール、次の一手、目標状態、期限、担当、関連ゲートを表示し、機関ごとに技術試験・資金・RACI・容量へ辿れる。PoCは進捗段階でなく横断属性で、候補・接触済み・調達済みをまとめてUI上は`PoC先`と呼ぶ。全件を同じ`role_kind × relationship_state`でgroup化し、同じ`停止 → 当方ボール → 表示期限`でsortする。`表示: 全関係先 / PoC先`と`役割`は独立したAND filter。PoC専用の調達段階group・段階説明・未接触折りたたみは持たず、各行は短い`PoC` badgeだけを持つ。未接触のPoC先は全関係先の母数へ含め、緊急・ボール等の実行中管制件数からは除く。母数帯のPoC固有指標は総数だけを表示する。desktopの初期行は `関係先 / 進行状況 / 直近接点 / 現在ボール・期限 / 関連・操作` の5列（`xl:grid-cols-[152px_minmax(0,2fr)_minmax(0,0.95fr)_118px_140px]`）。tablet/mobileは同じarticle内のlabel付きcellへ再配置する。旧 `当方の保有事項 / 先方の保有事項 / 次の一手 / 目標状態` は進行状況と重複するため初期列から外し、値・追加・編集は展開詳細に維持する。進行状況は確認済み接点 → 現在地 → 未完了作業 → 次の一手 → ゴールのdynamic stepsで、会社名下railも同じ配列を使うためsegment数とblock数が1対1になる。ユーザー向けラベルとして「次の受け渡し」「受け渡し先」は出さない。旧`next_ball_owner`/`handoff_to`列はDB互換のため内部に残すが、編集フォーム・表示ラベルには出さない。
