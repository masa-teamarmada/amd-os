...
- 抽出状況: dashboard 上段に admin 限定の `ExtractionStatusCard` を出す。Gmail / Drive / Calendar / Slack / Notion ごとの「OSへ最後に保存された時刻」を並べ、月次対象PJのメール・Slack・Drive設定不足は「設定が必要」に集約して `/admin/projects` へ送る。48時間以上保存がない情報源には、再認証・PJ設定不足・接続確認済みの保存停止を理由として出し、それぞれ再認証・PJ台帳・抽出運用を開く導線を付ける。これは接続の成否を推測する面ではなく、OSへ保存された証跡と復旧の入口を一体で見る運転盤。`project_config_gap` は採否通知に出さない。
