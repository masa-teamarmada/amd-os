...
- 関係先リストは大学・金融機関・試作先・PoC候補先・VCを含む唯一の一覧で、独立したPoC/VCリストは持たない。`全関係先 / PoC候補先 / VC` は同じ台帳を絞る排他的タブ。VCは保存済みroleの`role_kind='shareholder_investor'`だけで判定し、名称から推測しない。PoC候補先/VCタブでは役割groupを比較主軸から外し、対象全件を1本の高密度registerへまとめる。主列は`関係先 / 現在の状況 / ゴール / 詰まり・PJ影響 / 次にやること / 担当・期限 / 現在地の根拠 / 履歴・保有`。一覧は現在地stepと目標状態だけを別列で表示し、確認済み接点（古い順）→現在地→未完了work item→次の一手→ゴールの可変`進行状況`は詳細モーダルへ置く。会社名下の短いsegmentと詳細内の進行blockは同じ配列から描画し件数・順序を一致させる。行全体を1つの詳細ボタンにし、どこを押しても同じモーダルを開く。固定段階filter、固定分母、`x/7`は使わない。停止・期限超過・7日以内・保留・情報更新要・担当未確認・期限未設定は進行stepとは別の関係先単位判定として表示し、当方ボールは独立表示と要対応内の優先順に使う。情報更新要は確度未確認または最終確認14日超。要対応件数は該当先の絞り込み操作を兼ね、列見出しは長い一覧でも固定表示する。ボールは側と担当者を併記し、現在行の会社名だけを限定文型で「先方」へ短縮する。全履歴・保有事項・目標・編集は行下でなくモーダルへ出す。表示中集計は選択タブとquick filterへ追随する。p21のVCはPartners Fund、DAVP、BNV、いよぎんキャピタルを別partnerとして持ち、確認できない接触段階・合意・ボール・期限・ゴールは未確認のままにする。ユーザー向けラベルとして「次の受け渡し」「受け渡し先」は出さず、旧`next_ball_owner`/`handoff_to`列はDB互換のため内部に残すが編集フォーム・表示ラベルには出さない。
