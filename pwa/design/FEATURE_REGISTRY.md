...
- 関係先リストは大学・金融機関・試作先・PoC先を含む唯一の一覧で、独立したPoC候補先リストは持たない。通常表示は`role_kind × relationship_state`で整理するが、PoC filter中は役割groupを比較主軸から外し、候補→情報交換→条件整理→面談調整→検証準備→合意確認→実行中の共通7段階で一本化する。PoC比較行は`PoC先 / 関係段階 / 要対応 / ボール・期限 / 直近接点 / 次の一手 / 詳細`の高密度registerで、会社名下railの二重表示、各行のPoC badge反復、接点件数を進捗へ数えるdynamic stepsを持たない。停止・期限超過・7日以内・保留・情報更新要・担当未確認・期限未設定を段階とは別の関係先単位判定として表示し、当方ボールは独立列と並び順のtie-breakにだけ使う。情報更新要は確度未確認または最終確認14日超。要対応件数ピルは該当先の絞り込み操作を兼ね、並び帯と列見出しは長い一覧でも固定表示する。ボールは側と担当者を併記し、現在行の会社名だけを限定文型で「先方」へ短縮する。直近接点と次の一手は1行、全履歴・保有事項・目標・編集は行下でなくモーダルへ出す。段階filter後の要対応集計は表示中の行へ追随し、全PoC母数と表示件数を分ける。ユーザー向けラベルとして「次の受け渡し」「受け渡し先」は出さず、旧`next_ball_owner`/`handoff_to`列はDB互換のため内部に残すが編集フォーム・表示ラベルには出さない。
