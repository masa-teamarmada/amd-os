...
`project_pl_monthly` は BZM経済CFとは別の計画PLである。BZM 2.2の時間軸は、評価月M0から計算地平までこのテーブルを同じ月列に表示し、SXだけはその直下へ設備投資・株式調達・融資・助成金等入金・月次純C/F・月初/月末資金残高を続ける。$J/P$へ直接入るのはartifactの`monthlyEconomicCFMillionJpy`だけで、資金繰りのC/Fとは混ぜない。
...
SXの月次資金繰りは`src/lib/sx-monthly-finance-plan.ts`で再現する。設備投資は現行年次試算額を旧月次表の既存タイミング規則どおり各FYの4月へ置く。株式調達はactiveな`project_capital_plans`のイベント日と金額を優先し、取得不能時だけ現行初期原案（Seed 2027-04 1.5億円、Series A 2028-10 6億円、Series B 2031-04 15億円）へ戻す。助成金等入金は、年内の受領月を確定する一次資料がないため年次試算額を各FYの4月へ置く**計画値/推定値（低精度）**とする。融資は金額・実行時期の登録がないため0円へ丸めず`未計画`のまま表示する。

PSI Step 2の6,000万円はPhase 0（2026-01〜2027-03）と同じ非希薄化資金で、Phase 0の事業計画とFY2027助成金へ二重計上しない。月次ロールフォワード上は2026-07へ1回だけ置き、M0より前のためM0月初資金へ繰り越す。`project_grants`の採択額7,800万円は採択証拠として併記するが、実入金額が未登録なので6,000万円の計画cashを7,800万円の実績へ勝手に置換しない。

簡易資金繰りは`月末資金 = 月初資金 + 営業C/F − 設備投資 + 株式調達 + 融資実行 + 助成金等入金 + Phase 0非希薄化資金`で月次ロールフォワードする。営業C/Fは現段階では営業利益の代用であり、税金、運転資金増減、減価償却調整は未反映とラベルで明示する。残高行は月合計を出さない。

表の金額セルは右寄せ・等幅数字・14px以上とし、セル内に`M`や`¥`を繰り返さない。表の外へ`単位：百万円`を一度だけ置き、負数は括弧表記、ゼロと未計画はダッシュ表示にする。月列はdesktop/mobileとも同じ横長表を内部スクロールして読む。
