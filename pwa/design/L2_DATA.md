...
|---|---|---|---|---|---|
| M-1 **monthly report** | PJ 月次レポート本文 | `monthly_reports` / `monthly_reports_external` | **Claude Code Routine `amd-os-l2-monthend-evidence` Phase A** (= Fable 5固定、kaku-report必須、月末候補日16:00 JST、最終日だけ実行)。PWA manual routeは人の編集用、AMD-Report GAS R313と旧ローカルScheduled Taskは廃止 | `pwa/scheduled-tasks/amd-os-l2-monthend-evidence/SKILL.md` + `pwa/scheduled-tasks/amd-os-l2m1-monthly-report/SKILL.md` + `pwa/scheduled-tasks/shared/kaku-report/SKILL.md` + `pwa/scripts/ms_progress_review_tool.mjs` | ✅ Claude UI登録済み。提出版は `monthly_reports_external` だけを正本とし、社内final/draftへfallbackしない |
