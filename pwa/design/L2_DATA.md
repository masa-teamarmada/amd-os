...
| **M-1〜M-3** | Claude Code Routine `amd-os-l2-monthend-evidence` (Fable 5固定) | 毎月25日16:00 JST発火 (cron UTC `0 7 25 * *`)、Phase 0で25日判定、17:00完了 | M-1 M-2 M-3 |
| **W-1** | Claude routine `amd-os-l2-weekly-vc-funding-signals` | weekly Saturday 09:00 JST (`0 9 * * 6`)、週 +1 | W-1 |
| **H-1** | MMOマシン Windows Task Scheduler `amd-os-l6-meeting-flow-launcher` → `codex exec` Live launcher | 毎時 09:00-21:00 JST、Claude routine 化しない。並行実行は維持し、待機・実行ロック・別runを理由にしたskipは禁止。H-1本体はsanitized reportとOS通知の成功直後に非LLM helperで自身をアーカイブし、日次スレッド操作をしない。reviewer (`amd-os-l6-meeting-reviewer`) は未集約reportとreview結果をローカルreport・集約台帳・automation memoryへ確定し、日次taskを作らない。LaunchAgent `jp.teamarmada.codex-h1-thread-watchdog` はreported completion markerがあるH-1/reviewerだけをsession実体の有無にかかわらず回収し、markerのないrunを時間で閉じない | H-1 |
...
| **M-1** | M-1 | monthly_reports / monthly_reports_external | Claude Code Routine `amd-os-l2-monthend-evidence` (Fable 5 + kaku-report) | UI登録済み。旧ローカルScheduled Taskは廃止。M-1〜M-3を1本で依存順実行 |
| **M-2** | M-2 | XRL根拠 | Claude Code Routine `amd-os-l2-monthend-evidence` (Fable 5) | M-1保存・再読込後にcandidate生成 |
...
| **Slack** | 074b (threads → meeting summary) / PWA `/api/sources/slack/collect` / `ms_progress_review_tool collect-slack` | ✅ source refs 回収まで復旧。active 5 PJ × 202603-202605 を `source_cache(source='slack')` へ backfill 済み。取り込み対象は `project_slack_sources` を正とし、1PJが複数チャンネル・複数ワークスペースを持てる | monthsBack=6 自動化 + Slack source refs から各L2抽出への接続強化 |
...
|---|---|---|---|---|---|
| M-1 **monthly report** | PJ 月次レポート本文 | `monthly_reports` / `monthly_reports_external` | **Claude Code Routine `amd-os-l2-monthend-evidence` Phase A** (= Fable 5固定、kaku-report必須、毎月25日16:00 JST、25日だけ実行)。提出版はPJごとに同じPJの直前月実提出版を構造正本とし、`formatMatch=true` のときだけ保存する。前月なし・構造変更は人の承認へ回す。PWA manual routeは人の編集用、AMD-Report GAS R313と旧ローカルScheduled Taskは廃止 | `pwa/scheduled-tasks/amd-os-l2-monthend-evidence/SKILL.md` + `pwa/scheduled-tasks/amd-os-l2m1-monthly-report/SKILL.md` + `pwa/scheduled-tasks/shared/kaku-report/SKILL.md` + `pwa/scripts/ms_progress_review_tool.mjs` | ✅ Claude UI登録済み。提出版は `monthly_reports_external` だけを正本とし、社内final/draftや他PJ書式へfallbackしない |
...
| M-2 **XRL根拠** | AMD Score / XRL 算定に使う構造化根拠。`project_founding_members` は HRL 評価のベース = **関連メンバー** リストで、`category in ('amd','startup','university')` (= AMD 伴走 / 該当SU 社員・創業候補 / 大学キーパーソン) を HRL 算入対象にする。VC / 顧客 / 行政 / 産業パートナーは HRL根拠外として `status='invalid'` 化。AMDメンバーは `members.code_name` で記録 (フルネーム / 姓のみ表記は重複として invalid)。`projects.project_category='ecosystem'` は AMD Score 対象外 | `project_founding_members`, `project_xrl_evidence`, `project_xrl_log`, `amd_score_inputs.xrl_notes` | Claude Code Routine `amd-os-l2-monthend-evidence` Phase B。M-1保存・再読込後に `amd-os-l8-xrl-evidence-extract` を実行し、承認済みhelperでcandidate化 | Claude Code Routine + non-LLM helper | ✅ UI登録済み。詳細 [xrl_evidence.md](xrl_evidence.md) |
| D-6 **経営ハイライト** | MS進捗より上位の、経営上の重要方針・事業上の進捗・戦略転換・提携・資金・知財/規制・重要リスク・次の一手。PJ cockpit のMSリスト直下に表示する。外部リサーチは通知で採用したものだけ別棚へ残す | `project_strategy_signals` + `l2_notifications(l2_kind='project_strategy_signal')` | internalは Codex automation `amd-os` (= daily 03:20 JST)。外部公開情報は `tsukuyomi-external-research` (= 平日09:00 JST)。双方 `/Users/masa/.codex/automations/amd-os/strategy-signals-outbox/` → non-LLM applier `ms_progress_review_tool.mjs apply-outbox-dir`。初期backfillは `scripts/backfill_strategy_signals_from_activities.mjs` → `ms_progress_review_tool.mjs apply-outbox` | Codex automation / PWA | ✅ internalはcockpitの「重要な動き」。external_researchは全履歴重複排除→OS通知「採用/見送り」→confirmedだけ「採用リサーチ」。旧外部リサーチSlack配信は停止。詳細 [project_strategy_signals.md](project_strategy_signals.md) |
...
| **M-3** | M-3 **Management Monthly Signal Evaluation** | Management予実表から毎月25日に作る経営シグナル評価 | `company_management_signal_reviews` | Claude routine `amd-os-l2-monthend-evidence` 対象 (= M 群、毎月25日 17:00 完了) |
...
| **D-15** | **Important Evidence** | 5生データとPDF / Office / Google文書本文から、決算・ガバナンス・契約・資金・補助金・技術・計画・リスク・期限等を共通候補化。内容hash、PJ帰属、全所在lineage、field provenanceを残す | `l2_coverage_gaps`、採用後`project_important_evidence` | Codex collector + meaning extraction + deterministic validator → `amd-os-ms/outbox.coverageGaps` → non-LLM applier → 通知採否。LSTは最初の回帰例、27 PJ dry-run済 |
...
| **09:15 daily** | `cron/proactive-todo-extract` | **先手TODO lifecycle**。明示期限超過のred昇格と3日経過blockedの復帰だけ。新規TODOは作らない。汎用TODOの自動生成は停止中。既存 `amd-os-proactive-heartbeat` は10:15–20:15 JSTの毎時15分にL2 candidateの最終採否カードだけをレビューする。正本は `pwa/spec/2-4-proactive-todo-current-spec.md` / `pwa/spec/3-7-notifications-current-spec.md` | PWA non-LLM cron + Codex automation |
...
|---|---|---|---|---|
| M-1 monthly_reports | AMD-Report GAS R313 / PWA heavy route / 旧ローカルScheduled Task | Claude Code Routine `amd-os-l2-monthend-evidence` Phase A (Fable 5 + kaku-report) → validated outbox → helper → readback | 毎月25日16:00 JST、25日だけ実行 | ✅ UI登録済み。R313 triggerと旧local taskは使わない |
...
| M-2 XRL 根拠 | 旧Cloud routine案 / PWA LLM route | Claude Code Routine `amd-os-l2-monthend-evidence` Phase B。M-1完了後に `amd-os-l8-xrl-evidence-extract` → candidate | month-end | ✅ UI登録済み |
...
**管理場所**: M-1〜M-3はClaude Code Routines UIとvalidated outbox/helper/readback、D-5/D-6/D-7は各current writerの履歴とoutbox/applier、D-1〜H-1は各行のcurrent writerを確認する。D-7の `pwa/bzm` 追記はapproved後のlocal BZM applierとgit commit/pushが別段階。古いlocal taskや過去trigger IDは履歴確認用で、復旧の主導線ではない。
...
| 日付 | 変更 |
|---|---|
| 2026-08-12 | **D-15 Important DocumentsをLST再現ケースで追加**。Driveだけにある正式PDFが、PDF本文未取得・PJ一語上位検索・H-1/D-14の対象境界で漏れていたため、会社/本文/親/MIME/期間/監査の6面分類、内容hash重複排除、全所在lineage、field provenance、会計区分、BZM接続候補を実装。候補→通知採否→追記型正本を守り、新しい定期automationは登録しない。 |
| 2026-08-12 | **D-15をImportant Evidenceへ訂正**。決算書だけの抽出器を撤回し、5生データ、PDF/Word/Excel/PowerPoint/Google文書/textの共通reader、幅広い重要カテゴリ、強いPJ帰属anchor、本文hash重複排除、意味抽出fieldの原文照合へ一般化。LSTは特別分岐でなく最初の回帰fixtureとし、27 PJをdry-runした。 |
...
| 2026-09-09 | **SlackタブをSlackと同じ読み方に寄せ、発言者を名前で出す**: 並びを新しい順から**古い順 (上が古く下が新しい)** へ変更し、日付セパレータ・アバター・同一発言者の連続投稿のまとめ (5分以内) を入れた。チャンネルは**既定で1つだけ選択**し、混ぜて読みたい場合だけ「まとめて見る」を選ぶ (混在すると話が追えないため。まさ指摘 2026-09-09)。添付ファイルは種別アイコン付きのカードで並べ、Slackの元ファイルへリンクする (Slackのファイル実体URLは認証必須なのでプレビューはしない)。発言者名は SolvioraX 側が Slack IDのままだったので、アプリへ `users:read` を追加し、取り込み時に `users.info` で表示名を引いて `metadata_json.user_name` / `thread_replies[].user_name` に保存する。表示は AMDメンバー表(`members.slack_id`) → 取り込んだ表示名 → Slack ID の順で解決する。`users:read` が無いワークスペースでは解決を諦めてIDのまま出し、取り込み自体は止めない。 |
| 2026-09-09 | **取り込んだSlackをPJコックピットで読めるようにする (Slackタブ新設)**: 取り込みは動いているのに、貯めた会話がOSのどの画面にも出ていなかった (H-1 が 2026-09-04 のVercel消費事故で停止中のため、議事録経由でも出てこない)。`GET /api/slack/messages` (`requireMember` / `Cache-Control: private, max-age=60`) と参照系クライアント層 `lib/slack/slack-messages-client.ts` を追加し、進捗管理グループに `slack` タブを新設した。月セレクタ・チャンネル絞り込み・本文検索・スレッド返信の開閉・Slack元発言へのリンク・最終取り込み時刻を出す。**保存側は `metadata_json.text_full` と `metadata_json.thread_replies` に全文を持たせ、`content_text` は従来どおり700字snippetのまま据え置く** (L2抽出が読むのは `content_text` なので、全文保存でLLMへ渡す量とトークン消費を増やさない)。SolvioraXは90日で履歴が消えるため、全文を持つのはOS側が唯一の読み場所になるから。発言者は `members.slack_id` → `code_name` で解決し、社外の人はSlack IDのまま出す。参照系契約は `scripts/check_reference_data_cache_contract.mjs` に登録済み。 |
| 2026-09-06 | **Slack取り込みをdaily自動化し、SolvioraX側はまさの参加チャンネルだけに絞る**: `/api/cron/slack-source-sync` を `0 21 * * *` (JST 6:00) で追加。`project_slack_sources` に enabled 行を持つ全PJの当月分を `source_cache` へ upsert し、月初5日までは前月も締め直す。LLM非使用。daily にした理由はPJコックピットに古い会話を出さないため (まさ確定 2026-09-06)。あわせて **permalink をURL規則 (`https://<team>.slack.com/archives/<ch>/p<ts>`) で組み立てる**よう変更し、`chat.getPermalink` のメッセージ単位呼び出しを廃止した (100件で100リクエストになり、daily化するとレート制限と実行時間上限に当たるため。ワークスペースのドメインが取れない場合だけ従来のAPIへfallback)。収集本体は `lib/sources/slack-project-collect.ts` に切り出し、手動の `/api/sources/slack/collect` と cron が同じ経路を通る。SolvioraX側の取り込みは **まさが参加している `00_全体_連絡` / `01_定例` / `20_商談_アポ` の3チャンネルだけ**に限定し、残り6チャンネルは `enabled=false`。つくよみ本体も一度アンインストールしてこの3チャンネルだけへ入れ直した (公開チャンネルから抜けるには `channels:manage` が必要で、読み取り専用の建付けを崩すため)。再インストールでトークンが変わるので `SLACK_BOT_TOKEN_SOLVIORAX` も更新済み。 |
| 2026-09-05 | **SXの取り込み元にSolvioraXワークスペースを追加**: SolvioraX (`T0A7JFY6U9H`) 用のSlackアプリ「つくよみ」(`A0BV7LLTD8S`) を読み取り専用スコープ (`channels:history` / `channels:read` / `channels:join` / `groups:history` / `groups:read`。**`chat:write` は付けない = このワークスペースへは投稿しない**) で作成・インストールし、公開9チャンネル全部へ参加させた。トークンは Vercel production の `SLACK_BOT_TOKEN_SOLVIORAX`。`project_slack_sources` に `p21 × solviorax` の9行を登録済みで、SXは team ARMADA の `#p21_sx` と合わせて10チャンネルから吸い上がる。**SolvioraXはSlackフリープランなので、90日より古いメッセージはAPIからも取れない**。取り込みを止めると復元できないため、月次collectを欠かさないこと。石原先生 (ワークスペース管理者) の導入合意は 2026-09-04 にまさが取得済み。 |
| 2026-09-05 | **Slack取り込みを複数チャンネル・複数ワークスペース対応にする**: `project_slack_sources` (project_id × workspace_key × channel_id) を新設し、既存 `projects.slack_channel_id` の12PJを `workspace_key='armada'` として初期投入。`/api/sources/slack/collect` はこのテーブルの enabled 行を順に回し、チャンネルごとに `slackTokenForWorkspace()` でトークンを解決する (`armada` は従来の `SLACK_BOT_TOKEN`、それ以外は `SLACK_BOT_TOKEN_<大文字>`)。行が無いPJは従来どおり `projects.slack_channel_id` へfallback。1チャンネルが失敗しても他は続行し、内訳を `channels[]` で返す。`projects.slack_channel_id` は引き続き通知の宛先として使う。背景はSXが team ARMADA の `#p21_sx` と SolvioraX ワークスペースの両方を読む必要があるため。 |
