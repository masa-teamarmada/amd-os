...
| **H-1** | MMOマシン Windows Task Scheduler `amd-os-l6-meeting-flow-launcher` → `codex exec` Live launcher | 毎時 09:00-21:00 JST、Claude routine 化しない。毎時runの並行実行は仕様として維持し、待機・実行ロック・別runを理由にしたskipは禁止。報告は AMD OS 通知箱 (`app_notifications.kind='h1_report'`) へ送る。Codex表示は控えとしてJST日付ごとの `H-1 YYYY-MM-DD 日次まとめ` スレッドへ集約 (registry-first / direct-create 固定、query 付き `list_threads` 禁止、2026-07-20 まさ確定)。毎時runスレッドは **OS通知が成功したら必ずアーカイブする** (日次まとめ追記の成否とは独立の契約)。reviewer (`amd-os-l6-meeting-reviewer`) も同じ日次まとめへ結果を追記し、毎時runをアーカイブする | H-1 |
