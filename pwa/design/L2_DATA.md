...
| **M-1〜M-3** | Claude Code Routine `amd-os-l2-monthend-evidence` (Fable 5固定) | 月末候補日16:00 JST発火 (cron UTC `0 7 28-31 * *`)、Phase 0で最終日判定、17:00完了 | M-1 M-2 M-3 |
...
| **W-Prep** | weekly prep launch | visible prep thread 起動 | Codex automation `w-prep-launch` | 2026-07-29 current: 毎週水曜15:00 JST。実行日から数えて7日後の23:59:59.999 JSTまでの確定 upcoming MTG を Calendar + DB で照合し、DBに無い active/sales PJ の確定MTGは upcomingカード作成後に claim → visible thread作成 → title変更 → pin → DB session保存。厳密な `now()+7 days` で切らず、DBだけを見て完了扱いにしない。Calendar直読みのPJ推定は `CFG_ColorPJHistory` を先に使い、`2025-06-01` 以降の `colorId=4` と `SolvioraX` title alias は SX/p21 として扱う。重複防止は `calendar_event_id` exact identity と `upcoming:<calendar_event_id>` canonical を優先する。worker 第一声は3点完了報告、共有フォルダ資料は HTML 主成果物に統一 |
...
|---|---|---|---|---|---|
| M-1 **monthly report** | PJ 月次レポート本文 | `monthly_reports` / `monthly_reports_external` | **Claude Code Routine `amd-os-l2-monthend-evidence` Phase A** (= Fable 5固定、kaku-report必須、月末候補日16:00 JST、最終日だけ実行)。提出版はPJごとに同じPJの直前月実提出版を構造正本とし、`formatMatch=true` のときだけ保存する。前月なし・構造変更は人の承認へ回す。PWA manual routeは人の編集用、AMD-Report GAS R313と旧ローカルScheduled Taskは廃止 | `pwa/scheduled-tasks/amd-os-l2-monthend-evidence/SKILL.md` + `pwa/scheduled-tasks/amd-os-l2m1-monthly-report/SKILL.md` + `pwa/scheduled-tasks/shared/kaku-report/SKILL.md` + `pwa/scripts/ms_progress_review_tool.mjs` | ✅ Claude UI登録済み。提出版は `monthly_reports_external` だけを正本とし、社内final/draftや他PJ書式へfallbackしない |
...
|---|---|---|---|---|
| M-1 monthly_reports | AMD-Report GAS R313 / PWA heavy route / 旧ローカルScheduled Task | Claude Code Routine `amd-os-l2-monthend-evidence` Phase A (Fable 5 + kaku-report) → validated outbox → helper → readback | 月末候補日16:00 JST、最終日だけ実行 | ✅ UI登録済み。R313 triggerと旧local taskは使わない |
