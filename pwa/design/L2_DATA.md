...
| **09:15 daily** | `cron/proactive-todo-extract` | **先手 TODO control layer**。開催済みMTG `next_actions[]` と、PJ `report_emails` から届く Gmail 期限つき依頼 (`email_action_request`) を `proactive_todos` へ冪等 upsert。予定MTG準備は Codex の W-Prep / prep worker へ一本化し、`next_meeting_prep` は新規生成せず既存 open / blocked を dismissed へ退避する。LLM非使用で、メール本文全文・URL・パスワードは保存しない。正本は `pwa/spec/2-4-proactive-todo-current-spec.md` | PWA non-LLM cron |
