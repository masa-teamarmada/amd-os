...
| 2026-06-19 | **H-1 recurring 予定MTGカードを series card 化**。`calendar-sync` は `recurring_event_id` が取れる series を cadence 問わず次回1件だけ残し、2件目以降を `recurring_series_future_occurrence` として skip。`CockpitMeetingSummary` / `HudCockpitMeetingSummary` も既存DB rowを series ごとに次回1件だけ表示する。 |
