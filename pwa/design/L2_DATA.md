...
| ⑯ **Management Monthly Signal Evaluation** | `/management-score` 月次試算表下に出す、月末時点の会社経営状態評価。予実表の数字を再掲せず、状態ラベル/アイコン、自然文の一読評価、判断コメントへ変換する | `company_management_signal_reviews`。入力は `amd_management_score_snapshots`, `amd_management_score_evidence`, `company_budget_actual_monthly`, `company_budget_variance_notes`, L2⑨⑭⑮ など | 月末最終日 17:00 JST の Codex / subscription automation `amd-os-l16-management-monthly-signal-evaluation` 候補。PWA/Vercel background LLM cron は復活させない | `pwa/scheduled-tasks/amd-os-l16-management-monthly-signal-evaluation/SKILL.md` | 🟡 修正版UIを最終UX方向として正本化。schemaは `status_label` / `status_tone` / `status_icon` / `headline` / `summary` / `sections[]` / `source_confidence` へ拡張設計 |
...
| **月末最終日 17:00** | `amd-os-l16-management-monthly-signal-evaluation` candidate | **L2⑯ Management Monthly Signal Evaluation**。`/management-score` 月次試算表下の経営評価文を、数字再掲ではなく状態ラベル/アイコン・自然文評価・判断コメント・次に見ることへ変換し、保存済みL2として更新する | Codex / subscription automation candidate + management review |
...
6. **2026-06-03**: L2⑯ Management Monthly Signal Evaluation を追加。`/management-score` の月次試算表下に出す月末経営評価で、数字の再掲ではなく、状態ラベル/アイコン・自然文の一読評価・判断コメント・次に見ることを出す。修正版UIの方向を正とし、L2 schemaは `status_label` / `status_tone` / `status_icon` / `headline` / `summary` / `sections[]` / `source_refs_json` / `source_confidence` / `generated_at` / `reviewed_at` / `codex_thread_id` / `automation_id` を持つ形へ寄せる。増分 migration 案は `124_management_signal_reviews_l2_schema.sql`、automation正本は `amd-os-l16-management-monthly-signal-evaluation/SKILL.md`。
...
| ⑯ Management Monthly Signal Evaluation | `/management-score` 修正版UIの経営シグナル評価 | Codex / subscription automation candidate。数字を再掲せず、経営状態・先3か月の温度感・費用の見方・次の営業判断・乖離の読み方・今すぐ見ることへ変換する | 毎月末日 17:00 JST 候補 | 🟡 UI方向は正。保存schemaと月末生成/更新の責務をL2として確定する |
...
| ⑯ Management Monthly Signal Evaluation | `/management-score` の修正版UIは本番確認済み。状態ラベル/アイコン、自然文評価、判断コメントのUXをL2⑯の正方向として採用 | `company_management_signal_reviews` を `status_label` / `status_tone` / `status_icon` / `headline` / `sections[]` / `source_confidence` 中心へ拡張し、月末17:00の生成/更新と過去ログ折りたたみを確定 |

## L2 候補

現時点で実装/運用候補が残る L2 は、⑩ Textbook Insights、⑪ Atlas Signals、⑫ Macrotrend Evidence / Index、⑬ Member Weekly Activities、⑭ Finance Ops Evidence、⑮ VC News / Funding Signals、⑯ Management Monthly Signal Evaluation。⑩はDB/API/outbox/local applier の最小実装済みだが、実 schedule と BZM 追記レビュー運用は partial。⑪⑫は daily consolidated evidence 対象、⑬⑮は privacy / source cadence が違うため別 weekly 候補、⑭は finance non-LLM cron + admin review を primary にする。⑯は修正版UIのUX方向を正とし、保存schema・月末17:00生成・過去ログ化をL2として詰める。
...
| 日付 | 変更 |
|---|---|
| 2026-06-03 | **L2⑯ Management Monthly Signal Evaluation を追加**。`/management-score` 月次試算表下の経営シグナル評価を、数字再掲ではなく状態ラベル/アイコン・自然文の一読評価・判断コメント・次に見るべきことへ変換するL2として定義。修正版UIの方向を正とし、schemaは `status_label` / `status_tone` / `status_icon` / `headline` / `summary` / `sections[]` / `source_refs_json` / `source_confidence` / `generated_at` / `reviewed_at` / `codex_thread_id` / `automation_id` を持つ形へ寄せる。 |
