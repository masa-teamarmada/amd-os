...
|---|---|---|---|
| **D-1〜D-11** | Claude routine `amd-os-l2-consolidated-evidence` | daily 08:00 JST (`0 8 * * *`)、平常日 run +1 | AMD Protocol / MS Progress / Project Knowledge / Member Knowledge / Registry Diff / Strategy Signals / Textbook Insights / Atlas Signals / Macrotrend / Member Activity Evidence / Media Mentions |
...
|---|---|---|---|---|
| **D-1** | ② | AMD Protocol | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み |
| **D-2** | ③ | MS進捗 | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み。旧MMO個別automationはPAUSED |
| **D-3** | ④ | PJナレッジ | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み。旧MMO個別automationはPAUSED |
| **D-4** | ⑤ | メンバーナレッジ | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み。旧MMO個別automationはPAUSED |
| **D-5** | ⑦ | OS台帳差分 | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み |
| **D-6** | ⑨ | 経営ハイライト | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み |
| **D-7** | ⑩ | Textbook Insights | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み。approved後のBZM local applierは別段階 |
| **D-8** | ⑪ | Atlas Signals | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み |
| **D-9** | ⑫ | Macrotrend Evidence / Index | Claude routine `amd-os-l2-consolidated-evidence` + PWA non-LLM cron `macro-aggregate-indicators` | 差分なし: Claude UIでACTIVE / next run確認済み。index集計cronはPWA non-LLMとして別枠 |
| **D-10** | ⑬ | Member Activity Evidence | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み |
| **D-11** | ⑭ | Media Mentions | Claude routine `amd-os-l2-consolidated-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み |
| **D-12** | freee | freee Transaction Actuals / 月次実績取込 | PWA non-LLM cron `/api/cron/management-score-raw-data?includeFreee=1` | 差分なし: `pwa/vercel.json` で daily cron 定義済み。Claude routine / Codex automation には載せない |
| **M-1** | ① | monthly_reports | Claude routine `amd-os-l2-monthend-evidence` | 差分なし: Claude UIでACTIVE / next run確認済み。MMO暫定automationはPAUSED |
...
| ⑥ **MTGサマリ** | calendar event 1 回ごとの decided/progress/nextActions/risks (PK = calendar event id)。開催前/当日の準備ブリーフは `source_kinds='upcoming'` で同じ欄に出す。未来/同日Calendar予定同期では、`today 00:00 JST` から60日先までの確定予定をカード化し、PJ Drive folder の会議日サブフォルダ・議案資料・予実表・招集通知を `関連Drive資料` として載せる。ただし weekly recurring MTG は series ごとに次回1件だけ同期・表示し、それ以降の future occurrence は非表示/skip にする。逆向きの一次防御として、OS上のMTGカード/議事録に日時・場所・持参物・返信/宿題がある場合は `POST /api/meeting-calendar/upsert-plan` dry-run で Calendar upsert payload と duplicate match を作る。担当者が明確な MTG / OS task / Gmail / Slack 由来TODO は `POST /api/task-calendar/schedule-plan` dry-run で owner + まさ の共通空き枠に `+<PJコード> <task>` 作業枠候補を作り、実write成功後に内部Slack DM nudge候補も出す。画面共有・表・スライドなど自動メールに落ちない素材は `meeting_assets` + private Storage `meeting-assets` に添付し、`narrative_md` へ Markdown 画像/リンクとして挿入できる。 | `project_meeting_summaries` + `meeting_assets` | ~~Phase 3 = GAS 153 毎時 polling~~ ⛔ **2026-05-22 停止** → ✅ **Codex Desktop automation `amd-os-l6-meeting-flow` / repo正本 `pwa/scheduled-tasks/amd-os-l6-meeting-extract/SKILL.md`**。議事録抽出は過去60-180分終了events、予定MTGカードは `POST /api/meeting-prep/calendar-sync`、MTGカード由来Calendar一次防御は `POST /api/meeting-calendar/upsert-plan` dry-run、task作業枠は `POST /api/task-calendar/schedule-plan` dry-run、手動/仮置き予定MTG準備は `POST /api/meeting-prep`。`calendar-sync` は `drive_files` metadata と recurring metadata を受け取り、Driveを読みに行かずに予定カードへ反映する。MTG添付は PWA `POST /api/meeting-assets` で admin 手動保存し、LLM 抽出は routine 側の入力に寄せる。設計 [meeting_summaries.md](meeting_summaries.md)、マニュアル [8-3章](../manual/8-3-l2-extraction-routines-spec.md) | (旧) 本体GAS `152_NavigatorCron.js` + `153_MeetingHourlyTrigger.js` + `074_MeetingSummaryRepo.js`。iOS APNs 通知用 `meeting_notifications` テーブル (PK=meeting_id) は新 routine も維持予定 | ✅ 稼働。2026-05-27 に KUTE/CLG の予定カード生成境界と CLG取締役会Drive資料同期を本番検証済み。2026-05-29 に weekly recurring 予定MTGは次回1件だけ表示する guard を追加。2026-06-09 に MTGカード由来Calendar upsert plan dry-run guard と task作業枠schedule plan guard を追加 |
...
| **D-10** | ⑬ **Member Activity Evidence** | Dashboard / MyPage「今週やったこと」の根拠 | `member_activities` | Claude routine `amd-os-l2-consolidated-evidence` 対象。**daily 化** (= weekly廃止) |
| **D-11** | ⑭ **Media Mentions** | メディア掲載・公開露出 | `project_media_mentions` / `news_mention` notifications | Claude routine `amd-os-l2-consolidated-evidence` 対象 |
| **W-1** | ⑮ **VC News / Funding Signals** | VC・資金調達・投資家動向 | `vc_news` / funding signal tables | Claude routine `amd-os-l2-weekly-vc-funding-signals` 対象 |
...
| **D-12** | freee **Transaction Actuals** | freee取引履歴を月次試算表の実績値へ入れる非LLM同期 | freee transactions / `company_actual_monthly` | PWA non-LLM daily cron / freee sync。Claude routine / Codex automation に混ぜない |

**重要**: L2番号は current truth として、D-10 = Member Activity Evidence、D-11 = Media Mentions、W-1 = VC News / Funding Signals。過去の「L2⑪ = member weekly」や「L2⑭ = Finance Ops Evidence」記述は誤り。cadence 束ねの新ナンバリング (D / M / W / H) は上表と本章冒頭「cadence ベース束ね設計」を正本にする。
...
| **毎日 09:00-21:00 毎時** | `amd-os-l6-meeting-flow` (= Windows MMO Codex Desktop automation、repo正本 `pwa/scheduled-tasks/amd-os-l6-meeting-extract/SKILL.md`) | **L2 ⑥ MTG サマリ + MTGフロー**。過去60-180分終了eventsの議事録抽出、今日0:00 JSTから60日先の確定Calendar予定カード同期 (weekly recurring は series ごとに次回1件のみ)、MTGカード由来Calendar upsert plan dry-run、MTG / OS task / Gmail / Slack TODO のtask作業枠schedule plan dry-run、作業枠作成後の内部Slack DM nudge、Drive関連資料 metadata 反映、次MTGカード / Slack nudge / TODO→cockpit / Calendar作業枠 / 資料即生成 / Gmail draft | Windows MMO PC Codex Desktop + PWA `calendar-sync` / `meeting-prep` / `meeting-calendar/upsert-plan` / `task-calendar/schedule-plan` |
...
| 2026-06-09 | **L2⑥ MTGカード由来 Calendar upsert plan dry-run / task作業枠schedule plan dry-run を追加**。`POST /api/meeting-calendar/upsert-plan` は Calendar write を行わず、MTGカードと既存Calendar metadataから `update_existing` / `create_candidate` / `review_required` / `hold`、`extendedProperties.private` metadata、08:00-21:00広めブロック、強リマインド候補を返す。`POST /api/task-calendar/schedule-plan` は MTG / OS task / Gmail / Slack 由来TODO、owner + まさ のbusy window、既存 `+` event metadata から `+<PJコード> <task>` 作業枠候補、`already_scheduled` 重複guard、内部Slack DM nudge候補を返す。外部attendeesは招待せず `sendUpdates=none` 前提。 |
