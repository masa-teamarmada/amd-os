...
| **W-Prep** | weekly prep launch | visible prep thread 起動 | Codex automation `w-prep-launch` | 2026-07-09 current: 毎週水曜15:00 JST。今後7日以内の確定 upcoming MTG を Calendar + DB で照合し、DBに無い active/sales PJ の確定MTGは upcomingカード作成後に claim → visible thread作成 → title変更 → pin → DB session保存。DBだけを見て完了扱いにしない。worker 第一声は3点完了報告、共有フォルダ資料は HTML 主成果物に統一 |
