...
| **D-15** | **Important Documents** | 事業報告・計算書類・附属明細・監査報告を、本文・親フォルダ・会社・期間・監査で分類。内容hashで1候補へ束ね、全所在lineageとfield provenanceを残す | `l2_coverage_gaps`、採用後`project_important_documents` | Codex local collector + 決定論的extractor → `amd-os-ms/outbox.coverageGaps` → non-LLM applier → 通知採否。LSTで実証、全PJ scheduleは未登録 |
...
| **09:15 daily** | `cron/proactive-todo-extract` | **先手TODO lifecycle**。明示期限超過のred昇格と3日経過blockedの復帰だけ。新規TODOは作らない。新規生成ownerは既存 Codex automation `amd-os-proactive-heartbeat`（10:15–20:15 JSTの毎時15分）で、5系統source cache + 開催済み会議要約からapproved `decision` / `masa_action` を直接生成する。正本は `pwa/spec/2-4-proactive-todo-current-spec.md` | PWA non-LLM cron + Codex automation |
