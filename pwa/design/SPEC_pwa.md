...
| `/admin/invoices` | admin 請求書発行。SU × 月の請求状態を `未発行 / 発行済み / 送付済み / 入金済み / すべて` filter で一覧し、未発行行の `発行` / 行詳細の `請求書を発行` から OS → freee の請求書発行を実行する。旧 `/admin/billing` はこの route への互換 redirect のみ |
...
- `/admin/invoices` は未発行を先に処理する請求書発行キューとして残す。主画面は `未発行 / 発行済み / 送付済み / 入金済み / すべて` filter と、`予算 / 報告書 / 立替` の発行前確認だけを出す。旧 `/admin/billing` は互換 redirect のみ。
...
- 対象: 直近 13 か月の `billing_cycles`。PJ は `projects.status IN ('active','ended','frozen')`、終了PJは `end_ym` 以前のみ。
- 状態: `invoice_issued_at` なし = 未発行、`invoice_issued_at` あり = 発行済み、`invoice_sent_at` あり = 送付済み、`payment_confirmed_at` あり = 入金済み。
- 発行前確認: `budget_confirmed_at`、`report_fixed_at`、立替未処理有無だけを小さく表示する。`支払通知 / 報酬支払` など、請求書発行の前提でない全ステップ横並び matrix は戻さない。
