...
| `/project/[projectId]/navigation` | `/workspace`/`/weekly-control`を置換しない別URLのPJ管制ダッシュボード。重要経路を初期展開し、4本柱を期間要約で俯瞰してから、マイルストーン→技術試験・論点・履歴へ展開する階層WBSガントを表示する。ガントは1タスク1本で、予定期間・予定終了・進捗位置・同一バー末尾の遅延を区別し、重要経路の依存は直角線だけで結ぶ。関係先56件は共通7段階軸で比較する。工程、論点→仮説→根拠→検証→判断→action、技術試験、関係先・保有事項を同じ画面で追加・更新できる。`buildSxNavigationViewModel()`が作る最小view modelだけをClient Componentへ渡す |
...
| `/project/[projectId]/cockpit` | PJ コックピット (上 Header + Hero (AMD Score + XRL 横並び) + `進捗管理` / `スコア詳細` + MS / 資料 + 経営ハイライト + ガバナンス + 助成金 + 下段 月次 + MTGサマリ)。`?tab=score-detail` で SPS / R_net / FRL / XRL evidence と XRL チェックリストを直接開く。Header はPJリスト正本からPJメンバー、契約条件、業務委託料、支払い条件、提出物、立替精算の発生額/不可を表示する。資料室はモーダル式で、正本は `workspace_documents` テーブルとprivate Storage `workspace-files`。管理者はfolder内のentryをパンくずへドラッグして上位folderまたは資料直下へ移せるが、既存のserver-side整理検証を必ず通す（詳細は `cockpit.md` / `pwa/manual/2-3-pj-cockpit.md` 「## 資料」節）。旧 `proactive_outbox` 由来のTODO欄は表示しない。`max-w-[1600px]` で画面幅を広く使う。詳細は [`cockpit.md`](cockpit.md) / [`project_strategy_signals.md`](project_strategy_signals.md) |
...
| `/bzm/map` | BZM 2.0 理論マップ (論証台帳)。DBだけを正本に0件から本人が育てる。admin はマップ空白クリックでノードを作り、ノードを別ノードへ重ねて接続し、線クリックで接続解除する。作成・接続・削除確認panelはマップ内に表示し、member は閲覧のみ。旧Markdown 21ノード / 34関係は履歴資産で自動表示しない。「真理マップ」ではなく、ノード数・接続数は真偽・確信度を表さない。詳細は [`/spec/2-6-bzm-theory-map-current-spec`](/spec/2-6-bzm-theory-map-current-spec) |
...
| `/reimburse` | 立替精算 |
| `/admin/settings` | Operations Settings。admin限定で Raw Data / L2 Data / Cron Control を一覧化する。停止中cronはここに旧頻度・入力・出力・停止理由を表示する。`/settings` は一般ユーザー誤操作防止のため削除。PJ別の週次 Slack レポート配信は `/admin/projects` のPJ台帳で管理する |
| `/admin/change-history` | OS全体のデータ変更履歴。admin限定で、実行者・日本時間の日時・PJ/対象・人が読める変更要約を密な台帳で表示する。メンバー台帳は氏名またはコード名と`member_id`で対象者を示し、`last_login_at`の連続更新は同一メンバーごとに件数と実効差分へまとめる。日時型の差分値もAsia/Tokyoで表示し、同一秒内の差分を区別するときだけミリ秒を付ける。同一transaction等の安全な連続行は件数にまとめ、詳細展開で元の各履歴・ID・変更前後・「この変更を戻す」を確認する。DB triggerを正本とし、秘密値は伏せる。現在値が履歴の変更後と一致するときだけ逆操作し、戻し操作も元履歴へのリンク付きで記録する。`project_management_field_audit`の二重監査行は生ログに残すが台帳表示から除外する |
| `/atlas` | シグナル & ストーリー一覧 |
...
| `/reimburse` | 立替精算。PWAから申請/編集/削除、領収書添付、PM承認、admin承認まで実行。申請/編集は `/api/reimbursements` 経由で server-side 保存。status flow: `submitted` → `pmApproved` → `approved`。**新規申請 (`mode='create'`) の時だけ** admin (`members.is_admin=true` かつ `status='active'` かつ `slack_id` あり、申請者本人は除外) へ Slack DM を送る (`src/lib/reimbursement-notify.ts`)。同時に**申請者本人へも受付控えの DM** を送る。編集では鳴らさない。承認されないまま放置された分は `cron/reimbursement-reminders` (毎日 JST 10:00) が拾い、**前回送信から 24 時間経過するごとに admin 全員 + 申請者本人へ再送**する (`approved` / `rejected` / `paid` になった時点で自動停止)。添付領収書は Supabase Storage 保存と同時に共有ドライブ `ARMADA/a3_backoffice/立替精算領収書/<発生日YYYY-MM>/` へも複製する (`src/lib/reimbursement-receipt-drive.ts`)。Slack 送信・Drive 複製はどちらも best effort で、失敗しても申請は成立し `warnings[]` に入れて返す。承認待ちカードと Slack DM には交通費の移動区間 (`transport_from` → `transport_to` / `transport_mode` / `transport_trip`) を出し、往復申請では片道単価も併記する。admin 宛 DM には status に応じた承認ボタンが付き、押下は `POST /api/slack/reimbursement-decision` (共有シークレット `SLACK_DECISION_SECRET`、呼び出し元は `gas/081_SlackInteractive.js`) 経由で Supabase 正本へ反映される。交通費の金額欄は**片道入力**が仕様で、`transport_trip='round'` のとき API 側で `amount * 2` する |
| `/admin/invoices` | admin 請求書発行。締め済み稼働月のうち請求額がある行だけを、きよが処理する。初期表示は `未完了` (= `発行待ち / 要確認 / 設定不足 / 過去滞留`) とし、必要に応じて `発行済み / 送付済み / 入金済み / すべて` も見られる。対象月の `発行待ち` だけ `発行` / `請求書を発行` から OS → freee の請求書発行を実行でき、freee取引先未設定、請求額なし、請求月 (`invoice_ym || ym`) が対象月より古い未発行は発行対象から分ける。報告書FIXと立替精算は請求書発行の blocker にしない。`要確認 / 設定不足 / 過去滞留` はクリックで発行前チェックモーダルを開き、設定不足ではfreee取引先をプルダウンで選んで保存できる。請求書発行モーダルとfreee件名は `client_name` を使い、AMD内部の `project_name` / `project_id` を出さない。`budget_yen` は請求額判定に使わない。旧 `/admin/billing` はこの route への互換 redirect のみ |
| `/admin/payouts` | 報酬支払。メンバー支払月を選び、`billing_cycles.ym` (= 稼働月) を `/admin/projects` の支払条件 (`projects.payment_due_rule`) から支払月へ自動判定して報酬確定済みcycleを集約する。`billing_cycles.invoice_ym` はクライアント請求月なので、メンバー支払月判定には使わない。通常表示は `billing_cycles.reward_summary_json` の報酬キャッシュを読むだけにし、明示的な「報酬キャッシュ再計算」・発行/送付などのwrite処理・日次 `payout-reward-cache-refresh` cron だけが再計算する。画面を開いただけでは保存しない。画面の支払額とPDF生成元は最新計算額を正にし、`monthly_reward_payout.total_pay` / `payout_notices.total_yen` は税抜スナップショットとして、夜間の `payout-notice-prebuild` または正式PDF発行時に同期する。PWA集約済み明細からの改善版支払通知書PDF発行 (`notice_no` / `pdf_url` / `sent_at`)、契約由来の本契約cap確認、OkuDoor追加開発などの別財布支払分離、縦型PJ収支表、本契約cap超過チェック、後追い予算未確定 / 予算不足 / 失注ステータス警告、入金確認nudge、明細クリックから月次モーダルを開く導線を持つ。報酬額の手入力フォームは置かず、MS / PlanCycle / responsibility から計算できるものだけを支払対象にする。唯一の例外として、旧制度月 (`source_ym <= 202606`) の事前合意済み税抜額は `legacy_reward_payout_amount_override_events` のappend-only eventから支払snapshot/PDFにだけ適用し、MS・報酬cache・stock/carry・将来差額を変更しない。画面は `事前合意額：通常 X円 → 固定 Y円` を表示し、0円なら未送付旧PDFを削除して新規通知書を発行しない。`tag='cap_extra'` のMSは `extraBasePay` として通常枠から分離し、画面では `本契約発生` / `別財布発生` を別表示する。`メンバー別支払` は主作業表としてサマリ直下・報酬債務台帳より上に置き、PDF URL手入力欄は置かず、各行に `支払通知書発行` / `PDF確認` / `送付` を置く。メンバー別支払の `支払額` は画面上で税抜 / 税込を併記し、DB同期値 (`monthly_reward_payout.total_pay` / `payout_notices.total_yen`) は税抜。PDF宛先は `members.contractor_name` (= 未設定時は `member_name` / `code_name`) と `members.member_address`、メンバー側インボイス登録番号は `members.invoice_registration_number` を使い、PDF上のラベルは宛先側・発行者側とも `登録番号`、発行者側の値は AMD のインボイス登録番号 `T7021001064067` を表示する。PDFには振込先欄を出さない。行の `PDF確認` は保存済み正式PDFを開くだけで、PDF生成は行わない。確認用PDFを作る場合は別操作の `確認用PDF生成` を使い、`payout_notices` には保存しない。`payout-notice-prebuild` は支払データ同期後に正式PDFを事前生成する。正式な通知書発行・全員分PDF一括発行・強制再発行は、サーバー側で最新計算額を同期してから実行する。金額変更分、現行テンプレート更新時刻より古い未送付PDF、または `members.updated_at` より古い未送付PDFは再生成対象へ戻す。最新支払計算に対応する明細が無い未送付 `payout_notices` は孤立レコードとして削除し、古いPDFリンクを active な通知書として残さない。送付済み通知書は履歴保護し、`force=true` でも上書きしない。通常の差分は発行時に自動同期するため状態バッジを出さず、開きっぱなしの画面は 60 秒ごとに read-only 再取得して同期状態だけ追随する。月初合意gate・本契約cap blocker がある場合だけ `同期できない` と表示し、admin override または blocker 解消を待つ。一括PDF生成で失敗または強制再発行なのに未送付PDFが未再生成になった場合は `ok: false` として画面に失敗を出す。`送付` は確認モーダル (件名: 支払通知書のご案内 固定 / 本文編集可 / 添付PDF / Bcc: masa+kyoko 固定) を開く前にPDF生成せず、保存済み正式PDFが最新DBと一致し、確認用PDFではなく、未送付であることだけを照合する。そこで「はい・送信」を押すと保存済み正式PDFを添付して `keiri@team-armada.jp` から実メール送信し、成功時に `payout_notices.sent_at` を set する。支払通知書PDFは admin/payouts の支払額を税抜として扱い、GAS PDF生成時に消費税10%を上乗せして税込合計を出す。支払通知書PDFの見た目契約は [`FEATURE_REGISTRY.md`](FEATURE_REGISTRY.md) の `/admin/payouts` に固定し、`test:critical-ui` でGAS側の改善版フォーマット anchor も検査する |
...
| `/admin/kiyo` | きよが月次経理を完了する主作業台。メニュー名・ページtitle・見出しは `きよ`。`01 立替精算` / `02 請求書` / `03 メンバー支払` を `/admin/kiyo?task=...` で切り替え、別routeへ移動せず作業する。立替精算は `/reimburse` と同じ申請・編集・PM/admin承認UI、請求書は `/admin/invoices` と同じ発行前確認・freee取引先保存・発行・取消UI、メンバー支払は `/admin/payouts` と同じ報酬再計算・支払通知書発行・PDF確認・送付・入金確認nudge UIを埋め込む。きよ専用の別write endpointや計算を作らず、各既存componentとserver-side guardを共有する。状態確認だけのread-only台帳や専用画面へのリンク集へ戻さない。3taskの切替UIは丸角カードgridではなく横並びタブ (`role="tablist"` / `role="tab"` / `aria-selected`)。選択状態は下線+文字weightで示し、角丸ゼロ〜最小・影なし・border-onlyにする。PCは1行横並び、mobileはタップ領域44px以上を保った横スクロール1行タブにする。見出し・タブ・埋め込み作業面の余白は4/8/12px系で密にし、`AdminInvoicesPage` / `AdminPayoutsPage` / `ReimburseWorkspace` は `embedded` propで自身の重複見出し（`請求書発行` / `Payouts` / `立替精算` タイトルカード）を省いて用途文だけを出す。専用route (`/reimburse` / `/admin/invoices` / `/admin/payouts`) 側の見出し・write workflow・権限・URLは変えない |
...
| `/admin/private-wiki` | 裏wiki。`private_wiki_entries` を admin-only で読み書きし、AMDメンバー・取引先・クライアント・研究者・外部協力者などの人物単位メモをPJ別に保存する。通常PJ cockpit、公開ページ、研究機関外部workspaceには出さない。追加/編集/archive、検索、PJ/person_kind/status filter、誕生日・出身地・居住地・接点・家族・タブー、`source_kind` / `source_ref` / `source_excerpt` / `confidence` 表示を持つ。旧 `tags` は既存互換の DB 列として残すが UI/API の主導線にはしない。書き込みは `/api/admin/private-wiki` の `requireAdmin()` + service_role 経由 |
...
| `/poc` | PoC案件化。`seeds` と `poc_companies` (= PoC先) を一次入力として持ち、その掛け合わせから相性仮説、ヒアリング論点、謝礼・PoC費用、契約、資金、収益分配、次アクションを `poc_matches` に生成・保存する。議事録本文やURLは保存せず短い参照名だけを持つ。詳細は [`poc_matching.md`](poc_matching.md) |
...
| `cron/member-weekly-activities` | disabled legacy synthesis / evidence route only | — | Anthropic 経路を持つ legacy GET synthesis は active cron から退避。D-10定期は Codex automation が `GET ?mode=evidence` → `POST activities[]` で保存する。Calendarの開始・終了がある時間予定だけを週次実績に自動集計し、Gmail / Slack / Drive は時間へ換算しない |
| `cron/payout-reward-cache-refresh` | `5 18 * * *` | 03:05 daily | `/admin/payouts` の高速表示用に、前月 + 当月から先12か月のメンバー支払月を `projects.payment_due_rule` / `payment_due_day` で判定して対象cycleを集約し、同じ窓内の稼働月cycleも含めて `syncRewardSummariesForBillingCycles()` で `billing_cycles.reward_summary_json` を再生成する。`billing_cycles.invoice_ym` はクライアント請求月なので対象抽出に使わない。LLM/GAS非使用。手動実行は `?ym=YYYYMM` 指定可 |
...
| `cron/freee-payment-sync` | `10 0 * * *` | 09:10 daily | freee会計の収入取引 (`/api/1/deals`, `type=income`) と口座明細 (`/api/1/wallet_txns`, `entry_side=income`) を入金月で取得し、取引先ID・請求番号・入金額・PJ別 `payment_alias` からOSの入金予定と照合。入金済みなら `billing_cycles.payment_confirmed_at` を自動更新し、照合証跡を `billing_log.detail` に保存 |
| `cron/proactive-todo-extract` | `15 0 * * *` | 09:15 daily | 先手TODOの非LLM lifecycleだけ。明示期限超過のred昇格とblocked復帰を行い、新規候補は作らない。汎用TODOの自動生成は停止中。既存 `amd-os-proactive-heartbeat` はL2採否判断レビューを担当する。仕様正本は [`/spec/2-4-proactive-todo-current-spec`](/spec/2-4-proactive-todo-current-spec) |
| `cron/payment-confirm-nudges` | `30 0 * * *` | 09:30 daily | 入金月単位で未入金候補を集め、その中から入金期日が今日 (JST) と一致するPJだけ active admin (`members.is_admin=true`) のSlack DMへ入金確認nudgeを送る。入金日前は確認不能なので送らない。ボタンは「予定通り入金済み」(通常は既存URL confirm。`PAYMENT_CONFIRM_SLACK_INTERACTIVE=1` かつ GAS worker デプロイ済みなら Slack interactive action -> GAS worker -> `POST /api/admin/payment-confirm mode=expected` -> つくよみがSlackスレッド返信) と「金額を入力」(`/payment-confirm`)。LLM非使用なのでLLM系cron停止とは別枠で稼働 |
| `cron/reimbursement-reminders` | `0 1 * * *` | 10:00 daily | 未承認の立替 (`reimbursements.status` が `approved` / `rejected` / `paid` 以外) を対象に、`reminder_last_sent_at` (無ければ `created_at`) から 24 時間以上経過した行だけ、active admin 全員 + 申請者本人へ Slack DM リマインドを送る。送信後に `reminder_last_sent_at` / `reminder_sent_count` を更新。承認・却下・支払済で自動停止、回数上限なし。申請者が admin の場合は admin 宛から除外して本人向け DM に寄せる (二重送信回避)。LLM非使用 |
...
| `reimbursements` | 立替 (status: `submitted` `pmApproved` `approved` `paid` ...)。`receipt_storage_paths` / `receipt_file_names` で private Storage `reimbursement-receipts` の領収書添付を保持 (一次保管)。`receipt_drive_folder_id` / `receipt_drive_file_ids` / `receipt_drive_links` は共有ドライブ a3_backoffice への複製 (二次保管) の控え。Drive 複製失敗時は空のまま残る = 申請は成立している。`reminder_last_sent_at` / `reminder_sent_count` は 24 時間ごとの未承認リマインド (`cron/reimbursement-reminders`) の送信控え。null のときは `created_at` を起点にする |
...
| `project_documents` | 旧PJ cockpit資料のDrive link台帳。UI (`CockpitProjectDocuments`) とAPI (`/api/project-documents/**`) はどこからも参照されていないdead codeだったため2026-08-16 (v3.78.3) に削除した。テーブル自体は `app/api/project/monthly-report-print/route.ts` が月次レポート添付一覧の読み取り専用ソースとして使うため残す。現行のPJ cockpit資料室は `workspace_documents` が正本 |
...
| `poc_companies` | PoC先マスタ。企業、事業所、組合、自治体、施設カテゴリなど、PoCを受ける側の業界タグ、地域、規模感、PoC相性、過去PoC/紹介経路、謝礼メモ、担当、次アクションを保持 |
| `poc_matches` | シーズ x PoC先の案件候補台帳。相性仮説、ヒアリング論点、PoC目標、謝礼、契約、資金、収益分配、状態、優先度を保持 |
...
### 立替精算と請求書発行の境界
- `/admin/invoices` は請求書を発行する画面なので、発行条件は `freee取引先` と `請求額` に絞る。
- 立替精算は `/reimburse` と admin 承認側の責務。申請が締めに間に合わない分は請求書発行を止めず、翌月以降または別精算で扱う。
- `reimbursements.billed_ym` は取引先への請求月。メンバーへの支払通知書では、現存する `payout_notices.reimbursement_ids` を採用済みの正本にする。別月の通知書にある立替は二重計上せず、通知書が削除された立替は締切条件を満たす支払月へ再び自動で載せる。PDF生成で `billed_ym` を書き換えない。報酬が0円でも承認済み立替だけで通知書を生成し、送付前には立替額と明細IDの一致を確認する。
...
- 状態: `payment_confirmed_at` あり = 入金済み、`invoice_sent_at` あり = 送付済み、`invoice_issued_at` あり = 発行済み。未発行行は、請求月 (`invoice_ym || ym`) が対象月より古ければ `過去滞留`、対象月で freee取引先未設定なら `設定不足`、金額を読めなければ `要確認`、freee取引先と請求額がそろったものだけ `発行待ち`。稼働月が古くても `invoice_ym` が対象月なら、今月発行分として扱い `過去滞留` にはしない。
- 請求額: 明細合計 (`invoice_base_lines_json`) → 確定請求額 (`budget_reported_amount`) → 月額固定契約額 (`projects.fee_amount`) の順で見る。PJ 予算 (`budget_yen`) は AMD 側の原資/報酬予算なので、請求額 fallback には使わない。
- 発行条件: `取引先 / 金額` だけを小さく表示する。`報告書FIX / 立替精算 / 支払通知 / 報酬支払` など、請求書発行の前提でない全ステップ横並び matrix は戻さない。
- 発行モーダル: 件名、基本明細行、契約月額との差分確認、前月明細引き継ぎ、承認済み立替の読み取り専用明細、調整行、請求日、支払期日、備考、発行済み情報、発行取消を持つ。発行時は `issue-invoice` へ `allLinesJson` / `invoiceSubject` / `invoiceRemark` を渡す。
