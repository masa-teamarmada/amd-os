...
| `/knowledge-map` | AMD Materials。118元素の周期表、代表的な鉱物・鉱石、樹脂・高分子、2〜4件比較、従来の force graph を束ねる read-only 材料インテリジェンス workspace。周期表は注目度 / 需要 / 供給不安 / AMD相性を切り替え、注目度5を深紅とする熱色、cell内の日本語主用途、軸から独立した`警戒` / `危機`表示と供給警報帯で俯瞰する。cell内の`x/5`表示は持たず、未評価を低評価と分ける。別建ての総合評価は持たず、4指標合計（20点満点）を総合値とし、周期表以外の全体ランキング、鉱物一覧、樹脂一覧、比較列を合計値の高い順に並べる。全体tabの3入口はカード全面を操作対象にする。樹脂detailは原料と製造プロセスも示す。評価済み元素を押すと、用途・警報・直近公表相場・5年推移・産出国円グラフを絞ったmodalを開き、`詳細を開く`で同じmodalを拡大して埋蔵・需給 / 代替・循環性 / AMD接点 / sourceを追加する。相場は代表品目、単位、公表時点、非live値を明示する。従来の `protocols` 等の件数と直近代表 node は `材料マップ` tab に残し、raw本文、画面内Q&A、DB write、LLM呼び出しは持たない |
