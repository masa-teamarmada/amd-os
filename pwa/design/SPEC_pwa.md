...
| `/bzm/map` | BZM 2.0 理論マップ (論証台帳)。`pwa/bzm/theory-graph/*.md` (1 Markdown = 1 ノード) を fs で読み、`parseTheoryNode` / `buildTheoryGraph` (`lib/bzm-theory-graph.ts`) で解析し、定義・支持・異議・反証・依存・上書き・運用化・検証の8 relation で結んだ力学マップ + 一覧 + 選択ノード台帳 + カバレッジ欠落検知を表示する読み取り専用画面。「真理マップ」ではなく、ノード数・接続数は真偽・確信度を表さない。独立実装の contract validator は `scripts/check_bzm_theory_graph.cjs` (`npm run test:bzm-theory-graph`)。詳細は [`/spec/2-6-bzm-theory-map-current-spec`](/spec/2-6-bzm-theory-map-current-spec) |
