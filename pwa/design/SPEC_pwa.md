...
| `cron/proactive-todo-extract` | `15 0 * * *` | 09:15 daily | 先手 TODO control layer。開催済みMTG `next_actions[]`、7日以内の予定MTG準備、PJ `report_emails` から届く Gmail 期限つき依頼 (`email_action_request`) を `proactive_todos` に upsert する。LLM非使用、メール本文全文・URL・パスワードは保存しない。仕様正本は [`/spec/2-4-proactive-todo-current-spec`](/spec/2-4-proactive-todo-current-spec) |
