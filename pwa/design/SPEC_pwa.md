...
| パス | 機能 |
|---|---|
| `/dashboard` | トップ。PJ 一覧 + 先手力維持ループの TODO + Atlas/Venture Map/MyPage/Admin への入口。基本表示順は左/mainカラム内で PJ 一覧 → 研究機関ERSリスト、下段全幅で Company Content shelf。PJ一覧は通常PJだけを表示し、`projects.project_category='ecosystem'` または `p25` / KUTE名に該当する研究機関エコシステム構築PJは研究機関ERSリスト側へ寄せる。研究機関ERSリストはPJリストの続きとして、PJ名を主タイトルに寄せて KUTE / KGW / NIMS を title、工学院大学 / 香川大学 / 物質・材料研究機構を subtitle にする。KUTEカードは `/institutions/inst_kute/cockpit`、NIMSカードは `/institutions/inst_nims/cockpit` へ遷移する。Company Content shelf はメンバー / 沿革 / メディア掲載 / photo を preview する。右カラムの MyPage embed は「今週やったこと」までに留め、月別PJカードは `/mypage` 単体にだけ残す |
