...
| `/project/[projectId]/cockpit` | PJ コックピット (上 Header + Hero (AMD Score + XRL 横並び) + `進捗管理` / `スコア詳細` + MS / 資料 + 経営ハイライト + ガバナンス + 助成金 + 下段 月次 + MTGサマリ)。SX `p21`の進捗管理先頭には現行PJ workspaceと同じmanagement bundleのAMDレンズを置き、現在地・AMD判断・次期限・4本柱を出す。adminは共有候補、柱、ボール保持者、audienceを明示確認してimmutable publicationを発行する。`?tab=score-detail` で SPS / R_net / FRL / XRL evidence と XRL チェックリストを直接開く。Header はPJリスト正本からPJメンバー、契約条件、業務委託料、支払い条件、提出物、立替精算の発生額/不可を表示する。資料は Drive の当該PJ folder配下 `AMD OS 資料` folder に保存し、OSには `project_documents` のmetadata/linkだけを残す。旧 `proactive_outbox` 由来のTODO欄は表示しない。`max-w-[1600px]` で画面幅を広く使う。詳細は [`cockpit.md`](cockpit.md) / [`project_strategy_signals.md`](project_strategy_signals.md) |
...
| `cron/proactive-todo-extract` | `15 0 * * *` | 09:15 daily | 先手 TODO control layer。開催済みMTG `next_actions[]` と、PJ `report_emails` から届く Gmail 期限つき依頼 (`email_action_request`) を `proactive_todos` に upsert する。予定MTG準備は Codex の W-Prep / prep worker へ一本化し、`next_meeting_prep` は新規生成せず既存 open / blocked を dismissed へ退避する。LLM非使用、メール本文全文・URL・パスワードは保存しない。仕様正本は [`/spec/2-4-proactive-todo-current-spec`](/spec/2-4-proactive-todo-current-spec) |
