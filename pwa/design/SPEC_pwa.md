...
| `cron/proactive-todo-extract` | `15 0 * * *` | 09:15 daily | 先手TODOの非LLM lifecycleだけ。明示期限超過のred昇格とblocked復帰を行い、新規候補は作らない。新規生成は既存 Codex automation `amd-os-proactive-heartbeat` が10:15–20:15 JSTの毎時15分に元証跡から直接行う。仕様正本は [`/spec/2-4-proactive-todo-current-spec`](/spec/2-4-proactive-todo-current-spec) |
