...
| `/reimburse` | 立替精算 |
| `/admin/settings` | Operations Settings。admin限定で Raw Data / L2 Data / Cron Control を一覧化する。CTB / SE のつくよみ週次 Slack レポートはPJ単位で配信・停止を切り替え、設定欠落も停止として扱う。停止中cronはここに旧頻度・入力・出力・停止理由を表示する。`/settings` は一般ユーザー誤操作防止のため削除 |
