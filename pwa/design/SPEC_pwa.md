...
| `/knowledge-map` | AMD Materials。118元素の周期表、代表的な鉱物・鉱石、樹脂・高分子、2〜4件比較、従来の force graph を束ねる read-only 材料インテリジェンス workspace。周期表は総合注目度 / 需要 / 供給不安 / AMD相性を切り替え、総合注目度5を深紅とする熱色、cell内の日本語主用途、軸から独立した`警戒` / `危機`表示と供給警報帯で俯瞰する。cell内の`x/5`表示は持たず、未評価を低評価と分ける。詳細は特徴 / 用途 / 供給国 / 埋蔵・需給メモ / 代替・循環性 / 材料チェーン / source を表示する。従来の `protocols` 等の件数と直近代表 node は `材料マップ` tab に残し、raw本文、画面内Q&A、DB write、LLM呼び出しは持たない |
