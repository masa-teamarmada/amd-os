...
| パス | 機能 |
|---|---|
| `/dashboard` | トップ。PJ 一覧 + 先手力維持ループの TODO + Atlas/Venture Map/MyPage/Admin への入口。PJ一覧は通常PJだけを表示し、`projects.project_category='ecosystem'` または `p25` / KUTE名に該当する研究機関エコシステム構築PJは下段の研究機関ERSリスト側へ寄せる。PJ一覧と右カラム MyPage embed の下に Company Content shelf を置き、メンバー / 沿革 / photo を3カラムで preview する。右カラムの MyPage embed は「今週やったこと」までに留め、月別PJカードは `/mypage` 単体にだけ残す。下段の研究機関ERSリストで、KUTEカードは `/institutions/inst_kute/cockpit`、NIMSカードは `/institutions/inst_nims/cockpit` へ遷移する |
