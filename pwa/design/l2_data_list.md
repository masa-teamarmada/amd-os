...
- L2⑯ Management Monthly Signal Evaluation: 毎月末17:00に生成する設計。数字を再掲せず、「今の経営状態はどう見えるか」を1行ステータス + 判断理由 + 次に見ることへ変換する。migration 122 / API route / UI preview は実装済み、production migration適用とactive cron登録は未実施。
