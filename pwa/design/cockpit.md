...
[A4]  SxCockpitInterventionRail (p21 only)       進捗管理タブの先頭で、SXワークスペースのopen判断をAMDの介入順に最大4件表示。
                                                判断を別台帳へ複製せず、今週指定→期限超過→7日以内→将来→期限未設定で並べる。
                                                過去日は必ず「期限超過」。件数カード、4本柱の再評価、全未完了項目の転載は行わない。
                                                各行は`focusKind=decision&focusId=<id>`でワークスペースのexact work unitへ解決し、該当論点ワークベンチを直接開く。
