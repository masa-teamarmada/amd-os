...
2026-09-16: コックピットのタブを二階層へ整理。通常PJは`進捗管理 / 事業計画 / ドライブ / PJ管理 / 会社情報`、`institution_projects`にひもづく研究機関PJは`進捗管理 / シーズリスト / 規程・内規 / ドライブ / PJ管理 / 会社情報`とする。`PJ管理`はAMD内部で定義する`PJ概要`だけを持ち、会社概要は独立した`会社情報`へ置く。コックピットはAMDメンバー限定、PJワークスペースはAMD外を含む当該PJメンバー限定という入口・認可を維持する。ワークスペースは`進捗管理 / 事業計画 / ドライブ / 会社情報`を共有し、会社概要と事業計画の技術・競合比較・ビジネスモデル・事業計画・両コスト試算・知財・資本政策をPJメンバーへ読み取り表示する。キラー要素カタログはコックピットだけで取得・描画する。既存の子タブURLは維持する。現在のroute・認可契約は`../spec/3-8-cockpit-current-spec.md`と`../spec/3-16-project-weekly-control-current-spec.md`。

### 二階層ナビゲーション（2026-09-01）

上段でグループを選び、下段には選択中グループの子タブだけを表示する。子が1件だけのシーズリストと規程・内規は、上段から直接その内容を開き、同じ名前の子タブ列は重ねない。`MS・月次`はMS・月次カード・履歴・収支・休止期間を、`動向・会議`は経営ハイライトとMTGサマリを読む。`Slack`は `project_slack_sources` で決めた取り込み対象チャンネルの会話を、Slackと同じ時系列（上が古い）で読む。`ガント`は時間順と目的からの枝分かれを同じ行で表示し、独立した`目的構造`子タブは持たない。`活動・実績`、助成金、獲得台帳、AMDの貢献は`PJ概要`へ置く。資本政策は`事業計画`グループの子タブとし、会社概要は登記・機関決定・年度決算に限定する。研究機関のシーズと規程は、解決済みの機関IDに対応する共通台帳を読む。
