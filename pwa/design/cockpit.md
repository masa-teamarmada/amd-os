...
2026-09-01: p19のみ`テーマ`タブ（`?tab=themes`）を追加。PJワークスペースと同じ4テーマ作業画面を`CockpitProjectControl`から表示する。テーマ面は経緯表を複製せず、ガント区画の`ガント / 目的構造`切替と関係先リストへ接続する。目的構造は再帰的な親子タスクと、相手別アプローチ内の接点時系列を同時に表示し、管理権限者はタスク追加・ドラッグ・接続選択で構造を更新できる。水素は最上位目的と3成立条件へ分離し、供給元→シーズリストから堂脇先生／pHydrogen／その他探索へ分岐する。相手別接点とボールは関係先台帳、計画変更と助成金はタスクで追う。現在のroute・認可契約は`../spec/3-8-cockpit-current-spec.md`、正本と表示切替は`../spec/3-16-project-weekly-control-current-spec.md`。
