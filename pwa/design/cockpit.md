...
2026-09-17: コックピットのタブを二階層へ整理。通常PJは`進捗管理 / 事業計画 / ドライブ / PJ管理 / 会社情報`、`institution_projects`にひもづく研究機関PJは`進捗管理 / シーズリスト / 規程・内規 / ドライブ / PJ管理 / 会社情報`とする。`PJ管理`はAMD内部で定義する`PJ概要 / 契約 / 収支`、`会社情報`は`会社概要 / 資本政策表 / 活動実績`を持つ。コックピットはAMDメンバー限定、PJワークスペースはAMD外を含む当該PJメンバー限定という入口・認可を維持する。ワークスペースは`進捗管理 / 事業計画 / ドライブ / 会社情報`を共有し、会社概要・資本政策表と事業計画の技術・競合比較・ビジネスモデル・事業計画・両コスト試算・知財をPJメンバーへ読み取り表示する。キラー要素カタログ、契約、収支はコックピットだけで取得・描画する。既存の子タブURLは維持する。現在のroute・認可契約は`../spec/3-8-cockpit-current-spec.md`と`../spec/3-16-project-weekly-control-current-spec.md`。

### 二階層ナビゲーション（2026-09-01）

上段でグループを選び、下段には選択中グループの子タブだけを表示する。子が1件だけのシーズリストと規程・内規は、上段から直接その内容を開き、同じ名前の子タブ列は重ねない。`MS・月次`はMS・月次カード・履歴・収支・休止期間を、`動向・会議`は経営ハイライトとMTGサマリを読む。`Slack`は `project_slack_sources` で決めた取り込み対象チャンネルの会話を、Slackと同じ時系列（上が古い）で読む。`ガント`は時間順と目的からの枝分かれを同じ行で表示し、独立した`目的構造`子タブは持たない。`PJ概要`はPJの姿、`契約`は契約条件、`収支`はシーズン予算と消化を置く。助成金、獲得台帳、AMDの貢献は`会社情報 > 活動実績`へ置く。事業計画に付随する資本政策プランは将来の見通しを更新し、`会社情報 > 資本政策表`は過去ラウンドの確定事実を記録する。会社概要は登記・機関決定・年度決算に限定する。研究機関のシーズと規程は、解決済みの機関IDに対応する共通台帳を読む。

PCの親グループはホバーまたはキーボードフォーカス時に、直下へ子タブ一覧をフロート表示する。親からフロートへポインタを移せる安全なヒット領域を持ち、子タブはTab順で操作できる。タッチ端末はホバーを模倣せず、選択中グループの子列を常時表示する。既存の白・インク・AMDブルー、境界線中心の階層、focus-visible、44px目安は変えない。
...
[A3]  Cockpit tabs                              SU 系 PJ では Hero 下に「進捗管理 / スコア詳細 / 会社概要」タブ。
                                                ...
                                                「事業計画」は全PJ常設 (2026-08-21 まさ依頼)。SOL固有のフェーズ表・年次試算表だけを p21 で足す。

                                                事業計画 (全PJ) = 100%株主構成 → 資本政策表。BZM 2.2 pilot 対象PJはその前にイベントと月次試算表・年度別の事業・資金推移 (`Bzm22TimeLedgerSection`) を挟み、pilot が無いPJはそこへ月次試算表 (`CockpitPlMonthlySection`) を置く。事業計画 (SOL) = 4開発レーン×5フェーズ表 → 月次試算表・年度別推移 → 年次試算表 → 100%株主構成 → 資本政策表。共通の枠は直接的な表題・項目名だけで構成し、PJ固有の仮説はフェーズのセルと計画データに置く。フェーズ表の直下には、表示中の4レーン×全フェーズだけをブラウザ内で `.xlsx` 化する「Excel出力」を置く。出力は期間・フェーズ予算・調達ラウンド・資金源・到達XRL・固定費バーン上限と、各セルの費用・活動・出口条件・関連XRLを1枚の横長シートに出し、先頭2行と開発レーン列を固定する。保存API・DB更新は持たない。
...
│       └── SOLのworkspace由来signalは根拠詳細から同じproject_managementレコードへ戻る
│
