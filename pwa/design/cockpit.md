...
2026-09-01: コックピットのタブを二階層へ整理。通常PJは`進捗管理 / 事業計画 / PJ管理`、`institution_projects`にひもづく研究機関PJは`進捗管理 / シーズリスト / 規程・内規 / PJ管理`とする。既存の子タブURLは維持し、テーマ機能はPJワークスペースへ集約してコックピットのテーマタブ・`?tab=themes`導線は持たない。現在のroute・認可契約は`../spec/3-8-cockpit-current-spec.md`、ワークスペースのテーマ正本は`../spec/3-16-project-weekly-control-current-spec.md`。
