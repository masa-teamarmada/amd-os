...
上段でグループを選び、下段には選択中グループの子タブだけを表示する。子が1件だけのシーズリストと規程・内規は、上段から直接その内容を開き、同じ名前の子タブ列は重ねない。`MS・月次`はMS・月次カード・履歴・収支・休止期間を、`動向・会議`は経営ハイライトとMTGサマリを読む。`ガント`と`目的構造`は進捗管理グループの独立した子タブで、前者は時間順、後者は目的からの枝分かれを表示する。`活動・実績`、助成金、獲得台帳、AMDの貢献は`PJ概要`へ置く。資本政策は`事業計画`グループの子タブとし、会社概要は登記・機関決定・年度決算に限定する。研究機関のシーズと規程は、解決済みの機関IDに対応する共通台帳を読む。
