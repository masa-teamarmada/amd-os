...
2026-09-16: コックピットのタブを二階層へ整理。通常PJは`進捗管理 / 事業計画 / ドライブ / PJ管理`、`institution_projects`にひもづく研究機関PJは`進捗管理 / シーズリスト / 規程・内規 / ドライブ / PJ管理`とする。コックピットはAMDメンバー限定、PJワークスペースはAMD外を含む当該PJメンバー限定という入口・認可を維持する。ワークスペースの共有タブのグループ名も`進捗管理 / 事業計画 / ドライブ / PJ管理`へ揃え、事業計画の技術・競合比較・ビジネスモデル・事業計画・両コスト試算・知財・資本政策をPJメンバーへ読み取り表示する。既存の子タブURLは維持する。現在のroute・認可契約は`../spec/3-8-cockpit-current-spec.md`と`../spec/3-16-project-weekly-control-current-spec.md`。
