...
| **2026-05-29 以降** | LLM 課金抑制のため GAS 155 は kill switch 停止のまま。`protocols` の現行 writer は MMOマシン Codex Desktop automation `amd-os-l2-protocol-extract` (= daily 08:00 JST)。詳細は [../manual/8-3-l2-extraction-routines-spec.md](../manual/8-3-l2-extraction-routines-spec.md) |
...
現行は MMOマシン Codex Desktop automation `amd-os-l2-protocol-extract` が daily 08:00 JST に実行し、`protocols` / `protocol_examples` へ upsert する設計。
...
| 2026-05-29 | 正本訂正。GAS 155 は 5/22 kill switch で停止中、現行 writer は MMOマシン Codex Desktop automation `amd-os-l2-protocol-extract`。通知 yes は `confirmed` が正本で、`active` ではない。 |
