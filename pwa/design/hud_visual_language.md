...
- 連続型のMTG投影HTMLでは、sectionを独立したページやカードの連続として扱わない。`page-break-*` / viewport高の固定 / sectionごとの強い外枠・影・背景面を使わず、同一canvas上で本文の流れを維持する。区画は見出し、細いdivider、余白、必要最小限の本文中calloutで示す。
