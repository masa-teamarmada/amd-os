...
- Notion 議事録メタデータ補完: H-1 が該当 Notion 議事録ページを特定できた場合、本文抽出とは別に `eventId`、`PJ` relation、member relation (`NOTION_MINUTES_MEMBER_PROP`。現行DBでは `メンバー` / `参加メンバー` 相当) を best-effort で補完する。既存の relation は消さず、空欄補完または member relation の union 追加だけを行う。PJ不一致、参加者候補の重複、Notion member page 未解決、Notion connector 書き込み失敗は H-1 の停止理由にせず、要確認として残す。PWA から Notion 本文や relation を直接書かない。
