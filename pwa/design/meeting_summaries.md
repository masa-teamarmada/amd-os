...
- Notion 議事録メタデータ補完: H-1 は Calendar候補と独立して、議事録data sourceの全履歴から `eventId`、`PJ` relation、member relation (`NOTION_MINUTES_MEMBER_PROP`。現行DBでは `メンバー` / `参加メンバー` 相当)、`日付` の空欄pageを毎run最大25件scanする。pagination cursorをrun間で引き継ぎ、EOFで先頭へ戻すため、未解決の先頭25件に阻まれず古い後半pageまで巡回する。page fetchで空propertyが省略されても親data source schemaを正として解決する。既存値は消さず、eventId/PJ/日付は空欄だけ、member relationはunion追加だけを行い、patch後に4項目をreadbackする。PJ不一致、参加者候補の重複、Notion member page未解決、書込み/readback失敗は成功に数えず理由を残し、失敗時はcursorを進めない。PWAと旧GASはwriterにしない。
...
- **Notion relation 方針**: Calendar event と AMD OS の PJ/member 情報を同時に見ている MMO automation が、該当 Notion page の空の `PJ` relation と member relation を補完する。`PJ` は既存 relation が空で、H-1 の `project_id` から Notion PJ page が 1 件に定まる場合だけ入れる。member relation は `NOTION_MINUTES_MEMBER_PROP` を正とし、通常の H-1 では Calendar organizer / attendees の email と AMD members を exact match し、Notion member page が 1 件に定まる member だけ既存 relation に追加する。過去分 backfill では参加者推定をせず、`NOTION_MINUTES_DEFAULT_MEMBER_PAGE_ID` の既定 member だけを union 追加する。既存 relation の削除・別PJへの上書き・外部参加者の推測追加は禁止。
...
`POST /api/meeting-assets` / `POST /api/workspace-documents` は `requireAdmin` / `requireAuth`（ブラウザ session 必須）で、**CLI の Bearer (CRON_SECRET) では叩けない**。`meeting-assets` Storage bucket は `image/* + application/pdf` のみ許可で **md/docx 等は 415**。ローカル `.env.local` に Google Drive 認証は無く Drive API 直叩きもできない。

→ えいみ / Codex が CLI から資料をコックピットに載せるときは、**PDF 変換や Storage 直挿しでこじらせない**。次の 3 手順を使う（まさが何度も指示している正規ルート）:

1. **実ファイルは共有ドライブに置く**: ローカルにマウントされた Google Drive (`~/Library/CloudStorage/GoogleDrive-masa@team-armada.jp/共有ドライブ/...`) の、当該 PJ の `AMD OS資料室` フォルダへ `cp` する。例: p00 (AMD全社) = `共有ドライブ/ARMADA/a0_management/AMD OS資料室/`。`cp` だけで Drive へは配置されるが、**OS内の資料室一覧 (`workspace_documents`) へは自動反映されない**（自動フォルダスキャンは無い。資料室一覧に載せたい場合はブラウザから手動でリンク登録が必要）。CLIからはこの後の手順どおり、MTGカード本文へ直接リンクを埋める運用にとどめる。
