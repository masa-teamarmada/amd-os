...
題名は `automation.toml` の `name` と一致させる。状態は 2026-09-11 時点。
2026-09-04 0:31 に既存32本を一時停止した履歴は維持する。問いの書き漏らし検出は、Vercelを呼ばず専用outboxからSupabaseへ反映する独立経路として9月11日に追加した。
...
| 毎週金曜16:00 PJの週次レポートをPWA経由でSlackへ送る | 稼働 | PWAの週次レポート入口 → Slack | 本文はCodexが作る。PWAは許可PJをLLM前に絞り、投稿直前にも設定を読み直す。Geminiなど外部LLM APIは使わない |
...
| 毎日4:35 会議から論点・やることの書き漏らしを拾う | 稼働 | 専用outbox → Macの届ける係（5分ごと）→ 論点・仮説の提案欄 | 前日以降の議事録だけを読み、PJあたり最大5件。DBへ直接書かず、承認まで木に入らない |
...
| 題名 | 場所 | 状態 |
|---|---|---|
| 毎週金曜16:00 PJの週次レポートを作成してSlackへ送る（旧Weekly Intelligence） | GAS `gas-external-research` の `di_cron_all` | 停止。固定PJ設定とGemini API直接呼び出しを廃止し、PWA経路へ移行 |
