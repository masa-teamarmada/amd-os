...
ところが、ベンチャー評価の実務と研究の多くは、設立後を前提とした企業価値評価（Valuation）に集中してきた。DCF（割引キャッシュフロー）法は将来キャッシュフローの予測を、マルチプル法は類似する既存事業の指標を必要とする。設立前でも、事業計画から将来キャッシュフローを仮置きして DCF を形式的に算出すること自体はできる。しかし事業実績がなく前提の不確実性が極めて大きいため、そうして得た価値で設立前ベンチャーを評価・比較するには精度が足りない。また DCF は到達しうる事業規模（潜在規模）を金額に翻訳しようとするものであり、その規模に **到達できるか（到達度）**・そこに辿り着く前に **生き残れるか（生存確率）** を分けて測る枠組みを持たない。結果として設立前の評価は、ピッチの巧拙や評価者の主観に大きく依存し、再現可能な定量的枠組みを欠いてきた。
