...
| **メンバー原資 (PJ予算)** | `(請求額 − バッファ) × 65%` = `value_plan_cycles.budget_yen` | 現金支払 + 支払対象外配賦の累計 | reward_summary 集計 |
...
| 配賦済み (支払対象=現金 / 支払対象外=非現金配賦) 累計 | — | ✓ |
| 今月末 繰越stock (= 未払い債務) | — | ✓ |
| 差 (実績 − 予算) | — | 収束チェック (最終的に 0 が正) |

> **分類基準の訂正 (2026-07-29 まさ確定)**: 以下「役員」「非役員」という書き方は本設計時点 (2026-06-19〜07-02) の暗黙前提で、実際の区分根拠は `is_officer` ではなく `members.exclude_from_payout_notice` のみ。あき (ID029)・りり (ID006) は非役員だが支払対象外、既存役員は全員 `exclude_from_payout_notice=true` で揃っているため当時の記述は結果的に一致していたが、`is_officer` を分類根拠として読まないこと。詳細は [`manual/7-1-reward-calc-spec.md`](../manual/7-1-reward-calc-spec.md) を正本とする。

`exclude_from_payout_notice=true` のメンバーは「支払対象外配賦 (旧: 会社留保)」列、`exclude_from_payout_notice=false` のメンバーは「現金支払」列で分ける (6-5章の支払通知書対象と同じ区分)。いずれも 65% PJ 予算内の cap 按分には同じく参加する — 支払対象外だからといって cap 配分から落とすわけではない。

### 検知したい歪み (この表の目的)
- **閉じない**: `バッファ + 原資 + マージン ≠ 請求額` → 設定ミス。
- **未割当pt**: `total_points − Σ(MS points, goal_level≠monthly)` → 意図しない穴か、将来MS用の意図的な未配賦バッファかを確認する監査値。正数だけで予算不足や保存停止にはしない（SXは13ptを意図的に残す、2026-07-29確定）。
...
- **支払対象外配賦の取りこぼし**: 最終月で対象外メンバーのstockが0に収束しない → 非現金配賦の繰越が効いてない。
...
### ④ 表示・通知の側では、本契約と別財布を混ぜない (まさ確定 2026-08-28)

計算は同じ plan cycle の別プールで回すが、**メンバーへ見せる側では本契約 (regular) だけで出す**。別財布は支払条件が本契約と別で、完了月まで払わない積立だから。

**迷ったら本契約だけで出す。** 別財布を混ぜて「1本の数字」にしない。

- **支払通知書の稼働月の範囲**: 本契約の繰越だけで遡る (`src/lib/payout-source-span.ts` の `regularPoolAmounts()`)。混在値の `carryInYen` / `grossDueYen` / `stockYen` を範囲の判定に使わない。
- **支払通知書の備考**: 繰越の内訳を混在値で書かない。書くと、完了月まで払わない別財布の積立まで「翌月以降お支払いします」と約束することになる。
- **月初合意の「受け取る額」**: 同じ理由で本契約側の値を使う。

実例 (ZMP / OkuDoor, 2026-08-28): あびの2026年7月は本契約の発生40,170円に対して支払40,170円で未払いゼロ。それなのに混在値で範囲を遡ったため、別財布の積立10万円だけで「5〜7月稼働分」「残り10万円は翌月以降お支払いします」と通知書に出ていた。まさ「別勘定の方はまだ支払わない。10月にシステム納品し終わってから一気に支払う。だからそっちのことは考えないで。本契約の方だけで範囲を出して」。

検査: `npm run test:payout-source-span`。別財布の積立で範囲が伸びないことを固定する。
...
| 2026-07-29 | §1③の「役員/非役員」区分記述を訂正: 分類根拠は `is_officer` ではなく `members.exclude_from_payout_notice` のみ (あき/ID029・りり/ID006 は非役員だが支払対象外)。SX 会社留保 ¥9,069,525 の原因だった月末stock残高の月次重複加算バグを、MS編集保存前検算 (`route.ts`) と PJ cockpit (`supabase-data.ts`) の両方で修正 — stock は残高スナップショットであり月次合計せず、当月フロー (`companyReserveYen`) だけを合計する | えいみ |
