...
$$
J=\sum_t d_t s_t CF_t+d_HQ\,TV+\sum_i d_i s_{i^-}(1-p_i)RV_i
$$

$J$は「今の方針から生じる途中収支、成功時価値、失敗時残存価値をまとめた正味価値」と表示する。

$$
P=\sum_t d_t CF_t+d_H TV
$$

$P$は「登録した条件をすべて通過した場合に、このPJから生じる途中収支と将来価値」と表示する。
...
ここで、$CF_t$は月ごとの経済CF、$TV$は目標到達後の終端価値、$RV_i$はgate $i$で止まった時の残存価値、$d$は評価日時点への割引、$s$はその時点までの経路生存重み、$p_i$はgate通過値、$m_{i\delta}$は登録stress $\delta$の補正である。
