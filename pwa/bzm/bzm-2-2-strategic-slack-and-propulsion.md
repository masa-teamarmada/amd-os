...
> **状態**：`provisional-pilot-v0.1 / unvalidated`
>
...
> **前向き検証**：0件
>
> **計算状態**：12 PJの欠測を強制推定した内部shadow試算がある。
> 前向き検証は0件であり、測定済みの$q$または$q_{\mathrm{rob}}$、PJ間比較、投資判断、資源配分には使わない。
...
### 2026-08-12の内部shadow試算

12 PJについて、欠測へ再現可能な強制推定を入れた`provisional-pilot-v0.1`を作成した。

この試算は、103パラメータの型、由来、計算接続と欠測原因を検査する内部artifactである。

現在方針は`shadow_only`の登録制御であり、実行可能性を確認済みとする方針でも、行動集合から選んだ最適方針でもない。

試算の到達指標は、条件付き確率として校正していないgateの積`q_gate_product_proxy`と、登録stressごとの積の最小値`q_stress_proxy`である。

これらは通常法則下の$q^{\pi}$または非先見的方針で求める$q_{\mathrm{rob}}^{-}$ではない。

したがって、DBとartifactでは$q$または$q_{\mathrm{rob}}$へ改名せず、`q_gate_product_proxy`と`q_stress_proxy`を維持する。

### pilot画面の四指標

pilot画面では内部keyを見出しへ露出せず、BZM 2.2の版表示と$J$、$P$、$Q$、$S$の一文字を使う。

検証地位、情報締切、根拠状態は監査情報として保持するが、画面最上部へ免責文のように並べない。
...
$J$は「登録済み現在方針を続けたとき、途中収支、全gate通過時の価値、途中失敗時の残存価値を全分岐で重みづけした現在価値」と表示する。
...
$P$は「登録済み現在方針のまま、すべてのgateを通過した場合の途中収支と将来価値の現在価値」と表示する。

$P$は、複数の行動から最良の行動を選ぶ値ではない。

方針は$\pi_{\mathrm{reg}}$へ固定し、その方針の下で全gate通過という条件$G$を置く。

行動選択まで含む値を定義するなら$\max_{\pi}J_r^{\pi}$のような別の最適化問題になるが、現在のpilotはその最大化を実行していない。

$J$と$P$はいずれも$d_t$を使って評価日時点へ割り引く。

$P$は全gate通過を条件にするためgate通過値で重みづけず、失敗時精算も含めない。

$J$は各月までgateを通過している重み$W_t$、全gate通過指数$Q$、各gateで止まる重みを使う。
...
J-QP
=
\sum_t d_t(W_t-Q)CF_t
+
\sum_i d_{t_i}
\left(\prod_{j<i}p_j\right)(1-p_i)RV_i
$$

である。

$QP$へ縮約すると、gateへ着く前にすでに発生する支出と収益、どのgateで止まったか、停止時に残る知財、設備、契約または閉鎖費用が消える。
...
画面は上の式に現れる26記号を、固定方針、入力、集合・番号、計算途中、出力へ分類し、$d_H$とgateごとの$d_{t_i}$を含む選択中scenarioの実値を添える。さらに$J/P/Q/S$の式を項ごとに分け、各項の直下へ対応する実数と項の集計値を接続して表示する。$G$と$\Delta_{\mathrm{reg}}$は全構成員、$CF_t,d_t,W_t$は全$H$月、$m_{i\delta}$は全行列を展開する。103項目のartifact監査台帳と、数式の全記号一覧は別のものとして表示する。

$$
Q=\prod_{i\in G}p_i
$$

$Q$は「通常の想定で、目標までに登録した条件を順に通り切る強さをまとめた指数」と表示する。

$$
S=\min_{\delta\in\Delta_{\mathrm{reg}}}\prod_{i\in G}(p_i m_{i\delta})
$$

$S$は「登録した逆風の中で最も厳しい状況でも、目標までの経路を保てる強さ」と表示する。

ここで、$\pi_{\mathrm{reg}}$は最適化で選んだ方針ではなく登録済み現在方針、$r$は評価視点、$CF_{r,t}$は月ごとの経済CF、$TV_r$は目標到達後の終端価値、$RV_{r,i}$はgate $i$で止まった時の残存価値、$d_t$は評価日時点への割引、$W_t$はその時点までの経路生存重み、$p_i$はgate通過値、$m_{i\delta}$は登録stress $\delta$の補正である。

$Q=\prod_i p_i$を全gate通過の確率として読めるのは、各$p_i$が単独の周辺確率ではなく、

$$
p_i
=
\Pr\!\left(
G_i\mid G_1,\ldots,G_{i-1},\pi_{\mathrm{reg}},\mathcal I_{\tau}
\right)
$$

という、先行gate通過、登録方針、情報締切に条件づけた値である場合に限る。

共通要因へ依存する周辺確率を独立に掛けない。

途中で学習して方針を選び直す、失敗後にpivot、licenseまたは提携へ移る、複数の終端経路を持つ場合は、一本の$Q$ではなく状態遷移と判断分岐を使う。
...
\operatorname E
\left[
\sum_t d_tCF_t+d_{\tau}RV(s_{\tau})
\right]
$$

または本章9節のBellman式で評価し、$Q$は固定登録方針の一経路を要約する補助表示に限る。

$Q$の表示名は「基準到達指数」、$S$の表示名は「逆風耐久指数」とする。$S$を単独で「戦略余力」と呼ばない。BZM 2.2の戦略余力は、型付き状態、実行可能行動集合、頑健捕捉領域、経路喪失時点まで含むためである。

### pilot画面のパラメータ表示契約

103項目を、数式入力と同じ重要度で主表示へ並べない。

画面は次の三層へ分ける。

1. **今回の数字を動かす入力**：評価期間$H$、割引率$r$、月次経済CF $CF_t$、gate別の判定月$t_i$と条件付き通過値$p_i$、gate別の停止時価値$RV_i$、終端価値$TV$、stress補正$m_{i\delta}$。
2. **実行可能性と経営判断**：状態、必要資源、権限、同意、登録方針、行動束、資金制約、情報獲得。
3. **根拠・再現・監査**：モデル版、評価日、情報締切、通貨、ID、hash、根拠資料、税、換算、重複防止、現版未使用項目を含む103項目の完全台帳。

第一層では、記号式だけでなく基準、慎重、強気の各ケースについて実値を代入する。

$Q$はgate別の$p_i$を順に掛け、$S$は登録stress別の積と最小値を示す。

$P$は全gate通過経路の月次CF現在価値と終端現在価値を分け、$J$は生存加重CF現在価値、成功寄与、gate別停止寄与を分ける。

表示値は正規化した計算入力DTOから生成し、103項目のcontainer、aliasまたは派生値を直接計算器へ渡さない。

特に、月次経済CFの重複slotと、期待失敗損失の派生slotを独立入力として再加算しない。

gate別の符号付き停止時価値は、$J$が実際に読む原子的な入力としてDTOへ持つ。

各行は、日本語名、理論上定義済みならLaTeX記号、接続する式、数式上の役割、平易な説明、登録値、慎重・基準・強気の3ケース、値の決め方、根拠資料の順で表示する。

モデルID、日付、通貨、各種ID、根拠資料、hashのように数式上の量ではない項目へ、見かけだけの変数記号を発明しない。

この場合は第一層へ出さず、第二層または第三層で役割を日本語で示す。

内部key、英語の機械状態、raw rule、source IDは主表示へ出さず、閉じた監査情報へ隔離する。

根拠資料は件数だけで終わらせず、PJ資料室、Gmail、OSデータなど開ける参照先へ接続し、参照先を持たないconnectorは「参照先未接続」と表示する。

数式との関係は、直接入力、間接生成、整合検査、派生出力、現版未使用、同じ元データの別名を区別する。

artifactの`usedInCalculation`は、モデルIDや遷移IDを含む再現条件まで`true`になり得るため、「代数的に式へ入る」という表示には使わない。

月次経済CF、$J$、$P$、失敗脚などを複数slotへ投影している場合は同じ元データの別名と表示し、別々の価値として加算しない。

状態ベクトルのcontainerに含まれる個々の値へ、対応表と評価者再現性が閉じる前から独立したscalar記号を付けない。

特に、戦略余力診断ベクトル全体を$S$と呼ばず、その中から表示用に投影するstress下gate積だけを$S$とする。

### pilot画面のイベントと月次試算

パラメータは評価基準日時点の状態を表すが、それだけでは「いつ何を選び、その後に何が起きたか」を読めない。

pilot画面は評価月M0から$H$月までの一本の横時間軸に、登録済み現在方針、事業・技術・設備・資金イベントを表示する。

登録済み現在方針は評価地平全体の帯、各イベントは該当月の点と上下の注記として置き、縦の複数レーンを積み重ねない。

gate点は$t_i$と同じ月列へ置き、ブラウザ内の時期変更に追従する。

時間軸の直下には`project_pl_monthly`の売上・費用・利益と、$CF_t$の「BZM経済CF」を同じ月列で表示する。

月次PLと$CF_t$を自動同一視せず、$J/P$へ入る正本は$CF_t$だけとする。

確認済みの意思決定は、選択肢集合、選んだ行動、決定主体、決定日、権限、根拠が同じ情報締切までに揃った場合だけ表示する。

$$
z_k^{\mathrm{obs}}=(a_k,i_k),
\qquad
z_k^{\mathrm{obs}}\in\Gamma_{\mathrm{exec}}(s_{t_k^-})
$$

現在の12 PJ artifactには、この条件を満たす意思決定履歴が収載されていない。

したがって、gate判定月、資金イベント、generatorが計算対象に固定したcontrolを、経営陣がその時点で選んだ事実へ読み替えない。

登録中の進め方は$\pi_{\mathrm{reg}}$として評価基準日からの帯で表示し、各イベントへそのイベントの試算で前提にした進め方を表示する。

これは$\arg\max_{\pi}J_r^{\pi}$で選んだ最適方針でも、取締役会その他の権限者が承認した実行計画でもない。

将来の判断点は、判断期間、trigger、判断主体、選択肢集合が登録された場合だけ表示し、gate月から自動生成しない。

事業イベントは、gate判定、試験、工場の承認・発注・着工・完成、顧客契約を別のlife-cycleとして持つ。

資金イベントは、関心、term sheet、契約、条件充足、利用可能日、着金を分け、`conditional_future`を入金予定または確約済み資金として表示しない。

日付は少なくとも、決定日`decided_at`、発効日`effective_at`、発生日`occurred_at`、資金利用可能日`cash_available_at`、観測日`observed_at`、記録日`recorded_at`、予測日`forecast_at`を区別する。

単に同じ月に並ぶ項目を因果線で結ばない。

登録済みの関係がある場合だけ`modeled_effect`または`identified_effect`を明示し、現在のpilotで前者を後者へ昇格させない。

初期版は、登録済み現在方針、推定gate、資金イベント、cash cliffだけを表示し、確認済み意思決定と将来判断点は「未収載」と明示する。
...
試算値はブラウザmemoryだけに保持し、ページ離脱・PJ切替で破棄する。DB、URL、local storageまたは分析payloadへ保存しない。

日付を動かすだけで因果写像を作らない。CF発生日は$J/P$の割引、gate判定日は$J$の経路生存重み、資金利用可能日はcash cliffへ接続できるが、工場完成日の変更から売上、能力またはgate通過値を自動生成しない。現在のUIで動かせる日付は登録方針のgate判定月だけとし、$p_i/CF_t/TV/m_{i\delta}$を固定して$W_t$、停止時価値の割引、$J$だけを再計算する。$Q/S/P$は変えない。工場、資金、経営判断の一般日付入力は、依存関係と再計算器が閉じるまで操作対象にしない。

資金の崖は、未確約資金を現金へ足して越えない。

無資金経路で現金が初めて不足する月をcash cliffとするが、検査を通すために既存の技術、顧客、権利、資金gateをその月へ前倒ししない。

既存gateとは別に、初回cliff月の月初で`full_horizon_liquidity_package_proxy_before_first_cliff`を判定する。

この代理gateは初回入金の有無ではなく、評価期間中の全minimum-cash補填期限を賄う拘束力ある資金枠または共同調達packageを対象にする。

同じ資金証拠を既存gateと代理gateへ二重に掛けず、重なる既存gateでは金融成分を代理gateへ集約し、残る非金融条件だけを逐次評価する。

金融成分と非金融成分を資料から分離できない複合gateは、元の複合確率を監査用に保持し、代理積では乗法単位元として扱う。これは非金融残余の成功確率を1と置いたのではない。識別不能な残余を新たに発明せず代理積から除外するため、その非金融riskについて代理値が上方へ偏る可能性を明示する。

資金packageのcoverageは総額だけで判定しない。各補填期限$t$で利用可能な拘束力あるcapacity $C(t)$と累積必要額$R(t)$を比較し、$\min_t\min(1,C(t)/R(t))$を使う。入金日または利用可能日が不明な資金はcapacityへ入れない。

代理gateの失敗経路は初回cliff月の当月CFから停止し、以後の便益と終端価値を計上しない。

資金調達の入金は流動性と行動実行可能性を変えるが、会社PJの価値創造キャッシュフローへ便益として加えない。
...
台帳上のlegacy key `expected_cumulative_funding`は、資金受領期待額ではなく、経路生存で加重した無資金残高の最小補填必要額を保持する互換slotとしてだけ使う。

低位、中央、高位は、同じ母数から求めた信頼区間ではなく、仮定束を変えたscenarioである。

Gmail、Drive、Calendar、Slack、Notion、OS DBの六つは、索引またはローカル観測枠の列挙を進めたが、globalな本文取得、remote scope、thread再帰、権限範囲のいずれかが未完である。

したがって、六つすべてを`incomplete`とし、未発見を不存在の根拠にしない。

経済学者役と経営学者役の独立監査で指摘された、代理指標の誤称、資金の崖後の価値計上、$\pi_0$の偽装、行動の実行可能性と最適性の過大表示、103パラメータと計算入力の不一致は、pilot契約と機械検査で停止対象にした。

ただし、gateの条件付き確率としての妥当性、証拠間依存、状態遷移の因果識別、行動被覆、共有資源制約、前向き予測の較正は未検証である。

構造検査の通過を構成概念妥当性の証明とは扱わない。

この試算からPJ間順位、撤退または継続の自動判断、投資額、支援採択、資源配分を導かない。
