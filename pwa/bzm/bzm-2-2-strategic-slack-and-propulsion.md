...
pilot画面では内部keyを見出しへ露出せず、BZM 2.2の版表示と「暫定・未校正」を常設したうえで、$J$、$P$、$Q$、$S$の一文字へ短縮する。
...
画面上の「別の進め方を試す」は、計算bundleが揃う登録候補を丸ごと切り替え、同じscenarioの$J/P/Q/S$と基準との差を表示する。
...
日付を動かすだけで因果写像を作らない。CF発生日は$J/P$の割引、gate判定日は$J$の経路生存重み、資金利用可能日はcash cliffへ接続できるが、工場完成日の変更から売上、能力またはgate通過値を自動生成しない。現在のP0では方針bundle切替だけを数値へ接続し、日付変更は依存関係と再計算器が閉じるまで操作対象にしない。
