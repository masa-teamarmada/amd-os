...
各行は、日本語名、理論上定義済みならLaTeX記号、接続する式、数式上の役割、平易な説明、内部keyの順で表示する。

モデルID、日付、通貨、各種ID、根拠資料、hashのように数式上の量ではない項目へ、見かけだけの変数記号を発明しない。

この場合は記号を「—」とし、「計算範囲、証拠、単位または再現性を定める管理情報」と明示する。
...
登録済み現在方針は$\pi_{\mathrm{reg}}$として評価基準日からの帯で表示し、「固定shadow・最適化なし・権限未確認」を常設する。
...
タイムラインはread-onlyであり、承認、実行、外部送信または自動推薦を起動しない。
