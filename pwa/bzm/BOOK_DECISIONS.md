...
### D-058: 独禁法の位置づけを「書籍内の注意喚起」に限定 — 弁護士発注・印刷ゲート・KENQ 内部メモは全て不要 (D-057 K3 / PF-008 / PF-011 の一部撤回)
- **judgment**: RT の独禁論点は、書籍で「**RT を組成する際は独禁法・共同研究開発ガイドラインの観点で専門家と確認すること**」の注意喚起を1箇所書くだけで足りる。それ以上の対応は不要: (i) **弁護士発注をやらない** (Q4「やる」を撤回)。(ii) Book B 第III部の**印刷ゲートから「独禁法務確認完了」を撤回** (PF-008 の初期案から降ろす)。実地点火 1件のみをゲートに残す。(iii) KENQ が動く時のための内部メモも作らない。(iv) 別セッション task_2985c953「RT独禁法務確認の発注パッケージ作成」は既に start 済みだが成果物は不要 (まさが手動停止 or 成果放棄)。(v) Ch 9.5 §9.5.4 の仮定 L (D-057 K3) は残すが、その位置づけを「**書籍上の Tier 規律の留保 (学術書として断定を避ける)**」に統一し、「実務上のブロック」ではないと補注する。実務適法性の担保は AMD の実績 (下記 rationale) で足りる。
- **rationale**: まさ確定 2026-07-02。理由の内訳: (a) **KENQ は RT の説明例にすぎない**。まだ事業化構想段階で AMD ビジネスとして動くかも未定、AMD が仕事をもらえるかも不明。具体案件がない状態で抽象的な独禁確認をしても抽象的な回答しか返らず実務に使えない。(b) **AMD は20超SUで独占契約を締結してきて一度も独禁で問題になったことがない** = 実務適法性は既に確立している。SU の需要企業側 (事業会社) は「独占的に使えないなら費用負担して量産開発する意味がない」ため独占契約が普通、これは日本の産業実務でも普通のこと。(c) 書籍側で断定を避ける Tier 規律は Ch 9.5 の仮定 L Box と注意喚起で十分機能する。(d) 実際に法的リスクが具体化する時 (KENQ が実契約設計フェーズに入る等) は、その時点で当該案件の弁護士 (or 顧問弁護士) と確認すれば足りる — AMD の既存オペレーション通り。えいみが「クリティカルパス」に格上げしたのはワークフローペルソナ (VC・出版アーキ) の慎重論を AMD 実務とすり合わせずに翻訳したミス。
- **applies_to_chapters**: Ch 9.5 (§9.5.4 仮定 L の位置づけ補注)、Book B 第III部 (印刷ゲート・注意喚起記述)、Ch 37.5 (オープンプロブレム章の書き方に影響なし)、`BZSF/rt_roundtable_theory.md` §7.3 (書籍側の学術的留保として残す、実務ブロックの意味を持たせない)
- **date**: 2026-07-02
- **status**: active
