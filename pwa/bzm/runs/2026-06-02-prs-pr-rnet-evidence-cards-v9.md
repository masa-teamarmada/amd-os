# P/R_net evidence cards v9 — p11 system GP30 projection refresh
...
| source_cache `p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` + Gmail attachment `HH20260226②SRK試算_BWE財務計画_260221_y.xlsx` | found exact with Excel projection basis | `RED_10kWh_system_gross_margin_candidate` / `system_financial_projection` | `system_gp30_source` | `review-only` | raw textは「10kWhシステムの総益率GP30%」。添付Excelには10kW/100kW systemの売価・原価・GP30%、O&M売上/原価、売電売上/原価があるが、膜単体外販R_net根拠にしない。 | Excel projection basisは見つかったが、contract / invoice / payment / delivery acceptance / vendor-paid COGS / QA / logistics / warranty / inventory / yield が未join。 |
...
### GP30 raw source lane split / v9 flags
...
| p11 BWE | source_cache `p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` + Gmail attachment `HH20260226②SRK試算_BWE財務計画_260221_y.xlsx` | `RED_10kWh_system_gross_margin_candidate` / `system_financial_projection` | `system_gp30_source` | `review-only` | GP30 exact + Excel projection basisは見つかったが、system / financial plan / Sumitomo Riko investment review context。膜単体外販R_net根拠として使わない。contract / invoice / payment / vendor-paid COGS / delivery acceptance 未join。 |
...
| 型 | 該当PJ | PRSで見る意味 | ガード |
|---|---|---|---|
| 高P・低/未確定R_net | ティエム、BWE | 本命の天井は大きいが生存設計が弱い。BWE膜外販は `bottleneck_external_sale_candidate` だが `review-only`。p11 GP30 exact + Excel projection basisは見つかったが10kWh/10kW system laneで、膜外販laneとは別。 | 7軸のGO/NO_GOを上書きしない。BWEは `supply_capacity + gross_margin_basis + main_red_impact` と buyer/contract/invoice/payment/cost が揃うまでR_net昇格禁止。10kWh/10kW system GP30もcontract/invoice/payment/vendor-paid COGS/delivery acceptanceが揃うまで正式R_netへ使わない。 |
...
25. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v9カードをレビューし、finance/cash分類、JOYCLE damage source split、p07 bridge validation source join、p20-p21 validation value source split、p11 membrane gate source join、p11 system GP30 projection refresh の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v9 flags を採用・差し戻し判定する。
...
9. p11 10kWh/10kW system GP30は、添付Excel projection basisをsystem laneに保持しつつ、contract、invoice、payment、delivery acceptance、vendor-paid COGS、QA/logistics/warranty/inventory/yieldを別laneでjoinする。膜外販laneの欠損補完には使わない。
