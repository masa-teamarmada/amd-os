# P/R_net evidence cards v12 — YD source refresh
...
- v11では `p11 Sumitomo Riko management meeting result lookup` をBZM採用判断として戻し、2026-03-06時点のpre-result source `gmail_19cc206f936d1405` と、2026-03-13候補日のpost-result source未発見をcurrent truthへ追加する。
- v11では `p11_system_management_review_status=pending_or_no_result_source_found` を維持し、3/13以降のMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討は住友理工resultではなく funding/survival lane 文脈として扱う。
- v11でも `p11_sumitomo_riko_counterparty_status=review_counterparty_not_confirmed_buyer`、system transaction/order/contract/invoice/payment/delivery acceptance/vendor-paid COGSの `not_joined`、system lane と membrane lane の分離、system GP30をmembrane external-sale GPへ使わないguardを維持する。
- v12では `YD low-P / UE guard source split` と `YD UE/LCOE source join` をBZM採用判断として戻し、p18 Yellow Duckは `low_p_market_structure` を主分類、`rnet_gross_margin_viability` を補助分類として扱う。
- v12では同じLCOE/維持費/価格受容性sourceをP低下とR_net低下へ二重減点しない。PoC/TRL4、福岡市実証、受賞/登壇/メディア露出はvalidation/survival contextであり、商用UE成立やR_net粗利成立には使わない。
- v12では経済界記事の中山氏説明を `low_p_market_structure` の補強sourceとして採用する。ただしwave lane general source限定で、YD固有失敗証明にはしない。
- v12ではYellow Duck会社は公式source上2026-05まで活動継続、AMD関与はDB上endedと分ける。公式設立日 `2023-08-04` とDB `2019-01-01` の衝突はDB補正候補としてflagするが、このカードではDB補正しない。
- v12ではVC DDは `/Users/masa/projects/knowledge/yd.md` の要約のみ。VC別コメント、DD日、VC名、投資見送り理由原文は未接続として扱う。
...
- `pwa/bzm/runs/2026-06-02-p11-sumitomo-riko-management-meeting-result-lookup.md` (from `origin/codex/p11-sumitomo-riko-management-meeting-result`)
...
| source_cache `p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` + Gmail attachment `HH20260226②SRK試算_BWE財務計画_260221_y.xlsx` + Gmail `19cc206f936d1405` | found exact with Excel projection basis, Sumitomo Riko transaction proof lookup completed read-only, and management result lookup completed read-only | `RED_10kWh_system_gross_margin_candidate` / `system_financial_projection` / `management_review_pre_result_source` | `system_gp30_source` | `review-only` | raw textは「10kWhシステムの総益率GP30%」。添付Excelには10kW/100kW systemの売価・原価・GP30%、O&M売上/原価、売電売上/原価があるが、膜単体外販R_net根拠にしない。Sumitomo Riko / 住友理工はfinance plan / investment review / management meeting review counterpartyであり、confirmed buyer/order扱いしない。2026-03-06時点では経営会議報告は未reviewで、2026-03-13が候補日。 | Excel projection basisとreview counterparty、pre-result sourceは見つかったが、post-2026-03-13 result source / purchase order / signed contract / LOI / formal quote acceptance / invoice / payment / delivery acceptance / vendor-paid COGS / QA / logistics / warranty / inventory / yield が未join。 |
...
### GP30 raw source lane split / v11 flags
...
| `p11_sumitomo_riko_management_result_lookup_status` | `completed_read_only` | `2026-06-02-p11-sumitomo-riko-management-meeting-result-lookup.md` | `review-only` | 2026-03-13前後のmanagement meeting resultをread-only確認済み。DB write / DDL / code / deployはしていない。 |
| `p11_sumitomo_riko_management_meeting_pre_result_source` | `gmail_19cc206f936d1405` | Gmail thread `19c98f0f1d7cea62` dated 2026-03-06 | `review-only` | 経営会議報告がまだreviewされておらず、2026-03-13が候補日だったことを示すpre-result source。 |
| `p11_sumitomo_riko_management_meeting_candidate_date` | `2026-03-13` | Gmail `19cc206f936d1405` | `review-only` | 候補日であって、承認・投資決定・購入決定日ではない。 |
| `p11_sumitomo_riko_management_meeting_pre_result_status` | `not_yet_reviewed_as_of_2026-03-06` | Gmail `19cc206f936d1405` | `review-only` | 3/6時点では結果未確定。 |
| `p11_sumitomo_riko_management_result_source_status` | `not_found` | management result lookup broad read-only sweep | gate | post-2026-03-13の住友理工result sourceは見つかっていない。沈黙から結果を推測しない。 |
| `p11_sumitomo_riko_post_0313_context` | `no_srk_result_source_found; funding_context_shifted_to_mirai_creation_shareholder_reorganization_and_other_vc_cvc_discussions` | management result lookup | guard | 3/13以降の強いsourceはMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討であり、住友理工resultではなくfunding/survival laneに置く。 |
...
| `p11_system_management_review_status` | `pending_or_no_result_source_found` | Sumitomo Riko transaction proof lookup + management result lookup | `review-only` | 2026-03-06のpre-result sourceはあるが、2026-03-13以降の承認結果sourceは未発見。 |
...
- 仮説: low
- 確認済み根拠: 波力発電は過酷な海洋環境、導入コスト、海洋生物付着メンテ、政策/予算環境、初期投資/メンテ/技術成熟度の面で価格受容性が弱く見える。`/Users/masa/projects/knowledge/yd.md` と、経済界記事の中山氏説明、Japan Energy Timesの波力一般sourceを `low_p_market_structure` の補強sourceとして採用する。
- 未確認: Yellow Duck固有技術が波力レーン全体の構造的不利を覆せる余地。Yellow Duck固有LCOE、商用販売単価、製造原価、設置費、O&M、保険、検収、顧客支払意思は未接続。

### R_netの仮説

- 粗利貢献: YD固有の契約、見積/料金、請求、入金、製造原価、設置費、O&M、腐食/付着メンテ、保険、検収、顧客価格sourceは未join。したがって `rnet_gross_margin_viability` は補助分類かつ `gross_margin_not_joined` に留める。
- 運営コスト: 波力一般のLCOE/維持費/保守負担sourceはP側の主source。R_net側では「粗利成立source未接続」の補助理由に留め、同じsourceを独立に二重減点しない。
- 本命PJへのリソース毀損: 本命自体の商用UEが未接続。短期収益による毀損というより、低P/UE不成立観測項目として扱う。

### Finance/Cash source classification

今回のv12ではfinance分類ではなく、YD low-P/UE guard source split と YD UE/LCOE source joinを戻す。p18は `low_p_market_structure` を主分類、`rnet_gross_margin_viability` を補助分類に置く。福岡市実証、Yellow Duck公式news、受賞/登壇/メディア露出はvalidation/survival contextであり、商用UE成立やR_net粗利成立には使わない。

| source_id_or_path | source_status | classification | evidence_label | prs_use | double_count_guard | why_not_formal_score_yet |
|---|---|---|---|---|---|---|
| `/Users/masa/projects/knowledge/yd.md` | internal knowledge / current note / 2026-05まさ口述 + web確認メモ | `low_p_market_structure` primary; `rnet_gross_margin_viability` secondary; `vc_dd_summary` secondary | 販売未到達、CEO自己資金試作、波力発電はUE成立しにくい、各VC DDで出資断念、AMD関与は2025-06〜2025-09の資金調達サポート。 | `review-only` | internal noteをVC DD原文として扱わない。LCOE/維持費はP側主source、R_net側は粗利未接続の補助理由に留める。 | Yellow Duck固有LCOE、売電/販売単価、製造原価、設置費、O&M、保険、検収、VC別DD原文、DD日、VC名。 |
| 経済界 `https://net.keizaikai.co.jp/archives/3331` | public media/interview / live checked in YD source join | `wave_lane_failure_source`; `low_p_market_structure` primary | 中山氏自身が、波力は過酷な海洋環境でコストが合わないと見切られてきた、50年に一度の大波対応で設備が過剰になり導入コストが上がる、海洋生物付着メンテが重い、洋上風力偏重で予算が付きにくい、と説明。 | `review-only` | wave lane general source限定。YD固有失敗証明や正式P/R_net値にしない。R_net側へ同じ根拠を二重減点しない。 | Yellow Duck固有の実費、商用仕様、保守計画、顧客契約。 |
| Japan Energy Times `https://japan-energy-times.com/wave-power-generation-profitability-analysis-ocean-energy/` | public web source / live checked in YD source join | `wave_lane_failure_source`; `low_p_market_structure` secondary | 波力発電の初期投資、運用/メンテ、技術成熟度、他再エネ比のコスト差、メンテ難度を整理。 | `review-only` | 一般web記事の数値を正式LCOEにしない。レーン一般の論点整理に留める。 | 一次資料との再照合、YD固有モデルへの適用可否。 |
| 福岡市実証開始PDF / 福岡市実証結果PDF / Yellow Duck公式news | public municipal + official source / live checked in YD source join | `yd_specific_validation_source`; `survival_or_validation_context` | 博多漁港での5〜50W実証機、安全性/設置性/干満追従/24時間連続発電、2026-05までの不動テトラ実証関連、登壇/受賞/メディア掲載。 | `review-only` for validation context | PoC/TRL4、実証、受賞/登壇/メディア露出を、商用UE成立、販売成立、粗利成立、R_net正値へ転用しない。 | 商用契約、売上、原価、発電単価、保守費、投資回収、検収。 |
| Supabase `projects?project_id=eq.p18` | DB current truth / read-only in source join | `yd_current_status_source` for AMD relationship | `project_name=YD`, `status=ended`, `start_ym=202505`, `end_ym=202509`。AMD OS上はAMD関与終了PJ。 | `review-only` | AMD関与endedを、Yellow Duck法人の活動停止と読まない。 | AMD関与終了理由の一次source、契約/請求/支援範囲詳細。 |
| Yellow Duck official company/news pages | official public source / live checked in source join | `yd_current_status_source`; `survival_or_validation_context` | 公式company pageは設立 `2023-08-04`。公式newsでは2026-05-14不動テトラ実証関連、2026-03〜05の登壇/受賞/メディア掲載があり、AMD関与後も会社活動は継続。 | `review-only` | 活動継続/検証継続を、UE成立・販売成立・粗利成立へ転用しない。 | AMD関与終了後の資本政策、売上、商用実装、投資回収。 |
| Supabase `project_ventures?project_id=eq.p18` / `project_knowledge` basic facts | DB current truth with conflict / read-only in source join | `yd_current_status_source`; `db_conflict` | DBは `founded_at=2019-01-01`、basic factsも法人設立日2019-01-01。一方で公式company pageは設立 `2023-08-04`。 | `review-only` | DB補正候補としてflagするが、このworkerではDB補正しない。 | DB設立日の由来、2019が研究開始/個人活動/旧仮置きなのかの確認。 |
| VC DD具体論点 | summary only in `/Users/masa/projects/knowledge/yd.md`; DB/source rows not joined | `vc_dd_summary` secondary; `still_missing` | 「各VC DDでUE成立せず」はknowledge上の要約source。VC別コメント、DD日、VC名、投資見送り理由原文は未接続。 | `review-only` | 口述/要約をDD原文扱いしない。P側の市場構造とR_net側の粗利不成立のどちらを主論点にするか、sourceごとに切る。 | VC名、DD日、original_comment、no_go_reason、lcoe_or_cogs_or_price_assumption、source_path。 |

### v12 BZM fields

```yaml
p18_low_p_market_structure_status: review_only_general_lane_sources_joined
p18_low_p_primary_sources: /Users/masa/projects/knowledge/yd.md; https://net.keizaikai.co.jp/archives/3331; https://japan-energy-times.com/wave-power-generation-profitability-analysis-ocean-energy/
p18_yd_specific_ue_lcoe_status: yd_specific_commercial_ue_lcoe_not_joined
p18_yd_specific_validation_sources: Fukuoka City demo start PDF; Fukuoka City demo result PDF; Yellow Duck official news
p18_rnet_gross_margin_viability_status: gross_margin_not_joined
p18_rnet_missing_join: contract + quote_or_tariff + invoice + payment + manufacturing_cost + installation_cost + o_and_m + corrosion_biofouling_maintenance + insurance + acceptance + customer_price
p18_vc_dd_status: ue_fail_summary_available_but_dd_details_not_joined
p18_vc_dd_missing_join: vc_name + dd_date + original_comment + no_go_reason + lcoe_or_cogs_or_price_assumption + source_path
p18_wave_lane_source_status: general_lane_source_joined_not_yd_specific
p18_current_status_status: company_active_after_amd_support_but_amd_relationship_ended
p18_company_founded_at_source_status: official_company_page_2023_08_04_conflicts_with_db_2019_01_01
p18_survival_or_validation_context_status: active_demo_and_award_context_not_gross_margin
p18_double_count_guard: lcoe_maintenance_policy_budget_sources_primary_to_low_p_secondary_to_rnet_not_two_independent_penalties; poc_validation_not_commercial_ue; wave_lane_not_yd_specific
p18_ready_for_rubric_draft_scope: low_p_and_ue_failure_observation_items_only_not_formal_rnet_rubric
p18_prs_use: review-only
p18_do_not_use_as: formal_P_value, formal_R_net_value, 0-9_score, current_7_axis_replacement, sales_actual, gross_margin_actual, vc_dd_original
```
...
- Yellow Duck会社の活動継続と、AMD関与終了を混同しない。
- 福岡市実証、PoC/TRL4、受賞/登壇/メディア露出を商用UE/R_net粗利sourceとして扱わない。
- VC DD要約をVC DD原文として扱わない。

### 次に必要な観測データ

- Yellow Duck固有のLCOE、製造/設置/保守コスト、O&M、腐食/付着メンテ、保険、検収、想定売電/販売単価、顧客価格。
- VC DDでNO_GOになった具体論点、VC名、DD日、原文コメント、投資見送り理由。
- DB設立日 `2019-01-01` と公式設立日 `2023-08-04` の由来確認。
- wave lane general sourceを、YD固有技術が覆せる/覆せない範囲へどう接続するか。

### rubric化してよいかの暫定判定

`ready_for_observation_item_draft_only`

低P/UE不成立の観測項目draftに限定。正式P値、正式R_net値、0-9 score、現行7軸置換、売上/粗利実績、VC DD原文には使わない。
...
| p11 BWE | source_cache `p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` + Gmail attachment `HH20260226②SRK試算_BWE財務計画_260221_y.xlsx` + Gmail `19cc206f936d1405` + `2026-06-02-p11-sumitomo-riko-transaction-proof-lookup.md` + `2026-06-02-p11-sumitomo-riko-management-meeting-result-lookup.md` | `RED_10kWh_system_gross_margin_candidate` / `system_financial_projection` / `management_review_pre_result_source` | `system_gp30_source` | `review-only` | GP30 exact + Excel projection basisは見つかったが、system / financial plan / Sumitomo Riko investment review / management meeting review context。2026-03-06時点では経営会議報告未review、2026-03-13が候補日。post-2026-03-13の住友理工result sourceは未発見。Sumitomo Riko / 住友理工はconfirmed buyer/orderではない。膜単体外販R_net根拠として使わない。purchase order / signed contract / LOI / formal quote acceptance / invoice / payment / vendor-paid COGS / delivery acceptance 未join。 |
...
| p03 | 追加finance sourceなし | - | - | `exclude from PRS` | finance source不足を正式R_net値に変換しない。 |
| p18 Yellow Duck | YD固有 commercial UE/LCOE / contract / invoice / payment / COGS source not joined | `still_missing` | `yd_specific_commercial_ue_lcoe_not_joined` | `exclude from PRS score`; source classificationは `review-only` | finance/source不足を正式R_net値に変換しない。波力一般sourceはP側主分類、R_net側補助分類に留める。 |
...
| p11 BWE | `p11_sumitomo_riko_management_result_lookup_status` | `completed_read_only` | `review-only` | management meeting result lookupはread-only完了。 |
| p11 BWE | `p11_sumitomo_riko_management_meeting_pre_result_source` | `gmail_19cc206f936d1405` | `review-only` | 2026-03-06時点のpre-result source。 |
| p11 BWE | `p11_sumitomo_riko_management_meeting_candidate_date` | `2026-03-13` | `review-only` | 候補日であり、承認・購入・契約日ではない。 |
| p11 BWE | `p11_sumitomo_riko_management_meeting_pre_result_status` | `not_yet_reviewed_as_of_2026-03-06` | `review-only` | 3/6時点では経営会議報告未review。 |
| p11 BWE | `p11_sumitomo_riko_management_result_source_status` | `not_found` | gate | post-2026-03-13の住友理工result sourceは未発見。 |
| p11 BWE | `p11_sumitomo_riko_post_0313_context` | `no_srk_result_source_found; funding_context_shifted_to_mirai_creation_shareholder_reorganization_and_other_vc_cvc_discussions` | guard | 3/13以降のMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討はfunding/survival laneで扱う。 |
...
| p11 BWE | `p11_system_management_review_status` | `pending_or_no_result_source_found` | `review-only` | 2026-03-06 pre-result sourceはあるが、post-2026-03-13 result sourceは未発見。 |
...
| 型 | 該当PJ | PRSで見る意味 | ガード |
|---|---|---|---|
| 高P・低/未確定R_net | ティエム、BWE | 本命の天井は大きいが生存設計が弱い。BWE膜外販は `bottleneck_external_sale_candidate` だが `review-only`。p11 GP30 exact + Excel projection basisは見つかったが10kWh/10kW system laneで、膜外販laneとは別。住友理工はfinance plan / investment review / management meeting review counterparty止まりで、2026-03-06 pre-result source後のpost-result sourceも未発見。confirmed buyer/orderではない。 | 7軸のGO/NO_GOを上書きしない。BWEは `supply_capacity + gross_margin_basis + main_red_impact` と buyer/contract/invoice/payment/cost が揃うまでR_net昇格禁止。10kWh/10kW system GP30もcontract/order/invoice/payment/vendor-paid COGS/delivery acceptance/QA/logistics/warranty/inventory/yieldが揃うまで正式R_netへ使わない。3/13以降のMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討はfunding/survival laneで扱う。 |
...
| 低P/UE不成立 | Yellow Duck | 技術PoCがあっても事業の筋が悪い。v12では `low_p_market_structure` を主分類、`rnet_gross_margin_viability` を補助分類へ分け、経済界/Japan Energy Timesはwave lane general sourceとして採用。 | BRL不足だけに潰さない。同じLCOE/維持費/価格受容性sourceをP低下とR_net低下へ二重減点しない。福岡市実証・PoC・受賞/登壇/メディア露出を商用UE/R_net粗利へ転用しない。Yellow Duck活動継続とAMD関与終了を混同しない。 |
...
9. p03/p04は今回のfinance分類で追加sourceが薄い。p18 Yellow Duckはv12でYD low-P/UE guard source splitとYD UE/LCOE source joinを戻し、`low_p_market_structure` 主分類、`rnet_gross_margin_viability` 補助分類、`review-only` として扱う運用でよいか。
...
26. `p11 Sumitomo Riko management meeting result lookup` は採用し、`p11_sumitomo_riko_management_result_lookup_status=completed_read_only`、`p11_sumitomo_riko_management_meeting_pre_result_source=gmail_19cc206f936d1405`、`p11_sumitomo_riko_management_meeting_candidate_date=2026-03-13`、`p11_sumitomo_riko_management_meeting_pre_result_status=not_yet_reviewed_as_of_2026-03-06`、`p11_sumitomo_riko_management_result_source_status=not_found`、`p11_system_management_review_status=pending_or_no_result_source_found`、`p11_sumitomo_riko_post_0313_context=no_srk_result_source_found; funding_context_shifted_to_mirai_creation_shareholder_reorganization_and_other_vc_cvc_discussions` をv11 current truthとして扱うか。
27. v11でも住友理工をbuyer/orderへ昇格せず、3/13以降のMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討はfunding/survival laneとして分離する運用でよいか。
28. p18 Yellow Duckの経済界記事の中山氏説明は `low_p_market_structure` の補強sourceとして採用するが、wave lane general source限定で、YD固有失敗証明にはしない運用でよいか。
29. p18 Yellow Duckの公式設立日 `2023-08-04` とDB `2019-01-01` の衝突を、別worker/別タスクでDB補正候補にするか。このv12ではDB補正しない。
30. p18 Yellow Duckの福岡市実証、PoC/TRL4、受賞/登壇/メディア露出をvalidation/survival contextとして保持し、商用UE/R_net粗利sourceからは明示除外する運用でよいか。
31. p18 Yellow DuckのVC DDは `/Users/masa/projects/knowledge/yd.md` の要約のみとし、VC別コメント、DD日、VC名、投資見送り理由原文がjoinされるまで `vc_dd_original` 扱いにしない運用でよいか。
32. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v12カードをレビューし、finance/cash分類、JOYCLE damage source split、p07 bridge validation source join、p20-p21 validation value source split、p11 membrane gate source join、p11 system GP30 projection refresh、p11 Sumitomo Riko transaction proof lookup、p11 Sumitomo Riko management meeting result lookup、p18 YD source refresh の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v12 flags を採用・差し戻し判定する。
...
10. 住友理工の経営会議review結果はread-only確認済み。pre-result sourceはあるがpost-result sourceは未発見のため、結果sourceが新規提示されるまでは追加workerを切らず、buyer/orderへ昇格しない。
11. p06/p11はSurvival guard中心で、AMED/SIP/政策実証に加え、p11の3/13以降Mirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討をR_net粗利や住友理工resultへ混ぜない運用を維持する。
12. p18 Yellow Duckは、Yellow Duck固有commercial UE/LCOE、契約、見積/料金、請求、入金、製造原価、設置費、O&M、保険、検収、顧客価格、VC DD原文がjoinされるまで正式P/R_net値へ進めない。設立日衝突はDB補正候補として別タスクに切る。
...
- p11 Sumitomo Riko management meeting result lookupをBZM採用判断として戻し、`p11_sumitomo_riko_management_result_lookup_status=completed_read_only`、`p11_sumitomo_riko_management_meeting_pre_result_source=gmail_19cc206f936d1405`、`p11_sumitomo_riko_management_meeting_candidate_date=2026-03-13`、`p11_sumitomo_riko_management_meeting_pre_result_status=not_yet_reviewed_as_of_2026-03-06`、`p11_sumitomo_riko_management_result_source_status=not_found`、`p11_sumitomo_riko_post_0313_context=no_srk_result_source_found; funding_context_shifted_to_mirai_creation_shareholder_reorganization_and_other_vc_cvc_discussions` を明記した。
...
- p18 Yellow Duckは `low_p_market_structure` を主分類、`rnet_gross_margin_viability` を補助分類として戻した。
- p18 Yellow Duckは `p18_low_p_market_structure_status=review_only_general_lane_sources_joined`、`p18_yd_specific_ue_lcoe_status=yd_specific_commercial_ue_lcoe_not_joined`、`p18_rnet_gross_margin_viability_status=gross_margin_not_joined`、`p18_prs_use=review-only` として、正式P/R_net値へ進めない形にした。
- p18 Yellow Duckの経済界記事とJapan Energy Timesはwave lane general sourceとして扱い、YD固有失敗証明にはしていない。
- p18 Yellow Duckの福岡市実証、PoC/TRL4、受賞/登壇/メディア露出はvalidation/survival contextに限定し、商用UE成立やR_net粗利成立には使っていない。
- p18 Yellow Duckの公式source上の活動継続と、DB上のAMD関与終了を分けた。公式設立日 `2023-08-04` とDB `2019-01-01` の衝突はDB補正候補としてflagしたが、このworkerではDB補正していない。
- p18 Yellow DuckのVC DDは `/Users/masa/projects/knowledge/yd.md` の要約のみとし、VC別コメント、DD日、VC名、投資見送り理由原文は未接続のまま `vc_dd_original` 扱いにしていない。
