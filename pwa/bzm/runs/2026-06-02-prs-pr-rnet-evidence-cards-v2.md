# P/R_net evidence cards v2 — 9PJ finance/cash classification refresh
...
今回のfinance分類で追加sourceは薄い。p09はdamage guard/source pack側の対象であり、今回のfinance/cash source packではsource id単位の追加分類を採用しない。販売前夜・装置候補を実績R_netへ戻すのは危険なので、次research対象に留める。

| source_id_or_path | source_status | classification | evidence_label | prs_use | double_count_guard | why_not_formal_score_yet |
|---|---|---|---|---|---|---|
| p09 monthly_reports / project_knowledge / source_cache（source inventory上の候補） | mixed | `future_or_draft_plan` | `damage_guard_research_target` | `review-only` | JB-02/JB-02A販売前夜を正のR_netにしない。売上候補と本命R&D毀損を分ける。 | 装置原価、販売予定単価、導入/保守、R&D予算推移、AMD関与終結ログが未接続。 |
| 追加finance source | not currently available in current finance pack | - | - | `exclude from PRS` | finance source不足をもって売上ゼロ確定や負R_net確定にしない。 | 次のdamage/reinvestment source packで扱う。 |
...
| p03/p09/p18 | 追加finance sourceなし | - | - | `exclude from PRS` | finance source不足を正式R_net値に変換しない。 |
...
1. `research_contract_cash` は `amd_billing_or_research_contract` の下位ラベルとして採用済み前提でv2へ反映した。今後のreviewでは、p20 NIMS / p21 愛媛大学・PSII / p11 SIPをSU本体売上から分離する運用を継続するか確認する。
...
5. p03/p04/p09/p18は今回のfinance分類で追加sourceが薄い。次に進めるなら public/knowledge research、damage/reinvestment pack、UE/LCOE research に分ける。
6. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v2カードをレビューし、finance/cash分類の `classification` / `evidence_label` / `prs_use` / `double_count_guard` を採用・差し戻し判定する。
2. 次workerを切るなら、`bridge and validation source pack`、`damage/reinvestment source pack`、`public/knowledge research pack`、`BZM judgement brief` に分ける。
3. p21は処理単価485円/L・従量課金モデルの出典/計算前提、U1とU4/U5分離を追加確認する。
4. p20はNIMS研究契約cashと利益度外視テスト販売/評価データ価値を分ける。
5. p07はリサイクル商流の契約/粗利/入金サイト/補助金後払い/本命LISMIC影響をjoinする。
6. p06/p11はSurvival guard中心で、AMED/SIP/政策実証をR_net粗利に混ぜない運用を維持する。
...
- 9PJ横断で、R_netに入れてよい候補、Survivalに置く候補、PRSから除外する候補、研究契約cashとしてreview-onlyに置く候補を一覧化した。
