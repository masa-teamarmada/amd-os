# P/R_net evidence cards v7 — p11 membrane gate source join refresh
...
### Membrane gate source join / v7 flags

| field | v7 value | source / basis | prs_use | why_not_formal_score_yet |
|---|---|---|---|---|
...
| `p11_membrane_gp30_status` | `candidate_not_joined_to_cogs` | BWE GP30%メモ | `review-only` | GP30%候補の元row/source id、膜単価、COGS、工数、QA、物流、保証、歩留まりが未join。 |
...
- GP30%候補は、元row/source id・COGS・buyer・契約・請求・入金が未joinなので、`candidate_not_joined_to_cogs` の `review-only` に留める。
...
| p11 BWE | BWE GP30%メモ / `2026-06-02-p11-membrane-gate-source-join.md` | `su_revenue_candidate` | `bottleneck_external_sale_candidate` | `review-only` | 膜外販は重要なR_net候補だが、`supply_capacity + gross_margin_basis + main_red_impact`、さらに buyer/contract/invoice/payment/cost が揃うまで正のR_netへ昇格しない。 |
...
#### p11 membrane gate source join / v7 flags
...
| p11 BWE | `p11_membrane_gp30_status` | `candidate_not_joined_to_cogs` | `review-only` | GP30%候補は元row/source id、COGS、buyer未join。 |
...
| 型 | 該当PJ | PRSで見る意味 | ガード |
|---|---|---|---|
| 高P・低/未確定R_net | ティエム、BWE | 本命の天井は大きいが生存設計が弱い。BWE膜外販は `bottleneck_external_sale_candidate` だが `review-only`。 | 7軸のGO/NO_GOを上書きしない。BWEは `supply_capacity + gross_margin_basis + main_red_impact` と buyer/contract/invoice/payment/cost が揃うまでR_net昇格禁止。 |
...
16. p11 membrane gate source joinはBZM採用。p11膜外販を `p11_membrane_external_sale_status=bottleneck_external_sale_candidate` / `p11_membrane_prs_use=review-only` としてv7 current truthへ戻した。このままreview-only固定でよいか。
...
19. p11 GP30%候補は、元row/source id・COGS・buyer等が未joinなので `p11_membrane_gp30_status=candidate_not_joined_to_cogs` の `review-only` に留める運用でよいか。
20. p11膜外販の本命RED影響は、sourceだけで決めず `p11_membrane_main_red_impact_status=requires_masa_or_bzm_review` として、まさ/BZM review requiredにする運用でよいか。
21. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v7カードをレビューし、finance/cash分類、JOYCLE damage source split、p07 bridge validation source join、p20-p21 validation value source split、p11 membrane gate source join の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v7 flags を採用・差し戻し判定する。
...
8. p11はGP30%メモの元row/source id、膜SKU、単価、COGS、外販buyer、contract、invoice、payment、RED本命影響レビューをsource単位でjoinする。揃うまで `p11_do_not_use_as=positive_R_net, formal_R_net_value, sales_actual, gross_margin_actual` を維持する。
9. p06/p11はSurvival guard中心で、AMED/SIP/政策実証をR_net粗利に混ぜない運用を維持する。
