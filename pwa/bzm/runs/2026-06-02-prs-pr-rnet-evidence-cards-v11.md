# P/R_net evidence cards v11 — p11 Sumitomo Riko management result refresh
...
28. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v11カードをレビューし、finance/cash分類、JOYCLE damage source split、p07 bridge validation source join、p20-p21 validation value source split、p11 membrane gate source join、p11 system GP30 projection refresh、p11 Sumitomo Riko transaction proof lookup、p11 Sumitomo Riko management meeting result lookup の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v11 flags を採用・差し戻し判定する。
