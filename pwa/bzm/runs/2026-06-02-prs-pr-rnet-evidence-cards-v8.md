# P/R_net evidence cards v8 — p11 GP30 lane split
...
| source_cache `p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` | found exact | `RED_10kWh_system_gross_margin_candidate` | `system_gp30_source` | `review-only` | raw textは「10kWhシステムの総益率GP30%」。膜単体外販R_net根拠にしない。 | 価格、COGS、契約、請求、入金、system scopeが未join。添付Excel本文も未join。 |
...
### GP30 raw source lane split / v8 flags

| field | v8 value | source / basis | prs_use | why_not_formal_score_yet |
|---|---|---|---|---|
...
| `p11_gp30_missing_basis` | `price + COGS + contract + invoice + payment + system_scope` | GP30 raw source lookup | gate | 価格・COGS・契約・請求・入金・system scopeが未join。 |
...
| `p11_system_gp30_status` | `found_exact_but_missing_cogs_and_transaction_join` | source_cache `p11_gmail_19c98f0f1d7cea62` | `review-only` | exact sourceはあるが、価格・COGS・契約・請求・入金・system scope未join。 |
...
- GP30% exact raw sourceは見つかったが、本文は「10kWhシステムの総益率GP30%」であり、膜単体外販ではない。したがって `p11_membrane_gp30_status=not_supported_by_found_gp30_source`、`p11_system_gp30_status=found_exact_but_missing_cogs_and_transaction_join` と分ける。
- 10kWh system GP30はcomparison-layer sourceとして残せるが、価格・COGS・契約・請求・入金・system scope未joinのため `review-only`。膜外販R_net根拠、formal R_net value、sales actual、gross margin actualには使わない。
- SIP/共同研究cash、SIP/東京都実証、AMD billingを膜外販gross marginへ混ぜると二重カウントになる。

### 次に必要な観測データ

- 膜単体の量産可否、単価、粗利、外販先、供給余力。
- 10kWh system GP30の価格、COGS、契約、請求、入金、system scope、添付Excel/計算根拠。
...
| p11 BWE | source_cache `p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` | `RED_10kWh_system_gross_margin_candidate` | `system_gp30_source` | `review-only` | GP30 exactは見つかったが、raw textは10kWh system lane。膜単体外販R_net根拠として使わない。価格・COGS・契約・請求・入金・system scope未join。 |
...
| p11 BWE | `p11_gp30_missing_basis` | `price + COGS + contract + invoice + payment + system_scope` | gate | 価格・COGS・契約・請求・入金・system scopeが未join。 |
| p11 BWE | `p11_membrane_gp30_status` | `not_supported_by_found_gp30_source` | `review-only` | 見つかったGP30 sourceは膜単体外販ではない。 |
| p11 BWE | `p11_system_gp30_status` | `found_exact_but_missing_cogs_and_transaction_join` | `review-only` | 10kWh system GP30は見つかったが、COGSと取引joinがない。 |
...
| 型 | 該当PJ | PRSで見る意味 | ガード |
|---|---|---|---|
| 高P・低/未確定R_net | ティエム、BWE | 本命の天井は大きいが生存設計が弱い。BWE膜外販は `bottleneck_external_sale_candidate` だが `review-only`。p11 GP30 exactは見つかったが10kWh system laneで、膜外販laneとは別。 | 7軸のGO/NO_GOを上書きしない。BWEは `supply_capacity + gross_margin_basis + main_red_impact` と buyer/contract/invoice/payment/cost が揃うまでR_net昇格禁止。10kWh system GP30も価格・COGS・契約・請求・入金・system scopeが揃うまで正式R_netへ使わない。 |
...
21. p11 GP30を `p11_membrane_gp30_status=not_supported_by_found_gp30_source` と `p11_system_gp30_status=found_exact_but_missing_cogs_and_transaction_join` に分け、system側も価格・COGS・契約・請求・入金・system scope未joinの `review-only` に留める運用でよいか。
...
23. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v8カードをレビューし、finance/cash分類、JOYCLE damage source split、p07 bridge validation source join、p20-p21 validation value source split、p11 membrane gate source join、p11 GP30 lane split の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v8 flags を採用・差し戻し判定する。
...
9. p11 10kWh system GP30は、source_cache `p11_gmail_19c98f0f1d7cea62` の添付Excel/計算根拠、価格、COGS、契約、請求、入金、system scopeを別laneでjoinする。膜外販laneの欠損補完には使わない。
10. p06/p11はSurvival guard中心で、AMED/SIP/政策実証をR_net粗利に混ぜない運用を維持する。
...
- p11は `p11_membrane_supply_capacity_status=blocked_missing_supply_allocation`、`p11_membrane_gp30_status=not_supported_by_found_gp30_source`、`p11_system_gp30_status=found_exact_but_missing_cogs_and_transaction_join`、`p11_membrane_cogs_status=not_currently_available`、`p11_membrane_external_buyer_status=not_joined`、`p11_membrane_contract_status=not_joined`、`p11_membrane_invoice_status=not_joined`、`p11_membrane_payment_status=not_joined`、`p11_membrane_main_red_impact_status=requires_masa_or_bzm_review` を戻した。
...
- 9PJ横断summaryへ、p11 GP30 exactは見つかったが膜単体外販ではなく10kWh system laneなので、膜外販R_net根拠として使わない、と追記した。
