# P/R_net evidence cards v4 — JOYCLE damage source split refresh
...
- v4では JOYCLE damage source split を戻し、support end date、support end reason、R&D budget / engineering time damage、sales-before-proof、operating cost source、billing_cycles exclusion を明示的に分離する。
- v4では p09 JOYCLEの分類を `db_current_truth_plus_knowledge_current_with_normalized_column_gap` とする。DB/knowledgeの終了状態は一致し、gapは `project_ventures.amd_support_ended_at` 正規化カラムだけに限定する。
...
1. `research_contract_cash` は `amd_billing_or_research_contract` の下位ラベルとして採用済み前提でv4へ継続反映した。今後のreviewでは、p20 NIMS / p21 愛媛大学・PSII / p11 SIPをSU本体売上から分離する運用を継続するか確認する。
...
10. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v4カードをレビューし、finance/cash分類と JOYCLE damage source split の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v4 flags を採用・差し戻し判定する。
...
5. p21は処理単価485円/L・従量課金モデルの出典/計算前提、U1とU4/U5分離を追加確認する。
6. p20はNIMS研究契約cashと利益度外視テスト販売/評価データ価値を分ける。
7. p07はリサイクル商流の契約/粗利/入金サイト/補助金後払い/本命LISMIC影響をjoinする。
8. p06/p11はSurvival guard中心で、AMED/SIP/政策実証をR_net粗利に混ぜない運用を維持する。
...
- 9PJ横断で、R_netに入れてよい候補、Survivalに置く候補、PRSから除外する候補、研究契約cashとしてreview-onlyに置く候補、JOYCLE current truth gap / source hygiene issue、JOYCLE damage source split / v4 flagsを一覧化した。
