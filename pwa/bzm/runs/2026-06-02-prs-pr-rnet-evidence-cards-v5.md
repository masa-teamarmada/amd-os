# P/R_net evidence cards v5 — p07 bridge validation source join refresh
...
1. `research_contract_cash` は `amd_billing_or_research_contract` の下位ラベルとして採用済み前提でv5へ継続反映した。今後のreviewでは、p20 NIMS / p21 愛媛大学・PSII / p11 SIPをSU本体売上から分離する運用を継続するか確認する。
...
12. p20/p21の `su_revenue_candidate` を、いつ `review-only` から正式なusable sourceへ上げるか。最低条件は、契約/請求/入金/原価/本命毀損の突合。
13. R_net rubric draftへ進む場合も、0-9値表、DB列、正式score、過去score再計算は引き続き行わない。

## 12. 次アクション

1. BZM司令塔が本v5カードをレビューし、finance/cash分類、JOYCLE damage source split、p07 bridge validation source join の `classification` / `evidence_label` / `prs_use` / `double_count_guard` / v5 flags を採用・差し戻し判定する。
...
- 9PJ横断で、R_netに入れてよい候補、Survivalに置く候補、PRSから除外する候補、研究契約cashとしてreview-onlyに置く候補、JOYCLE current truth gap / source hygiene issue、JOYCLE damage source split / v4 flags、p07 bridge validation source join / v5 flagsを一覧化した。
