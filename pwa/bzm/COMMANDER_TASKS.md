...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch、JOYCLE damage source split、PRS evidence cards v4 JOYCLE damage split refresh、p07 bridge validation source join、PRS evidence cards v5 p07 bridge refresh、p20-p21 validation value source split、PRS evidence cards v6 p20-p21 validation refresh、p11 membrane gate source join は作成済み。p11膜外販は `bottleneck_external_sale_candidate` / `review-only` としてBZMレビュー採用し、SIP/共同研究cash・政策実証・AMD billingは膜外販粗利と恒久分離する。次の `PRS evidence cards v7 p11 membrane refresh` worker `019e848e-4adc-7253-9364-fb1aa8743c86` がActive。
   - 残課題: evidence cards v7に、p11膜外販の3条件 `supply capacity + gross margin/COGS + main RED impact`、追加gate `external buyer / contract / invoice / payment / cost`、GP30%候補は `review-only`、本命RED影響はまさ/BZM review required、`do_not_use_as=positive_R_net, formal_R_net_value, sales_actual, gross_margin_actual` を戻す。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
