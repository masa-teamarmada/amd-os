...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch、JOYCLE damage source split、PRS evidence cards v4 JOYCLE damage split refresh は作成済み。v4は、JOYCLEの終結日、口述由来終結理由、sales-before-proof、R&D damage、operating cost、billing exclusionを分け、すべて正式値付けせずreview-only/除外に留めているためBZMレビュー採用。JOYCLEはOS/DB hygieneと追加source待ちに移し、次の `p07 bridge validation source join` worker `019e8471-69f0-77f0-b8fe-9a9fd8112ea1` がworktree `/Users/masa/.codex/worktrees/2012/amd-os` でActive。
   - 残課題: p07 bridge validationで、リサイクル商流を契約・請求・入金・原価・本命LISMIC影響の5点でsource joinし、review-only解除条件がどこまで埋まっているか確認する。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
