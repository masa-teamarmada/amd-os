...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7/v8/v9 refresh、p11 GP30 raw source lookup、p11 system GP30 source join、p11 Sumitomo Riko transaction proof lookup は作成済み。住友理工は finance plan / investment review / management meeting review counterparty としては強いが、confirmed buyer / purchase order / signed contract / LOI / formal quote acceptance / invoice / payment / delivery acceptance / vendor-paid COGS は未発見。system lane と membrane lane は分離維持し、system GP30をmembrane external-sale GPに使わない判断でBZMレビュー採用。次の `PRS evidence cards v10 p11 transaction proof refresh` worker `019e84b8-a710-7a91-9751-4f968b0c3ba2` がActive。
   - 残課題: v10で `p11_sumitomo_riko_context=finance_plan_investment_review_and_management_meeting_review`、`p11_sumitomo_riko_counterparty_status=review_counterparty_not_confirmed_buyer`、system transaction / contract / order / invoice / payment / delivery acceptance / vendor-paid COGS / QA・物流・保証・在庫・歩留まり未joinを戻す。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
