...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch、JOYCLE damage source split、PRS evidence cards v4 JOYCLE damage split refresh、p07 bridge validation source join、PRS evidence cards v5 p07 bridge refresh、p20-p21 validation value source split、PRS evidence cards v6 p20-p21 validation refresh は作成済み。v6は、p20/p21の `validation_value_source_standard`、`validation_value_not_rnet_gross_margin`、研究契約cash分離、p21 485円/Lの `usable_source_not_formal_rnet_value`、U1短期cashとU4/U5本命Pの二重カウント防止を戻しており、BZMレビュー採用。次の `p11 membrane gate source join` worker `019e8489-e596-78e3-a7af-77e9f58dbdf5` がworktree `/Users/masa/.codex/worktrees/ed89/amd-os` でActive。
   - 残課題: p11 Blue Water Energyの膜外販/GP30%/SIP/共同研究cash/RED本命影響を、R_net候補・Survival guard・研究契約cash・本命毀損guardに分け、供給余力・粗利・本命RED影響の3条件が揃うまで外販を正式R_netへ上げない判断を確認する。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
