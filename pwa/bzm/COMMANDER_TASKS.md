...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch、JOYCLE damage source split は作成済み。JOYCLE splitは、終結日と終結理由を分け、口述由来理由を `oral_history_pending_confirmation` に留め、JB-02/JB-02A販売前夜・装置導入/保守・R&D予算/人員をすべて `review-only` とし、`billing_cycles` をPRS本体から除外しているためBZMレビュー採用。次の `PRS evidence cards v4 JOYCLE damage split refresh` worker `019e846a-bbb1-7ad1-b957-33afedb93d52` がworktree `/Users/masa/.codex/worktrees/3ee8/amd-os` でActive。
   - 残課題: evidence cards v4に、JOYCLE splitの `support_end_reason_status=oral_history_pending_confirmation`、`sales_before_proof_status=review-only`、`rd_budget_engineering_time_damage=review-only`、`billing_cycles_use=exclude_from_prs` などを戻す。OS側では `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
