...
8. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Watch（BZM司令塔レビュー待ち）。v7までのcurrent truthに加え、`pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v8.md` を作成済み。v8では `p11_gp30_raw_source_status=found_exact` として `source_cache.cache_id=p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd`、derived knowledge `904dbf36-5194-4df2-a860-d23810be0054` / `239c5076-c1b5-4455-b6d1-34434bad6fa1` を戻した。raw textは「10kWhシステムの総益率GP30%」なので、膜単体外販ではなく `RED_10kWh_system_gross_margin_candidate` とし、`p11_membrane_gp30_status=not_supported_by_found_gp30_source` と `p11_system_gp30_status=found_exact_but_missing_cogs_and_transaction_join` を分離済み。P/R_netは正式DB列・rubric未採用のまま扱う。
   - 残課題: BZM司令塔レビュー。10kWh system GP30はPRS comparison-layer sourceとして残すが、価格、COGS、契約、請求、入金、system scopeが未joinなので `review-only` 維持。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。司令塔判断事項は、JOYCLEの `project_ventures.amd_support_ended_at` 補正をOS/DB workerへ切るか、口述由来の終結理由を `oral_history_pending_confirmation` のまま追加source待ちにするか、CTB型のSurvival guard分離の強さ、JOYCLE型damage guardの名称/扱い、YD型をP側/粗利不成立側の両方へ置く粒度、p11 BWE膜外販を `review-only` から動かす3条件+buyer/contract/invoice/payment/cost gate、p11本命RED影響をまさ/BZM review requiredにする運用、CX/SXの利益度外視PoC/テスト販売を粗利ではなく評価データ価値へ置くか、研究機関契約cashをSU本体売上から恒久分離するラベル採用、p07 bridge businessを5点セットが揃うまでreview-only維持するか、p07 cost constraint source 2件を正式値ではない `usable source` として扱うか。
