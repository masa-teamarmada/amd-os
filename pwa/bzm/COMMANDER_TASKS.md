...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7/v8/v9/v10 refresh、p11 GP30 raw source lookup、p11 system GP30 source join、p11 Sumitomo Riko transaction proof lookup、p11 Sumitomo Riko management meeting result lookup は作成済み。2026-03-06 Gmailに「まだ審議できていない」「2026-03-13が候補日」というpre-result sourceはあるが、3/13後のresult sourceは未発見。BZMレビューでは `p11_system_management_review_status=pending_or_no_result_source_found` を採用し、住友理工をbuyer/orderへ昇格しない判断。次の `PRS evidence cards v11 p11 management result refresh` worker `019e84c4-80d7-7473-b8e1-41a8d7fa4ff8` がActive。
   - 残課題: v11で `p11_sumitomo_riko_management_result_lookup_status=completed_read_only`、`p11_sumitomo_riko_management_meeting_pre_result_source=gmail_19cc206f936d1405`、`p11_sumitomo_riko_management_result_source_status=not_found`、`p11_system_management_review_status=pending_or_no_result_source_found` を戻す。3/13以降のMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討は住友理工resultではなくfunding/survival laneとして扱う。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
