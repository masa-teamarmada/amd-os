...
| N | 文体「である」化変換 (PF-020) | 🟡 **フェーズA (Ch1) 確定・O に統合** | O (ナラティブ推敲) と1パスで統合実施、下記参照 |
...
- **2026-07-12 O に統合**: まさが全体通読でナラティブパートの表現面 (比喩・言い回し) にも違和感を指摘 → 語尾だけの機械変換 (N) を単独で正本反映せず、**O (ナラティブ推敲) の1パスに統合** (である調統一は O のワーカー指示に組み込み済み)。Ch1 のフェーズA成果は O のワーカーに参考資料として渡す。N 単独のフェーズBは実施しない。
