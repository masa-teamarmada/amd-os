...
8. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Watch（BZM司令塔レビュー待ち）。v7/v8までのcurrent truthに加え、`pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v9.md` を作成済み。v9では `p11 system GP30 source join` を採用し、Gmail thread + 添付Excel `HH20260226②SRK試算_BWE財務計画_260221_y.xlsx` までjoinできた状態を戻した。Excelには10kW/100kW systemの売価・原価・GP30%、O&M売上/原価、売電売上/原価があるため、system laneは `p11_system_gp30_status=found_exact_with_excel_basis_but_no_transaction_join`、`p11_system_cogs_basis_status=projection_only_not_cogs_proof` として更新。膜単体外販ではなく、Sumitomo Rikoは `review_counterparty_not_confirmed_buyer`。P/R_netは正式DB列・rubric未採用のまま扱う。
   - 残課題: BZM司令塔レビュー。10kW/10kWh system GP30はPRS comparison-layer sourceとして残すが、contract / invoice / payment / vendor-paid COGS / delivery acceptance / QA・物流・保証・在庫・歩留まりが未joinなので `review-only` 維持。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。司令塔判断事項は、system GP30をreview-onlyで採用するか、`projection_only_not_cogs_proof` の厳しめ表現で固定するか、Sumitomo Rikoをconfirmed buyerではなくreview counterpartyとして扱うか、JOYCLEの `project_ventures.amd_support_ended_at` 補正をOS/DB workerへ切るか、口述由来の終結理由を `oral_history_pending_confirmation` のまま追加source待ちにするか、CTB型のSurvival guard分離の強さ、JOYCLE型damage guardの名称/扱い、YD型をP側/粗利不成立側の両方へ置く粒度、p11 BWE膜外販を `review-only` から動かす3条件+buyer/contract/invoice/payment/cost gate、p11本命RED影響をまさ/BZM review requiredにする運用、CX/SXの利益度外視PoC/テスト販売を粗利ではなく評価データ価値へ置くか、研究機関契約cashをSU本体売上から恒久分離するラベル採用、p07 bridge businessを5点セットが揃うまでreview-only維持するか、p07 cost constraint source 2件を正式値ではない `usable source` として扱うか。
