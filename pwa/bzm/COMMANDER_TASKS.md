...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch、JOYCLE damage source split、PRS evidence cards v4 JOYCLE damage split refresh、p07 bridge validation source join、PRS evidence cards v5 p07 bridge refresh は作成済み。v5は、p07 bridge businessを5点セット未充足の `review-only` に固定し、初期R&D費用回収難と柏市初期費用をR_net押し下げ方向のsourceとして戻しつつ、formal revenue/正式R_netへ昇格していないためBZMレビュー採用。次の `p20-p21 validation value source split` worker `019e847d-1543-79c0-9641-e89f54810c27` がActive。
   - 残課題: p20/p21 validation valueで、利益度外視PoC・テスト販売・有料製作・研究契約cashを、粗利目的 / 評価データ目的 / 研究契約cash / Survival cashへ分ける。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
