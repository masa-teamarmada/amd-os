...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7/v8/v9/v10/v11 refresh、p11 GP30 raw source lookup、p11 system GP30 source join、p11 Sumitomo Riko transaction proof lookup、p11 Sumitomo Riko management meeting result lookup は作成済み。v11はBZMレビュー採用。2026-03-06 Gmailにpre-result sourceはあるが、3/13後のresult sourceは未発見のため `p11_system_management_review_status=pending_or_no_result_source_found` をcurrent truth化し、住友理工をbuyer/orderへ昇格しない。次の `YD low-P / UE guard source split` worker `019e84c9-afb2-7a33-887b-d0c82b716d43` がActive。
   - 残課題: p18 Yellow Duck / YDについて、P側の低P/市場構造の筋悪さとR_net側のUE/粗利不成立をsource単位で分け、同じ根拠を二重カウントしないguardを作る。p11は新しい住友理工result source ID / Drive file ID / Gmail thread IDが出るまで追加lookupを切らず、system lane / membrane lane 分離とreview-onlyを維持する。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
