...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7 refresh、p11 membrane source join deep dive、p11 GP30 raw source lookup は作成済み。GP30 raw sourceは `source_cache.cache_id=p11_gmail_19c98f0f1d7cea62` / row `d083a88f-04d8-4d99-9f61-b1be87b586cd` としてfound exact。ただしraw textは「10kwhシステムの総益率GP30%」で、膜単体外販ではなく `RED_10kWh_system_gross_margin_candidate` と分類するのが安全。次の `PRS evidence cards v8 p11 GP30 lane split` worker `019e849e-9c09-7230-9ca6-029397385530` がActive。
   - 残課題: v8で `p11_membrane_gp30_status` と `p11_system_gp30_status` を分離し、10kWh system GP30はPRS comparison-layer sourceとして残すが膜外販R_net根拠には使わない形へ戻す。価格、COGS、契約、請求、入金、system scopeが未joinなのでreview-only維持。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
