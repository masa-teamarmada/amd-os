...
8. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Watch（BZM司令塔レビュー待ち）。v7/v8/v9までのcurrent truthに加え、`pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v10.md` を作成済み。v10では `p11 Sumitomo Riko transaction proof lookup` を採用し、住友理工は finance plan / investment review / management meeting review counterparty として強いが、confirmed buyer / purchase order / signed contract / LOI / formal quote acceptance ではないと戻した。system laneは `p11_system_gp30_status=found_exact_with_excel_basis_but_no_transaction_join`、`p11_system_cogs_basis_status=projection_only_not_cogs_proof`、`p11_sumitomo_riko_counterparty_status=review_counterparty_not_confirmed_buyer` を維持。P/R_netは正式DB列・rubric未採用のまま扱う。
   - 残課題: BZM司令塔レビュー。10kW/10kWh system GP30はPRS comparison-layer sourceとして残すが、contract / order / invoice / payment / vendor-paid COGS / delivery acceptance / QA・物流・保証・在庫・歩留まりが未joinなので `review-only` 維持。必要なら2026-03-13前後の住友理工management meeting resultだけを別workerでread-only確認する。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
