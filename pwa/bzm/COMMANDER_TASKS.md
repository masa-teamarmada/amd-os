...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch は作成済み。classification patchは、comparison layer current truth、review-only継続、PRS scoreから除外するcontext、OS/DB hygiene required、formal rubric化禁止項目を分け、正式rubric/DB化へ進めない境界を守っているためBZMレビュー採用。次の `JOYCLE damage source split` worker `019e8466-5ba4-7511-8d4a-900302338459` がworktree `/Users/masa/.codex/worktrees/205d/amd-os` でActive。
   - 残課題: JOYCLE damage source splitで、終結日、終結理由、R&D予算/人員、JB-02/JB-02A販売前夜、装置導入/保守、billing/cash timingを、DB current truth・knowledge・口述・raw source・future/draft・cash timingに分ける。OS側では `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
