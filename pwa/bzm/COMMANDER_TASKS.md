...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7/v8/v9/v10 refresh、p11 GP30 raw source lookup、p11 system GP30 source join、p11 Sumitomo Riko transaction proof lookup は作成済み。v10では住友理工を finance plan / investment review / management meeting review counterparty として扱い、confirmed buyer / purchase order / signed contract / LOI / formal quote acceptance / invoice / payment / delivery acceptance / vendor-paid COGS へは昇格しない判断をBZMレビュー採用。system lane と membrane lane は分離維持し、system GP30をmembrane external-sale GPに使わない。次の `p11 Sumitomo Riko management meeting result lookup` worker `019e84be-022d-7290-be3c-105cf2c1ecf6` がActive。
   - 残課題: 2026-03-13前後の住友理工management meeting resultをread-onlyで確認し、投資 / 協業 / 購入 / LOI / 契約 / 発注 / 見送りのsource有無を判定する。結果sourceが出るまでは、住友理工をbuyer/orderへ昇格しない。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
