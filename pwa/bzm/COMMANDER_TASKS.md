...
8. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Watch（BZM司令塔レビュー待ち）。v7/v8/v9/v10までのcurrent truthに加え、`pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v11.md` を作成済み。v11では `p11 Sumitomo Riko management meeting result lookup` を採用し、2026-03-06時点のpre-result source `gmail_19cc206f936d1405`、候補日 `2026-03-13`、post-result source未発見をsource-backed current truthとして戻した。`p11_system_management_review_status=pending_or_no_result_source_found`、`p11_sumitomo_riko_counterparty_status=review_counterparty_not_confirmed_buyer`、system transaction/order/contract/invoice/payment/delivery acceptance/vendor-paid COGSの `not_joined`、system lane / membrane lane分離は維持。P/R_netは正式DB列・rubric未採用のまま扱う。
   - 残課題: BZM司令塔レビュー。10kW/10kWh system GP30はPRS comparison-layer sourceとして残すが、contract / order / invoice / payment / vendor-paid COGS / delivery acceptance / QA・物流・保証・在庫・歩留まりが未joinなので `review-only` 維持。3/13以降のMirai Creation / 株式集約 / 会社存続 / 他VC・CVC検討は住友理工resultではなくfunding/survival laneとして扱う。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
