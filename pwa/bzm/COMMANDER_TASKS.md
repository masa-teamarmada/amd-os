...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7 refresh、p11 membrane source join deep dive は作成済み。deep dive後もp11膜外販は `bottleneck_external_sale_candidate` / `review-only` 維持が安全。BWE.mdにより膜外販がライスワーク候補であること、同時に膜が本命RED最大ボトルネックで、2026-05時点で調達目処未確立であることをsource basisとして強化できる。一方、GP30%元row/source id、膜SKU/単価/COGS、buyer/contract/invoice/payment、本命RED impact分類は未join。次の `p11 GP30 raw source lookup` worker `019e8499-2639-77c1-8ef4-d813ad6f14a1` がActive。
   - 残課題: service_role read-onlyまたはOS/source workerで `source_cache` / `project_knowledge` / `monthly_reports` / `project_meeting_summaries` を検索し、GP30 raw row/source idを探す。見つかった場合も膜単体外販・RED装置部材・研究契約内費目を分類し、COGS範囲とbuyer/contract/invoice/payment joinへ進む。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
