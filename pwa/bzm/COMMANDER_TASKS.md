...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7/v8/v9 refresh、p11 GP30 raw source lookup、p11 system GP30 source join は作成済み。v9は `p11_system_gp30_status=found_exact_with_excel_basis_but_no_transaction_join`、`p11_system_cogs_basis_status=projection_only_not_cogs_proof`、`p11_system_counterparty_status=review_counterparty_not_confirmed_buyer` として、system GP30をExcel projection basisあり・transaction proofなしの `review-only` に固定しておりBZMレビュー採用。次の `p11 Sumitomo Riko transaction proof lookup` worker `019e84b2-71d6-7933-8a88-da5db363f4d3` がActive。
   - 残課題: Sumitomo Riko周辺の contract / LOI / order / invoice / payment / delivery acceptance / vendor-paid COGS / QA・物流・保証・在庫・歩留まりをread-onlyで確認し、confirmed buyer/order sourceがあるか、review counterparty止まりかを判定する。膜外販は別sourceでSKU/COGS/buyer/contract/invoice/payment/RED影響が揃うまで `review-only`。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
