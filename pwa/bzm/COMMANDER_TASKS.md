...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS BZM judgement brief、classification adoption patch、JOYCLE damage source split、evidence cards v3/v4/v5/v6/v7 refresh は作成済み。v7はp11膜外販を `bottleneck_external_sale_candidate` / `review-only` とし、3条件 `supply_capacity + gross_margin_basis + main_red_impact` と追加gate `buyer / contract / invoice / payment / cost` が揃うまでR_net昇格禁止に戻しているためBZMレビュー採用。次の `p11 membrane source join deep dive` worker `019e8493-fbb0-76e0-a757-042e26b65923` がworktree `/Users/masa/.codex/worktrees/fd47/amd-os` でActive。
   - 残課題: p11のGP30%元row/source id、膜SKU/単価/COGS、供給余力、buyer/contract/invoice/payment、本命RED影響をsource単位で深掘りし、v7 fieldのうち更新可能なものとblocked維持のものを分ける。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
