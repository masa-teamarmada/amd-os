...
- BZM司令塔からworkerを切る場合、worker promptには `worker quiet mode` を明記する。workerは原則、親司令塔チャットへ進捗・中間報告・自己判断の完了報告を送らない。
- workerは、workerスレッド内でまさが「完全に完了」「OK」「これでよし」等と明示した後に、1回だけBZM司令塔へ最終closeoutを送る。
- 例外として、`UU` conflict、未分類dirty、権限/破壊的操作/外部判断、同じblocking conditionで進行不能など、司令塔介入が必要な場合だけ短いblocker/handoffを1回送る。
- BZM司令塔は、worker報告で親チャットを流さず、必要ならheartbeat / read_thread / 手動巡回で静かにworker状態を確認する。
- 台帳にはworker詳細ログを貼らず、Active workerあり、worker id、状態、次回確認条件、まさ要判断だけを短く残す。
...
2. worker稼働監視 / heartbeat運用
   ...
   - 現状: Active。全司令塔共通ルールとして採用。BZM司令塔の運用ルールへ `worker quiet mode` として反映済みで、次回worker promptにも入れる前提。
   - 残課題: 次にBZM workerを切る時、親チャットへの原則報告なし、まさ明示OK後の最終closeout 1回、例外blockerだけ短く報告、司令塔側heartbeat/read_thread確認をpromptへ明記する。
