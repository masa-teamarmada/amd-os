...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh は作成済み。v3は、JOYCLEへ `support_end_status=ended_current_truth_candidate`、`support_end_date=2026-03`、`support_end_db_gap=project_ventures.amd_support_ended_at_null`、`damage_guard_use=review-only`、分類 `db_stale_knowledge_current` を戻し、`negative R_net` を正式用語にせず、口述由来の終結理由をdamage guard理由へ直接混ぜないためBZMレビュー採用。次の `PRS BZM judgement brief` worker `019e845e-fe92-7f01-8495-14987a8336e6` がworktree `/Users/masa/.codex/worktrees/0af2/amd-os` でActive。
   - 残課題: BZM judgement briefで、OS/DB側の `project_ventures.amd_support_ended_at='2026-03-01'` 補正要否、JOYCLE終結理由の口述由来部分の扱い、CTB Survival guard分離、p20/p21 validation value、p07 bridge business昇格条件、p11膜外販解除条件、YD二重配置ルールを短い判断肢にする。0-9値表、DB化、過去score再計算はまだしない。
