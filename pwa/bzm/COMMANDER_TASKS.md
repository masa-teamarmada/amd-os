...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief は作成済み。briefは、OS/DB補正、JOYCLE口述由来理由、CTB Survival guard、p20/p21 validation value、p07 bridge business昇格条件、p11膜外販解除条件、YD二重配置ルールを短い判断肢に整理しているためBZMレビュー採用。次の `PRS classification adoption patch` worker `019e8462-e1e9-7132-9bf1-39d771ac21a3` がActive。
   - 残課題: classification adoption patchで、comparison layer current truthとして採用する分類、review-only継続、PRS scoreから除外するcontext、OS/DB hygiene required、formal rubric化禁止項目を一覧にする。OS側では `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
