...
| N | 文体「である」化変換 (PF-020) | 🔥 **drafting** (Ch1〜8 一括変換ワーカー起票) | ワーカー成果 → Opus 差分監査1名 → まさ確認 → 正本反映 |
...
- **担当**: 別セッション (spawn_task で起票、Sonnet 5)。**司令塔は変換自体を引き受けない** — 提案 md (before/after) を受け取り、Opus 差分監査1名 → まさ確認 → 正本反映の裁定だけ担当。
- **次アクション**: ワーカー成果 (`2026-07-12_style_conversion_v1.md` に集約) が上がったら → 語尾変換の網羅性・台詞誤変換ゼロ・数式/図表番号不変を機械検査 + Opus 監査 → まさ判定 → 承認後 Ch1〜8 正本一括反映 → push。
- **第9〜15章の並走ワーカーは触らない** — 現起草はそのまま走らせ、上がってきた時の司令塔ゲートで同じ機械変換を挟む。ワーカーへの追加通達なし。
- **まさ要判断**: 変換 v1 が上がったら Ch1 変換版でノリ確認 (「このノリで残り7章も」承認したら並列反映)。
