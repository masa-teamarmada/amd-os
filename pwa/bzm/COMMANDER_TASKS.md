...
8. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: workerが `pwa/bzm/runs/2026-06-01-prs-seven-axis-alignment.md` を作成。BZM一次判断は `Adopt as comparison layer`。続くworkerが `pwa/bzm/runs/2026-06-01-prs-9pj-delta-review.md` を作成し、9PJ差分としてティエム商社化/JOYCLE本命毀損/YD UE不成立/BWE初期自走性を強いPRS差分候補、CTBをR_net単純化の反例として整理した。さらにworkerが `pwa/bzm/runs/2026-06-01-prs-pr-rnet-evidence-cards.md` を作成し、9PJごとにP仮説、R_netの粗利貢献/運営コスト/本命毀損、7軸で説明できること、PRS追加説明、反例、次観測、rubric化暫定判定をカード化した。R_net guard workerが `pwa/bzm/runs/2026-06-01-prs-rnet-guard-memo.md` を作成し、CTB/JOYCLE/YD/BWEを中心に、売上ゼロ成立型・damage guard・低P/UE不成立・膜外販未確定の誤判定防止guard、R_netと混ぜない観測項目、次の観測項目draftへ渡せる/まだ使わない項目を整理した。P/R_net観測項目draft workerが `pwa/bzm/runs/2026-06-01-prs-pr-rnet-observation-items-draft.md` を作成し、P/R_net/Survival guardごとに観測項目・source候補・入力主体候補・更新頻度候補・guard条件・未確認flagを整理した。続くsource map workerが `pwa/bzm/runs/2026-06-01-prs-unverified-source-map.md` を作成し、未確認flagを9PJ evidence cardsへ戻し、各flagのsourceを `l2_extractable` / `worker_research` / `masa_or_bzm_judgement` に分類した。P/R_net L2 source inventory workerが `pwa/bzm/runs/2026-06-01-prs-l2-source-inventory.md` を作成し、既存L2/DB/docs/outbox/scriptから拾えるsource候補を `available_now` / `available_but_needs_join` / `not_currently_available` / `unsafe_to_infer` / `requires_masa_or_bzm_review` に分類した。finance/cash source pack workerが `pwa/bzm/runs/2026-06-02-prs-finance-cash-source-pack.md` を作成し、p07/p20/p21/p06/p11の売上・粗利・入金・助成金候補をsource id単位で整理した。billing vs SU revenue join map workerが `pwa/bzm/runs/2026-06-02-prs-billing-vs-su-revenue-join-map.md` を作成し、`billing_cycles` / 研究機関契約 / SU本体売上候補 / Survival cashを分けるラベル案と二重カウント防止ルールを整理した。PRS evidence cards v2 finance classification refresh workerが `pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v2.md` を作成し、9PJ evidence cardsへ `Finance/Cash source classification`、3値の `prs_use`、`research_contract_cash` 下位ラベル、`billing_cycles` 原則除外、横断分類一覧を戻した。bridge/validation source pack workerとdamage/reinvestment source pack workerがsource候補を整理し、JOYCLE AMD support end current truth review workerが `project_ventures.amd_support_ended_at` のNULLをsource hygiene issueとして確認した。PRS evidence cards v3 current truth refresh workerが `pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v3.md` を作成し、p09 JOYCLEへ `support_end_status=ended_current_truth_candidate`、`support_end_date=2026-03`、`support_end_db_gap=project_ventures.amd_support_ended_at_null`、`damage_guard_use=review-only`、分類 `db_stale_knowledge_current` を戻した。JOYCLE damage source split refresh workerが `pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v4.md` を作成し、p09 JOYCLEへ `current_truth_classification=db_current_truth_plus_knowledge_current_with_normalized_column_gap`、`support_end_reason_status=oral_history_pending_confirmation`、`sales_before_proof_status=review-only`、`rd_budget_engineering_time_damage=review-only`、`rd_source_status=mixed_raw_and_oral`、`operating_cost_source_status=review-only`、`billing_cycles_use=exclude_from_prs`、`do_not_use_as=positive_R_net, formal_damage_value, sales_actual, gross_margin_actual` と、`can_feed_evidence_cards_v4` / `needs_masa_confirm` / `needs_os_db_hygiene` / `needs_more_source` を戻した。PRS evidence cards v5 p07 bridge refresh workerが `pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v5.md` を作成し、p07 LiSTieへ `bridge_business_classification=bridge_business_candidate`、`bridge_revenue_stage=nda_mou_or_pipeline_candidate`、`bridge_invoice_status=not_currently_available`、`bridge_payment_status=not_currently_available`、`bridge_cost_constraint_status=available_constraint_source_not_value`、`main_lismic_impact_status=requires_masa_or_bzm_review`、`survival_cash_separate_status=grant_debt_equity_excluded_from_rnet`、5点セットrelease condition、double count guardを戻した。PRS evidence cards v6 p20-p21 validation refresh workerが `pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v6.md` を作成し、p20/p21へ `validation_value_source_standard=adopt_as_comparison_layer_classification`、`validation_value_not_rnet_gross_margin=true`、`research_contract_cash_parent_label=amd_billing_or_research_contract`、p20/p21別のvalidation / research contract cash / Survival / gross margin release condition / p21 485円/L UE source / U1-vs-U4/U5 double count guardを戻した。PRS evidence cards v7 p11 membrane refresh workerが `pwa/bzm/runs/2026-06-02-prs-pr-rnet-evidence-cards-v7.md` を作成し、p11 BWEへ `p11_membrane_external_sale_status=bottleneck_external_sale_candidate`、`p11_membrane_prs_use=review-only`、review解除条件 `supply_capacity + gross_margin_basis + main_red_impact`、buyer/contract/invoice/payment/cost必須gate、SIP/共同研究cash・政策実証・AMD billingの恒久分離、`p11_do_not_use_as=positive_R_net, formal_R_net_value, sales_actual, gross_margin_actual` を戻した。P/R_netは正式DB列・rubric未採用のまま扱う。
   - 残課題: BZM司令塔レビュー。司令塔判断事項は、JOYCLEの `project_ventures.amd_support_ended_at` 補正をOS/DB workerへ切るか、口述由来の終結理由を `oral_history_pending_confirmation` のまま追加source待ちにするか、CTB型のSurvival guard分離の強さ、JOYCLE型damage guardの名称/扱い、YD型をP側/粗利不成立側の両方へ置く粒度、p11 BWE膜外販を `review-only` から動かす3条件+buyer/contract/invoice/payment/cost gate、p11本命RED影響をまさ/BZM review requiredにする運用、CX/SXの利益度外視PoC/テスト販売を粗利ではなく評価データ価値へ置くか、研究機関契約cashをSU本体売上から恒久分離するラベル採用、p07 bridge businessを5点セットが揃うまでreview-only維持するか、p07 cost constraint source 2件を正式値ではない `usable source` として扱うか。
