...
9. BZM 7軸モデルとP×R×S/9軸候補の整合
   ...
   - 現状: Active。整合メモ、9PJ差分レビュー、P/R_net evidence cards、R_net guard memo、P/R_net観測項目draft、未確認flag source map、L2 source inventory、finance/cash source pack、billing vs SU revenue join map、evidence cards v2 finance classification refresh、bridge/validation source pack、damage/reinvestment source pack、JOYCLE AMD support end current truth review、PRS evidence cards v3 current truth refresh、PRS BZM judgement brief、PRS classification adoption patch、JOYCLE damage source split、PRS evidence cards v4 JOYCLE damage split refresh、p07 bridge validation source join、PRS evidence cards v5 p07 bridge refresh、p20-p21 validation value source split は作成済み。p20/p21 splitは、利益度外視PoC・テスト販売・有料製作を `validation_value_source` として標準分類に置き、R_net粗利へ入れず、NIMS/愛媛大学/PSIIの研究契約cashをSU本体売上から恒久分離しているためBZMレビュー採用。次の `PRS evidence cards v6 p20-p21 validation refresh` worker `019e8481-fd08-7822-9688-5e7a0a5977b5` がworktree `/Users/masa/.codex/worktrees/78bc/amd-os` でActive。
   - 残課題: evidence cards v6に、p20/p21の `validation_value_source_standard`、`validation_value_not_rnet_gross_margin`、研究契約cash分離、p21 485円/Lの `usable_source_not_formal_rnet_value`、U1短期cashとU4/U5本命Pの二重カウント防止を戻す。OS側ではJOYCLE `project_ventures.amd_support_ended_at='2026-03-01'` 補正workerの要否判断が残る。0-9値表、DB化、過去score再計算はまだしない。
