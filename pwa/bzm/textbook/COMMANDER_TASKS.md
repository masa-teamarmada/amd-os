...
- **page-turner編集レビューと素材発掘ループ**
  ...
  - 追加進捗: 2026-06-02にdeep OS/source mining workerが `pwa/bzm/textbook/runs/2026-06-02-deep-os-source-mining-for-missing-book-content.md` を追加。OS/spec/manual/旧BZM/既存source mining/read-only transcript themeから、composite scene seed 28件、tool/question/checklist seed 40件、Ch00〜Ch24全章insertion map、reader-hook / budget-owner / paid-evidence / failure-repair / institution-nursery scene bank、安全分類と次rewrite promptを整理。
  - 残課題: `table extraction / toolkit appendix`、`support boundary pass`、Ch00〜Ch24 surgical residue passを切り、v6素材台帳、Act III source mining素材、Act V source mining素材、deep OS/source mining素材を本文へ直接貼らずcomposite scene / tool artifact化して反映する。
