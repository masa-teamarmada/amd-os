...
- 2026-06-03: `Vercel deploy quota hard gate` Active / freeze。Vercel 1日100deploy上限に再到達したため、旧worker close gateのpush/deploy要件よりquota保護を優先する。当面 `vercel deploy` とVercel自動deployを起こす可能性があるpushは禁止。mainだけでなくpreview対象branchも含む。workerはlocal build/test/check、必要ならlocal commitまでで止め、push/deployは `withheld due to Vercel quota gate` としてhandoffする。文言・微細UI・md・CSS・コメント・ログ文言ごとのdeployは禁止。quota回復後の自動retry方針も停止。deploy前には必ずdeploy bundleを作り、含める変更/除外する変更/local検証/予定deploy回数/push先/rollback/確認方法を提示し、承認後に1回だけpush/deployする。`pwa/scripts/deploy.sh` も `AMD_OS_VERCEL_DEPLOY_APPROVED=1` なしでは実行不可に変更。成果物: `pwa/bzm/textbook/runs/2026-06-03-vercel-quota-hard-gate-freeze.md`。この更新自体のpushもfreeze中のため withheld。
