...
> この章は何か: Before Zero の現場で繰り返し出てくる GO / WAIT / NO_GO、設立時期、律速軸、資源配分の判断を、後から再利用できる意思決定パターンとして整理する章です。実ケース本文は、D-7 Textbook Insights Textbook Insights の承認済み候補が来てから追記します。
...
主な D-7 Textbook Insights `practice_kind` は `decision_branch` です。既存の D-7 Textbook Insights schema では `insight_type='before_zero_knowhow'` を中心に受け、横断傾向が強い場合は `cross_project_pattern` として扱います。
...
- D-7 Textbook Insights candidate をこの章へ入れるべきか判断できる
