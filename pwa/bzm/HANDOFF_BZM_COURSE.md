...
- 一つの評価シナリオでは、権限を持つ単一主体が事前登録した目的で方針を一度だけ選ぶ。会社、BZSF、公的支援者が別々の方針を同時に選ぶのではなく、選ばれた同じ方針を三視点から再評価する。会社欄は会社全体の時価総額ではなく、会社が保有する対象PJの増分正味価値である。
...
BZM 2.1は、2.0凍結、理論仕様、パラメータ抽出台帳、最小算術エンジン、合成回帰テストまで完了した。
