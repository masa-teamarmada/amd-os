...
- **表示面は2つ。同じ `CockpitIpPortfolio` を共有し、知財タブの実装を面ごとに分岐させない**。
  ...
  - 外部ワークスペースアカウント (`principal: "workspace_account"`) も、当該PJを名指しする active な `project_access_memberships` を再確認できたときだけ同じ知財タブを読み取り専用で開ける。`GET /api/project-ip` は通常のAMD認証に加え、この再確認済みの共有ワークスペース権限だけを読む経路として受け付け、`POST/PATCH/DELETE` は引き続きadmin限定
