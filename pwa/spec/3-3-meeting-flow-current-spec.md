...
| 結果 | OS通知 | 意味 |
|---|---|---|
| `updated` | 作らない | 会議記録・予定カード・ノーションひも付けを新規保存または更新しただけ。まさの判断・操作が不要 |
| `review_required` | 通常通知 | 人の判断が必要になった |
| `blocked` | 緊急性の高い通知 | 必要な会議処理が止まった |
| 対象なし / 変更なし / 既存カード確認のみ | 作らない | 内部reportとmemoryだけを残す |

通知writerは `npm run notify:h1-report -- --outcome "<review_required|blocked>" --run-key "<run id>" --body-file <sanitized report>` を使う。結果区分なし・旧 `--title` 指定の送信はhelperが拒否する。通知本文の先頭には、H-1の役割と今回見るべきことを日本語で入れる。`--outcome updated` を渡した呼び出し（旧呼び出しや誤り含む）は、helper が `app_notifications` へ何も書かずに成功終了する実装ガードがあり、呼び出し側の完了処理を失敗扱いにしない。
...
### 日付が一致するだけのフォルダを取り込まない

PJフォルダ直下の `YYMMDD_件名` は、MTG資料置き場と成果物置き場の両方に使われている (AGENTS.common「成果物の保存先」)。**日付が会議日と一致することは、その会議の資料である根拠にならない。** 過去分をまとめて取り込むときは、route の出力をそのまま入れず、フォルダ名・ファイル名・保存日時・会議カードのタイトルを突き合わせて1件ずつ判定する。判定は入れる側に倒さない。

除外する典型 (2026-08-30 の全PJ棚卸しで実際に出たもの):

| 除外する | 例 |
|---|---|
| 会議と別の作業・事務ファイル | 履行完了報告書の作成作業、外来研究員登録の様式、相見積の提出、研究員登録の計画様式 |
| 会議資料ではない画像 | 懇親会・BBQの写真、ロゴ素材 |
| 同じ資料の古い版 | `_v1` 〜 `_v4`、`_old`、同フォルダに送付版・最終版があるもの |
| 同じ資料の別形式 | HTML/PDFが揃っているときの `.md` 元テキスト、PDFと同内容の `.zip`、実体のない `.gdoc` 参照 |
| 会議前の準備下書き | `_prep` フォルダ配下 (route も末尾 `_prep` は除外する。`_prep (1)` のような枝番は素通りするので目視で落とす) |

保存日時が会議日より後、または大きく前のファイルは、その会議の配布資料ではなく前後の作業ファイルであることが多い。迷ったら除外し、除外した件数と理由を残す。

### 書面決議 (会議を開かない取締役会決議)

会社法370条の決議の省略は、取締役会の正式な決議でありながら会議が存在しない。カレンダー予定も開催時刻もないが、記録としては開催済みMTGと同じ場所に置く。

| 列 | 値 |
|---|---|
| `meeting_id` | `written-resolution:<project_id>:<YYYYMMDD>` |
| `meeting_date` | 決議があったものとみなされた日 (提案書・同意書の日付)。**Driveのフォルダ名の日付ではない** |
| `meeting_start_at` | `null`。UIは開始時刻が無くても日付だけで表示する |
| `calendar_event_id` | `null` |
| `title` | `<PJ略号> 取締役会（書面決議）` |
| `source_kinds` | `drive` |
| `decided` | 提案事項ごとに1項目。書面決議は同意書が揃った時点で可決なので、資料のみでも `decided` に書いてよい |

フォルダ名の日付と書類の日付がずれている場合 (`260628_取締役会書面決議` の中身が6月29日付など) は、書類の日付を採り、ずれていることを `risks` に残す。この場合 adopt route の日付前方一致は当たらないので `drive_folder_id` を明示して呼ぶ。
...
- W-Prepが起動した時点で、対象会議・契約/PJ文脈・7ソースの読取り、`prep_draft_md` / readiness保存、当日Notion AI Meeting Notesへのappend-only context、opening prep brief表示は自動実行する。まさの追加承認は不要。権限が自動審査で却下された時だけ、同じ要求を連打せず理由を保存して `preparing` / `failed` とする。
...
- 読取り・正規の準備状態保存を継続するために、browser / Chrome / SSH / Vercelなどの別経路へ切替えて人の承認を求めない。
