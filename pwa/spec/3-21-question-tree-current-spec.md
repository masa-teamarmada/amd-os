...
正本ステータス: 確定（まさ 2026-09-10）。実装済み。仕様変更はこのファイルと同じcommitで反映する。
...
**やることも押せば詳細が開く。** 問いと同じモーダルで、種類・状態・担当・期限・着手予定・完了日・目標値・実測値・単位・見出し・方法条件・完了条件・完了証跡・詰まりをその場で直せる。末尾に「これが答えを出す論点」を出し、押すとその論点の詳細へ移る。
