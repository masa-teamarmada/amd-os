...
| 画面 | 内容 |
|---|---|
| 問いの木 | PJワークスペースの主画面。根から辿れる木を、状態・担当・期限・次の一手つきで表示。行を開くと背景・答え・ぶら下がるやること・分かったことを同じ場所で読み書きする |
| ガント | 左列が問いの木、バーがやること。工程レーンは持たない |
| PJの現在地 | 根の問いごとに、閉じた数と残りの数、枝が死んだ数 |

問いの木は既存の`論点・仮説`タブを置き換える。`目的構造`タブは廃止する。
