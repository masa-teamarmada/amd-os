...
| データ | tier | writer | 根拠 |
|---|---|---|---|
| D-1/D-3/D-4/D-5/D-6/D-7/D-8/D-10/D-11/D-13/D-14/D-15, M-1/M-2/M-3, W-1 | **L2** | LLM + deterministic extraction | 主に Codex automation と MMO 側 Codex が抽出する。D-10 は PWA route が evidence を集め、Codex automation が合成して POST 保存する。D-13/D-14 など一部は既存PWA APIを入口に使う。D-15は5生データを正規化し、PDF/Office/Google文書本文、意味分類、PJ帰属、hash集約を通す。 |
...
| **D-15** | 重要情報 | LSTを初例に、5生データ・PDF/Office/Google文書の共通reader、重要カテゴリ、PJ帰属、dedupe、provenance、outbox/applierを実装 | はい | 27 PJ dry-run済 / 定期schedule未登録 | Drive root未登録PJ、scan cursor、OCR owner、既存L2 coverage照合を段階導入 |
...
| **D-15** | L2 | 重要情報 | 決算、ガバナンス、契約、資金調達、補助金、技術、計画、事業進展、リスク、人事、期限を5生データから共通候補化。本文hashで重複排除 | 通知採否、採用後の重要情報正本、既存L2/BZMへの接続候補 | Codex collector + meaning extraction + deterministic validator + `amd-os-ms` non-LLM applier。定期scheduleは未登録 | `pwa/scripts/lib/important_evidence_extraction.mts` / `project_important_evidence` |
