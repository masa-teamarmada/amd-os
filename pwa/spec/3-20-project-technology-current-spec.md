...
- 構成: 説明帯 (形式別の件数 + adminの「＋トピック追加」) → **全体像** → (xl以上) 左に固定の**目次** ／ (xl未満) 上に固定の**現在地バー** → 技術区分ごとのトピック → まだ整理していない技術の断片。詳細は §5.1
...
### 5.1 全体像と目次 (2026-09-14)

まさの依頼 (2026-09-14):「技術タブのコンテンツが増えてきて、何がどこにあるのか分からなくなってきてるから、全体感も見えるようにしてほしいし、それぞれのコンテンツへのアクセスもしやすくしてほしい」。SXのトピックが20件・行が200を超えた時点で起きた。

| 部品 | 置き場所 | 中身 |
|---|---|---|
| 全体像 (`data-testid="tech-overview"`) | 説明帯の直下 | 区分ごとのカード。各トピックの形式・タイトル・行数・要確認件数。押すとその位置へ移動。上に「区分で絞る」チップ (見出し帯にあった select はここへ移した)。未整理の断片へのカードもここ |
| 目次 (`data-testid="tech-toc"`) | xl (1280px) 以上、左の列に固定 | 区分 → トピック (「 — 」より後ろの補足は落として短く出す)。いま読んでいるトピックを `aria-current="location"` で光らせる。「全体像へ」で先頭へ戻る |
| 現在地バー (`data-testid="tech-jump-bar"`) | xl 未満、本文の上に固定 (`sticky top-0`) | 「いま 区分 › トピック」と「目次」ボタン。目次は**本文に重ねて開く** (押し下げると、閉じた瞬間に着地位置がずれる) |

決めごと:

- **本文は畳まない。** 全件を出したまま、全体像と目次で行き来する (前提と明細を畳まない、とした 2026-08-23 の方針と同じ)。
- 各トピックと区分見出しは `data-tech-anchor` を持つ。いま読んでいる場所は、画面上端から120px を越えた最後の見出し。ページ全体のスクロールを capture で拾う。
- 移動は `scrollIntoView` + `scroll-margin-top` (xl 未満 80px = 現在地バーの下、xl 16px)。着地したカードは1.6秒だけ枠を光らせる。
- 上にある解説の mermaid 図は描画が遅れて高さが変わるので、移動の 0.9秒後と1.8秒後に着地位置を確かめ、24px 以上ずれていたら合わせ直す。
- 区分で絞っている最中に、ほかの区分のトピックを押したら、絞り込みを外して描画し終えてから移動する。**この待ちはアニメーションフレームではなくタイマーで取る** (非表示のタブではアニメーションフレームが止まり、移動が起きなかった)。
