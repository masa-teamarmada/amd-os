...
1. **デフォルト月割り**: N か月で完了する計画の MS は 1 か月あたり 100/N % の累積進捗がデフォルトで自動的に入る。計算正本は `pwa/src/lib/ms-schedule-shared.ts` の `expectedCumPctForYm`。
...
| デフォルト進捗ロジック | `progress-estimator.ts` の `applyScheduleDefaultsForProject(projectId, ym)` → `applyScheduleAutoProgress` |
...
2. 各 MS について `expectedCumPctForYm(ym, startYm, endYm)` を計算 (開始前=0 / 最終月以降=100 / 途中=経過月数比、 小数1桁)
3. `milestone_monthly_progress` の既存行が PM locked なら skip、それ以外 (無い/`routine_auto`/旧 source) は `source='routine_auto'` で upsert
...
| 乖離判定 | `abs(LLM推定累積 - デフォルト按分)` < **10pt** なら提案しない (デフォルトが有効のまま) |
...
| rule | detail |
|---|---|
| 参照する進捗 | **PM locked 行 (consumed_pt、null なら progress_pct×points/100) と、コード計算のスケジュール按分デフォルトのみ**。DB の非確定行 (`routine_auto` 含む) は参照しない |
| cumulative max | `payableCum(ms, ym) = max over m≤ym of effectiveCumAt(ms, m)`。巻き戻り→再上昇の差分二重払いを構造的に排除 |
...
D-2 の source 契約、デフォルト按分 cron、LLM 提案化の判定基準、revision confirm/discard の全経路、報酬計算との接続 (cumulative max / is_active guard)、DB出力、disabled fallback、cockpit反映を再構築できる。
