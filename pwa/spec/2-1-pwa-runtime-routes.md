...
| `/project/[projectId]/cockpit` | PJ cockpit。Status / AMD Score / XRL / MS / 資料 / 経営ハイライト / ガバナンス / 助成金 / 月次 / MTGサマリ。SX `p21`の進捗管理先頭は現行PJ workspaceと同じmanagement bundleを、現在地・AMD判断・次期限・4本柱へ投影する。adminは候補・柱・ボール保持者・audienceを確認してimmutable publicationを発行する。旧 `proactive_outbox` TODO は表示しない |
...
- `/api/cron/proactive-todo-extract` は先手 TODO の PWA non-LLM cron。開催済みMTG `next_actions[]` と、PJ `report_emails` から届く Gmail 期限つき依頼 (`email_action_request`) を `proactive_todos` に upsert する。予定MTG準備は Codex の W-Prep / prep worker が担うため、`next_meeting_prep` は新規生成せず、既存 open / blocked 行を dismissed へ履歴退避する。Gmail API は読むが、Anthropic / OpenAI / Gemini 等の従量課金LLMは呼ばない。メール本文全文・URL・パスワードは保存しない。
