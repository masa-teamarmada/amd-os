...
- `/api/cron/proactive-todo-extract` は先手TODOの非LLM lifecycle cron。新規候補は作らず、`due_basis='explicit'` の期限超過openをredへ上げ、3日経過したblockedをopenへ戻すだけ。新規TODOの唯一の意味抽出ownerは既存 Codex automation `amd-os-proactive-heartbeat` で、5系統の `source_cache` と開催済み会議要約からapproved `decision` / `masa_action` を直接生成する。
