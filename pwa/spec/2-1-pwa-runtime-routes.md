...
| `/bzm/map` | 理論マップ (論証台帳)。`pwa/bzm/theory-graph/*.md` の主張・概念・測定・決定・外部ソース・未解決問いを、定義・支持・異議・反証・依存・上書き・運用化・検証の8関係で結んだカバレッジ可視化。真理マップではなく、件数・接続数は真偽・確信度を表さない。詳細契約は `/spec/2-6-bzm-theory-map-current-spec` |
