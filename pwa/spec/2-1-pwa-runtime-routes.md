...
| `/bzm/map` | 理論マップ (論証台帳)。共有正本 `bzm_theory_nodes` / `bzm_theory_edges` の主張・概念・測定・決定・文献・未解決論点を9関係で結ぶ。admin は「理論を書く」と選択ノードの「根拠を足す / 異論を足す / 論点を残す」から追加・接続・編集でき、member は閲覧できる。Markdown 21ノード / 34関係は初期スナップショット兼DB障害時の読み取り専用 fallback。件数・接続数は真偽・確信度を表さない。詳細契約は `/spec/2-6-bzm-theory-map-current-spec` |
