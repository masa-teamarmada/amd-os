...
| D-14 Action Items | PWA route はあるが source sweep runner 不在。2026-06-16 に総会/取締役会候補 route も追加 | Gmail first、将来 Drive/Calendar/Slack/Notion | `action_items` / `project_shareholder_meetings` candidate / notifications | daily target | 旧 target が Claude | **route 前段の Codex collector を新設**。governance系の最優先候補。期日つき要対応は `/api/action-items/extract`、総会・取締役会・書面決議は `/api/governance/extract` | 要 | route POST only | governance / action-item review、health対象拡張候補 | P0 |
