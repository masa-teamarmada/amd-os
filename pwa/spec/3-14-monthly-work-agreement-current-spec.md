...
- 状態欄の直下に `確認して合意する2点` を置き、各PJを1つの判断単位として `(1) PJごとの担当内容` と `(2) その対価としての予定額` を横並びで対応づける。狭い画面では同じPJ内で担当内容→予定額の順に縦積みし、別PJの情報を間に挟まない。`milestones[].taskDescription`（無い場合はMS名）を担当内容として表示する。`milestones[].title` はMS名であり、月次の到達目標は現在の snapshot に無いため、目標として表示しない。予定額合計もこの確認領域に集約する。現在 snapshot に独立した発注条件値はなく、抽象的な `必須2点` だけの案内へ戻さない。未合意 / 条件更新ありでは、全PJの2点を読んだ後に `確認して合意` を置き、この操作まで当該稼働月の支払いに進めないことを明示する。
