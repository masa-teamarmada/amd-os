...
### SXの成果物pt試行（2026年10月稼働分から）

SX（p21）だけは、タスクpt対象MSを月割りで予定報酬に入れない。`project_actions` の期限・見積pt・担当割合から当月の成果物を並べ、未検収で期限が当月以前のものは当月へ持ち越す。検収済みの仕事は確定ptを別表示する。画面の「タスク分の見込み額」は `value_plan_cycles.budget_yen ÷ (シーズン月数×10pt) × 見込みpt` で、**支払上限を通す前の見込み**。確定支払額ではない。定常MSは従来の月割りを残す。

この試行の合意対象は「担当する成果物と、検収ptで報酬が変動する方式」。タスクの増減・検収pt・支払キャッシュの変化は terms hash に入れず、月中の完了を理由に再合意を要求しない。タスク一覧と見込みは snapshot には残す。月次支払の確定額は従来どおり報酬キャッシュと支払通知書で確認する。202609以前とSX以外のPJは従来契約のまま。

月初合意は **月初計画の表示・合意 snapshot・未合意管理レイヤー**。SXの成果物pt試行では、表示する見込み額と支払確定額を分ける。月初合意そのものは報酬計算の入力や支払確定額を変更しない。
...
- 配分表には `exclude_from_payout_notice=true` のメンバーも出す。隠すと残りの配分先が見えなくなり、合計と内訳が合わない表になって透明化の目的を失う。ただし**その人が支払対象外であることを配分表に書かない**。受け取る額は `¥0` とだけ出す (まさ確定 2026-08-29「明記する意味が全くないから書かないで。それをメンバーが見て何か意味があるとは思えない」)。本人の予定額欄に出す `monthly-agreement-payout-excluded-note` は本人向けの注意なので別で、こちらは残す。
...
2026年8月以前の稼働月 (`source_ym <= 202608`) は月初合意導入前/移行月のため、支払 gate 上は `agreed` 扱いで通す。実際の `member_monthly_work_agreements` 行を偽造せず、gate の理由を「導入前/移行月のため合意済み扱い」として保持する。**2026年9月以降の稼働月から通常どおり** `pending` / `stale` / `revision_requested` を blocker にする。

移行月をどこまでにするかは `MONTHLY_WORK_AGREEMENT_PAYOUT_GATE_START_YM` で持つ。延ばした経緯:

| 稼働月 | 延ばした理由 |
|---|---|
| 〜202606 | 契約改定前かつシステム未完成期間 |
| 202607 | 内部の状態が動くたびに再合意を求める実装だったため、合意が成立しないまま支払時期を迎えた (2026-08-28 修正)。まさ確定「月初合意のシステムが整いきってなかったので、特別に残り3人にも支払うことにした」 |
| 202608 | 合意の単位を member 単位から PJ 単位へ作り替えている最中のため。まさ確定 2026-08-28「今月はまだ合意がなくても支払ってあげようって話になったので、全員分発行できるように」 |

**毎月延ばさない。** 延ばすたびに、その月は「合意なしで払った月」になる。次に延ばすかどうかは、仕組みが変わっている最中かどうかで判断する。
...
- `projectId` を指定して開いた場合は、そのPJのブロックだけを表示し、上部に「{メンバー名} さんが {PJ名} で見ている画面です」と「この人の全PJを見る」を出す (`data-testid="monthly-agreement-project-focus"`)。表示内容は本人が見るものと同じにし、確認用の別画面を作らない。
