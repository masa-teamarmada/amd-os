...
- 合意状態の直下に `合意前に必ず確認すること` を置く。必須は `(1) 各PJで担当する内容と今月の到達目標` と `(2) その対価としての予定額` の2点だけ。モーダルでは予定額合計と全PJの予定額を、この必須枠の中に実数で集約する。現在 snapshot に独立した発注条件値は無く、必須カードの表示名は実データに合わせて `担当内容・到達目標` とする。未合意 / 条件更新ありでは、`確認して合意` を押すまで当該稼働月の支払いに進めないことを、警告本文とボタン直下で明示する。
- 強制表示モーダルでは、参加PJ数、支払済み実績(税込)、実績未照合(税込)、これから支払予定(税込)、今月末未払い残合計を `参考情報: 支払い状況と対象PJ` の初期折りたたみへまとめる。各PJの担当割合と今月のptは `参考情報: 予定額の根拠` にまとめる。支払い予定・未払残・予定額の根拠は、合意する発注条件そのものではなく、判断を助ける参考情報として扱う。支払済み実績は `reward_paid_at` ではなく、`monthly_reward_payout` の保存済み明細と `freee_wallet_txn_verified:` 証跡がそろった行だけを、税込額 (`round(total_pay × 1.1)`) で集計する。`reward_paid_at` はあるが実支払証跡とPJ別明細額が一致していない行は `実績未照合` へ分け、実績にも「これから支払予定」にも混ぜない。
- モーダルでは必須枠より下を、独立した説明カードにせず `参考情報` の短い区切りにする。PJごとの担当内容・到達目標の詳細、予定額の根拠、支払い予定・今月末未払い残・前月繰越/当月発生/支払の内訳をこの二次領域に置く。`修正要望` は `確認して合意` の横に置き、押したときだけ同じ状態カードの中に入力欄を開く。`stockYen` は「今月は支払われない」別枠で強調し、必須枠の合意用予定報酬と同じ見え方にしない。
