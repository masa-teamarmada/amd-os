...
- 合意状態の直下に `合意前に必ず確認すること` を置く。必須は `(1) 各PJの担当内容` と `(2) その対価としての予定額` の2点だけ。モーダルではPJごとに `担当内容` を一度だけ置き、`milestones[].taskDescription`（無い場合はMS名）をその右に複数並べる。`milestones[].title` はMS名であり、月次の到達目標は現在の snapshot に無いため、目標として表示しない。予定額合計と全PJの予定額も同じ必須枠に集約する。現在 snapshot に独立した発注条件値は無く、必須カードの表示名は実データに合わせて `担当内容` とする。未合意 / 条件更新ありでは、`確認して合意` を押すまで当該稼働月の支払いに進めないことを、警告本文とボタン直下で明示する。
