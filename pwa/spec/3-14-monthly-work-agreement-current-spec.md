...
- 合意状態の直下に `合意前に必ず確認すること` を置く。必須は `(1) 各PJで担当する内容と今月の到達目標` と `(2) その対価としての予定額` の2点だけ。モーダルでは予定額合計と全PJの予定額を、この必須枠の中に実数で集約する。未合意 / 条件更新ありでは、`確認して合意` を押すまで当該稼働月の支払いに進めないことを、警告本文とボタン直下で明示する。
...
- モーダルでは必須枠より下に `参考情報・修正依頼` 見出しを置き、PJごとの発注条件の詳細、予定額の根拠、支払い予定・今月末未払い残・前月繰越/当月発生/支払の内訳、修正依頼をこの二次領域へまとめる。`stockYen` は「今月は支払われない」別枠で強調し、必須枠の合意用予定報酬と同じ見え方にしない。
