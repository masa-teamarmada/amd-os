...
- 合意状態の直下に `合意前に必ず確認すること` を置く。必須は `(1) 各PJで担当する内容と今月の到達目標` と `(2) その対価としての予定額` の2点だけ。未合意 / 条件更新ありでは、`確認して合意` を押すまで当該稼働月の支払いに進めないことを、警告本文とボタン直下で明示する。
...
- PJごとに、必須確認として合意用予定報酬、担当MS、到達目標を表示する。支払い予定・今月末未払い残・前月繰越/当月発生/支払の内訳は、モーダルでは `参考情報: 支払い予定・未払残` の初期折りたたみへ置く。`stockYen` は「今月は支払われない」別枠で強調し、支払額や合意用予定報酬と同じ見え方にしない。
