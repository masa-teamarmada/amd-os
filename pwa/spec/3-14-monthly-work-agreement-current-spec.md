...
2026年7月以前の稼働月 (`source_ym <= 202607`) は月初合意導入前/移行月のため、支払 gate 上は `agreed` 扱いで通す。6月は契約改定前かつシステム未完成期間、7月は内部状態が動くたびに再合意を求める実装だった期間 (2026-08-28 に修正) で、どちらも合意が成立しないまま支払時期を迎えたため、合意条件として支払いを止めない。まさ確定 2026-08-28「月初合意のシステムが整いきってなかったので、特別に残り3人にも支払うことにした」。実際の `member_monthly_work_agreements` 行を偽造せず、gate の理由を「導入前/移行月のため合意済み扱い」として保持する。**2026年8月以降の稼働月から通常どおり** `pending` / `stale` / `revision_requested` を blocker にする。
