...
| SX business plan tab | `CockpitBusinessPlan`, `src/lib/sx-business-plan.ts` | `projectId === 'p21'` のみ表示。順序は (1) 4開発レーン×5フェーズの表、(2) `CapitalPlanWorkspace` の100%株主構成推移と資本政策表、(3) 年次試算表。フェーズ表の各セルは活動、予算、出口条件、関連XRLを持ち、列見出しはフェーズ総額、調達源、固定費バーン上限、5軸到達XRLを持つ。自社培養・包装・輸送、自社工場の段階投資を前提とし、Seedは2028年10月までのSeries A実行を資金ゲートとする。資本政策の作業台帳はSXに限り会社概要タブから事業計画タブへ移し、migration 196が既存プランを上書きせず初期原案を1件投入する |
