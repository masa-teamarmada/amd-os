...
`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。子が1件だけのグループは上段から内容へ直接移り、冗長な子タブ列を表示しない。通常PJは `進捗管理`（MS・月次 / 動向・会議 / 週次差分 / ガント / 関係先 / 論点・仮説）、`事業計画`（スコア詳細 / 技術 / 事業計画 / コスト試算 / 知財 / 資本政策）、`PJ管理`（PJ概要 / ドライブ / 会社概要）の3グループ。タスク階層はガントの固定左列へ統合し、独立した目的構造タブは持たない。PJ概要には活動・実績、助成金、獲得台帳、AMDの貢献を含める。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理`（タスク階層を含むガントを含む） / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `PJ管理`の4グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。

KUTEのシーズ一覧タブは、一覧の説明・注意書き・集計サマリを上部へ重ねず、操作列と比較表を直ちに表示する。シーズの状態や評価は各行・詳細面で確認する。

PCでは親グループへホバーまたはキーボードフォーカスすると、親の直下に子タブ一覧をフロート表示する。フロートまでポインタを移せるヒット領域を確保し、子タブは通常のTab順で操作する。タッチ端末ではホバーを模倣せず、選択中グループの子タブ列を常時表示する。コックピットでは `?tab=gantt` がタスク階層と時間軸を一体で表示する。共有済み旧URLの `?tab=objective-structure` はガントへ解決する。

2026-09-09以降、下表や履歴に残る「PJ管制5タブ」「`ganttDisplayMode`」「ガントと目的構造の切替」という記述は退役仕様である。現行は`週次差分 / ガント / 関係先 / 論点・仮説`の4タブで、目的構造の独立タブを表示しない。ガント左列の全ノードはタスクであり、右側に日程と依存線を置く。
...
| `?tab=weekly` / `?tab=gantt` / `?tab=partners` / `?tab=issues` | PJ管制の4タブを初期表示する。`gantt`はタスク階層と時間軸を一体表示する。旧`?tab=objective-structure`は`gantt`へ解決する (下記 project control tabs 行が正本) |
...
| project control tabs | `CockpitProjectControl`, `SxWeeklyControlDashboard` (embedded), `/api/project/[projectId]/workspace-bundle` | (2026-08-28 まさ「コックピットとワークスペースを統合できそうじゃない？」→「コックピット側を12タブにしよう」) `週次差分` / `ガント` / `目的構造` / `関係先` / `論点・仮説` の5タブ。PJワークスペース (`/project/[projectId]/workspace`) と**同じ `SxWeeklyControlDashboard` を埋め込みモード (`embedded` + `view`) で共有**し、二重実装を作らない。埋め込み時はコンポーネント自前のタイトル行・タブ列・ページ枠を出さず、表示タブは `CockpitView` のタブ列が決める。埋め込みの`ganttDisplayMode`は外側の子タブから固定し、内部のガント/目的構造切替は表示しない。画面内の導線 (関係先→ガント等) は `onViewChange` でコックピットのタブへ戻す。5タブは1つのマウントを共有するので、行き来しても束を読み直さない。束は可変系なのでキャッシュせず、baseline へ理由付きで登録済み。**認可**: この route は `getCurrentMemberAccess()` だけで認可する AMD メンバー専用面で、外部アカウントの認可解決を持ち込まない (`scripts/check_pwa_critical_ui.cjs` が禁止アンカーで固定)。外向けの共有ワークスペースは `(shared-workspace)` 配下の別ルートのまま残し、認可の境界をルートで持つ設計を変えない |
