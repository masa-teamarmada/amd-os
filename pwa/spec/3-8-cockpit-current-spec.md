...
| `?tab=score-detail` | SU 系 PJ の `スコア詳細` を初期表示し、BZM 2.2暫定主表示、選択・イベント時間軸、103項目の全パラメータ台帳を最上段、現行SPS / BZM 2.1、BZM 2.0、SPS 1.0 / Legacy AMDを最下部の初期閉じ・初回open時lazy mountアーカイブとして同じcockpit内で開く |
...
| score detail tab | `CockpitAmdScoreDetailTab`, `Bzm22ProvisionalObservatory`, `LazyArchiveDisclosure`, `Bzm21DynamicPolicyObservatory`, `Bzm2ModelObservatory`, `AmdScoreView embedded` | 正規URLは `/project/[projectId]/cockpit?tab=score-detail`。最上段の `data-testid="bzm22-provisional-primary"` はread-only `/api/project/[projectId]/bzm-2-2-pilot`から該当PJのcompact projectionを読み、`BZM 2.2 暫定主表示`、低位/中央/高位scenarioの$J/P/Q/S$、初回経路喪失、選択・イベントの5レーン時間軸、7群103項目を省略しない `全パラメータ台帳` を表示する。$P$は登録済み方針を固定した全gate通過条件であり最適方針値ではない。時間軸は根拠付き意思決定、固定shadow登録方針、将来判断点、事業・技術・資金イベント、外部イベントを分離し、gate月から意思決定を生成せず、条件付き資金を着金予定へ格上げしない。現在の意思決定履歴と将来判断点が未収載なら空状態を表示する。各パラメータは日本語名、LaTeXデータ名、理論記号または`—`、参照式、関係種別、説明、内部keyを値と一緒に示す。`unvalidated` / `shadow_only` / 前向き検証0件、ランキング・撤退/継続・投資・配分への自動利用禁止を常設し、DBとSPS primary registryは更新しない。最下部の `data-testid="score-model-archives"` には `現行SPS / BZM 2.1`、`BZM 2.0`、`SPS 1.0 / Legacy AMD` の三トグルを順に置く。すべて初期閉じで、native `details` にchildを直置きせず初回open時だけmountし、その後は再開閉で状態を保持する。BZM 2.2未登録・API取得不能でも旧モデルを失敗させず、BZM 2.2区画だけに状態を示す。desktopは共通横軸swimlaneと高密度表、390px mobileは同じレーン順とlabel/valueを縦再配置しページ全体を横スクロールさせない。旧 `/venture-map/amd-score/[projectId]` はこのタブへredirectする（`p99`デモを除く） |
