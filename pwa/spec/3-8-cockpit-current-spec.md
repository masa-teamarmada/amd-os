...
| killer factor catalog | `CockpitKillerFactorCatalog`、`/api/governance/killer-factors` | (2026-08-09 追加、migration 246) 会社概要タブの基本情報直下に置く発生駆動台帳。`killer_factor_catalog` は全PJ共通の要素マスタで、追加時triggerが全既存PJへ `project_killer_factor_states` の未発生行を補完し、新規PJ作成時も全有効要素を補完する。観測状態はPJ×要素で `not_occurred` / `occurred` の2値。`occurred` は `occurred_on` / 非空の `evidence_note` / `recorded_by_member_id` / `recorded_at` が全て必須で、DB CHECKでも不完全な発生行を拒否する。members登録済みAMDメンバーが `requireMember` gate経由で一覧・共通要素追加・PJ別発生記録を行える。初版は経営整合 / ガバナンス / 管理体制 / 金銭感覚 / 経営チーム / 社会的信用 / 出自機関の7型。今回のAPIは記録と表示だけを担い、通知、成功確率再計算、LLM呼び出しを行わない |
