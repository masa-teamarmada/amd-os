...
| venture status | `CockpitVentureStatus`, `Bzm22CockpitSummary` | 上部主スコアはread-only BZM 2.2 summary APIの$J/P/Q/S$評価日時点スナップショット。desktopは4指標を左の縦レール、XRLを右へ置く一体レイアウトにし、上下配置による空白を作らない。$J$は全分岐込み現在価値、$P$は登録済み現在方針を固定した全条件通過時の現在価値で、どちらも評価日時点へ割り引く。各指標は数式と肯定形の一文を併記する。従来SPSの折れ線とM/P/R/Sカードは初期閉じの「旧SPS履歴」トグルへ残し、XRLは常時表示する。`project_ventures`, `project_xrl_log`, related data |
