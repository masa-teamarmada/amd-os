...
## Current navigation contract (2026-09-09)

`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。子が1件だけのグループは上段から内容へ直接移り、冗長な子タブ列を表示しない。通常PJは `進捗管理`（ゴールツリー / タスク / ガント / MS・月次 / 動向・会議 / Slack / 週次差分 / 関係先。既定タブはゴールツリーで、グループの一番左と揃える。2026-09-13 まさ「進捗グループを使うときは最初に論点タブを開く」）、`事業計画`（スコア詳細 / 技術 / 事業計画 / コスト試算 / 知財 / 資本政策）、`PJ管理`（PJ概要 / ドライブ / 会社概要）の3グループ。目的構造はガントの固定左列へ統合する。PJ概要には活動・実績、助成金、獲得台帳、AMDの貢献を含める。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理`（目的構造を内包するガントを含む） / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `PJ管理`の4グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。
