...
| KUTE 規程・内規 | `CockpitKuteRegulations` | KUTE (`p25`) only。`/project/p25/cockpit?tab=regulations` の専用タブ。認定規程、認定委員会内規、支援細則、認定審査チェックシートを1行ずつ表示し、現状、次のTODO、AMD管理期限、最新版docx、全途中版docxへ到達できる。期限は大学の公式決裁日でなくAMD管理目標と明記する。文書実体はKUTE共有Driveの版管理フォルダへ集約し、画面にはメール本文・個人情報・secretを保存・表示しない。 |
