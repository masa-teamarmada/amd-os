...
**PoC比較レンズ（2026-07-30 現行）**: PoCは`roleLabel`接頭辞で判定する同一partner台帳上の横断属性であり、独立台帳やPoC固有段階は作らない。ただしPoC filter中は役割groupを比較主軸から外し、`SX_PARTNER_STAGE_ORDER`の共通7段階（候補→情報交換→条件整理→面談調整→検証準備→合意確認→実行中）で全PoC先を1本の一覧にする。`sxPartnerIsOnHold()`が`relationshipStage='on_hold'`と`deferredLowPriority`を一つのoverlay状態へ正規化し、rail・段階filter・件数・sortで共用する。既定は段階降順の「関係段階順」、切替可能な「要対応順」は停止→日精度期限超過→7日以内→保留→情報更新要→担当未確認→期限未設定→期限内で並べ、段階の浅さだけを「遅れ」と断定しない。情報更新要は`confidence='unknown'`または最終確認から14日超で導出する。当方ボールは要対応単一状態へ混ぜず、独立列と同rank内のtie-breakに使う。過去区間は完了証跡と誤認させる`✓`を使わず、7区間railの塗りと現在位置だけを表示する。接点件数、work item、次の一手、ゴールは段階分母へ混ぜない。PoC比較は専用compact rowを使い、社名/7区間/要対応/側＋担当者/期限/直近接点1行/次の一手1行/詳細だけを初期表示する。全履歴・保有事項・目標・編集は行下へ展開せずモーダルに表示する。段階filter後の要対応集計は表示中partner単位へ追随し、全PoC母数と表示件数を分ける。停止・期限超過・7日以内・情報更新要・当方ボール・判定材料不足の件数ピルはそのまま該当先の絞り込み操作として使え、関係段階順を崩さず要対応先名へ到達できる。並び/要対応帯とデスクトップ列見出しは長い一覧でもsticky表示する。モバイルは最終確認を社名行へ同居し、直近接点と次の一手を同じ段へ置く。「接点記録あり」は`interactions.length > 0 || lastContactDate != null`で導出し、roleLabelから接触済みを推測しない。行自身の会社名短縮は「会社名から/へ/に/と/が/の/は/を/より」「会社名（担当者）」等の限定文型だけを「先方」へ置き換え、保存値・詳細履歴・複合名・第三者名は変更しない。通常の関係先一覧も現在ボールの側だけでなく担当者名を表示する。
