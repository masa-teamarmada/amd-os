...
`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。通常PJは `進捗管理`（ゴールツリー / タスク / ガント / MS・月次 / 動向・会議 / Slack / 週次差分 / 関係先。既定タブはゴールツリーで、グループの一番左と揃える。2026-09-13 まさ「進捗グループを使うときは最初に論点タブを開く」）、`事業計画`（スコア詳細 / 技術 / 競合比較 / ビジネスモデル / 事業計画 / コスト試算 / コスト試算（燃料） / 知財 / 資本政策）、`ドライブ`、`PJ管理`（PJ概要）、`会社情報`（会社概要）の5グループ。`PJ管理`と`会社情報`は子が1件でも、それぞれ`PJ概要`と`会社概要`の子タブ列を常時出す。ドライブは独立タブのままとし、同名の子タブを重ねない。目的構造はガントの固定左列へ統合する。PJ概要には活動・実績、助成金、獲得台帳、AMDの貢献を含める。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理`（目的構造を内包するガントを含む） / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `ドライブ` / `PJ管理`（PJ概要） / `会社情報`（会社概要）の6グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。
...
| navigation | `CockpitView`, `src/lib/cockpit-tabs.ts` | 通常PJは `進捗管理 / 事業計画 / PJ管理` の3グループ、研究機関PJは `進捗管理 / シーズリスト / 規程・内規 / PJ管理` の4グループ。上段はmobileで通常3列、研究機関2列×2行、640px以上で4列。PCの親グループはhover/focus時に子一覧を直下へフロートし、タッチではホバーを模倣せず子列を常時表示する。下段は選択中グループの子が複数ある場合だけ表示し、狭幅では子タブだけを横スクロールする。所属とURL正規化は `cockpit-tabs.ts` を正本にする |
