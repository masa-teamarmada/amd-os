...
| `?tab=score-detail` | SU 系 PJ の `スコア詳細` を初期表示し、BZM 2.2のJ/P/Q/S、実数代入、gate別の通過・停止分岐、ブラウザ内の方針比較、選択・イベント時間軸、計算入力、経営判断、初期閉じ103項目監査台帳を最上段、現行SPS / BZM 2.1、BZM 2.0、SPS 1.0 / Legacy AMDを最下部の初期閉じ・初回open時lazy mountアーカイブとして同じcockpit内で開く |
