...
## Current navigation contract (2026-09-01)

`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。子が1件だけのグループは上段から内容へ直接移り、冗長な子タブ列を表示しない。通常PJは `進捗管理`（MS・月次 / 動向・会議 / 週次差分 / ガント / 目的構造 / 関係先 / 論点・仮説）、`事業計画`（スコア詳細 / 技術 / 事業計画 / コスト試算 / 知財 / 資本政策）、`PJ管理`（PJ概要 / ドライブ / 会社概要）の3グループ。PJ概要には活動・実績、助成金、獲得台帳、AMDの貢献を含める。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理`（ガント / 目的構造を含む） / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `PJ管理`の4グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。

KUTEのシーズ一覧タブは、一覧の説明・注意書き・集計サマリを上部へ重ねず、操作列と比較表を直ちに表示する。シーズの状態や評価は各行・詳細面で確認する。

PCでは親グループへホバーまたはキーボードフォーカスすると、親の直下に子タブ一覧をフロート表示する。フロートまでポインタを移せるヒット領域を確保し、子タブは通常のTab順で操作する。タッチ端末ではホバーを模倣せず、選択中グループの子タブ列を常時表示する。ガントと目的構造は同じ `SxWeeklyControlDashboard` の束を共有するが、コックピットでは `?tab=gantt` がガント、`?tab=objective-structure` が目的構造だけを表示し、内部の切替ボタンは出さない。
...
| `?tab=weekly` / `?tab=gantt` / `?tab=objective-structure` / `?tab=partners` / `?tab=issues` | PJ管制の5タブを初期表示する。`gantt`はガント、`objective-structure`は目的構造だけを表示し、中身はPJワークスペースと同一 (下記 project control tabs 行が正本) |
