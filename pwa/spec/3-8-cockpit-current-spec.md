...
| tabs | `CockpitView` | `進捗管理` / `スコア詳細` (該当PJのみ) / `事業計画` (SXのみ) / `会社概要` (常設) の display state。SX (`p21`) は4タブ、他SU系は3タブ、非SU系は2タブで、各タブのクリック領域はタブ数で均等分割する |
...
| SX business plan tab | `CockpitBusinessPlan`, `src/lib/sx-business-plan.ts` | `projectId === 'p21'` のみ表示。順序は (1) 4開発レーン×5フェーズの表、(2) `CapitalPlanWorkspace` の100%株主構成推移と資本政策表、(3) 年次試算表。年次試算の直下には初期閉じの `data-testid="sx-annual-parameters"` があり、年次の売上・原価・助成金・人員・役員/社員別の報酬・旅費・消耗品費・その他設備投資・IPO以外の資金調達、4段階の自社工場の建設年度/投資額、IPO年度/公募額を変えると、PL・設備投資・株式調達・期末現預金を再導出する。これはページ内の未保存シナリオで、保存済みの資本政策・会社情報を更新せず再読み込みで初期値に戻る。フェーズ表の各セルは活動、予算、出口条件、関連XRLを持ち、列見出しはフェーズ総額、調達源、固定費バーン上限、5軸到達XRLを持つ。自社培養・包装・輸送、自社工場の段階投資を前提とし、Seedは2028年10月までのSeries A実行を資金ゲートとする。資本政策の作業台帳はSXに限り会社概要タブから事業計画タブへ移し、migration 196が既存プランを上書きせず初期原案を1件投入する |
...
- **PoC候補先リスト**（`data-testid="sx-poc-board"`、節ID `management-poc`）: 排液提供先の確保状況を、調達済 → 合意済・排液取得前 → 相談中 → 未接触の候補 の順で並べる。件数は登録データだけを数え、母集団や必要社数を画面へ書かない（台帳に無い数値のため）。役割ラベルの接頭辞（`PoC候補先` / `PoC接触先`）で判定し、関係段階では判定しない（EWIR候補機関など別の候補を巻き込むため）。
