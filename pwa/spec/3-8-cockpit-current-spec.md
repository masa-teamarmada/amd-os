...
| tabs | `CockpitView` | `進捗管理` / `スコア詳細` (該当PJのみ) / `事業計画` (常設) / `規程・内規` (KUTE `p25`のみ) / `知財` / `資料室` / `会社概要` (常設) の display state。`事業計画` は 2026-08-21 (v3.87.3) に全PJ常設へ拡大した。各タブのクリック領域はタブ数 (`tabs.length`) で均等分割するので、タブ追加時に grid の列数計算を触る必要はない |
