...
`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。通常PJは `進捗管理`（ゴールツリー / タスク / ガント / MS・月次 / 動向・会議 / Slack / 週次差分 / 関係先。既定タブはゴールツリーで、グループの一番左と揃える。2026-09-13 まさ「進捗グループを使うときは最初に論点タブを開く」）、`事業計画`（スコア詳細 / 技術 / 競合比較 / ビジネスモデル / 事業計画 / コスト試算 / コスト試算（燃料） / 知財 / 資本政策）、`ドライブ`、`PJ管理`（PJ概要 / 契約・収支）、`会社情報`（会社概要 / 活動実績）の5グループ。`PJ概要`はPJの見出し・レーン・担当・事業概要だけ、`契約・収支`はシーズン予算と契約上の実行条件だけを読む。助成金・獲得台帳・AMDの活動履歴は`会社情報`の`活動実績`へ置く。ドライブは独立タブのままとし、同名の子タブを重ねない。目的構造はガントの固定左列へ統合する。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理`（目的構造を内包するガントを含む） / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `ドライブ` / `PJ管理`（PJ概要 / 契約・収支） / `会社情報`（会社概要 / 活動実績）の6グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。
...
| `?tab=project-contracts` | 常設「契約・収支」タブを初期表示する。シーズン予算と現行契約の実行条件を読む |
...
| navigation | `CockpitView`, `src/lib/cockpit-tabs.ts` | 通常PJは `進捗管理 / 事業計画 / ドライブ / PJ管理 / 会社情報` の5グループ、研究機関PJは `進捗管理 / シーズリスト / 規程・内規 / ドライブ / PJ管理 / 会社情報` の6グループ。`PJ管理`は`PJ概要 / 契約・収支`、`会社情報`は`会社概要 / 活動実績`を持つ。PCの親グループはhover/focus時に子一覧を直下へフロートし、タッチではホバーを模倣せず子列を常時表示する。下段は選択中グループの子が複数ある場合だけ表示し、狭幅では子タブだけを横スクロールする。所属とURL正規化は `cockpit-tabs.ts` を正本にする |
...
| project contracts tab | `CockpitProjectOverview` | 常設「契約・収支」タブ (`?tab=project-contracts`)。`CockpitSeasonBudget` と現行契約の実行条件を読む。契約は現行契約ごとに `契約期間` / `請求・振込` / `業務・成果物` / `経費申請` / 必要時だけ `推進条件` の最大5項目を出す (NDAは `契約期間` / `利用目的` / `運用条件`)。短文の正本は `projects.contract_terms_json.currentContracts[].terms.cockpitSummary`。読むだけで編集はせず、知財・秘密保持・解除・責任・準拠法などの詳細条項は admin 契約台帳が正本 (`pwa/spec/5-6-contracts-management-current-spec.md`)。 |
