...
`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。子が1件だけのグループは上段から内容へ直接移り、冗長な子タブ列を表示しない。通常PJは `進捗管理`（MS・月次 / 動向・会議 / 週次差分 / ガント / 関係先 / 論点・仮説）、`事業計画`（スコア詳細 / 技術 / 事業計画 / コスト試算 / 知財 / 資本政策）、`PJ管理`（PJ概要 / ドライブ / 会社概要）の3グループ。目的構造はガントの固定左列へ統合する。PJ概要には活動・実績、助成金、獲得台帳、AMDの貢献を含める。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。
