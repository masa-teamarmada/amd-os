...
- ZMPはコックピットを開くと「タスク」を既定表示する。未完了を一列で見せず、`いま動かす`（期限超過・停止・緊急）→`7日以内`→`割り当てる`（担当または期限が未設定）の3区分を最初に出す。`その先`と全件は利用者が明示して開く。
- 各行は担当・期限に加え、議事録 / つくよみ / 旧管理表 / 画面入力の発生元と最終確認日を短く表示する。画面の基準日はbundleの`asOf`を使う。
- 手動並べ替えは全件表示でだけ有効。優先区分は`urgent / isOverdue / blocked`、7日以内、未設定、その他の順で排他的に導出し、同じタスクを複数区分へ重複表示しない。
...
`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。通常PJは `進捗管理`（ゴールツリー / タスク / ガント / MS・月次 / 動向・会議 / Slack / 週次差分 / 関係先。既定タブはゴールツリーで、グループの一番左と揃える。2026-09-13 まさ「進捗グループを使うときは最初に論点タブを開く」）、`事業計画`（スコア詳細 / 技術 / 競合比較 / ビジネスモデル / 事業計画 / コスト試算 / コスト試算（燃料） / 知財）、`ドライブ`、`PJ管理`（PJ概要 / 契約 / 収支）、`会社情報`（会社概要 / 資本政策表 / 活動実績）の5グループ。`PJ概要`はPJの見出し・レーン・担当・事業概要、`契約`は契約上の実行条件、`収支`はシーズン予算と消化を読む。事業計画は将来の資本政策プラン、会社情報の資本政策表は過去ラウンドの確定事実を扱う。助成金・獲得台帳・AMDの活動履歴は`会社情報`の`活動実績`へ置く。ドライブは独立タブのままとし、同名の子タブを重ねない。目的構造はガントの固定左列へ統合する。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。
...
| business plan tab | `CockpitBusinessPlan`, `CapitalPlanWorkspace`, `src/lib/sx-business-plan.ts`, `src/lib/sx-business-plan-xlsx.ts` | 全PJ常設。`CapitalPlanWorkspace` は、事業計画に連動してこれからの資本政策を更新する作業面である。過去ラウンドの確定事実は会社情報の資本政策表へ記録し、ここへ混ぜない。SX (`p21`) だけは4開発レーン×5フェーズの表と年次試算表を足す。BZM 2.2 pilot 対象PJでは、フェーズ表の下にイベント・月次試算表・年度別の事業・資金推移を置く。暫定試算が無いPJでも、`CockpitPlMonthlySection` に登録済みの月次試算表を表示する。 |
...
| capital policy tab (資本政策表) | `CockpitCapitalPolicy` / `CapitalPolicyTable` / `CapitalRoundsTable` (`pwa/src/components/cockpit/`)、`buildCapitalPolicyTable()` (`src/lib/company-overview.ts`) | **会社情報グループの常設タブ** `/project/[projectId]/cockpit?tab=capital-policy`。過去ラウンドの確定事実を、列 = confirmed の `project_equity_transactions`、行 = 株主で記録する。事業計画に連動して更新する将来の資本政策は、事業計画タブの `CapitalPlanWorkspace` が正本である。株式イベント / ラウンド / 転換前証券の追加はこのタブで行う。 |
