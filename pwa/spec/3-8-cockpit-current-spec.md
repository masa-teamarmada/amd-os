...
## ZMP実行面（p19、2026-09-17）

- ZMPはコックピットを開くと「タスク」を既定表示する。未完了を一列で見せず、`いま動かす`（期限超過・停止・緊急）→`7日以内`→`割り当てる`（担当または期限が未設定）→`保留`（未完了の前提タスクあり）の4区分を最初に出す。`その先`と全件は利用者が明示して開く。
- 各行は担当・期限に加え、議事録 / つくよみ / 旧管理表 / 画面入力の発生元と最終確認日を短く表示する。画面の基準日はbundleの`asOf`を使う。
- 手動並べ替えは全件表示でだけ有効。優先区分は、未完了の前提タスクがある場合を`保留`へ先に分離し、それ以外を`urgent / isOverdue / blocked`、7日以内、未設定、その他の順で排他的に導出する。同じタスクを複数区分へ重複表示しない。保留カードは判断待ちの印と、完了を待つ前提タスク名を表示する。
- 担当の有無は`project_action_owners`だけでなく、画面に担当として表示する`owner_label`も読む。外部関係者はメンバー台帳へ存在しないため、`owner_label`に実名・役割名があれば担当済みとし、`担当未確認`、`担当確定待ち`など欠測語しかない場合だけ未設定とする。期限が空の場合は担当があっても`割り当てる`へ置く。
- 水素RTは提案段階であり、`KRはRTを主催できるのか？`を先行論点、`KRのRT実施可否を確認する`を判断タスクとする。KR主催・別主体・保留・見送りのいずれかが決まるまで、参加者設計、対価、MOU、外部打診、資料作成、開催準備は同タスクへの依存関係で`保留`に置く。
- p19の事業計画グループは、現行根拠の最新化が終わるまで通常ナビゲーションから隠す。データと既存URLは削除せず、再整備・受入後に再表示する。
...
## Current navigation contract (2026-09-17)

`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。通常PJは `進捗管理`（ゴールツリー / タスク / ガント / MS・月次 / 動向・会議 / Slack / 週次差分 / 関係先。既定タブはゴールツリーで、グループの一番左と揃える。2026-09-13 まさ「進捗グループを使うときは最初に論点タブを開く」）、`事業計画`（スコア詳細 / 技術 / 競合比較 / ビジネスモデル / 事業計画 / 試算表 / 資本政策表 / コスト試算 / コスト試算（燃料） / 知財）、`ドライブ`、`PJ管理`（PJ概要 / 契約 / 収支）、`会社情報`（会社概要 / 資金調達履歴 / 活動実績）の5グループ。`PJ概要`はPJの見出し・レーン・担当・事業概要、`契約`は契約上の実行条件、`収支`はシーズン予算と消化を読む。事業計画はフェーズの計画、試算表はP/L・資金推移、資本政策表は将来の資本政策、会社情報の資金調達履歴は過去ラウンドの確定事実を扱う。助成金・獲得台帳・AMDの活動履歴は`会社情報`の`活動実績`へ置く。ドライブは独立タブのままとし、同名の子タブを重ねない。目的構造はガントの固定左列へ統合する。デスクトップの寸法契約は親グループ36px、常設子タブ32px、ホバーフロート項目28pxを基準とし、スマホでは操作性を優先して44px以上を維持する。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理`（目的構造を内包するガントを含む） / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `ドライブ` / `PJ管理`（PJ概要 / 契約 / 収支） / `会社情報`（会社概要 / 資本政策表 / 活動実績）の6グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。

KUTEのシーズ一覧タブは、一覧の説明・注意書き・集計サマリを上部へ重ねず、操作列と比較表を直ちに表示する。シーズの状態や評価は各行・詳細面で確認する。

PCでは親グループへホバーまたはキーボードフォーカスすると、親の直下に子タブ一覧をフロート表示する。フロートまでポインタを移せるヒット領域を確保し、子タブは通常のTab順で操作する。タッチ端末ではホバーを模倣せず、選択中グループの子タブ列を常時表示する。コックピットの`?tab=gantt`は時間軸と目的構造を同じ行で表示する。共有済みの旧`?tab=objective-structure`は互換のため受け付けるが、`resolveCockpitTab()`でガントへ解決し、独立タブや内部切替は出さない。

2026-09-09以降、下表や履歴に残る「PJ管制5タブ」「`ganttDisplayMode`」「ガントと目的構造の切替」という記述は退役仕様である。現行は`週次差分 / ガント / 関係先 / 論点・仮説`の4タブで、目的構造の独立タブを表示しない。ZMPでは目的・成立条件・タスク、再帰タスク型PJでは親子タスクを、同じガント左列と時間軸で読む。

既存のフラットな `?tab=` は同じ子タブへ復元し、`capital-policy` は資本政策へ復元する。研究機関PJで事業計画系、通常PJで研究機関専用の値を指定した場合は `progress` へ正規化する。旧ZMPテーマ画面はコックピットにもPJワークスペースにも表示せず、ワークスペースの旧`#theme-progress`はゴールツリーへ戻す。
...
| `?tab=overview` | 常設「PJ概要」タブを初期表示する。PJ の見出し・担当・事業概要・XRL進捗 (`CockpitVentureStatus`) を読む |
...
| `?tab=activity` | 常設「活動実績」タブを初期表示する。助成金、獲得台帳、AMDの貢献を読む |
...
| 研究機関 規程・内規 | `InstitutionRegulationsPanel` | `institution_projects` にひもづくPJの `institution_id` を解決して共通規程台帳を表示する。PJ番号やKUTE固有データは使わない。 |
...
| navigation | `CockpitView`, `src/lib/cockpit-tabs.ts` | 通常PJは `進捗管理 / 事業計画 / ドライブ / PJ管理 / 会社情報` の5グループ、研究機関PJは `進捗管理 / シーズリスト / 規程・内規 / ドライブ / PJ管理 / 会社情報` の6グループ。`PJ管理`は`PJ概要 / 契約 / 収支`、`会社情報`は`会社概要 / 資本政策表 / 活動実績`を持つ。事業計画は将来の資本政策プラン、会社情報の資本政策表は過去ラウンドの確定事実を扱う。PCの親グループはhover/focus時に子一覧を直下へフロートし、タッチではホバーを模倣せず子列を常時表示する。下段は選択中グループの子が複数ある場合だけ表示し、狭幅では子タブだけを横スクロールする。所属とURL正規化は `cockpit-tabs.ts` を正本にする |
...
| financial projection tab (試算表) | `CockpitFinancialProjection`, `Bzm22TimeLedgerSection`, `CockpitPlMonthlySection`, `AnnualProjectionTable` | 全PJ常設。月次P/L・BZM 2.2 のイベントと時間軸・SXの年次事業/資金推移を読む。 |
...
| project control tabs | `CockpitProjectControl`, `SxWeeklyControlDashboard` (embedded), `/api/project/[projectId]/workspace-bundle` | (2026-08-28 まさ「コックピットとワークスペースを統合できそうじゃない？」→「コックピット側を12タブにしよう」) `週次差分` / `ガント` / `関係先` / `論点・仮説` の4タブ。PJワークスペース (`/project/[projectId]/workspace`) と**同じ `SxWeeklyControlDashboard` を埋め込みモード (`embedded` + `view`) で共有**し、二重実装を作らない。埋め込み時はコンポーネント自前のタイトル行・タブ列・ページ枠を出さず、表示タブは `CockpitView` のタブ列が決める。ガントは固定左列へ目的→成立条件→タスクを持ち、独立した表示モードは作らない。画面内の導線 (関係先→ガント等) は `onViewChange` でコックピットのタブへ戻す。4タブは1つのマウントを共有するので、行き来しても束を読み直さない。束は可変系なのでキャッシュせず、baseline へ理由付きで登録済み。**認可**: この route は `getCurrentMemberAccess()` だけで認可する AMD メンバー専用面で、外部アカウントの認可解決を持ち込まない (`scripts/check_pwa_critical_ui.cjs` が禁止アンカーで固定)。外向けの共有ワークスペースは `(shared-workspace)` 配下の別ルートのまま残し、認可の境界をルートで持つ設計を変えない |
| project overview tab | `CockpitVentureStatus`, `CockpitManagementScoreHero` | 常設「PJ概要」タブ (`?tab=overview`)。PJの姿を読む場所で、`CockpitVentureStatus`（p00 は `CockpitManagementScoreHero`、ecosystem PJ は AMD Score 対象外なので出さない）だけを開いた時にマウントする。最上段の `CockpitHeader` に残すのは PJ名・状態・分類・PJメンバー・共有ワークスペース導線だけ |
| project contracts tab | `CockpitProjectOverview` | 常設「契約」タブ (`?tab=project-contracts`)。現行契約の実行条件を読む。契約は現行契約ごとに `契約期間` / `請求・振込` / `業務・成果物` / `経費申請` / 必要時だけ `推進条件` の最大5項目を出す (NDAは `契約期間` / `利用目的` / `運用条件`)。短文の正本は `projects.contract_terms_json.currentContracts[].terms.cockpitSummary`。読むだけで編集はせず、知財・秘密保持・解除・責任・準拠法などの詳細条項は admin 契約台帳が正本 (`pwa/spec/5-6-contracts-management-current-spec.md`)。 |
| project finance tab | `CockpitSeasonBudget` | 常設「収支」タブ (`?tab=project-finance`)。シーズン予算と消化を読む。 |
| activity tab | `CockpitGrants`, `Bzm22AcquisitionLedger`, `CockpitAmdContributions` | 常設「活動実績」タブ (`?tab=activity`)。会社情報グループの子タブとして、助成金、獲得台帳、AMDの貢献を読む。 |
...
- SX（p21）の`Bzm22TimeLedger`（事業計画タブ）は、採用した月次試算表 v3.2 の2027年4月〜2037年3月を表示する。P/Lは`project_pl_monthly`、資金繰りは`project_monthly_cashflow`を読む。営業C/F・投資C/F・株式調達・助成金入金には専用のnullable列を使用し、月初・月末資金も原表の値を表示する。未登録月は「—」、融資未計画は「未計画」とし、営業利益や旧版固定値で補完しない。今回の採用試算は初回調達1.5億円、STS入金なし。既存の採択証拠・資本政策登録は変更しない。月軸は保存データの最終月まで延長するが、BZM経済CFの計算地平は変更せず地平外は「—」とする。表外に`単位：百万円`を一度だけ置き、月・金額は右寄せ、等幅数字、14px以上で表示する。
...
- 暫定試算が無いPJ（SE `p10` / ZMP `p19` / RTM `p32` / SMILE `p33` 等）の`CockpitPlMonthlySection`は、`project_pl_monthly`の登録行だけを4月始まりの年度で束ねて表示する。年度の出所区分はその年度の月の多数決で決め、月ごとの区分と食い違う場合も月側のバッジを優先して読ませる。値ゼロは「—」で、未登録月の行を生成して0で埋めない。月列は横スクロールで、項目列を左に固定する。
...
`track` は表示用の固定分類ではなく、`project_management_tracks` にPJごとに登録した正本だけを使う。画面で互換表示している分類と保存時の許可値を分けない。p20（CX）は migration 358 で従来の4分類（事業開発 / 技術開発 / 資金調達 / 体制構築）を登録した。追加のみで、既存の分類・業務記録は上書きしない。
...
- **PoC候補先リスト**（`data-testid="sx-poc-list"`、節ID `management-poc`。2026-07-30追補6でカードグリッドから罫線区切りの表/リストへ変更、testidも`sx-poc-board`→`sx-poc-list`）: 排液提供先の確保状況を、調達済 → 合意済・排液取得前 → 相談中 → 未接触の候補 の順で並べる。件数は登録データだけを数え、母集団や必要社数を画面へ書かない（台帳に無い数値のため）。役割ラベルの接頭辞（`PoC候補先` / `PoC接触先`）で判定し、関係段階では判定しない（SIER候補機関など別の候補を巻き込むため）。
...
PJ管理に「月次報告書」（`?tab=monthly-reports`）を常設する。通常PJ・研究機関PJとも対象月、社内版・提出版を切り替えて、既存の帳票画面をタブ内に表示する。「別画面で開く・印刷」から編集・印刷画面を開ける。当月も選択でき、未保存の版は「未生成」と表示する。ドライブの月次帳票入口は維持する。PWAのみの追加で、iOS・macOS・Androidは未移植。
