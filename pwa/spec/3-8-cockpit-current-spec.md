...
| `?tab=score-detail` | SU 系 PJ の `スコア詳細` を初期表示し、BZM 2.0観測画面と、現行SPS / R_net / FRL / XRL evidence / XRLチェックリストを同じcockpit内で開く |
...
| score detail tab | `CockpitAmdScoreDetailTab`, `Bzm2ModelObservatory`, `AmdScoreView embedded` | 正規URLは `/project/[projectId]/cockpit?tab=score-detail`。`/api/project/[projectId]/amd-score-detail` の`bzm2`から、BZM 2.0の式、現在値、欠測、共通状態の影響先、初期折りたたみのq版推移、出所履歴を先に表示する。トップ式の直下ではq、P、BZM 2.0 SPSを代入列として表示し、Pが未着手・欠測または尺度・合成規則が未登録ならSPSは0でなく未測定理由を示す。到達競争式の直下では共通状態、二つの時計、計画期限、qを同じ判定表へ置く。下付きのある記号はLaTeXで組み、パラメータ台帳は記号、現在値、測定・出所、反映先、履歴を横比較できる圧縮表とする。SX/LST以外で現行SPSを持つPJも`観測収集中`として、DBから凍結したPJ文脈、政策支援、現行XRL、証拠被覆、資金履歴を表示するが、未接続値をqへ変換しない。その下へ現行SPS Primary / SPS history / R_net / FRL / XRL evidence / XRLチェックリストを別区画で表示し、legacy AMD / M-X-Fはcomparisonとevidence用に残す。BZM 2.0台帳が未適用または取得不能でも現行SPSを失敗させず、BZM 2.0側だけ欠測表示にする。cockpit mount時にhidden panelとして先読みし、client memory cache 5分TTL + private HTTP cacheで再表示待ちを減らす。TTL超過後にタブがactiveになったら、表示済み内容を保ったまま背景再取得する。旧`/venture-map/amd-score/[projectId]`はこのタブへredirectする（`p99`デモを除く） |
