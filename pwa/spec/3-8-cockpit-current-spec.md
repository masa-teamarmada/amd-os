...
既存のフラットな `?tab=` は同じ子タブへ復元し、`capital-policy` は資本政策へ復元する。研究機関PJで事業計画系、通常PJで研究機関専用の値を指定した場合は `progress` へ正規化する。ZMPのテーマはPJワークスペースの機能として残し、コックピットのタブや `?tab=themes` の導線は持たない。
...
ZMP（p19）のテーマ作業面はPJワークスペースに置く。コックピットにはテーマタブやテーマ用の`?tab=`を追加せず、`ProjectThemeRoutes`と`project_management_*`の正本はワークスペース側だけから読む。コックピットへのアクセス権は拡大しない（2026-09-01、詳細は3-16）。
