...
| `?tab=score-detail` | SU 系 PJ の `スコア詳細` を初期表示し、BZM 2.2のJ/P/Q/S、ブラウザ内の方針比較、選択・イベント時間軸、103項目の全パラメータ台帳を最上段、現行SPS / BZM 2.1、BZM 2.0、SPS 1.0 / Legacy AMDを最下部の初期閉じ・初回open時lazy mountアーカイブとして同じcockpit内で開く |
...
| score detail tab | `CockpitAmdScoreDetailTab`, `Bzm22ProvisionalObservatory`, `LazyArchiveDisclosure`, `Bzm21DynamicPolicyObservatory`, `Bzm2ModelObservatory`, `AmdScoreView embedded` | 正規URLは `/project/[projectId]/cockpit?tab=score-detail`。最上段の `data-testid="bzm22-provisional-primary"` はread-only `/api/project/[projectId]/bzm-2-2-pilot`から該当PJのcompact projectionを読み、`BZM 2.2`、慎重/基準/強気scenarioの$J/P/Q/S$、ブラウザ内だけの方針比較、選択・イベントの5レーン時間軸、7群103項目を省略しない `全パラメータ台帳` を表示する。方針比較は計算bundleが揃う候補だけを丸ごと切り替え、元の進め方との差を表示し、DBへ保存しない。時間軸は各イベントへ試算上の進め方または「方針選択ではない」を表示する。各パラメータは日本語名、理論記号または「数式には直接入らない」、参照式、説明、登録値、3つの前提ケース、値の決め方、開ける根拠資料を示す。内部keyと英語の機械状態は閉じた監査情報に隔離する。最下部の `data-testid="score-model-archives"` には `現行SPS / BZM 2.1`、`BZM 2.0`、`SPS 1.0 / Legacy AMD` の三トグルを順に置く。desktopは共通横軸swimlaneと高密度表、390px mobileは同じレーン順とlabel/valueを縦再配置しページ全体を横スクロールさせない。旧 `/venture-map/amd-score/[projectId]` はこのタブへredirectする（`p99`デモを除く） |
