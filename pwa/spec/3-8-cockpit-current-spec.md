...
| score detail tab | `CockpitAmdScoreDetailTab`, `Bzm2ModelObservatory`, `AmdScoreView embedded` | 正規URLは `/project/[projectId]/cockpit?tab=score-detail`。`/api/project/[projectId]/amd-score-detail` の`bzm2`から、BZM 2.0の式、現在値、欠測、共通状態の影響先、q版推移、出所履歴を先に表示する。SX/LST以外で現行SPSを持つPJも`観測収集中`として、DBから凍結したPJ文脈、政策支援、現行XRL、証拠被覆、資金履歴を表示するが、未接続値をqへ変換しない。その下へ現行SPS Primary / SPS history / R_net / FRL / XRL evidence / XRLチェックリストを別区画で表示し、legacy AMD / M-X-Fはcomparisonとevidence用に残す。BZM 2.0台帳が未適用または取得不能でも現行SPSを失敗させず、BZM 2.0側だけ欠測表示にする。cockpit mount時にhidden panelとして先読みし、client memory cache 5分TTL + private HTTP cacheで再表示待ちを減らす。TTL超過後にタブがactiveになったら、表示済み内容を保ったまま背景再取得する。旧`/venture-map/amd-score/[projectId]`はこのタブへredirectする（`p99`デモを除く） |
