...
協力機関は機関別に段階（候補 / 情報交換 / 条件整理 / 面談調整 / 検証準備 / 合意確認 / 実行中 / 保留）、合意状態、最終接点、SX側の次アクション、期限、担当、関連ゲート、約束履歴を表示する。約束履歴は相手の約束とSX側の次アクションを分け、相手担当・約束日・一次根拠が揃う場合だけ相手の約束にする。協力機関パイプライン（`SxPartnerPipeline`）は主要view（重要経路の協力機関）を`dueDate`昇順で並べ、現在の関係段階→SX側の次アクション（期限つき）・合意済み/未合意・担当・最終接点・関連ゲートを同一行で密に表示する。優先度低・保留のファインケムは主要viewから除外し、末尾の注記行にのみ機関名を残す（地理名を付けた旧称は表示レイヤーで一切出さない、`sxPartnerDisplay`）。技術試験・資金スナップショット・組織役割・RACI・人員容量は各測定値を画面に描画し、編集可能にする。週次エフォートは柱/マイルストーン/次の成果に接続する。
