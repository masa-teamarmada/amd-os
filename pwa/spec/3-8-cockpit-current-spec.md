...
| `?tab=score-detail` | SU 系 PJ の `スコア詳細` を初期表示し、BZM 2.2のJ/P/Q/S、実数代入、gate別の通過・停止分岐、ブラウザ内の方針比較、一本のイベント時間軸と同じ月列の月次PL/BZM経済CF、計算入力、経営判断、初期閉じ103項目監査台帳を最上段、現行SPS / BZM 2.1、BZM 2.0、SPS 1.0 / Legacy AMDを最下部の初期閉じ・初回open時lazy mountアーカイブとして同じcockpit内で開く。月次PLの入力とつくよみヒアリングは時間軸直下に置き、PJヘッダーの独立した試算表ボタンは出さない |
| `?tab=business-plan` | SX (`p21`) の `事業計画` を初期表示する。他PJには同タブを出さない |
...
| venture status | `CockpitVentureStatus`, `Bzm22CockpitSummary` | 上部主スコアはread-only BZM 2.2 summary APIの$J/P/Q/S$評価日時点スナップショット。desktopは4指標を左の縦レール、XRLを右へ置く一体レイアウトにし、上下配置による空白を作らない。XRLは`ResizeObserver`で右ペインの実寸を測り、その幅・高さをSVG座標系へ使って左レールと揃う縦高を描画する。mobileは600pxの内部横スクロールだけを許す。$J$は全分岐込み現在価値、$P$は登録済み現在方針を固定した全条件通過時の現在価値で、どちらも評価日時点へ割り引く。各指標は数式と肯定形の一文を併記する。スコア詳細では主スコア直下に旧SPSの全幅開閉帯を置かず、旧SPS履歴はページ最下部のアーカイブへ集約する。`project_ventures`, `project_xrl_log`, related data |
| AMD / Management score hero | `CockpitManagementScoreHero` | AMD Score / Management Score derived data |
| tabs | `CockpitView` | `進捗管理` / `スコア詳細` (該当PJのみ) / `事業計画` (SXのみ) / `規程・内規` (KUTE `p25`のみ) / `会社概要` (常設) の display state。SX (`p21`) は4タブ、KUTE (`p25`) は3タブ、他SU系は3タブ、非SU系は2タブで、各タブのクリック領域はタブ数で均等分割する |
| score detail tab | `CockpitAmdScoreDetailTab`, `Bzm22ProvisionalObservatory`, `Bzm22TimeLedger`, `LazyArchiveDisclosure`, `Bzm21DynamicPolicyObservatory`, `Bzm2ModelObservatory`, `AmdScoreView embedded` | 正規URLは `/project/[projectId]/cockpit?tab=score-detail`。最上段の `data-testid="bzm22-provisional-primary"` はread-only `/api/project/[projectId]/bzm-2-2-pilot`から該当PJのcompact projectionを読み、`BZM 2.2`、慎重/基準/強気scenarioの$J/P/Q/S$、実数代入、gate別の条件付き通過値・累積生存・停止分岐、ブラウザ内だけの方針比較を表示する。数式代入の各項は内容幅で止め、`flex-grow`や`stretch`で空白の枠を作らない。警告banner、画面の役割を言い換えるcatchcopy、操作を重複説明する副文は置かず、値・定義・状態を直接表示する。`Bzm22TimeLedger` はM0から計算地平までの一本の時間軸へ登録方針とイベントを置き、その下に同じ月列で `project_pl_monthly` とBZM経済CFを表示する。SXの2027-01以前は費用入力合計を正の「設立前PJ支出」とし、NewCoの粗利・営業利益は対象外表示、2027-02以後だけNewCo P/Lとする。横スクロールは `data-testid="bzm22-shared-month-scroll"` の内部だけに閉じ、desktop/mobileともイベントと月次値の月位置を共有する。月次PLは同区画で直接編集し、つくよみヒアリングを開く。コックピット上部とHUDには独立した試算表ボタンを置かない。パラメータは「全パラメータ」「実行可能性と経営判断」「根拠・再現・監査」の三層へ分け、第一層は26項目の表とする。「選択中ケースの値」は条件名・範囲・方針名を含むため左寄せ、純粋な数値列だけ右寄せにする。7群103項目の完全台帳は最後の初期閉じ詳細に置き、件数だけの匿名表示ではなく、SXならPSI GAPファンド Step 2、VC DD、共同開発等の制度・金額・時期・確度・制約を登録値から具体表示する。方針比較は計算bundleが揃う候補だけを丸ごと切り替え、元の進め方との差を表示し、DBへ保存しない。イベントカードは副説明を付けず同月の全タイトルを省略せず表示する。最下部の `data-testid="score-model-archives"` には `現行SPS / BZM 2.1`、`BZM 2.0`、`旧SPS履歴 / Legacy AMD` の三トグルを順に置く。`CockpitAmdScoreDetailTab` は `/api/project/[projectId]/sps-current` に加え、`assessment.seed_id` があるときだけ `/api/seeds/screening-bands?seedId=` を追加取得し、`CurrentSpsAssessmentCard` の `band` へ渡す。カードはSPS帯・根拠Lv・評価日・対応シーズの直下に q帯 (q_lower_pct〜q_upper_pct と主要因)、P^ind帯、段階仮説、`p_class`、共通UI `SpsBandRationale` による総合判断 (`notes`) とq要因11項目の折りたたみ、「投資判断・対外表示には使わない」注記を展開する。帯の取得に失敗しても現行SPS本体の表示は落とさない。`compact` 表示 (PJ Status / HUD / 現行SPS一覧) では根拠を展開しない。根拠列は一覧DTOに含めず、この画面だけがseedId指定で読む (2026-08-20 追加。判断記録の md だけに根拠を置かない)。旧 `/venture-map/amd-score/[projectId]` はこのタブへredirectする（`p99`デモを除く） |
| SX business plan tab | `CockpitBusinessPlan`, `src/lib/sx-business-plan.ts`, `src/lib/sx-business-plan-xlsx.ts` | `projectId === 'p21'` のみ表示。順序は (1) 4開発レーン×5フェーズの表、(2) `CapitalPlanWorkspace` の100%株主構成推移と資本政策表、(3) 年次試算表。共通の枠は直接的な表題・項目名だけで構成し、PJ固有の事業仮説はフェーズのセルと計画データに置く。フェーズ表の直下の `data-testid="sx-phase-matrix-xlsx-export"` は、表示と同じフェーズ条件・4レーンの活動/費用/出口条件/XRLだけを横長1シートで出力する。先頭2行とレーン列を固定し、数式開始記号の文字列は無害化する。クライアント内のダウンロードだけで、API・DB・保存済み計画を更新しない。年次試算の直下には初期閉じの `data-testid="sx-annual-parameters"` があり、年次の売上・原価・助成金・人員・役員/社員別の報酬・旅費・消耗品費・その他設備投資・IPO以外の資金調達、4段階の自社工場の建設年度/投資額、IPO年度/公募額を変えると、PL・設備投資・株式調達・期末現預金を再導出する。これはページ内の未保存シナリオで、保存済みの資本政策・会社情報を更新せず再読み込みで初期値に戻る。フェーズ表の各セルは活動、予算、出口条件、関連XRLを持ち、列見出しはフェーズ総額、調達源、固定費バーン上限、5軸到達XRLを持つ。資本政策の作業台帳はSXに限り会社概要タブから事業計画タブへ移し、migration 196が既存プランを上書きせず初期原案を1件投入する |
...
- SX（p21）の`Bzm22TimeLedger`はP/Lの同じ月列へ、月初資金、営業C/F（営業利益代用の簡易値）、設備投資、株式調達、融資、助成金等入金、月次純C/F、月末資金を続ける。融資未登録は0円でなく未計画。PSI Step 2の6,000万円はPhase 0非希薄化資金の同一cashなのでFY2027へ再計上しない。採択額7,800万円は`project_grants`の採択証拠として表示するが、`disbursed_yen`未登録の間は実績入金へ置き換えない。BZM経済CFはJ/P用の別計算。表外に`単位：百万円`を一度だけ置き、月・金額は右寄せ、等幅数字、14px以上で表示する。
