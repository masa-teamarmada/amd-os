...
| KUTE 規程・内規 | `CockpitKuteRegulations` | KUTE (`p25`) only。`/project/p25/cockpit?tab=regulations` の専用タブ。契約対象の7規程（兼業、知財・ライセンス、出資・新株予約権、共同研究、利益相反、認定、共有機器利用）を最上段に置き、認定委員会内規・支援細則・認定審査チェックシートは随伴する認定運用文書として分離する。各行は現状、次のTODO、AMD管理期限、実在する最新版、原典、全途中版へ到達できる。原案未作成・原典回収待ちはリンクを偽装せず明示する。期限は大学の公式決裁日でなくAMD管理目標（2027/01目途）と明記する。文書実体はKUTE共有Driveの版管理フォルダへ集約し、画面にはメール本文・個人情報・secretを保存・表示しない。 |
