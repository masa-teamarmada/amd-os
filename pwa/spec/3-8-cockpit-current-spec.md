...
KUTEガントの年度末マーカーは「年度末 YYYY-MM（目途）」で、会社の「設立」や確定日と表示しない。ロードマップ由来 (`source_ref` が `KUTE年度内ロードマップ /` で始まる) かつ `status='unassessed'`・仮日程のKUTEタスクは、予定月を過ぎても「進捗未登録」とする。通常編集後・進捗登録後・確定日程・他PJの既存期限判定は変えない。DBの `progress_pct` はNOT NULLのため初期値0、未登録かどうかはstatusで区別する。時間軸は既存の非表示phaseコンテナ2件で6月〜翌3月を確保し、通常の期間バー6本と終端MS2件だけを表示する。
...
| KUTE annual roadmap | `CockpitProjectControl` → `SxWeeklyControlDashboard` → `SxUnifiedTimeline` | KUTE (`p25`) の年度内ロードマップ6工程を既存 `project_management_*` 台帳へ移し、`?tab=gantt` の全体ガントに通常の編集可能なタスクとして描画する。制度整備5工程とシーズ発掘・after GTIE 1工程、2026-06〜2027-03。制度整備は2027-01、支援実務型化は2027-03の終端MSに対応。旧 `CockpitKuteAnnualRoadmap` と静的カード表示は廃止し、ガントの二重実装・表示専用の擬似行を作らない。根拠は6/11キックオフ資料 / `PROJECT_BRIEF`。月初・月末は月単位計画を描画する仮日程で `date_certainty='provisional'`、目的・全成果物・月精度注記を保持する。完了実績や担当を補完しない。data-only migration `ios/supabase/migrations/20260831223000_kute_annual_roadmap_gantt.sql` は固定ID・追加のみで再実行時も人の編集と削除状態を保持する。既存認可・書込み経路・シーズタブ・他PJは不変（2026-08-31訂正） |
