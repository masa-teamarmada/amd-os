...
`CockpitView` は上段のグループと、選択中グループの子タブだけを表示する二階層ナビゲーションを使う。子が1件だけのグループは上段から内容へ直接移り、冗長な子タブ列を表示しない。通常PJは `進捗管理`（MS・月次 / 動向・会議 / 週次差分 / ガント / 関係先 / 論点・仮説）、`事業計画`（スコア詳細 / 技術 / 事業計画 / コスト試算 / 知財 / 資本政策）、`PJ管理`（PJ概要 / ドライブ / 会社概要）の3グループ。PJ概要には活動・実績、助成金、獲得台帳、AMDの貢献を含める。

`institution_projects` に現在ひもづく研究機関PJは、事業計画グループを持たず、`進捗管理` / `シーズリスト`（シーズ一覧） / `規程・内規`（規程一覧） / `PJ管理`の4グループとする。シーズと規程は、PJ番号の固定表ではなく解決済みの機関IDを使う。研究機関ページが `CockpitView` を埋め込む場合は `hideNavigation` を渡し、ページ側のECRタブと二重表示しない。
...
| navigation | `CockpitView`, `src/lib/cockpit-tabs.ts` | 通常PJは `進捗管理 / 事業計画 / PJ管理` の3グループ、研究機関PJは `進捗管理 / シーズリスト / 規程・内規 / PJ管理` の4グループ。上段はmobileで通常3列、研究機関2列×2行、640px以上で4列。下段は選択中グループの子が複数ある場合だけ表示し、狭幅では子タブだけを横スクロールする。所属とURL正規化は `cockpit-tabs.ts` を正本にする |
