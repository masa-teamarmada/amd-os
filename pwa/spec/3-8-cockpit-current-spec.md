...
| `?tab=score-detail` | SU 系 PJ の `スコア詳細` を初期表示し、BZM 2.2暫定主表示と103項目の全パラメータ台帳を最上段、現行SPS / BZM 2.1、BZM 2.0、SPS 1.0 / Legacy AMDを最下部の初期閉じ・初回open時lazy mountアーカイブとして同じcockpit内で開く |
...
| venture status | `CockpitVentureStatus`, `Bzm22CockpitSummary` | 上部主スコアはread-only BZM 2.2 summary APIの$J/P/Q/S$評価日時点スナップショット。各指標は数式と肯定形の一文を併記する。従来SPSの折れ線とM/P/R/Sカードは初期閉じの「旧SPS履歴」トグルへ残し、XRLは常時表示する。`project_ventures`, `project_xrl_log`, related data |
...
| score detail tab | `CockpitAmdScoreDetailTab`, `Bzm22ProvisionalObservatory`, `LazyArchiveDisclosure`, `Bzm21DynamicPolicyObservatory`, `Bzm2ModelObservatory`, `AmdScoreView embedded` | 正規URLは `/project/[projectId]/cockpit?tab=score-detail`。最上段の `data-testid="bzm22-provisional-primary"` はread-only `/api/project/[projectId]/bzm-2-2-pilot`から該当PJのcompact projectionを読み、`BZM 2.2 暫定主表示`、低位/中央/高位scenarioの代理指標・価値、初回経路喪失、7群103項目を省略しない `全パラメータ台帳` を表示する。`unvalidated` / `shadow_only` / 前向き検証0件、ランキング・撤退/継続・投資・配分への自動利用禁止を常設し、DBとSPS primary registryは更新しない。最下部の `data-testid="score-model-archives"` には `現行SPS / BZM 2.1`、`BZM 2.0`、`SPS 1.0 / Legacy AMD` の三トグルを順に置く。すべて初期閉じで、native `details` にchildを直置きせず初回open時だけmountし、その後は再開閉で状態を保持する。BZM 2.2未登録・API取得不能でも旧モデルを失敗させず、BZM 2.2区画だけに状態を示す。desktopは高密度表、390px mobileはlabel/valueを縦再配置しページ全体を横スクロールさせない。旧 `/venture-map/amd-score/[projectId]` はこのタブへredirectする（`p99`デモを除く） |
