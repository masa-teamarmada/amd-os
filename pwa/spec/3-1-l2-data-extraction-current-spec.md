...
| **D-12** | freee Transaction Actuals / 月次実績取込 | freee `trial_pl` / `company_actual_monthly` / `amd_management_score_raw_signals` | PWA non-LLM cron `/api/cron/management-score-raw-data?includeFreee=1` | freee取引履歴 → 月次試算表の実績値 |
