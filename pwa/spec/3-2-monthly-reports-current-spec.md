...
| `/(app)/project/[projectId]/report/[ym]/print` | 集約 JSON を A4 縦に表示。認証・PJアクセス判定後は通常のOSシェルを経由せず、左メニュー・月初合意・通知・チャットを画面にもPDFにも含めない。名前付き `@page` は使わず、文書内で唯一の既定 `@page` を `isSubmission` に応じて丸ごと差し替える。社内版は既定 `@page` を `A4 / margin 14mm 14mm 18mm 14mm` とし、各sheetの章分離・フッター・ページ番号を維持する。提出版は既定 `@page` を全案件共通で `margin 13mm 0 0 0` に切り替え、上部へ「提出先 / 月次報告 対象月」と「取扱注意 / Confidential」を左右に配置し、下部フッター・ページ番号は `content: none` にする。名前付き `@page` + `page` プロパティの併用は、フォールバック時に文書内の既定 `@page` (社内版仕様) へ戻り、本文終了後に社内版ヘッダー・フッターだけの余分な最終ページが出る事故につながるため禁止する。ブラウザ既定の日時・ページタイトル・URLは印刷領域へ出さず、印刷開始時だけ `document.title` を空にし、終了後に戻す二重ガードを持つ |
