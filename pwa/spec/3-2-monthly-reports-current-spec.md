...
| `/(app)/project/[projectId]/report/[ym]/print` | 集約 JSON を A4 縦に表示。認証・PJアクセス判定後は通常のOSシェルを経由せず、左メニュー・月初合意・通知・チャットを画面にもPDFにも含めない。名前付き `@page` は使わず、文書内で唯一の既定 `@page` を `isSubmission` に応じて丸ごと差し替える。社内版は既定 `@page` を `A4 / margin 14mm 14mm 18mm 14mm` とし、各sheetの章分離・フッター・ページ番号を維持する。提出版はPJ固有の前月実提出書式を優先する。KUTEは `A4 / margin 14mm 14mm 15mm`、SOLは `A4 / margin 0` と本文側の15mm/18mm余白・36em幅を継承し、前月にないヘッダーを追加しない。CX等の既存帳票は従来の `margin 13mm 0 0 0` と上部ヘッダーを維持する。下部フッター・ページ番号は表示しない。名前付き `@page` + `page` プロパティの併用は、フォールバック時に文書内の既定 `@page` (社内版仕様) へ戻り、本文終了後に社内版ヘッダー・フッターだけの余分な最終ページが出る事故につながるため禁止する。ブラウザ既定の日時・ページタイトル・URLは印刷領域へ出さず、印刷開始時だけ `document.title` を空にし、終了後に戻す二重ガードを持つ |
...
- 既存報告書の見出し・項目順・表構造・配色・書体・余白を変更しない。既存欄の日付・本文・リンクだけを修正し、既存のない主要成果物欄を新設しない。「本書」リンクは既存の月次帳票を開き、汎用Markdown readerへ置き換えない。PDFの改訂は元の生成経路・スタイルを維持し、別のデザインコードで作り直さない。
...
- 主要成果物は同月の `月次報告書 / YYYY年M月` にfile/linkとして置く。社内版・提出版とも、本文にある既存の「主要成果物」表の**成果物名そのもの**をその資料へのリンクにする。表の外に重複した成果物欄は置かない。対外共有可否は各資料の可視性と既存アクセス制御に従う
