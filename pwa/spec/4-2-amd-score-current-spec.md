...
ただし、値の版推移を初期から観察するため、`parameter_key='P'`には`P^(0)`（初期投影）を追記できる。現行スキーマは`value_json.schema='bzm2-potential-proxy-v0.2'`で、値は`initial_value`へ保存する。画面は数値を換算せず、分母やパーセント記号も付けない。規則は次のどちらかに限る。

| `source_mode` | 規則 | 前提 |
|---|---|---|
| `legacy_prs_potential` | `prs_potential`をそのまま使用 | 情報締切以前の`prs_potential`が記録済み |
| `readiness_mean_fallback` | `mean(TRL, BRL, GRL, SRL, HRL, FRL)` | `prs_potential`が欠測で、6軸すべてが記録済み |

`P^(0)`はBZM 2.0の価値ベクトル、社会的価値、条件付きDCF、校正済みの価値評価ではない。`components.V_soc`と`components.V_econ`は`unmeasured`のまま保存する。`q`が数値のPJだけ、UIは`SPS^(0)=q×P^(0)`の計算結果を無次元の初期値として表示する。`q`が欠測ならSPSは計算しない。

2026-08-11以降の現行Pは、`value_json.schema='bzm2-potential-vector-v0.1'`で保存する。
`components.V_soc`と`components.V_econ`は、それぞれ0から20の五軸と、その和である0から100の暫定指数を持つ。
社会と経済は別成分のまま保持し、`scalar_projection=null`とする。

各軸は`observed`、`proxy`、`imputed`の根拠状態、出所、注記を持つ。
直接証拠が足りない軸は値を欠測のまま止めず、中立点10と`imputed`を同時に保存する。
これは台帳全体の`missing`を0へ変える規則ではなく、P暫定尺度だけの明示的な補完規則である。

Pの評価条件は共通到達目標を保った事業化シナリオへの到達後とする。
XRL、`T_C`、`T_Y`、現在現金はPの十軸へ入れず、qとの二重計上を避ける。
経済成分は条件付きDCFへ置き換える前の代理指数であり、DCF、企業価値、売上予測として表示しない。
...
`Bzm2ModelObservatory`は数式記号をLaTeXで組む。たとえば`T_C`という生文字列を表示せず、$T_C$として下付きを含めて描画する。トップ式と到達競争式にはそれぞれ現在の代入・判定値を直下へ置く。`P^(0)`がある場合は$\mathbf P^{(0)}$と$\mathbf{SPS}^{(0)}$を使い、`q`との掛け算を表示する。P 2.0暫定値がある場合は$V_{\mathrm{soc}}$と$V_{\mathrm{econ}}$を別々に表示し、$q$があれば二成分のSPSを計算する。確率値はパーセントで表示し、`95%CI`という略語ではなく「95%信頼区間（計算上のぶれ）」を併記する。数値は本文書体の`tabular-nums`で揃え、等幅書体は版IDや内部キーだけに限る。P測定台帳は十軸の値、代理推定、欠測補完、出所を圧縮表へ並べ、代理推定は青、欠測補完は橙で文字とラベルを併用する。共通状態、時計、入力、資金、品質の台帳は、記号と変数名、現在値、測定状態・出所、反映先、履歴を同じ圧縮表へ並べる。各行の詳細で説明、出所参照、版別の値・状態・情報締切を確認する。$q$の版推移は初期状態で閉じ、必要なときだけ開く。
...
現行SPS入力は`quality.current_sps_input`として保持する。`prs_potential`をBZM 2.0の価値ベクトル`P`へ、XRLを条件付き確率へ自動変換しない。一方で`P^(0)`だけは、この節の二つの明示規則、情報締切、出所を満たす場合に限り、初期投影として追記する。P 2.0の暫定十軸は別版として保存し、旧Pを暗黙に流用しない。
