...
SXの資金時点は`project_meeting_summaries`から「2027年3月までの会社設立・資金調達」という内部目標を抽出し、`project_capital_plans.status=active`の初回株式イベント「2027-04 Seed 150百万円」と横並びで表示する。会議の内部目標は投資家合意、契約、着金実績ではなく、active資本政策の計上月も実績ではない。二つを同じ時点へ丸めず、1か月の差と証拠状態を残す。
