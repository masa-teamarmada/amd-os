...
2026-08-28 (まさ「これがまだトップに残ってるよ」) に、`CockpitVentureStatus` の上段から`Bzm22CockpitSummary`の$J/P/Q/S$を外した。2026-08-27「古いモデルの試算結果は、混乱の元になるのですべて削除してほしい」でスコア詳細タブから外したのと同じ表示で、上段にだけ残っていた。上段に残るスコア表示はXRLグラフだけで、XRLは`ResizeObserver`の実測幅・高さをSVG座標系へ使い、固定880×220のaspect比へ押し込めない。mobileはXRLの600px横長面だけを内部スクロールする。`Bzm22CockpitSummary`コンポーネントと`?view=summary`の単体画面は残すが、コックピットからは開かない。スコア詳細タブは現行SPSの評価カードとBZM 3.0のスコアパネルだけを持つ (`pwa/spec/3-8-cockpit-current-spec.md` の score detail tab 行が正本)。
