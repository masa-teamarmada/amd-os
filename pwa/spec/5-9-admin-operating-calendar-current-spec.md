...
> **状態**：実装済み（migration 178、`/admin/schedule`、派生生成・通知・再生成API）。
> **対象画面**：`/admin/schedule`。
> **画面名**：管理カレンダー。
...
管理カレンダーは、税務、労務、契約、報告、支払、ガバナンスに関する期限と、その前に必要な書類作成工程を、既存の正本と公的ルールから自動生成するadmin画面である。
...
| ガバナンス | 定時株主総会、招集通知・総会資料、議事録・登記要否確認 | `project_shareholder_meetings(project_id='p00')`、AMD運営ファクト、公的ルール |

### 後続拡張に含めるもの

- 補助金と助成金の実績報告、精算、事業終了期限。
- 特許、商標、年金、審査請求などの知財期限。
- 取締役会、公告など、定時株主総会以外のAMD本体ガバナンス期限。
- 契約外の外部機関向け定期報告。
...
納期限を過ぎたまま納付を確認できていない法定納付は、カレンダーの先頭で名指しする。ただし赤い警告へ出すのは、確定額で、支払済み・取消・要確認ではなく、freeeの照合窓に同額出金も無い行だけである。見積額、金額未取得、同額出金の月割当待ちは未納と断定せず`要確認`として扱う。

過去の月へ遡らないと気づけない状態だと、加算税と延滞税が積み上がってから郵送で届く。実際に2026年7月10日期限の源泉所得税が未消込のまま8月31日に不納付加算税の賦課決定通知が届いた。

先頭には、件数、本税・保険料の合計、加算税・延滞税の見込み合計を出し、行ごとに超過日数と見込みの内訳を出す。見込みの算式と割合の出典は`pwa/manual/6-9-company-payment-obligations-spec.md`が正本である。

#### 加算税・延滞税

加算税、延滞税、延滞金、督促は`tax_penalty_payment`または`social_insurance_penalty_payment`として、本税とは別の予定種別にする。所得税徴収高計算書の作成のような社内締切を派生させないためである。

未納から見積もった額は独立した予定にしない。納期限が通知書の到着まで存在せず、日付を推定すると架空の期限になる。親の法定納付の`metadata_json.penaltyEstimate`として持ち回り、先頭の警告で見せる。

賦課決定通知が届いたら`/admin/finance#payment-obligations`へ`source_kind='mail_notice'`で登録する。実受領行は期日を持つので、通常の納付と同じ形でカレンダーに並ぶ。

Gmailまたはemail抽出由来の行は、現行正本の`reviewed_at`、または互換入力の`review_status in ('reviewed','manual')`がある場合だけ取り込む。未確認メールの分類語だけで納税・社会保険予定へ昇格させない。
...
納付だけを期限順に見る画面は `/admin/payments` に分けた。仕様は `pwa/spec/5-15-payment-ledger-current-spec.md` が正本である。

管理カレンダーは契約、月次報告、提出物、ガバナンスも同じ時間軸へ並べるため、納付が他の予定に埋もれる。カレンダー側は期限超過の警告と時間軸の見通しを持ち、納付の一覧と会社が負う義務の全件目録は納付ページが持つ。

カレンダー先頭の警告からは納付ページへ導く。どちらの画面も金額・期日・納付済みの正本は `company_payment_obligations` のままで、手入力は持たない。

### 会社が負う税・保険料・提出の全件

カレンダーは生成できた予定しか出さないため、生成ルールが無い義務は画面から静かに消える。源泉所得税の不納付加算税が郵送で届くまでどこにも現れなかったのは、この構造による。

そこで、AMDに発生しうる義務の全件を静的な目録（`pwa/src/lib/admin-schedule/rules/statutory-obligation-catalog.ts`）として持ち、実際の生成結果と突き合わせて画面へ常時出す。各行は次のいずれかを必ず表示する。

| 状態 | 意味 |
|---|---|
| カレンダーに出ている | 対応する予定が生成されている。件数と次の期日を併記する |
| 会社の情報が足りない | 生成ルールはあるが、判定に必要な会社属性が未取得。足りない属性名を日本語で出す |
| 該当しない | AMDには発生しないと判定できた |
| 仕組みがまだ無い | 生成ルール自体が無い。何を作れば埋まるかを注記する |

該当するかどうか分からない行も、分からないまま残す。目録から行を落とさない。

目録には納付先、時期、根拠となる公的ページを持たせる。納付先が税務署・都道府県・市町村に分かれるものは、合算せず別の行にする。納付書が別に届くためである。

### 法定納付に先立つ社内締切

確定日を持つ法定納付から、次の社内工程を決定的に派生する。公式ルール行だけでなく、重複回避で公式ルール行に代わって表示される`company_payment_obligations`正本からも予定名を種別へ正規化して派生する。派生行は親予定の`occurrence_key`、種別、正本ID、正本hash、観測時刻を保持し、親が`needs_source`なら作らない。

| 親予定 | 社内締切 | 前倒し |
|---|---|---:|
| 法人税確定申告・納付 | 決算書・法人税申告書の作成 | 30日前 |
| 法人税中間申告・納付 | 中間申告書・納付額の確認 | 14日前 |
| 源泉所得税納付 | 所得税徴収高計算書・納付額の確認 | 5日前 |
| 社会保険料納付 | 納入告知額・口座残高の確認 | 5日前 |
| 労働保険年度更新 | 年度更新申告書・賃金集計の作成 | 14日前 |

### 定時株主総会

AMD本体は`project_shareholder_meetings(project_id='p00')`の定時株主総会日を最優先し、なければ`company_operating_facts.shareholder_meeting_date`を見る。確定日がある年は21日前の資料確定、14日前の招集通知・総会資料発送、当日の総会、7日後の議事録・登記要否確認を生成する。これらの前後日数はAMDの社内工程目安であり、会社法上の一律の法定期限とは表示しない。

確定日がない年は、2月の資料作成目安と3月の開催目安を`month`精度・`needs_source`・`正本要確認`で表示する。架空の日付を作らない。
...
AMD運営ファクト同期を毎日09:05 JST、既存の支払義務同期を09:20 JST、期限発生日生成を09:35 JSTとする。`/api/cron/company-schedule`はVercel cronで毎日実行し、支払義務の更新を同日中に派生予定へ反映する。修復実行では`?notify=0`を指定し、予定再生成と通知送信を分離できる。
...
このページ固有の構造として、今日の属する月を軸に、連続する12か月を実日付グリッドで置く。

表示範囲は`current month - 3`から`current month + 8`までとする。月曜始まりの各月を1280px以上は3列、1024px以上は2列、それ未満は1列で並べ、今月を強調する。カレンダー幅を奪う右レールは置かない。

```text
管理カレンダー                         2026年5月〜2027年4月
────────────────────────────────────────────────────
...
- 初期タブは`カレンダー`。画面には対象年月だけを表示し、期間計算の説明文は出さない。
- `年間運営`は副タブとして残し、暦年単位の支払・口座流出確認を担う。
- mobileも12か月を1列で連続表示し、`今日へ移動`と選択日のagendaを用意する。実日付グリッドを単なるリストへ置き換えない。
- `/admin`とGlobalNavのAdmin導線は`/admin/schedule`へ着地する。AdminSidebarでは管理カレンダーを最上段の独立グループに置く。
...
日付セルは日付と件数だけを表示する。予定名を狭い日付セルへ入れない。各月のグリッド直下に`この月にやること`を置き、日付、予定名、区分、状態、金額を省略せず時系列に並べる。`due_on`がない `month` / `period` は架空の日へ置かず、同じ一覧で`月内` / `期間内`と示す。月自体も不明な`unknown`は年間下段の「日付を生成できない締切」laneへ置く。
...
**12か月連続グリッド**を、desktop、tablet、mobileで一貫して使う。
...
既定表示は`カレンダー`とし、連続12か月の実日付グリッドを主表示に置く。`年間運営`は法定納付と口座流出を確認する副タブとして保持する。
...
- `/admin`、GlobalNav、AdminSidebarから`管理カレンダー`へ着地する導線。
- `current month - 3`から`current month + 8`までの12か月と、年またぎ・閏年末日の計算。
- 1280px以上3列、1024px以上2列、それ未満1列の月曜始まり実日付グリッド。
- 法定納付ごとの社内書類作成工程と親予定metadataの保持。
- 定時株主総会日の正本優先順位、確定時の4工程、未確定時の月精度`正本要確認`。
- 未確認Gmail/email分類行を法定納付へ昇格させないこと。
- `生成不能`欄。
- `元の正本で確認`導線。
...
11. 12か月表示から直近の期限、社内書類作成、期限集中月、生成不能を10秒以内に特定できる。

## 変更時の運用

公式期限を変更する時は、実装日の一次情報、参照URL、確認日、日付単体テストを同じbundleで更新する。`npm run test:admin-schedule`、型検査、production build、desktop/mobile実画面を確認し、仕様・FEATURE_REGISTRY・附則を同期する。
