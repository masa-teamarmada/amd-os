...
このページ固有の構造として、今日の属する月を軸に、過去3か月と今月から9か月の合計12か月を実日付グリッドで置く。
...
- 初期タブは`カレンダー`。過去3か月＋今月から9か月を表示する。
...
| 採用しない定番 | 代わりに使うもの |
|---|---|
| 月間カレンダーだけを置く | 過去3か月＋今月から9か月の連続表示 |
...
既定表示は`カレンダー`とし、過去3か月＋今月から9か月の実日付グリッドを主表示に置く。`年間運営`は法定納付と口座流出を確認する副タブとして保持する。
