...
| `GET /api/contracts/apply?termId=...&dryRun=1` | `src/app/api/contracts/apply/route.ts` | 反映プランのプレビュー (DB write なし)。admin または契約抽出secret限定 |
| `POST /api/contracts/apply { termId }` | 同上 | 契約正本とPJ/請求へ実反映。admin または契約抽出secret限定 |

締結済み原本を管理者が直接確認した場合は `POST /api/contracts/terms` で `status='candidate' / review_status='pending'` の terms-only 候補を作る。原本本文は Drive に残し、DBには Drive metadata・短い根拠注記・`source_term_hash` だけを保存する。`PATCH /api/contracts/terms` は `termId` と期待hashを照合してレビュー結果を `applied` または `rejected` に確定するだけで、正本への書き戻しは行わない。admin sessionのほか、D-13と同じ `CONTRACTS_EXTRACT_SECRET` (未設定時だけ `CRON_SECRET`) を使うoperatorを許可する。承認後も必ず `GET /api/contracts/apply?...&dryRun=1` で反映計画を確認してから、既存の `POST /api/contracts/apply` を使う。
...
| **p19 ZMP** | 葛飾ロード | **2 契約を過去セッションで抽出済み**: ① 月額 advisory 本契約 (¥300,000/月)、② OkuDoor システム開発 単発 (¥2,000,000 税抜、開発期間 202605〜202610) | ① 本契約 月額 ¥300,000 (税抜) = DB に反映済み (`monthly_fixed / 300,000`)。② 単発開発 ¥2,000,000 は `extra_revenue_json` (別財布) でPLだけを202605〜202610に一定按分 (≈¥333,333/月)。現金は実入金月を優先し、未確認時の予測は請求日2026-03-31と翌月末条件から202604に総額計上 | **null = 無期限 (= まさ確定: 契約期間は現状確認できないため endless で計上)** | ✅ DB は既に正しい。① 本契約 ¥300,000 と ② 単発 ¥2,000,000 (別財布) は別契約として両方反映済み。**追加 apply 不要 (触らない)**。本契約capは常に ¥300,000×65%=¥195,000 を基準にし、OkuDoor分は `tag='cap_extra'` の別財布支払として分離する。開発期の `budget_yen` 増額で通常capに混ぜる運用は使わない |
...
> - **(確定・触らない) p19 ZMP**: **2 契約構造を過去セッションで抽出済みで両方反映済み** — ① 本契約 月額 ¥300,000 (`monthly_fixed`)、② 単発 OkuDoor システム開発 ¥2,000,000 (税抜) を `extra_revenue_json` (別財布) でPLだけ202605〜202610に一定按分 (≈¥333,333/月)。現金は実入金月を優先し、未確認時の予測は請求日2026-03-31と翌月末条件から202604に総額を置く。**¥300,000 は単発ではなく月額本契約**。end_ym=null は endless (まさ確定: 契約期間は現状確認できない)。本契約capは ¥195,000 固定で確認し、OkuDoor分は別財布として分ける。**追加 apply 不要・触らない**。
