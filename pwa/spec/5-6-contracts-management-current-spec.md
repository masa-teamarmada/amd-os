...
## 契約からの契約書リンク (2026-09-17)

`projects.contract_terms_json.currentContracts[].documentUrl` に契約書の Drive webViewLink を入れると、PJコックピットの `契約` タブの契約カード見出しへ `契約書を開く` が出る (`CockpitProjectOverview.tsx`)。値が無ければ何も出さない。
