...
オーバーレイは小型のインスペクタとして動く (build v3.55.3)。desktopは幅約360px、内側の余白は4/8/12px、本文は `data-bzm-composer-scroll` の内部スクロール領域にする。ヘッダーのグリップだけを掴んで小窓を移動でき、可視帯の外へは出さない。閉じる×は置かず、**小窓そのもの以外**の pointer/tap（マップ空白、フィルタ、ページ余白、別ノード）で閉じる。小窓内の入力・ボタン・ヘッダードラッグは閉じない。外側クリックを捕捉しても元のclickは止めないため、別ノードの通常クリックはそのまま小窓を対象ノードへ切り替え、マップ空白では閉じるだけで下書きを作らない。新規作成・編集の種別選択は単一の `<select data-bzm-kind-select>` に統一した。`source`/`question` を選んだ時のlayer/status既定値ロジック (source→evidence/established、question→cross-layer/unknown) は変更しない。
