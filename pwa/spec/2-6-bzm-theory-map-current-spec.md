...
エッジはノード中心まで描かず、from側・to側それぞれの形状外周との交点で止め、矢印の先端をto側外周へ接触させる。エッジをクリックすると予約区画に確認パネルを出して接続解除できる。新規・派生・編集はマップ内オーバーレイ、削除確認は `graph + right/lower panel` とする。Escapeで閉じられるが、保存・削除処理中は閉じない。エッジ削除は線クリックのほか、選択ノード台帳「接続しているノード」の各行にも44pxの削除ボタンを置き、細い線を狙わなくても同じ削除確認へ入れる (`canEdit && edge.editable && edge.id` の時だけ表示、navigation用ボタンとは別要素で `stopPropagation`)。どちらの入口も同じ確認panelと `DELETE ?edgeId=` を再利用する。
