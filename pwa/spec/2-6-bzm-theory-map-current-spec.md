...
オーバーレイが開いている間に別ノードを通常クリックすると、確認や一旦閉じる操作を挟まず、そのノードの編集オーバーレイへ即座に切り替わる。切替元が新規作成の下書き (draft node) だった場合は、切り替える前にその下書きを破棄し、未保存の入力値は残さない。`Cmd`/`Ctrl` を押した接続クリックの分岐は、この通常クリックの切替判定より先に評価する。マップの空白 (背景) をクリックしたとき、オーバーレイが開いていれば同じクリックではオーバーレイを閉じるだけで、新しい下書きノードは作らない。次に背景をクリックしたときだけ新しい下書きが生まれる。オーバーレイ内部の `mousedown`/`click` は引き続き `stopPropagation` し、背景クリック判定へ伝播させない。
