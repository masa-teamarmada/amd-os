...
| `DELETE` | ノードのsoft-delete | query `nodeId=<id>`。`edgeId` と `nodeId` は片方だけを受け、両方指定・両方なしは400。active ノードだけが対象で、`archived_at` に現在時刻、`updated_by` に操作者を記録する |

`create_memo` は対象ノードが存在しactiveであること、`memoType` が5種のいずれか、`body` がtrim後1〜2000文字であることをAPIで検証する。

ノード作成後に1本目のエッジ作成が失敗した場合は、そのリクエストで作ったノードだけを補償削除し、片方だけ残さない。DBの生エラーはサーバーログに限定し、画面には安全な日本語エラーを返す。ノードのsoft-delete (`archiveNode`) も同じ境界で、DBエラーはログに限定し画面へは「ノードの削除に失敗しました。」を返す。

## UI / UX

### 全体構造 (2026-08-02 右台帳撤去、build v3.54.15)

ヘッダ → 検索・フィルタ → マップ/一覧の順。desktopは常設の右側読取りパネルを持たず、マップが利用可能な横幅いっぱいに広がる。ノードが存在するmobileは可読性の高い一覧を初期表示する。件数は真偽・確信度ではない。

2026-08-02以前は右側に選択ノードの summary / source_ref / body / カバレッジ警告 / メモ一覧 / 接続しているノード一覧を常設する読取りウィンドウを desktop で地図と並べていたが、まさの確定要件でこれを全廃した。ノードを読む・書く導線はマップ内編集オーバーレイ (「ノードを育てる」節) と、選択ノードの下部操作帯 (編集・メモを追加・`Cmd`/`Ctrl` 接続) の2つだけに一本化し、右の空白や固定幅の予約列 (旧 `lg:grid-cols-[minmax(0,1fr)_380px]`) は残さない。カバレッジの欠落警告・常設メモ一覧・常設接続ノード一覧はこの撤去にあわせて廃止し、他画面へ移設していない。
...
オーバーレイは小型のインスペクタとして動く (build v3.55.2)。desktopは幅約360px、内側の余白は4/8/12px、本文は `data-bzm-composer-scroll` の内部スクロール領域にする。ヘッダーのグリップだけを掴んで小窓を移動でき、可視帯の外へは出さない。閉じる×は置かず、**小窓そのもの以外**の pointer/tap（マップ空白、フィルタ、ページ余白、別ノード）で閉じる。小窓内の入力・ボタン・ヘッダードラッグは閉じない。外側クリックを捕捉しても元のclickは止めないため、別ノードの通常クリックはそのまま小窓を対象ノードへ切り替え、マップ空白では閉じるだけで下書きを作らない。新規作成・編集の種別選択は単一の `<select data-bzm-kind-select>` に統一した。`source`/`question` を選んだ時のlayer/status既定値ロジック (source→evidence/established、question→cross-layer/unknown) は変更しない。
...
オーバーレイが開いている間に別ノードを通常クリックすると、確認や一旦閉じる操作を挟まず、そのノードの編集オーバーレイへ即座に切り替わる。切替元が新規作成の下書き (draft node) だった場合は、切り替える前にその下書きを破棄し、未保存の入力値は残さない。`Cmd`/`Ctrl` を押した接続クリックの分岐は、この通常クリックの切替判定より先に評価する。`react-force-graph` の `onBackgroundClick` はCanvas背景にしか届かないため、`BzmTheoryMapView` は `pointerdown` captureで `composerOverlayHostRef.contains(event.target)` を判定し、小窓の外側なら画面のどこでも先に閉じる。capture handlerはclickを止めない。よって別ノードは続く`onNodeClick`で切り替わり、マップ空白は続く`onBackgroundClick`でもclose-onlyになり、新しい下書きノードは作らない。オーバーレイ内部の `mousedown`/`click` は引き続き `stopPropagation` し、内部操作を背景クリック判定へ伝播させない。
...
- **編集**: 選択ノードの内容を更新する。
- **削除** (2026-08-02 追加、build v3.54.8): editモードのfooter左に44px hit targetの控えめなvermilion削除ボタンを置く。押すと同じオーバーレイ内でフォームを削除確認へ切り替える (中央モーダルや三点メニューは出さない)。
...
エッジはノード中心まで描かず、from側・to側それぞれの形状外周との交点で止め、矢印の先端をto側外周へ接触させる。エッジをクリックすると予約区画に確認パネルを出して接続解除できる。新規・派生・編集はマップ内オーバーレイ、削除確認は `graph + right/lower panel` とする。Escapeで閉じられるが、保存・削除処理中は閉じない。この一時的な確認パネルは常設の右台帳ではなく、`edgeToRemove` が立っている間だけマップ領域の右/下に現れる。エッジ削除の入口は線クリックだけで、`DELETE ?edgeId=` を呼ぶ (2026-08-02 右台帳撤去で、旧「接続しているノード」一覧の行内44px削除ボタンは廃止した)。

### ノードを削除する (2026-08-02 追加、build v3.54.8)

管理者はノードを削除できるが、`bzm_theory_nodes` の物理DELETEではなく `archived_at` を設定するrecoverable soft-deleteで、store関数 `archiveNode` が担う。active ノードだけを対象にし、`updated_by` に操作者を記録する。archive後のノードはGETの `.is("archived_at", null)` フィルタで一覧から外れ、そのノードを指す既存edge/memoは孤児として `loadTheoryMap` が検出時にログへ残しつつ表示から静かに落とす (物理削除ではないため、DBの行自体は残る)。

editモードのフォーム footer 左に44px hit targetの控えめなvermilion削除ボタンを置く (desktopは`Trash2`アイコン+「削除」、狭幅はアイコン主体でも`aria-label="ノードを削除"`を必ず持つ)。押すと即削除はせず、同じオーバーレイ内で対象ノード名・接続しているノードの件数・メモの件数・影響説明を示す削除確認へ切り替える。切替前のフォーム未保存値は「編集へ戻る」で保持され、削除確認から編集へ戻ってもタイトル・要約等の入力は消えない。確認画面の「ノードを削除する」を押した時だけ `DELETE /api/bzm/theory-map?nodeId=<id>` を呼ぶ。

`edgeId` と `nodeId` は同じDELETEエンドポイントで片方だけを受け、両方指定・両方なしは400。削除成功後は、対象ノード・そのノードに接続する全edge (incident edges)・そのノードのメモ・保存済み位置をUI stateから取り除き、選択ノードとcomposerオーバーレイを閉じ、成功通知を出す。削除確認中もマップ背景クリックや別ノードクリックの既存契約は変えない (オーバーレイが開いていれば閉じるだけ、下書きは作らない等)。

### 閲覧

- 全文検索: id / title / summary / source_ref。
- filter: layer、status、relation type。relation filter は地図のエッジだけに作用する。
- map: kind を固有の塗り色（色覚に依存しない形も併用）、接続本数を半径、layer を横方向の帯で表す。status は一覧表示・編集オーバーレイの文字ラベルで確認し、ノード外周に点線・破線を重ねない。ノード中心に略字は置かない。
- list: kind / layer / status / 接続本数を1行で比較する。行クリックはそのノードの編集オーバーレイを開く。
- summary / source_ref / body / メモ一覧 / 接続しているノード一覧を常設表示する読取りウィンドウ (旧「選択ノード台帳」) は2026-08-02に全廃した。summary・source_ref・bodyは編集オーバーレイのフォームで、メモは選択ノードの `bzm_theory_node_memos` を編集オーバーレイの「メモを追加」フォームで、それぞれ書く。既存メモの一覧表示・既存接続の一覧表示・カバレッジ (外部支持なし/異議反証なし/tests接続なし) の欠落警告は、この撤去にあわせて画面から廃止し、他へ移設していない。
- タイトルのLaTeX (`$...$` / `$$...$$` / `\(...\)` / `\[...\]`) はKaTeXで数式表示し、不正な式でも画面全体を壊さない。一覧表示ではタイトルと要約、選択ノードの操作帯ではタイトルにこの表示を適用する。編集オーバーレイの各フォーム項目はプレーンテキスト入力で、保存前にKaTeXプレビューはしない。本文 (`body_md`) の常設Markdown/KaTeXレンダリングは、右台帳撤去にあわせて廃止した。
...
| `src/components/bzm/BzmTheoryMapView.tsx` | map/list (右台帳なし全幅)、空白クリック、通常クリック編集、Cmd二点接続、配置ドラッグ、線クリック、選択ノード操作帯 |
...
本番確認では保存・接続・削除・メモ追加を実行せず、確認前後で既存ノード数 / 関係数 / メモ件数が変わっていないことを確認する。desktop / mobile で空白クリック→クリック座標への保存前下書き即時生成 (Canvas描画1件だけ、`data-bzm-draft-node` などの重複HTMLマーカーが無いこと) ＋マップ内オーバーレイ1件を確認し、Escapeで破棄する。通常クリック→ノードを覆わない編集オーバーレイ、**小窓外のフィルタ・ページ余白・マップ空白を押す→保存なしで確実に閉じる、小窓内の入力・グリップを押す→閉じない、別ノードを押す→対象ノードへ切替**、ドラッグ→配置変更のみ、Cmd二点クリック→確認ボタンなしで仮線を即時表示、エッジ→両端がノード外周で停止、線クリック→非重複削除確認、種類色・外周点線なし・中心略字なし・LaTeX表示・非admin閲覧は、保存を伴わない検証環境または自動テストで確認する。desktopでマップが右側に固定幅の予約列 (旧380px台帳) を残していないこと、選択ノードの下部操作帯に「編集」「メモを追加」の2ボタンだけが出ること、旧「選択ノード台帳」(summary/source_ref/body表示、カバレッジ欠落警告、常設メモ一覧、常設接続ノード一覧、行内44px削除ボタン) が画面のどこにも再出現しないことを確認する。成長操作は「メモを追加」1ボタンで draft node を作らず `create_memo` だけを呼ぶこと、メモ追加後も nodes/edges の件数表示が不変であることをローカル/自動テストで確認する。ノード削除は、editモードfooterの削除ボタン→確認画面への切替とその表示内容 (ノード名・接続件数・メモ件数・影響説明)、「編集へ戻る」でのフォーム値保持、確認後の `DELETE ?nodeId=` 呼び出しまでを確認し、**本番確認で確定削除ボタンを実際に押さない**。DELETE ?nodeId= / ?edgeId= の実行確認はステージングまたは自動テストのdry-run範囲に限定する。

## 既知の制約

- ノード削除は `archived_at` によるrecoverable soft-deleteで、`bzm_theory_nodes` の行自体は消えない。誤記は編集で直せる範囲なら編集を優先し、反証済み理論は `refuted` で履歴を残す。
