...
| docs | `/bzm/map` | 理論マップ (論証台帳)。DBだけを正本に0件から本人が育てる。admin は空白クリックでノード作成、選択操作帯で育成・編集、ノード重ね合わせで接続、線クリックで解除する。全panelはマップ内。旧Markdownは履歴資産で自動表示しない。真理マップではなく件数・接続数は真偽・確信度を表さない | `bzm/map/page.tsx`, `BzmTheoryMapView.tsx`, `BzmTheoryComposerDialog.tsx`, `lib/bzm-theory-store.ts`, `/api/bzm/theory-map` |
