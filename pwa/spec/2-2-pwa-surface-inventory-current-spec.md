...
| admin audit | `/admin/change-history` | public schemaの変更を新しい順に密な要約台帳で表示し、実行者・日本時間の日時・PJ・対象名・変更内容を確認するadmin-only監査面。メンバー台帳は氏名またはコード名とmember_idで対象者を示し、最終ログインの連続更新は同一メンバーごとに件数と実効差分へまとめる。同一transactionと連続する自動処理を表示上まとめ、詳細に元の履歴行・ID・undoを残す | `admin/change-history/page.tsx`, `AdminChangeHistoryClient.tsx`, `change-history-presentation.ts`, `/api/admin/change-history`, `amd_os_data_change_history` |
...
| admin audit / access requests | `/api/admin/change-history`, `/api/admin/workspace-access` | 全体変更履歴のread、外部アクセス要求の承認/拒否 | `/spec/2-1-pwa-runtime-routes`, `/manual/2-6-admin-ops` |
