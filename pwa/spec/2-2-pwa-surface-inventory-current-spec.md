...
| admin audit | `/admin/change-history` | public schemaの変更を新しい順に密な要約台帳で表示し、実行者・日時・PJ・対象名・変更内容を確認するadmin-only監査面。同一transactionと連続する自動処理を表示上まとめ、詳細に元の履歴行とundoを残す | `admin/change-history/page.tsx`, `AdminChangeHistoryClient.tsx`, `change-history-presentation.ts`, `/api/admin/change-history`, `amd_os_data_change_history` |
