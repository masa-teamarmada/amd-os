...
| knowledge | `/knowledge-map` | AMD Materials。全118元素を淡黄→深紅のheatmap、日本語主用途、軸から独立した供給`警戒` / `危機`で俯瞰し、代表鉱物・鉱石、代表樹脂・高分子、2〜4件比較、従来のL2 force graphまで統合する。初期評価と未評価を分け、根拠sourceを示す。raw本文、画面内Q&A、DB write、LLM呼び出しは持たない | `knowledge-map/page.tsx`, `MaterialsKnowledgeView.tsx`, `KnowledgeMapView.tsx`, `materials-data.ts` |
