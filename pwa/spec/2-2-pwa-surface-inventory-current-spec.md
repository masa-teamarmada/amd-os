...
| admin audit | `/admin/change-history` | public schemaの変更を新しい順に一覧化し、実行者・日時・対象行・変更前後を同じ履歴行で表示するadmin-only監査面 | `admin/change-history/page.tsx`, `AdminChangeHistoryClient.tsx`, `/api/admin/change-history`, `amd_os_data_change_history` |
