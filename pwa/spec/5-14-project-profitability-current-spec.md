...
| 働いた分÷配れる額 | `稼働需要総額 ÷ 実効枠`。実効枠 = Σ `effectiveCapBudgetYen`（前月からの未使用枠繰越を含む按分上限） |
| 未払残 | 最新の実績月の、`payoutExcluded` でないメンバーの Σ `members[].stockYen` |
...
| 警報 | 条件 | 意味 |
|---|---|---|
| 未払いが積み上がる | `働いた分÷配れる額 > 1.5` | 働いた分が配れる額の1.5倍超。配りきれない差額が翌月へ送られ続けている |
| 報酬計算まだ | `配分枠 > 0` かつ `稼働需要 = 0` | 配分枠はあるが、この年の報酬計算が動いていない |

どちらも並び順には混ぜず、PJ名の横にラベルで出す。並び順は配分枠の降順。

> **2026-08-30 まさ指摘 — 「枠超え」という言い方をやめた**: 初版は警報名を「稼働が枠超え」、
> 列名を「需要/枠」としていたが、まさから「そもそも枠ってなに？ ちゃんと65%の予算に収まってる
> ように見える」と指摘された。指摘のとおり**配った額が上限を超えることはない**（`capped=true` の月は
> 上限ぴったりで止まる）。超えるのは配った額ではなく、メンバーがポイントを消化して発生した
> 「働いた分」の請求権であり、配りきれない差額は `stockYen` として翌月へ送られる。
> 予算オーバーと誤読されるため、列名を「働いた分÷配れる額」、警報名を「未払いが積み上がる」へ変更し、
> 実際に残っている未払残を金額の列として追加した。
>
> SX 2026年01月の実データ: 配れる上限 422,209円 / 配った合計 422,209円（上限ぴったり）/
> 働いた分の請求権 1,001,967円 / 翌月へ繰越 579,758円。

### 残高と月次フローを混ぜない

`stockYen` は残高スナップショットなので**月をまたいで合計しない**（7-1章）。未払残は
**最新の実績月の値だけ**を読む。また期末未払残は支払対象メンバー分だけを見る契約なので、
`payoutExcluded` のメンバー（まさ・きよ）の繰越は未払残へ含めない。
