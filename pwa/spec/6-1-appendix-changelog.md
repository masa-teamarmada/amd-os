...
| 2026-06-04 | 3-1 / 5-3 | 変更 | L2 routine 未登録事故を受け、cadence ベース束ね設計の新ナンバリング (D / M / H) を current truth として反映。D-1〜D-10 = Claude routine `amd-os-l2-consolidated-evidence` (daily 08:00 JST、旧②③④⑤⑦⑨⑩⑪⑫⑬)、M-1〜M-3 = `amd-os-l2-monthend-evidence` (月末最終日16:00→17:00、旧①⑧⑯)、H-1 = MMO automation `amd-os-l6-meeting-flow` (旧⑥)。旧③⑬を daily 化、旧①⑧⑯を月末1本へ統合。Claude routine = cloud = マシン非依存 (Mac 閉じても発火、MMO 不要)、Desktop/Local scheduled task (マシン依存) と区別、最小1h間隔・daily run cap 制約も明記 | Claude routine を止めた理由 (run cap) に対し複数 L2 を 1 routine に束ねる設計が未着手で、かつ Local task に全 disabled で置かれ L2 抽出が 2026-05-29 以降停止していた事故への正本是正。SKILL 2 本を `pwa/scheduled-tasks/` に追加 | えいみ |
