...
| 2026-06-04 | 3-1 / 5-3 | 変更 | L2 routine 未登録事故を受け、cadence ベース束ね設計の新ナンバリング (当時 D / M / H) を current truth として反映。Claude routine = cloud = マシン非依存 (Mac 閉じても発火、MMO 不要)、Desktop/Local scheduled task (マシン依存) と区別、最小1h間隔・daily run cap 制約も明記 | Claude routine を止めた理由 (run cap) に対し複数 L2 を 1 routine に束ねる設計が未着手で、かつ Local task に全 disabled で置かれ L2 抽出が 2026-05-29 以降停止していた事故への正本是正。2026-06-08 に W-1 と D-11 / D-12 を追加して現行化 | えいみ |
| 2026-06-08 | 3-0 / 3-1 / 5-3 | 追加・変更 | `/spec/3-0-l2-data-list-current-spec` を追加し、L2 を旧番号ではなく D/M/W/H の新番号で一覧化。D-10 を `Member Activity Evidence` に改名、D-11 `Media Mentions`、D-12 `freee Transaction Actuals / 月次実績取込`、W-1 `VC News / Funding Signals` を追加。M-2 / M-3 は M-1 Monthly Reports 抽出後でないと正規完了扱いにしない注記を追加 | まさ指摘「CodexはH系のみ」「L2-15はweeklyなのでW系」「L2-14はメディア掲載」「freee取引履歴→月次試算表実績値はdailyで回す」をOS設計書トップに反映するため | えいみ |
| 2026-06-09 | 3-3 | 追加 | MTGカード由来 Calendar upsert 一次防御を追加。`POST /api/meeting-calendar/upsert-plan` dry-run、deterministic key、extendedProperties metadata、重複判定、08:00-21:00広めブロック、強リマインド、外部attendees非招待、Gmail cron二次防御を明記 | KUTE/工学院の対面MTGを見落としかけたため、Gmail cronだけでなくOS上のMTGカード生成時点でCalendar欠落を検知する一次防御を固定するため | えいみ-worker |
| 2026-06-09 | 3-3 | 追加 | TODO / task 由来 Calendar 作業枠を追加。`POST /api/task-calendar/schedule-plan` dry-run、owner + まさ calendar の共通空き枠、`+<PJコード> <task>` title、attendees空、owner calendar不明時hold、H-1 automation chatへのevent id付き報告を明記 | MTGスケジュールだけでなく、AMDメンバーが抱えた作業タスクもCalendarに入り、まさが見落とさない一次防御にするため | えいみ-worker |
| 2026-06-09 | 3-3 | 変更 | task Calendar 作業枠を Gmail / Slack TODO 由来にも拡張。`source_kind='gmail_todo'|'slack_todo'`、`source_id/source_url/source_confidence`、`already_scheduled` 重複guard、Calendar write成功後の内部Slack DM nudge候補を追加 | H-1でGmail/Slackから拾ったTODOも見落とさず、automation chatだけでなくSlack DMで気づけるようにするため | えいみ-worker |
| 2026-06-09 | 3-8 / 4-1 / 4-2 | 変更 | AMD Score の current contract を PRS primary へ更新し、legacy M-X-F / 7軸 Cobb-Douglas を comparison / evidence / Appendix 扱いに整理。Cockpit score detail も PRS Primary / PRS history を主表示、legacy AMD は比較用と明記。FRL は PRS の S 要素、legacy F 軸として読む境界を補足 | UI実装側の PRS primary 化後も設計書が MXF 中心に残っていたため、読者が現行 primary を誤読しないよう正本を揃えるため | えいみ-worker |
