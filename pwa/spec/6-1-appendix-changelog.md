...
| 2026-06-01 | 5-6 | 追加 | 特許案 retrofit 実装分解案を追加。outcome ledger read UI、複数evidence refs、generic parameter governance、Before-Zero設立時期推奨、関連画面群の安全境界とworker分解を整理 | 特許案に書かれているが現OSで薄い箇所を、DB write / production migration / 外部送付なしで次workerが着手できる小単位へ分解するため | えいみ-worker |
