 ...
 *
 * **202608 まで移行月に含める (まさ確定 2026-08-28)。202609 稼働分から通常どおり合意を条件にする。**
 *
 * 延ばした経緯:
 * - 202606 まで: 契約改定前かつシステム未完成期間
 * - 202607 まで: 内部の状態が動くたびに再合意を求める実装だったため、合意が成立しないまま
 *   支払時期を迎えた (2026-08-28 修正)。まさ「月初合意のシステムが整いきってなかったので、
 *   特別に残り3人にも支払うことにした」
 * - 202608 まで: 合意の単位を member 単位から PJ 単位へ作り替えている最中のため。
 *   まさ「今月はまだ合意がなくても支払ってあげようって話になったので、全員分発行できるように」
 *
 * **毎月延ばさない。** 延ばすたびに、その月は「合意なしで払った月」になる。
 * 次に延ばすかどうかは、仕組みが変わっている最中かどうかで判断する。
 */
 ...
 *
 * まさ確定 2026-08-28「このPJ単位での合意は9月からにしてください。いままさに発行しようとしている分には
