...
const CLAUDE_ROUTINE_MANUAL_REASON =
  "Subscription automation / Codex Desktop automation は PWA から直接叩けない。手動キックは該当環境の automation 履歴または repo 内 SKILL.md を見て実行する";
...
export const rawDataSources: RawDataSource[] = [
  ...
  },
  {
    ...
    landsIn: ["atlas_signals", "observation_log", "macro_index_log", "atlas_stories", "vc_news"],
    notes: "Atlas Signals と Macrotrend Evidence/Index は L2 ⑪⑫。VCニュース等は canonical evidence に入った分だけ L2 扱い。",
  },
];
...
export const l2Datasets: L2Dataset[] = [
  {
    ...
    source: "Codex automation AMD OS L2① 月次報告抽出 / amd-os-ms outbox.monthlyReports (通常入力はSupabase内OS/L2 evidence、R313 triggerなし)",
  ...
  },
  {
    ...
    source: "MMOマシン Codex Desktop automation amd-os-l2-protocol-extract / legacy GAS 155 stopped",
    cadence: "daily 08:00 JST (MMO automation)",
  ...
  },
  {
    ...
    source: "MMOマシン Codex Desktop automation amd-os-l3-ms-progress-extract / Codex automation review",
    cadence: "毎時0分 JST (existing MMO automation)",
  ...
  },
  {
    ...
    source: "MMOマシン Codex Desktop automation amd-os-l4-project-knowledge-extract / legacy GAS 155 stopped",
    cadence: "daily 08:15 JST (MMO automation)",
  ...
  },
  {
    ...
    source: "MMOマシン Codex Desktop automation amd-os-l5-member-knowledge-extract / legacy GAS 155 stopped",
    cadence: "daily 08:30 JST (MMO automation)",
  ...
  },
  {
    ...
    source: "MMOマシン Codex Desktop automation amd-os-l6-meeting-flow / legacy GAS 153 + 074 stopped",
    cadence: "09:00-21:00 hourly JST (MMO automation)",
  ...
  },
  {
    ...
    source: "Codex automation amd-os-ms / outbox.registryDiffs / LaunchAgent applier",
    cadence: "6h review batch",
  ...
  },
  {
    ...
    source: "Codex automation amd-os-ms / outbox.xrlEvidence / LaunchAgent applier",
    cadence: "6h review batch",
  ...
  },
  {
    ...
    source: "Codex automation amd-os / strategy-signals outbox / LaunchAgent applier",
  ...
  },
  {
    id: "atlas_signals",
    label: "⑪ Atlas Signals",
    table: "atlas_signals (derived: atlas_stories / atlas_reports)",
    source: "Codex automation AMD Atlas外部シグナルレビュー / atlas_signal_review_tool.mjs / amd-atlas outbox",
    cadence: "daily 08:10 JST collection; report synthesis is derived",
    purpose: "外部機会・政策・市場・競合の atomic evidence。Atlas story/report は派生表示。",
  },
  {
    id: "macrotrend_evidence",
    label: "⑫ Macrotrend Evidence/Index",
    table: "observation_log / macro_index_log (derived: macro_lane_weights / triple_helix_state_log)",
    source: "PWA non-LLM macro-aggregate-indicators + Codex/Claude subscription review candidates",
    cadence: "monthly deterministic aggregate; LLM PWA routes disabled",
    purpose: "ASPI lane × month の numeric/index evidence。Atlas narrative signal とは別layer。",
  },
  {
    id: "member_weekly_activities",
    label: "⑬ Member Weekly Activities",
    table: "member_activities(source=member_weekly)",
    source: "future subscription automation + outbox/applier contract; old PWA route guarded manual only",
    cadence: "weekly candidate",
    purpose: "メンバー別の週次活動 evidence。source=inferred は低確信度fallback。",
  },
  {
    id: "amd_score_inputs",
    label: "Score Inputs",
...
export const cronOperations: CronOperation[] = [
  ...
  disabledCron({
    id: "pwa-atlas-daily",
    label: "Atlas daily report (L2⑪ derived / disabled PWA LLM)",
    layer: "PWA",
    cadence: "日次 21:00",
  ...
  disabledCron({
    id: "pwa-atlas-weekly",
    label: "Atlas weekly report (L2⑪ derived / disabled PWA LLM)",
    layer: "PWA",
    cadence: "週次 金08:00",
  ...
  disabledCron({
    ...
    label: "Atlas monthly report (L2⑪ derived / disabled PWA LLM)",
    layer: "PWA",
    cadence: "月次 1日22:00",
  ...
  disabledCron({
    ...
    label: "Atlas collect (L2⑪ old PWA LLM route disabled)",
    layer: "PWA",
  ...
  disabledCron({
    ...
    label: "L2⑬ メンバー週次活動抽出 (old PWA LLM route disabled)",
    layer: "PWA",
  ...
  },
  {
    ...
    label: "L2⑫ Macro aggregate indicators",
    layer: "PWA",
  ...
  // === Subscription automation / evidence routing (= L2 ①〜⑬。PWA LLM cronはdisabled維持) ===
  // Daily consolidated routine candidate: L2②④⑤⑦⑧⑨⑩⑪⑫。L2③/⑥は既存MMO automation、L2①は月末、L2⑬はweekly候補。
  {
    ...
    input: "Supabase内OS/L2 evidence / active・sales PJ 当月。raw 5生データ再スキャンは通常run対象外",
    output: "monthly_reports via amd-os-ms/outbox.monthlyReports",
    run: { type: "manual", reason: "Codex automationがoutboxを作り、LaunchAgentの非LLM applierが反映する" },
  },
  {
    id: "claude-l2-protocol-extract",
    label: "L2 ② AMDプロトコル抽出 (MMOマシン Codex Desktop automation)",
    layer: "Codex",
    cadence: "daily 08:00 JST",
    trigger: "amd-os-l2-protocol-extract (MMO automation)",
    defaultParams: "(SKILL.md に従う)",
  ...
  },
  {
    ...
    label: "L2 ③ MS進捗推定 (MMOマシン Codex Desktop automation)",
    layer: "Codex",
    cadence: "毎時0分 JST",
    trigger: "amd-os-l3-ms-progress-extract (MMO automation)",
  ...
  },
  {
    ...
    label: "L2 ④ PJナレッジ抽出 (MMOマシン Codex Desktop automation)",
    layer: "Codex",
    cadence: "daily 08:15 JST",
    trigger: "amd-os-l4-project-knowledge-extract (MMO automation)",
  ...
  },
  {
    ...
    label: "L2 ⑤ メンバーナレッジ抽出 (MMOマシン Codex Desktop automation)",
    layer: "Codex",
    cadence: "daily 08:30 JST",
    trigger: "amd-os-l5-member-knowledge-extract (MMO automation)",
  ...
  },
  {
    ...
    label: "L2 ⑥ MTGサマリ + フロー (MMOマシン Codex Desktop automation)",
    layer: "Codex",
    cadence: "09:00-21:00 hourly JST (= +60-180 分終了 events 拾い上げ)",
    trigger: "amd-os-l6-meeting-flow (MMO automation)",
  ...
  },
  {
    ...
    label: "L2 ⑦ OS台帳差分抽出 (Codex automation/outbox)",
    layer: "Codex",
    cadence: "6h ごと (:00)",
  ...
  },
  {
    ...
    label: "L2 ⑧ XRL根拠抽出 (Codex automation/outbox)",
    layer: "Codex",
  ...
  },
  {
    ...
    label: "L2 ⑨ 経営ハイライト抽出 (Codex automation/outbox)",
    layer: "Codex",
    cadence: "daily 03:20 JST",
    trigger: "Codex automation amd-os / strategy-signals outbox",
  ...
  },
  {
    ...
    label: "L2 ⑩ Textbook Insights 抽出 (Codex/local worker)",
    layer: "Codex",
    cadence: "TBD / manual start",
  ...
  {
    id: "codex-l11-atlas-signal-review",
    label: "L2 ⑪ Atlas Signals review (Codex automation/outbox)",
    layer: "Codex",
    cadence: "daily 08:10 JST",
    trigger: "AMD Atlas外部シグナルレビュー / atlas_signal_review_tool.mjs",
    defaultParams: "{\"outbox\":\"/Users/masa/.codex/automations/amd-atlas/outbox\"}",
    input: "public/external sources + recent atlas_signals health/dedupe",
    output: "atlas_signals (canonical evidence); atlas_stories/atlas_reports are derived",
    run: { type: "manual", reason: "Codex automationがoutboxを作り、LaunchAgentの非LLM applierが反映する。PWA atlas LLM cronはdisabled維持" },
  },
  {
    id: "pwa-l12-macro-aggregate-indicators",
    label: "L2 ⑫ Macro aggregate indicators (PWA non-LLM)",
    layer: "PWA",
    cadence: "月初 04:00 JST",
    trigger: "/api/cron/macro-aggregate-indicators",
    defaultParams: "{\"query\":{\"since\":\"YYYY-MM\"}}",
    input: "observation_log + atlas_signals",
    output: "macro_index_log aggregates",
  ...
  {
    id: "future-l13-member-weekly-synthesis",
    label: "L2 ⑬ Member Weekly Activities (future subscription/outbox)",
    layer: "Codex",
    cadence: "weekly candidate",
    trigger: "future weekly-member-activity-synthesis",
    defaultParams: "(next workerでoutbox contract確定)",
    input: "member-specific Gmail / shared calendar / source_cache / meeting summaries",
    output: "member_activities(source=member_weekly); source=inferred is fallback",
    run: { type: "manual", reason: "未登録。旧PWA /api/cron/member-weekly-activities はAnthropic経路があるためactive Vercel cronに戻さない" },
  },
];
