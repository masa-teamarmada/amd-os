...
// 金額を画面から伏せる仕組みは 2026-09-02 に外した。
// まさ「おれしか見てないのに伏せる意味ないよ。常に最新のスコアでちゃんと表示して。
// 表示してくれないと、さっき書いたような検証ができないじゃん。」
// 入力がどこまで埋まっているかは「入力の充足」の表（出どころの色と根拠の列）が見せる。
// 数字が古い定義で計算されていないかは、スコアの承認番号と現行の承認番号の照合が見せる。
