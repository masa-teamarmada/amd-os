...
export function computeSxJudgment(
  ...
  const overdueCommitments = commitments.filter((item) => item.status !== "completed" && Boolean(item.dueDate && item.dueDate < today));
  ...
  const dueSoon = [...milestones.filter((item) => item.status !== "completed").map((item) => item.forecastEnd || item.plannedEnd), ...decisions.filter((item) => item.status === "open").map((item) => item.dueDate), ...actions.filter((item) => item.status !== "completed").map((item) => item.dueDate), ...commitments.filter((item) => item.status !== "completed").map((item) => item.dueDate)].filter((value): value is string => Boolean(value && value >= today && value <= new Date(Date.parse(`${today}T00:00:00.000Z`) + 7 * 86400000).toISOString().slice(0, 10))).length;
  ...
  const nextDeadline = [...milestones.filter((item) => item.status !== "completed").map((item) => item.forecastEnd || item.plannedEnd), ...decisionQueue.map((item) => item.dueDate), ...actions.filter((item) => item.status !== "completed").map((item) => item.dueDate), ...commitments.filter((item) => item.status !== "completed").map((item) => item.dueDate)].filter((value): value is string => Boolean(value)).sort()[0] || null;
...
export async function getSxManagementBundle(projectId: string, canManage: boolean): Promise<SxManagementBundle> {
  ...
  const partners: SxManagementPartner[] = partnerRows.map((row) => {
    ...
    return { id: partnerId, slug: stringValue(row, "slug"), track: tracks.find((item) => item.isPrimary)?.track || tracks[0].track, tracks, name: stringValue(row, "name"), roleLabel: stringValue(row, "role_label"), relationshipStage: asPartnerStage(row.relationship_stage), agreementState: asAgreementState(row.agreement_state), agreedScope: stringValue(row, "agreed_scope"), unagreedScope: stringValue(row, "unagreed_scope"), lastContactDate: nullableString(row, "last_contact_date"), nextCommitment: stringValue(row, "next_commitment"), dueDate: nullableString(row, "due_date"), ownerLabel: stringValue(row, "owner_label", "担当未確認"), lastVerifiedAt: stringValue(row, "last_verified_at"), confidence: asConfidence(row.confidence), sourceKind: asSourceKind(row.source_kind), sourceRef: nullableString(row, "source_ref"), relatedMilestoneSlugs: relatedMilestoneIds.map((id) => stringValue(milestoneById.get(id) || {}, "slug")).filter(Boolean), relatedIssueSlugs: relatedIssueIds.map((id) => stringValue(issueById.get(id) || {}, "slug")).filter(Boolean), commitments: commitments.filter((item) => item.partnerId === partnerId) };
  });
  ...
  const judgment = computeSxJudgment(tracks, milestones, decisions, today, { kpis, actions, commitments, roles: organizationRoles, dag, objectivePresent: Boolean(objective), outcomesCount: outcomes.length });
