...
/**
 * VC 編集本体。/vcs/[id]/edit ページと、リスト画面の詳細モーダルから共通で使われる。
 * data の再読込は呼び出し側で onChange に実装する。
 */
export function VcEditBody({
  vcId,
  data,
  projects,
  members,
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold pr-10">{data.vc.name} を編集</h2>
      ...
      <FundsSection vcId={vcId} funds={data.funds} contacts={data.contacts} onChange={onChange} />
      ...
      <ContactsSection vcId={vcId} contacts={data.contacts} onChange={onChange} />
      ...
      <NewsSection vcId={vcId} onChange={onChange} />
    </div>
  );
}
    ...
    <Section title="基本情報 + AMD 相性">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <Field label="logo URL">
          <input className="i" value={form.logo_url} onChange={(e) => setForm({ ...form, logo_url: e.target.value })} />
        </Field>
        <Field label="slug (URL 用、英数小文字・ハイフン)">
          <input className="i" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} />
        </Field>
