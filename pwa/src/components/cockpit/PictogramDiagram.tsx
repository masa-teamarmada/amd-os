 ...
 *
 * 配置は src/lib/pictogram.ts の純関数。図は実寸 (1000px) で描き、狭い画面では横スクロールで読ませ、
 * 流れの一覧を図の下に出す。定義が読めないときは元の文をそのまま出す（MermaidDiagram と同じ考え方）。
...
export function PictogramDiagram({ code, tone = "light" }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* 図の実寸 (1000px) で親の幅を押し広げないよう、幅は親から受け取り、はみ出す分はこの枠の中で横に動かす */}
      <div className="overflow-x-auto [contain:inline-size]" data-testid="pictogram-scroll">
        <div
          aria-hidden="true"
      ...
      <div
        ...
        className={`border-t px-3 py-2 sm:sr-only ${isHud ? "border-cyan-300/20" : "border-[#e5e5ea]"}`}
      >
