...
const MEETING_LABEL: Record<string, string> = { agm: "定時株主総会", egm: "臨時株主総会", board: "取締役会" };
    ...
    <section className="rounded-lg border border-border bg-background">
      ...
      {!loading && !isEmpty && (
        <div className="divide-y divide-border text-[11px]">
          ...
          {meetings[0] && (
            <div className="px-3 py-2">
              <div className="mb-1 text-[10px] font-semibold text-muted-foreground">直近の総会</div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded border border-border bg-muted/40 px-1.5 py-0.5 text-[10px]">{MEETING_LABEL[meetings[0].meeting_type || ""] || meetings[0].meeting_type}</span>
                <span className="font-medium">{meetings[0].meeting_date || meetings[0].agenda_summary ? (meetings[0].meeting_date ?? "") : ""}</span>
                {meetings[0].amd_response && (
                  <span className="rounded border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-[10px] text-emerald-700">AMD: {RESP_LABEL[meetings[0].amd_response] || meetings[0].amd_response}</span>
                )}
              </div>
              {meetings[0].agenda_summary && <p className="mt-1 leading-relaxed text-muted-foreground">{meetings[0].agenda_summary}</p>}
              {Array.isArray(meetings[0].resolutions_json) && meetings[0].resolutions_json.length > 0 && (
                <ul className="mt-1 list-disc pl-4 text-muted-foreground">
                  {meetings[0].resolutions_json.slice(0, 5).map((r, i) => (
