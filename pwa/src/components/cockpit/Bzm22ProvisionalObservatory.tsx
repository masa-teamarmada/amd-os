...
function ParameterLedger({ groups, project }: { groups: Bzm22PilotParameterGroup[]; project: ProjectValueContext }) {
  ...
  return (
    <details data-testid="bzm22-parameter-ledger" className="border-t border-[#7898a5] bg-white">
      <summary className="cursor-pointer list-none bg-[#edf3f5] px-3 py-2 marker:content-none sm:px-4"><span className="text-[12px] font-semibold text-[#173f51]">103項目の監査台帳</span></summary>
    ...
    <section className="border-b border-[#b9cbd1] bg-white" aria-labelledby="execution-judgement-title">
      <header className="border-b border-slate-200 bg-[#edf3f5] px-3 py-2 sm:px-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-[8px] font-medium text-slate-500">保存なし</span>
        </div>
      </header>
...
function CalculationConditions({ pilot }: { pilot: Bzm22PilotProject }) {
  return <details className="border-b border-[#b9cbd1] bg-[#f8fafb]"><summary className="cursor-pointer list-none px-3 py-2 text-[9px] font-semibold text-[#376274] marker:content-none sm:px-4">計算条件</summary><div className="space-y-1 border-t border-slate-200 px-3 py-2 text-[8px] leading-4 text-slate-500 sm:px-4"><div><Tex tex={BZM22_FIXED_POLICY.formula} /> — {BZM22_FIXED_POLICY.label}</div><div>価値基準日 {formatDate(pilot.valuationDate)} / 情報締切 {formatDate(pilot.informationCutoff)}</div><div>前向き照合 {pilot.claimBoundary.forwardValidationCount}件</div><div>情報源走査: {pilot.sourceCoverage.sources.map((source) => `${source.key} ${source.paginationComplete ? "完了" : "継続中"}`).join(" / ")}</div></div></details>;
}
...
export function Bzm22ProvisionalObservatory({ projectId, active = true }: { projectId: string; active?: boolean }) {
  const [state, setState] = useState<LoadState>(() => { const cached = getCachedBzm22Pilot(projectId); return cached ? { status: "ready", projectId, pilot: cached } : { status: "idle", projectId }; });
  useEffect(() => {
    if (!active) return;
    let cancelled = false;
    loadBzm22Pilot(projectId)
      .then((pilot) => { if (!cancelled) setState({ status: "ready", projectId, pilot }); })
      .catch((error) => { if (!cancelled) setState({ status: "error", projectId, error: error instanceof Error ? error.message : "BZM 2.2の取得に失敗" }); });
