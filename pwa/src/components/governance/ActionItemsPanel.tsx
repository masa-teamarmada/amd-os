...
function chip(a: ActionItem) {
  if (a.status === "responded" || a.status === "done")
    return {
      txt: "対応済",
...
export function ActionItemsPanel({
  ...
  const load = useCallback(() => {
    ...
    fetch("/api/action-items")
      ...
      .then((j) => {
        if (j?.ok) setItems(j.items as ActionItem[]);
      })
      .catch(() => {})
  ...
  const markDone = async (id: string) => {
    const index = items.findIndex((item) => item.action_id === id);
    const resolvedItem = items[index];
    if (!resolvedItem) return;

    setItems((prev) => prev.filter((item) => item.action_id !== id));
    try {
      ...
      if (!response.ok || !body?.ok)
        throw new Error("要対応を更新できませんでした");
    } catch {
      setItems((prev) => [
    ...
    <section className="rounded-lg border border-border bg-background">
      ...
      {!loading && shown.length > 0 && (
        <div className="divide-y divide-border text-[11px]">
          {shown.map((a) => {
            const c = chip(a);
            const pjLabel = a.project_id
              ? projectLabels[a.project_id] || a.project_id
              : a.scope === "personal"
                ? "個人"
                : "全社";
            return (
              ...
              >
                <span
                  className={`rounded border px-1.5 py-0.5 text-[10px] ${c.cls}`}
                >
                  {c.txt}
                </span>
