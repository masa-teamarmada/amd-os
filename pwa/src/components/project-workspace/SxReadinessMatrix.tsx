...
function Valve({ filled, label }: { filled: boolean; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5" title={label}>
      <span className={`h-3 w-3 rounded-full border-2 ${filled ? "border-[#38745d] bg-[#38745d]" : "border-[#8e88a5] bg-transparent"}`} aria-hidden="true" />
    ...
    <span className="inline-flex items-center gap-1.5" title={label}>
      <span
        className={`h-3 w-3 rounded-full border-2 ${
          state === "confirmed" ? "border-[#38745d] bg-[#38745d]" : state === "provisional" ? "border-dashed border-[#bf7b2c] bg-transparent" : "border-[#8e88a5] bg-transparent"
        }`}
        aria-hidden="true"
      />
...
export function SxReadinessMatrix({ management }: { management: SxManagementBundle }) {
  ...
  return (
    <section className="rounded-xl border border-[#ada18a] bg-[#fffdf7]/95 p-4 sm:p-5" aria-labelledby="readiness-matrix-heading">
      ...
      <div className="mt-3 overflow-x-auto">
        <table className="w-full min-w-[520px] border-collapse text-xs">
          <thead>
            <tr className="border-y border-[#ada18a] bg-[#f2eee0] text-left text-[10px] font-semibold text-[#5f5a4d]">
          ...
          <tbody>
            {rows.map(({ track, cells, dueState }) => (
              <tr key={track.key} className="border-b border-[#c5bba5]">
      ...
      <p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] text-[#5f5a4d]">
        ...
        <span className="inline-flex items-center gap-1"><span className="inline-block h-2.5 w-2.5 rounded-full border-2 border-[#8e88a5]" aria-hidden="true" />未確認</span>
      </p>

      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div>
          <h3 className="text-xs font-semibold text-[#24231f]">協力機関の段階</h3>
          <ol className="mt-2 flex flex-wrap gap-1.5 text-[9px] font-semibold text-[#5f5a4d]" aria-label="協力機関の関係段階（候補から実行中・保留まで）">
          ...
          <div className="mt-3 space-y-2">
            ...
            {partners.map((partner) => {
              ...
              return (
              <div key={partner.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[#c5bba5] bg-[#f2eee0] px-3 py-2">
                <div className="min-w-0"><p className="truncate text-[11px] font-semibold text-[#24231f]">{display.name}</p><p className="text-[10px] text-[#5f5a4d]">{partner.roleLabel}</p></div>
                <span className="shrink-0 rounded-full border border-[#86a2b3] bg-white px-2 py-0.5 text-[10px] font-semibold text-[#315f7d]">{PARTNER_STAGE_LABEL[partner.relationshipStage]}</span>
              </div>
              );
            })}
            {deferredPartners.map((partner) => <p key={partner.id} className="px-1 text-[9px] text-[#5f5a4d]">{sxPartnerDisplay(partner).name}: 優先度低・保留（重要経路外）</p>)}
          </div>
        </div>
        <div>
