...
**背景・経緯は文章、決定・アクション・残課題は箇条書きの Markdown narrative** に書き直す。
...
## ✅決まったこと
（1項目1論点の Markdown 箇条書き。各項目は \`- \` で始め、決定内容・条件・前提・数字を1文または短い2文で書く。raw decided が空ならこのセクション自体を出さない）

## ▶️次の一手
（1項目1アクションの Markdown 箇条書き。各項目は \`- \` で始め、担当と期限が分かる場合は **担当 / 期限** を文頭に置く。担当が未定なら **要確認**、期限が未定なら **期限未定** と書く。チェックボックスにはしない。raw next_actions が空ならこのセクションを出さない）

## ⚠️残課題
（1項目1論点の Markdown 箇条書き。各項目は \`- \` で始め、未決事項・リスク・確認待ちと、その影響または確認条件を簡潔に書く。重要な期限・閾値は **太字** で。raw risks が空ならこのセクションを出さない）
...
6. **セクション別の表現を固定する**: \`## 🎯背景\` と \`## 📊経緯\` は段落で書く。\`## ✅決まったこと\`、\`## ▶️次の一手\`、\`## ⚠️残課題\` は \`- \` の Markdown 箇条書きで書く。番号付きリストとチェックボックスは使わない。
7. **強調の使い分け**:
   - **太字** = 重要キーワード・固有名詞・数字・日付
   - \`|...|\` table = 並列構造の情報（マトリクス・対応表・候補リスト）
8. 見出しは \`## 🎯背景\` → \`## 📊経緯\` → \`## ✅決まったこと\` → \`## ▶️次の一手\` → \`## ⚠️残課題\` の固定順とし、絵文字と語の間に空白を入れない。
