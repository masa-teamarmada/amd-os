...
export async function PATCH(request: Request) {
  ...
  const nextTags = Array.from(new Set([...baseTags, cropTag(crop)]));

  const { error: updateError } = await admin
    .from("media_assets")
    .update({ tags: nextTags, updated_by: "company_media_asset_crop_api" })
    .eq("asset_id", assetId);
  ...
  return NextResponse.json({ ok: true, assetId, crop });
}
