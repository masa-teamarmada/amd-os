...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.projectsPatch && Object.keys(body.projectsPatch).length > 0) {
  ...
  if (body.venturesPatch && Object.keys(body.venturesPatch).length > 0) {
    const { error } = await db
