...
// 権限は admin 専用 (人別の報酬金額を含むため member へ露出しない)。
...
import type { KiyoMoneyFlowPeriod } from "@/lib/finance/kiyo-money-flow-types";

export const runtime = "nodejs";

const CACHE_CONTROL = "private, max-age=60, stale-while-revalidate=600";
const PERIODS: KiyoMoneyFlowPeriod[] = ["month", "season", "all"];
...
export async function GET(req: NextRequest) {
  ...
  const period: KiyoMoneyFlowPeriod = PERIODS.includes(periodParam as KiyoMoneyFlowPeriod)
    ? (periodParam as KiyoMoneyFlowPeriod)
    : "season";
  const force = searchParams.get("fresh") === "1";

  try {
    const result = await getKiyoMoneyFlow(period, force);
