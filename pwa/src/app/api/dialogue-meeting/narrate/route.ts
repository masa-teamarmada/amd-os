 ...
 * その MTG に参加していなかった人でも 🎯背景 → 📊経緯 → ✅決まったこと → ▶️次の一手 → ⚠️残課題 が分かる Markdown narrative に
...
**その MTG に参加していなかったチームメンバーでも**「なぜこの議論をしたか」「議論の中で何が動いたか」
...
（3-8 文の物語形式。何から議論が始まり、どの視点で深まり、論点がどう動いたか。
 箇条書きではなく文章でつなぐ。事実だけ淡々と書くのではなく、会議にいなかった人が流れを復元できるように議論の含意を残す。
 重要な転換点・キーワードは **太字** で強調する。
 **入力 raw progress[] / decided[] の中に Markdown 表 (\`| ... | ... |\` 形式) があれば、このセクション内に必ず本文として再現する**。
 表ごとに前後 1 行で「これは何の表か」を説明する short caption を付ける）

## ✅決まったこと
（1項目1論点の Markdown 箇条書き。提案前の論点整理セッションでは、ここに「会社として正式決定済みの事項」ではなく、
チームに相談する前提で **この場で提案として固まったこと** を書く。各項目は \`- \` で始め、提案内容・条件・タイミング・前提を簡潔に書く）

## ▶️次の一手
（1項目1アクションの Markdown 箇条書き。各項目は \`- \` で始め、担当と期限が分かる場合は **担当 / 期限** を文頭に置く。担当が未定なら **要相談**、期限が未定なら **期限未定** と書く。チェックボックスにはしない）

## ⚠️残課題
（1項目1論点の Markdown 箇条書き。各項目は \`- \` で始め、未決事項・リスク・確認待ちと、その影響または確認条件を簡潔に書く。重要な期限・閾値は **太字** で）

# 絶対ルール
1. 見出しは必ず \`## 🎯背景\` → \`## 📊経緯\` → \`## ✅決まったこと\` → \`## ▶️次の一手\` → \`## ⚠️残課題\` の5つをこの順で出す。絵文字・文言・順序を変えず、絵文字と語の間に空白を入れない。
2. **このセッションは「提案前の論点整理セッション」と呼ぶ**。raw 内に別呼称があっても、narrative ではこの呼称へ統一する。
3. **\`## ✅決まったこと\` はチームへの相談前に提案として固まったことを書く欄**。特定個人だけで会社として確定したように読める言い方は避ける。
...
7. **セクション別の表現を固定する**: \`## 🎯背景\` と \`## 📊経緯\` は段落で書く。\`## ✅決まったこと\`、\`## ▶️次の一手\`、\`## ⚠️残課題\` は \`- \` の Markdown 箇条書きで書く。番号付きリストとチェックボックスは使わない。
8. **強調の使い分け**:
   - **太字** = 重要キーワード・固有名詞・数字・日付
   - \`>\` blockquote = セクション全体の前置き・補足
   - \`|...|\` table = 並列構造の情報（マトリクス・対応表・候補リスト）
9. **固定の絵文字見出し** を使って各セクションの目的を直感的に示す（🎯背景 / 📊経緯 / ✅決まったこと / ▶️次の一手 / ⚠️残課題）。
10. **入力 raw に書かれていない情報を勝手に追加しない**（推測・創作禁止）。ただし raw 内の別文脈から自明な補足（例: 略称→正式名称の展開）は許可。
11. **5見出しは必ず出す**。raw が薄いセクションでも、創作せずに「この回では新しい決定はなく、主に論点整理に留まった」のように文章で状態を書く。
12. **文章は途中で切らない**。出力全体が長くなっても全セクションを完結させる。表を含めて最後まで書ききる。
13. 想定文字数は問わない（短い議論セッションは短く、表を含む重い議論は長くてよい）。`;
