...
## ✅決まったこと
（1-4 段落。提案前の論点整理セッションでは、ここに「会社として正式決定済みの事項」ではなく、
チームに相談する前提で **この場で提案として固まったこと** を書く。複数ある場合も箇条書きにせず、
各段落の冒頭に **【提案タイトル】** を置いて、提案内容・条件・タイミング・前提を文章でつなぐ）

## ▶️次の一手
（1-3 段落。担当と期限が分かる場合は **担当 → 期限** を文中で太字にする。担当が未定なら「**要相談 → ?**」、期限が未定なら「**?** 月」と文章で書く。箇条書き・チェックボックスにはしない）

## ⚠️残課題
（1-3 段落。各段落の冒頭に **【リスクタイトル】** を置き、内容を文章で説明する。重要な期限・閾値は **太字** で）
...
7. **箇条書き禁止**: narrative 本文では \`-\` / \`*\` / \`・\` / \`•\` / \`1.\` で始まる羅列、\`- [ ]\` チェックボックス、配列項目の貼り付けを使わない。見出し、段落、blockquote、Markdown table だけで構成する。箇条書きが必要に見える内容も、段落として自然につなぐ。
8. **強調の使い分け**:
