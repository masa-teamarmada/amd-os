 ...
 * その MTG に参加していなかった人でも 背景 → 議論の流れ → 提案 → 残課題 が分かる Markdown narrative に
...
## 💬 チームへの提案案
> このセクションは「提案前の論点整理セッションで整理した、チームに『こうしてはどうか』と出す提案」を文章で説明する。
> **「決定」「決まったこと」とは書かない**。必ず「提案」「相談」「議論したうえでの方向性」のニュアンス。

（1-4 段落。複数提案がある場合も箇条書きにせず、各段落の冒頭に **【提案タイトル】** を置いて、提案内容・条件・タイミング・前提を文章でつなぐ）
...
9. **絵文字見出し** を使って各セクションの目的を直感的に示す（🎯 背景 / 💭 議論 / 📊 マップ / 💬 提案 / ✅ TODO / ⚠️ 残課題）。
10. **入力 raw に書かれていない情報を勝手に追加しない**（推測・創作禁止）。ただし raw 内の別文脈から自明な補足（例: 略称→正式名称の展開）は許可。
11. **入力 raw が空のセクションは出力にも出さない**。「(まだ記録なし)」のような空行も書かない。
