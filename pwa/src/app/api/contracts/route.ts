...
export async function POST(req: Request) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      is_current_for_project: Boolean(body.is_current_for_project ?? body.isCurrentForProject ?? false),
      amd_entity_name: optionalText(body.amd_entity_name ?? body.amdEntityName) || "株式会社チームアルマダ",
      amd_party_role: optionalText(body.amd_party_role ?? body.amdPartyRole),
      party_confirmation_note: "管理画面からAMD当事者契約として作成",
      party_confirmed_at: new Date().toISOString(),
      party_confirmed_by: auth.user.email,
