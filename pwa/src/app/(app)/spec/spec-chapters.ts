...
export interface SpecChapterConfig {
  ...
  title: string;
  summary: string;
  aliases?: SpecChapterAliasConfig[];
}

export interface SpecChapterAliasConfig {
  slug: string;
  title: string;
  summary: string;
}
...
export const SPEC_CHAPTERS: SpecChapterConfig[] = [
  ...
  {
    slug: "3-1-l2-data-extraction-current-spec",
    title: "L2 データ抽出 / Outbox 仕様",
    summary: "5 生データ、L2 ①〜⑩、subscription automation、outbox / LaunchAgent 反映の確定仕様。",
    aliases: [
      {
        slug: "3-0-l2-data-list-current-spec",
        title: "L2データリスト",
        summary: "旧設計書目次で使っていたL2 ①〜⑯の正本リスト入口。現行のL2データ抽出 / Outbox 仕様へ接続。",
      },
    ],
  },
