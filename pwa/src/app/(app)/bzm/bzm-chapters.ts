...
export const BZM_CHAPTERS: BzmChapterConfig[] = [
  ...
  { slug: "new-book1-ch-1-section-0-2", title: "§1.0.2 — ずれの肥大: 状態と観測量の差が Before Zero で開く理由", summary: "翌週月曜の URA との後日談 (IF 11.2 で「数字は上、感触は下」) を冒頭 blockquote に、観測のずれ (observation gap) を写像 g の見落とし・ノイズ ε・自選バイアス b の三層に分解。$y_t = g_{\\text{見せる}}(s_t) + \\varepsilon_t + b_t$ と $\\mathrm{Var}(y_t|s_t) \\approx \\mathrm{Var}_g + \\mathrm{Var}_\\varepsilon + \\mathrm{Var}_{\\text{sel}}$ を display で。雨樋とバケツ / 霧の湖で釣り / 三人の語尾・沈黙・語気の揺らぎ の比喩。Heckman 1979 / Goodhart 1975 / Cover-Thomas 2006 / Knight 1921 引用。完成 v1 (約 3,100 字 6 段落、まさレビュー待ち)。", status: "completed", level: 3 },
  { slug: "new-book1-ch-1-section-0-3", title: "§1.0.3 — 測らない自由はない: 共通言語・判断の構造化・再現可能性", summary: "金曜夜の C 氏 (駅前珈琲店で十五分動けない) を冒頭 blockquote で呼び戻し、観測の手続きが背負わされた三つの実務的責務 (common vocabulary / structured decision / reproducibility) を立てる。display 式 $\\mathbb{E}[L(d, s) | y] = \\int L(d, s) p(s|y) ds$ で「観測しないとは事後分布を事前分布に縮退させること」を一行で支える。Hayek 1945 / Simon 1955 (限定合理性) / Popper 1959 (反証可能性) / Schön 1983 引用。比喩: 同じ盤面を見るための辞書 / サッカーのフォーメーション図 / 山に入る登山者のロガー / 地図と歩き方と足跡。完成 v1 (約 2,470 字 6 段落、まさレビュー待ち)。", status: "completed", level: 3 },
  { slug: "new-book1-ch-1-section-0-4", title: "§1.0.4 — 本章の道筋: 二項関係を二層に開く", summary: "§1.0 全体を閉じるロードマップ章 (冒頭 blockquote 省略)。§1.1〜§1.4 = 一階建ての観測装置 (測るの三項関係 / 状態空間 (P, R, S) / 観測量の構造 / Before Zero 観測問題)、§1.5 = 二階建てに増築 (案件層 PRS + 機関層 ERS の二層観測)。display 式 $\\mathrm{PRS} = P \\times R \\times S$ と $\\mathrm{ERS} = 100 \\cdot \\frac{\\sum_k w_k A_k}{\\sum_k w_k}$ を予告、乗法 vs 加重和の対比と PRS×ERS 結合禁止 (Ch 9-10) も予告。Stevens 1946 / Marschak-Radner 1972 引用。比喩: 道のり地図 / 一階建てと二階建ての観測装置 / 家計簿の収支と純資産 / レンズの倍率切り替え / 地図と地形を分けて持つ。§1.0.1 の三人 (A/B/C 氏) を呼び戻し、s-survival 末尾オマージュ「あの違和感に、構造を持って答えるための章が、ここから始まります」で §1.0 全体を閉じる。完成 v1 (約 1,920 字 4 段落、まさレビュー待ち)。", status: "completed", level: 3 },
