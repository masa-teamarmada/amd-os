...
export const BZM_CHAPTERS: BzmChapterConfig[] = [
  ...
  { slug: "book-a-ch-4-5", title: "第4章 — 【統合章タイトル未確定】— P と R を観測で採点する", summary: "現行第4章(P)と第5章(R)を統合する章。前半は断熱素材の P を証拠の質で採点し、後半は同じ素材の R を5軸XRLで実測する。P/R は主張の査定ではなく観測から採点する、という第II部の背骨を立てる。54p目安。章タイトルは未確定。", status: "not-started" },
  ...
  { slug: "book-a-ch-11", title: "第10章 — 苗床を測る — エコシステム構築率 ECR の8軸と加重和", summary: "案件を生み出す装置を測る第二の観測層。ECR 加重和 (充足率)・8軸 rubric・軸7のゲート性・unknown vs not_started。「張るのは案件、通うのは苗床」。28p。", status: "not-started" },
  { slug: "book-a-ch-12", title: "第11章 — 二層非可換性 — 案件と機関を混ぜてはいけない理由", summary: "SPS (乗法・案件) と ECR (加重和・機関) を単一スコアへ結合してはならないことを、公理 A1-A4 の不可能性定理ステートメント+証明スケッチと Simpson 反転 worked example で示す「読む数学」の回。26p。", status: "not-started" },
  { slug: "book-a-ch-13", title: "第12章 — ラウンドテーブル — 二層を結合する組成機構 [仮説的第三柱]", summary: "RT = Before Zero の DTSU を keystone に据えた多者・連鎖・相互牽制の共同体 ℛ。成立3条件の命題化・CRL/ICT レンズ・Ψ 分解仮説 (検証プログラム付き仮説と明示)。独禁注意喚起 Box 必置。32p。", status: "not-started" },
  { slug: "book-a-ch-13-5", title: "第13章 — CEOという難問 — 機能は分割できても、愛情の総量だけは外注できない", summary: "章頭は「肩書のまま決める機能を失った男」(一枚の組織図)。CEO 像の通時変化・制度と市場の不整合・機能分割 (新第5章=旧第6章 §6.7 の実務適用)・エバンジェリスト機能の非外注性・育成可能性・肩書だけの CEO という7つの問いを実話で辿り、Before Zero から法人化後までの一気通貫供給を解として示す「問い×実話」変奏章。AMD の実践は序章/あとがき/著者性 Box でのみ明かす (a案)。26p。", status: "not-started" },
