...
export const metadata: Metadata = { title: { absolute: "管理カレンダー - AMD OS" } };

export default async function AdminSchedulePage() {
  const supabase = await createClient();
  const data = await loadScheduleView(supabase, scheduleGenerationRange(todayJst()));
  return <Suspense fallback={<div className="rounded-xl border border-border bg-card p-6 text-sm text-muted-foreground">管理カレンダーを読み込み中…</div>}><AdminScheduleClient initialData={data} /></Suspense>;
}
