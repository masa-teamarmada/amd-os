...
export default async function AdminProjectsPage() {
  ...
  const projects: ProjectRow[] = (pjData ?? []).map((p) => {
    ...
    return {
      ...
      monthly_report_scope: (p.monthly_report_scope === "internal_only" || p.monthly_report_scope === "internal_and_external") ? p.monthly_report_scope : "none",
      report_local_alias: p.report_local_alias ?? null,
      report_extra_allow_terms: Array.isArray(p.report_extra_allow_terms) ? p.report_extra_allow_terms : [],
      work_content: Array.isArray(p.work_content) ? p.work_content : [],
  ...
  return (
    <div>
      ...
      <p className="text-xs text-muted-foreground mb-3">
        PJ情報の一覧。請求先・Slack・週次レポート配信・Drive・メール・PL/PM/クローザー・ステータス・支払期日等を変更できます。
      </p>
