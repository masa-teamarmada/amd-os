...
function manualTopicHref(topicKey: string) {
  return `/manual?topic=${encodeURIComponent(topicKey)}`;
}
...
function selectedTopicFromUrl(
  ...
  chapterBySlug: Map<string, ManualNumberedChapter>,
) {
...
function SectionBlock({
  ...
  sectionChapters: ManualNumberedChapter[];
  topicByKey: Map<string, ManualTopicNodeConfig>;
...
function ManualGlobalToc({
  ...
  groups: { section: ManualSectionConfig; chapters: ManualNumberedChapter[] }[];
  activeChapterSlug?: string;
}) {
...
export function ManualMapClient({
    ...
    () =>
      MANUAL_SECTIONS.map((section) => ({
        ...
        chapters: section.slugs
          ...
          .filter((chapter): chapter is ManualNumberedChapter => chapter != null),
  ...
  function selectTopic(key: string) {
    if (!showDirectory) {
      router.push(manualTopicHref(key));
      return;
    }
