...
- **残る楽観バイアス (未解決)**: 将来月の `billing_cycles` 行を `budget_yen` 付きで作っても**売上原価は0円のまま**。原価の正本は `/admin/payouts` の capped 外部支払キャッシュ (`billing_cycles.reward_summary_json` → `externalYen`) であり `budget_yen` ではないため。2026-08-16時点で 2027-04〜2028-03 の p10/p19/p25 は台帳行 (計 685,454円/月) はあるが配分が無く、`cost_member` が0。試算表はこの分だけ粗利・現預金を多く見せている (キャッシュベースで月約754千円、12か月で約900万円)。
