...
- **対応内容**: 横枝は収まるときだけ中央、超えるときは左端始まりになるsafe alignmentへ変更し、ツリー自身の実内容幅を横移動枠へ渡した。目的構造には`partner_id`で明示接続したアプローチタスク内の関係先だけを出し、未接続の関係先は関係先タブに残す。
- **再発防止策**: `test:ui-design-code`でsafe alignment、実内容幅、`unlinkedPartners`描画の不在を固定する。狭いdesktopとmobileの実ブラウザで、最初のカードが切れないこと、未接続関係先が独立ノードにならないことを確認する。
