...
- **症状**: メンバー台帳の最終ログイン履歴が「誰の変更か」を示さず、変更前後は `+00:00` 付きのISO文字列だった。同じ自動更新が一件ずつ並び、raw table名・UUID・payloadと広い行のため、変更の意味を一覧で把握できなかった。
- **根本原因**: 履歴APIの対象解決がタスクとaction中心で、`members.id`から人間向けラベルを引く経路がなかった。field表示も値の型や業務上の意味を見ずに汎用文字列化し、自動処理の連続更新を一つの出来事として扱っていなかった。画面も詳細payloadを主役にした疎なレイアウトだった。
- **対応内容**: `members.id`を `member_name`、`code_name`、`member_id`へ解決し、メールアドレスなしで対象者を表示した。監査日時と日付型の値をJSTへ統一し、同一秒内だけミリ秒を残した。同じ自動処理・対象・fieldの連続した最終ログイン更新を一件へ集約し、一覧は対象・項目・変更前後を一行で読める約52pxの行へ圧縮した。
- **再発防止策**: 利用者向け監査履歴は `実行者 / 対象 / 項目 / 変更前 / 変更後 / JST日時` が意味として揃うことを受入条件にする。raw ID、raw ISO、payload全体は詳細へ退避する。対象tableごとの正本キーで実データを使ったpresentation testを行い、UTC offsetや`Z`が画面へ出ないことを自動検査する。

---

### [goal-tree/action-link] TODOを論点間で移せなかった (2026-09-16)

- **状態**: クローズ（`bf0a34c3`、production確認は `v3.140.4`）。
- **症状**: SOLの「愛媛大学名義の基本合意書のベース作成と学内承認確認」など、ゴールツリーに表示されるTODOに移動用のつまみがなく、別の論点へ付け替えられなかった。
- **根本原因**: ツリーのドラッグ操作は論点の親子関係を動かすものだけで、TODOと論点の多対多リンク（`project_question_actions`）を変更するAPI・DB関数・UIが存在しなかった。TODOを動かせないとする仕様や判断ログは無かった。
- **対応内容**: 最上位TODOのつまみ、複数論点に付くTODOの `移し替える` / `両方に残す` 選択、`move_project_action_question_link` を追加した。DB関数が同一PJ・最上位TODO・承認済みかつ開いている論点を検証し、親子・日程・担当・前後関係は更新しない。
- **再発防止策**: ツリーの見た目の親子関係と、TODOが答える論点の多対多リンクを別のデータとして扱う。操作できない状態を追加するときは、対象・理由・解除経路を仕様へ残す。UIのdisabledだけで制約を守らず、書込み関数でも同じ条件を検証する。
...
- **症状**: まさ「昨日100/100で使い切る事故が起きたから、二度と無駄にデプロイしまくることがないように対策をとってもらったはずだけど、どうしてまたそんなにデプロイしてるの？」。えいみが1セッションで docs だけの push を6回に分け、そのたびに amd-os-pwa のビルドが走っていた
- **原因**: 2つある。
  (1) `pwa/vercel.json` の `ignoreCommand` は 2026-08-30 15:13 の `76cd42d6` で「main は必ずビルド」へ戻っている（それ以前の「pwa に差分が無ければ飛ばす」判定が、ビルド環境の git で過去 commit と比較できず、本番を丸1日止めたため）。つまり **push 1回 = 必ず1件消費**が正しい挙動だった。
  (2) `check_deploy_quota.mjs` の警告文が「docs だけの変更は build されないので気にしなくてよい」のままで、**15:13 の変更に追随していなかった**。えいみはその文を読んで、押しても消費しないと判断した
- **対応内容**: 警告文を「main への push は内容によらず必ず1件消費する」へ直した（`e9eaa770`）。あわせて、**判定を Vercel 側ではなく手元でやる**仕組みを入れた（`pwa/scripts/deploy_skip.mjs`）——staged が「画面に出ないと確かめた場所」だけに収まっていれば `prepare-commit-msg` が件名へ `[skip ci]` を足し、Vercel はデプロイを作る前にそれを見る。`pre-push` は、いま本番に載っている地点と HEAD を比べ、画面に出る場所に差があるのに先頭 commit へ印が付いていたら止める
- **追記 (2026-08-30 20:07)**: **`[skip ci]` を commit メッセージへ入れるだけでは Vercel は止まらなかった。** 実測——件名に `[skip ci]` を付けた `e5c8b9d4` を push したところ、deployment が作られて BUILDING まで進んだ。この構成では Git 連携側のメッセージ判定が効いていない。そこで `pwa/vercel.json` の `ignoreCommand` で `VERCEL_GIT_COMMIT_MESSAGE`（無い環境では `git log -1` の HEAD 自身のメッセージ）を見て飛ばす形にした。**HEAD 自身のメッセージしか読まないので、2026-08-29 に壊れた「過去 commit との比較」には依存しない**（浅いクローンでも成立する）
- **実測 (2026-08-30 20:20)**: 仕組みを入れたあと、画面に出ない変更だけの push（`62af786e`）を1本流して確かめた。**deployment は作られたが state は CANCELED で、ビルドは走らなかった**（ignoreCommand が飛ばした）。**ただし、飛ばした分が 100/日 の上限に数えられるかどうかは手元からは確かめられない。** 直近24時間の内訳は 実ビルド46・中止/スキップ16 で、枠の計算は安全側（全部数える）のままにし、内訳を警告へ出して学習できるようにした
- **訂正 (2026-08-30 20:40・まさの指摘)**: この記録の初版で「きのう丸1日、反映が止まった」と書いたが**事実ではない**。Vercel の deployment 一覧を数えた結果はこう:
  - 2026-08-29 21:15（`04de79cf`）が amd-os-pwa の最後の成功。次の成功は 2026-08-30 14:24（`1d5a32bc`）
  - そのあいだ本番は約17時間古いままだったが、**うち14時間半は誰も押していない夜間**
  - 実際に「押したのに出なかった」のは **2026-08-30 12:20〜14:24 の約2時間・6回**（12:20 が ERROR、14:12〜14:17 の5件が誤った見送り）
  - **昨日（08-29）起きた事故はデプロイ上限の枯渇だけで、判定による反映停止は今日の出来事**

  えいみは `76cd42d6` と `1d5a32bc` の commit メッセージ（「止め続けた」と読める書き方）を裏取りせずに引き写した。**他人の結論を正本で裏取りする、というルールを自分で破った**（[[feedback_verify_subagent_output]] と同じ形）。数えれば5分で分かることだった。
- **訂正2 (同)**: 枠の内訳に出していた「中止/スキップ」は**2種類が混ざっている**。ignoreCommand で飛ばした分（`62af786e`）と、**後から来た押しに追い越されて自動で止まった分**（`007f463e` がこれ）。API からは区別できないので、ラベルを「中止（飛ばした分と追い越された分）」へ直した
- **再発防止策**: **押す前に、警告文ではなく設定そのものを見る。** 反映の要否を語る文言は、設定を変えた commit と同じ commit で直す（片方だけ変わると、次に読む人が事実と違う判断をする）。判定の一覧は「画面に出る場所」ではなく **「画面に出ないと確かめた場所」** の側で持つ——逆にすると、新しく md を画面へ出したときに足し忘れて、その文書だけ永久に反映されなくなる
...
#### 全PJへの展開 (2026-08-30 同日、まさ承認のうえ実施)

- **対象**: 会議カードの日付と一致するDriveの日付フォルダにあって、まだ `meeting_assets` に無かった123件 (SX78 / CX17 / KUTE13 / ZMP10 / SE3 / LiSTie2)
- **やったこと**: 74件を取り込み、49件を除外した。あわせてCLGとLiSTieの書面決議3回分 (資料8件) をカード化した
- **除外の内訳**: 会議と別の作業・事務ファイル22 / 同じ資料の古い版13 / 同じ資料の別形式8 / 会議資料ではない画像5 / 会議前の準備下書き1
- **学び**: **`YYMMDD_件名` フォルダは、MTG資料置き場と成果物置き場の両方に使われている。**日付一致だけを根拠に機械的に取り込むと、履行完了報告書の作業ファイル、外来研究員登録の様式、懇親会の写真、旧版の中間ファイルがMTGカードに並ぶ。adopt route は候補を出すところまでで、その会議の資料かどうかの判定は人 (またはLLM) が1件ずつやる。判定基準は spec 3-3「日付が一致するだけのフォルダを取り込まない」に落とした
- **書面決議**: 会議を開かない取締役会決議 (会社法370条) は `meeting_id='written-resolution:<pj>:<YYYYMMDD>'`、`meeting_start_at=null` で開催済みカードと同じ場所に置く。日付はDriveのフォルダ名ではなく提案書・同意書の日付を採る (LiSTieの `260628_取締役会書面決議` は中身が6月29日付だった)
...
- **症状**: `pwa/AGENTS.md` が何度戻しても `M` のまま残る。`deploy.sh` は tracked に未コミット変更があると hard-stop するため、**別セッションの deploy まで止まる**。実際にこのセッションでは45分待っても解消せず、待機ジョブが空振りした
- **原因**: AMD が書いた注記（「共通ルールは `~/.claude/CLAUDE.md` 経由で自動で読まれる」の3行）が、`<!-- BEGIN:nextjs-agent-rules -->` 〜 `<!-- END:nextjs-agent-rules -->` の**内側**に置かれていた。`next dev` は `generate-agent-files.js` の `upsertFile` でマーカーの間だけを毎回置換するので、AMD の注記が書き戻しのたびに消える。誰かが戻す → `next dev` が消す、の往復で永久に dirty
- **対応内容**: 注記をマーカーの外へ移し、`pwa/AGENTS.md` 自体を殻（ルートの `AGENTS.md` への導線だけ）にした。同じ構造の `AMD/kagawa/psi-step2-review/AGENTS.md` も外側へ入れた
- **再発防止策**: **`BEGIN:nextjs-agent-rules` 〜 `END` の内側には何も書かない。** 自動生成ブロックを持つファイルに追記するときは、マーカーの外であることを機械的に確認する（`t.index(追記内容) > t.index("<!-- END:")`）。tree が理由不明に dirty のままなら、まず「そのファイルにツールが書き戻すブロックが無いか」を疑う

---

### [process/verification] 「認証が要るから画面を確認できない」と誤って報告した (2026-08-29)

- **状態**: クローズ (2026-08-29 — まさの指摘で Chrome MCP を使って実見し、表示バグ3件を検出)
- **症状**: コスト試算タブをデプロイしたあと「認証が要る画面なので、レンダリング後の見た目はまさのログインでしか見られない」と3回報告した。まさ「これは嘘。いつも外部ブラウザでチェックしてくれてるじゃん」
- **原因**: ブラウザ系のツールが2系統あることを取り違えた。`mcp__Claude_Browser__*`（アプリ内ブラウザ、ログインセッションを持たない）でログイン画面が出たのを見て、「確認できない」と結論した。実際には `mcp__claude-in-chrome__*`（まさの実 Chrome、既存セッションあり）を使えば見られる
- **対応内容**: Chrome 側で実見し、literal な `\n` の表示 / `**強調**` の未レンダリング / 確度タグ149個中10個の2行折返し、の3件を検出して修正した。「右端が切れている」はスクショ取得幅の制約による誤検知だと実測（`scrollWidth - clientWidth = 0`）で切り分けた
- **再発防止策**: **ログインが要る画面の確認は `mcp__claude-in-chrome__*`（実 Chrome）を使う。** アプリ内ブラウザでログイン画面が出ても「確認できない」と結論しない。「見られない」と報告する前に、もう一方のブラウザ系統を試したかを必ず確認する。スクショが空白・見切れのときは、DOM を直接叩いて（`elementFromPoint` / `scrollWidth`）実在と寸法を確かめてから、レイアウトバグかキャプチャの制約かを分ける
...
## 2026-08-27〜28 ホームの重なり・見込みPJの誤設計・正本の誤引用

### 1. sticky な右カラムが、下段の全幅セクションを最後まで覆い隠していた

- **症状**: ホーム最下段の「会社の記録」が4カラム（メンバー / 沿革 / メディア掲載 / photo）で表示されるが、**右端の写真列がまるごと読めない**。まさ「４カラムになっててメディア掲載と写真とマイページが被ってカオスになってる」。
- **原因**: `position: sticky` の可動域は「その要素のグリッド行」ではなく **grid コンテナ全体**。ホームの2カラム grid（左=判断領域 / 右=マイページ埋め込み）の中へ、会社の記録を `xl:col-span-2` で3行目に置いていたため、sticky な右カラムが grid の下端まで貼り付き続け、会社の記録の右400pxへ被さっていた。sticky は positioned なので、後続の static 要素より前面に描画される。実測（幅1646px、下端）: aside が top=8 / bottom=818 / left=1230 / right=1620、company-content が top=734 / bottom=818 / left=272 / right=1620。マイページ埋め込みのルートに `bg-[#f5f5f7]` が付いているため、重なった部分は「読みにくい」ではなく**完全に見えなくなる**。
- **対応内容**: 会社の記録を grid の外の兄弟へ移した（`xl:col-span-2` を外し、親に `space-y-4`）。grid の高さが左カラムの高さで終わるので、sticky の可動域が構造として左カラム分に閉じる。z-index も transform も足していない。
- **再発防止策**: **2カラム grid の中へ全幅要素を置かない。全幅で敷きたい節は grid の外に兄弟として置く。** `scripts/check_portfolio_home_contract.mjs` に `assert.doesNotMatch(dashboard, /col-span-2/)` を打った。マークアップにも「この grid の高さが右カラムの sticky が止まる位置になる」とコメントを残した。

### 2. 「PJ化検討中だけ」の絞り込みで、正式にPJ化した研究機関が1件も出なくなっていた

- **症状**: ホーム上段の「研究機関PJ」に香川大学（まだPJでない）1件しか出ない。まさ「研究機関PJはKUTEだけじゃなくてEHMも正式にp30として立ち上がってるよ。手動で表示させるのではなく、どうしてここに自動的に表示されないのか、原因を特定してほしい」。
- **原因**: データ欠落ではなくフィルタ条件。パネルが `row.lifecycle === "considering"`（PJ化検討中）だけを拾っていた。`institution_projects` には p25 KUTE / p28 NIMS / p30 EHM が正しく登録されていて3件とも `projects.status='active'`、つまり `institutionProjectLifecycle` は `realized` を返す。**PJになった瞬間にこのパネルから消える**作りだった。同じ理由でシーズ側も稼働中PJに紐づく7件が全部落ちていた。
- **対応内容**: 両パネルとも PJ化済み + PJ化検討中を出すようにした。ただし `realized` は `active` / `ended` / `frozen` を含むので、そのままだと終了PJが「今日動かす対象」に並ぶ。稼働中かどうかは紐づくPJの `status === 'active'` で判定する（研究機関は `row.projectLink?.projectStatus`、シーズは `seed.project_links[].project_status`）。
- **再発防止策**: **リストの絞り込み条件が、そのリストの目的と食い違っていないかを確かめる。** 「〜PJリスト」と名乗る面が「PJになったら消える」条件を持っていたら、それは条件の側が間違っている。guard に稼働中判定の2本を追加した。

### 3. まさが答えていない問いの承認として、正本に「まさ確定」と書かれていた

- **症状**: `manual/4-5` に、見込みPJ3本（p32 NewCo業務委託 / p33 CX再開 / p34 PSI Step2新規）を独立PJとして立てたことが「まさ確定 (2026-08-17) の前提」として記録されていた。まさ「これはメンバーに支払うかどうか？という質問に対して、支払う余裕がないからおれ１人で対応すると答えただけ。SXのPJと別のPJにしていいとは一言も言ってない」。
- **原因**: まさの発言「Aだな。多分おれ1人で対応するし。メンバーに払う余裕がない」は**原価0（メンバーに支払わない）**への回答だったが、正本ではその発言を引用したまま「3本を別PJとして立てたこと」まで含む前提の根拠として書かれていた。発言だけを引き、何を聞かれて答えたのかを残していなかった。
- **対応内容**: 正本を訂正（原価0だけがまさ確定、PJ分割は確定ではない、と明記）。そのうえで3本を親PJへ統合した（p32→p21 SX、p33→p20 CX）。p34 はどのPJになるかが未定なので、PJではなく `company_budget_inputs` の臨時収入（spot）へ移した。
- **再発防止策**: **「まさ確定」と正本に書くときは、まさが実際に答えた問いの範囲で書く。** 発言を引くなら、何を聞かれて答えたのかも一緒に残す。承認の範囲を超えて設計判断の根拠に流用しない。

### 4. 将来の原価を自動で伸ばす仕組みが、契約期間を延ばしただけで発動した

- **症状**: 見込みPJを親PJへ統合した際、2027-04〜2028-03 の12か月で外部メンバー支払が月137,280円 → 223,322円（月+86,042円、年間約103万円）に増えた。まさ確定「1人で対応する＝原価0」と食い違う。
- **原因**: `live-monthly-pl-inputs.ts` の `carryForwardExternalByPj`（契約が続くのに報酬プランが無い月へ、直近3ヶ月の外部支払中央値を伸ばす仕組み）。統合のために SX の契約終了月（`end_ym=202703`）を外したことで、それ以降も `isProjectBudgetActive` が true になり、SX の過去実績が横引きされた。
- **対応内容**: まさ確定「この仕組みをやめたらいいんじゃないかな。やめて額が変わっちゃうところは、個別でデータを入れて変わらないようにすればいい」で横引きを撤去。売上原価は `billing_cycles.reward_summary_json` の値だけを使う。撤去で額が変わる唯一のPJだった p19 ZMP の 202701〜202803（月137,280円）は、同額を `manualForecast` 付きで台帳へ明示した。
- **再発防止策**: **将来の数字を自動で伸ばす仕組みは、別の変更の副作用で動く。** 見込みは台帳へ明示的に入れる（入れなければ0）。移行の前後で会社全体の月別の売上と原価を突き合わせ、1円も動かないことを確かめてから閉じる（`scripts/pj_merge_snapshot.mts`）。

### 5. 背面タブでは交差の監視が動かないので、遅延読み込みの不具合と誤診しかけた

- **症状**: 本番のホームで「会社の記録を読み込み中…」が消えない。ページ自身が `/api/dashboard/company-content` を1回も呼んでいなかった。
- **原因**: Chrome MCP はタブを背景で操作するため `document.visibilityState === 'hidden'`。この状態では `IntersectionObserver` が発火しない。自前の observer を張っても3秒で timeout することで確認した。実装ではなく検証環境の制約だった。
- **対応内容**: 誤診の手前で気づき、revert しなかった。あわせて、交差の監視だけに任せず 2.5秒の時間切れを併走させる形へ変えた（月初合意ゲートが出ていて本文をスクロールできないときにも読み込むため）。
- **再発防止策**: **交差の監視・`requestAnimationFrame`・自動再生に依存する挙動を、背面タブの観測結果で判定しない。** `document.visibilityState` を先に確認する。既知の教訓（2026-08-08 の背面タブ hydration 誤診）と同じ罠。

## 2026-08-29 マニュアルのつくよみ Q&A が、何を聞いても「該当箇所を見つけきれなかった」

### 1. 日本語の質問文を丸ごと1語として検索していたので、全42章が0件だった

- **症状**: `/manual` 右下の Manual Q&A が、どの質問にも「マニュアル内では該当箇所を見つけきれなかったよ」と返す。参照章リンクも出ない。まさ「全然機能しない。何を聞いても見つからん、わからんって言われる」。
- **原因**: `manual-search.ts` の `queryTerms` が日本語の切れ目を**助詞の位置だけ**（`[はがをにでとへもやのか]`）で判定していた。助詞を挟まない口語だと文全体が1語になる。「支払フローってどこにある？」の検索語は `支払フローってどこにある？` と `支払フローってどこ` の2つで、本文のどこにも部分一致しない。`searchManualDocuments` は `score > 0` で絞るため42章すべてが落ち、`context` が空のまま Gemini へ渡っていた。プロンプトは「根拠が足りない場合は見つけきれなかったと言う」と指示しているので、**モデルは指示どおりに答えていた**。旧コードで口語の質問15問を測ったところ**15問すべてが0件**。
- **対応内容**: ひらがなを区切りにして漢字・カタカナ・英数のかたまりを検索語にする `contentWordTerms` と、章タイトル・見出し・画面名・テーブル名を辞書にして質問文から逆引きする `dictionaryTerms` を追加。あわせて `/api/manual/tsukuyomi/ask` が検索0件でも全章の索引を必ず渡すようにし、「見つけきれなかった」と言う条件を「索引にも無いときだけ」へ狭めた。
- **再発防止策**: **日本語を空白と助詞で切る検索は、口語の質問文で必ず 0 件になる。** ひらがなを区切りにするか、正本側の語彙を辞書にして逆引きする。`scripts/check_manual_search_japanese.mts`（`npm run test:manual-search-japanese`）で口語7問の想定章が上位3件に入ることを固定した。旧コードでこのテストが落ちることも確認済み。

### 2. LLM の「分かりません」を、LLM の性能問題として見てしまう

- **症状**: 画面の見え方は「BOT が賢くない」。実際に壊れていたのは、そのBOTへ渡す材料を選ぶ検索の側で、Gemini は毎回 `(関連するマニュアル本文を特定できなかった)` という文脈を受け取って、プロンプトの指示どおりに正しく答えていた。
- **原因**: 検索が 0 件でも呼び出しは HTTP 200 で成功し、ログにもエラーが出ない。**失敗が失敗の形をしていなかった。**
- **対応内容**: 検索0件でも索引を渡す形にしたので、少なくとも「どの章か」は答えが出る。
- **再発防止策**: **LLM が「分からない」と答えたら、モデルより先に、渡している文脈が空でないかを見る。** 検索の件数を実データで数えるのが最短（今回は `searchManualDocuments` を実 md で走らせて 15問全滅を確認した）。文脈が空のまま生成へ進む経路は、静かに成功するので気づけない。

## 2026-08-29 排液の成分バッジが、閉じたプルダウンへのキー入力で黙って増えていた

### 1. `autoFocus` した `<select>` + 外側クリックの自動保存 = 気づけないデータ混入

- **症状**: 関係先リストの排液セルを開くと、プルダウン「成分を追加…」に太い青枠が付いた状態で開く。その状態で何かキーを打つと、`<select>` の先頭一致でその文字から始まる成分が選ばれてバッジが1つ増える。バッジが増えたことに気づかないまま編集面の外を押すと、`InlineCellEditor` は「変更があれば保存して閉じる」ので**そのまま保存される**。矢印キーでも同じ。
- **原因**: 開いた瞬間の入り口として `<select autoFocus>` を置いていた。閉じた `<select>` にフォーカスがあると、キー入力は「文字の入力」ではなく「選択肢の選択」になる。編集面の自動保存と組み合わさると、利用者の操作意図なしにDBが書き変わる経路になっていた。UIUX監査で指摘されるまで気づいていない（本番で実際に混入したかは**未確認**）。
- **対応内容**: `autoFocus` を外し、開いた直後のフォーカスは編集面のpanel本体（`tabIndex={-1}`）へ当てる。Escで閉じる動作とタブ順は変わらない。
- **再発防止策**: **閉じた `<select>` を編集面の初期フォーカスにしない。** 自動保存する編集面ではとくに危険で、「フォーカスを置く=値を変えうる」になる。初期フォーカスは、値を持たない入れ物（panel本体）か、打鍵がそのまま文字になるテキスト入力に限る。

### 2. 文章をそのまま保存する列を、あとから構造化した表示で読んでいた

- **症状**: 排液の成分バッジが「固体製品工場はリン負荷」のような文の断片になっていた。プルダウンの語彙と1つも対応しない。
- **原因**: `effluent_components` は議事録の表現をそのまま残す自由文の列として作った（2026-08-06）。あとから同じ列をカンマ区切りの構造化データとして読むバッジ表示を足した（2026-08-08）ので、旧データは読点の位置で機械的に切られた。表示側の分岐（`token.length <= 16` なら構造化とみなす）で逃げていたため、**データはずっと直らないまま**残った。
- **対応内容**: migration 336 で、語彙の成分と本文をDB側で分離（本文は新設の `effluent_note` へ全文を移す）。表示側の「作文かどうか」の分岐は廃止した。
- **再発防止策**: **自由文の列を構造化して読み始めるときは、同じ作業単位でデータも移す。** 表示側の分岐で旧形式を受け流すと、その分岐が半永久に残り、データは一生直らない。移行のUPDATE文は、表示に使うのと同じ判定関数を走らせて生成する（正規表現をSQLへ書き直すと「亜鉛」と「鉛」のような取り違えが起きる）。

## 2026-08-28〜29 人・組織の話が、行き場が無いまま経営ハイライトへ流れ込んでいた

### 1. モデルが定義している観測を、OS が受け取る場所を持っていなかった

- **症状**: SX の経営ハイライトに「杉浦先生の対人の壁が事業会社との商談に出ている」が並んでいた。
  まさ「これどうみても経営ハイライトじゃなくない？」。DB を見ると CX・ORB・SE にも同性質の行があり、
  いずれも 8/26〜28 に `created_by='amie'` で入っていた。
- **原因**: 経営ハイライトは日付を持つ事象の棚なのに、人の性質・組織の状態を入れる棚が OS に無かった。
  モデル台帳は §6.B（八機能と充足の判定）・§6.C（観測の登録簿）で**受け皿を定義済み**だったが、
  それを入れるテーブルも画面も作られていなかった。**定義があることと、OSが受け取れることは別**。
- **対応**: `project_org_observations` とスコア詳細タブの《組織》を作り、4PJぶんを移設。
  spec 3-6 に判定表、L9 抽出 SKILL に除外規定を入れた。
- **教訓**: **まさが口頭で話した内容を、置き場所を決めずに既存の棚へ入れない。**
  「この情報はどの棚のものか」を先に決める。決まらないなら棚を作るところまでが仕事で、
  近い棚へ入れて済ませると、その棚の定義が壊れる。
  モデル正本に定義がある観測は、OS 側に受け取る場所があるかを実際に探してから書く。

### 2. 記録が無いことを「空席」と出しかけた

- **症状**: 《組織》の最初の実装で、八機能すべてが「空席」と表示された。SX は実際には
  まさが BizDev、石原先生が大学調整、AMD3名が事業計画・知財・納品を担っていて、実態と真逆。
- **原因**: 充足の判定を実働の記録だけで行うのは正しいが、記録がまだ1件しか無い状態で
  「記録が無い ＝ 空席」と出していた。正本 §6.C-3 の3 は
  「記録密度が薄い案件は記録薄フラグを併記し、充足の失効判定を保留する。
  記録の薄さを案件の悪材料と混同しない」と明示している。**読んでいたのに実装に落ちていなかった。**
- **対応**: 直近6か月の記録が1件以下なら状態を「未記帳」にし、空席と区別。表の上に断りを出す。
- **教訓**: 判定の本体（充足の条件）だけを実装して、**正本が併記している例外規定を落とさない**。
  正本の節を実装するときは、その節が参照している別の節（ここでは §6.B-2 → §6.C-3）まで追う。
  「データが無い」を「値が悪い」として表示すると、画面が実態と逆のことを言う。

### 3. スコアを計算し直すコマンドが、書いてある使い方だと動かなかった

- **症状**: `node model/tools/bzm30_score_seeds.cjs <seed_id>` が `seed_id を渡す（--list で一覧）` で落ちる。
  spec 4-8 §7.3 と提案 md にこの呼び方が載っているのに動かない。
- **原因**: `const seedId = args.find((a, i) => !a.startsWith('--') && i !== implIdx + 1)` で、
  `--impl` を渡していないとき `implIdx = -1` なので `implIdx + 1 = 0` になり、**第1引数が常に除外される**。
  `--impl <path>` を明示するか、フラグを先に置く呼び方でしか動かなかった。
- **対応**: `(implIdx < 0 || i !== implIdx + 1)` へ。計算式には触れていない。
- **教訓**: **仕様書に書いてある呼び方を、一度も実行しないまま仕様書へ書かない。**
  「見つからないインデックス」を表す `-1` に算術を足すと、正当な位置を指してしまう。

### 4. 同じ人が、姓だけとフルネームで二重に登録されていた

- **症状**: SX のメンバー一覧に「石原」（愛媛大学・事業アドバイザ）と「石原 裕香」（産連メンバー）が別行で並んだ。
  まさ「同一人物なので揃えて」。
- **原因**: 議事録からの自動抽出が、重複防止のために既存名を LLM へ渡していたが、
  渡していたのは `project_founding_members` の名前だけ。**SU側の登録（`project_venture_members`）を見ていない**ので、
  SU側にフルネームで登録済みの人が、議事録の「石原先生」から姓だけの別行として作られた。
- **対応**: データを揃え（全PJ走査で該当はこの1件）、抽出へ `project_venture_members` の名前も既知として渡し、
  プロンプトへ「姓だけで出てきても既知のフルネームで返す」を追加。
  画面側は推測で寄せない（別人を混ぜるため）。役割が違う場合だけ `／` で併記する。
- **教訓**: 重複防止のために既存を参照するとき、**同じものを持っている別のテーブルを見落とさない**。
  1つのテーブルの中だけを見た重複防止は、テーブルをまたいだ二重登録を防げない。

---

## 2026-08-30: Vercel の ignoreCommand が PWA の本番反映を丸1日止めた

**症状**: 2026-08-29 21:15 のビルドを最後に、`pwa/` を含む main への push が本番へ反映されなくなった。
08-30 12:20 の push は `ERROR`、その後の複数の push は `CANCELED`（スキップ扱い）。
本番は `v3.100.2` のまま、画面上は何も壊れていないので気づきにくい。

**原因**: 2件が重なっていた。

1. `235b333f`（08-29 21:23）がデプロイ枠削減のため `pwa/vercel.json` に
   `git diff --quiet HEAD^ HEAD -- pwa ...` のパス限定 `ignoreCommand` を入れた。
   しかし Vercel は **clone → `.vercelignore` 適用 → ignoreStep** の順で走り、`.vercelignore` に
   `.git` が入っていたため git リポジトリごと消えていた。`git diff` が
   "Not a git repository" で **exit 129** になり、Vercel は `errorStep: ignoreStep` として
   deployment 全体を ERROR にする（スキップではなく失敗）。
2. 判定基準が `HEAD^` だと、pwa を変えた commit の後に docs だけの push が続いた場合、
   後続がすべて「pwa 差分なし」と判定して **pwa の変更が永久にビルドされない**。

`VERCEL_GIT_PREVIOUS_SHA` を基準にしても解決しなかった。ログへ値を出して
`prev=1d5a32bc head=5c5df534` と正しい2点が渡っていることを確認したが、それでも exit 0（差分なし）。
同じ2点をローカルで diff すると差分ありになる。**ビルド環境の浅いクローンでは過去commitとの
比較が成立しない**。

**解決策**: `76cd42d6` で pwa 側のパス判定を外し、
`if [ "$VERCEL_GIT_COMMIT_REF" = "main" ]; then exit 1; else exit 0; fi`（main は必ずビルド）に戻した。
`235b333f` 以前の、何ヶ月も問題なく動いていた挙動。あわせて `.vercelignore` から `.git` を外した
（`ignoreCommand` が git を使う設計である限り必要）。枠の2倍消費という本来の問題は
kiyo-amd-os 側の設定で解消しているので、そちらは触っていない。

**教訓**:
- **push が通った ≠ 反映された。** `/api/build-info` の `git_sha` を見て、期待した commit が
  本番に入っているか確かめる。ビルドが `CANCELED` は正常なスキップ、`ERROR` なら ignoreStep を疑う。
  原因は Vercel API の `errorStep` / `errorMessage` に出る。
- **ビルド最適化の設定を入れたら、その直後に pwa を含む push を1回通して反映まで見届ける。**
  今回は設定投入の8分前が最後の成功ビルドで、以降誰も pwa を反映していなかったため丸1日気づかれなかった。
- 推測で直しを重ねない。3回作り直して外したあと、判定値をログへ出させて初めて原因が確定した。

---

## 2026-08-30: 報酬モデルを読まずに実装し、経営判断を逆へ倒す画面を作った

**症状**: PJ別 利益構造ダッシュボード（`/admin/project-profitability`）で、まさから11点の指摘を受けた。
うち3点は数字そのものが誤り、残りは指標の意味が経営実態と逆または無意味だった。

**原因**: `pwa/manual/7-1-reward-calc-spec.md`（報酬計算の正本）を先に読まずに実装した。
司令塔が渡した仕様書の前提（「`budget_yen` = 売上」「まさの `totalPay=0` = 持ち出し」）を
検証せずに受け入れたのも同じ根。

- `billing_cycles.budget_yen` は**請求額ではなく請求額の65%（配分原資）**。
  比率の分母にさらに `× 0.65` を掛けて**65%を二重適用**し、ZMPを2.78×、CXを1.67×と誤表示して誤警報を出した
  （正しくは 1.01× と 1.08× でどちらも枠内）。
- まさの `grossDueYen>0` かつ `totalPay=0` を「持ち出し警報」として赤で出した。**意味が逆**で、
  実際はまさがポイントを消化して外部への現金流出を抑えている望ましい状態。
- ポイント（MS pt / 稼働需要）を収益率の指標にした。cap按分で支出は必ず原資内に収まるので、
  ポイントの絶対値は会社の収支に効かない。**指標として無意味**。
- `tally_weekly_effort_entries` を使って「まさ依存度」を作った。このテーブルは**まさ専用**で、
  `member_id='ID001'` の行しか存在しない。CXでは まさ30.8時間の記録だけであきの行は0件なのに
  「まさが79%背負っている」と表示し、まさの実感（「ほとんどあきが稼働してくれてる」）と食い違った。
- OSに計算データが無いだけの状態を「未配分 ¥275万」と**お金が残っているかのように表示**した。
  実際は支払い済みで、計算が入っていないだけ。

**解決策**: まさ判断で白紙から作り直し。引き継ぎに指摘11点を禁止事項として明記し、
実データで裏を取った事実も併記した（`SESSION_MIGRATION_PROMPT_PROJECT_PROFITABILITY_2026-08-30.md`、`c219a6eb`）。

**教訓**:
- **お金の集計画面を書く前に、計算正本を Read で全文通す。** grep で拾い読みしない。
  今回の誤りはすべて 7-1章に答えが書いてあった（`companyReserveYen` は35%の会社取り分ではなく
  65%枠内の非現金配賦、支払対象判定は `exclude_from_payout_notice` で `is_officer` ではない、など）。
- **指標を出したら、実態を知るまさに見せて違和感を聞く。** 「CXは残る率100%で優秀」と出した順位は、
  まさの一言（「CXはおれそんなに稼働してる？」）で崩れた。データが実態を持っているかは、
  データを見ても分からない。
- **OSにデータが無いことを、事実として語らない。** 「未配分」「報酬計算が動いていない」は
  OSの欠落であって、お金や業務の状態ではない。→ [[feedback_never_infer_external_state_from_os]] と同型。
- 司令塔から渡された仕様書の前提も、実データで裏を取ってから実装する。

---

## 2026-08-30: 別セッションが同じ機能を実装中なのに気づかず着手しかけた

**症状**: 未追跡ファイルとして対象ファイル一式が既に存在していた。前セッションの残骸に見えたが、
実際は**別セッションが秒単位で同じ機能を書いている最中**だった。

**原因**: 着手前の `git status` は「未追跡ファイルがある」としか教えない。誰がいつ書いたかは出ない。
`claude agents --json` と ccd のセッション一覧にも出なかった（前面セッションは取りこぼす）。

**解決策**: ファイルの更新時刻を見て、20秒あけてハッシュを取り直して変化を確認。
実装せず報告へ切り替え、相手が commit（`e40a5ec8`）して19分止まったのを確認してから引き取った。

**教訓**: **未追跡ファイルが既にある状態で作業を頼まれたら、書き始める前に更新時刻を見る。**
数分以内なら他セッションが実行中。`ls -lT` と 20〜30秒あけた `md5` で確定できる。
進行中を検知したら実装せず報告へ切り替える。読み取りだけの検算（DBで実データを当てて前提を確かめる）は
安全なので、そこまではやって材料として渡す。
## 2026-09-01: 主要成果物を複数社混在・別書式・Markdownで作成した

**症状**: SX 2026年8月の主要成果物として、日本食研とユナイテッドシルクを1件に混ぜた低密度のMarkdown資料を作成し、月次報告書と異なる配色・書式でPDF化した。月次報告書の主要成果物リンクも、会社別PDFではなく統合Markdownを指していた。

**原因**: 月次報告書の保存・リンク変更を、新規資料の自由なデザイン作業として扱った。数十社を横断比較するための単位が「1社」であること、主要成果物は提出可能なPDFであること、既存月次帳票のデザインコードが固定であることを、作成前の検査条件にしていなかった。
...
## [ultracode/workflow] 検証段が調査の後ろに溜まり、セッション上限で全滅した (2026-09-05)

- **症状**: 研究機関48件の調査→検証を `pipeline()` で組んだところ、60分経っても検証が1件も始まらず、そこへ Claude のセッション上限（20:10 JST 回復）が来て走行中の108エージェントが一斉に失敗した。調査は40/48件が終わっていたが、検証は0件、新規機関の調査も0件で終了した。
- **原因**: `pipeline()` は全件の第1段（調査）を先に投入し、第2段（検証）はその後ろのキューに並ぶ。1ワークフローの並列枠は `min(16, CPU-2)` = 8本で、調査1本が8〜10分かかるため、48件×2段＋新規候補で3時間規模の待ち行列になっていた。
- **対応内容**: 3本を一度止め、検証プロンプトだけ軽くして `resumeFromRunId` で再開したが、上限に当たって完走しなかった。完了済みの結果JSON・入力・内部資料・生成スクリプトを `pwa/output/institution_support_research_20260905/` へ退避し、次セッションが再開できるようにした。UI・DDL（migration 375/376）は本番反映済みで、データ投入だけが残っている。
- **再発防止策**: 調査→検証は機関を数件ずつに割った小さな `pipeline` を `parallel` で束ね、検証が調査に追従するようにする。並列枠が8本のときは1ワークフローに100本超のエージェントを積まない（別ワークフローへ分ける）。長時間ワークフローの結果は完了通知を待たず、途中で journal から退避スクリプトを回す。セッション上限の回復時刻を確認してから大規模ワークフローを起動する。

## [PWA/project-workspace] 目的構造の左端が欠け、未接続の関係先がタスクの背面へ重なった (2026-09-08)

- **症状**: 横に広い目的構造で最初のカードの左側へ戻れず、成立条件を開くと「関係先」カードが大量にタスクの背面へ重なった。
- **原因**: 横枝を`justify-content:center`で中央寄せしたため、表示枠を超えた分が左側の負の領域へはみ出した。また同じ柱に成立条件が1件だけなら、その柱の未接続関係先をすべて当該成立条件へ所属させ、タスクと同列へ描画していた。
- **対応内容**: 横枝は収まるときだけ中央、超えるときは左端始まりになるsafe alignmentへ変更し、ツリー自身の実内容幅を横移動枠へ渡した。さらに固定幅の親カードより広い子枝は、親の左端から右方向へ展開して負方向のoverflowを発生させない。目的構造には`partner_id`で明示接続したアプローチタスク内の関係先だけを出し、未接続の関係先は関係先タブに残す。
- **再発防止策**: `test:ui-design-code`でsafe alignment、実内容幅、子枝の右方向展開、`unlinkedPartners`描画の不在を固定する。狭いdesktopとmobileの実ブラウザで、成立条件を開いた後も最初のタスクが横移動の開始位置で読めること、未接続関係先が独立ノードにならないことを確認する。

## [handoff/並行セッション] 3日空けた再開で、別セッションの実装を上書きしかけた (2026-09-09)

- **症状**: 9/6に「daily自動化はこれから」と報告して中断したSlack取り込みを、9/9に同じ手順で作り直し始めた。`vercel.json` のcron 11本を1本へ潰し、既に本番稼働していた `cron/slack-source-sync` / `lib/sources/slack-project-collect.ts` / `lib/slack/workspace-token.ts` を上書きした。
- **原因**: 再開時に `git log` で対象領域の直近commitを見ないまま着手した。9/6 00:49 の `be85ea5b` で別セッションが daily 自動化・3チャンネル限定・つくよみ入れ直しまで完了させており、会話履歴の「未完了」という記憶が現物と食い違っていた。`vercel.json` は `json.load` → `append` → `json.dump` で書き戻したが、結果として既存10本が消えた。
- **対応内容**: `git status` で M を見て気づき、`git checkout` で4ファイルと `vercel.json` を戻した。push前だったので本番への影響はない。その後は正規checkoutを触らず、使い捨てクリーンクローンから作業した。
- **再発防止策**: 中断したタスクを再開するときは、着手前に `git fetch` に加えて **対象ディレクトリの `git log --oneline -5`** を読む。会話の記憶ではなく現物を正とする。設定ファイルを機械的に書き戻すときは、書き込み後に必ず件数を読み返す（今回は「crons: 1」を出力していたのに、その値を異常として扱わなかった）。

## [PWA/slack] Slack取り込みの表示で、会話でないものが会話として並んだ (2026-09-09)

- **症状**: コックピットのSlackタブに「@Uxxxさんがチャンネルに参加しました」が発言として並び、スレッド返信が「親の中の返信」と「独立した発言」の両方に出て二重になった。絵文字は `:blush:` のまま、ファイルだけの投稿は「（本文なし）」と表示された。
- **原因**: 取り込みが `subtype`（`channel_join` 等のシステムメッセージ）を除外していなかった。またスレッド返信は `conversations.replies` で取った分も `byTs` に入るため、タイムラインにも独立行として残っていた。表示側はSlackの絵文字コードとファイル専用投稿を想定していなかった。
- **対応内容**: 取り込み側で `SYSTEM_SUBTYPES` を除外し、表示側で「親が同じ月にいる返信」をタイムラインから外した（親が無い返信は行き場が無くなるので残す）。絵文字は主要60個を変換し、未知のコードは `:name:` のまま残す。ファイルだけの投稿は「（本文なし）」を出さない。既にDBへ入っていた参加メッセージ16件は削除した。
- **再発防止策**: 外部サービスの生データを画面へ出すときは、**そのサービスで「会話」として表示されないものを除外してから**保存する。除外を後から足しても、既に保存された行には効かない（upsertでは消えない）ので、DB側の掃除とセットで行う。

## [運用/認証情報] ブラウザとローカルの間でトークンを受け渡せない (2026-09-09)

- **症状**: Slackアプリの再インストールで変わったBot Tokenを、Vercelの環境変数へ反映できなかった。ページのCopyボタン、`navigator.clipboard.writeText`、`cmd+c` / `cmd+v` のいずれもローカルの `pbpaste` へ届かず、ブラウザ内のタブ間ペーストも効かなかった。
- **原因**: Chrome拡張経由のキーイベントは実際のクリップボード操作を伴わず、`navigator.clipboard` はバックグラウンドタブでは system clipboard へ届かない。
- **対応内容**: まさに手でコピー&ペーストしてもらった（Slackの設定画面とVercelの環境変数編集画面を両方開いた状態で渡した）。値を会話へ貼れば自動化できたが、認証情報を会話ログへ残さない原則を優先した。
- **再発防止策**: トークンが変わる作業（スコープ追加・再インストール）を含む場合は、**最初からまさへコピペを依頼する前提で段取りを組む**。えいみ側は両方の画面を開いて、押す場所まで用意しておく。環境変数の変更は既存デプロイに効かないので、更新後に必ず再デプロイまで確認する。

## [ultracode/workflow] セッション上限で照合段だけが全滅し、調査結果は残った (2026-09-10)

- **症状**: 研究機関の調査ワークフロー2本（既存37機関の21項目、新規16機関の21項目）で、調査は全件終わったのに照合が 34/37 と 9/16 で失敗した。失敗理由はいずれも `You've hit your monthly spend limit`（リセットは 5:20 JST）。
- **原因**: 上限は走行中のエージェントを一斉に落とす。1機関あたり「調査→照合」の2段のうち、後段だけが上限に当たった。調査は完了していたが、`pipeline()` の返り値は最終段の結果なので、そのままでは調査結果を取り出せない形になりかけた。
- **対応内容**: 調査結果はワークフローの返り値（`rows[].result`）に残っていたので `pwa/output/institution_support_research_20260905/results/` へ退避し、**照合だけを流し直す専用ワークフロー**（`scripts/wfE.js`）を組んだ。主張は機関ごとの md（`claims/`）へ書き出してパスだけ渡し、照合エージェントに Read させる。43件を回収し、調査は1件も再実行していない。
- **再発防止策**: 調査と照合を1つの pipeline に閉じ込めても、**各段の結果を段ごとに退避できる形にしておく**。照合は「主張ファイル＋URL」だけで独立に走らせられるので、再実行の単位を機関×段にする。上限は起動前の確認では防げない（消費量は走らせてみないと分からない）ため、落ちても捨てずに済む形を先に用意する。あわせて `agent()` の `model` を省略しない（省略すると起動セッションの最上位モデルを継承する。同日の費用事故を参照）。

## [PWA/institutions] 別セッションが同じ推奨表を先に投入し、統計が古いまま本番に出た (2026-09-10)

- **症状**: 推奨20件（`institution_policy_recommendations`）の本文の統計が、画面が `cells` から自動計算して隣に出す件数と食い違っていた。同じ行の中でも矛盾していた（整備済み49%なのに「過半数」、未確認17と13の併記、根拠「未整備0機関」と統計補足「未整備2」）。
- **原因**: 同じ日に2つのセッションが同じ成果物（`results/wfRec_raw.json`）を投入しようとした。先に投入した側が、まだ適用されていない migration 389（53機関ぶんの21項目）の前に統計を数えたため、機関数が最大2件ずれた。加えてテンプレート一致の置換では、機関名を挙げている文（「未整備の東京大学、早稲田大学、九州大学は」）と、件数を前提にした形容（「過半数」「整備の有無が割れていない」）が直らなかった。
- **対応内容**: 行を作り直さず、migration 390 で16行の `rationale` / `stat_note` だけを現在の集計へ更新した。推奨・条件・代表例・論点・スタンスは変えていない。訂正はエージェントを使わず、DBの集計値でテンプレート置換し、機関名を挙げている文は実データ（`status='not_started'` の機関一覧）で書き直し、20件すべての数値を機械で突き合わせて一致を確認した。
- **再発防止策**: **本文へ数字を書く画面では、書いた時点のDB集計をその場で突き合わせてから投入する。** 画面が自動計算する欄がある場合、本文の数字はその計算式と同じ母集団（ここでは `institutions` 全件、未確認は分母に入れる）で数える。テンプレート置換で数字を直したら、機関名・割合の形容（過半数、大部分、割れていない）も併せて見直す。同じ成果物を複数セッションが投入し得るときは、着手前に `git fetch` して origin/main の migration 一覧を見る。

## [PWA/seeds] 既存の補助金履歴を確認せず同じ申請を二重登録した (2026-09-11)

- **症状**: 小金沢シーズ（関西大学）の「補助金履歴」に、2025年度KSAC申請の行が2件並んだ。提出日も「2025年10月31日」と「2025-11-13」で食い違って表示された。
- **原因**: `seed_funding` に既存行があるかを確認せずINSERTした。既存行は2026-08-25に入っていた。加えて提出日をメール（申請の最終形を受領した日）から推定して書いたため、既存行の実提出日とずれた。同じ日付の接触履歴も1件重複させた。
- **対応内容**: 後から入れた行を削除し、既存行の `notes` へ書面審査期間・不採択通知日（2025-12-25）・再申請方針を統合した。接触履歴の重複1件も削除した。
- **再発防止策**: `seed_funding` / `seed_contact_log` / `project_grants` のような追記型の履歴テーブルへ書く前に、同じ seed_id / project_id の既存行を必ずSELECTする。日付は推定で書かず、確定できないものは本文ではなく notes に経緯として残す。

## [PWA/question-tree] 単独タスクの却下が採否APIへ届かず失敗した (2026-09-16)

- **症状**: コックピットのタスクタブで、論点・仮説に紐づかない「承認待ちのタスク」を却下すると、画面に `操作対象の種類が不正です` が表示された。
- **原因**: 画面は `proposal_bulk` の採否を `PATCH` で送っていたが、このリソースを受け取るのは `POST` のみだった。`PATCH` は通常の単一リソース更新へ落ち、種類の検証で拒否された。
- **対応内容**: `POST { resource: "proposal_bulk", decision, ids }` に統一した。承認は単独タスクとして残し、却下は論理削除する。画面・API・仕様の契約を `test:question-tree-task-review` で固定した。
- **再発防止策**: 同じ機能の操作経路とAPI methodを分けて実装しない。UI actionを増やすときは、method・resource・状態遷移を仕様へ明記し、画面とrouteをまたぐ契約検査を追加する。実データを消す操作の本番確認は、対象と復元手順を先に確定する。


### [SOL/gantt] 論点展開・資料風UI・名前列のはみ出し (2026-09-16)
- 状態: 修正済み（4dfb771d / b0c2d67a / 11db2794）。
- 症状: 展開すると論点が並ぶ、子期間が親へ正しく集約されない、名前が左へ突出、資料風の見た目に変化。工程へ子タスクを作る導線もなかった。
- 原因: 資料の工程内容と視覚様式を混同し、分類用の論点を日程行へ投影した。短期間ラベルのabsolute外付けと開閉による列幅変更がはみ出しを起こした。
- 対応: MS/タスクのみ展開、子期間の再帰集計、420px/狭幅300pxの固定名前列と列内折返し、既存配色・行構造へ戻した。工程単位の作成・既存紐付けと共有タスクの明示所属を追加。
- 教訓: 参照資料の内容とUIデザインを別に扱う。親子の日程意味を先に固定し、長文・短期間・狭幅・開閉前後で境界を測る。実装者レビューを独立監査と呼ばない。
- 検証上の注意: 日付の自動fillはDOM表示だけ変化しReactへ反映されない場合がある。実キー入力と保存結果で確認する。IABのnative日付pickerがクラッシュしたためChromeで検証。UI文言は中立的な業務文言にしcritical UI gateを通す。

## [支払通知書/立替] 旧月の印が残り、未払い立替が9月PDFから欠落 (2026-09-25)

- 症状: かるの2026年9月通知書で、承認済みのSX立替2件・計82,500円が0円。報酬明細は「4〜6月稼働分」となり、3か月分を全額払うようにも読めた。
- 原因: 支払通知書の生成時に `reimbursements.billed_ym=202608` を書き、別月での採用を排除していた。8月通知書が後で消えても印だけ残った。`billed_ym` は取引先への請求月の帰属にも使われ、メンバー支払月と意味が衝突していた。9月通知書は未送付・立替0円、8月通知書は現存しない。
- 対応: 現存する `payout_notices.reimbursement_ids` を採用済み正本に変更。通知書が無ければ締切に合う支払月へ自動で戻し、別月の通知書にあれば二重計上しない。PDF差分・送付前照合に立替額と明細IDを加え、`billed_ym` への支払側書込みを削除。複数月の繰越支払は「発生分の一部」と表記する。
- 検査: 8月の印だけ残る2件が9月へ計上されること、実際に別月の通知書に載る立替が再計上されないこと、締切超過と明細ID差分を `test:payout-reimbursements` / `test:payout-source-span` で固定。
