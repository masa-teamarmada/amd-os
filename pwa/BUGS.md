...
- **対応内容**: 先12か月表を `キャッシュ支払` / `会社留保` / `報酬債務` / `cap超過チェック` の4表へ分解。`computeForwardCappedMemberCosts` は外部支払、会社留保、gross due、carry over を分けて返す。報酬債務表は12か月合計を出さず、各月残・ピーク・最終月残で読む。
