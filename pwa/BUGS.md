...
- **対応内容**: 旧 `amd-os-l2` を PAUSED に戻した。primary writer を月末の `amd-os-l2m1-monthly-report` に統一し、内部版プロンプトを従来の固定8見出しナラティブへ戻した。`ms_progress_review_tool.mjs` は draft/final の保存前に、見出し順・重複・生ログ行・概要への処理メタ混入・句読点崩れ・途中省略を検査し、`eLAD` を `e-Rad` へ正規化する。提出版は別 helper で `monthly_reports_external.body_md` へ保存し、氏名を姓だけへ正規化する。
