...
- **状態**: クローズ (2026-07-03 — `v0.38.11` で背景クリック dismissal を削除し、合意保存後だけ閉じる形へ修正)。
...
- **再発防止策**: 月初合意 gate のモーダルに、背景クリック・Esc・閉じるボタンなどの「後で見る」導線を入れない。表示対象PJがあり status が未合意/条件更新ありなら、合意保存または対象外化まで通常画面へ進ませない。
