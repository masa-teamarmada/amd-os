...
Default is production deploy. --dry-run runs the approval-free rollback guard
and build stamp preparation only; it never calls Vercel.
EOF
      exit 0
      ;;
    *)
...
if [ -z "$BUILD_STAMP_GIT_BRANCH" ]; then
  BUILD_STAMP_GIT_BRANCH=$(git name-rev --name-only HEAD 2>/dev/null || echo "detached")
fi
