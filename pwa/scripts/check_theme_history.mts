...
const hub = fs.readFileSync("src/components/project-workspace/ProjectThemeRoutes.tsx", "utf8");
assert.ok(!hub.includes('`/workspace-document/'), "テーマの資料をMarkdown専用URLへ送らない");
assert.equal((hub.match(/\/open\?download=0/g) ?? []).length, 4, "資料・成果物・関連・出典は既存の形式別open経路へ送る");
const cockpit = fs.readFileSync("src/components/cockpit/CockpitView.tsx", "utf8");
const cockpitPage = fs.readFileSync("src/app/(app)/project/[projectId]/cockpit/page.tsx", "utf8");
assert.ok(!cockpit.includes('key: "themes"'), "テーマはPJコックピットのタブへ戻さない");
assert.ok(!cockpitPage.includes('tab=themes'), "テーマのコックピットURL導線を受け付けない");
const themeCss = fs.readFileSync("src/components/project-workspace/theme-history.module.css", "utf8");
assert.ok(themeCss.includes('.section .header button { min-height: 32px; height: 32px; }'));
