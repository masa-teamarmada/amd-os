-- 381_vc_investment_redirected_homepage_cleanup.sql
-- 個別記事URLが企業トップへリダイレクトされた1件を、根拠不十分として候補から除外する。
...
WITH dismissed_investment AS (
  ...
  WHERE collected_by_model = 'gemini-3.5-flash-lite'
    ...
    AND source_url IN ('https://www.kaicoltd.jp', 'https://www.kaicoltd.jp/')
