-- SOL（p21）: JMTCキャピタルを四国化成CVCの運営会社（GP）として表す。
...
SET
  ...
  connection_context = 'JMTC（日本材料技研）の100%子会社JMTCキャピタルは、四国化成CVC「SHIKOKUイノベーションファンド」の運営会社（GP）として、案件発掘、DD、投資判断・投資実行を担う。四国化成との投資委員会の構成や最終決裁手続は未確認。JMTCキャピタルは三菱マテリアルCVCも運営している',
...
SET
  detail_md = 'JMTCキャピタルは四国化成CVCの運営会社（GP）として、案件発掘、DD、投資判断・投資実行を担う。この案件で正式なDD・投資検討が始まったこと、出資判断、条件合意はいずれも未確認。四国化成との投資委員会の構成や最終決裁手続も未確認。主な確認事項は、色素分解・金属取り込みの仕組み、高温排水での違い、規模、コスト、金属の選択性。',
  updated_at = now()
