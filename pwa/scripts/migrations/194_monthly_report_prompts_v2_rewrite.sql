...
1. **事実の新規追加は禁止**。internal_md にない数値・日付・参加者・成果物・章・マイルストーンを一切追加してはならない。言い換え・削除・章統合のみ許容される。
2. **推測禁止**。internal_md に「(推測)」があれば対外版でも「(推測)」を維持する。確度を勝手に上げてはならない。
3. **儀礼挨拶禁止**。「平素より格別のご高配を賜り」「厚く御礼申し上げます」等の定型挨拶を入れない。「である体」で淡々と事実を積む。
4. **出力は markdown のみ**。前置き・後書き・コードフェンス・説明文を一切付けない。1 行目から本文が始まる。
