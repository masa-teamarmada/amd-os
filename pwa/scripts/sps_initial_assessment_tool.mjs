...
async function env() { const out = {}; for (const file of ["pwa/.env.production.local", "pwa/.env.local", ".env.production.local", ".env.local"]) { try { for (const line of (await fs.readFile(path.join(ROOT, file), "utf8")).split(/\r?\n/)) { const at = line.indexOf("="); if (at > 0 && !line.trim().startsWith("#")) out[line.slice(0, at).trim()] = line.slice(at + 1).trim().replace(/^['\"]|['\"]$/g, ""); } } catch {} } return { ...out, ...process.env }; }
...
function latestFactTime(facts) { const dates = [...(facts?.news ?? []).map((x) => x.occurred_on), ...(facts?.contacts ?? []).map((x) => x.contacted_on), ...(facts?.company_facts ?? []).map((x) => x.observed_on)].filter(Boolean).map((x) => Date.parse(`${x}T00:00:00.000Z`)).filter(Number.isFinite); return dates.length ? Math.max(...dates) : 0; }
...
async function pendingSeedIds(c) { const out = new Set(); for (let from = 0; ; from += 1000) { const { data, error } = await c.from("sps_initial_assessment_candidates").select("seed_id").eq("status", "pending").range(from, from + 999); if (error) throw new Error(error.message); for (const x of data ?? []) out.add(x.seed_id); if (!data || data.length < 1000) return out; } }
async function remediationTargets(c) { const { data, error } = await c.rpc("sps_initial_assessment_remediation_targets"); if (error) throw new Error(error.message); return (data ?? []).map((x) => ({ seed_id: x.seed_id, assessment_id: x.assessment_id })); }
async function status() {
  const c = await db();
  const ids = await seedIdList(c, null);
  const assessed = await assessedSeedIds(c, ids);
  const defective = await remediationTargets(c);
  console.log(JSON.stringify({ seeds: ids.length, assessed: assessed.size, remaining: ids.length - assessed.size, defective: defective.length }, null, 2));
}
...
async function prepare(a) {
  ...
  const remediate = a.remediate === true || String(a.remediate) === "true";
  const only = a["seed-id"] && a["seed-id"] !== true ? String(a["seed-id"]) : null;
  const supersedes = new Map();
  let seedIds;
  if (remediate) {
    const [targets, pending] = await Promise.all([remediationTargets(c), pendingSeedIds(c)]);
    const rows = targets.filter((x) => !pending.has(x.seed_id) && (!only || x.seed_id === only));
    seedIds = rows.slice(0, limit).map((x) => x.seed_id);
    for (const x of rows) supersedes.set(x.seed_id, x.assessment_id);
  } else {
    const assessed = await assessedSeedIds(c, ids);
    seedIds = ids.filter((id) => !assessed.has(id)).slice(0, limit);
  }
  const generatedAt = new Date().toISOString();
  const snapshots = await Promise.all(seedIds.map((id) => snapshot(c, id, generatedAt)));
  const inputs = seedIds.map((seed_id, i) => ({ seed_id, source_fingerprint: snapshots[i].fingerprint, source_facts: snapshots[i].facts, ...(remediate ? { supersedes_assessment_id: supersedes.get(seed_id) } : {}) }));
  const out = { version: 1, contract: CONTRACT, generated_at: generatedAt, prompt: currentPrompt, model: currentModel, q_factor_ids: Q_FACTORS, ...(remediate ? { mode: "remediation" } : {}), inputs }; out.prepared_hash = stableHash(out);
...
export function validateInitialPayload(payload, prepared) {
  ...
  const remediation = prepared.mode === "remediation";
  ...
  for (const [i, p] of (payload.proposals ?? []).entries()) {
    ...
    if (remediation && !UUID.test(String(input?.supersedes_assessment_id ?? ""))) errors.push(`${at}.prepared supersedes_assessment_id is invalid`);
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) { const x = parseArgs(process.argv.slice(2)); try { if (x.command === "status") await status();
  else if (x.command === "prepare") await prepare(x.args); else if (x.command === "validate") { const [p, r] = await Promise.all([readJson(x.args.file, "file"), readJson(x.args.prepared, "prepared")]); const errors = validateInitialPayload(p, r); console.log(JSON.stringify({ ok: !errors.length, errors }, null, 2)); if (errors.length) process.exitCode = 1; } else if (x.command === "submit") await submit(x.args); else if (x.command === "apply") await apply(x.args); else if (x.command === "reject") await reject(x.args); else console.log("status | prepare | validate | submit | apply | reject"); } catch (e) { console.error(e instanceof Error ? e.message : String(e)); process.exitCode = 1; } }
