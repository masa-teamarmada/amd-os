 ...
 *
 * ── なぜこの形なのか ────────────────────────────────────────────
 * 2026-08-29 21:23 に「pwa に差分が無ければビルドしない」という判定を Vercel 側（vercel.json の
 * ignoreCommand）へ入れたが、ビルド環境の git では過去の commit との比較が成立せず、
 * **押しても本番へ出ない状態が 2026-08-30 12:20〜14:24 のあいだ続いた**（1件がエラー、5件が
 * 誤った見送り。実測は Vercel の deployment 一覧）。2026-08-30 15:13 に外して
 * 「main は必ずビルド」へ戻っている（commit 76cd42d6）。
 *
 * **「丸1日止まった」ではない。** 76cd42d6 と 1d5a32bc の commit メッセージはそう読める書き方を
 * しているが、記録を数えると本番が古いままだった約17時間のうち14時間半は誰も押していない夜間で、
 * 実際に押して出なかったのは約2時間・6回である（まさの指摘 2026-08-30）。
 *
