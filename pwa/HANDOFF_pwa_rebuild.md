...
## 2026-09-01 BWE論点・仮説追加の保存復旧

- BWE (`p11`) は、共有ワークスペースが互換表示した標準4分類に対して、`project_management_tracks` の実行時登録が0件だった。画面は`business_development`を送る一方、保存APIは正規登録済み分類だけを受け付けるため、「trackが不正だよ」で追加不能になっていた。
- `20260901190000_bwe_management_tracks.sql`は、既存の画面表示と同じ事業開発／技術開発／資金調達／体制構築を追加するだけ。BWEの既存論点・MS・タスクはすべて0件だったことを適用前に確認済み。競合時も既存の分類を上書きしない。
- PWA画面・認可・モデル・Nativeクライアントに変更なし。migration適用後にBWEの4分類をreadbackし、論点追加APIが受け付ける分類集合と一致することを確認した。回帰は`npm run test:critical-ui`へ追加済み。
...
- 目的構造は任意深度で分岐し、相手へのアプローチ枝には接点履歴→現在のボール→次の行動を内包する。管理権限者はタスク／子タスクを手動追加し、カードのドラッグまたは`接続変更`で親を付け替える。循環参照はUI/API/DBで拒否する。
...
- `20260901223000_zmp_objective_branch_history.sql`も本番適用済み。供給元→シーズリストから堂脇先生／pHydrogen／その他探索へ3分岐し、taskのnullable `partner_id`で関係先正本へ接続。堂脇先生はコンタクト→MTG→継続→レスなしで一旦停止の4イベント。
...
- 今期の再編内容は `/Users/masa/projects/AMD/kute/docs/FY2026_TASK_REVIEW.md`。契約期間は5/1〜翌3/31。2026-09-01に旧6工程を6区分38タスクへ置換し、根拠を確認できた9件だけ完了へ更新した。日程根拠のない行は未設定。Before Zeroの未承認システム設計やファンド組成は採用していない。
...
- PWAのKUTE (`p25`) だけ、今期業務を6区分38件で既存ガントへ表示する。旧6件は編集・依存関係がないことを確認してsoft-delete。作成完了と決裁、調査完了と受注を分け、9件だけ完了。連携シーズ比較は「シーズ」タブのまま。
...
- 同期棚卸し: KUTE今期タスクの再編とシーズタブは上記設計・マニュアルへ反映済み。共通DBはdata-only migration `20260901184500_kute_fy2026_task_rebuild.sql` を適用済み。新スキーマ・理論・外部共有の権限は変更なし。既存のAMDメンバー向けワークスペースも同じ計画を読む。
