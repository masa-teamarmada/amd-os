...
- `origin/main` とlocal `main` は一致（ahead 0 / behind 0）。
...
- handoff作成中に `pwa/src/components/cockpit/CockpitKuteSeeds.tsx` へ別作業の新規dirtyを検知。評価フィルタを列見出しへ移す途中差分で、現行productionには未反映。所有元が確定するまでstage・revert・整形しない。
