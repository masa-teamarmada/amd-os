...
AMD OS は、**株式会社チームアルマダ (= AMD)** が手がけるディープテック PJ (= スタートアップ準備中の研究シーズ含む) を経営する **専属の経営 OS**。AMD 経営チーム、AMD メンバー、SU 側メンバー (= PI / 創業候補) が日々使う。
...
- AMD 全体コックピット (= p00) で会社全体の Management Score・経営判断 dialogue を回す

**やらないこと**:
- 公開ダッシュボード (= 機密データを含む、社外には見せない)
- 自動意思決定 (= チームの判断を置き換えない。LLM はあくまで候補抽出)

## 0.2 想定ユーザー

| ロール | 主な使い方 | よく見る画面 |
|---|---|---|
| **AMD 経営チーム** | 全 PJ 経営判断、Management Score、提案前の判断整理 | `p00 cockpit` / `各 PJ cockpit` / `/notifications` |
...
| **投資家** (= 将来) | 各 SU の進捗・ピッチ素材 | (= 公開予定なし、AMD が説明会で使う想定) |
...
| **Gmail** | AMD 関係者の受信箱 / 会社アカウント | 外部関係者 (= VC / 顧客候補 / 大学) との連絡 |

→ ユーザー向けには「どの一次情報が OS に入るか」だけ掴めれば OK。抽出 pipeline や復旧履歴は開発者向け章に分ける。
...
| **L2 ③** | `project_milestones` + 進捗 | MS 達成度 | 「事業計画策定 = PM 30% + 伴走メンバー 70%」 |
...
→ L2 の内部抽出仕様は開発者向け章で管理する。通常利用では、各画面に出てくる「経営ハイライト」「MS 進捗」「MTG サマリ」がどの L2 から来ているかを見れば十分。

## 0.5 章の読み方ガイド

このマニュアルは大きく 2 つに分ける。

- **ユーザー向け**: AMD メンバー / SU 側メンバー / admin がざっくり使い方を知るための章
- **開発者向け**: 実装・抽出 pipeline・過去判断・復旧ログまで追う章

| あなたが何をしたいか | まずどの章 |
|---|---|
| OS を初めて触る / ざっくり使い方を知りたい | **[08 章 はじめて使う人向け](08-member-quick-start.md)** |
| 自分の `/mypage`、立替、月次TODOを日常的に回したい | **[10 章 メンバーの日常ワークフロー](10-member-workflows-quick-start.md)** |
| PJ の状況を見たい・経営判断したい | **[01 章 PJ コックピット](01-pj-cockpit.md)** |
| 会社全体の経営状況を見たい・提案前の論点整理をしたい | **[02 章 AMD 会社全体](02-amd-cockpit.md)** |
| AMD Score の数式や軸の意味まで知りたい | **[21 章 AMD Score 詳細仕様](21-amd-score-spec.md)** |
| HRL / FRL / 関連メンバーの考え方を知りたい | **[21 章 21.9](21-amd-score-spec.md#219-hrl--frl-の詳細ロジック)** |
| AMD Management Score の見方を知りたい | **[02 章 2.1](02-amd-cockpit.md#21-management-score)** |
...
| DB / API / cron / 抽出 pipeline / 過去判断ログまで見たい | **[開発者向けマニュアル](/manual?audience=developer)** |
