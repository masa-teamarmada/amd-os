...
メンバーが直接触る `/mypage` / `/reimburse` と、 admin が運用する `/admin/billing` / `/admin/prompts` の開発者向け仕様。 メンバー視点の使い方早見表は [10 章](10-member-workflows-quick-start.md) を見る。
...
- 再計算は `/api/cron/payout-reward-cache-refresh` (= 日次 03:05 JST) または手動 `Run Now` (= 24 章)
...
admin (`members.is_admin=true`) は `/mypage?memberId=<member_id>` で他メンバーのページを見られる。 メンバーコードネームを文中に出すときは共通 UI `LinkedMemberText` で `/mypage?memberId=` リンクにする。
...
つくよみ呼び出し側は `tags` をフィルタしながら `priority` 順に system_prompt を連結し、 LLM に system role として渡す ([27 章](27-knowledge-admin-tsukuyomi-spec.md))。
...
- 10 章 [メンバーの日常ワークフロー](10-member-workflows-quick-start.md) (= 使い方早見表)
- 31 章 [Admin Payouts / 支払通知書](31-admin-payouts-reward-notice-spec.md) (= 報酬 → 支払通知書)
- 32 章 [Invoice / Billing Routine](32-invoice-and-billing-routine-spec.md) (= billing_cycles 全体)
- 27 章 [Knowledge Admin / Tsukuyomi](27-knowledge-admin-tsukuyomi-spec.md) (= つくよみ context 詳細)
- 24 章 [Operations Settings](24-operations-settings-spec.md) (= cron 実行)
