...
トップ式の直下では、`q`、`P`、BZM 2.0の`SPS`を同じ代入列で表示する。社会的価値と経済的価値を分けた本来の`P`はまだ測定中だが、最初の変化を観察するため、画面は0〜100の`P^(0)`（初期投影）をパーセントで表示する。既存SPSの潜在性入力があるPJはそれを100へ換算し、無いPJだけは記録済みの6軸平均を同じ尺度へ換算する。これは本来の価値ベクトルではなく、根拠・版・更新規則を残した暫定の基準線である。
...
特に、現行SPSの`prs_potential`はBZM 2.0の価値ベクトル`P`そのものではない。ただし初期投影`P^(0)`では、変換規則と出所を明示したうえで100点の基準線として使う。累積調達額は現在現金`C_0`でも`T_Y`でもない。
