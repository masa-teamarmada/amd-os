...
トップ式の直下では、`q`、`P`、BZM 2.0の`SPS`を同じ代入列で表示する。
`P`は共通到達目標へ着いた場合の会社全体の条件付き理論時価総額で、単位は円である。
BZSFが取得する持分比率や投資契約は入れない。

社会的な要素は、独立した点数として足さない。
工程や資金接続を動かすなら`q`へ、売上、補助、公共調達、規制、費用を動かすなら将来キャッシュフローへ、投資家需要や要求収益率を動かすなら市場の価格づけへ入れる。
どの経路を通るか確認できない間は、存在を記録しても数字は動かさない。

`q`と円建て`P`が両方ある場合だけ、画面はSPSを計算する。
片方が欠測なら未計算理由を表示する。

以前の`P^(0)`と社会・経済の二成分指数は撤回済みで、履歴にだけ残す。
円への換算規則が無いため、現行`P`またはSPSへ読み替えない。
...
| `P` | 到達した場合の会社全体の条件付き理論時価総額、評価日、DCF方式、代理・補完状態、撤回済み指数の履歴 |
...
通常の欠測を0へは変換しない。Pも、円へ変換する参照クラスまたは機械的規則が無ければ欠測にする。
...
特に、現行SPSの`prs_potential`はBZM 2.0の円建て`P`ではない。旧指数から円への換算も行わない。累積調達額は現在現金`C_0`でも`T_Y`でもない。
...
全パラメータの抽出規則は12群の折りたたみ表で確認できる。詳細正本は`pwa/bzm/BZM_2_0_PARAMETER_EXTRACTION_REGISTER.md`に置く。
