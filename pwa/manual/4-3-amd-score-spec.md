...
最上段では低位・中央・高位scenarioの`q_gate_product_proxy`と`q_stress_proxy`、$J$、成功条件付き$P$、初回経路喪失を要約する。金額は`¥4,010M`のように円記号・桁区切り・`M`で表示し、小数は四捨五入する。直下の「全パラメータ台帳」は7群の103項目を1件も間引かず、各項目の値、推定状態、確度、根拠・注記、scenario別入力を確認できる形で表示する。長い構造化値は形・hash・previewを要約しても、パラメータ行そのものは省略しない。

前向き検証は0件である。

表示される`q_gate_product_proxy`と`q_stress_proxy`は未校正の代理指標であり、到達確率$q$または頑健到達確率$q_{\mathrm{rob}}$ではない。
