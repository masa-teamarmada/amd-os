...
2026-08-13以降、この表示は「イベントと月次試算」へ統合する。評価月M0から経済計算地平までを一本の横時間軸とし、出来事を該当月の点と上下の説明として置く。登録方針は地平全体の帯として表示する。時間軸直下には`project_pl_monthly`の売上、売上原価、粗利、人件費、研究開発費、マーケ費、その他販管費、営業利益を同じ月列で揃える。SXは2027-02を会社設立月として「設立前PJ / NewCo」の計上主体行と縦境界を置く。2027-01以前は費用入力の合計を正の「設立前PJ支出」として表示し、NewCoの粗利・営業利益は対象外を示す`—`とする。設立前PJ支出をNewCoの営業損失へ読み替えない。NewCo P/LとC/Fは設立月から開始し、設立時残高が未確認なら月初・月末残高を0円へ推定しない。Phase 0の設立前PJ資金はNewCo C/Fへ算入しない。BZMの$CF_t$は「BZM経済CF」として別行に置き、月次PLと同じ値だと仮定しない。金額は表の外に`単位：百万円`を一度だけ置き、各セルへ`M`や`¥`を繰り返さない。DB円額と入力小数の精度は保持する。月次セルを押した編集は表の下へ追加せず、現在位置を保ったまま中央モーダルへ出す。入力・削除・つくよみヒアリングはこの区画へ統合し、コックピットヘッダーの独立した試算表ボタンは廃止する。
