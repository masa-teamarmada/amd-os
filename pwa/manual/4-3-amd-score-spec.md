...
スコア詳細の主表示は`SPS Primary Model Registry`で切り替える。`sps_2_1 / active`では **SPS 2.1の方針条件付き将来正味PJ価値**を主表示し、従来の9軸診断SPS (`M x P x R x S`) とBZM 2.0は復元可能なarchiveへ折りたたむ。切替準備中・読出し不能・ロールバック時は、従来の9軸診断SPSを安全側の主表示へ戻す。
...
主モデルregistryが`active`なら、画面の先頭にSPS 2.1を置く。

BZM 2.0と現行9軸SPSは、復元用snapshotの情報とともに「旧SPSアーカイブ」へ折りたたんで残す。

registryが未登録、取得不能、`preparing`、`rolled_back`なら旧SPSを主表示に保ち、SPS 2.1はpreviewとして折りたたむ。
...
このpilotは、パラメータの不足箇所と計算接続を検査するshadow試算であり、PJコックピット、DB、SPS primaryには反映していない。
