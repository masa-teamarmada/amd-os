...
BZM 2.0と現行9軸SPSは、復元用snapshotの情報とともに別々の下段アーカイブへ残す。

registryが未登録、取得不能、`preparing`、`rolled_back`なら、「現行SPS / BZM 2.1」内で旧9軸SPSを運用上のfallbackとし、SPS 2.1はpreviewとする。この切替はBZM 2.2の表示順序を動かさない。
...
現行運用SPSは9軸の診断指数として別枠に残し、BZM 2.0または2.1の確率・価値で置き換えない。
...
全条件通過時の現在価値$P$、失敗を含む全分岐込み現在価値$J$、現行SPSの9軸診断指数は別の出力として読む。
