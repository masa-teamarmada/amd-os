...
2026-08-13以降、この表示は「イベントと月次試算」へ統合する。評価月M0から経済計算地平までを一本の横時間軸とし、出来事を該当月の点と上下の説明として置く。登録方針は地平全体の帯として表示する。時間軸直下には`project_pl_monthly`の売上、売上原価、粗利、人件費、研究開発費、マーケ費、その他販管費、営業利益を同じ月列で揃える。BZMの$CF_t$は「BZM経済CF」として別行に置き、月次PLと同じ値だと仮定しない。金額は表の外に`単位：百万円`を一度だけ置き、各セルへ`M`や`¥`を繰り返さない。DB円額と入力小数の精度は保持する。月次PLの入力・削除・つくよみヒアリングはこの区画へ統合し、コックピットヘッダーの独立した試算表ボタンは廃止する。

スコア詳細は4px基準で縦余白を圧縮し、本文・金額の大きさを落とさず情報密度を上げる。月次表の通常行は短い一段見出し、累計だけを圧縮した二段目とし、C/Fで繰り返していた「計画値」「残高は合計しない」は共通説明へ集約する。フェーズ開始、設備投資、株式調達、助成金等の大きな変動セルには`◆`を置き、マウスオーバーまたはキーボードフォーカスで理由、金額、時点、計画／観測の別を表示する。Series Aなどの調達はactive資本政策、工場投資はフェーズ別事業計画と年次設備投資、採択名・採択額・受領実績は`project_grants`を根拠にする。採択と入金を同じ出来事にせず、受領未確認分は当月入金へ計上しない。
...
画面の「式と入力のつながり」は、$J/P/Q/S$ごとに記号式を置き、その各項の直下へ選択中ケースの実数を線で結んだ代入ボードとして表示する。$J$は「経路中の月次収支」「全条件通過時の寄与」「条件別の停止寄与」、$P$は「全条件通過時の月次収支」「最終月の将来価値」、$Q$はgate別$p_i$の積、$S$はstress別の積と最小値を、式から目を移さず読めるようにする。desktopは横方向の式、mobileは同じ項を縦方向へ再配置し、ページ全体の横スクロールを発生させない。
