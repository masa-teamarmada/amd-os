...
スコア詳細の画面最上段は「BZM 2.2」とし、現在の進め方、J・P・Q・S、選択とイベントの時間軸、103項目の計算前提を確認する。`SPS Primary Model Registry`は引き続き運用SPS内部の`legacy_sps / sps_2_1`選択だけを管理し、その結果は画面最下部の「現行SPS / BZM 2.1」で確認する。BZM 2.0とSPS 1.0 / Legacy AMDもその下に独立した折りたたみで残す。三つの下段アーカイブはすべて初期状態を閉じ、初めて開いた時だけ内容を読み込む。
...
PJコックピットのスコア詳細では「BZM 2.2」と表示する。登録済みの比較候補から別の進め方を選ぶと、同じ前提ケースのJ・P・Q・Sをその場で切り替え、元の進め方との差を確認できる。この操作はブラウザ内だけで完結し、DBと`SPS Primary Model Registry`を書き換えない。
...
直下の「全パラメータ台帳」は7群の103項目を1件も間引かず、各行を「日本語名 / この項目を表す記号 / 関係する式 / 平易な説明 / 登録値 / 慎重・基準・強気の3ケース / 値の決め方 / 根拠資料」の順で表示する。理論上の量ではないモデルID、日付、通貨、各種ID、根拠資料、hashなどは「数式には直接入らない管理項目」と明示する。`section.key`形式の内部名や英語の機械状態は主表示へ出さず、「監査用の内部情報」の中だけに置く。根拠は件数だけでなく、PJ資料室、Gmail、OSデータなど開ける参照先へ接続し、未接続の参照先はそう表示する。
