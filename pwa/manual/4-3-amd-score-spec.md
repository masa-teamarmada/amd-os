...
直下は三層で読む。第一層「今回の数字を動かす入力」は、評価期間、割引率、月次経済CF、gate別の判定月と条件付き通過値、gate別の停止時価値、終端価値、stress補正だけを出す。記号式と実数代入を一緒に示し、$Q$はgate積、$S$はstress別積の最小、$P$は全gate通過経路のCF現在価値と終端現在価値、$J$は生存加重CF現在価値・成功寄与・gate別停止寄与として分解する。第二層「実行可能性と経営判断」は状態、資源、権限、同意、行動束、資金制約を示す。第三層「根拠・再現・監査」は7群103項目を1件も間引かず初期状態を閉じて収載する。`section.key`形式の内部名や英語の機械状態は第三層の詳細だけに置く。根拠は件数だけでなく、PJ資料室、Gmail、OSデータなど開ける参照先へ接続し、未接続の参照先はそう表示する。
