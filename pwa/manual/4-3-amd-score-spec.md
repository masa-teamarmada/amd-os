...
> **2026-07-29 主張境界（当時の運用版＝旧9軸SPSについての記録）**: 当時の運用版SPSは診断指数であり、企業価値、期待事業価値、成功確率、生存確率を表す検証済みモデルではない。0〜9点は順序尺度として扱い、点数差・点数比・`alpha`に経済的な間隔や弾力性の意味があるとはみなさない。SPS順位と律速表示だけでGO、NO_GO、投資額、投入人月を決めない。改訂要件は [`BZM_2_0_REVISION_REQUIREMENTS.md`](../bzm/BZM_2_0_REVISION_REQUIREMENTS.md) を参照する。
>
> 9軸の集約スコアは 2026-08-15 まさ確定で退役した。現行SPSは円建てで単位も尺度も違うが、予測妥当性を主張しない点と、順位・GO/NO_GO・投資額・資源配分へ単独利用しない点は変わらない。

## 先にここだけ読む

AMD Score は PJ / SU の比較と弱点診断に使う指数。AMD 全社の健康度を見る AMD Management Score とは別物。

現行SPSは、そのシーズが日本国内に生む産業の大きさを円で見積もる指数である。式は冒頭の `SPS = Σ q_o P^ind_o` で、二つの量の掛け算を価値実現経路について足す。

| 記号 | 読み | 何を見るか | 単位 |
|---|---|---|---|
| `q_o` | 到達見込み | 経路 `o` まで到達する確率 | % |
| `P^ind_o` | 産業創出価値 | 到達した場合に国内へ生む付加価値の割引現在価値の桁 | 円 |

画面はこの積の**中央値**を主表示にし、括弧で最小〜最大の帯を併記して、中央値の降順で並べる。帯は接触前の粗い下書きで、精度は接触で上げる。

中央値だけを見ない。必ず一緒に見るものが二つある。

**根拠Lv** (スコア成熟度) は Lv0＝ネット情報のみ / Lv1＝ヒアリング済 / Lv2＝OSにPJ情報抽出済 / Lv3＝月次試算表あり。同じ中央値でも Lv0 と Lv2 では確からしさが違う。

**q要因11項目**は、二大要素 (ユニットエコノミクス成立性・資本集約度) に9つの要因を足したもので、各項目が事実と、その事実が `q` をどちらへどれだけ動かすかの意味づけを持つ。判断の定義は [`SEED_Q_RUBRIC_2026-08-15.md`](../bzm/SEED_Q_RUBRIC_2026-08-15.md)、`P^ind` の作り方は [`SPS_NAT_VALUE_MEASURE_PROPOSAL_2026-08-16.md`](../bzm/SPS_NAT_VALUE_MEASURE_PROPOSAL_2026-08-16.md) が正本。

帯の中央値だけを投資判断、対外表示、PJ間の資源配分へ使わない。幅と根拠Lvを外して数字だけを引用しない。

9軸を一つの点数へ畳んだ旧SPSは 2026-08-15 まさ確定で退役した。式と当時の読み方は本章の「旧SPS (9軸診断) の式」に履歴として残す。

## BZM 2.0観測画面

スコア詳細の下段には、現行運用SPSとは別に、BZM 2.0の数式と測定状態を「BZM 2.0」折りたたみで残す。

BZM 2.0の到達見込みモデルの式は、2026-08-15 まさ確定で現行SPSの式になった。この折りたたみはその式のパラメータ観測台帳で、上段に出るSPSの帯そのものとは別の画面である。
...
=\Pr\!\left(T_C<T_Y,\ T_C\le H_v\mid\mathcal I_\tau\right)
$$

$$
Q_\tau(h)
=\Pr\!\left(T_C<T_Y,\ T_C\le h\mid\mathcal I_\tau\right)
$$

トップは三段に分かれる。

1. 全価値実現経路の確率加重持分価値。
2. 共通経済評価地平までの、計画期限後を含む資本自立経路全体の価値寄与。
3. PJ固有の計画期限に間に合う資本自立経路の価値寄与。

いまのSXの4.15%は三段目の計画達成診断であり、会社の成功確率そのものではない。
全PJ比較には三段の価値式へ混ぜず、同じ期間$h$の累積到達曲線$Q_\tau(h)$を使う。
旧`q × P`は三段目の内訳としてだけ残し、会社全体の期待時価総額とは呼ばない。
非上場PJの出力は市場で観測された時価総額ではなく、列挙した経路にもとづくモデル内の持分価値である。

社会的な要素は、独立した点数として足さない。
工程や資金接続を動かすなら経路別の確率へ、売上、補助、公共調達、規制、費用を動かすなら将来キャッシュフローへ、投資家需要や要求収益率を動かすなら市場の価格づけへ入れる。
どの経路を通るか確認できない間は、存在を記録しても数字は動かさない。

全価値実現経路は、計画期限内の資本自立、期限後の資本自立、M&A、ライセンス、知財売却、ピボット、撤退、清算、共通経済評価地平時点の継続中を重複なく分ける。
各経路の確率と円建て価値がそろったときだけ全経路SPSを計算し、欠測なら未計算理由を表示する。
到達前開発費、将来調達、希薄化、負債、税、残存価値も落とさない。

以前の`P^(0)`と社会・経済の二成分指数は撤回済みで、履歴にだけ残す。
円への換算規則が無いため、現行の経路価値またはSPSへ読み替えない。

右の到達競争式の直下では、共通状態、到達時間、余力喪失時間、計画期限、共通経済評価地平、計画期限内診断、共通期間比較値を現在の判定値として並べる。これにより、共通状態がどの時計と到達見込みへ条件づけられているかを、式と値を往復せず確認できる。

画面は、式の記号を次の現在値へ結ぶ。

| 記号 | 画面で確認するもの |
|---|---|
| `q_plan` | PJ固有の計画期限内に資本自立へ着く診断値、95%信頼区間（計算上のぶれ）、版ごとの変化 |
| `Q(h)` | 全PJへ同じ期間を置いた資本自立の累積到達見込み |
| `q_G` | 共通経済評価地平までの、計画期限後を含む資本自立経路全体の到達見込み |
| `P_o` | 各価値実現経路の条件付き正味持分価値、評価日、評価方式、欠測状態 |
| `SPS_all` | 相互排他的な全経路の確率加重持分価値。全経路未測定なら欠測 |
| `T_C` | 到達時間の分布または時間入力の欠測 |
| `T_Y` | 戦略余力の喪失時間。資金成分だけの部分実装を全体と同一視しない |
| `H_v` | 計画版ごとの期限。資金の崖とは分けて表示する |
| `H_econ` | 全PJへ同じ月数で置く共通経済評価地平。PJごとの情報締切から導く対応日も併記し、未決着経路を継続中へ分類する |
| `Z` | 複数工程へ作用する共通状態と、その影響先 |

たとえばLSTの`Z_policy`は、独立した加点係数ではない。

現在の`#2=90%`と`#6=60%`を`Z_policy=present`に条件づけた入力として表示し、影響先を`#2`、`#6`、`T_C`、`T_Y`、`q_plan`へつなぐ。

政策支援がない状態の入力は未取得なので、ロビイングの追加効果は表示しない。

通常の欠測を0へは変換しない。経路価値も、円へ変換する参照クラスまたは機械的規則が無ければ欠測にする。
...
特に、現行SPSの`prs_potential`はBZM 2.0の円建て経路価値ではない。旧指数から円への換算も行わない。累積調達額は現在現金`C_0`でも`T_Y`でもない。

終了状態を過去の予測入力へ戻して使うと後知恵になるため、現行状態は文脈として残すだけで過去の経路確率へ接続しない。
...
全パラメータの抽出規則は折りたたみ表で確認できる。詳細正本は`pwa/bzm/BZM_2_0_PARAMETER_EXTRACTION_REGISTER.md`に置く。

数式の記号は画面上でLaTeXとして組み、たとえば$T_C$、$T_Y$、$H_v$、$Z_{\mathrm{policy}}$を下付きが分かる形で表示する。

$q_{\mathrm{plan}}$の版推移は初期状態では折りたたみ、必要なときだけ変更理由と出所を開く。

各パラメータの説明、出所参照、版ごとの値・状態・情報締切も行内の「詳細」から開く。現在値を比べるだけなら、表を縦に読み続けずに済む。

新しい共通状態や工程入力は、DB列を増やさず`parameter_key`の新しい行として追加する。

SXはv0.2からv0.5までの計画達成診断$q_{\mathrm{plan}}$と現行入力を表示する。

LSTは事前登録中として、8ノードと`Z_policy`を表示するが、依存グラフ、時間三点幅、`C_0`が欠測のため計画達成診断を出さない。

そのほかのPJは、事前登録または観測が始まるまで必須記号を欠測のまま表示する。

画面に出る`parameter_key=q`は、$q_{\mathrm{plan},\tau}(H_v)$として定義に従って計算した仮説出力である。

前向き検証件数を併記し、GO、NO_GO、投資額へ単独利用しない。

## BZM 2.1動的方針評価

BZM 2.1は、BZM 2.0の固定計画を消さず、その計画を基準線として判断時点ごとの行動を比較する。

主モデルregistryが`active`なら、下段の「現行SPS / BZM 2.1」内でSPS 2.1を現行運用モデルとして読む。画面上段のBZM 2.2より上へは出さない。

BZM 2.0と旧9軸SPSは、復元用snapshotの情報とともに別々の下段アーカイブへ残す。

registryが未登録、取得不能、`preparing`、`rolled_back`なら、その区画のSPS 2.1はpreview扱いになる。旧9軸SPSへ運用上のfallbackをする挙動は 2026-08-18 のlatest-only統治で廃止し、現行版の完全一致行が無いときは「最新版未評価」と表示する（冒頭の現行SPS節）。この切替はBZM 2.2の表示順序を動かさない。
行動は意思決定者の目的で一度だけ選び、その同じ方針を三視点から読み直す。
視点ごとに別の「最適行動」を作ったり、三つの価値を足したりしない。

会社欄は会社全体の時価総額ではなく、会社が保有する対象PJの方針条件付き将来正味価値である。

増分として表示するのは、同じ条件で固定方針と選択方針を比べた差だけである。
BZSF欄は対象証券のキャッシュフローであり、他PJとの資源配分は別のポートフォリオ層で扱う。
公的欄は基金財政と社会便益を内部で分け、換算・集約規則が閉じない限り単一額を出さない。

CF事象ごとの基金財政、社会便益成分、換算、移転の反対符号は台帳で確認できる。
方針全体の公的価値を経路別に集計する処理は未実装なので、公的な価値欄だけを`計算不能`に保つ。
別視点の権限主体が選んだ同じ方針と物理的な到達見込みまで消さない。

最小実装が支援条件から計算へ反映するのは、単一キャッシュフロー台帳の費用・終端価値、必要資金、遷移確率までである。
支援による行動集合、所要時間、情報獲得の変更は未実装として表示する。

未調査の行動が残る場合は「測定済み行動内」と表示し、PJ全体の最適行動とは呼ばない。

遷移確率、費用、資金、撤退価値、実行権限、相手方同意が欠測なら、数値を0へ置き換えず`計算不能`と表示する。

版、共通入力、判断状態、行動束、遷移、支援、単一CF、行動評価、方針評価は、画面内の折りたたみ台帳で確認する。
古い計算結果、情報締切後の根拠、実PJへ混ざった合成根拠、単一CFの重複が見つかった場合は、警告だけを付けて数字を残さず、計算値を抑止する。

2026-08-12の`estimated-v0.1`は、12PJすべてに共通推定器のshadow値を表示する。

推定値は観測値と別の琥珀色で示し、数値入力に占める推定率、前向き検証0件、感度で行動が反転するかを主値と同じ範囲に置く。

SXのBZM 2.0計画達成診断4.15%は旧版の基準線として残し、2.1の選択方針下の到達見込みへ読み替えない。

12PJの実測版は行動別入力が閉じていないため、`estimated-v0.1`を測定済み価値とは表示しない。

合成テストの架空数値は、実PJ画面へ表示しない。

BZM 2.1は前向き検証0件であり、選択方針、GO、NO_GO、投資額の自動推薦には使わない。

現行SPSはBZM 2.0の式による円建ての価値量であり、BZM 2.1の行動別価値や選択方針と同じ数値として扱わない。

1.0から2.1までの読み方は[進化編](../bzm/bzm-1-0-to-2-1-evolution-guide)で確認する。

数式と計算契約の正本は[動的な事業価値モデル](../bzm/bzm-2-1-dynamic-business-value-model)に置く。

## BZM 2.2全PJ内部pilot

2026-08-12時点で、現行SPS対象の12 PJについて、103パラメータの欠測を強制推定した`provisional-pilot-v0.1 / unvalidated`がrepo内にある。

PJコックピットのスコア詳細では「BZM 2.2」と表示する。評価に用いた登録方針と未登録のshadow比較案を切り替えると、評価日時点で凍結済みのJ・P・Q・Sを比較できる。未登録案を将来の意思決定候補または実行推奨へ読み替えない。この操作はブラウザ内だけで完結し、DBと`SPS Primary Model Registry`を書き換えない。

最上段では低位・中央・高位scenarioの4指標を、$J$「全分岐込み現在価値」、$P$「全条件通過時の現在価値」、$Q$「基準到達指数」、$S$「逆風耐久指数」として表示する。各指標は一文字、基準値、低位・基準・高位、数式、肯定形の一文を同じカードへ収める。金額は`¥4,010M`のように円記号・桁区切り・`M`で表示し、小数は四捨五入する。

四指標と式の下には「選択とイベントの時間軸」がある。同じ時間軸でも、①根拠を確認できた意思決定、②試算で使う現在の進め方、③これから判断する地点、④技術・工場・顧客・資金の出来事、⑤外部の出来事を別の段で読む。イベントカードは副説明を付けず、同月の全タイトルを省略せず表示する。現在のpilotには根拠付きの選択履歴がまだ入っていないため、意思決定の段は「未収載」と表示される。gateの月を選択日へ読み替えず、条件付き資金を着金予定へ読み替えない。

2026-08-13以降、この表示は「イベントと月次試算」へ統合する。評価月M0から経済計算地平までを一本の横時間軸とし、出来事を該当月の点と上下の説明として置く。登録方針は地平全体の帯として表示する。時間軸直下には`project_pl_monthly`の売上、売上原価、粗利、人件費、研究開発費、マーケ費、その他販管費、営業利益を同じ月列で揃える。SXは2027-02、CXは`project_ventures.founded_at`の2026-11を会社設立月として「設立前PJ / NewCo」の計上主体行と縦境界を置く。設立前は費用入力の合計を正の「設立前PJ支出」として表示し、NewCoの粗利・営業利益は対象外を示す`—`とする。設立前PJ支出をNewCoの営業損失へ読み替えない。NewCo P/LとC/Fは設立月から開始し、設立時残高が未確認なら月初・月末残高を0円へ推定しない。CXのBZM月次資金前提は計画・低精度としてセルの◆から読み、実績P/Lへ混ぜない。資本政策の株式調達は財務C/Fだけへ出し、売上とは扱わない。BZMの$CF_t$は「BZM経済CF」として別行に置き、月次PLと同じ値だと仮定しない。金額は表の外に`単位：百万円`を一度だけ置き、各セルへ`M`や`¥`を繰り返さない。DB円額と入力小数の精度は保持する。月次セルを押した編集は表の下へ追加せず、現在位置を保ったまま中央モーダルへ出す。入力・削除・つくよみヒアリングはこの区画へ統合し、コックピットヘッダーの独立した試算表ボタンは廃止する。

スコア詳細は用途を説明できない固定高さを置かず、4px基準で縦余白を圧縮する。月次表は固定見出し108px（`sm`以上172px）、月列76px、金額11px、通常行20px前後を密度基準とし、純粋な数値列だけ右寄せと等幅数字を維持する。イベント軸は上下2段・高さ101px、数式代入は接続線を持つ二段カードではなく式・名称・値を同じ一段セルへまとめ、各セルは内容幅で止める。短い項を`flex-grow`や親の`stretch`で行幅・行高いっぱいへ引き伸ばさない。条件判定月の入力は初期状態を閉じる。C/Fで繰り返していた「計画値」「残高は合計しない」は共通説明へ集約する。フェーズ開始、設備投資、株式調達、助成金等の大きな変動セルには`◆`を置き、マウスオーバーまたはキーボードフォーカスで理由、金額、時点、計画／観測の別を表示する。設立前フェーズ開始の注記は「設立前PJ支出」、設立後は「営業利益（NewCo）」へ付ける。Series Aなどの調達はactive資本政策、工場投資はフェーズ別事業計画と年次設備投資、採択名・採択額・受領実績は`project_grants`を根拠にする。採択と入金を同じ出来事にせず、受領未確認分は当月入金へ計上しない。SXの前提順序は「2027-01までに設立前DD完了」「2027-02会社設立」「2027-03までの初回調達内部目標」とし、DD完了実績・投資家合意・着金実績へ勝手に昇格しない。設立前DDが現計算ゲートへ未接続の間は、J/P/Q/Sを前提監査未通過として扱う。

このpilotの$P$は、登録済み現在方針$\pi_{\mathrm{reg}}$を固定したまま、すべてのgateを通過した場合の途中収支と終端価値を評価日時点へ割り引いた値である。

複数の行動から最良方針を選ぶ値ではない。最適化まで行うなら$\max_{\pi}J_r^{\pi}$という別の問題になるが、現在のpilotはその最大化を実行していない。

$$
P_{r\mid G}^{\pi_{\mathrm{reg}}}
=\sum_t d_tCF_{r,t}^{\pi_{\mathrm{reg}}}+d_HTV_r^{\pi_{\mathrm{reg}}}
$$

$J$は同じ登録済み現在方針について、各月まで経路が続く重み、全gate通過時の価値、途中失敗時の残存または精算価値をすべて現在価値へまとめる。

$$
J_r^{\pi_{\mathrm{reg}}}
=\sum_t d_tW_tCF_{r,t}^{\pi_{\mathrm{reg}}}
+d_HQ^{\pi_{\mathrm{reg}}}TV_r^{\pi_{\mathrm{reg}}}
+\sum_i d_{t_i}\left(\prod_{j<i}p_j^{\pi_{\mathrm{reg}}}\right)(1-p_i^{\pi_{\mathrm{reg}}})RV_{r,i}
$$

$J$は各月の経路継続重み、全gate通過時の価値、途中停止時の精算価値を同じ評価日時点へ集約する。$P$は全gate通過経路だけの現在価値を示す。

画面の「式と入力のつながり」は、$J/P/Q/S$ごとに記号式を置き、各項の式・日本語名・選択中ケースの実数を同じ一段セルへまとめる。$J$は「経路中の月次収支」「全条件通過時の寄与」「条件別の停止寄与」、$P$は「全条件通過時の月次収支」「最終月の将来価値」、$Q$はgate別$p_i$の積、$S$はstress別の積と最小値を、式から目を移さず読めるようにする。各項は内容幅の独立セルとし、全画面幅を押し広げず、必要な場合だけ代入セル列の内部を横スクロールする。

直下は三層で読む。第一層「全パラメータ」は、式で使う26項目を、入力、計算途中、集合・番号、固定方針、出力の一つの高密度表へまとめる。$a,H,r_d,t,\mathcal T,CF_t,d_t,d_H,i,G,t_i,d_{t_i},p_i,W_t,W_{t_i^-},1-p_i,W_{t_i^-}(1-p_i),RV_i,TV,\delta,\Delta_{\mathrm{reg}},m_{i\delta},Q,S,P,J$の全てに日本語名、記号、選択中ケースの実値を添える。「選択中ケースの値」は数値だけでなく条件名、範囲、方針名を含む混合データ列なので左寄せとし、数字だけの比較列として扱わない。$G$は条件名を全件、$\Delta_{\mathrm{reg}}$は逆風名を全件、$CF_t,d_t,W_t$は60か月全行、$m_{i\delta}$は逆風×条件の全行列を展開できる。$W_t$は経路継続重みであり、BZM理論の意思決定状態$\mathbf s_t$とは別概念とする。経済計算地平はengineの`horizonMonths`へ接続する$H$に一意化する。第二層「実行可能性と経営判断」は状態、資源、権限、同意、行動束、資金制約を示す。第三層「根拠・再現・監査」は7群103項目を1件も間引かず初期状態を閉じて収載する。`section.key`形式の内部名や英語の機械状態は第三層の詳細だけに置く。

$J$ではgate前後の各月CFへ異なる生存重みを掛け、各gateで止まる場合の符号付き残存価値を加える。早く安く失敗を観測できる方針と、大きく支出した後に失敗する方針は異なる$J$になる。

ブラウザ内では、登録方針のgate判定月だけを動かす時期試算も行える。条件付き通過値、月次CF、終端価値、stress補正は固定し、月ごとの生存重みと停止時価値の割引だけを更新して$J$を再計算する。$Q/S/P$は変えず、工場、資金、経営判断の日付から未登録の因果効果を生成しない。

前向き検証は0件である。

UIの$Q$と$S$は、artifactの`q_gate_product_proxy`と`q_stress_proxy`を短く読むpilot記号である。$S$は戦略余力全体ではなく、登録stress下の最小gate積を示す。

現在方針も`shadow_only`であり、実行可能性を確認した方針または最適方針を意味しない。

全条件通過時の現在価値$P$、失敗を含む全分岐込み現在価値$J$、現行SPS $\sum_o q_oP^{\mathrm{ind}}_o$は別の出力として読む。

三つを合算したり、一つから別の値へ換算したりしない。

資金調達入金はcash cliffを越える流動性には使うが、会社PJの$J$へ便益として足さない。

cash cliffを理由に既存の技術・顧客・権利gateの時点を前倒ししない。

初回cliff月の月初で、全期間の補填期限を賄う資金packageの代理gateを別に一度だけ判定する。同じ資金証拠は既存gateと二重に掛けない。

各期限までに使える資金だけを数え、後日入金や日付不明の資金で前の崖を越えたことにしない。金融・非金融を分離できない旧複合gateは元値を監査用に残し、代理積からは除外するため、非金融riskを過小評価する可能性が残る。

既存の固定方針を測定していないPJでは$q_{\mathrm{plan}}^{\pi_0}$を`not_applicable`とする。

`expected_cumulative_funding`というlegacy keyは、経路生存で加重した無資金残高の最小補填必要額を入れる互換slotであり、資金受領期待額ではない。

Gmail、Drive、Calendar、Slack、Notion、OS DBは、globalな本文、remote scope、thread再帰または権限範囲の取得が残っているため、六つすべて`incomplete`である。

経済学者役と経営学者役の独立監査で見つかった構造上の停止要因はpilot契約へ反映したが、条件付き確率の校正、因果識別、行動被覆、証拠間依存、共有資源制約の妥当性は証明していない。

このpilotの値でPJ間順位、撤退または継続、投資額、支援採択、資源配分を決めない。

## 旧SPS（9軸診断）の式

> 2026-08-15 まさ確定で退役した旧版の記録。実行・表示・新しい計算には使わない。現行SPSは冒頭の `SPS = Σ q_o P^ind_o`。

### 旧SPSの4つの問い

旧SPS (M·P·R·S) は、PJを診断する9軸を4つの問いへ整理して表示していた。4因子は概念上のグループであり、実計算は9軸を一層で乗法集約する。
...
足し算ではなく掛け算にした理由は、弱い軸が総合点を抑える診断規則を採用したため。Potential が大きくても Reach が低ければ届きにくく、Macrotrend が吹いていても自走力が無ければ環境への依存が残る。ただし、この乗法集約が将来成果を最もよく予測することは未検証であり、他の符号化・重み・集約方法に対する感度確認が必要。

`P` / `R_net` が未入力なら、旧SPS は `INPUT NEEDED` / review pending として止めていた。

旧SPSの読み方の骨は、次の4問だった。大事なのは、旧SPS が「このPJは大きくなりそうか」を1つの数字に潰しているだけではなく、次の4つの質問に分解して見ていること。
