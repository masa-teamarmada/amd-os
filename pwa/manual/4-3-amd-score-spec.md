...
四指標と式の下には「選択とイベントの時間軸」がある。同じ時間軸でも、①根拠を確認できた意思決定、②試算で使う現在の進め方、③これから判断する地点、④技術・工場・顧客・資金の出来事、⑤外部の出来事を別の段で読む。各イベント行には「このイベントの試算で前提にした進め方」を表示し、資金入金など方針選択ではない出来事は明確に分ける。現在のpilotには根拠付きの選択履歴がまだ入っていないため、意思決定の段は「未収載」と表示される。gateの月を選択日へ読み替えず、条件付き資金を着金予定へ読み替えない。

2026-08-13以降、この表示は「イベントと月次試算」へ統合する。評価月M0から経済計算地平までを一本の横時間軸とし、出来事を該当月の点と上下の説明として置く。登録方針は地平全体の帯として表示する。時間軸直下には`project_pl_monthly`の売上、売上原価、粗利、人件費、研究開発費、マーケ費、その他販管費、営業利益を同じ月列で揃える。BZMの$CF_t$は「BZM経済CF」として別行に置き、月次PLと同じ値だと仮定しない。金額は表の外に`単位：百万円`を一度だけ置き、各セルへ`M`や`¥`を繰り返さない。DB円額と入力小数の精度は保持する。月次セルを押した編集は表の下へ追加せず、現在位置を保ったまま中央モーダルへ出す。入力・削除・つくよみヒアリングはこの区画へ統合し、コックピットヘッダーの独立した試算表ボタンは廃止する。

スコア詳細は用途を説明できない固定高さを置かず、4px基準で縦余白を圧縮する。月次表は固定見出し108px（`sm`以上172px）、月列76px、金額11px、通常行20px前後を密度基準とし、右寄せと等幅数字を維持する。イベント軸は上下2段・高さ101px、数式代入は接続線を持つ二段カードではなく式・名称・値を同じ一段セルへまとめ、条件判定月の入力は初期状態を閉じる。C/Fで繰り返していた「計画値」「残高は合計しない」は共通説明へ集約する。フェーズ開始、設備投資、株式調達、助成金等の大きな変動セルには`◆`を置き、マウスオーバーまたはキーボードフォーカスで理由、金額、時点、計画／観測の別を表示する。Series Aなどの調達はactive資本政策、工場投資はフェーズ別事業計画と年次設備投資、採択名・採択額・受領実績は`project_grants`を根拠にする。採択と入金を同じ出来事にせず、受領未確認分は当月入金へ計上しない。SXの初回調達は、会議証跡の内部目標「2027-03」とactive資本政策の計上「2027-04 Seed 150百万円」を別表示し、前者を合意・着金へ昇格しない。
...
$J$と$P$はいずれも将来のCFと終端価値を評価日時点へ割り引く。$P$にはgate通過値による重みと失敗時精算を入れず、$J$には入れるため、一般に$J\neq QP$である。
...
直下は三層で読む。第一層「式に出てくる全記号」は、式で使う26記号を、入力、計算途中、集合・番号、固定方針、出力に分ける。$a,H,r_d,t,\mathcal T,CF_t,d_t,d_H,i,G,t_i,d_{t_i},p_i,W_t,W_{t_i^-},1-p_i,W_{t_i^-}(1-p_i),RV_i,TV,\delta,\Delta_{\mathrm{reg}},m_{i\delta},Q,S,P,J$の全てに日本語名と選択中ケースの実値を添える。$G$は条件名を全件、$\Delta_{\mathrm{reg}}$は逆風名を全件、$CF_t,d_t,W_t$は60か月全行、$m_{i\delta}$は逆風×条件の全行列を展開できる。$W_t$は経路継続重みであり、BZM理論の意思決定状態$\mathbf s_t$とは別概念とする。経済計算地平はengineの`horizonMonths`へ接続する$H$に一意化する。第二層「実行可能性と経営判断」は状態、資源、権限、同意、行動束、資金制約を示す。第三層「根拠・再現・監査」は7群103項目を1件も間引かず初期状態を閉じて収載する。103項目を数式記号一覧とは呼ばない。`section.key`形式の内部名や英語の機械状態は第三層の詳細だけに置く。

$J$を$QP$として表示してはいけない。$P$には全gateを通過した経路の月次CFがすべて入る一方、$J$ではgate前後の各月CFへ異なる生存重みを掛け、各gateで止まる場合の符号付き残存価値を加える。同じ$Q$でも、早く安く失敗を観測できる方針と、大きく支出した後に失敗する方針は異なる$J$になる。
