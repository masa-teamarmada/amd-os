...
トップ式の直下では、`q`、`P`、BZM 2.0の`SPS`を同じ代入列で表示する。現行Pは、社会的価値とPJ自身の経済的価値を別々の暫定指数として表示する。社会と経済は足さず、`P=(社会, 経済)`の二成分で読む。

各成分は五軸、各軸0から20の和で作る。記録から暫定判断した軸は青い「代理推定」、直接証拠が足りず中立点10を置いた軸は橙の「欠測補完」として、数字、根拠状態、出所を同じ行に出す。欠測補完は観測済みを意味せず、どの数字を次の証拠で置き換えるかを見えるようにするための処理である。

`q`が計算済みなら、画面は社会と経済それぞれについて`q×P`を計算する。`q`が欠測のPJではPの二成分だけを表示し、SPSは未計算とする。経済成分は条件付きDCFへ置き換える前の代理指数であり、企業価値、売上予測、投資順位ではない。

以前の`P^(0)`は履歴に残す。既存SPSの潜在性入力を換算せず使った初期基準線であり、P 2.0の二成分へ読み替えない。
...
| 記号 | 画面で確認するもの |
|---|---|
| `q` | 現行版の到達見込み、95%信頼区間（計算上のぶれ）、版ごとの変化 |
| `P` | 社会的価値と経済的価値の暫定二成分、十軸の代理推定と欠測補完、旧`P^(0)`の履歴 |
...
通常の欠測を0へは変換しない。P暫定尺度だけは、値10と「欠測補完」を同時に置く明示規則を使う。
...
特に、現行SPSの`prs_potential`はBZM 2.0の価値ベクトル`P`そのものではない。初期投影`P^(0)`では元の数値をそのまま使ったが、P 2.0は社会と経済の十軸を別版として採点する。累積調達額は現在現金`C_0`でも`T_Y`でもない。
