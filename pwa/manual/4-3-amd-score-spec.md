...
最上段では低位・中央・高位scenarioの4指標を、$J$「動的正味PJ価値」、$P$「成功時価値」、$Q$「基準到達指数」、$S$「逆風耐久指数」として表示する。各指標は一文字、基準値、低位・基準・高位、数式、肯定形の一文を同じカードへ収める。金額は`¥4,010M`のように円記号・桁区切り・`M`で表示し、小数は四捨五入する。直下の「全パラメータ台帳」は7群の103項目を1件も間引かず、各項目の値、推定状態、確度、根拠・注記、scenario別入力を確認できる形で表示する。長い構造化値は形・hash・previewを要約しても、パラメータ行そのものは省略しない。
...
成功条件付き価値$P$、失敗を含む全経路の動的正味PJ価値$J$、現行SPSの9軸診断指数は別の出力として読む。
