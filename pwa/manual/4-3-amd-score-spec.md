...
画面では、BZM 2.0の固定方針下の値、BZM 2.1の測定状態、会社・BZSF・公的支援者の三視点を順に置く。
行動は意思決定者の目的で一度だけ選び、その同じ方針を三視点から読み直す。
視点ごとに別の「最適行動」を作ったり、三つの価値を足したりしない。

会社欄は会社全体の時価総額ではなく、会社が保有する対象PJの増分正味価値である。
...
SXは固定方針下の計画達成診断4.15%を基準線として残すが、行動別入力が閉じていないため、2.1の動的正味価値と選択方針はまだ計算しない。

LSTとそのほかのPJも、入力が閉じるまでは同じ扱いにする。
