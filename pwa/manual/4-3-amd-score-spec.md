...
2026-08-13以降、この表示は「イベントと月次試算」へ統合する。評価月M0から経済計算地平までを一本の横時間軸とし、出来事を該当月の点と上下の説明として置く。登録方針は地平全体の帯として表示する。時間軸直下には`project_pl_monthly`の売上、売上原価、粗利、人件費、研究開発費、マーケ費、その他販管費、営業利益を同じ月列で揃える。SXは2027-02を会社設立月として「設立前PJ / NewCo」の計上主体行と縦境界を置き、設立前の月次値をNewCo P/Lへ見せない。NewCo C/Fは設立月から開始し、設立時残高が未確認なら月初・月末残高を0円へ推定しない。Phase 0の設立前PJ資金はNewCo C/Fへ算入しない。BZMの$CF_t$は「BZM経済CF」として別行に置き、月次PLと同じ値だと仮定しない。金額は表の外に`単位：百万円`を一度だけ置き、各セルへ`M`や`¥`を繰り返さない。DB円額と入力小数の精度は保持する。月次セルを押した編集は表の下へ追加せず、現在位置を保ったまま中央モーダルへ出す。入力・削除・つくよみヒアリングはこの区画へ統合し、コックピットヘッダーの独立した試算表ボタンは廃止する。

スコア詳細は用途を説明できない固定高さを置かず、4px基準で縦余白を圧縮する。月次表は固定見出し108px（`sm`以上172px）、月列76px、金額11px、通常行20px前後を密度基準とし、右寄せと等幅数字を維持する。イベント軸は上下2段・高さ101px、数式代入は接続線を持つ二段カードではなく式・名称・値を同じ一段セルへまとめ、条件判定月の入力は初期状態を閉じる。C/Fで繰り返していた「計画値」「残高は合計しない」は共通説明へ集約する。フェーズ開始、設備投資、株式調達、助成金等の大きな変動セルには`◆`を置き、マウスオーバーまたはキーボードフォーカスで理由、金額、時点、計画／観測の別を表示する。Series Aなどの調達はactive資本政策、工場投資はフェーズ別事業計画と年次設備投資、採択名・採択額・受領実績は`project_grants`を根拠にする。採択と入金を同じ出来事にせず、受領未確認分は当月入金へ計上しない。SXの前提順序は「2027-01までに設立前DD完了」「2027-02会社設立」「2027-03までの初回調達内部目標」とし、DD完了実績・投資家合意・着金実績へ勝手に昇格しない。設立前DDが現計算ゲートへ未接続の間は、J/P/Q/Sを前提監査未通過として扱う。
...
画面の「式と入力のつながり」は、$J/P/Q/S$ごとに記号式を置き、各項の式・日本語名・選択中ケースの実数を同じ一段セルへまとめる。$J$は「経路中の月次収支」「全条件通過時の寄与」「条件別の停止寄与」、$P$は「全条件通過時の月次収支」「最終月の将来価値」、$Q$はgate別$p_i$の積、$S$はstress別の積と最小値を、式から目を移さず読めるようにする。全画面幅を押し広げず、必要な場合だけ代入セル列の内部を横スクロールする。

直下は三層で読む。第一層「全パラメータ」は、式で使う26項目を、入力、計算途中、集合・番号、固定方針、出力の一つの高密度表へまとめる。$a,H,r_d,t,\mathcal T,CF_t,d_t,d_H,i,G,t_i,d_{t_i},p_i,W_t,W_{t_i^-},1-p_i,W_{t_i^-}(1-p_i),RV_i,TV,\delta,\Delta_{\mathrm{reg}},m_{i\delta},Q,S,P,J$の全てに日本語名、記号、選択中ケースの実値を添える。$G$は条件名を全件、$\Delta_{\mathrm{reg}}$は逆風名を全件、$CF_t,d_t,W_t$は60か月全行、$m_{i\delta}$は逆風×条件の全行列を展開できる。$W_t$は経路継続重みであり、BZM理論の意思決定状態$\mathbf s_t$とは別概念とする。経済計算地平はengineの`horizonMonths`へ接続する$H$に一意化する。第二層「実行可能性と経営判断」は状態、資源、権限、同意、行動束、資金制約を示す。第三層「根拠・再現・監査」は7群103項目を1件も間引かず初期状態を閉じて収載する。`section.key`形式の内部名や英語の機械状態は第三層の詳細だけに置く。
