...
トップ式の直下では、`q`、`P`、BZM 2.0の`SPS`を同じ代入列で表示する。社会的価値と経済的価値を分けた本来の`P`はまだ測定中だが、最初の変化を観察するため、画面は`P^(0)`（初期投影）も表示する。既存SPSの潜在性入力があるPJはその数値を換算せずそのまま使い、無いPJだけは記録済みの6軸平均を使う。`P^(0)`は確率ではないため、パーセント記号も分母も付けない。これは本来の価値ベクトルではなく、根拠・版・更新規則を残した暫定の基準線である。

`q`が計算済みなら、画面は`SPS^(0)=q×P^(0)`も出す。これは企業価値、期待事業価値、投資順位、GO判定ではない。社会的価値と条件付きDCFを測れたら、初期投影を上書きせず新しい版として並べる。値が無いPJへ感覚で0を置くことはしない。
...
| `P` | 潜在価値ベクトルの測定状態と、値の変化を先に観察するための初期投影`P^(0)` |
...
特に、現行SPSの`prs_potential`はBZM 2.0の価値ベクトル`P`そのものではない。ただし初期投影`P^(0)`では、出所を明示したうえで元の数値をそのまま使う。累積調達額は現在現金`C_0`でも`T_Y`でもない。
