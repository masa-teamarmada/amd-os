...
スコア詳細の画面最上段は、BZM 2.2の`provisional-pilot-v0.1 / unvalidated`を「暫定主表示」として読む。これは現行SPSの正本切替ではない。`SPS Primary Model Registry`は引き続き運用SPS内部の`legacy_sps / sps_2_1`選択だけを管理し、その結果は画面最下部の「現行SPS / BZM 2.1」で確認する。BZM 2.0とSPS 1.0 / Legacy AMDもその下に独立した折りたたみで残す。三つの下段アーカイブはすべて初期状態を閉じ、初めて開いた時だけ内容を読み込む。
...
主モデルregistryが`active`なら、下段の「現行SPS / BZM 2.1」内でSPS 2.1を現行運用モデルとして読む。BZM 2.2の暫定主表示より上へは出さない。

BZM 2.0と現行9軸SPSは、復元用snapshotの情報とともに別々の下段アーカイブへ残す。

registryが未登録、取得不能、`preparing`、`rolled_back`なら、「現行SPS / BZM 2.1」内で旧9軸SPSを運用上のfallbackとし、SPS 2.1はpreviewとする。この切替はBZM 2.2 pilotの表示順序を動かさない。
...
このpilotは、パラメータの不足箇所と計算接続を検査するshadow試算である。PJコックピットのスコア詳細には画面上の「BZM 2.2 暫定主表示」として接続するが、DBへは保存せず、`SPS Primary Model Registry`の正本も書き換えない。
...
四指標と式の下には「選択とイベントの時間軸」がある。同じ時間軸でも、①根拠を確認できた意思決定、②試算のために登録した現在方針、③これから判断する地点、④技術・工場・顧客・資金の出来事、⑤外部の出来事を別の段で読む。現在のpilotには根拠付きの選択履歴がまだ入っていないため、意思決定の段は「未収載」と表示される。登録済み現在方針は経営陣が最良案として承認した計画ではなく、計算対象へ固定したshadow方針である。gateの月を選択日へ読み替えず、条件付き資金を着金予定へ読み替えない。工場は承認・発注・着工・完成、資金は関心・契約・条件充足・着金を一つの出来事へまとめない。
...
直下の「全パラメータ台帳」は7群の103項目を1件も間引かず、各行を「日本語名 / LaTeXで示すデータ名 / 理論記号 / 接続する式 / 数式上の関係 / 平易な説明 / 内部key / 値・推定状態・根拠」の順で表示する。理論上の量ではないモデルID、日付、通貨、各種ID、根拠資料、hashなどには理論記号を発明せず`—`とする。数式上の関係は、直接入力、間接生成、実行条件、整合検査、派生出力、現版未使用、同じ元データの別名を分ける。長い構造化値は形・hash・previewを要約しても、パラメータ行そのものは省略しない。
