...
AMD Protocol (= L2 ②) の確認・編集画面。 詳細仕様は [`pwa/design/amd_protocol.md`](../design/amd_protocol.md) と [38 章 §② AMD Protocol](38-l2-extraction-routines-spec.md)。
...
つくよみが LLM 呼び出し時に system role として注入する **context** (= 旧スプシ system prompt 群) を管理する画面。 26 章の `/admin/prompts` と並列。
...
`status='active'` の `l2_feedbacks` は、 次回 Cloud routine 発火時に同 `l2_kind` / `target_id` / `scope_key` で SELECT され、 LLM の prompt に「過去にまさが指摘した修正依頼」として注入される (= [38 章 §冪等性と通知](38-l2-extraction-routines-spec.md#冪等性と通知))。
...
- 38 章 [L2 Extraction Routines](38-l2-extraction-routines-spec.md) (= 各 L2 routine と feedback 接続)
- 28 章 [通知レビュー UI / 経営ハイライト確認](28-notification-review-and-strategy-signals-spec.md) (= notifications 側の UI)
- 22 章 [通知・修正依頼・正本反映ゲート](22-notifications-and-tsukuyomi.md) (= ユーザー向け概念)
