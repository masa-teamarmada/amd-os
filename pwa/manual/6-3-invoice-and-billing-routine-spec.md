...
- admin 請求書発行 (`/admin/invoices`): 直近13か月の締め済み稼働月だけを対象に、請求額がある行を初期表示 `未完了` (= `発行待ち / 要確認 / 設定不足 / 過去滞留`) で出す発行キュー。各行には `取引先 / 金額` の発行条件だけを出し、freee取引先と請求額がそろった対象月の `発行待ち` 行だけ `発行` から明細確認 → freee 発行へ進む。報告書FIXと立替精算は請求書発行の blocker にしない。請求月 (`invoice_ym || ym`) が対象月より古い未発行行は `過去滞留` として今月発行分から分ける。`要確認 / 設定不足 / 過去滞留` の状態バッジまたは行操作から詳細モーダルを開き、発行前チェックと解消方法を確認する。設定不足では freee取引先をプルダウンで選んで保存できる。発行モーダルは件名、基本明細行、承認済み立替、調整行、請求日、支払期日、備考を扱い、件名・ヘッダー・freee fallback には `client_name` を使う。AMD内部の `project_name` / `project_id` は請求書発行モーダルやfreee件名に出さない。
- CTB見積: CTB停止中のため一旦廃止
- **古い月が上**
...
立替確認は PM 月次タスクから外す。`/admin/invoices` でも請求書発行の条件にはしない。申請が締めに間に合わない立替は、申請者側の未対応として請求発行を止めず、`/reimburse` と admin 承認側で扱う。
