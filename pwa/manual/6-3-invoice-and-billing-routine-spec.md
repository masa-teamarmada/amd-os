...
- admin 請求書発行 (`/admin/invoices`): `未発行 / 発行済み / 送付済み / 入金済み / すべて` で絞り込む発行キュー。各行には `予算 / 報告書 / 立替` の発行前確認だけを出し、未発行行の `発行` から明細確認 → freee 発行へ進む。
- CTB見積: CTB停止中のため一旦廃止
- **古い月が上**
...
立替確認は PM 月次タスクから外す。`/admin/invoices` では `立替` を発行前確認チップとして残すが、PMの `/mypage` 通知には使わない。
