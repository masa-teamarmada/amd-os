...
AMD OS は、**株式会社チームアルマダ (= AMD)** が手がけるディープテック PJ (= スタートアップ準備中の研究シーズ含む) を経営する **専属の経営 OS**。日々の全PJ横断面を使うのは **まさ (= 代表パートナー)、AMD メンバー、admin** で、ここは **社内専用**。ただし、PJごとに明示招待されたPI / 創業候補 / 研究開発メンバーには、社内コックピットとは分離した `/project/{project_id}/workspace` だけを共有できる。PJ限定ユーザーは他PJ、報酬、契約、内部判断、生データへ入れない。
...
| **PJ限定メンバー** | 自分の参加PJの研究開発エフォートと進捗を確認・入力 | `/project/{project_id}/workspace` |
