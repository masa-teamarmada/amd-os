...
> 関連: 入金確認 nudge 経由で更新される `billing_cycles.payment_confirmed_at` は [01 章 1.5 月次ルーティン](01-pj-cockpit.md#15-月次ルーティン--報告書--請求--会計) の「入金確認」ステップの正本。 freee API + Slack DM + admin 画面の 3 経路から更新される。
...
月初の漏れ確認や送信先確認に使う。 `/admin/settings` の Cron Control からは default `dryRun=0` だが、 手動 `Run Now` で `dryRun=1` に書き換え可能 ([24 章](24-operations-settings-spec.md))。
...
| nudge Slack DM が飛ばない | `SLACK_BOT_TOKEN` 設定、 admin の `members.slack_id`、 [24 章 トラブル時](24-operations-settings-spec.md#トラブル時) |
...
- 32 章 [Invoice / Billing Routine](32-invoice-and-billing-routine-spec.md) (= 請求書発行とサイクル全体)
- 31 章 [Admin Payouts / 支払通知書](31-admin-payouts-reward-notice-spec.md) (= 反対側、 AMD から SU への支払)
- 24 章 [Operations Settings](24-operations-settings-spec.md) (= cron の Run Now と dryRun)
- 04 章 [admin オペ](04-admin-ops.md) (= 月次ルーティン早見表)
- 29 章 [Management Score](29-management-score-and-finance-simulation-spec.md) (= finance 軸の入力に `billing_cycles` 入金状況を利用)
