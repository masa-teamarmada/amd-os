...
cadence: 毎時 0 分 JST (= [38 章 §③](38-l2-extraction-routines-spec.md))。 PWA `/api/cron/hourly-estimate` (= 並行稼働中) と Claude routine `amd-os-l3-ms-progress-extract` の dual writer。
...
通常月の backfill は `/api/cron/monthly-reports-backfill` (= manual operation、 [24 章](24-operations-settings-spec.md))。
...
ステップ行クリックでこのモーダルを開いてはいけない (= 31 章で書いた回帰多発エリア、 `CockpitView.resolveStepModalFromTap()` 経由で stepId 別モーダル)。
...
[24 章](24-operations-settings-spec.md) に詳細。 `pwa-hourly-estimate` の `Run Now` default は `maxItems=3` (= 詰まり確認用)。 全件再推定したい時は `force=1` / `ym=YYYYMM` / `maxItems` の意味を確認してから実行する。
...
| advisor PJ で MS が出る | `projects.project_category='advisor'` の判定、 月次ノート側に寄せる ([04 章](04-admin-ops.md)) |
...
- 38 章 [L2 Extraction Routines §③](38-l2-extraction-routines-spec.md) (= L2 ③ 抽出 routine)
- 01 章 [PJ コックピットの見方](01-pj-cockpit.md) (= 月次モーダル UI)
- 26 章 [Member Ops / Billing / Prompt](26-member-billing-prompts-spec.md) (= mypage 月次進捗表示)
- 27 章 [Knowledge Admin / Tsukuyomi](27-knowledge-admin-tsukuyomi-spec.md) (= dialog 対話型ループ全体)
- 24 章 [Operations Settings](24-operations-settings-spec.md) (= cron Run Now)
