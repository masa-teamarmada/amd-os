...
上記以外の PJ は `社内版プレビュー` を表示する。提出版は `monthly_reports_external.body_md` を「業務遂行レポート」本文へ優先採用する。先月提出版で使っていた表紙・エグゼクティブサマリ・進捗/成果・Gantt・実施体制・次月計画・添付資料の帳票構成はそのまま残し、提出版だけ章ごとの強制改頁を外して自然改頁で流す。印刷ビューを開いて Cmd+P → PDF保存でも出力できる。
