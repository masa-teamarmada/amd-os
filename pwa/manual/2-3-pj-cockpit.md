...
## 🏛 会社概要タブ + 要対応 (2026-06-15 ガバナンス欄として追加、2026-07-16 常設「会社概要」タブへ統合)
...
- **資本政策ワークスペース（cap table 統合グリッド + 次回ラウンド試算）** (2026-07-16 追加、同日 UX 再設計で1つのワークスペースに統合): cap table は「設立・Seed・QST現物出資」等のイベント列を横に並べた1枚のグリッドで見る。各イベント列の見出し (日付+ラベル、クリック可能) の直下にそのイベント時点の株主構成を縦積み100%グラフで表示し、共通の0-100%スケールで隣の列と比較できる。グラフの下には発行済株式・新規発行・調達額・発行価額・pre/post-money・株主ごとの持株数と比率が、イベント列ごとにコンパクトな表として並ぶ。正史が設立イベントから始まっていないPJでは「創業時のシェアを遡って再現できない」旨の警告を出し、その場で創業時の株式イベントを追加する導線を出す。表の一番右の列が「**次回ラウンド（試算）**」で、想定pre-money・調達額・ストックオプションプール目標・転換前証券を試算に含めるかの明示チェック・保護したい株主と目標最低比率を入力すると、その場で試算結果 (発行価格・新規投資家の取得株数・完全希薄化後の内訳・保護株主の比率を維持するために必要な最低pre-money) が同じ表の最後の列に他のイベント列と同じ形式で表示される。イベント列をクリックして試算のベース断面を切り替えられるので、確認と試算の入力を画面内で往復せずに行える。試算コントロールは表と同じ画面内 (広い画面は右側、狭い画面は表の直前) にあり、下にスクロールする必要はない。入力していない転換前証券の株数を勝手に見積もることはない。**この試算欄はあくまで仮のシミュレーションで、保存されず、正式な現在値のcap tableや台帳には反映されない**。Excel出力にも同じ計算式で「ラウンド別cap table」「次回ラウンド試算」の2シートが追加される。
