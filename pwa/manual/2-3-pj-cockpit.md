...
4. 協力機関パイプラインで、上部の小集計帯（対応中件数 / 相手ボール件数 / SXボール件数 / ボール未確認件数 / 期限未設定件数）を見てから、機関ごとの現在の関係段階→現在ボール（どちら側+実名の担当）→次の受け渡し（次に渡る相手）→次の一手（実際に次に起こること。受け手や目標状態と別行で常に表示される）→目標状態→期限（精度付き）→最終接点→合意/未合意→関連ゲート→履歴件数/最新記録を同じ行（画面幅1280px以上）またはカード（1280px未満、768〜1279pxは空白を作らないカード内2カラム表示）で見る。期限は精度が「月」なら「2026年8月（日付未確認）」、「未確認」なら「期限未設定」と表示し、存在しない具体日を出さない。各機関の行/カードから「履歴」を開くと、やり取り履歴（面談・メール・合意・成果物・引き継ぎ・状況更新・メモ）の全件タイムラインと、相手の約束/SX側の次アクションを同じ画面で確認できる。閉じた状態でも最新記録の要約は見える。最新記録は発生日が未確認の場合、記録した日時を基準に並ぶ（未確認だからといって発生日が確認済みの記録より新しいとは限らない）。管理者はここから履歴の追加・編集ができる。優先度低・保留のファインケムはパイプライン本体には出ず、末尾の注記だけに残る。
