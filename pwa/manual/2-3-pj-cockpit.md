...
SU 系 PJ では、AMD スコアグラフ / M-X-F / XRL グラフは常時表示し、その下で **進捗管理** と **スコア詳細** をタブ切り替えする。進捗管理タブは従来のコックピット本文、スコア詳細タブは `/venture-map/amd-score/{projectId}` 相当の Score / 経時 / M-X-F 詳細 / FRL / XRL チェックリストを cockpit 内に埋め込む。
