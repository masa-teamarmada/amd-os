...
1. 4本柱カードで、現在ゲート・derived状態・予定/予測差分・担当・次の成果・最大論点・確認日・確度を同じ軸で比較する。現在ゲートは完了済みの古いゲートではなく、次に進む未完了ゲートへ移る。
2. 全体ガントで2026年7月から2027年3月までを見て、行を押す。計画・予測・実績・仮置きの凡例と遅延日数を確認し、同じ詳細面で論点、複数仮説、根拠、反証/不足、検証、意思決定、action、協力機関、担当、依存を辿る。
3. `目的から成果・測定値まで`で目的→4本柱の成果→KPI→ゲートのつながりを見る。KPIの基準値・目標・実績・閾値・測定日・根拠・確度を確認し、技術試験、資金、体制/RACI/容量の測定値へ進む。
4. `意思決定の記録`と決定ループで、会議後の判断が次action（担当・期限・完了条件・次回確認・証跡）へ閉じているか確認する。協力機関は合意済み/未合意、段階、次の約束、期限、担当、関連ゲート、約束履歴を見る。
5. 最後に週次エフォートを入力し、柱・接続ゲート・次の成果へ紐付ける。これは実行能力とデータ鮮度の補助面で、経営判定の代替にしない。AMD OS側の4人を研究チーム全体とみなさず、「研究側メンバー未確認」を体制上の不足として扱う。
