...
年度内ロードマップの直後、進捗管理タブ内には **連携シーズ一覧** を表示する (2026-07-20 導入 / 2026-07-20 SPS 対応)。工学院大学の Seeds (シーズ一覧の同じテーブルを再利用、別台帳は作らない) を、カードではなく **比較優先のテーブル** で並べる。SPS (Seed Prospect Score — 全国全シーズ共通の評価。KUTE 専用スコアではない) を筆頭列に、`M` / `P` / `R` / `S` を個別列、`TRL` / `BRL` / `GRL` / `SRL` / `HRL` を個別列として横比較できる。加えて想定用途・最初の顧客、市場規模レンジと確からしさ、最大の事業化ネック、知財状況、事業化タイプ (大型スタートアップ / 10億円級スモールビジネス / ライセンス / JV・M&A / 共同研究・PoC、主1つ+副複数)、次の検証、深掘り資料の有無を同じ行で比較できる。長文と計算済みの SPS / M / P / R / S / XRL 内訳は詳細モーダルで確認できるが、評価根拠・評価者は内部専用のため出さない。SPS の計算に必要な軸が1つでも欠けているシーズは「未評価」とし、部分点を総合点として出さない。深掘り資料があるシーズはOS内プレビューモーダルで確認でき、無いシーズは「資料なし」と表示する (実在しないリンクは出さない)。
