...
├─────────────────────────────────────────────────────┤
│  PRS primary     │ legacy M/X/F  │ XRL 進捗グラフ      │
├──────────────────┬──────────────────┬──────────────┤
│  タブ: 進捗管理 / スコア詳細                         │
├──────────────────┬──────────────────┬──────────────┤
...
- 現行 primary は PRS (`P x R x S`)
  - `P` = Potential / 潜在規模
  - `R` = Reach / Readiness (= TRL/BRL/GRL/SRL/HRL)
  - `S` = Survival (= `σ_SU` + FRL + R_net)
- legacy M-X-F / 7軸 Cobb-Douglas は comparison と evidence 用に残す
...
- 右上 pill (大数字) は **現在の PRS primary**。P/R_net 未入力時は review pending として止め、legacy AMD を主表示に戻さない
- M/X/F カード = legacy comparison。現在の `M` (MACROTREND) / `X` (XRL) / `F` (FRL) 内訳
