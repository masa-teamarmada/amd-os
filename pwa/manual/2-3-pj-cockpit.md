...
- 表示: 会社基本情報 (登記・定款等の現在値)、**cap table** (発行済み株数・完全希薄化後株数、確定した株式イベントを積み上げた100%資本構成の推移グラフ)、資金調達ラウンド/バリュエーション、転換前証券 (J-KISS・SAFE等。現在の持株比率には混ぜず、転換シナリオとして別枠表示)、**総会・取締役会履歴一覧**と決議・AMD対応 (委任状提出、書面決議への同意など)、年度決算、そのPJの確定済み要対応。登記済み株式数とcap table合計が一致しているかの照合 (tie-out) も表示する。
- **cap table 履歴マトリクス + 次回ラウンド試算** (2026-07-16 追加): cap table は現在の一覧だけでなく、確定した株式イベント (設立・新規発行・現物出資等) から株主名ごとの100%推移タイムラインと、設立から現在までのラウンドを横並びにした「ラウンド別cap table」マトリクスを表示する。正史が設立イベントから始まっていないPJでは「創業時のシェアを遡って再現できない」旨の警告を出し、その場で創業時の株式イベントを追加する導線を出す。あわせて「次回ラウンド試算」欄では、現在の断面・想定pre-money・調達額・ストックオプションプール目標・転換前証券を試算に含めるかの明示チェック・保護したい株主と目標最低比率を入力すると、発行価格・新規投資家の取得株数・完全希薄化後の内訳・保護株主の比率を維持するために必要な最低pre-moneyを試算できる。入力していない転換前証券の株数を勝手に見積もることはない。**この試算欄はあくまで仮のシミュレーションで、保存されず、正式な現在値のcap tableや台帳には反映されない**。Excel出力にも同じ計算式で「ラウンド別cap table」「次回ラウンド試算」の2シートが追加される。
