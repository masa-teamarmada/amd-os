...
月次モーダルの `社内版を編集` は全 PJ 共通の `monthly_reports` 本文を生成・修正・FIXする場所。ここで確定した本文が AMD 社内保存用の正本で、提出用テンプレートの本文ソースにもなる。

CX (`p20`) / SX (`p21`) / KUTE (`p25`) は月次提出が必要なため、月次モーダルのヘッダと本文エリアに提出先を明記したリンクを出す。各リンクは印刷ビューへ `template` query を付けて開き、提出先ごとの構成へ分岐する。それ以外の PJ は `社内版プレビュー` を表示する。

| PJ | 提出用リンク | 出力フォーマット |
|---|---|---|
| CX (`p20`) | `NIMS提出版` | NIMS Pilot CX月次レビュー |
| SX (`p21`) | `愛媛大提出版` | 愛媛大学 SX月次報告 |
| KUTE (`p25`) | `工学院提出版` | 工学院大学 KUTE月次報告 |

上記以外の PJ は `社内版プレビュー` を表示する。`社内版プレビュー` は表紙・要約・進捗・工程表・実施体制・次月計画・添付資料を持つ社内レビュー帳票を維持する。提出版は `monthly_reports_external.body_md` を正本とし、2026年6月の KUTE 実提出版と同じく、契約情報から来月予定までの章を1本の連続文書で表示する。章ごとの強制改頁は入れず、自然改頁で流す。印刷ビューを開いて Cmd+P → PDF保存でも出力できる。
