...
### 最初の1画面の読み方 = 経営状況図（2026-07-25更新）

最初の1画面は、上から4つの帯で読む。目的は、スクロールせずに「予定どおりか / どの経路のどこに居るか / どこが止まり誰がボールを持つか / 次に何へ介入するか」へ答えること。

1. **経営判定**: 左端の大きな判定語（オンスケ / 要注意 / 危険 / 判定不能）と理由、重大未確認・停止の件数、次期限、設立判断日。
2. **4本柱の信号**: 柱ごとの状態・予定差・現在ゲート・次期限・証拠充足・最大の詰まり。柱の色は下の経営状況図のノードの色と対応する。
3. **経営状況図**: 重要経路を左（今日）から右（設立判断）へ一本のレールで描く。ノードの間隔は予測日の日数に比例し、`今日`の印は現在地ノードの直前に付く。各ノードには予定→予測と、遅延幅を長さで示すバー（縦線=予定、◇=予測）、担当が付く。破線は仮日程。停止・対応待ちの旗はそのノードの真下にぶら下がり、誰（組織・実名）がボールを持ち、いつまでかを示す。ノードに接続できない対応待ちは「経路ノード未接続の対応待ち」として別に明示する。
4. **次の経営介入**: 優先順の上位3件だけを①②③で表示し、同じ番号が経営状況図の旗にも付く。各行に対象・ボール/担当・期限・止まるゲートが並ぶ。それ以外の予定は「直近アクション」の1行に畳む。
