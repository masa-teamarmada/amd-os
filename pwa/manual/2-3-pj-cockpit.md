...
年度内ロードマップの直後、進捗管理タブ内には **連携シーズ一覧** を表示する (2026-07-20)。工学院大学の Seeds (シーズ一覧の同じテーブルを再利用、別台帳は作らない) をカード形式で並べ、想定用途・最初の顧客、市場規模レンジと確からしさ、最大の事業化ネック、知財状況、事業化タイプ (大型スタートアップ / 10億円級スモールビジネス / ライセンス / JV・M&A / 共同研究・PoC、主1つ+副複数)、次の検証、将来性60・現在地30・KUTE支援効果10の採点状態、深掘り資料の有無を一覧だけで比較できる。長文と8項目の採点内訳は「詳細を見る」で確認する。全項目が揃うまで総合点は「未評価」とし、部分点を総合点にしない。深掘り資料があるシーズはOS内プレビューモーダルで確認でき、無いシーズは「資料なし」と表示する (実在しないリンクは出さない)。
