...
月次モーダル右上の名称は全 PJ 共通で `社内版` / `提出版` に固定する。クライアント名は帳票の契約情報に表示し、操作名へ埋め込まない。`社内版` は `monthly_reports` の本文を開き、`本文を編集` / `保存` / `確定` で直接扱う。`提出版` は印刷ビューを新しいタブで開く。
...
社内版は固定8見出しのナラティブ本文を正本とし、表紙・要約・進捗・工程表・実施体制・次月計画・添付資料を持つ既存のリッチ帳票を維持する。提出版も先月提出版と同じリッチ帳票を維持し、`monthly_reports_external.body_md` を業務遂行レポートへ差し込む。提出版だけ章ごとの強制改頁を入れず、自然改頁で流す。自社メンバーは姓、府省共通研究開発管理システムは `e-Rad` と表記する。
