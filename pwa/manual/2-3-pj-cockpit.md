...
`proactive_outbox` に積まれた、その PJ の「次に AMD から打つべき一手」を read-only で表示する。外部送付や状態更新はこのカードからはしない。

表示するもの:
- 状態: 未送信 / 司令塔送信済み / 下書き済み / 相手へ送信済み / 完了 / ブロック
- 誰のボールか: AMD / 相手 / 双方 / 曖昧
- 期限、優先度、下書き種別、トリガー理由
- 担当司令塔 thread の短縮ID
- 推奨 first move と、遅れた場合のリスク

対象 status は、初期表示では `queued`, `sent_to_commander`, `drafted`, `blocked`。`closed` や `sent_to_counterpart` は完了・送付済みの履歴であり、通常の「今日打つ一手」には出さない。RLS は admin の authenticated read 前提なので、権限がないユーザーにはキューは表示されない。
