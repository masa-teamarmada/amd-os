...
SXの共有面を開いたら、まず上部の経営判定を見る。判定・理由・今週決めること・次期限・最終確認日・必須充足率が連続している。`順調`は入力率の高さではなく、重要ゲートの必須KPI、担当、期限、鮮度、依存、完了条件、閾値、actionの状態をすべて満たした場合だけ出る。`重大な未確認`、停止中の柱、期限超過があれば必ず画面に残る。
...
3. 全体ガントで2026年7月から2027年3月までを見て、行を押す。計画・予測・実績・仮置きの凡例と遅延日数を確認し、同じ詳細面で論点、複数仮説、根拠、反証/不足、検証、意思決定、action、協力機関、担当、依存を辿る。
4. 協力機関パイプラインで、機関ごとの現在の関係段階→期限付きの次の状態（SX側の次アクション・期限）を、合意/未合意・担当・最終接点・関連ゲートと同じ行で見る。優先度低・保留のファインケムはパイプライン本体には出ず、末尾の注記だけに残る。
...
390pxでは4本柱、論点、協力機関、週次エフォートをカードへ組み替え、ガントは期限順の縦ロードマップにする。desktopの表や左ナビを縮小して横スクロールさせない。選択状態、フォーカス、無効、読み込み、空、エラーを色以外の文言でも確認する。
