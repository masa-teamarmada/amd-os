...
- **資本政策プラン台帳** (2026-07-17 追加、旧・単発で保存されなかった「次回ラウンド試算」欄と「保護株主」概念を置き換え): cap table グリッドの下に、設立からIPOまでの複数ラウンドを名前を付けて保存できる作業台帳が出る。**シナリオは名前を付けて複数保存でき**、入力すると約800ms後に自動保存される (自分で保存ボタンを押す必要はない)。他のメンバーが同時に同じシナリオを編集した場合は、保存時に競合を検知して最新のサーバー内容を提示する。株数・金額・単価・評価額など台帳上のあらゆる数値は自由に編集でき、それぞれ「入力値・自動計算・確定値・取込・手動上書き」のどれかの由来ラベルが付く。自動計算の値も手動で上書きでき、上書きした値と元の自動計算値の両方を後から確認できる。**「保護株主」という概念はこの台帳には無い**。増資イベントごとに株主の目標保有比率を個別入力するだけで、特定の株主を自動的に守る仕組みは作らない。イベント列に縦積み100%グラフを揃えて表示し、株数が整数でない・比率合計が100%にならない・pre/post-moneyが不整合、などのエラーが1つでも残っている間は**確定（freeze）ボタンが押せない**。確定すると、その時点の内容が改変不可の確定版として1件追加され、過去の確定版は履歴として残り続けて後から書き換えられない。**VC向けのExcel出力は、この確定版からしか作れない**（作業中の未確定シナリオからは出力できない）。スマホ幅では、表やグラフの代わりに選択中の1イベントだけを◀/▶で切り替えながら編集する画面になる。
