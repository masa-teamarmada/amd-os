...
| D-10 Member Activity Evidence | `member_activities` | Dashboard / MyPage「今週やったこと」の根拠 | PWA `member-weekly-activities` route | 現行の Mac/MMO writer はどちらもこの route を叩く。内部では `@anthropic-ai/sdk` の従量課金が残るが、**まさ判断で D-10 は例外許容**。writer の起点は Codex 側 |
...
| H-1 | H-1 MTG サマリ + フロー | Windows MMO Codex Desktop automation | `amd-os-l6-meeting-flow` / SKILL `amd-os-l6-meeting-extract` | 毎日 09:00-21:00 毎時 | MMOマシン側 automation 履歴、`amd-os-l6-meeting-extract/SKILL.md` |
...
| D-10 | D-10 Member Activity Evidence | **3系統並走 writer (2026-06-12 まさ確定)**: Claude routine `amd-os-l2-consolidated-evidence` Phase J / Mac Codex automation `amd-os-l2-2` / MMO Windows Task Scheduler launcher `amd-os-l2-member-weekly-activities`。**2026-06-16 current truth** では、Mac / MMO の Codex writer を継続し、内部で叩く PWA route の Anthropic API 依存は D-10 のみ例外許容とする | 全系統が PWA route `cron/member-weekly-activities?interactive=1` を叩く。`member_google_oauth_tokens` の per-member refresh_token で全メンバー本人として Gmail/Calendar を読む。窓単位 delete-then-upsert + UNIQUE 制約で複数 writer でも重複行はできない。将来は抽出本体を Codex automation 内へ寄せる余地があるが、今は「Codex 側 writer が route を起動する」構成を current truth とする | daily 08:00 / 18:30 / 19:30 JST | `member_activities(source='member_weekly')`、Dashboard / MyPage read evidence |
...
**Calendar work-block review (= dry-run / admin確認)**:
- MTGカード / 議事録 / Gmail TODO / Slack TODO から Calendar 作業枠候補を作る時は、まず `POST /api/meeting-calendar/upsert-plan` または `POST /api/task-calendar/schedule-plan` の dry-run を通す。
- admin は `/admin/calendar-review` で planner payload を確認する。表示対象は `plans`、`calendar_writes`、`slack_nudge_candidates`、`review_reasons`、`duplicate_match`。
- `/admin/calendar-review` は write bundle を作る画面であり、Calendar event 作成、Slack DM送信、Gmail返信、外部attendee招待は実行しない。
- 実writeへ進む場合は H-1 / Calendar connector 側で、対象・件数・rollback・通知有無を確認してから行う。PWA route は `dry_run=false` / `execute=true` を `calendar_write_disabled` で拒否する。
