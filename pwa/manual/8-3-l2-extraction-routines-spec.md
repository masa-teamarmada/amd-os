...
| H-1 | (MMOマシン Codex Desktop automation `amd-os-l6-meeting-flow`、Claude routine 化しない) | 毎時 09:00-21:00 JST。毎時runは並行実行を維持し、待機・実行ロック・別runを理由にしたskipは禁止。報告は AMD OS 通知箱 (`app_notifications.kind='h1_report'`) へ送る。Codex表示は毎時スレッドを残さず、控えとしてJST日付ごとの `H-1 YYYY-MM-DD 日次まとめ` スレッドへ追記する (registry-first / direct-create 固定、query 付き `list_threads` 禁止)。毎時runスレッドは OS通知が成功したら必ずアーカイブする (日次まとめ追記の成否とは独立) | H-1 |
