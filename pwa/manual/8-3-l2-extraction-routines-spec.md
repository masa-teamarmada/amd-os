...
| control | つくよみ 書き漏らし検出 | Codex automation + 専用outbox applier | `4-35` | daily 04:35 JST | 前日以降の議事録と問い・やることを意味照合し、PJあたり最大5件を提案へ。抽出はGETとJSON作成だけ |
...
| W-Prep | visible prep thread launch | Codex automation `w-prep-launch-2` | 実行日から数えて7日後の23:59:59.999 JSTまでの確定 upcoming MTG を Calendar + DB で照合し、必要な prep thread を作る。厳密な `now()+7 days` で切らず、DBだけで完了扱いにしない。Calendar直読みでは `CFG_ColorPJHistory` を先に使い、`2025-06-01` 以降の `colorId=4` は SX/p21、`SolvioraX` は SX high-confidence alias として扱う。重複防止は `calendar_event_id` exact identity と `meeting_id='upcoming:<calendar_event_id>'` canonical を優先する。thread は PJディレクトリ target、`{meeting_title} prep` title、pinを`list_threads`でreadbackする。workerは契約範囲・同シリーズ/PJ横断の履歴・未完了action・保留・直近チーム入力を論点台帳へ集め、各論点の二次影響と今回prepへの配置を `l6_prep_scope_coverage_gate.cjs` で検査する。読取り・正規の準備状態保存・当日AI Meeting Notes context・opening briefは追加承認なしで自動完遂し、自動審査が却下した時だけ理由を保存して止める。共有資料と通常Notion draftはまさの明示write後にHTMLで作る。session ID・pin・readiness・scope coverage・Notion gate・brief実表示がそろった時だけ`ready`にする | weekly Wednesday 15:00 JST | `/Users/masa/.codex/automations/w-prep-launch-2/automation.toml`、`pwa/scheduled-tasks/amd-os-l6-meeting-prep-worker/SKILL.md` |
