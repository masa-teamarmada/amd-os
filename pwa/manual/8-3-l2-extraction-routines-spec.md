...
| D-10 | ⑬ Member Weekly | (Claude routine target / 旧 PWA route) | `cron/member-weekly-activities` (退避済)。2026-06-12 から `member_google_oauth_tokens` の per-member refresh_token で **全メンバー本人として** Gmail/Calendar を読む (token 無し connected メンバーはまさ代理 calendarList fallback)。token は PWA ログインで自動保存 | daily (target) | `member_activities(source='member_weekly')` |
