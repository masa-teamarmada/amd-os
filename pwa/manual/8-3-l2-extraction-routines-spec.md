...
> **2026-08-29 M系 current**: `AMD OS L2 月次抽出・毎月25日 (M-1月次レポート/M-2 XRL/M-3経営シグナル)` はClaude Code RoutinesでFable 5固定。M-1はkaku-report必須、旧ローカルScheduled Task・CLI・従量課金API・別モデル・subagent/workflowへのfallbackは禁止。cronはUTC `0 7 25 * *` (= 毎月25日 16:00 JST) で、JSTの25日だけ本処理を行う (2026-08-29 まさ確定、旧: 月末最終日)。
...
- **Claude Code Routine `amd-os-l2-monthend-evidence`** = 表示名「**AMD OS L2 月次抽出・毎月25日 (M-1月次レポート/M-2 XRL/M-3経営シグナル)**」(Fable 5固定、毎月25日16:00 JST発火、cron UTC `0 7 25 * *`、Phase 0で25日判定、17:00完了): M-1〜M-3を依存順に1本で実行する。M-3 (Management Signal) を月次振り返りMTG前に出揃わせる。
...
| M-3 Management Monthly Signal Evaluation | `company_management_signal_reviews` | Management予実表から月次評価を作る | 専用Codexチャット/heartbeat案 | Claude routine別枠、毎月25日17:00 JST。UI証跡必須 |
...
| M-1〜M-3 | `amd-os-l2-monthend-evidence` | 毎月25日 16:00 JST 発火 (cron UTC `0 7 25 * *`)、25日判定、17:00 完了 | M-1M-2M-3 |
...
| W-Prep | visible prep thread launch | Codex automation `w-prep-launch` | 実行日から数えて7日後の23:59:59.999 JSTまでの確定 upcoming MTG を Calendar + DB で照合し、必要な prep thread を作る。厳密な `now()+7 days` で切らず、DBだけで完了扱いにしない。Calendar直読みでは `CFG_ColorPJHistory` を先に使い、`2025-06-01` 以降の `colorId=4` は SX/p21、`SolvioraX` は SX high-confidence alias として扱う。重複防止は `calendar_event_id` exact identity と `meeting_id='upcoming:<calendar_event_id>'` canonical を優先する。thread は PJディレクトリ target、`{meeting_title} prep` title、pinを`list_threads`でreadbackする。workerは契約範囲・同シリーズ/PJ横断の履歴・未完了action・保留・直近チーム入力を論点台帳へ集め、各論点の二次影響と今回prepへの配置を `l6_prep_scope_coverage_gate.cjs` で検査する。黙った欠落、期限/ownerなしの先送り、欠けた全体日程があればreadyにしない。まさが開く最後の応答は、`前回までの流れ / 今回の論点 / 推定着地 / まさがやること / 相談入口`を具体的に含むopening prep brief。初回は共有資料と通常Notion draftを作らず相談から始め、Notion AI Meeting Notes contextだけappend-onlyで入れる。共有資料はまさの明示write後にHTMLで作る。session ID・pin・readiness・scope coverage・Notion gate・brief実表示がそろった時だけ`ready`にする | weekly Wednesday 15:00 JST | `/Users/masa/.codex/automations/w-prep-launch/automation.toml`、`/Users/masa/.codex/automations/w-prep-launch/memory.md`、`pwa/scheduled-tasks/amd-os-l6-meeting-prep-worker/SKILL.md` |
...
| M-3 | M-3 Management Signal | (Claude routine target、新規) | M routine Phase C inline | 毎月25日 | `company_management_signal_reviews`、`/management-score` |
