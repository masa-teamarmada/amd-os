...
**2026-06-02 正本拡張**: L2 ⑪ Atlas Signals、⑫ Macrotrend Evidence/Index、⑬ Member Weekly Activities を L2 evidence taxonomy に追加する。①〜⑩は 5 生データ由来の internal operating-memory layer として維持し、⑪⑫は external / numeric evidence、⑬は member weekly internal hybrid evidence として provenance を明示する。PWA / Vercel background LLM cron は復活させない。
...
- **Subscription synthesis mode**: subscription token を使う場合でも、L2ごとに細かく routine を増やさず、`daily-internal-l2-synthesis`、`daily-external-evidence-review`、`weekly-member-activity-synthesis`、`weekly-atlas-macro-synthesis` のように束ねる。atomic evidence collection と derived report synthesis は分ける。
