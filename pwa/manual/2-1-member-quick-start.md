...
左メニューは「研究ポートフォリオ」グループが最上位にあり、**ホーム**（= `/dashboard`）・**研究機関**・**シーズ**・**PJ運用** の4つを切り替えられる。ホームにマウスを重ねると、いまアクティブなPJが右側に一覧で開く。目的のPJを選べば、そのままPJコックピットへ進める。「PJ運用」はホーム下段の全PJ一覧へ直接移動する。
