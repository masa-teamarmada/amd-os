...
左メニューは「PJポートフォリオ」グループが最上位にあり、**ホーム**（= `/dashboard`）・**研究機関**・**シーズ**・**PJ運用** の4つを切り替えられる。ホームにマウスを重ねると、いまアクティブなPJが右側に一覧で開く。さらに目的のPJへマウスを重ねると、Chromeのブックマークフォルダのように右側へ移動先が開く。**コックピット**はAMD内部の判断・経営情報を見る面、**ワークスペース**はPJの共有作業面。使いたい面を選んで進む。「PJ運用」はホーム下段の全PJ一覧へ直接移動する。
