...
発行不可のblocker条件: `月次台帳未生成`、`freee取引先未設定`、`請求額なし`、`立替未承認 N件`、`月報未確定`。未承認は月報に関係なく必ず発行を止める。担当PMまたはadminが申請中を「PM承認」、adminがPM承認済みを「admin承認」できる。クリック時だけサーバーが権限と遷移元statusを検査する。freee発行側も送信直前に未承認/月報/発行済みを再検査し、画面の候補額と同じ月帰属・statusの立替だけを明細へ載せる。
