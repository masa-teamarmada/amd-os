...
### 何をする画面か
月次の請求書発行ページ。締め済み稼働月のうち、請求額があるものだけをきよが上から処理する。画面は `発行待ち / 要確認 / 設定不足 / 過去滞留` を分け、対象月の `発行待ち` 行だけ「請求書を発行」から OS 上で明細確認 → freee 請求書発行まで進める。
...
請求書発行モーダルは、件名、基本明細行、承認済み立替、調整行、請求日、支払期日、備考を確認してから freee 発行する。月額固定契約で基本行合計が契約月額と違う場合は、差分を確認してから発行する。
