...
`/admin/change-history` は、OS内のデータ変更を新しい順に読むadmin専用の監査台帳。初期表示は「実行者・時刻・PJ・対象名・何をどう変えたか」の短い日本語要約を1〜2行で表示する。同一transactionの連続行に加え、service roleが同じテーブルへ1秒以内に連続実行した別transactionの行は、表示上の「同じ自動処理」として件数と追加・変更・削除の内訳へまとめる。メタ監査行（`project_management_field_audit`）はDBの生ログには残すが、二重表示で本体変更を埋めないよう台帳からは除く。画面、iOS、macOS、GAS、定期処理のどこから書き換わっても、DBの同一トランザクションで記録する。
