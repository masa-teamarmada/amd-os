...
- 押してから反映されるまでは数秒。2026-08-13 に、ボタンの押下を受け取る仕組みを外部の中継サーバー経由から OS 本体へ直結させた。以前は中継側が1分ごとにまとめて処理していたため、押してから反映まで最大1分待つことがあった。中継の設定はそのまま残してあるので、問題が起きたら送信先を戻すだけで元の動きに復帰できる。
