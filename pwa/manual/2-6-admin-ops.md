...
`/admin/change-history` は、OS内のデータ変更を新しい順に読むadmin専用台帳。各行に、実行者、変更日時、対象データ、追加・変更・削除の種別、変更前と変更後をセットで表示する。画面、iOS、macOS、GAS、定期処理のどこから書き換わっても、DBの同一トランザクションで記録する。
