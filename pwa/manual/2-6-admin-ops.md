...
右列は上から: ①契約条件（期間、基本報酬、請求配分、支払条件、立替精算、月報）、②13か月分の請求台帳（稼働月/請求月/基本額/承認済立替/請求候補/状態/発行条件）、③選んだ月の内訳ワークベンチ、の3段。
...
発行不可のblocker条件: `freee取引先未設定`、`請求額なし`、`立替未承認 N件`、`月報未確定`。未承認は月報に関係なく必ず発行を止める。担当PMは申請中を「PM承認」、adminはPM承認済みを「admin承認」できる。クリック時だけサーバーがactive member、PM担当/admin、遷移元statusを検査して条件付き更新する。自動承認や通知はしない。
