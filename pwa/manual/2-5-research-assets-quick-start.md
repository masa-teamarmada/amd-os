...
- マップの線をクリックし、マップ内の確認パネルからそのエッジだけを削除する。右側台帳「接続しているノード」の各行にも同じ削除ボタンがあり、細い線を狙わなくても解除できる。
...
画面はマップ表示と一覧表示を切り替えられる。ノードがある時のスマホは一覧が初期表示だが、0件の時は最初のノードを作れるようマップを出す。検索と層・状態filterはノードを絞り、relation filterはマップ上のエッジだけを絞る。台帳の「外部ソース支持なし」「異議・反証なし」「検証なし」は記録が手薄な場所の合図で、誤り判定ではない。
