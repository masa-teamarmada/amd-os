...
| カラム | 研究機関、研究者/PI、PJ状態、SPS帯(億円)・根拠Lv、旧SPSと各軸を1シーズ1行で比較する |
| SPS帯・根拠Lv (2026-08-15〜) | 一次選別で見積もった「億円レンジ」と、そのスコアの裏付けの深さを表す根拠Lv (Lv0=ネット情報のみ〜Lv3=月次試算表あり) を並べて表示する。列見出しをクリックすると帯の上限で並び替えできる。旧来の9軸ルーブリック (M×P×R×S) は「旧SPS」列に残り、機能・値は変わらない |
...
| 行クリック | 詳細 modal で概要、機関・研究者、AMD 評価、補助金、接触履歴、ニュースを見る。一次選別のスクリーニング帯 (段階仮説・q帯・P帯・SPS帯・根拠Lv・評価根拠) がある場合は「一次選別スクリーニング帯」セクションにも表示する (2026-08-15〜) |
