...
| 行クリック | `/seeds` では内部向け詳細 modal を開き、概要、機関・研究者、AMD 評価、補助金、接触履歴、ニュースを見る・編集する。接触履歴の `+ 追加` から、Slack、Teams、MTG/面談などのやり取りを日付・担当・内容・次の一手とともに残せる。研究機関PJコックピットでは外部共有可能な詳細だけを表示する。一次選別のスクリーニング帯 (段階仮説・q帯・P帯・SPS中央値・根拠Lv・評価根拠) がある場合は「一次選別スクリーニング帯」セクションにも表示する (2026-08-15〜) |
