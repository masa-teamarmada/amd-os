...
| カラム | シーズNo.、シーズ名、会社名、研究機関、研究者/PI、産業創出価値、TRL/BRL/HRL、用途、次の検証、追加研究による市場創出案を1シーズ1行で比較する。追加研究案はAMD側の未検証仮説 |
| 列幅 | 各見出しの右端を左右へドラッグして調整する。左右キーでも変更でき、ダブルクリックで既定幅に戻る。調整結果はこの端末だけに保存される |
...
| 行クリック | `/seeds` とAMD内部の研究機関PJコックピットでは同じ完全版詳細 modal を開き、シーズNo.、概要、機関・研究者、AMD評価、追加研究による市場創出案、補助金、接触履歴、ニュースを見る・編集する。外部共有面だけは追加研究案などの社内項目を返さない。**PJ化しても modal の中身は変わらない** |
...
議事録・Slack・Teamsからシーズ情報を追加するときは、要約だけで終えず構造化項目も確認する。助成金・採択は補助金履歴、接触は接触履歴、ニュース・論文・特許はニュース欄、PJ化はPJ関係へ分けて保存する。根拠にない年度・金額・期間は埋めない。
...
| タブ | 使う場面 |
|---|---|
| `VC一覧` | ファンド残、投資領域、チームアルマダのPJとの接点を見て、誰にアプローチするか考える |
| `投資履歴` | どのVCが、いつ、どのSUへ、いくら出資したかを横断比較する |
| `ニュース受信箱` | 自動・手動収集された未確認ニュースを採否する |

`投資履歴` の「VC個別出資額」はそのVC自身の出資額、「ラウンド総額」はSUが同じ資金調達全体で集めた額。片方しか公表されていない場合は、もう片方を推定で埋めない。
`収集候補` は軽量モデルが公開根拠から見つけた未確認情報、`既存・要確認` は旧台帳から引き継いだ情報、`確認済み` だけが人の確認を終えた情報を表す。
`出資・完了日` と `公表日` も別欄。発表された事実だけで `払込確認済み` や資金受領済みとして扱わない。
公開記事の本文に同じ日付・金額が無い場合は、モデルが返した値を使わず `未確認` と表示する。根拠リンクを開き、SU、VC、ラウンド、金額のどこまで明記されているかを確認して使う。
