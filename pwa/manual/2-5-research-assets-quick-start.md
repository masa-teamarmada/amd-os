...
ノードを通常クリックすると、そのノードの編集パネルが開く。ノードの場所を変える時は普通にドラッグする。ドラッグは配置変更だけで、接続は作らない。

- **メモを追加**: 選択ノードの操作帯にあるこの1ボタンから、根拠・異論・論点・検証などのメモを書く。まずメモ本文を書き、次にそのメモの役割 (支持・異議・反証・未解決論点・検証の5種類) を選ぶ。役割を選ぶと接続の向きは自動で決まり (未解決論点だけ「選択ノードが新しい問いを生む」向き、それ以外は「新しいメモが選択ノードへ向く」向き)、保存前に向きのプレビューが出る。保存後もボタンは残るので、同じノードへ何件でもメモを追加できる。定義する・依存する・上書きする・運用化するの4種は、向きが用途で変わるためメモの役割には出さず、既存の `Cmd`/`Ctrl` を押した2ノード接続から選ぶ。
