...
ノードを通常クリックすると、まずそのノードの小型**閲覧**パネルが開く。種別・層・状態、要約、本文、メモを高密度に読み、数式は式のまま表示される。編集したい時だけ「編集する」を押す。パネルはdesktopで幅約360px、本文は内部でスクロールする。閉じる×はなく、地図の何もない場所をクリックすると閉じるだけで、その同じクリックでは下書きを作らない。別ノードを通常クリックすれば確認なしでそのノードへ切り替わる。ノードの場所を普通にドラッグすると位置が保存され、次のリロードでも残る。過去の座標未保存ノードは散らして表示するだけで、読むだけではデータを書き換えない。
