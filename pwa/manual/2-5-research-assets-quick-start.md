...
作成パネルはページ外ではなく、クリックしたマップの上に出る。背景クリックかEscapeで閉じられる。

編集できるのはadminだけ。一般メンバーには同じ地図が読み取り専用で見える。

### 選んだノードを育てる

ノードを選ぶと、マップ下端に小さな操作帯が出る。

- **根拠を足す**: 新しい文献・証拠を作り、選択ノードを支持する向きでつなぐ。
- **異論を足す**: 新しい文献・証拠を作り、選択ノードへ異議をつなぐ。
- **論点を残す**: 選択ノードから、新しい未解決論点を生やす。

既にある要素をつなぐ時は、始点にしたいノードを相手ノードへドラッグして重ねる。接続パネルでrelationと向きを選び、保存前に表示される「A —関係→ B」を読んで、矢印の向きを必ず確認する。

### 直す・外す

- マップ下端の「編集」から見出し、要約、分類、本文、文献情報を直す。
