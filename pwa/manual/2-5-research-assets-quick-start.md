...
ノードを選ぶと台帳に3つの大きな入口が出る。

- **根拠を足す**: 新しい文献・証拠を作り、選択ノードを支持する向きでつなぐ。
- **異論を足す**: 新しい文献・証拠を作り、選択ノードへ異議をつなぐ。
- **論点を残す**: 選択ノードから、新しい未解決論点を生やす。

既にある要素をつなぐ時は「既存ノードとつなぐ」を押し、相手を検索してrelationと向きを選ぶ。保存前に表示される「A —関係→ B」を読んで、矢印の向きを必ず確認する。

### 直す・外す

- 選択ノードの「編集」から見出し、要約、分類、本文、文献情報を直す。
- 関係行の「接続を外す」は確認後、そのエッジだけを削除する。
- ノード削除は持たない。棄却した理論は消さず、状態を「反証済み」にして履歴を残す。

画面はマップ表示と一覧表示を切り替えられる。スマホは一覧が初期表示。検索と層・状態filterはノードを絞り、relation filterはマップ上のエッジだけを絞る。台帳の「外部ソース支持なし」「異議・反証なし」「検証なし」は記録が手薄な場所の合図で、誤り判定ではない。

データの共有正本はDB。`pwa/bzm/theory-graph/*.md` の21ノード / 34関係は初期スナップショット兼、DB障害時の読み取り専用表示に使う。障害時は上部に読み取り専用の案内が出て、保存操作は隠れる。
