...
作成パネルはマップと同じ作業領域に出る。地図を覆う中央モーダルではなく、PCでは地図の右、スマホでは地図の下に作業区画を確保するので、元のノードを見ながら書ける。Escapeで閉じられる。パネルは常に「いま実際に見えている画面の範囲」に収まるので、マップの下の方の空白をクリックしても、下部の入力欄や保存・削除ボタンが画面外へ落ちない。収まりきらない内容はパネル内側をスクロールして読む。

編集できるのはadminだけ。一般メンバーには同じ地図が読み取り専用で見える。

### 選んだノードを育てる

ノードを通常クリックすると、そのノードの小型編集パネルが開く。パネルはdesktopで幅約360pxのコンパクトなインスペクタで、見出しや飾りの余白を持たず、種別は6枚のカードではなく1つのプルダウンから選ぶ。パネル本体は内部でスクロールし、上下のボタン帯は固定されたまま地図の外へはみ出さない (2026-08-02 改修)。編集パネルが開いたまま別のノードを通常クリックすると、確認を挟まず即座にそのノードの編集へ切り替わる。新規作成の下書きを開いたまま別ノードへ切り替えた場合、その下書きは保存されず消える。地図の何もない場所をクリックしたときは、パネルが開いていればまずパネルを閉じるだけで、同じクリックで新しい下書きは作らない。ノードの場所を変える時は普通にドラッグする。ドラッグは配置変更だけで、接続は作らない。
...
- ノードを通常クリックし、見出し、要約、分類、本文、文献情報を直す。
