...
- **根拠を足す**: 新しい文献・証拠を作り、選択ノードを支持する向きでつなぐ。
- **異論を足す**: 新しい文献・証拠を作り、選択ノードへ異議をつなぐ。
- **論点を残す**: 選択ノードから、新しい未解決論点を生やす。

既にある要素をつなぐ時は、`Cmd` を押したまま始点ノード、接続先ノードの順にクリックする（Mac以外は `Ctrl`）。1つ目には外周のハローと「接続元」案内が出る。2つ目を選ぶと接続パネルが開くので、relationと向きを選び、保存前に表示される「A —関係→ B」を読んで矢印の向きを確認する。同じノードを2回選んでも自己接続は作られない。空白クリックかEscapeで接続待ちを解除できる。
