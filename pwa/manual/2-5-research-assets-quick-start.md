...
| 元素 | 全118元素の周期表。総合注目度 / 需要 / 供給不安 / AMD相性の色を切り替え、cell内の日本語主用途と供給`警戒` / `危機`で全体を読む。評価済み元素を押すと相場・推移・産出国構成がmodalで開く |
...
4. 候補を `比較に追加` し、最大4件まで同じ軸で比べる
5. AMD OS 内のノウハウやPJ事例を探すときだけ `材料マップ` を開き、検索・domain filter・node detailから正本画面へ戻る
