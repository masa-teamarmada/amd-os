...
| tab | 見るもの |
|---|---|
| 全体 | カード全面から元素・鉱物・樹脂へ移動し、4指標合計（20点満点）が高い材料から見る |
| 元素 | 全118元素の周期表。注目度 / 需要 / 供給不安 / AMD相性の色を切り替え、cell内の日本語主用途と供給`警戒` / `危機`で全体を読む。評価済み元素を押すと相場・推移・産出国構成がmodalで開く |
| 鉱物・鉱石 | 元素がどの鉱物・鉱石から得られ、精製後にどの用途へ向かうか |
| 樹脂・高分子 | 4指標合計の高い順に、原料、製造プロセス、物性、用途、循環性、規制・代替、AMDとの接点を見る |
...
2. 周期表cellの主用途で候補を絞り、cellを押す。最初のmodalでは用途、供給警報、代表品目の直近公表相場、5年価格推移、産出国構成を見る。相場は単位・代表品目・公表時点を確認し、live価格として扱わない。生産量が非公表の元素では輸入供給元などの代替指標を円グラフ注記で確認する
3. さらに必要なときだけ`詳細を開く`を押し、同じmodalを拡大して特徴、埋蔵・需給メモ、代替・循環性、AMDとの接点、sourceまで読む。`GaN power device`や`RF`のような略語だけでなく、用途の意味を日本語で表示する
4. 鉱物・樹脂の一覧は `注目度 + 需要 + 供給不安 + AMD相性` の合計が高い順に読む。樹脂detailでは「原料」と「作り方」を確認する
