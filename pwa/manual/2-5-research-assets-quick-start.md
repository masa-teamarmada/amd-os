...
| 行クリック | `/seeds` とAMD内部の研究機関PJコックピットでは同じ完全版詳細 modal を開き、概要、機関・研究者、AMD 評価、補助金、接触履歴、ニュースを見る・編集する。接触履歴の `+ 追加` から、Slack、Teams、MTG/面談などのやり取りを日付・担当・内容・次の一手とともに残せる。外部共有面だけは社内項目を返さない。一次選別のスクリーニング帯 (段階仮説・q帯・P^ind帯・P帯・SPS中央値・根拠Lv・評価者/評価日時/版) がある場合は「一次選別スクリーニング帯」セクションに表示し、その下に評価の総合判断 (段落) と q要因11項目 (折りたたみ) を出す (社内モーダルでの表示は2026-08-20〜)。**PJ化しても modal の中身は変わらない** — PJ化済みでも未PJ化と同じセクション構成で、PJへの導線はヘッダーのPJバッジのリンクだけ (2026-08-20〜) |
