...
月初はまず `/monthly-agreement` で、各PJの「担当内容」と「その対価としての予定額」の2点を確認して合意する。この合意がないままでは、その月の支払いに進めない。モーダル上部の `合意前に必ず確認すること` には、PJごとに担当内容を一度だけ置き、その右に担当MSを並べる。予定額合計と全PJの予定額も実データで同じ枠に表示する。月次の到達目標は現在の snapshot に無いため、MS名を目標として表示しない。`確認して合意` を主ボタンとし、`修正要望` はその右の小さいボタンから開く。必須枠より下は、カードにせず `参考情報` の短い区切りだけを置く。`支払い状況と対象PJ` は初期状態で閉じ、開いたときだけ合計とPJごとの支払い内訳を表示するため、同じ情報を下段のPJカードで繰り返さない。未合意または条件更新ありの間は、OS内の他画面を開くと、開いた画面の上に月初合意モーダルが表示される。背景クリックで一時的に閉じられるが、合意した扱いにはならない。未合意のままダッシュボードなどを開き直すとまた表示される。その後は「マイページ / PJ コックピット / 通知レビュー / 立替」を毎日 1 回ずつ見て、未対応カードを片付けるのが日常の最小単位。
...
`/monthly-agreement` では、当月参加中の PJ、担当MS / share、担当内容、月初合意用の想定報酬を確認する。月次の到達目標は現時点で定義・保存していないため、MS名を目標として見せない。cap / carry-over / 条件/前提 / 未確定・要確認などの内部確認情報は本人画面に出さない。`status='frozen'` / `freeze_from_ym <= ym` / active `project_freeze_periods` の PJ、当月報酬も担当MSもないPJは表示対象外。`members.exclude_from_payout_notice=true` かつ `is_admin=false` のメンバー (= りり / ID006 NIMS 無償出向、あき / ID029 無報酬稼働) は月初合意も対象外。`is_admin=true` のメンバーは、テスト確認のため支払通知対象外でも月初合意対象に含める。報酬キャッシュがあるPJで担当MSの報酬行がない場合は、未確定ではなく `0円` と表示する。`reward_summary_json.members[].stockYen > 0` の場合だけ、支払予定とは分けて翌月以降へ繰り越される残額として `今月末未払い残（今月は支払われない）` を read-only 表示し、前月繰越・今月発生・今月支払の内訳を添える。合意ボタンを押すと `member_monthly_work_agreements` に表示内容の `snapshot_json` と `snapshot_hash` が保存される。
