...
月初はまず `/monthly-agreement` で今月の遂行対象・想定報酬・条件を確認して合意する。未合意または条件更新ありの間は、OS内の他画面を開くと、開いた画面の上に月初合意の必須モーダルが表示される。背景クリックなどでは閉じられず、合意が完了した後だけ通常画面に戻れる。その後は「マイページ / PJ コックピット / 通知レビュー / 立替」を毎日 1 回ずつ見て、未対応カードを片付けるのが日常の最小単位。
