...
月初はまず `/monthly-agreement` で、各PJの「担当する内容と今月の到達目標」と「その対価としての予定額」の2点を確認して合意する。この合意がないままでは、その月の支払いに進めない。モーダル上部の `合意前に必ず確認すること` には、予定額合計と全PJの予定額を実数でまとめて表示する。`修正要望` は `確認して合意` の横から開く。必須枠より下は、カードにせず `参考情報` の短い区切りだけを置き、担当内容・到達目標の詳細、予定額の根拠、支払い予定、未払残を分ける。未合意または条件更新ありの間は、OS内の他画面を開くと、開いた画面の上に月初合意モーダルが表示される。背景クリックで一時的に閉じられるが、合意した扱いにはならない。未合意のままダッシュボードなどを開き直すとまた表示される。その後は「マイページ / PJ コックピット / 通知レビュー / 立替」を毎日 1 回ずつ見て、未対応カードを片付けるのが日常の最小単位。
