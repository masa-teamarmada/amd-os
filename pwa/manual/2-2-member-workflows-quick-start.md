...
月初はまず `/monthly-agreement` で、各PJの「担当内容」と「その対価としての予定額」の2点を確認して合意する。この合意がないままでは、その月の支払いに進めない。モーダルは、ヘッダー→`合意状態：未合意 / 条件更新あり / 合意済み / 対象外` と理由→`確認して合意する2点`→`確認して合意` の順に読む。2点はPJごとに担当内容と予定額を横並びで対応づけ、狭い画面では同じPJの中で担当内容→予定額の順に縦積みする。月次の到達目標は現在の snapshot に無いため、MS名を目標として表示しない。`修正要望` は主ボタンの横から必要なときだけ開く。支払い状況・予定額の根拠・snapshot IDなどは、合意操作より後ろの初期状態で閉じた `参考情報` にまとめる。未合意または条件更新ありの間は、OS内の他画面を開くと、開いた画面の上に月初合意モーダルが表示される。背景クリックで一時的に閉じられるが、合意した扱いにはならない。未合意のままダッシュボードなどを開き直すとまた表示される。その後は「マイページ / PJ コックピット / 通知レビュー / 立替」を毎日 1 回ずつ見て、未対応カードを片付けるのが日常の最小単位。
