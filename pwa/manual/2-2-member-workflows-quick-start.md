...
| 名刺管理 | `/business-cards` | 会った相手の名刺を撮影し、OCR結果を直して関係するPJへ紐付ける |

月初はまず `/monthly-agreement` で、各PJの「担当する仕事」と「その対価としての予定額」の2点を確認して合意する。この合意がないままでは、その月の支払いに進めない。モーダルは、ヘッダー→`合意状態：未合意 / 条件更新あり / 合意済み / 対象外` と理由→全幅の `01 担当する仕事`→全幅の `02 その対価としての予定額`→`確認して合意` の順に読む。`01` では全PJの担当内容を確認し、`02` では予定額合計と、同じPJ順の内訳を確認する。月次の到達目標は現在の snapshot に無いため、MS名を目標として表示しない。`内容が違う場合は修正要望` は主操作の次に置き、必要なときだけ入力欄を開く。支払い状況・予定額の根拠・snapshot IDなどは、合意操作より後ろの初期状態で閉じた `参考情報` にまとめる。未合意または条件更新ありの間は、OS内の他画面を開くと、開いた画面の上に月初合意モーダルが表示される。モーダルは画面いっぱいに広がらず、ウィンドウの大きさにかかわらず上下左右へ背景クリック用の余白を残す。閉じ方はヘッダー右上の閉じるボタン、`Escape` キー、背景クリックの3通りで、どれで閉じても合意した扱いにはならない（画面にもその旨を表示する）。未合意のままダッシュボードなどを開き直すとまた表示される。その後は「マイページ / PJ コックピット / 通知レビュー / 立替」を毎日 1 回ずつ見て、未対応カードを片付けるのが日常の最小単位。

外部の人と会って名刺を受け取った日は `/business-cards` を開き、スマホで撮影する。OCR結果は候補なので、氏名・所属・連絡先を直し、関係するPJを1件以上選んでから確定する。確定すると連絡先は名刺台帳、人物関係は選択PJのナレッジへ入る。詳しい手順は [2-8章 名刺管理](2-8-business-cards.md)。
...
- `members.member_id = ID006` (= りり / NIMS 無償出向) と `ID029` (= あき / 無報酬稼働) は `/mypage` / `/dashboard` 上の報酬額が `ー` 表示 (= 他メンバーの集計には含めない)
...
`/monthly-agreement` では、当月参加中の PJ、担当MS / share、担当内容、月初合意用の想定報酬を確認する。画面上部では `合意状態：` の接頭辞と理由を必ず出し、状態値だけを見せない。月次の到達目標は現時点で定義・保存していないため、MS名を目標として見せない。cap / carry-over / 条件/前提 / 未確定・要確認などの内部確認情報は本人画面に出さない。`status='frozen'` / `freeze_from_ym <= ym` / active `project_freeze_periods` の PJ、当月報酬も担当MSもないPJは表示対象外。`members.exclude_from_payout_notice=true` かつ `is_admin=false` のメンバー (= りり / ID006 NIMS 無償出向、あき / ID029 無報酬稼働) は月初合意も対象外。`is_admin=true` のメンバーは、テスト確認のため支払通知対象外でも月初合意対象に含める。報酬キャッシュがあるPJで担当MSの報酬行がない場合は、未確定ではなく `0円` と表示する。`reward_summary_json.members[].stockYen > 0` の場合だけ、支払予定とは分けて翌月以降へ繰り越される残額として `今月末未払い残（今月は支払われない）` を read-only 表示し、前月繰越・今月発生・今月支払の内訳を添える。合意ボタンを押すと `member_monthly_work_agreements` に表示内容の `snapshot_json` と `snapshot_hash` が保存される。

月中に MS / share / 報酬キャッシュなどが変わって現在の snapshot hash が前回合意時とズレた場合、本人画面と admin 画面に「条件更新あり」と出る。これは報酬計算を変えるものではなく、再確認が必要なサイン。「条件更新あり」画面には合意状態バナーの直後・確認事項の手前に「今回の変更点」を出し、前回合意時から何がどう変わったか（PJ担当の追加・除外、予定額、MSごとの割合・役割など）を PJ ごとに「前回: 〜 → 今回: 〜」で表示する。前回の記録が見つからない、または古い形式で保存されている場合は詳細比較の代わりに理由メッセージのみ表示する。生の JSON や snapshot hash はここには出さない。

担当内容、想定報酬、条件/前提が違う場合は、同じ画面の「修正要望」から送る。修正要望は `member_monthly_work_agreement_requests` に保存され、admin/PM側で確認する。

### 「いまやること」生成ルール

`/mypage` は月次確認 TODO を生成しない。報告書確認の軽い連絡は Slack 側で扱い、OS には nudge カードを出さない。
...
iOS Swift版では下部の `登録` タブを開き、`立替申請` カードから同じ申請一覧へ入る。`登録` タブには `名刺登録` も並ぶ。月次ルーティンの立替確認から開いた場合は、登録ハブを挟まず立替申請一覧へ直接移動する。
...
| N+1 月 3日まで | 報告書確認の軽い連絡が Slack に来たら返答する |
