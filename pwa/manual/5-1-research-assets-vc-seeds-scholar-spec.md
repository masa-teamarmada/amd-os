...
| `next_action` | 次の一手 |
| `company_note` | 会社名セルに添える但し書き。会社名が未確定でも「設立予定」「大学の認定第1号」などを一覧のまま読ませる。研究機関向け公開ビューにも出す (2026-09-01〜) |
| `management_team_note` | 経営チームの状況。CEO/CTO候補、経営経験、研究者との役割分担。AMD内部のみ (2026-09-01〜) |
| `referral_note` | 紹介経路・リファラル。誰の紹介か、研究者本人の人脈から経営人材が出ているか。AMD内部のみ (2026-09-01〜) |
...
### 入力を動かして見る（`/seeds/sensitivity`）

シーズリストの右上「入力を動かして見る」から開く（2026-08-30〜）。
**入力の数字を変えたら金額と順位がどう動くかを、その場で見る画面**。

画面は上下2段になっている。

**上段は全21件の一覧。** 金額（下限・中央・上限）と天井のほかに、その案件の価値が何で決まっているかを
見分ける3列がある。

- **量産到達率** … 自力で量産採用まで届く確率。0 の案件は、価値がライセンス・M&A・知財売却の裾から立っている
- **ゲートまで** … いまの現在地から量産採用まで、平均で何か月かかるか
- **資金÷ゲート** … 資金が何か月もつか ÷ ゲートまでの月数。**1前後の案件は、残高が価値をそのまま決める。**
  1.5 以下をオレンジで出している。ここに乗っている案件の残高は、推定ではなく会社の資料で取らないといけない

**下段は、上段でクリックした案件のつまみ。** 自由資金の残高・バーンレート・用途の天井・証拠水準・担い手・
変換能力・無風期間・専有可能性・産官学モメンタムの9本が1行ずつ並ぶ。

つまみを動かすと、その場で

- そのつまみだけを動かしたときの金額が、右の「その点での金額」に出る
- 上の表のその案件の行が「105万 → 232万」に変わり、順位が「21 → 20 ＋1」と動く。
  **他の案件の順位も一緒に動く**
- 各行の右端に、その入力をなぜその値に置いたかの根拠が出る。長い文はマウスを載せると全文が出る

「ぜんぶ元に戻す」で、いま置いてある入力へ戻る。青い目盛りが、いま置いてある入力そのもの。

**大事な注意が2つある。**

1. **この画面の曲線は、入力を1本ずつ振って先に計算したもの。** つまみを1本だけ動かしているあいだは
   計算した値そのものだが、**2本以上を同時に動かした結果は、それぞれの倍率を掛け合わせた近似**で、
   厳密な計算ではない。画面の上にも常時そう書いてある。組み合わせの正確な値が要るときは計算をやり直す
2. **まだ計算が終わっていない案件は「計算中」と出る。** 1件あたり10秒〜数分かかるので順に埋まっていく。
   天井をまだ調べていない案件は、金額の代わりに「天井が未調査」と出る
