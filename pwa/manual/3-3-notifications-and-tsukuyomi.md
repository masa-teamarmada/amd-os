...
通知の件名は「会議確認: 記録または予定を更新」「会議確認: 確認が必要」「会議確認: 処理が止まった」のいずれか。本文の最初にH-1の役割と、今回まさが見るべきことを日本語で出す。処理停止だけは緊急性の高い通知、それ以外は通常通知に入る。
...
判断が必要なカードには、必ず「確認・反映先」「追加・更新する情報」「この操作の結果」が出る。ここが出ていない通知は、OSの実行結果を読むための報告で、カードから何かを追加・更新する合図ではない。
