...
平日09:00の外部リサーチは Slack へ送らず、AMD OSの通常通知へ1件ずつ届く。候補は業界・市場 / 助成金 / 協業候補の3種類。同じURLや同じ出来事は過去の採用・見送りを含む全履歴と照合するため、日付を変えただけの再通知はしない。

- **採用**: 該当PJコックピットの `経営ハイライト → 採用リサーチ` に残る。
- **見送り**: コックピットへ出ない。履歴は次回の重複防止に使う。
- **候補なし**: 通知を作らない。古い記事で穴埋めしない。
