...
すでに正本へ保存済みの通知 (`saved_count >= total_count`) は、画面上では「はい・確認済み」として扱う。D-11メディア掲載のように collector が `project_media_mentions` と通知を同時に作る通知は、採否というより確認・学習フィードバック用になる。ただし `coverage_gap` は例外。候補行が保存済みでも採否は未完了なので、「はい・確認済み」ではなく「重要メモに残す / 見送る」の判断ボタンを出す。
...
`coverage_gap` は、会議メモにあった話がいまの要約では目立たないときに出る確認通知。PWAでは `未OS化の可能性`、`D-6`、`raw transcript`、`取りこぼし`、`条件付き投資家関心` のような内部語を出さず、「重要メモに残す？: ...」という質問として出す。展開すると「会議メモにあった話」「いまの要約で目立たない話」「残すとどうなる？」を表示する。「重要メモに残す」は、あとで見返すメモとして残す操作で、出資決定や正式合意としては扱わない。H-1要約本文を元に戻す操作でもない。
