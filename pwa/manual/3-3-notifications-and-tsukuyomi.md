...
`textbook_insight` は、AMD OS内の記録から、BZM / Before Zero 実践テキストに残せそうな学びを抜き出した通知。展開すると「元情報」「通知の種類」「追記先」「BZMに追記される内容」「判断の目安」「押すと起きること」「AMDプロトコルとの関係」を表示する。元情報がAMDプロトコルの場合でも、追記先はBZMで、AMDプロトコル本文は書き換えない。「分岐点」は自問自答ではなく、BZM側で判断の条件・材料・結果を再利用可能に残すための分類。ボタンは「BZM追記を承認 / BZMには入れない」。承認しても、この画面からBZM本文を直接書き換えず、後続のローカル反映処理が承認済み候補だけを追記する。
...
- 判断カードには `追加先` と `追加・更新する情報` も表示する。開催履歴候補は `会社概要 → 総会・取締役会`、会議種別・開催日・議題・決議・添付ファイル名を明示し、`この開催履歴を追加する / 追加しない` を出す。採用はPWAの同じfeedback APIを通り、開催履歴だけを1件追加する
