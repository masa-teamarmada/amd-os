...
`coverage_gap` は、会議メモにあった内容が保存済みの会議要約に入っていないか、弱く書かれている可能性があるときに出る確認通知。PWAでは `未OS化の可能性`、`D-6`、`raw transcript`、`取りこぼし`、`条件付き投資家関心`、`目立たない話` のような内部語を出さず、「重要メモにコピーする？: ...」という質問として出す。展開すると「会議メモで見つかった内容」「通知した理由」「ボタンを押すと起きること」を表示する。「重要メモにコピー」は、保存済みの会議要約とは別に重要メモへコピーする操作で、出資決定や正式合意としては扱わない。H-1要約本文を元に戻す操作でも、書き換える操作でもない。
