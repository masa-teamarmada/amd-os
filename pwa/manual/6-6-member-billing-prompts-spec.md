...
当月の本人合意が未完了または条件更新ありで、表示対象PJがある場合、OS内の他画面を開くと、開いた画面を背景に残したまま月初合意の必須モーダルを前面に表示する。モーダル内のボタンから `/monthly-agreement` へ進み、合意完了後は通常どおりマイページ、PJコックピット、admin画面などへ入れる。
