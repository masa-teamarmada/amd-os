...
当月の本人合意が未完了または条件更新ありで、表示対象PJがある場合、OS内の他画面を開くと、開いた画面を背景に残したまま月初合意モーダルを前面に表示する。モーダルでは必須2点を先に表示し、予定額合計と全PJの予定額を `合意前に必ず確認すること` の枠内へ実数で集約する。`修正要望` は `確認して合意` の横のボタンから必要なときだけ開く。必須枠の下は、独立した説明カードを作らず `参考情報` の短い区切りにして、担当内容・到達目標の詳細、予定額の根拠、支払い済み・支払い予定・未払残を二次情報として分ける。背景クリックで表示だけ一時的に閉じられるが、合意状態は保存されないため、未合意のままダッシュボードなどを開き直すと再表示される。合意完了後は、次回からこの確認モーダルを出さずにマイページ、PJコックピット、admin画面などへ入れる。
