...
当月の本人合意が未完了または条件更新ありで、表示対象PJがある場合、OS内の他画面を開くと、開いた画面を背景に残したまま月初合意モーダルを前面に表示する。モーダルでは必須2点を先に表示し、予定額合計と全PJの予定額を `合意前に必ず確認すること` の枠内へ実数で集約する。その枠より下は `参考情報・修正依頼` と明示し、発注条件の詳細、予定額の根拠、支払い済み・支払い予定・未払残、修正依頼を二次情報として分ける。背景クリックで表示だけ一時的に閉じられるが、合意状態は保存されないため、未合意のままダッシュボードなどを開き直すと再表示される。合意完了後は、次回からこの確認モーダルを出さずにマイページ、PJコックピット、admin画面などへ入れる。
