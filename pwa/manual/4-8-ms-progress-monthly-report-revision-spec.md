...
提出版PDFは同じPJの前月実提出書式を継承する。KUTEは2026年8月の修正版の書体・表幅・余白・見出しを、SOLは2026年8月31日改訂版の書式を使い、前月にないヘッダーを追加しない。CXは既存の提出帳票のヘッダーを維持する。下部フッター・ページ番号は表示せず、本文終了後にヘッダーやフッターだけの空白最終ページを生成しない。実装はCSSの名前付き `@page` を使わず、文書内で唯一の既定 `@page` を `isSubmission` に応じて丸ごと差し替える。名前付き `@page` と `page` プロパティを本文要素にだけ適用すると、指定が途切れた場所で常に既定 `@page` (社内版仕様) へフォールバックし、本文終了後に社内版のヘッダー・フッターだけの余分な最終ページが出る事故になるため、名前付き `@page` は使わない。
