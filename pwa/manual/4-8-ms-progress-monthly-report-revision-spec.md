...
- 期限後の当月行が無いMS、または翌月以降の初期値0%行だけがあるMSは、その月以前の最新進捗を現在値として見る。target_ym 月に100%まで進んでいるMSを、翌月の行が無い/初期0%なだけで0%遅延扱いしない。
...
提出版PDFは同じPJの前月実提出書式を継承する。KUTEは2026年8月の修正版の書体・表幅・余白・見出しを使い、前月にないヘッダーを追加しない。SOLは2026年8月31日に愛媛大学へ送付した添付PDFの書式を使い、提出先・対象月・取扱注意の上部ヘッダーを維持する。Driveにある別組版の6ページ版を基準にしない。CXは章立てのOS報告書とNIMSへ送付した従事時間表を区別する。下部フッター・ページ番号は表示せず、本文終了後にヘッダーやフッターだけの空白最終ページを生成しない。実装はCSSの名前付き `@page` を使わず、文書内で唯一の既定 `@page` を `isSubmission` に応じて丸ごと差し替える。名前付き `@page` と `page` プロパティを本文要素にだけ適用すると、指定が途切れた場所で常に既定 `@page` (社内版仕様) へフォールバックし、本文終了後に社内版のヘッダー・フッターだけの余分な最終ページが出る事故になるため、名前付き `@page` は使わない。
