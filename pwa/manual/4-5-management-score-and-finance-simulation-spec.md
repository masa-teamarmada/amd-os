...
> ✅ **(B)(C) の財布分離を本契約使用 / 別財布使用まで拡張 (2026-06-19, v0.28.5)**: `/admin/payouts`「先12か月 本契約cap / 別財布」表と `/management-score` 下部「PJ別 先12か月 本契約cap / 別財布」表は、合算収支を主表示にしない。本契約側は `使用 / cap` と `本契約残` を表示し、使用額には非役員への本契約支払だけでなく役員の本契約会社留保も含める。別財布側は `別財布売上` / `別財布使用` / `別財布残` を別行で表示する。`computeForwardCappedMemberCosts` は `cappedTotalYen` に加えて `cappedRegularYen` / `cappedExtraYen` を返す。
