...
| └ 報酬プラン未作成月は直近実績を横引き | `live-monthly-pl-inputs.ts` の `carryForwardExternalByPj` | 契約が続く PJ でも報酬プランは期末までしか作られていないため、プランが無い月の原価をゼロのままにすると試算表が楽観側に振れる。「エフォートは今のまま続く」前提で、**プラン実在月の直近3ヶ月の外部支払の中央値**を、契約期間内 (`isProjectBudgetActive`) でプランが無い月へ横引きする。中央値にするのは別財布スポット (`externalExtraPayoutCapYen`) が乗った単月に引きずられないため (p19 202610 は ¥536,890 だが中央値は ¥137,280)。**プラン実在の判定は `reward_summary_json.meta.planCycleId` が非空かどうか**で行う。行の有無だけで見ると、プラン切れ月に残る空キャッシュ (external 0) が中央値を 0 に潰して横引きが無効化される。横引きが起きた PJ は `warnings` に出す。2026-08-17 時点の発動は p19 のみで ¥137,280/月。p25/p10 は外部支払が元々0のため対象外、p21 は `end_ym='202703'` で契約期間ガードに弾かれる |
| └ 見込 (`status='sales'`) PJ の原価0は意図した前提 | — | 横引きは**プラン実在月が1つ以上ある PJ** の実績を伸ばす仕組みなので、まだ稼働実績が無い見込 PJ には基準が無く発動しない。2026-08-17 時点で該当は p32 NewCo業務委託 (2027-05〜 月¥600,000)、p33 CX再開 (2027-01〜 月¥274,000)、p34 PSI Step2新規 (2027-04〜2028-03 月¥400,000) の3本、計 月¥1,274,000 が原価0のまま粗利に乗る。**原価0であること自体はまさ確定 (2026-08-17) の前提**: 「Aだな。多分おれ1人で対応するし。メンバーに払う余裕がない」。この3本はまさ単独稼働を想定しており、外部メンバーへの支払が発生しないので原価0が正しい。実際にメンバーを入れる判断をした時点で報酬プランを作り、そこから原価が入る (プランができれば以後は横引きの基準にもなる)。**⚠️ ただし「3本を既存PJと別のPJとして立てたこと」はまさ確定ではない** — 上の発言は「メンバーに支払うか」への回答であって、PJ分割の承認ではない (2026-08-27 まさ指摘「SXのPJと別のPJにしていいとは一言も言ってない」)。まさの整理では p32 NewCo は **p21 SX へ**、p33 CX再開は **p20 CX へ**、p34 PSI Step2新規は **p26 VasculaX へ** 入れるべきもの。別PJのまま残っているのは、`projects` が1PJにつき料金体系を1組しか持てず (`fee_type` + `fee_amount` + `start_ym` + `end_ym`)、期間で金額が変わる契約を1行で表せないという実装上の制約による。統合するには `fee_type='variable'` + `billing_cycles` の月別金額へ移す必要があり、p21 は現在 `monthly_fixed` (月¥1,048,000、`end_ym=202703`) なので既存15ヶ月ぶんの入れ直しと請求フローの再確認を伴う。**未統合のまま残す場合も「SXの後継を別PJで持っている」状態であることを忘れない** |
