...
| └ 原価は `billing_cycles` を作っただけでは入らない | `billing_cycles.reward_summary_json` の capped 外部支払 | ⚠️ 将来月の `billing_cycles` 行を `budget_yen` 付きで作っても、**売上原価は 0 円のまま**。原価の正本は `/admin/payouts` の capped 外部支払キャッシュ (`reward_summary_json`) であり、`budget_yen` ではないため。したがって報酬配分が未確定の将来月は、売上・入金だけが立って原価が抜けた**楽観側の試算表**になる。2026-08-16 時点で 2027-04〜2028-03 の p10/p19/p25 は台帳行 (`budget_yen` 計 ¥685,454/月) はあるが `/admin/payouts` の配分が無く、原価 0 円・粗利過大の状態 |
