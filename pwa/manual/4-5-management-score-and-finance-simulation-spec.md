...
> ✅ **(B)(C) の財布分離を入/出/残チェックへ整理 (2026-06-19, v0.28.10)**: `/admin/payouts`「先12か月 本契約cap / 別財布」表と `/management-score` 下部「PJ別 先12か月 本契約cap / 別財布」表は、合算収支を主表示にしない。セル表示は `本契約` / `別財布` の2行にし、それぞれ `入` / `出` / `残` だけを並べる。残がマイナスなら `払いすぎ確認` を出す。使用額には非役員への支払だけでなく役員の会社留保も含める。MS月割やstockはこの財布チェック表の主表示に混ぜない。`computeForwardCappedMemberCosts` は `cappedTotalYen` に加えて `cappedRegularYen` / `cappedExtraYen` を返す。

> ✅ **(C) も capped + 別財布に同期 (2026-06-18, v0.27.7 / 2026-06-19, v0.28.5 更新)**: (C) `/management-score` 下部表も支払予定を出す表なので、(B) と同じく `computeForwardCappedMemberCosts` の capped を使う。`forecastCapped` が計算済みなら 0 も正値として扱い、plan期間内だけ本契約cap fallbackを許す。v0.28.5 以降、最終表示は合算収支ではなく `本契約残 = budgetYen - cappedRegularYen` と `別財布残 = extraRevenueYen - cappedExtraYen` を分ける。`budgetYen + extraRevenueYen - forecastPayoutYen` を主表示に戻さない。
