...
`/notifications` 画面と、 PJ コックピット内 `CockpitStrategySignals` の **経営ハイライト確認** UI、 修正依頼履歴の見方をまとめる。 ユーザー向け概念は [22 章](22-notifications-and-tsukuyomi.md) を、 つくよみ修正依頼 dialog API は [27 章](27-knowledge-admin-tsukuyomi-spec.md) を見る。
...
- アクション:
  - ✅ 確定 (= `confirmed`)
  - ❌ 不採用 (= `rejected`)
  - ⚠️ つくよみに修正依頼 (= 27 章の dialog 対話型ループ起動)
  - 🗨️ コメントだけ送信
...
`l2_feedbacks` の `feedback_text` には dialog 対話型 (= 27 章) の **conversation 全体** が保存される。 同 `(l2_kind, target_id, scope_key)` 単位で時系列に表示:
...
- 27 章 [Knowledge Admin / Tsukuyomi](27-knowledge-admin-tsukuyomi-spec.md) (= 修正依頼 API 詳細)
- 38 章 [L2 Extraction Routines](38-l2-extraction-routines-spec.md) (= 各 L2 抽出側との feedback 接続)
