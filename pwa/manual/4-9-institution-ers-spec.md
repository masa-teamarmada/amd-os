...
- **2026-09-01 ultracode並列ワークフローで公開情報desk research実施、40機関ぶんを新規追加**（`pwa/scripts/migrations/361_su_regulations_bulk_research_2026-09-01.sql`）。対象は東京大学・京都大学・大阪大学など主要国立大学、公立・私立大学、国立研究開発法人。各機関の公式規程集（Reiki-Base / g-reiki.net / LegalCRUD等の例規集ベンダーサイト含む）・産学連携部門ページで実在確認できたURLだけを登録し、推測でのURL補完は行っていない。既存4機関（愛媛大・香川大・工学院大=KUTE・NIMS）と合わせ、機関マスタ48件中44件が最低1件の規程を持つ状態になった。
- **2026-09-10 の追加調査で全機関が埋まった**（migration 385 / 387）。機関マスタは 48 → **64 件**（認定制度か支援制度を公式ページで確認できた 16 機関を追加）、規程は **422 件**、**64 機関すべてが最低1件の施行中の規程を持つ**。前回「未確認のまま残った」東京電機大学・産業技術総合研究所・国立がん研究センター・森林研究・整備機構 森林総合研究所も、今回の探索で規程が見つかった。
...
- **入っているデータ（2026-09-10 時点）**: 全 **64 機関** × 16 項目 = **1,024 セル**のうち、根拠まで確認できたものが **831 セル**。制度比較の詳細 21 項目も 64 機関すべてに入っている。数字は `/institutions` の「分析」タブが常に最新を出すので、この手引きの件数は目安として読む。
...
`/institutions` の「分析」タブ（2026-09-05 新設）は、支援プログラム比較のデータを別の角度から見る画面で、入力欄は持たない。

- **どの支援が一般的か**: 16 項目それぞれについて、確認済み機関のうち整備済みの割合を高い順に並べる。AMD の推奨で「大部分の機関が認めている」と言うときの根拠はここ。
- **地域ブロック別の整備率**: 所在地を北海道・東北 / 関東 / 中部 / 近畿 / 中国・四国 / 九州・沖縄 に寄せ、項目ごとの整備率を濃淡で出す。括弧は整備済み / 確認済み。
- **機関ごとの支援充実度**: 整備済みの項目数が多い順。未確認が多い機関は「調べきれていない」のであって充実度が低いのではない、と説明文で明示する。
- **AMDが提案できる余地**: 根拠を見た上で未整備・検討中と分かっている項目が多い順。未確認は余地に数えない。
- **制度の型の分布**: 退職・卒業後期限、更新、呼称、規程主体、支援期間、有効期間、認定決裁者、支援の対価を、値の先頭の語で束ねて数える。
- 種別（大学 / 研究機関）で絞れる。集計はすべて `pwa/src/lib/institution-support-analysis.ts` の純粋関数で、`npm run test:institution-support-analysis` が「未確認を安全側へ数えない」「分母は確認済み」「地域寄せ」を検査する。

機関詳細 `/institutions/{institutionId}` のヘッダー直下には「支援プログラム」節を置く。その機関の 16 項目を群ごとに状態・内容・根拠・出典つきで並べ、比較列に無い項目（制度整備の詳細）は折りたたみで状態と値だけ出す。冒頭の所見は制度比較マトリクスの `attr_support_summary`（支援制度の所見）。
