...
週次 Slack レポートの設定は、`/admin/projects` のPJ台帳で管理する。`Slack CH` の右にある「週次レポート」列がすべてのPJに表示されるため、新規PJも追加時から設定対象になる。列幅は一覧上部の「週次レポート列」で調整でき、同じブラウザでは次回もその幅を使う。
...
- 保存はadmin専用のPJ台帳更新経路を通す。つくよみレポートは投稿直前に同じ設定を読み、`true`以外ならSlackへ送らない。画面は設定と別に、Slack生データから「実投稿: 配信中」（直近10日以内の週次レポートを検出）、「実投稿: 前回あり」（停止前の証跡）、「実投稿: 直近未検出」、「実投稿: 未判定」を出す。PJ名・チャンネル名の固定リストは使わない。実投稿が見つからなくても停止とは断定しない。
...
- つくよみレポートの現行送信元は、投稿直前に同じPJ列と宛先チャンネルを読み直す。`true` 以外ならSlackへ投稿せず、ほかの通知本文には影響しない。`true` は送信を許可するだけで、停止している送信処理を勝手に起動しない。
