...
- Slackチャンネルが設定済みのPJだけ、「配信設定を許可」を選べる。`チャンネルなし`またはチャンネル未設定のPJは配信許可にできない。
- 保存はadmin専用のPJ台帳更新経路を通す。画面は設定と別に、Slack生データから「実投稿: 配信中」（直近10日以内の週次レポートを検出）、「実投稿: 直近未検出」、「実投稿: 未判定」を出す。PJ名・チャンネル名の固定リストは使わない。実投稿が見つからなくても停止とは断定しない。
...
- 現行の外部送信元はこの設定をreadbackしていないため、設定の停止・許可は実投稿の送信制御とは別状態である。
- 将来の送信処理は、本文生成とSlack投稿の直前に同じPJ列と宛先チャンネルを読み直す。`true` 以外なら生成も投稿もしない。`true` は送信を許可するだけで、停止している送信処理を勝手に起動しない。
