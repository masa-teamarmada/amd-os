...
- ただし、同じ plan cycle の active/manual 台帳に `metadata_json.tolerated_members` があり、該当 member が「経営判断で許容する過払い」として記録されている場合、`origin_type='ms_overview_edit'` の負の pending 回収行は報酬計算へ入れない。正の追加支払は通常どおり入れる。
