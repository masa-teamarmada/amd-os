...
### 参画終了後の新規ptと、過去の未払分を分ける（2026-09-26）

従来方式の新規ptは、各稼働月の `project_members.join_ym` から `leave_ym` まで（両端の月を含む）の担当者へ配分する。終了月を過ぎた担当者のshareはその月だけ除外し、残る担当者で正規化する。過去の月を再計算するときも当時の参画期間を使う。離脱だけを理由に `is_active=false` にして過去の担当を消さない。

終了後も、既に発生した未払報酬は支払上限の範囲で返済し、残額を繰り越す。「10月から新規ptが0」と「10月の支払が0」は同じではない。

SOL（p21）のかる・ちこは2026年9月を終了月とし、10月以降の新規ptは0。定例会運営は4〜9月のかる3.5ptと10〜3月のまさ3.5ptに分割し、コスト試算・DD対応の10月以降は参画中のまさへ配分する。9月以前の支払台帳・実績行は変更しない。年明けの再参画は未設定で、実在する委託業務が生じた時点で別途決める。

### SXの10月以降だけ：成果物の検収ptを使う

SX（p21）の2026年10月稼働分から、シーズンMSと結び付いた成果物は「期限が来た」「完了ボタンを押した」だけではptを獲得しない。担当者が完了証跡を残し、別のPM/PLが検収した月に、確定ptを担当割合で分ける。検収されない成果物は翌月へ持ち越す。検収時点の成果物名・MS・月・担当別ptは `project_action_pt_reviews` と配分台帳に固定し、あとからツリーを付け替えても過去の報酬を動かさない。

この対象MSには従来の月割り下限を適用しない。検収ptゼロならそのMSの当月発生ptもゼロ。定常MS・SX以外のPJ・SXの2026年9月以前は従来の月割りと予定shareの計算を続ける。pt単価、月次支払上限、未払の繰越は従来の計算を再利用する。月初画面のタスクpt×単価は支払枠前の見込みで、検収後の支払額とは区別する。
...
> **2026-08-28 まさ確定 — 支払対象外メンバーも枠の按分から外さない**: 支払対象外メンバーの繰越が枠を占め、現金を受け取るメンバーへの支払いが遅れることを踏まえたうえで、現行どおり全員を同じ按分に入れる。まさ「なんで会社側が我慢して彼らに先に払わないとなの？ みんなで平等に我慢しないとだよ」。実例として SX 2026年6月は枠 564,200円のうち 331,440円 (59%) が支払対象外メンバー (まさ・きよ) に割り当たり、現金で出たのは かる 145,575円 / ちこ 87,185円 の 232,760円だった。この配分は意図どおりであり、支払対象外メンバーを按分から外して現金支払対象へ枠を寄せる変更は採らない。

**表示・通知の側では本契約 (regular) と別財布 (cap_extra) を混ぜない (まさ確定 2026-08-28)**。別財布は支払条件が本契約と別で、完了月まで払わない積立。支払通知書の稼働月の範囲・備考・月初合意の受け取る額は、本契約側の値だけで出す。迷ったら本契約だけで出す。手順とその理由は [`design/season_budget_actual.md`](../design/season_budget_actual.md) §5.2 ④ を正本にする。
