...
対象は決算書だけではない。
決算、株主総会、取締役会、契約、資金調達、補助金、技術結果、特許、事業計画、重要リスク、明示期限など、会社やPJの判断を変える情報を共通の入口で見る。

DriveではPDF、Word、Excel、PowerPoint、Google文書、text系を本文まで読む。
Gmail、Calendar、Slack、Notionも同じ重要度とPJ帰属の判定へ流す。
ファイル名だけで決めず、本文、親フォルダ、PJ root、発行主体を合わせて見る。

同じ内容のコピーは1候補にするが、コピーを消さず全所在をlineageとして残す。
画像PDFなど本文が取れない時は「OCR待ち」で残し、非該当や0へ変換しない。
観測、推定、計算、欠測も分ける。

決算値については、年度決算から月次実績を作らず、現金は期末残高、売上・損失・研究開発費・設備投資は年度累計、J-KISSと借入は資金調達CF、補助金の受領済み預りと上限は別区分にする。
借入・補助金・J-KISSを売上や会社価値へそのまま足さない。

抽出結果は`amd-os-ms/outbox`から候補へ入り、通知で採用された後だけ重要情報正本へ移る。
BZM 2.1や契約・ガバナンス等への反映先もまず接続候補にし、現行正本を自動上書きしない。

通知で「重要情報として保存」を押して正本化が成功した時だけ、同じ出来事がPJコックピットの「これまでのPJ活動のなかで得てきたもの」台帳へ1行並ぶ。
同じ資料を何度採用しても行は増えない。台帳側の書き込みが失敗しても重要情報の保存は取り消さず、通知の返事に失敗した旨だけ添える。
台帳へ写すのは日付、表題、監査タグ、どの層の証拠が増えたか、証拠の所在まで。
どの制約が動いたか、何を使ったか、何ができるようになったかは意味の判断なので、抽出では埋めず空のまま残す。空欄は未判定であって0ではない。
...
| D-3 PJ ナレッジ | Claude routine + PWAの人確認済み名刺同期 | 定期抽出 `amd-os-l2-consolidated-evidence`、名刺確定 `PATCH /api/business-cards/[cardId]` | 定期抽出はroutine枠。名刺OCRは撮影時だけGeminiを呼び、自動cronではない | Claude routine履歴、`/business-cards`、`pwa/spec/2-5-business-cards-current-spec.md` |
...
PWA `/api/report/generate` と `/api/monthly-report/edit-by-tsukuyomi` は410停止済み。月次報告モーダルの社内版は非LLMの直接編集・保存・確定だけで、token課金は発生しない。`/api/cron/monthly-reports-backfill` は対象と費用を確認した手動復旧に限定し、通常UIや定期実行には使わない。
...
| D-3 PJ ナレッジ | `project_knowledge` | ~~GAS 155~~ ⛔ 5/22 停止 | **Claude routine `amd-os-l2-consolidated-evidence`** + **確認済み名刺のPWA直接同期** | daily + 名刺確定時 | ✅ OCR候補は自動確定せず、確認後だけ `source='business_card'` の人物行を作る。連絡先や画像はD-3へ複製しない |
...
**W-Prep Launch** (= visible prep thread 起動レーン): H-1 議事録抽出とは別に、毎週水曜15:00 JSTに Codex automation `w-prep-launch` が、実行日から数えて7日後の23:59:59.999 JSTまでの確定 upcoming MTG を確認し、必要な prep thread を作る。厳密な `now()+7 days` では切らず、水曜実行なら翌水曜終日のMTGまで含める。DBの `project_meeting_summaries` だけを見て完了扱いにせず、必ず同じ拡張7日窓の Calendar を直接確認する。CalendarにあるがDBに無い active/sales PJ の確定MTGは、まず `source_kinds='upcoming'` のカードを作ってから claim → thread作成 → title変更 → pin → DB session保存へ進む。重複防止は `calendar_event_id` exact identity と `meeting_id='upcoming:<calendar_event_id>'` の canonical を優先し、同じ event id の準備済みsessionがある行は二重起動しない。作成した thread は必ずピン留めする。worker は契約範囲とPJ横断の未完了論点を台帳化し、今回prepから黙って消えた論点がないかを機械検査する。初回は共有資料を作らずopening prep briefで相談を始め、まさの明示write後だけHTML資料を作る。Google Docs / Markdown / Slides / Sheets は主成果物にしない。
