...
メンバーへの支払月を決めるのは `effectiveMemberPayoutYmForCycle` **だけ**。`/admin/payouts` (支払通知書) と `/admin/monthly-work-agreements` (月初合意) は同じ関数を使う。かつて月初合意側が `effectivePaymentYmForCycle` (クライアント請求月ベース) を呼んでいたため、同じ月を開いているのに片方が 0 円、もう片方が実額という食い違いが起きた (まさ指摘 2026-08-26)。判定を分けると必ずずれる。
