...
メンバー行の明細 (`PJ名 / 稼働月 / base / 繰越 / 支払 / 未払い残` の1行) をクリックすると、そのメンバー × PJ × 稼働月の **支払額の内訳** が開く。内訳は3つの区画を持つ。

1. **この月のお金の流れ** — `当月発生 + 前月までの未払い残 = 支払対象 → 当月支払 → 月末未払い残`。支払対象外メンバーの場合は、当月支払が現金支払ではなく会社側の配賦であることを併記する。
2. **当月発生の中身** — MS別に `当月の進み (pt) × 担当割合 × 1ptあたりの単価 = 金額`。合計は `basePay` に一致する。1pt単価は本契約 (`regular`) と別財布 (`cap_extra`) を分けて表示する。
3. **繰越はいつの分か** — 当月が属する plan cycle の先頭から当月までの月次表 (`当月発生 / 繰越入 / 支払対象 / 支払 / 月末未払い残`) と、当月の繰越額を発生月へ割り戻した列。

繰越の発生月への割り戻しは **「古い稼働月の発生分から順に支払われた」という表示上の仮定**で計算する。報酬計算は `当月発生 + 繰越` の合計に対して cap を按分するだけで、どの月の発生分を先に消したかは持っていないため。合計額と月末残高は計算どおりの値で、順序の仮定はこの列にしか効かない。plan cycle が変わると繰越の鎖は切れるので、遡る範囲は当月が属する plan cycle 内に限る。

金額は `GET /api/admin/payouts/member-breakdown?projectId=&ym=&memberId=` (admin) が `billing_cycles.reward_summary_json` を読んで返す。**このモーダルでは報酬を再計算しない**。再計算すると一覧の行とモーダルの金額がずれるため、一覧と同じキャッシュだけを読む。過去月のキャッシュが欠けていて繰越の発生月を辿りきれない場合は、辿れなかった額を注記として出す (報酬キャッシュ再計算で解消する)。

MSそのものの進捗を見たい場合は、モーダル右上の `MSの進捗を見る` から従来の月次モーダル (PJ × 稼働月のMS進捗) を開く。
...
2026年7月以前の稼働月 (`source_ym <= 202607`) は、月初合意機能の導入前/移行月として支払 gate 上 `合意済` 扱いにする。6月は契約改定前かつシステム未完成期間、7月は内部状態が動くたびに再合意を求める実装だった期間 (2026-08-28 修正) で、どちらも合意が成立しないまま支払時期を迎えた (まさ確定 2026-08-28)。実際の合意 row を作るのではなく、gate の表示理由を「導入前/移行月のため合意済み扱い」とし、2026年8月以降の稼働月から通常どおり未合意・条件更新・修正要望を blocker にする。
...
sequenceDiagram
  ...
  PWA->>DB: 保存済み正式PDFと最新DBを照合
  PWA-->>Admin: PDFが最新なら確認モーダル (件名固定 / 本文テンプレ / 添付PDF / Bcc) を表示
...
メンバーへの支払月は PJ の支払条件から決まる。`invoice_received_60_days` (請求書受理後60日) のように請求書の受領日を起点にする条件では、**請求書を実際にいつ送ったかが台帳に無いと「翌月◯日に送る」という保守的な推定**になり、実際にはもう入金があるのに支払月が1か月後ろへずれる。

**支払月は稼働月の月初合意の時点で本人へ示し、合意している。あとから入金が早く着いたことを理由に前倒ししない。** まさ「月初合意では8月の報酬は元々0円になってた。それが急に振り込まれたからって、月初合意をやぶって早めに支払うってのはルールに反する」。

合意 snapshot の `payoutSchedule[].paymentYm` と `memberPayoutYmForCycle` の推定は一致する。実測 (2026-08-28):

| メンバー | PJ | 稼働月 | 合意記録の支払月 | 推定 |
|---|---|---|---|---|
| うめ / しん | ZMP | 202606 | 202607 | 202607 |
| うめ / しん | ZMP | 202607 | 202608 | 202608 |
| まさ / きよ | SX | 202606 | 202609 | 202609 |
| まさ / きよ | SX | 202607 | 202610 | 202610 |

2026-08-26 から 2026-08-28 まで、`payment_confirmed_at` があれば推定より早い月へ前倒ししていた。当時は「入金があるのに9月払いは推定が保守的すぎる」と読んだが、合意記録では最初から9月だった。推定は合っていて、前倒しの方が合意を破っていた。この結果 SX の2026年6月稼働分 (かる 145,575円 / ちこ 87,185円) が9月から8月へ1か月早く出ていた。**この前倒しを戻さない。**

**請求書の送付日は台帳へ入れない (まさ確定 2026-08-28)**。SX のように「当月末に請求書を送り、翌々月5日に入金される」実運用でも、`billing_cycles.invoice_sent_at` / `invoice_issued_at` は空のままにする。メンバーへは契約の支払条件どおり「請求書受領後60日まで」で伝えているため、送付日を入れて支払月を前倒しすると、伝えている条件より早い支払を先に約束することになる。**この空欄はデータ欠落ではない。送付日を自動で埋める仕組みを足さない。手で埋めない。**

実測 (2026-08-28 にきよの送信履歴を確認): SX は 6月分を 6/30、7月分を 7/31 に送付し、6月分の入金は 8/5。台帳の登録 (`invoice_received_60_days` + 翌月10日送付) からの推定は 6月分が9月払い・7月分が10月払いで、実運用より1か月遅い。この差は把握したうえで登録を変えない。

入金が確認できても支払月は動かない (上の「前倒ししない」)。したがって送付日を空にしておけば、支払月は契約の支払条件どおりの推定 = 月初合意で示した月のままになる。

入金側も、推定した支払月より早く着いた入金を拾えるようにしてある。`/api/cron/freee-payment-sync` は、当月の入金明細を **当月ぶんだけでなく先2か月ぶんの請求グループ**とも照合し、一致したら `payment_confirmed_at` に **実際の入金日** (freee の取引日) を入れる。

> 実例 (SX / 愛媛大学): 6月分の請求書は6月30日に送付済みで、入金は8月5日。台帳に送付日が無いため OS は「7月10日受領 → 9月払い」と推定する。**この9月払いが月初合意の記録と一致する正しい値**であり、入金月の8月へ動かさない。

### 合意した額を払う。金額の計算は月初合意が正本

**次に払う金額は、月初合意の時点で確定している** (まさ確定 2026-08-27「次回払う金額が月初合意の段階で計算されてないといけない」)。支払通知書の側で別の計算をして金額を作り直さない。

合意額に含めるのは、その稼働月として支払うべき **すべての金額**。

```
合意額 = 当月のMS消化から発生する分 + 過去の未払いの返済分  (月次の支払枠を通した後の額)
```

かつて月初合意は「当月の予算 × 当月の予定MS消化 × share」だけを予定額として出しており、過去の未払いの返済分が入っていなかった。そのため合意額と支払通知書の額が食い違った (かる 2026年6月: 合意 187,598円 / 支払通知書 145,575円)。**過去の未払いの返済分も含めて合意額を計算しなければ、その計算が間違っている。**

実装は `billing_cycles.reward_summary_json` の `members[].totalPay` (支払通知書とまったく同じ値) を合意額に使う。当月発生分は `currentMonthAccrualYen` として内訳に残し、画面では「当月発生」として補助表示する。

### 支払の条件と期日 (まさ確定 2026-08-27)

合意額を払うのは、次の2つがそろった状態になってから。

1. そのPJのクライアントからの入金があること
2. 当人の月初合意が済んでいること

**両方そろった翌月7日まで**に支払う。請求書をいつ出したか、支払条件が何日かは、メンバーへの支払時期の計算には使わない。

### 「入金月」と「支払月」を混ぜない

`billing_cycles` の1行から導ける「月」は2つある。名前が似ているので、正本ではこう呼ぶ。

| 関数 | 意味 | 使う場所 |
|---|---|---|
| `clientReceiptYmForCycle` | クライアントからAMDへ**入金される月** | 売上・入金予定・資金繰り |
| `memberPayoutYmForCycle` | AMDから**メンバーへ払う月** | 報酬・配賦・支払通知書・月初合意 |

2つとも必要な概念だが、どちらも `string` を返すので取り違えてもコンパイルは通る。実際 2026-08-26 に、月初合意・役員配賦・経営スコアの3か所がクライアント入金月でメンバーへの支払を数えており、同じ月を開いているのに画面ごとに金額が違っていた。

機械の防波堤は `npm run test:payment-month-usage` (`scripts/check_payment_month_usage.mts`)。メンバーへの支払を扱うファイルが入金月の関数を呼んだら落とす。例外を足すときは理由を書いて登録する。旧名 (`effectivePaymentYmForCycle` / `effectiveMemberPayoutYmForCycle`) が復活した場合も落とす。

メンバーへの支払月を決めるのは `memberPayoutYmForCycle` **だけ**。`/admin/payouts` (支払通知書) と `/admin/monthly-work-agreements` (月初合意) は同じ関数を使う。かつて月初合意側がクライアント入金月の関数を呼んでいたため、同じ月を開いているのに片方が 0 円、もう片方が実額という食い違いが起きた (まさ指摘 2026-08-26)。判定を分けると必ずずれる。

月初合意の一覧に出る3つの金額は意味が違うので、画面にもその説明を出す。

| 表示 | 意味 |
|---|---|
| 月初計画 | その稼働月の月初に合意した予定報酬 (計画レイヤー) |
| 今月支払 | その月の支払通知書に載る額。支払通知書ページと同じ計算 |
| 立替精算 | その月の支払通知書へ合算する実費 |
| 未払い残 | 月末時点の未払い残高 |

### 同じ入金を二度使わない

`/api/cron/freee-payment-sync` は、入金1件 (freee の口座明細 / 取引) を複数の請求月へ当ててはいけない。当てると、実際には1か月ぶんしか入っていないのに複数月が入金済みになる。過去の入金確認ログ (`billing_log.detail.freee.wallet_txn_id` / `deal_id`) から使用済みの id を集め、それらは新しい照合から除外する。

> 実例 (2026-08-26): 照合対象を先2か月へ広げた初日に、8月5日の愛媛大からの入金 1,152,800円 が SX の6月分と7月分の両方へ当たり、7月分まで入金済みになった。7月分の入金確認は取り消し済み (`billing_log` の `payment_confirm_reverted`)。
...
- 添付: 保存済み正式PDFの `payout_notices.pdf_url` の Drive fileId から `DriveApp.getFileById().getBlob()` で実 PDF 添付。ファイル名は `支払通知書_{ym}_{memberName}.pdf`
- 作成日: 「送付」クリック時にはPDFを生成・再生成しない。PDF右上の `作成日` は、最後に `支払通知書発行` / `強制再発行` / cron prebuild で正式PDFを生成した日付。最新の住所・宛名・登録番号・金額を送付したい場合は、先に正式PDFを再発行してから送付する。
- 本文テンプレ (確認モーダル既定値):
  ```
  {memberName}様
...
| 明細の稼働月 | **繰越の鎖を遡った範囲で書く** (`ymSpanLabel`)。当月だけを「6月稼働分」と書かない。繰越があると、その支払には過去月の未払い分も乗るため。範囲の先頭は `carryInYen` が 0 になる月まで遡って決め (`loadPayoutSourceSpans`)、plan cycle をまたぐ手前で止める。例: かる 2026年8月支払 = 「4〜6月稼働分」(2026-08-28 まさ指摘「正しくは4月から6月の3ヶ月分が今回支払われる」) |
| 備考 | その期間に発生した額と今回のお支払いの差を書く。「4〜6月稼働分 145,575円」だけだと3か月分がその額に見えるため、`SX：4〜6月の稼働で発生した ¥601,349（税抜）のうち、今回のお支払いは ¥145,575（税抜）です。残り ¥455,774 は翌月以降の支払枠で順にお支払いします。` を PWA が組み立てて `noteText` で渡す |
| 税内訳 | `小計（税抜）` = admin/payouts の支払額、`消費税（10%）` = 税抜額 × 10%、`合計（税込）` = 小計 + 消費税 |
| 支払予定 / 方法 | 支払予定日と支払方法を表示。振込先欄はPDFから削除する |
| 右上情報 | 作成日 / 通知書番号を右寄せで表示 (= 最後に正式PDFを生成した日付と通知書番号) |
...
`/admin/payouts` の行の「PDF確認」ボタンは、保存済み正式PDFを開くだけで PDF 生成しない。PDF 未生成・メンバー台帳更新後・金額不一致・テンプレート更新後で古い場合は押せず、先に「支払通知書発行」または「強制再発行」で正式PDFを作り直す。確認用 PDF を作る場合は別操作の「確認用PDF生成」を使う。確認用 PDF は **`payout_notices` に保存しない** (= 番号採番もしない)。
...
`payout-notice-prebuild`、正式PDF発行、全員分PDF一括発行、強制再発行の直前で、既存 `payout_notices.total_yen` と新計算値を比較し、 **金額が変わったメンバーは `pdf_url` / `last_generated_at` を NULL クリア**する (`sent_at` が立っている行は触らない)。 これで次回 cron / 一括発行で差分検出が再生成を発火させる仕組み。

正式PDF発行・強制再発行のような admin の明示操作では、金額差分の有無に関係なく同期後に DB を再読込し、`members` の最新宛先情報を GAS へ渡す。送付確認はPDF生成せず、保存済み正式PDFが最新DBと一致するかだけを照合する。cron の先回り生成だけは、未送付PDFの `last_generated_at` / `total_yen` / テンプレート更新時刻 / `members.updated_at` による差分検出で GAS 呼び出しを抑制できる。
