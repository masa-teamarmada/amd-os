...
UI では上部の一括操作列と「メンバー別支払」見出しに `同期済み` / `発行時に同期` / `同期できない` の状態を表示する。差分があっても画面表示中に自動POSTはしない。開きっぱなしのタブでは 60 秒ごとに read-only 再取得し、夜間 cron や別操作で同期済みになったら表示も追随する。正式な個別発行・全員分PDF一括発行・強制再発行・送付は、サーバー側で最新計算額を同期してから実行するため、運用者が先に保存ボタンを押す必要はない。月初合意 gate や本契約cap blocker がある場合だけ同期を止め、admin override または blocker 解消を待つ。
