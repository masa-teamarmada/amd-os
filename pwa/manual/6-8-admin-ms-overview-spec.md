...
- `budgetImpact` — freee銀行出金と `monthly_reward_payout` 明細が一致した支払済み実績だけを固定し、これから支払う見込みと期末未払い残を足した PJ 予算影響。表示項目は `PJ予算` / `支払済み固定` / `実績未照合` / `これから支払予定` / `期末未払い残` / `保存後残予算`。`remainingBudgetYen < 0` の場合は赤字見込みとして warning 表示する。
- `blockers` — 保存不可理由。旧 reward cache が無い、次回精算先が無い、過払い回収がシーズン内で吸収できない可能性がある、支払済み印はあるが実支払証跡と明細額が未照合の月がある、など。

このパネルの目的は「差分を見せる」ことではなく、MS を期中変更しても **払いすぎ・払い足りなさを作らず、かつ証跡つき支払済み実績を固定した結果として PJ 予算が赤字にならないか** を編集中に判定すること。実支払額と同一だと確認できない月は `実績未照合` に分け、保存は `blocked` にする。`warning` は保存可能だが精算または赤字見込みが発生する状態、`blocked` は保存不可。
