...
編集モードでは、MS 一覧の先頭に **全MS pt配分スライダー** を置く。これは MS 名 / pt数値入力 / スライダー / 現在pt / **MS金額** を並べた配分専用パネルで、全 MS の pt と金額インパクトを比較しながら調整するための入口。各編集カード内にも pt 数値入力 + pt配分スライダーを残し、どちらを動かしても同じ編集中 state を更新する。`cap_extra` は MS 期間の月数×10pt固定なので、まとめパネル・個別カードの両方で disabled 表示にする。
