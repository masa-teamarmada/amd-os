...
個人名を並べるカードは上段メトリクスに出さない。メンバーごとの担当量と設計額は、下段の **メンバー別 pt配分 / 設計額** で確認する。

回帰禁止: この 4 枚構成を 3 枚に減らさない。4 枚目を個人名カードへ戻さない。4 枚目は保存前支払検算 `budgetImpact` の安全状態を表示するカードとして固定する。
...
残り割り振り可能ptが正数でも、それは未払い債務や予算不足ではない。将来の追加MSに備える意図的な未配賦バッファとして残せる。SX 2026/04–2027/03 は 120pt のうち13ptをこの用途で残す方針（2026-07-29 まさ確定）。未配賦ptは誰の `earnedPt` にもならず、`companyReserveYen` や `stockYen` を発生させないため、保存停止条件にしない。
...
- `budgetImpact` — freee銀行出金と `monthly_reward_payout` 明細が一致した支払済み実績だけを固定し、これから支払う見込み・支払対象外メンバーへの当月配賦済み額・支払対象メンバーの期末未払い残を足した PJ 予算影響。表示項目は `クライアント支払` / `バッファ` / `原資上限` / `PJ予算` / `メンバー支払` / `対象外配賦` (= 旧「会社留保」、支払対象外メンバー配賦・現金支払なし) / `支払済み固定` / `実績未照合` / `これから支払予定` / `期末未払` / `保存後残予算または予算不足`。`クライアント支払` は本契約に別財布売上を加算し、schedule_based 契約では `contract_terms_json.monthlySchedule.amountTaxExcl` も予定売上として読む。`バッファ` は `value_plan_cycles.buffer_breakdown_json` を優先する。`原資上限 = (クライアント支払 - バッファ) × 65%`。`PJ予算 = 本契約原資 + 別財布原資`、`メンバー支払 = 支払済み固定 + これから支払予定`。対象外配賦の判定基準は `members.exclude_from_payout_notice=true` のみ (`is_officer` は支払分類に使わない、2026-07-29 まさ確定)。対象外配賦として合計するのは各月に cap から実際に配賦済みの `companyReserveYen` だけ。`stockYen` は月末残高スナップショットなので月次合計しない。支払対象メンバーの最終月 `stockYen` だけを期末未払として不足額へ入れ、対象外メンバーの繰越は非現金配賦の内部収束チェックで追う。原資超過も不足額そのものとして赤表示する。
