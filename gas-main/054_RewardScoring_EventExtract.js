...
function reward_extractEvents(payload) {
  ...
  // 1.5) 同一PJ+ymに既存イベントがあればスキップ（重複防止）
  var existSh = reward_repo_getSheet_("DB_ProgressEvents");
  var existHeader = reward_repo_headerMap_(existSh);
  var existLastRow = existSh.getLastRow();
  if (existLastRow >= 2) {
    var existVals = existSh.getRange(2, 1, existLastRow - 1, existSh.getLastColumn()).getValues();
    for (var chk = 0; chk < existVals.length; chk++) {
      var chkPid = String(existVals[chk][existHeader.projectId - 1] || "").trim();
      var chkYm  = String(existVals[chk][existHeader.ym - 1] || "").trim();
      if (chkPid === projectId && chkYm === ym) {
        return { ok: true, message: "既存イベントあり（スキップ）", eventCount: 0, skipped: true };
      }
    }
  }
  ...
  var userPrompt = "## PJ情報\n"
    ...
    + "各イベントには initiativeOrigin を必ず付与してください。\n"
    + "initiativeOriginの分類基準はsystem promptに記載の通りです。判断に迷う場合は無理に推定せず unknown としてください。";
  ...
  var eventRows = events.map(function(e) {
    return {
      ...
      initiativeOrigin: e.initiativeOrigin || "unknown"
    };
  });
...
function reward_extract_parseResponse_(text) {
  try {
    ...
    var validOrigins = { amd_proposed: true, co_decided: true, partner_proposed: true, external: true, unknown: true };
    ...
    // initiativeOriginの正規化
    events.forEach(function(e) {
      // 旧値の互換変換
      if (e.initiativeOrigin === "client_requested") e.initiativeOrigin = "partner_proposed";
      if (!e.initiativeOrigin || !validOrigins[e.initiativeOrigin]) {
        e.initiativeOrigin = "unknown";
      }
    });
