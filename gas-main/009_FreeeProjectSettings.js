...
function b_getFreeeProjectSettings_(projectId) {
  ...
  b_ensureHeader_("DB_Projects", ["projectId", "freeePartnerId", "clientName", "freeeInvoiceTemplateId"]);
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    const partnerName = (iClientName >= 0) ? String(values[r][iClientName] || "").trim() : "";
...
function b_updateDbProjectsFreee_(projectId, patch) {
  ...
  b_ensureHeader_("DB_Projects", ["freeePartnerId", "clientName", "freeeInvoiceTemplateId"]);
  ...
  // freeePartnerName → clientName に読み替え
  const normalizedPatch = {};
  Object.keys(patch).forEach(k => {
    const key = (k === "freeePartnerName") ? "clientName" : k;
    normalizedPatch[key] = patch[k];
  });

  const targets = Object.keys(normalizedPatch).filter(k => headers.indexOf(k) >= 0);
...
function api_searchFreeePartnersForCharter(payload){
  ...
  const json = b_freeeRequestAccounting_("get", "/partners", {
