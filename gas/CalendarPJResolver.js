// CalendarPJResolver.gs
// ============================================================
// Calendar Color -> PJ Resolver
// - CFG_ColorPJHistory を読み、colorId + startDate の履歴から pjCode を決定する
//
// 必須シート:
// - CFG_ColorPJHistory
//
// CFG_ColorPJHistory headers:
//   colorId | startDate | pjCode | note
//
// map[colorId] = [{ startMs, pjCode }, ...] を作り、eventDateに対して
// startMs <= eventDate(00:00) のうち最大のものを採用する
// ============================================================

/**
 * CFG_ColorPJHistory を読み取って履歴マップを作る
 * @param {SpreadsheetApp.Spreadsheet} ss
 * @returns {Object} historyMap
 */
function readColorPJHistory_(ss) {
  // ssが渡されても、基本は外部設定スプシを優先する（単一ソース化）
  const props = PropertiesService.getScriptProperties();
  const cfgId = String(props.getProperty("COLOR_PJ_CONFIG_SPREADSHEET_ID") || "").trim();

  let targetSs = null;

  if (cfgId){
    // 外部スプシを参照
    targetSs = SpreadsheetApp.openById(cfgId);
  } else {
    // フォールバック：呼び出し元ss（無ければActive）
    targetSs = ss || SpreadsheetApp.getActiveSpreadsheet();
  }

  const sh = targetSs.getSheetByName("CFG_ColorPJHistory");
  if (!sh) throw new Error("CFG_ColorPJHistory not found");

  const values = sh.getDataRange().getValues();
  if (!values || values.length < 2) return {};

  const header = (values[0] || []).map(h => String(h).trim());
  const iColor = header.indexOf("colorId");
  const iStart = header.indexOf("startDate");
  const iPJ = header.indexOf("pjCode");

  if (iColor < 0 || iStart < 0 || iPJ < 0) {
    throw new Error("CFG_ColorPJHistory headers must include: colorId, startDate, pjCode");
  }

  const map = {};

  for (let i = 1; i < values.length; i++) {
    const row = values[i];

    const colorId = String(row[iColor] || "").trim();
    const pj = String(row[iPJ] || "").trim();
    const sd = row[iStart];

    if (!colorId || !pj || !sd) continue;

    const startDate = (sd instanceof Date) ? sd : new Date(String(sd));
    if (isNaN(startDate.getTime())) continue;

    const normalized = new Date(startDate);
    normalized.setHours(0, 0, 0, 0);

    map[colorId] = map[colorId] || [];
    map[colorId].push({ startMs: normalized.getTime(), pjCode: pj });
  }

  Object.keys(map).forEach(k => {
    map[k].sort((a, b) => a.startMs - b.startMs);
  });

  return map;
}


/**
 * 色×履歴（CFG_ColorPJHistory）で PJ を引く
 * @param {string} colorId
 * @param {Date|string} eventDate
 * @param {Object} historyMap (readColorPJHistory_の返り値)
 * @returns {string} pjCode or ""
 */
function pickPJByColorHistory_(colorId, eventDate, historyMap) {
  const cid = String(colorId || "").trim();
  if (!cid) return "";

  const list = historyMap ? historyMap[cid] : null;
  if (!list || !list.length) return "";

  const t = (eventDate instanceof Date) ? new Date(eventDate) : new Date(String(eventDate));
  if (isNaN(t.getTime())) return "";

  t.setHours(0, 0, 0, 0);
  const ms = t.getTime();

  // startMs <= ms のうち最大を拾う
  let best = null;
  for (let i = 0; i < list.length; i++) {
    const it = list[i];
    if (it.startMs <= ms) best = it;
    else break;
  }
  return best ? String(best.pjCode || "").trim() : "";
}
