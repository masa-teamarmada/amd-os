...
function getCharterData(payload) {
  ...
  const out = {
    ...
    // ★freeePartnerName廃止：clientNameはcharter.clientNameを参照
...
function getCharterDataLite(payload) {
  ...
  // ★追加：freee取引先（表示の唯一正）
  _profMark_(prof, "freee settings (use clientName from charter)");
  ...
  const res = {
    ...
    // ★freeePartnerName廃止：clientNameはcharter.clientNameを参照
    freeePartnerId: String((charter && charter.freeePartnerId) || "").trim(),
