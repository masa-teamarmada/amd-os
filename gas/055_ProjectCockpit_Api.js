...
function cockpit_api_getData(payload) {
  ...
  var summaries = ymList.map(function(ym) {
    ...
    return {
      ...
      invoiceYm: String(b.invoiceYm || '').trim() || ym,
...
function cockpit_getProject_(projectId) {
  try {
    var sh = b_ss_().getSheetByName("DB_Projects");
    if (!sh) return null;
...
function cockpit_getBillingMap_(projectId, ymList) {
  ...
  try {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      var rawYm = values[i][(header.ym || header.yearMonth) - 1];
      var ym = String(rawYm instanceof Date ? Utilities.formatDate(rawYm, 'Asia/Tokyo', 'yyyyMM') : rawYm || "").trim();
...
function cockpit_api_getRoutineFlow(payload) {
  ...
  for (var fi = 0; fi < bizYms.length; fi++) {
    ...
    // invoiceYm: 空 or bizYmと同じ → 通常、それ以外 → 一括請求月に繰延
    var rawInvoiceYm = String(cycle.invoiceYm || '').trim();
    var effectiveInvoiceYm = rawInvoiceYm || bizYm;
    var isDeferred = (effectiveInvoiceYm !== bizYm);
    var deferredLabel = isDeferred ? (Number(effectiveInvoiceYm.slice(4)) + '月にまとめて請求') : '';

    // 一括請求月（このbizYmに他の月の分が集約される）かどうか
    var batchTargetYms = [];
    if (!isDeferred) {
      for (var bi = 0; bi < cycles.length; bi++) {
        var bc = cycles[bi];
        var bcYm = String(b_normYm_(bc.ym) || '');
        var bcInvYm = String(bc.invoiceYm || '').trim();
        if (bcInvYm === bizYm && bcYm !== bizYm) {
          batchTargetYms.push(bcYm);
        }
      }
    }
    ...
    var steps = [
      ...
      { id: "invoiceIssue", label: isDeferred ? deferredLabel : (batchTargetYms.length > 0 ? "請求書発行（" + (batchTargetYms.length + 1) + "ヶ月分）" : "請求書発行"),
        doneAt: isDeferred ? "deferred" : (cycle.invoiceIssuedAt || ""),
        doneBy: isDeferred ? "" : (cycle.invoiceIssuedBy || ""),
        deadline: isDeferred ? "" : deadlines.invoiceIssue,
        action: isDeferred ? "" : "cpOpenInvoiceModal('" + bizYm + "')",
        deferred: isDeferred },
      { id: "invoiceSend",  label: isDeferred ? deferredLabel : "請求書送付",
        doneAt: isDeferred ? "deferred" : (cycle.invoiceSentAt || ""),
        doneBy: isDeferred ? "" : (cycle.invoiceSentBy || ""),
        deadline: isDeferred ? "" : deadlines.invoiceSend,
        action: isDeferred ? "" : "cpOpenAdminBilling('" + projectId + "','" + bizYm + "')",
        deferred: isDeferred },
      { id: "payoutNotice", label: isDeferred ? deferredLabel : "支払通知書確認",
        doneAt: isDeferred ? "deferred" : (cycle.payoutNoticeUploadedAt || ""),
        doneBy: isDeferred ? "" : (cycle.payoutNoticeUploadedBy || ""),
        deadline: isDeferred ? "" : deadlines.payoutNotice,
        action: isDeferred ? "" : "cpOpenAdminPayouts('" + bizYm + "')",
        deferred: isDeferred },
      { id: "payment",      label: isDeferred ? deferredLabel : "入金確認",
        doneAt: isDeferred ? "deferred" : (cycle.paymentConfirmedAt || ""),
        doneBy: isDeferred ? "" : (cycle.paymentConfirmedBy || ""),
        deadline: isDeferred ? "" : deadlines.payment,
        action: isDeferred ? "" : "cpOpenModalToTab('" + nextYm + "','reward')",
        deferred: isDeferred },
      { id: "payout",       label: isDeferred ? deferredLabel : "報酬支払い",
        doneAt: isDeferred ? "deferred" : (payoutDone ? "done" : ""),
        doneBy: "",
        deadline: isDeferred ? "" : deadlines.payout,
        action: isDeferred ? "" : "cpOpenAdminPayouts('" + bizYm + "')",
        deferred: isDeferred }
    ];
...
function cockpit_updateMsProgressSummary_(projectId, ym) {
  try {
    ...
    // periodStartYm〜ymまでの全月の進捗を読み、各MSの最大値を取る
    // （前月に更新済みだが当月未タッチのMSが0に戻るのを防ぐ）
    var progMap = {};
    var allProgRows = rv2_readProgressForPlanCycle(planCycleId);
    for (var i = 0; i < allProgRows.length; i++) {
      var pr = allProgRows[i];
      var prYm = String(pr.ym || '').trim();
      if (prYm > ym) continue; // 未来月は除外
      if (periodStartYm && prYm < periodStartYm) continue; // 期間外は除外
      var msKey = String(pr.milestoneKey || '');
      var pct = Number(pr.progressPct || 0);
      if (pct > (progMap[msKey] || 0)) {
        progMap[msKey] = pct;
      }
    }
...
/**
 * invoiceYmを設定/解除する
 * @param {Object} payload { projectId, ym, invoiceYm }
 *   invoiceYm が空 → 解除（通常のym=請求月に戻す）
 */
function cockpit_api_setInvoiceYm(payload) {
  payload = payload || {};
  var projectId = String(payload.projectId || '').trim();
  var ym = String(payload.ym || '').trim();
  var invoiceYm = String(payload.invoiceYm || '').trim();
  if (!projectId || !ym) return { ok: false, message: 'projectId/ym required' };
  if (invoiceYm && !/^\d{6}$/.test(invoiceYm)) return { ok: false, message: 'invoiceYm invalid format' };

  try {
    b_ensureBillingCycleHeader_();
    var sh = b_ss_().getSheetByName('DB_BillingCycle');
    if (!sh) return { ok: false, message: 'DB_BillingCycle not found' };
    var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(function(h){ return String(h||'').trim(); });
    var iPid = headers.indexOf('projectId');
    var iYm = headers.indexOf('ym');
    var iInvYm = headers.indexOf('invoiceYm');
    if (iPid < 0 || iYm < 0 || iInvYm < 0) return { ok: false, message: 'header not found' };

    var data = sh.getDataRange().getValues();
    for (var r = 1; r < data.length; r++) {
      var rowPid = String(data[r][iPid] || '').trim();
      var rowYm = String(data[r][iYm] instanceof Date ? Utilities.formatDate(data[r][iYm], 'Asia/Tokyo', 'yyyyMM') : data[r][iYm] || '').trim();
      if (rowPid === projectId && rowYm === ym) {
        sh.getRange(r + 1, iInvYm + 1).setValue(invoiceYm);
        return { ok: true, ym: ym, invoiceYm: invoiceYm || ym };
      }
    }
    return { ok: false, message: 'row not found: ' + projectId + '/' + ym };
  } catch(e) {
    return { ok: false, message: String(e) };
  }
}
...
function cockpit_api_refreshMonthCache(payload) {
  ...
  if (!projectId || !ym) return { ok: false, message: 'projectId/ym required' };
  try {
    Logger.log('[refreshMonthCache] start: ' + projectId + '/' + ym);
    cockpit_updateRewardSummaryCache_(projectId, ym);
    cockpit_updateMsProgressSummary_(projectId, ym);
    Logger.log('[refreshMonthCache] done: ' + projectId + '/' + ym);
    return { ok: true };
  } catch(e) {
    Logger.log('[refreshMonthCache] ERROR: ' + projectId + '/' + ym + ' → ' + e);
    return { ok: false, message: String(e) };
  }
}
