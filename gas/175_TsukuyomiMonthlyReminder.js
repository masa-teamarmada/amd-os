...
function cron_meetingScheduleReminder() {
  return { ok:true, disabled:true, posted:0, note:"Meeting schedule reminders are abolished; no OS task is created." };
}
...
function cron_invoiceWorkflowDaily(){
  return { ok:true, disabled:true, posted:0, note:"Invoice workflow reminders are abolished; admin billing handles execution." };
}
...
function cron_reportFixReminder(){
  return { ok:true, disabled:true, posted:0, note:"Report confirmation reminders stay outside AMD OS surfaces." };
}
