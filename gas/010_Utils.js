...
/**
 * google.script.run 返却値の安全化
 * Date型を JST文字列に変換し、シリアライズ失敗を防ぐ
 * 使い方: return toClientSafe_({ ok: true, rows: rows });
 */
function toClientSafe_(obj) {
  return JSON.parse(JSON.stringify(obj, function(key, val) {
    if (val instanceof Date) {
      return Utilities.formatDate(val, "Asia/Tokyo", "yyyy年M月d日 HH:mm");
    }
    return val;
  }));
}