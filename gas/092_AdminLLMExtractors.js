...
function meeting_extract_basePrompt_(){
  return [
    ...
    "ソースは次の 2 種類が混ざる (どちらか / 両方):",
    "  === notion ===  Notion 議事録ページ本文 (人手 or 自動議事録ツールによる本文)",
    "  === gmail ===   会議日 ±1日に reportEmails 宛に届いたメール群",
    "                  (CircleBack 要約 / GMeet recording 通知 / 関係者やりとり 等)",
    "",
    "目的:",
    "その 1 回の MTG で何が起きたかを 4 軸 + 短いまとめで構造化する。",
    "",
    "重要:",
    "- gmail セクションには **会議と関係ないメール** が混ざりうる (営業メール・自動通知・別件)。",
    "  会議に関係する内容だけを拾い、関係ないものは無視する。",
...
function run_installMeetingExtractorConfig(){
  ...
  const desired = {
    ...
    note: "Phase 2: 1 calendar event ごとに Notion 本文 + Gmail (reportEmails ±1日) を結合 → 4 軸抽出",
    kind: "base",
    scopeType: "global",
    scopeKey: "",
    tags: "",
