...
function api_issueInvoiceFreee(projectId, ym, payload) {
  ...
  // 日付（必須）
  const billingDate = String(payload.issueDate || "").trim();
  const paymentDate = String(payload.dueDate || "").trim();
  ...
  var ymY = String(ym).substring(0, 4);
  var ymM = String(Number(String(ym).substring(4, 6)));
  const legacy =
  ...
  // ★基本行合計（=税抜請求額）をbaseLinesから算出
  const gross = baseLines.reduce((s, x) => s + (Number(x.unit_price || 0) * Number(x.quantity || 1)), 0);
  if (gross <= 0) {
    throw new Error("基本行の合計が0円。明細の単価を入力してね");
  }
  ...
  // PJマスタからprojectName + clientName
  var clientName = "";
  var projectName = String(cur.projectName || "");
  if (!projectName) {
    try {
      var pjRowsName = b_readTable_("DB_Projects");
      for (var pni = 0; pni < pjRowsName.length; pni++) {
        if (String(pjRowsName[pni].projectId || "").trim() === projectId) {
          projectName = String(pjRowsName[pni].projectName || "").trim();
          clientName = String(pjRowsName[pni].clientName || "").trim();
          break;
        }
      }
    } catch (e) {}
  ...
  let templateId = String(payload.templateId || settings.templateId || "").trim();

  if (!partnerId) throw new Error("freeePartnerId が未設定。クライアント確定→partner確定してね");
  if (!templateId) {
    var templateFetchErr = "";
    try {
      var templates = freee_listInvoiceTemplatesForSelect();
      if (templates && templates.length > 0) {
        templateId = String(templates[0].id || "").trim();
      }
    } catch (e) { templateFetchErr = String(e.message || e); }
    if (!templateId) throw new Error("templateId が空でfreeeからも取得できなかった" + (templateFetchErr ? "（" + templateFetchErr + "）" : ""));
  }

  // freeeの件名（clientNameは上で取得済み）
  const subject =
    invoiceSubject
      ? invoiceSubject
      : `${clientName || projectName} ${baseDesc}`;
  ...
  if (!totalBody || totalBody <= 0) {
    throw new Error("請求金額が0になってる。基本行/調整行の入力を確認してね");
  }
  ...
  const slackPayload = {
    ...
    invoicePdfFileId: (savedPdf && savedPdf.ok) ? String(savedPdf.fileId || "") : "",
    gross,
    reimbYen,
    uploadUrl: ""
  };
  ...
  try {
    ...
    if (baseUrl2) {
      ...
      slackPayload.uploadUrl = `${baseUrl2}?page=admin&tab=billing&projectId=${encodeURIComponent(projectId)}&ym=${ymForUi}`;
    }
  } catch(e) {}
