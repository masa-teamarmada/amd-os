...
function listProjects(payload) {
  ...
  const iFreeePnm = idx_("freeePartnerName");
  ...
  // ---- 非admin向け：参画PJのprojectId集合（isActive=trueのみ）
  ...
  if (!canSeeAll) {
    ...
    if (!email || !myMemberId) {
      const outEmpty = { ok:true, projects:[], userEmail:email, isAdmin:admin, email, profile: prof.marks };
      try{ cache.put(cacheKey, JSON.stringify(outEmpty), 30); }catch(e){}
      return outEmpty;
    }

    try {
      const psh = getSheet_("DB_ProjectMembers");
      const lastRowPM1 = psh.getLastRow();
      const lastColPM1 = psh.getLastColumn();
      const pvals = (lastRowPM1 >= 1 && lastColPM1 >= 1)
        ? psh.getRange(1, 1, lastRowPM1, lastColPM1).getValues()
        : [[]];
      _profMark_(prof, "DB_ProjectMembers read (for access)");
      ...
      if (pvals.length >= 2) {
        ...
        if (pi_projectId >= 0 && pi_memberId >= 0) {
          for (let r=1; r<pvals.length; r++){
            const row = pvals[r];
            const mid = String(row[pi_memberId]||"").trim();
            if (mid !== myMemberId) continue;

            const active = (pi_isActive >= 0) ? toBool_(row[pi_isActive]) : true;
            if (!active) continue;

            const pid = String(row[pi_projectId]||"").trim();
  ...
  _profMark_(prof, "access filter built");
  ...
  for (let i=0; i<targetRows.length; i++){
    ...
    projects.push({
      ...
      freeePartnerName: (iFreeePnm>=0) ? String(row[iFreeePnm]||"").trim() : "",
...
function createProject(payload) {
  ...
  const iFreeePnm = header.indexOf("freeePartnerName");
  if (iFreeePnm >= 0) outRow[iFreeePnm] = ""; // ★空
