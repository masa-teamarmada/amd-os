/**
 * R305_MonthlyReport_NotionExtract.gs
 * 
 * 役割:
...
function mr_notion_getProjectName_(projectId) {
  ...
  if (!ssId) throw new Error('MAIN_SPREADSHEET_ID 未設定');
...
function mr_notion_getContentPreview_(token, pageId) {
  try {
    return _notion_pageToPlainText_(token, pageId, { maxDepth: 6, maxChars: 4000 });
  } catch (error) {
...
/**
 * Notion議事録を last_edited_time >= sinceDate でフィルタして取得する（差分収集用）。
 * 通常版（mr_extractFromNotion_）は「日付」プロパティで月範囲フィルタするが、
 * 差分版は「過去24hに編集されたページ」を拾うために last_edited_time を使う。
 *
 * @param {string} projectId
 * @param {Date} sinceDate - この日時以降に編集されたページを取得
 * @return {Object} 抽出結果（mr_extractFromNotion_ と同じ構造）
 */
function mr_extractFromNotionDelta_(projectId, sinceDate) {
  try {
    var props = PropertiesService.getScriptProperties();
    var token = props.getProperty('NOTION_TOKEN');
    if (!token) return { success: false, error: 'NOTION_TOKEN未設定', pages: [] };

    var minutesDbId = props.getProperty('NOTION_DATABASE_ID');
    if (!minutesDbId) return { success: false, error: 'NOTION_DATABASE_ID未設定', pages: [] };

    var pjDbId = props.getProperty('NOTION_PJ_DATABASE_ID');
    if (!pjDbId) return { success: false, error: 'NOTION_PJ_DATABASE_ID未設定', pages: [] };

    var projectName = mr_notion_getProjectName_(projectId);
    if (!projectName) return { success: false, error: 'projectName not found: ' + projectId, pages: [] };

    var pjMap = _notion_buildPjCodeToPageIdMap_(token, pjDbId);
    var pjPageId = pjMap[projectName];
    if (!pjPageId) return { success: false, error: 'Notion PJ DB にPJなし: ' + projectName, pages: [] };

    // last_edited_time でフィルタ
    var pages = mr_notion_queryMinutesByPjDelta_(token, minutesDbId, pjPageId, sinceDate);
    var enrichedPages = mr_notion_enrichPages_(token, pages);
    ...
    return {
      ...
      pjPageId: pjPageId,
      pages: enrichedPages,
      stats: { totalPages: enrichedPages.length },
      period: { start: mr_formatReadable_(sinceDate), end: mr_formatReadable_(mr_now_()) }
    };

  } catch (error) {
    Logger.log('Notion差分抽出エラー: ' + error);
    return { success: false, error: error.toString(), pages: [] };
  }
}

/**
 * PJリレーション + last_edited_time で議事録DBをquery（差分用）
 * @param {string} token
 * @param {string} minutesDbId
 * @param {string} pjPageId
 * @param {Date} sinceDate
 * @return {Array}
 */
function mr_notion_queryMinutesByPjDelta_(token, minutesDbId, pjPageId, sinceDate) {
  var dbId = _notion_normId_(minutesDbId);
  var url = 'https://api.notion.com/v1/databases/' + dbId + '/query';
  ...
  var filter = {
    and: [
      {
        property: 'PJ',
        relation: { contains: pjPageId }
      },
      {
        timestamp: 'last_edited_time',
        last_edited_time: { on_or_after: sinceDate.toISOString() }
      }
    ]
  };
  ...
  Logger.log('Notion差分取得: ' + allPages.length + '件 (since: ' + sinceDate.toISOString() + ')');
  return allPages;
}