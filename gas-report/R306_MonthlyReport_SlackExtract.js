/**
 * R306_MonthlyReport_SlackExtract.gs
 * 
 * 役割:
...
function mr_slack_getMessages_(channelId, startDate, endDate) {
  ...
  const MAX_PAGES = 10; // 安全弁: 最大10ページ = 1000メッセージ
  const messages = [];
  let cursor = null;
  let hasMore = true;
  let page = 0;
  ...
  while (hasMore && page < MAX_PAGES) {
    ...
    const response = b_retry_(function() {
      return slack_callApi('conversations.history', params);
    }, { maxRetries: 2, label: 'Slack history page ' + page });
  ...
  if (page >= MAX_PAGES) {
    Logger.log('[Slack] ページ上限(' + MAX_PAGES + ')到達。取得メッセージ: ' + messages.length + '件');
  }
...
function mr_slack_getThreadReplies_(channelId, threadTs) {
  try {
    ...
    const response = b_retry_(function() {
      return slack_callApi('conversations.replies', {
        channel: channelId,
        ts: threadTs,
        limit: 100
      });
    }, { maxRetries: 2, label: 'Slack replies ' + threadTs });
