...
// ============================================================
// Time Budget（GAS 6分制限対策）
// ============================================================

/**
 * タイムバジェットを作成する。
 * 呼び出し時刻を基準に、残り時間を管理する。
 * @param {number} [budgetSec=300] - 全体の時間予算（秒）。デフォルト300秒（=5分、1分のマージン込み）
 * @return {Object} { start, budgetMs, isExpired(), remainMs(), label() }
 */
function b_timeBudget_(budgetSec) {
  var start = Date.now();
  var budgetMs = (budgetSec || 300) * 1000;
  return {
    start: start,
    budgetMs: budgetMs,
    /** 残り時間がゼロ以下なら true */
    isExpired: function() { return (Date.now() - start) >= budgetMs; },
    /** 残りミリ秒 */
    remainMs: function() { return Math.max(0, budgetMs - (Date.now() - start)); },
    /** ログ用ラベル */
    label: function() { return Math.round((Date.now() - start) / 1000) + '/' + Math.round(budgetMs / 1000) + '秒'; }
  };
}
...
/**
 * 関数を最大 maxRetries 回リトライする。
 * 指数バックオフ付き。GAS の UrlFetchApp 一時障害やSlack/Claude API の429対策。
 *
 * @param {Function} fn - 実行する関数（引数なし、戻り値あり）
 * @param {Object} [opts]
 * @param {number} [opts.maxRetries=3] - 最大リトライ回数（初回含まず）
 * @param {number} [opts.baseMs=1000] - 初回待機ミリ秒
 * @param {string} [opts.label=''] - ログ用ラベル
 * @return {*} fn() の戻り値
 * @throws 全リトライ失敗時は最後のエラーをそのまま throw
 */
function b_retry_(fn, opts) {
  opts = opts || {};
  var maxRetries = (opts.maxRetries != null) ? opts.maxRetries : 3;
  var baseMs = opts.baseMs || 1000;
  ...
  for (var attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      var waitMs = baseMs * Math.pow(2, attempt); // 1s, 2s, 4s
      Logger.log('[b_retry_] ' + label + ' attempt ' + (attempt + 1) + ' failed: ' + e + ' → wait ' + waitMs + 'ms');
