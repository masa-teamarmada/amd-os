/**
 * R060_RewardV2_Estimator.gs
 * つくよみによるprogress自動推定（L2 cron）。
 * R313_MonthlyReport_Cron.gsのcron完了後にチェーン呼び出しされる。
 *
 * 依存:
 *   - R058_RewardV2_Repo.gs（rv2_upsertProgress, rv2_readProgressForProjectYm）
 *   - R086_ValuePlanRepo.gs（valuePlan_repo_getCycleSheet, valuePlan_repo_getMilestoneSheet）
 *   - R302_MonthlyReport_Collector.gs（mr_collectDeltaActivities_）
 *   - R311_SourceCacheRepo.gs（srcCache_listByProjectYm）
 *   - R172_TsukuyomiContextRepo.gs（tsukuyomi_getActiveSystemPrompt）
 *   - R000_Sheets.gs（b_readTable_, ensureHeader_）
 */
...
function rv2_estimateFromSources_(projectId, ym, fullScan) {
  ...
  if (sourceTexts.length === 0) {
    return { ok: false, message: "推定ソースなし" };
  }
  ...
  Logger.log("[estimate] LLM raw=" + (raw ? raw.substring(0, 500) : "null"));
  ...
  for (var i = 0; i < progressArr.length; i++) {
    ...
    for (var mi = 0; mi < msList.length; mi++) {
      if (String(msList[mi].milestoneId || "") === msKey) {
    ...
    if (cur && (cur.source === "pm_manual" || cur.source === "criteria_toggle")) continue;
    if (cur && newCumPct <= cur.pct) continue;
...
function rv2_est_getMembers_(projectId) {
  ...
  for (var i = 0; i < memberRows.length; i++) {
    var mm = memberRows[i];
    nameMap[String(mm.memberId || "")] = String(mm.codeName || mm.memberId || "");
  }
  var result = [];
...
function rv2_est_buildPrompt_v2_(allSourcesText, planInfo, members, prevMap, currentMap, ym) {
  ...
  if (!staticRules) {
    Logger.log("WARNING: reward_estimate context not found in DB_TsukuyomiContext.");
...
function rv2_est_parseProgressOnly_(raw, planInfo) {
  try {
    ...
    for (var i = 0; i < rawProg.length; i++) {
      ...
      progress.push({ milestoneKey: key, progressPct: pct, reason: String(p.reason || "") });
    }
    ...
    for (var i = 0; i < planInfo.milestones.length; i++) {
      ...
      if (!found) progress.push({ milestoneKey: msId, progressPct: 0, reason: "情報ソースに言及なし" });
    }
