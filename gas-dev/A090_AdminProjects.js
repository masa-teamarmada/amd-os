...
function admin_listProjectsFull(){
  ...
  try{
    ...
    const iCl  = h.indexOf("clientName");
    ...
    const iStartYm = h.indexOf("startYm");
    const iEndYm = h.indexOf("endYm");
    ...
    for (let r=1; r<vals.length; r++){
      ...
      projects.push({
        ...
        clientName: (iCl>=0) ? String(vals[r][iCl]||"").trim() : "",
        ...
        startYm: (iStartYm>=0) ? String(vals[r][iStartYm]||"").trim() : "",
        endYm: (iEndYm>=0) ? String(vals[r][iEndYm]||"").trim() : "",
    ...
    try {
      ...
      if (mvals && mvals.length >= 2) {
        const mh = mvals[0].map(x => String(x||"").trim());
        const iMid  = mh.indexOf("memberId");
        const iCode = mh.indexOf("codeName");
        const iMst  = mh.indexOf("status");
        for (let r = 1; r < mvals.length; r++) {
          const st = (iMst >= 0) ? String(mvals[r][iMst]||"").trim().toLowerCase() : "";
          if (st !== "active") continue;
          const mid  = (iMid  >= 0) ? String(mvals[r][iMid] ||"").trim() : "";
          const code = (iCode >= 0) ? String(mvals[r][iCode]||"").trim() : "";
          if (!mid) continue;
          activeMembers.push({ memberId: mid, codeName: code });
        }
        activeMembers.sort((a,b) => String(a.codeName).localeCompare(String(b.codeName), "ja"));
      }
    } catch(e) {}

    return toWireSafe_({ ok:true, projects, activeMembers });
...
function admin_listFreeePartners(){
  function toWireSafe_(obj){
    try { return JSON.parse(JSON.stringify(obj)); }
    catch(e){ return { ok:false, message:"toWireSafe failed: "+String(e?.message||e), partners:[] }; }
  }
  ...
  try{
    ...
    if (cached){
      try{
        ...
        if (parsed && parsed.ok && Array.isArray(parsed.partners)) return toWireSafe_(parsed);
      }catch(e){}
    }

    const companyId = String(PropertiesService.getScriptProperties().getProperty("FREEE_COMPANY_ID") || "").trim();
    if (!companyId) return toWireSafe_({ ok:false, message:"FREEE_COMPANY_ID not set", partners:[] });

    const all = [];
    const limit = 500;
    for (let offset = 0; offset < 20000; offset += limit){
      const json = b_freeeRequestAccounting_("get", "/partners", { company_id: companyId, limit, offset }, null);
      const part = (json.partners || [])
        .map(p => ({ id: String(p.id||"").trim(), name: String(p.name||p.display_name||"").trim() }))
    ...
    const result = { ok:true, partners: all };
    cache.put(cacheKey, JSON.stringify(result), 300);
    return toWireSafe_(result);

  }catch(err){
    return toWireSafe_({ ok:false, message:"admin_listFreeePartners failed: "+String(err?.stack||err?.message||err), partners:[] });
  }
}
...
function admin_updateProjectAllFields(payload){
  ...
  try{
    ...
    const startYm        = String(payload.startYm        || "").trim();
    const endYm          = String(payload.endYm          || "").trim();
    ...
    var iStartYm = h.indexOf("startYm");
    var iEndYm   = h.indexOf("endYm");
    if (iStartYm >= 0)                  sh.getRange(hitRow, iStartYm+1).setValue(startYm);
    if (iEndYm   >= 0)                  sh.getRange(hitRow, iEndYm+1).setValue(endYm);

    if (iFreee  >= 0 && !clientName)     sh.getRange(hitRow, iFreee+1).setValue(freeePartnerId);
    // ↑ clientNameが指定された場合はupdateProjectClient内で既にfreeePartnerIdが書かれてるのでスキップ
