...
- 現状: 2026-06-02に完全セルフ出願 readiness worker `019e86a7-eaa4-7923-9f3c-bdda2346a601` をactiveとして切り出し済み。成果物候補は `docs/ip/2026-06-02_self_filing_readiness_check_internal.md`。JPO/特許庁公式情報など一次情報のWeb調査は可、外部送付・弁理士問い合わせ・DB write・production DB接続は禁止。
- 残課題: workerから、セルフ出願に必要な書類一式、既存成果物の対応状況、出願前TODO、拒絶対応を見越した逃げ道、セルフ出願の地雷、まさ判断事項5個以内、終了ゲートの報告を受ける。
