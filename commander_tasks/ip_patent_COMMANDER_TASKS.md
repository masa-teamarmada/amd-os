...
- 現状: 2026-06-02に実名調査worker `019e8434-52c8-7b83-9cbe-520a3aadc4ca` をactiveとして切り出し済み。成果物候補は `docs/ip/2026-06-02_patent_attorney_candidate_research_internal.md`。Web調査可だが、一次情報・公式プロフィール・事務所公式・公開インタビュー等を優先し、外部送付・弁理士問い合わせ・DB write・production DB接続は禁止。
- 残課題: workerから、候補表、参照情報源、相性分類、まさ判断事項5個以内、未確認/要確認、終了ゲートの報告を受ける。候補の良し悪しは断定せず、初回打診先を選ぶための事前整理に留める。
