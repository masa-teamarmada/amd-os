...
struct AMDOSScreenView: View {
    ...
    var body: some View {
        Group {
            ...
            case .amdScore: AMDOSCurrentSpsView(onSelectProject: onSelectProject)
            case .amdScoreDetail: AMDOSCurrentSpsDetailView(projectId: selectedProjectId)
            case .amdScoreRetrofit: AMDOSRetiredScoreRouteView()
            case .ventureCyberspace: AMDOSCurrentSpsView(onSelectProject: onSelectProject)
