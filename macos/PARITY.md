...
| `/institutions`, `/institutions/[institutionId]`, `/institutions/[institutionId]/cockpit`, `/institutions/assess` | `institutions` | `institution_*`, `institution_projects`, `projects` | assessment API / 既存PJ cockpit API | member / APIごとの権限 | 実装済み。契約前を含む研究機関カタログを初期表示し、稼働中PJを上位・色帯表示。関連PJは固定表でなく `institution_projects` から解決し、KUTE p25 / NIMS p28 / 愛媛大全体p30を同じ経路で扱う。ECR 8軸、関連PJ cockpit、月別MTGツリーを維持。実データ読取・代表遷移は未確認 |
