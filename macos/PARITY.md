...
| `/project/[projectId]/cockpit?tab=technology&tech=cost-fuel`, `/project/[projectId]/workspace#technology` | 未割当 | 技術タブの中の「コスト試算（燃料）」(2026-09-14追加、燃料の試算 `project_cost_models.case_kind = 'biodiesel'` を持つPJだけ。いまは SX)。`/api/project-cost-model?kind=fuel` の束（`project_cost_*`）を読み、FAME転換（外部に委託 / 自社で行う）× 収率（低位 / 基準 / 改善）の6通りの燃料1Lあたりの総コストをクライアントで再計算する（`project-fuel-cost-model.ts`）。左の操作パネル（作業の流れと工数、事業と製造の条件・収率の3ケースの表・CAPEX・OPEX・残渣の行き先。保存しない試算、admin は「この値を保存」）と右の結果（6通りの積み上げ棒・内訳の棒グラフ・売価で成立する菌体の原価・事業全体の年間）を1枠に並べ、下に読み物。PWA/ワークスペース共通コンポーネント。macOS Native未移植 |
