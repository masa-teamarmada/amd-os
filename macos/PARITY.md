...
| `/project/[projectId]/cockpit?tab=competition`, `/project/[projectId]/workspace#competition` | 未割当 | 事業計画グループの「競合比較」タブ（2026-09-14追加、同日に技術タブの区分から「技術」の右隣のタブへ移した。技術台帳に区分 `tech_domain = '競合比較'` のトピックを持つPJだけ。いまは SX）。`/api/project-tech` の束（`project_tech_*`）から区分「競合比較」だけを出し、技術タブからは外す。形は技術タブと同じ4つで、区分のタブ・全体像・未整理の断片は出さず先頭のトピックを開く（`?competition=`）。SX は先頭2枚（排水処理と金属回収の星取り表・燃料の比較表）が公開可。PWA/ワークスペース共通コンポーネント `CockpitTechnology`（`mode="competition"`）。macOS Native未移植 |
