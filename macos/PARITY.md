...
| `/my-projects`, `/project/[projectId]/workspace` | `myProjects`, `projectWorkspace` | PWA共有 `getCurrentMemberAccess` / `getProjectWorkspaceBundle` DTO | `POST /api/project-workspace/[projectId]/effort` | PJ限定member / portfolio member | **P0 Native 実装済み（テーマタブは未移植）**。参加設定済みPJのみの一覧、週次の予定/実績入力、メンバー別配分、MS、活動件数・source集計、6週推移をSwiftUIで実装。2026-08-26にPWAへ追加、2026-08-31拡張したp19の4テーマ（KR経営改革 / 水素循環PJ / OkuDoor運営 / OkuDoorシステム開発＆運用）、9成果目標の4/2/2/1接続、予定進行と確定進捗の区別、AMD内部のホーム/コックピット導線に加えて、運用タスク/運用マイルストーン/論点・決定/予定成果物/作業間の関連の作成・編集ハブはNative未移植。MTGの新規作成・編集はPWA側でも一時停止中（既存の会議記録の公開範囲ポリシー承認待ち）。PJ限定ユーザーは通常のSupabase table readを使わず共有DTOだけを読む。PWA実データでの確認・書込みは未実施 |
| `/project/[projectId]/cockpit`, `/project/[projectId]/config`, `/project/[projectId]/report/[ym]/print` | `projectCockpit`, `projectConfig`, `projectReportPrint` | Cockpit / `projects`, `ms_*`, `billing_cycles`, `project_meeting_summaries`、提出用月次集約 | PWA既存の進捗・月次ノート・報酬同期・MS修正・資料・会議・レポートAPI | member / APIごとの権限 | Native実装。`ym`/`tab`/`meeting`/`document` deep link (`tab=cost-model` = コスト試算)、PJ資料のDrive導線とMarkdown本文のPWA同一PATCH、助成金台帳への既存管理導線、戦略シグナルの修正履歴と `dialog/start → refine → confirm`、会議履歴の再読込・全件表示、`monthly-report-print`集約値をA4標準印刷で全章出力する。2026-09-01にPWAへ入った通常3グループ／研究機関4グループの二階層ナビ、進捗管理内のガント／目的構造分割、PCの親グループ子一覧フロート、活動・実績のPJ概要移設、資本政策の事業計画所属、テーマタブ廃止はNative未移植。実認証・実書込み・実印刷は未確認 |
...
| `/vcs`, `/vcs/[id]`, `/vcs/[id]/edit`, `/vcs/inbox`, `/vcs/investments` | `vcs` | `vcs`, `vc_funds`, `vc_investments`, `startup_companies`, `startup_funding_rounds`, `vc_contacts`, `project_vc_relations`, `vc_news` | PWA同一RLS直書き | member / admin API | VC一覧・詳細・編集・受信箱は実装済み。2026-09-09にPWAへ追加した投資履歴の横断表（出資・完了日/公表日、VC個別額/ラウンド総額、成立/確認状態、根拠URL、絞り込み）はNative未移植。既存Native出資先表示を候補と確認済みが混ざった状態へ拡張しない |
...
| `/project/[projectId]/cockpit?tab=cost-model`, `/project/[projectId]/workspace#cost-model` | 未割当 | コスト試算タブ（燃料の試算を持つPJでは PWA のタブ名は「コスト試算（廃液）」、2026-09-14）(2026-08-23追加、全PJ常設、2026-09-13 シミュレーター化・作業の流れとオフサイト、2026-09-14 方式と装置の分離・誰がやるか・誰が持つか・年間処理量から年に作る量・共通の作業単価と CAPEX / OPEX の区分・上端の槽を外し効かない前提を薄く出す・顧客工場の装置の費用を顧客に・金額をカンマ区切りの円に・オフサイトの売価と年間処理量を別に・明細の行の下に計算の式と根拠・オフサイトで引き取る液の濃さを別に、2026-09-15 菌体のバッチ間のマスバランス（回収率は次のバッチへ回す割合）・目標放流水濃度・金属回収の中央回収設備の CAPEX)。`project_cost_models` / `_assumptions` / `_items` / `_tasks` / `_questions` / `_notes` を読み、株×用途×方式（オンサイト / オフサイト）×装置（循環カートリッジ / 直接投入）×槽をクライアントで再計算する（作業の `performer` と明細の `bearer` で、SX がやる作業と SX が持つ明細だけを原価に入れる。年に作る量は年間処理量から計算し、製造拠点の明細を培養設備の系列として並べる）。左の操作パネル（一番上に総コストの内訳（大きい順。区分の札を押すとその額を乗せている小分けへ移る、2026-09-14）、作業の流れと工数の下に、前提・作業リストと誰がやるか・明細と誰が持つかを「事業と処理の条件 / CAPEX / OPEX」の枠と小分けに並べて書き換える。作業単価は共通の1つ、数字の欄は3桁カンマ、開いたときは自然株、上端に槽は出さず選んだ組み合わせで効かない前提は薄く出す。保存しない試算、admin は「この値を保存」）と右の結果（方式と装置ごとの積み上げ棒・内訳の棒グラフ・SXの作業工数・年に作る量と系列数・事業全体の年間。顧客がやる作業の時間は出さない）を1枠に並べ、下に想定系・CAPEX/OPEX内訳・確度別内訳・確認事項を出す。PWA/ワークスペース共通コンポーネント。macOS Native未移植 |
| `/project/[projectId]/cockpit?tab=cost-fuel`, `/project/[projectId]/workspace#cost-model-fuel` | 未割当 | 事業計画グループの「コスト試算（燃料）」タブ（2026-09-14追加、同日に技術タブの中から「コスト試算（廃液）」の右隣へ移した。燃料の試算 `project_cost_models.case_kind = 'biodiesel'` を持つPJだけで、そのPJではコスト試算タブを「コスト試算（廃液）」と呼ぶ。いまは SX）。`/api/project-cost-model?kind=fuel` の束（`project_cost_*`）を読み、FAME転換（外部に委託 / 自社で行う）× 収率（低位 / 基準 / 改善）の6通りの燃料1Lあたりの総コストをクライアントで再計算する（`project-fuel-cost-model.ts`）。左の操作パネル（一番上に総コストの内訳（大きい順、札から小分けへ移る）、作業の流れと工数、事業と製造の条件・収率の3ケースの表・CAPEX・OPEX・残渣の行き先。明細の行の下に、数量 × 単価 から円/L までの計算の式と、根拠（明細の説明）を出す（2026-09-14）。保存しない試算、admin は「この値を保存」）と右の結果（6通りの積み上げ棒・内訳の棒グラフ・売価で成立する菌体の原価・事業全体の年間）を1枠に並べ、下に読み物。PWA/ワークスペース共通コンポーネント。macOS Native未移植 |
| `/project/[projectId]/cockpit?tab=competition`, `/project/[projectId]/workspace#competition` | 未割当 | 事業計画グループの「競合比較」タブ（2026-09-14追加、同日に技術タブの区分から「技術」の右隣のタブへ移した。技術台帳に区分 `tech_domain = '競合比較'` のトピックを持つPJだけ。いまは SX）。`/api/project-tech` の束（`project_tech_*`）から区分「競合比較」だけを出し、技術タブからは外す。形は技術タブと同じ4つで、区分のタブ・全体像・未整理の断片は出さず先頭のトピックを開く（`?competition=`）。SX は先頭3枚（競合の会社との星取り表・既存の方式との星取り表・燃料の比較表）が公開可で、`presentation`（migration 425）を持つ星取り表は PDF と同じ並び（見出し・表の上の一文・説明・表・注記、自社の列と強調する行に色）で出す（`MatrixSheet`）。PWA/ワークスペース共通コンポーネント `CockpitTechnology`（`mode="competition"`）。macOS Native未移植 |
| `/project/[projectId]/cockpit?tab=business-model`, `/project/[projectId]/workspace#business-model` | 未割当 | 事業計画グループの「ビジネスモデル」タブ（2026-09-14追加、「競合比較」の右隣。技術台帳に区分 `tech_domain = 'ビジネスモデル'` のトピックを持つPJだけ。いまは SOL）。`/api/project-tech` の束（`project_tech_*`）から区分「ビジネスモデル」だけを出し、技術タブと競合比較からは外す（`techLedgerTabOf`）。形は技術タブと同じ4つで、区分のタブ・全体像・未整理の断片は出さず先頭のトピックを開く（`?business-model=`）。SOL は先頭に事業全体のまとめ（本文のピクト図 `PictogramDiagram`〔2026-09-17、本文のコードブロック `pictogram` を3×3の枠の図にする。モノ・サービスは青、お金は橙と ¥ の印〕と、案Cの役割分担・収益の柱の表）、その後ろにオフサイトの廃液処理の検証（計11トピック、社内限定）。PWA/ワークスペース共通コンポーネント `CockpitTechnology`（`mode="business-model"`）。macOS Native未移植 |
