...
| `/project/[projectId]/cockpit?tab=business-model`, `/project/[projectId]/workspace#business-model` | 未割当 | 事業計画グループの「ビジネスモデル」タブ（2026-09-14追加、「競合比較」の右隣。技術台帳に区分 `tech_domain = 'ビジネスモデル'` のトピックを持つPJだけ。いまは SOL）。`/api/project-tech` の束（`project_tech_*`）から区分「ビジネスモデル」だけを出し、技術タブと競合比較からは外す（`techLedgerTabOf`）。形は技術タブと同じ4つで、区分のタブ・全体像・未整理の断片は出さず先頭のトピックを開く（`?business-model=`）。SOL はオフサイトの廃液処理の検証10トピック（社内限定）。PWA/ワークスペース共通コンポーネント `CockpitTechnology`（`mode="business-model"`）。macOS Native未移植 |
