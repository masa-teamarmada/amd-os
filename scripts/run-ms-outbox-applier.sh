...
STRATEGY_AUTOMATION_DIR="/Users/masa/.codex/automations/amd-os-strategy-signals"
...
mkdir -p "${MS_AUTOMATION_DIR}/outbox" "${STRATEGY_AUTOMATION_DIR}/outbox" "${ATLAS_AUTOMATION_DIR}/outbox" "${LOG_DIR}"

MS_COUNT=$(find "${MS_AUTOMATION_DIR}/outbox" -maxdepth 1 -type f -name '*.json' 2>/dev/null | wc -l | tr -d ' ')
STRATEGY_COUNT=$(find "${STRATEGY_AUTOMATION_DIR}/outbox" -maxdepth 1 -type f -name '*.json' 2>/dev/null | wc -l | tr -d ' ')
...

{
  ...
  if [ "${STRATEGY_COUNT}" != "0" ]; then
    node pwa/scripts/ms_progress_review_tool.mjs apply-outbox-dir --dir "${STRATEGY_AUTOMATION_DIR}/outbox" || STATUS=$?
  fi
