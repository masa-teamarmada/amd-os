...
mkdir -p "${MS_AUTOMATION_DIR}/outbox" "${ATLAS_AUTOMATION_DIR}/outbox" "${LOG_DIR}"
...
if [ "${MS_COUNT}" = "0" ] && [ "${ATLAS_COUNT}" = "0" ]; then
  exit 0
fi

{
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] applying amd-os-ms=${MS_COUNT} amd-atlas=${ATLAS_COUNT} outbox file(s)"
  cd "${REPO_DIR}" || exit 1
  STATUS=0
  if [ "${MS_COUNT}" != "0" ]; then
