...
ATLAS_STAGING_OUTBOX_DIR="/Users/masa/.codex/automations/amd-atlas-2/outbox"
LOG_DIR="${MS_AUTOMATION_DIR}/logs"
LOG_FILE="${LOG_DIR}/outbox-applier.log"

mkdir -p "${MS_AUTOMATION_DIR}/outbox" "${STRATEGY_OUTBOX_DIR}" "${ATLAS_AUTOMATION_DIR}/outbox" "${ATLAS_STAGING_OUTBOX_DIR}" "${LOG_DIR}"
...
ATLAS_STAGING_COUNT=$(find "${ATLAS_STAGING_OUTBOX_DIR}" -maxdepth 1 -type f -name '*.json' 2>/dev/null | wc -l | tr -d ' ')
if [ "${MS_COUNT}" = "0" ] && [ "${STRATEGY_COUNT}" = "0" ] && [ "${ATLAS_COUNT}" = "0" ] && [ "${ATLAS_STAGING_COUNT}" = "0" ]; then
  exit 0
fi

{
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] applying amd-os-ms=${MS_COUNT} amd-os-strategy-signals-outbox=${STRATEGY_COUNT} amd-atlas=${ATLAS_COUNT} amd-atlas-staging=${ATLAS_STAGING_COUNT} outbox file(s)"
  cd "${REPO_DIR}" || exit 1
  STATUS=0
  if [ "${MS_COUNT}" != "0" ]; then
  ...
  if [ "${ATLAS_STAGING_COUNT}" != "0" ]; then
    node pwa/scripts/atlas_signal_review_tool.mjs apply-outbox-dir --dir "${ATLAS_STAGING_OUTBOX_DIR}" || STATUS=$?
  fi
