...
2. **`seed_bzm30_inputs` の p22（OptQC）の手元資金に 70億円が入っていた**。OSにあるのは
   シリーズA1 15億円（2025-10）と累計調達額21.5億円で、70億円の根拠が無い。直近ラウンドの調達額へ置き直した。
3. **p06（CrestecBio）は `project_freeze_periods` で「202605に凍結状態」（status=active）なのに、
   `projects.status` は active のまま**。台帳の二か所が食い違っている。
