...
## 0. この理論は何のためにあるか {#purpose}

出典は[領域定義](../bzm/sps-current-domain-definition.md) §1・§3・§4・§5・§6（`domain-fixed v1.0`、2026-08-09 確定）。この節はその要約であり、新しい主張を足していない。

**目的は二つある。**

> 目的は BZSF での投資先の判定で使うことと、before zero の領域の新しい学術体系構築にある
> — まさ、2026-08-09（同 §1 に引用）

**対象は「研究成果が世に出てから法人化に至るまで」**（領域としての Before Zero、同 §4）。問うのは**このシーズは事業になるか**である。誰が担い手になるか、どの組織が運用するかは、理論の変数ではない。

**中心命題は二つある**（同 §3）。

1. **タイミングの窓は両側から閉じる。** 早すぎる設立も、遅すぎる決断も失敗である。
2. **見えない失敗。** 会社になる前に立ち消えたシーズはどこにも記録されず、失敗として数えられない。この存在が理論の核心的な発見である。

**予測地平がゼロの先まで伸びる理由**（同 §3）。設立をゴールにすると三つ壊れる。設立は成果ではなく人間の判断の記録にすぎないこと。早すぎる設立が「到達成功」に化けて第一の中心命題と矛盾すること。遅すぎた失敗を永久に検証できないこと。だから Before Zero で観測・診断・介入し、**ゼロの先の到達で検証する**。予測地平が先にあることは、特化の放棄ではなく、**特化した理論が反証可能であるための要件**である。

**投資判定は二階建て**（同 §5）。一階が BZM の出力（到達見込み $q$ と価値 $\mathbf P$）、二階が BZSF 投資層（回収経路の設計、投資契約、追加投資と停止の判断、ポートフォリオ管理）。**BZM 単体で GO・投資額・投入人月を決めない。**

**自前で主張してよい範囲**（同 §6）。BZM 理論としての新規性の主張は、**Before Zero と設立境界に固有の測定に限定する**。設立後のモジュールは、確立された既存理論の輸入として構成する。ここを混ぜると既存のスタートアップ研究との差が消え、学術体系としての足場を失う。

> **この節の位置づけ（2026-08-22 追記）**：まさ指摘「この理論を構築した目的は、正本に書かれてないの？」を受けて置いた。目的は領域定義に最初から書かれていたが、版数台帳と OS 画面には出ていなかった。えいみが目的を実務（735件の接触順）から推定して設計を進めてしまう事故が起きたため、正本入口の先頭へ移した。**以後の要素・要因・数式は、この節の目的から導かれているかを先に確認する。** 作業の都合から逆算しない。
...
**SPS は BZM から独立した別系列ではない。BZM の中の出力である。**

SPS の式を定める到達見込みモデルは、正本自身が「固定方針下の到達見込みを定めた **BZM 2.0 の履歴版**」と位置づけている（[到達見込みモデル](../bzm/sps-current-reachability-model.md) 冒頭「2.2との版境界」）。

$q$（到達見込み）の定義に入る $T_Y$（戦略余力が尽きる時点）は、BZM 2.2 が BZM 2.0 の定義を置き換えて供給する（[BZM 2.2](../bzm/bzm-2-2-strategic-slack-and-propulsion.md) §2「BZM 2.0で定義した $T_Y$ は、2.2では互換表示を残しながら中身を置き換える」）。

構造は [§5](#lineage-relation) に置いた。

> **初版の誤り（2026-08-22 訂正）**：初版はこの2つを「別系列」「別の問いに答える」と書いた。誤りである。正本が「合算しない・相互変換しない」と定めているのは、**同じモデルから出る別々の出力**（推進力・戦略余力・動的正味PJ価値 $J$・成功条件付き価値 $P$・SPS）を同じ数値として扱うな、という運用規律であって、構造として無関係だという意味ではない。
>
> まさ指摘 2026-08-22:「SPSとBZM2.2が別系列にされてる時点で全く認識があってない。BZM2.2で定量化しているパラメータがSPSだよ。」

一つの合成スコアにはしない（これは運用規律として有効）。
...
出典: [到達見込みモデル](../bzm/sps-current-reachability-model.md) §1・§3。
...
割引率の置き方について、正本は評価方式を一つだけ選び、同じリスクを期待キャッシュフロー・経路確率・割引率へ重複投入しないことを規律として定めている（[到達見込みモデル](../bzm/sps-current-reachability-model.md) §1）。
...
出典: [到達見込みモデル](../bzm/sps-current-reachability-model.md) §1、[BZM 2.0 改訂要求書](../bzm/BZM_2_0_REVISION_REQUIREMENTS.md) §4.2。
...
**2026-08-22 に、SPS現行正本3本のファイル名から世代名を外した。**

`sps-2-0-reachability-model.md` → `sps-current-reachability-model.md`、`sps-2-0-domain-definition.md` → `sps-current-domain-definition.md`、`sps-2-0-measurability-gate.md` → `sps-current-measurability-gate.md`（まさ指示「改名して」、承認 [#2026-08-22-2](APPROVALS.md#2026-08-22-2)）。旧名は世代名 2-0 が残ったままだったため、「2.0=古い版」という誤解を生んでいた（まさが実際にそう誤読した）。中身は改名前から現行のままで、式・定義・数値は変わっていない。名前構造が同じ `sps-2-0-domain-definition.md`・`sps-2-0-measurability-gate.md` も同時に改名した。旧slugは `/bzm`・`/model` の両方でリダイレクトする（`BZM_SLUG_ALIASES`、`pwa/src/app/(app)/bzm/bzm-chapters.ts`）。事前登録・監査・変更履歴などの凍結記録に残る旧ファイル名の記述は、当時の名前のまま書き換えていない。
...
| 2.0 | 動的到達予測モデル | 固定した計画の到達経路と到達見込み | $q_{\mathrm{plan},\tau}(H_v)=\Pr^{\mathbb P}(T_C<T_Y,\ T_C\le H_v\mid\mathcal I_\tau)$、$\mathrm{SPS}_{\mathrm{all},\tau}=\sum_o q_{o,\tau}P_{o,\tau}$ | `theory-fixed v1.4`。前向き検証0件。凍結状態は [BZM 2.0 凍結スナップショット](../bzm/BZM_2_0_FROZEN_SNAPSHOT_2026-08-11.md) | [到達見込みモデル](../bzm/sps-current-reachability-model.md)／[BZM 2.0 診断スコア暫定仕様](../bzm/BZM_2_0_DIAGNOSTIC_SCORE_SPEC.md)（`draft v0.7`） |
...
### 推進力 — 資源を進捗へ変える能力 {#bzm-propulsion}

まさの言う「変換能力」がこれである。

> 2.2でいう**推進力**は、資源の量でも支出額でもない。潜在能力 $\mathbf c_t$ は行動前に会社へ蓄積しており、意思決定ではその posterior summary $\widehat{\mathbf c}_t$ を使う。推進力は能力そのものではなく、その能力、状態、行動、投入束から予測される**次状態と所要時間の分布**である。
> — [BZM 2.2](../bzm/bzm-2-2-strategic-slack-and-propulsion.md) §6

| 記号 | 意味 |
|---|---|
| $\mathbf c_t$ | 潜在能力。行動より前に会社へ蓄積している、観測できない能力の束 |
| $\widehat{\mathbf c}_t$ | その事後要約。意思決定で実際に使う推定値 |

同じ一億円でも、何も閉じなかった一億円と、量産条件・顧客契約・資金調達の三つを前進させた一億円は異なる（同 §6、§11.1）。

**一つの点数へ潰さない。** 正本は少なくとも次を別に測れと定める（同 §6）。
...
4. 契約、許認可、顧客確約の前進
5. 新しく実行可能になった行動
6. 同じ進捗を得るために消費した現金、時間、注意、権利
7. 下方失敗時に残る資産と次の手

### 戦略余力 — 目標到達経路の壊れにくさ {#bzm-slack}

まさの言葉では「次の行動を起こすための燃料。それが減って x 軸に交差したら、もう会社を清算するしかない」。

正本の定義は集合で与える（[BZM 2.2](../bzm/bzm-2-2-strategic-slack-and-propulsion.md) §7）。

$$
\mathcal K_{\mathcal T}^{-}(\alpha,h;\Delta)=\left\{\mathbf s \mid q_{\mathrm{rob}}^{-}(\mathbf s,h;\Delta)\ge\alpha\right\}
$$

| 記号 | 意味 |
|---|---|
| $\mathcal K_{\mathcal T}^{-}$ | 頑健捕捉領域。最悪の登録ショックを受けても、閾値 $\alpha$ 以上の見込みで目標へ届ける状態の集合。ここに居る限り「まだ手がある」 |
| $\mathcal T$ | 目標集合 |
| $\mathcal F$ | 許容不能状態。倒産、法的禁止、支払不能、不可逆な期限失効、目標達成を不可能にする技術失敗 |
| $\Delta$ | 登録ショック集合。入金遅延、主要人材の離脱、設備費上振れ、顧客予算の失効、規制遅延、それらの相関ショック |
| $q_{\mathrm{rob}}^{-}$ | 最悪の登録ショック下での到達見込みの**下限** |
| $\alpha$ | 「まだ届く」と認める閾値 |

**燃料は金だけではない。** $\mathcal K^{-}$ に居るかどうかは、現金だけでなく、能力、契約、許認可、代替経路、信用といった状態 $\mathbf s$ の全体で決まる。現金がゼロでも別の燃料が残っていれば $\mathcal K^{-}$ の中に居られる。

**$T_Y$（ティー・ワイ）＝ 戦略余力が尽きる時点。** まさの言う「x 軸との交差点」である。

$$
T_Y^{2.2,\pi_d^*}=\inf\left\{h\in\mathcal H_{\mathrm{dec}}^{\pi_d^*} \mid \mathbf s_{t+h}^{\pi_d^*}\notin\mathcal K_{\mathcal T}^{-}(\alpha,H-h;\Delta)\right\}
$$

| 記号 | 意味 |
|---|---|
| $T_Y$ | 選択した方針を進めた結果、最悪の登録ショック下で目標へ届く方針を必要な確度で保持できなくなる**初回時点** |
| $\pi_d^*$ | 権限主体 $d$ が価値評価で選んだ方針 |
| $\mathcal H_{\mathrm{dec}}$ | 目標到達・許容不能状態到達・評価地平のいずれかに着くまでの判断時点の集合 |

これは固定した五つの時計のうち最初にゼロになる時間ではない（BZM 2.0 の旧定義がそれだった）。

**$T_Y$ 一本へ潰さない。** 正本は戦略余力について、少なくとも次の7つを分けて表示せよと定める（同 §7「一つの数字へ潰さない出力」）。$T_Y$ は③にすぎない。

1. 現在状態が $\mathcal K^{-}$ と $\mathcal K^{+}$ のどこにあるか
2. 通常の物理分布の下で、目標到達前に許容不能状態へ着く確率
3. ショック別の $T_Y$ 分布
4. 共通故障点を持たない代替方針の有無
5. ショック別の許容量
...
経路数をそのまま加点しない。十本の経路が同じ許認可・供給者・投資家判断に依存していれば、実質は一本である。

また、通常の到達確率 $q^{\pi}$ と、最悪ショック下の下限 $q_{\mathrm{rob}}^{-}$ は同じではない（同 §7）。
...
## 5. BZM の中で SPS がどこに位置するか {#lineage-relation}

```
BZM（事業価値モデル ＝ モデル本体）
│
├─ 状態と動学 ······ 八層状態、推進力、戦略余力
│      └→ T_Y（戦略余力が尽きる時点）を定量化   ← 2.2 が 2.0 の定義を置き換えた
│
├─ 到達見込み層 ···· q = Pr( T_C < T_Y, T_C ≦ H_v )   ← 上から T_Y を受け取る
│
├─ 価値層 ·········· P^ind（産業創出価値）             ← 2026-08-16 に持分価値から差し替え
│
└─ 出力 ············ SPS = Σ_o q_o × P^ind_o          ← 一次選別で使うのはこれ
       他の出力: 動的正味PJ価値 J、成功条件付き価値 P、gate積 Q、逆風耐久 S
```

| 記号 | 意味 |
|---|---|
| $T_C$ | 資本自立（新規調達なしで12か月は通常運営と既存債務履行を続けられる状態）へ着くまでの時間 |
| $T_Y$ | 戦略余力が尽きる時点（[#bzm-slack](#bzm-slack)） |
| $H_v$ | 計画版 $v$ の判定期限 |

**ただし、この接続はいま働いていない。** 詳細は下の「未解決」を参照。

**価値の物差しは 2026-08-16 に分かれた。**

それ以前の SPS の価値項 $P_o$ は、BZM 2.0 が定義した「評価日時点で存在する全持分証券への条件付き正味現在価値」だった（[到達見込みモデル](../bzm/sps-current-reachability-model.md) §1）。
...
BZM の第一地平は $G_{\mathrm{self}}(12\mathrm m)$ であり、回収・出口・投資収益はBZM の地平外で、BZSF 投資層が受け持つ（[領域定義](../bzm/sps-current-domain-definition.md) §4・§5、[用語集 §6](../bzm/terminology_glossary.md)）。

投資判定は BZM 出力（$q$、$\mathbf P$）と BZSF 投資層の**二階建て**で、BZM 単体でGO・投資額・投入人月を決めない。

### 未解決 — 推進力と戦略余力が SPS の式に入っていない（2026-08-22 起票、設計中）

**まさ指摘（2026-08-22、原文）**

> 一般的にはお金だけが余力だと思われると思うけど、ビジネスを生み出せる燃料は色々あって、それらを総合して表したものが戦略余力。例え話したじゃん、与沢翼の。彼は無一文になってもすぐに億単位の売上を生み出せた。あれは戦略余力があったから。最初は技術と資本金しかないわけで、それらをいかに効率よく戦略余力に変換していくか、その変換能力（これを戦略余力と同等に重要なパラメータと定義していたはず）が高ければSPSが高くなるべき。そしてその変換能力に次いで重要なのが戦略余力で、それ自体が高ければさらに加点されるべき。それがSPSの式に入ってないなら、SPSの式を変えないといけない。

**確認した事実（正本を読んで裏を取ったもの）**

1. まさの言う「変換能力」は、正本の**推進力**（[#bzm-propulsion](#bzm-propulsion)）にあたる。
2. 現行 $\mathrm{SPS}=\sum_o q_o P^{\mathrm{ind}}_o$ に、推進力も戦略余力も記号として現れない。$q$ の理論定義に $T_Y$ が入るだけである。
3. その $T_Y$ も実運用では計算していない。735件の $q$ は11要因ルーブリックの判断帯であり、BZM 2.2 の状態から導いた値ではない（BZM 2.2 は前向き検証0件の pilot、本実装前）。
4. 11要因のうち余力に触れるのは資本集約度（資金の量）・規制認証の年数・顧客の検証コストだけで、**金以外の燃料と変換能力は入っていない**。さらに[ルーブリック](../bzm/SEED_Q_RUBRIC_2026-08-15.md) §3 は「研究者の事業化シグナルは考慮しない（担い手は AMD が後から供給する設計）」として、担い手の変換能力を意図的に外している。
5. 経路集合 $\mathcal O$ にはピボット・ライセンス・M&A・知財売却が入っており、**変換能力の高い主体ほど多くの経路で価値を実現できる構造は式に用意されている**。しかし Tier 0 の運用は単一経路に縮退（$\mathrm{SPS}=P^{\mathrm{ind}}\times q/100$）しており、その構造が働いていない。
6. 既存12PJの retrofit（2026-08-17）は、盲検規律として BZM 2.2 の量を評価の入力にしないと決めていた。12PJ は状態が埋まっているので繋げる情報はあるが、繋がない側に倒していた。

**要件（まさ確定 2026-08-22）**

- 変換能力（推進力）が高ければ SPS が高くなる。これが最重要。
- 戦略余力が高ければ、さらに加点される。これが次に重要。
- 入っていないなら SPS の式を変える。

**進行中**

改訂案を `model/proposals/2026-08-22_sps-propulsion-and-slack.md` に起草中（独立3案を経済役・経営役の二重批判監査で潰し合わせ、生き残りを統合する方式）。数理設計・測り方・単位整合はえいみが負い、まさへは完成案の採否だけを仰ぐ（まさ指摘 2026-08-22「1も2も3も、経済学素人のおれが決めるべきじゃないのでは？」）。

**式の改訂が承認されるまで、現行式 $\mathrm{SPS}=\sum_o q_o P^{\mathrm{ind}}_o$ を変えない。** 735件の既存評価もそのまま据え置く。
...
| 2026-08-11 | **`P measurement v0.1`**（社会・経済の二成分指数） | $P_{o,\tau}$ の代替として作った指数 | 定義と単位が一致しない。指数値と根拠状態は履歴として残すが、現行 $P_{o,\tau}$ またはSPSに使わない | [到達見込みモデル](../bzm/sps-current-reachability-model.md) §1 |
| 2026-08-04 | **試行 v0.1**（SX 2/10,000、CX 0/10,000） | 初回のモンテカルロ試行 | 有効な試行ではない。成功確率・モデル整合性の警報・投資判断・GO判定のいずれにも使わない | [到達見込みモデル](../bzm/sps-current-reachability-model.md) §6 |
...
| 2026-08-22 | ledger v1 | SPS現行正本3本を改名（`sps-2-0-reachability-model.md`→`sps-current-reachability-model.md`、`sps-2-0-domain-definition.md`→`sps-current-domain-definition.md`、`sps-2-0-measurability-gate.md`→`sps-current-measurability-gate.md`）。式・定義・数値は無変更、参照リンクとファイル名の置換のみ。旧slugは `/bzm`・`/model` でリダイレクト。凍結記録内の旧ファイル名は当時のまま | まさ指示「改名して」→ [APPROVALS.md](./APPROVALS.md) #2026-08-22-2 | えいみ |
