...
function buildParams(cfg) {
  ...
  out.push(P(G.VALUE, 'alphaLoc', '\\alpha^{\\mathrm{loc}}', '国内立地差の水準', cfg.alphaLoc.toFixed(2), 'C', '5.8',
    'スコアの計算には入らない。対外的に「他者がやっていた分を除いた追加分」を問われたときの換算にだけ使う（追加分 = 0.15 × スコア）。全案件一律の定数倍なので案件の比較には使えない', []));
